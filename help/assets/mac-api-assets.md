...
[!DNL Assets] HTTP API를 사용하면 메타데이터, 변환 및 댓글에 대한 등 디지털 자산에 대해 [!DNL Experience Manager] 컨텐츠 조각을 사용하여 구조화된 컨텐츠와 함께 CRUD(create-read-update-delete) 작업을 수행할 수 있습니다. 이것은 `/api/assets`에 노출되며 REST API로 구현됩니다. 여기에는 [컨텐츠 조각 지원](/help/assets/assets-api-content-fragments.md)이 포함됩니다.

API에 액세스하려면:

1. `https://[hostname]:[port]/api.json`에서 API 서비스 문서를 엽니다.
1. `https://[hostname]:[server]/api/assets.json`로 연결되는 [!DNL Assets] 서비스 링크를 따릅니다.
...
자세한 내용은 Experience Manager 에셋 HTTP API](/help/assets/assets-api-content-fragments.md)의 [콘텐츠 조각 지원을 참조하십시오.

## 데이터 모델 {#data-model}

[!DNL Assets] HTTP API는 두 개의 주요 요소, 폴더 및 자산을 노출합니다(표준 자산의 경우).
...
컨텐츠 조각의 요소에 대한 자세한 내용은 Experience Manager 자산 HTTP API[의 컨텐츠 조각 지원을 참조하십시오.](/help/assets/assets-api-content-fragments.md#content-fragments)
...
* 속성 **[!UICONTROL 필터 메서드]**&#x200B;에 다음이 포함되어 있는지 확인하십시오.`POST`, `PUT`, `DELETE`.
...
제공된 파일을 제공된 경로에 배치하여 DAM 저장소에 자산을 만듭니다. 노드 이름 대신 `*`이 제공되면 서블릿은 매개 변수 이름이나 파일 이름을 노드 이름으로 사용합니다.

**매개 변수**:매개 변수 `name` 는 자산 이름 및 파일 참조 `file` 에 사용됩니다.
...
API 메서드는 `jcr` 네임스페이스의 메타데이터 속성을 업데이트합니다. 사용자 인터페이스를 사용하여 수행한 업데이트는 `dc` 네임스페이스의 메타데이터 속성을 변경합니다. `dc`과 `jcr` 네임스페이스 간에 메타데이터 값을 동기화하려면 워크플로우를 만들고 자산 편집 시 워크플로우를 실행하도록 Experience Manager을 구성할 수 있습니다. ECMA 스크립트를 사용하여 필요한 메타데이터 속성을 동기화합니다. 다음 샘플 스크립트는 `dc:title`과 `jcr:title` 사이에 제목 문자열을 동기화합니다.
...
URL에 `/content/dam`을 사용하지 마십시오. 자산을 이동하고 기존 자산을 덮어쓰는 샘플 명령은 다음과 같습니다.
...
* 자산 API가 전체 메타데이터를 반환하지 않습니다. API에서 네임스페이스는 하드 코딩되며, 네임스페이스만 반환됩니다. 전체 메타데이터가 필요한 경우 자산 경로 `/jcr_content/metadata.json`를 확인합니다.
