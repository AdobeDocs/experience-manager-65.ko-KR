---
title: 모니터링 우수 사례 [!DNL Assets] 배포
description: 환경 및 성능 모니터링 우수 사례 [!DNL Adobe Experience Manager] 배포 후 배포
contentOwner: AG
...
# 모니터링 우수 사례 [!DNL Adobe Experience Manager Assets] 배포 {#assets-monitoring-best-practices}

에서 [!DNL Experience Manager Assets] 모니터링에는 다음 프로세스 및 기술에 대한 관찰과 보고가 포함되어야 합니다.
...
일반적으로 [!DNL Experience Manager Assets] 라이브 모니터링과 장기 모니터링이라는 두 가지 방법으로 모니터링할 수 있습니다.
...
* 상단: Iotop은 디스크 입출력 사용을 위한 세부 대시보드입니다. 여기에는 디스크 입출력(I/O)을 사용하는 프로세스와 사용 양을 나타내는 바와 미터(M)가 표시됩니다. Iotop은 `yum install iotop` 또는 `apt-get install iotop`.
...
* [!DNL Experience Manager] `error.log` 파일: 당신은 [!DNL Experience Manager] `error.log` 파일에 시스템에 기록된 오류에 대한 세부 정보가 있습니다. 명령 사용 `tail -F quickstart/logs/error.log` 확인할 오류를 식별합니다.
* [워크플로우 콘솔](/help/sites-administering/workflows.md): 워크플로우 콘솔을 활용하여 지연 또는 문제가 발생하는 워크플로우를 모니터링합니다.

일반적으로 이러한 도구를 함께 사용하여 성능에 대한 포괄적인 아이디어를 얻을 수 있습니다 [!DNL Experience Manager] 배포.
...
장기 모니터링 [!DNL Experience Manager] 배포에는 라이브 모니터링되는 동일한 부분을 장기간 모니터링하는 작업이 포함됩니다. 또한 환경에 따른 경고를 정의하는 작업도 포함됩니다.

### 로그 집계 및 보고 {#log-aggregation-and-reporting}

로그를 집계하는 데 사용할 수 있는 도구(예: Splunk(TM) 및 Elastic Search, Logstash 및 ELK)가 있습니다. 작동 시간을 평가하려면 [!DNL Experience Manager] 배포하려면 시스템에 관련된 로그 이벤트를 이해하고 이를 기반으로 경고를 만들어야 합니다. 개발 및 운영 방침에 대한 좋은 지식이 있으면 로그 수집 프로세스를 튜닝하여 중요한 경고를 생성하는 방법을 더 잘 이해할 수 있습니다.
...
* 경보 임계값: 값이 `QueueBlocked` is `true` 또는 값 `QueueNumEntries` 은 기준선의 150%보다 큽니다.

* 경고 정의: 시스템에서 차단된 큐가 존재하여 복제 대상이 다운되었거나 접근할 수 없음을 나타냅니다. 네트워크 또는 인프라 문제로 인해 과도한 항목이 큐에 올라가 시스템 성능에 부정적인 영향을 줄 수 있습니다.

>[!NOTE]
>
>MBean 및 URL 매개 변수의 경우 을 `<AGENT_NAME>` 모니터링할 복제 에이전트의 이름으로 구성됩니다.
...
* 클라이언트 네트워크에서 송신 지점을 조사하고 수신 포인트는 [!DNL Experience Manager] dispatcher를 포함한 배포 네트워크. 주로 병목영역입니다. 자세한 내용은 [자산 네트워크 고려사항](/help/assets/assets-network-considerations.md).
* 크기 조정 [!DNL Experience Manager] server. 당신의 사이즈가 적당하지 않을 수도 있습니다 [!DNL Experience Manager] 배포. Adobe 고객 지원 을 통해 서버의 크기가 더 작은지 확인할 수 있습니다.
