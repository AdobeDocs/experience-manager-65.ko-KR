...
description: AEM Assets HTTP API의 컨텐츠 조각 지원에 대해 알아봅니다.
seo-description: AEM Assets HTTP API의 컨텐츠 조각 지원에 대해 알아봅니다.
...
>
>
AEM Assets HTTP API의 현재 구현은 REST 입니다.

Adobe Experience Manager (AEM) [Assets REST API](/help/assets/mac-api-assets.md)를 사용하면 개발자가 CRUD 작업(만들기, 읽기, 업데이트, 삭제)을 통해 HTTP API를 통해 직접 컨텐츠(AEM에 저장됨)에 액세스할 수 있습니다.

API를 사용하면 JavaScript 프런트 엔드 애플리케이션에 컨텐츠 서비스를 제공하여 AEM을 헤드리스 CMS(Content Management System)로 운영할 수 있습니다. 또는 HTTP 요청을 실행하고 JSON 응답을 처리할 수 있는 다른 애플리케이션입니다.

예를 들어, 단일 페이지 애플리케이션(SPA), 프레임워크 기반 또는 사용자 지정 환경에서는 HTTP API를 통해 제공되는 컨텐츠(일반적으로 JSON 형식)가 필요합니다.

AEM 코어 구성 요소는 이 목적을 위해 필요한 읽기 작업을 제공할 수 있고 JSON 출력을 사용자 지정할 수 있는 매우 포괄적이고 유연한 사용자 지정 가능한 API를 제공하지만, 전용 AEM 템플릿을 기반으로 하는 (API) 페이지에서 호스팅해야 하므로 구현을 위한 AEM WCM(Web Content Management) 노하우가 필요합니다. 모든 SPA 개발 조직이 이러한 리소스에 액세스할 수 있는 것은 아닙니다.

이때 Assets REST API를 사용할 수 있습니다. 이를 통해 개발자는 페이지에 먼저 포함시키지 않고도 자산(예: 이미지 및 컨텐츠 조각)에 직접 액세스하여 직렬화된 JSON 형식으로 콘텐츠를 제공할 수 있습니다. (Assets REST API에서 JSON 출력을 사용자 지정할 수 없습니다.) Assets REST API를 사용하면 기존 자산, 컨텐츠 조각 및 폴더를 새로 만들거나, 업데이트하거나, 삭제하여 컨텐츠를 수정할 수 있습니다.
...
Assets REST API는 AEM 인스턴스 내에 저장된 자산에 대한 [REST](https://en.wikipedia.org/wiki/Representational_state_transfer) 스타일 액세스를 제공합니다. 이 플러그인은 `/api/assets` 종단점을 사용하고 선행 `/content/dam` 없이 자산에 액세스할 자산의 경로가 필요합니다.
...
지원되는 요청의 정확한 형식은 [API 참조](/help/assets/assets-api-content-fragments.md#api-reference) 설명서에서 정의됩니다.
...
<table>
 <tbody>
  ...
  <tr>
   <td>액세스</td>
   <td><p>직접 액세스할 수 있습니다.</p> <p><code>/content/dam</code>에 매핑된 <code>/api/assets </code>끝점을 사용합니다(저장소에서).</p> <p>예를 들어, 액세스:<code class="code">
       /content/dam/we-retail/en/experiences/arctic-surfing-in-lofoten</code><br /> 요청:<br /> <code>/api/assets/we-retail/en/experiences/arctic-surfing-in-lofoten.model.json</code></p> </td>
   <td><p>AEM 페이지에서 AEM 구성 요소를 통해 참조되어야 합니다.</p> <p><code>.model</code> 선택기를 사용하여 JSON 표현을 만듭니다.</p> <p>예제 URL은 다음과 같습니다.<br /> <code>https://localhost:4502/content/we-retail/language-masters/en/experience/arctic-surfing-in-lofoten.model.json</code></p> </td>
  </tr>
  <tr>
   <td>보안</td>
...
* [자산 ](/help/assets/mac-api-assets.md#assets) REST API의 사용 가능한 기능
* [엔티티 유형](/help/assets/assets-api-content-fragments.md#entity-types)
...
Assets REST API는 폴더의 속성에 대한 액세스 권한을 노출합니다.예를 들어 해당 이름, 제목 등이 있습니다. 자산은 폴더의 하위 엔티티로 노출됩니다.

>[!NOTE]
>
>자산 유형에 따라 하위 엔티티 목록에 이미 각 하위 엔티티를 정의하는 전체 속성 세트가 포함되어 있을 수 있습니다. 또는 이 자식 엔티티 목록에서 엔티티에 대해 축소된 속성 집합만 노출될 수 있습니다.

### 자산 {#assets}

자산이 요청되면 응답은 해당 메타데이터를 반환합니다.제목, 이름 및 각 자산 스키마에 의해 정의된 기타 정보와 같은 정보를 제공합니다.

자산의 이진 데이터는 `content` 유형의 SIRN 링크로 노출됩니다(`rel attribute` 라고도 함).
...
새 컨텐츠 조각을 생성하려면(내부 저장소) 경로를 제공해야 합니다.
...
* 만들기는 절대적으로 작성자 인스턴스([에 속하며, 현재 이 API](/help/assets/assets-api-content-fragments.md#limitations))를 사용하여 게시할 조각을 복제할 방법이 없습니다.
...
>[!NOTE]
>
>자세한 내용은 [API 참조](/help/assets/assets-api-content-fragments.md#api-reference)를 참조하십시오. 특히 [Adobe Experience Manager Assets API - 컨텐츠 조각](https://helpx.adobe.com/experience-manager/6-5/sites/developing/using/reference-materials/assets-api-content-fragments/index.html).
...
컨텐츠 조각, 단일 요소 또는 모든 요소 값 및/또는 메타데이터의 제목 또는 설명일 수 있습니다. 또한 업데이트에 유효한 `cq:model` 속성을 제공해야 합니다.
...
* **변형을 작성하고 업데이트할 수 없습니다.** 이러한 변형을 페이로드에 추가하면(예: 업데이트) 무시됩니다. 그러나 변형은 게재( `GET`)를 통해 제공됩니다.
...
* **참조는 무시됩니다**. 현재 기존 컨텐츠 조각을 참조하는지 여부에 대한 검사가 없습니다. 따라서, 예를 들어 컨텐츠 조각을 삭제하면 참조가 포함된 페이지에 문제가 발생할 수 있습니다.
...
* **500(내부 서버 오류)**
   ...
   * 컨텐츠 조각 모델이 제공되지 않음(null 값), 리소스가 null(권한 문제일 수 있음) 또는 리소스가 올바른 조각 템플릿이 아닙니다.
...
* [Adobe Experience Manager Assets API - 컨텐츠 조각](https://helpx.adobe.com/experience-manager/6-5/sites/developing/using/reference-materials/assets-api-content-fragments/index.html)
