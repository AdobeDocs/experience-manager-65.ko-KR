...
통합에 [대한 사용 사례에서](#use%20cases%20for%20the%20integration)언급했듯이 AEM Sites의 새로운 Adobe Launch 통합과 임베드 코드를 사용하여 Dynamic Media 뷰어를 추적할 수 있습니다.
...
Adobe Analytics [구성 및 Adobe Launch](#configuringadobeanalytics) 구성 섹션에서 구성 단계를 [](#configuringadobelaunch) 완료해야 합니다. 하지만 AEM 관련 구성 단계는 필요하지 않습니다.
...
1. Adobe Launch에 먼저 로그인하여 Adobe Launch 라이브러리에 대한 포함 코드를 얻습니다(Adobe Launch [구성 참조](#configuringadobelaunch)).
...
* (선택 사항) 비디오 추적이 필요한 경우에만 필요) *오디오 및 비디오* 익스텐션을 위한 Adobe Media Analytics
...
Adobe Launch 구성(속성, 확장 기능, 규칙 및 데이터 요소 설정 포함)을 변경하려면 *이러한 변경 사항을 게시해야* 합니다.** Adobe Launch에서 게시는 속성 구성 아래의 게시 탭에서 수행됩니다.
...
1. 공개 키 **[!UICONTROL 다운로드를]** 클릭하여 로컬 시스템에 공개 키 파일(*.crt)을 다운로드합니다.
