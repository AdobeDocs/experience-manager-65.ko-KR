...
기본적으로 [!DNL Assets]에서는 파일 크기 제한으로 인해 2GB를 초과하는 에셋을 업로드할 수 없습니다. 그러나 CRXDE Lite으로 이동하여 `/apps` 디렉터리 아래에 노드를 만들면 이 제한을 덮어쓸 수 있습니다. 노드에는 동일한 노드 이름, 디렉토리 구조 및 비교 가능한 노드 속성이 있어야 합니다.
...
1. [!DNL Adobe Experience Manager] [!UICONTROL 웹 콘솔 번들] 페이지의 테이블의 이름 열에서 **[!UICONTROL Granite Workflow 외부 프로세스 작업 처리기 Adobe]**&#x200B;을(를) 찾아 클릭합니다.
1. [!UICONTROL Adobe Granite 워크플로 외부 프로세스 작업 핸들러] 페이지에서 **[!UICONTROL 기본 시간 초과]** 및 **[!UICONTROL 최대 시간 초과]** 필드 모두에 대한 초 수를 `18000`(5시간)으로 설정합니다. **[!UICONTROL 저장]**&#x200B;을 클릭합니다.
...
비디오 자산을 YouTube에 게시하려면 태그를 사용하여 Experience Manager Assets을 설정합니다. 이러한 태그를 YouTube 채널과 연결합니다. 비디오 에셋의 태그가 YouTube 채널의 태그와 일치하는 경우 비디오는 YouTube에 게시됩니다. Publish to YouTube은 연결된 태그가 사용되는 한 비디오의 일반 게시와 함께 발생합니다.
...
비디오 처리 프로필을 건너뛰고 YouTube에 바로 게시할 때 Experience Manager 에셋의 비디오 에셋에 볼 수 있는 썸네일이 표시되지 않는다는 의미입니다. 또한 `dynamicmedia` 또는 `dynamicmedia_scene7` 실행 모드에서 실행하는 경우 인코딩되지 않은 비디오는 Dynamic Media 에셋 유형에서 작동하지 않습니다.
...
1. [YouTube Publish 복제 에이전트 활성화](#enabling-the-youtube-publish-replication-agent)
...
비디오에 YouTube을 게시하려면 Experience Manager이 태그를 하나 이상의 YouTube 채널에 연결합니다. 게시할 태그를 추가하려면 [태그 관리](/help/sites-administering/tags.md)를 참조하십시오.

또는 Experience Manager에서 기본 태그를 사용하려는 경우 이 작업을 건너뛰고 [YouTube Publish 복제 에이전트 사용](#enabling-the-youtube-publish-replication-agent)(으)로 이동할 수 있습니다.

### YouTube Publish 복제 에이전트 활성화 {#enabling-the-youtube-publish-replication-agent}

YouTube Publish 복제 에이전트를 사용하도록 설정한 후 Google Cloud 계정에 대한 연결을 테스트하려면 **[!UICONTROL 연결 테스트]**&#x200B;를 선택합니다. 브라우저 탭에 연결 결과가 표시됩니다. YouTube 채널을 추가한 경우 테스트 일부로 채널 목록이 표시됩니다.

1. Experience Manager 왼쪽 상단 모서리에서 Experience Manager 로고를 클릭한 다음 왼쪽 레일에서 **[!UICONTROL 도구]** > **[!UICONTROL 배포]** > **[!UICONTROL 복제]** > **[!UICONTROL 작성자의 에이전트]**&#x200B;를 클릭합니다.
1. 작성자 에이전트 페이지에서 **[!UICONTROL YouTube Publish]**&#x200B;을(를) 클릭합니다.
...
1. 왼쪽 상단 모서리에서 Experience Manager 로고를 선택한 다음 왼쪽 레일에서 **[!UICONTROL 도구]**(망치 아이콘) > **[!UICONTROL Cloud Service]** > **[!UICONTROL YouTube 게시 구성]**&#x200B;을 선택합니다.
...
1. 왼쪽 상단 모서리에서 Experience Manager 로고를 선택한 다음 왼쪽 레일에서 **[!UICONTROL 도구]**(망치 아이콘) > **[!UICONTROL 배포]** > **[!UICONTROL Cloud Service]**&#x200B;을 선택합니다.
...
>
>**[!UICONTROL Publish 즉시]**&#x200B;은(는) 업로드된 에셋이 IPS로 동기화된 후 자동으로 게재 시스템에 게시됨을 의미합니다. Dynamic Media의 경우에는 사실이지만 YouTube의 경우에는 사실이 아닙니다. YouTube에 게시하려면 Experience Manager 작성자를 통해 게시해야 합니다.
...
1. 워크플로 모델 페이지에서 **[!UICONTROL YouTube으로 Publish]**&#x200B;를 선택한 다음 도구 모음에서 **[!UICONTROL 편집]**&#x200B;을 선택합니다.
1. Publish에서 YouTube으로 이동 워크플로 페이지의 오른쪽 상단 모서리에서 **[!UICONTROL 편집]**&#x200B;을 선택합니다.
...
1. 다음 확인란을 선택하거나 선택 취소할 수 있습니다.
   ...
   * 게시 시작
   * 게시 실패
   * Publish 완료 - 채널 및 URL에 대한 정보가 포함되어 있습니다.

   확인란을 선택 취소하면 YouTube Publish 워크플로우에서 지정된 이메일 알림을 받지 못합니다.
...
1. 오른쪽 상단 근처에 있는 Publish-YouTube 워크플로 페이지에서 **[!UICONTROL 동기화]**&#x200B;를 선택합니다.
...
1. 비디오에 주석을 달려면 **[!UICONTROL 주석 달기]**&#x200B;를 클릭하세요. 비디오의 특정 시간(프레임)에 주석이 추가됩니다. 주석을 추가할 때 캔버스에 그림을 그리고 그림에 주석을 포함할 수 있습니다. 주석은 자동으로 저장됩니다. 주석 마법사를 종료하려면 **[!UICONTROL 닫기]**&#x200B;를 클릭하십시오.
   ...
   ![비디오 프레임의 Draw 및 주석 달기](assets/annotate-video.png)
