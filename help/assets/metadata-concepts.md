...
Dublin Core 메타데이터는 보다 쉽게 찾을 수 있도록 자산을 설명하는 표준 규칙 세트를 제공합니다. Dublin Core [!DNL Assets]에서는 비디오, 사운드, 이미지 및 문서를 비롯한 디지털 에셋에 대해 설명합니다.
...
| 날짜 | 자산과 관련된 날짜 또는 기간 |
| 설명 | 자산에 대한 자세한 정보입니다. |
| format | 자산의 파일 형식, 실제 미디어 또는 크기입니다. [!DNL Experience Manager] 자산 `dc:format` 의 MIME 형식을 나타내는 데 사용됩니다. |
...
모든 파일 포맷에 임베드할 수 있는 범용 메타데이터 인코딩 기능 외에도 XMP은 리치 [컨텐츠 모델을](#xmp-core-concepts) 제공하고 Adobe [및 다른 회사에서](#advantages-of-xmp) 지원되므로 XMP을 조합하여 사용하는 사용자는 강력한 플랫폼을 통해 구축할 수 [!DNL Assets] 있습니다.
...
[!DNL Adobe Experience Manager Assets] 은 자산을 설명하는 데 널리 사용되는 IPTC 메타데이터 표준을 지원합니다. 이러한 방식으로 사진 작가, 크리에이티브 에이전시, 라이브러리, 박물관 등 다양한 파티에 참여하여 이미지를 더욱 [!DNL Assets] 효과적으로 전달할 수 있습니다.
...
1. 사용자 [!DNL Assets] 인터페이스에서 이미지를 선택합니다.
...
1. 자산에 대한 IPTC **[!UICONTROL 확장]** 메타데이터를 보려면 IPTC 확장 탭을 클릭합니다.
...
속성 **[!UICONTROL 페이지의]** IPTC [!UICONTROL 탭에는 Adobe Bridge 및 기타 Adobe Creative Cloud 앱을 통해 자산에 추가된 키워드도] 표시됩니다. 이러한 키워드를 편집하고 **[!UICONTROL IPTC]** 탭에서 키워드를 더 추가할 수도 있습니다.
