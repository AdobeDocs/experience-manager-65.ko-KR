...
참조된 자산이 이미 있습니다 [!DNL Experience Manager Assets]. InDesign Server을 [구성하여 하위 자산을 추출할 수 있습니다](indesign.md). 파일에 포함된 에셋이 하위 [!DNL InDesign] 자산으로 추출됩니다.

>[!NOTE]
>
>프록시가 [!DNL InDesign Server] 있는 경우 [!DNL InDesign] 파일의 미리 보기가 XMP 메타데이터 내에 임베드됩니다. 이 경우 축소판 추출이 명시적으로 필요하지 않습니다. 그러나 프록시가 [!DNL InDesign Server] 되지 않은 경우 [!DNL InDesign] 파일에 대해 축소판을 명시적으로 추출해야 합니다.

### 자산을 드래그하여 참조 만들기 {#create-references-by-dragging-aem-assets}

이 절차는 Adobe Illustrator의 참조로 디지털 자산을 [추가하는 것과 유사합니다](#refai).
