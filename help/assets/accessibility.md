...
[!DNL Adobe Experience Manager]을(를) 사용하면 콘텐츠 작성자 및 게시자가 웹에서 놀라운 경험을 제공할 수 있습니다. Adobe은 [!DNL Experience Manager]의 접근성을 개선하여 장애가 있는 작성자를 포함하도록 노력하고 있습니다. 이 소프트웨어는 모든 유형의 사용자의 요구 사항을 충족하도록 지속적으로 향상됩니다. 이는 시각, 청각, 이동성 또는 기타 장애가 있는 개인을 포함하는 전 세계 표준을 준수합니다.
...
포인터로 클릭 가능하거나 동작 가능한 많은 사용자 인터페이스 요소는 키보드를 사용하여 와 결합할 수도 있습니다. 키보드를 사용하여 UI 요소에 집중하고 적절한 작업을 수행할 수 있습니다. 사용자는 키보드 단축키를 사용하여 UI 요소에 집중하지 않고 키보드를 사용하여 명령 또는 작업을 트리거할 수 있습니다. 예를 들어, 사용자는 사용자 인터페이스의 왼쪽에서 에셋의 타임라인을 열 수 있습니다. 키보드를 사용하여 `Return`을(를) 선택하고 `Alt + 2` 바로 가기 키를 선택하여 사용자 인터페이스 컨트롤을 찾습니다.
...
사용자는 키보드를 사용하여 로 이동하고 로그인 필드를 입력하여 로그인할 수 있습니다. 화면 판독기는 사용자가 잘못된 사용자 이름과 암호 조합을 입력할 때마다 로그인 페이지에 오류 메시지를 표시합니다.

로그인 후 DAM 사용자는 키보드를 사용하여 [!DNL Assets] 사용자 인터페이스 내에서 탐색할 수 있습니다. 왼쪽 레일, 메뉴, 사용자 프로필, 검색 창, 파일 및 폴더, 관리 및 구성 설정과 같은 사용자 인터페이스 요소는 키보드를 사용하여 탐색할 수 있습니다. 키보드 탐색 순서는 왼쪽에서 오른쪽, 위에서 아래입니다. 사용자가 키보드로 탐색할 때 UI는 향상된 색상 대비로 포커스가 있는 실행 가능한 옵션을 강조 표시하고 화면 판독기에서 내레이션을 적용합니다. 해당하는 경우 화면 판독기는 포커스가 있는 메뉴 옵션의 상태(예: 확장, 축소 또는 혼합)를 알려줍니다. 또한 화면 판독기는 모양이나 인터페이스 배치 대신 실행 가능한 옵션의 용도를 알려줍니다.
...
[!DNL Assets] 사용자 인터페이스에서 사용자는 키보드를 사용하여 DAM 저장소의 디지털 에셋을 탐색하고 에셋을 미리 보거나 다운로드할 수 있습니다. 사용자는 생성된 변환을 보고, 보기를 전환하고, 타임라인, 버전 기록, 주석 및 참조를 검토할 수도 있습니다. 또한 사용자는 메타데이터를 보고 관리할 수 있습니다.
...
* 사용자는 키보드 키를 사용하여 자산의 참조 목록에서 대화형 사용자 인터페이스 옵션에 액세스하고 포커스를 맞출 수 있습니다.
* 목록 보기의 각 행에 있는 요소는 화면 판독기에서 동일한 행의 요소로 표시됩니다.
* `Tab` 키를 사용하여 탐색할 때 포커스가 버전 미리 보기에서 닫기 옵션으로 이동할 수 있습니다.
* 키보드를 사용하여 검색할 때 강조 표시된 실행 가능한 사용자 인터페이스 옵션의 시각적 포커스가 보다 눈에 띄고 대비가 개선되었습니다. 집중 영역이 사용자에게 더 잘 식별될 수 있도록 합니다.
...
마케터 및 관리자와 같은 역할이 일반적으로 수행하는 메타데이터 작업의 경우 다음 기능을 사용하면 접근성이 향상됩니다.
...
* 사용자는 키보드와 함께 날짜 선택기 팝업 대화 상자를 사용할 수 있습니다. 날짜 선택기 사용자 인터페이스 요소를 사용하여 설정 시간 및 해제 시간을 설정하고 날짜를 선택합니다.
* 키보드를 사용하는 드래그 기능은 화면 판독기의 찾아보기 모드에서 [!UICONTROL 메타데이터 스키마 편집기]에서 올바르게 작동합니다.
* 사용자는 키보드를 사용하여 **사용자 또는 그룹 추가** 필드로 포커스를 이동할 수 있습니다.
...
* 화면 판독기는 사용자가 검색 결과를 필터링할 때 필터 패널에서 혼합 상태 확인란을 식별하고 알릴 수 있습니다. 혼합 상태에서는 사용자가 중첩된 술어를 모두 선택할 때까지 첫 번째 수준 확인란이 취소됩니다.
...
* 화면 판독기에는 혼합 상태 옵션이 포함된 술어가 표시됩니다.
...
* 이미지와 그림에는 대체 텍스트를 사용할 수 있습니다.
* 키보드 탐색이 가능합니다.
* 대비 비율은 설명서 웹 사이트의 일부 부분을 강조 표시하는 데 도움이 됩니다.

## 피드백 제공 {#a11y-feedback}

접근성과 관련된 피드백을 제공하고, 질문하고, 제품 개선을 요청하려면 다음 방법을 사용하여 `access@adobe.com`(으)로 전자 메일을 보내십시오.
