...
description: Dynamic Media의 선택적 게시 작업 방법에 대한 정보입니다.
...
Dynamic Media 인스턴스의 모든 폴더에 전역 설정을 적용하는 **[!UICONTROL Dynamic Media 구성]**&#x200B;에 전적으로 의지하는 대신 **[!UICONTROL Manage Publication]** 또는 **[!UICONTROL Quick Publish]**&#x200B;를 사용하여 폴더 수준에서 AEM 또는 Dynamic Media으로 또는 그 반대로 Assets의 게시 또는 게시 취소를 선택할 수 있습니다.

예를 들어 선택적 게시 기능을 사용하면 아직 라이브가 아닌 제품의 자산으로 작업할 수 있습니다. 이러한 경우 마케팅 팀은 Dynamic Media에 동기화되는 스마트 자르기 이미지 및 동적 변환에 액세스할 수 있으므로 전역 전송을 위해 Dynamic Media에 이러한 자산을 게시하지 않고도 홍보 자료를 만들 수 있습니다.

>[!IMPORTANT]
>
>선택적 게시 는 Dynamic Media - Scene7 모드에서만 사용할 수 있습니다.
...
1. **[!UICONTROL Dynamic Media 게시 모드]** 드롭다운 목록에서 옵션을 선택합니다. **[!UICONTROL Dynamic Media 게시 모드]** 옵션은 항상 **[!UICONTROL Dynamic Media 구성에 설정된 값으로 기본 설정됩니다.]** 그러나 다음 옵션 중 하나를 사용하여 이 기본  **[!UICONTROL Dynamic Media]** 구성 값을 수동으로 재정의할 수 있습니다.
   ...
   >[!IMPORTANT]
   >
   >선택한 Dynamic Media 게시 모드 옵션에 관계없이 나중에 *이미 게시된* 자산에 대한 업데이트를 추가 사용자 작업 없이 즉시 게시합니다.

   | Dynamic Media 게시 모드 옵션 | 설명 |
   | --- | --- |
   | **[!UICONTROL 즉시]** | 자산이 이 폴더에 업로드되면 시스템은 자산을 AEM에 수집하여 URL/포함을 즉시 제공합니다. 이 옵션은 AEM 게시에만 연결되어 있으며 자산을 게시하는 데 사용자 개입이 필요하지 않습니다.<br>이전 단계에서  ** Dynamic Media 동기화 모드 **[!UICONTROL 에서 이 폴더 하위 트리의 모든 항목]** 제외 를 선택한  **[!UICONTROL 경우 이]** 옵션을 사용할 수 없습니다. |
   | **[!UICONTROL 활성화 시]** | 자산이 이 폴더에 업로드되면 URL/포함 링크를 제공하기 전에 먼저 자산을 명시적으로 게시해야 합니다. 이 옵션은 AEM 게시에만 연결되어 있습니다.<br>이전 단계에서  ** Dynamic Media 동기화 모드 **[!UICONTROL 에서 이 폴더 하위 트리의 모든 항목]** 제외 를 선택한  **[!UICONTROL 경우 이]** 옵션을 사용할 수 없습니다. |
   | **[!UICONTROL 선택적 게시]** | 자산은 공개 도메인에서 전달되도록 AEM 또는 Dynamic Media 중 선택한 항목에 게시됩니다. 두 게시 방법은 서로 사용할 수 없습니다.  즉, 자산을 DMS7에 게시하여 스마트 자르기 또는 동적 변환과 같은 기능을 사용할 수 있습니다. 또는 AEM에만 자산을 게시하여 보안 미리 볼 수 있습니다.동일한 자산은 *DMS7에 게시되지 않고 공용 도메인에 전달됩니다.* 이전 단계의 **[!UICONTROL 동기화 모드]**&#x200B;에서 **[!UICONTROL 이 폴더 하위 트리의 모든 항목을 dynamic media 동기화]**&#x200B;에서 제외를 선택한 경우에는 이 옵션을 사용할 수 없습니다. |

1. 페이지의 오른쪽 위 모서리에서 **[!UICONTROL Save &amp; Close]**&#x200B;를 누른 다음 **[!UICONTROL OK]**&#x200B;를 탭하여 AEM Assets으로 돌아갑니다.

## 게시 관리{#selective-publish-manage-publication}를 사용하여 Dynamic Media 또는 AEM에 자산을 선택적으로 게시합니다.

**[!UICONTROL 게시 관리]**&#x200B;를 사용하여 자산을 Dynamic Media 또는 AEM에 선택적으로 게시하려면 먼저 **[!UICONTROL Dynamic Media 구성]**&#x200B;에서 **[!UICONTROL 자산 게시]** 옵션을 **[!UICONTROL 선택적 게시]**&#x200B;로 설정하거나 폴더 수준에서 선택적 게시를 구성했는지 확인하십시오.
...
1. **[!UICONTROL 게시 관리 - 옵션]** 페이지의 **[!UICONTROL 작업]**&#x200B;에서 원하는 활성화 유형을 선택합니다.
   ...
   | 작업 | 설명 |
   | --- | --- |
   | ****  게시(AEM에) | 보안 미리 보기를 위해 AEM에 자산을 게시하려면 이 옵션을 선택합니다. |
...
1. **[!UICONTROL 게시 관리 - 옵션]** 페이지의 **[!UICONTROL 작업]**&#x200B;에서 원하는 비활성화 유형을 선택합니다.
   ...
   | 작업 | 설명 |
   | --- | --- |
   | **[!UICONTROL 게시 취소]** (AEM에서) | AEM에서 자산 게시를 취소하려면 이 옵션을 선택합니다. |
   | **[!UICONTROL Dynamic Media에서 게시 취소]** | Dynamic Media에서 자산을 게시 취소하려면 이 옵션을 선택합니다.<br>이 옵션은  **[!UICONTROL Dynamic Media 게시 모드]** 가 폴더 속성에  **[!UICONTROL 선택적]** 게시로 설정된 경우에만 사용할 수 있습니다. |
...
단순 자산 활성화 사례에 **[!UICONTROL 빠른 게시]**&#x200B;를 사용할 수 있습니다. **[!UICONTROL 빠른]** 게시에서는 추가 사용자 상호 작용 없이 선택한 자산을 즉시 게시합니다. 이로 인해 게시되지 않은 참조도 자동으로 게시됩니다.

>[!NOTE]
>
>**[!UICONTROL 빠른 게시]**&#x200B;를 사용하여 자산을 Dynamic Media 또는 AEM에 게시하려면 **[!UICONTROL Dynamic Media 구성]** 또는 선택한 폴더의 폴더 속성에서 **[!UICONTROL 선택적 게시]**&#x200B;가 활성화되어 있는지 확인하십시오.
...
선택 사항으로, **[!UICONTROL 게시]** 검색 조건자와 함께 이러한 확인란을 사용하여 **[!UICONTROL 게시됨]** 및 **[!UICONTROL 게시되지 않은]** AEM 자산의 검색 결과를 세분화할 수 있습니다.
1. 다음 중 하나를 수행하십시오.
...
1. **[!UICONTROL 게시 관리 - 옵션]** 페이지에서 원하는 작업을 선택합니다.
   ...
   | 게시 | 선택적 게시 | AEM에만 게시됩니다. |
   | 게시 취소 | 즉시 또는 활성화 시 | AEM 및 Dynamic Media에서 게시 취소되었습니다. |
   | 게시 취소 | 선택적 게시 | AEM에서만 게시 취소됨. |
   | Dynamic Media에 게시 | 즉시 또는 활성화 시 | AEM, Dynamic Media 또는 둘 다에 게시되지 않습니다. |
   | Dynamic Media에 게시 | 선택적 게시 | Dynamic Media에만 게시됩니다. |
   | Dynamic Media에서 게시 취소 | 즉시 또는 활성화 시 | AEM, Dynamic Media 또는 둘 다에서 게시 취소되지 않았습니다. |
...
AEM에서 **[!UICONTROL 타임라인]**, **[!UICONTROL 카드 보기]**, **[!UICONTROL 열 보기]** 또는 **[!UICONTROL 목록 보기]**&#x200B;를 사용하여 자산의 게시 상태를 빠르게 확인할 수 있습니다.
