...
하지만 복잡한 경우, Quickview URL은 카테고리 ID, 색상 코드 및 크기 코드와 같은 SKU 외에도 다양한 요소를 가지고 있습니다. 이러한 경우 모든 요소는 Experience Manager Assets에서 쇼퍼블 대화형 이미지 기능에서 핫스팟 데이터 정의에 있는 별도의 변수입니다.
...
뷰어 사전 설정을 저장하면 Assets Experience Manager의 뷰어 사전 설정 목록 페이지에서 자동으로 활성화(켜짐)됩니다. 이 기능은 Interactive Media 구성 요소에 표시되고 자산을 볼 때마다 표시됩니다. 그러나 이 뷰어 사전 설정을 사용하는 대화형 배너를 *게재*&#x200B;하려면 *게시*&#x200B;해야 합니다. 이 규칙은 사용자 지정 또는 기본 제공 뷰어 사전 설정에 대해 true입니다.
...
Experience Manager 사이트 고객인 경우 대화형 미디어 구성 요소를 페이지로 드래그하여 대화형 이미지를 추가할 수 있습니다. [페이지에 Dynamic Media 자산 추가](/help/assets/adding-dynamic-media-assets-to-pages.md)를 참조하십시오.
