...
자산을 다운로드하려면 자산으로 이동하고 자산을 선택한 다음 도구 모음에서 **[!UICONTROL 다운로드를]** 클릭합니다. 결과 대화 상자에서 다운로드 옵션을 지정합니다.

자산 유형 이미지 세트, 스핀 세트, 혼합 미디어 세트 및 회전판 세트는 다운로드할 수 없습니다.

![Adobe Experience Manager Assets에서 자산을 다운로드할 때 사용 가능한 옵션](assets/asset_download_dialog.png)

*그림: 자산을 다운로드할 때 사용할 수 있는 옵션[!DNL Experience Manager Assets].*

다음은 사용 가능한 내보내기 또는 다운로드 옵션입니다. 동적 표현물은 [!DNL Dynamic Media] 제공할 수 있습니다. 이 옵션을 사용하면 선택한 자산 외에 새 표현물을 실시간으로 생성할 수 있습니다. 이 옵션은 활성화된 경우에만 사용할 수 [!DNL Dynamic Media] 있습니다.
...
| [!UICONTROL 동적 표현물] | 동적 표현물은 다른 표현물을 실시간으로 생성합니다. 이 옵션을 선택할 때 [이미지 사전 설정](image-presets.md) 목록에서 선택하여 동적으로 만들 표현물도 선택할 수 있습니다. <br>또한 측정 크기 및 단위, 형식, 색상 공간, 해상도 및 이미지 수정자(예: 이미지 반전) 선택 |
...
| [!UICONTROL 각 자산에 대해 별도의 폴더 만들기] | 자산을 다운로드할 때 폴더 계층을 유지하려면 옵션을 선택합니다. 기본적으로 폴더 계층은 무시되며 모든 자산은 로컬 파일 시스템의 한 폴더에 다운로드됩니다. |
