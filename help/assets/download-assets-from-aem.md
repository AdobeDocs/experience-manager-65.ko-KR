...
다음은 사용 가능한 내보내기 또는 다운로드 옵션입니다. 동적 표현물은 [!DNL Dynamic Media] 제공할 수 있는 고유한 기능입니다. 이 옵션을 사용하면 선택한 자산 외에 새 변환을 실시간으로 생성할 수 있습니다. 이 옵션은 [!DNL Dynamic Media] 활성화된 경우에만 사용할 수 있습니다.

| 내보내기 또는 다운로드 옵션 | 설명 |
|---|---|
| [!UICONTROL 자산] | 변환 없이 자산을 원래 형식으로 다운로드하려면 옵션을 선택합니다. |
...
| [!UICONTROL 동적 표현물] | 동적 표현물은 다른 표현물을 실시간으로 생성합니다. 이 옵션을 선택할 때 이미지 사전 설정 목록에서 선택하여 동적으로 만들 표현물도 [선택합니다](image-presets.md) . <br>또한 측정 크기 및 단위, 형식, 색상 공간, 해상도 및 이미지 수정자(예: 이미지 반전) 선택 |
...
| [!UICONTROL 각 자산에 대해 별도의 폴더 만들기] | 자산을 다운로드할 때 폴더 계층 구조를 유지하려면 옵션을 선택합니다. 기본적으로 폴더 계층은 무시되고 모든 자산은 로컬 파일 시스템의 한 폴더에 다운로드됩니다. |

자산에 변환이 있는 경우 변환 옵션을 사용할 수 있습니다. 원본 자산에 하위 자산이 있는 경우 하위 자산 옵션을 사용할 수 있습니다.
...
의 기본 서블릿을 [!DNL Experience Manager] 사용하면 인증된 사용자가 보이는 자산의 ZIP 파일을 만들기 위해 서버와 네트워크를 오버로드할 수 있는 임의의 큰 동시 다운로드 요청을 발행할 수 있습니다. 이 기능으로 인해 발생할 수 있는 DoS 위험을 줄이기 위해 `AssetDownloadServlet` 게시 인스턴스에 대해 기본적으로 OSGi 구성 요소가 비활성화됩니다.
...
1. 게시 실행 모드(`config.publish`)를 대상으로 하는 이름 지정 규칙이 있는 폴더를 만듭니다. `/apps/<your-app-name>/config.publish`Adobe 실행 모드에 대한 구성 속성을 정의하려면 실행 [모드를 참조하십시오](/help/sites-deploying/configure-runmodes.md#defining-configuration-properties-for-a-run-mode).

1. 구성 폴더에서 `nt:file` 이름이 `com.day.cq.dam.core.impl.servlet.AssetDownloadServlet.config`지정된 유형의 파일을 만듭니다.
...
1. 발송자 구성을 통해 자산 다운로드 요청을 차단하려면 `dispatcher.any` 구성을 편집하고 [필터 섹션에](https://docs.adobe.com/content/help/en/experience-manager-dispatcher/using/configuring/dispatcher-configuration.html#defining-a-filter)규칙을 추가하십시오.

   `/0100 { /type "deny" /url "*.assetdownload.zip/assets.zip*" }`

1. 의 OSGi 콘솔로 이동하여 게시 인스턴스에서 OSGi 구성 요소를 비활성화합니다 `http://[aem_server]:[port]/system/console/components`. [비활성화]를 `com.day.cq.dam.core.impl.servlet.AssetDownloadServlet` 찾아 **[!UICONTROL 클릭합니다]**.

>[!MORELIKETHIS]
>
>* [DRM 보호 에셋을](drm.md)다운로드합니다.
>* [Win 또는 Mac 데스크탑에서](https://helpx.adobe.com/experience-manager/desktop-app/aem-desktop-app.html)Experience Manager 데스크탑 앱을 사용하여 에셋을 다운로드할 수 있습니다.
>* [지원되는 Adobe Creative Cloud 앱에서](https://helpx.adobe.com/enterprise/using/manage-assets-using-adobe-asset-link.html)Adobe Assets Link를 사용하여 에셋을 다운로드할 수 있습니다.
