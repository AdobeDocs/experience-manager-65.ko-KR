...
ImageMagick을 사용하여 표현물을 생성하기 위해 DAM 자산 업데이트 워크플로우를 사용자 지정하는 경우, Adobe는 에서 `policy.xml` 파일을 수정할 것을 `/etc/ImageMagick/`권장합니다. 기본적으로 ImageMagick은 OS 볼륨에서 사용 가능한 전체 디스크 공간과 사용 가능한 메모리를 사용합니다. 의 `policymap` 섹션 내에서 다음 구성을 변경하여 이러한 리소스를 `policy.xml` 제한하십시오.
...
>[!NOTE]
>
>ImageMagick `policy.xml` 및 `configure.xml` 파일은 `/usr/lib64/ImageMagick-&#42;/config/` 대신 에서 사용할 수 있습니다.구성 `/etc/ImageMagick/`파일의 위치는 ImageMagick 설명서를 [](https://www.imagemagick.org/script/resources.php) 참조하십시오.
