...
ImageMagick를 사용하여 표현물을 생성하도록 [!UICONTROL DAM 자산 업데이트] 워크플로우를 사용자 지정하는 경우 `policy.xml`에서 `/etc/ImageMagick/` 파일을 수정하는 것이 좋습니다. 기본적으로 ImageMagick은 OS 볼륨에서 사용 가능한 전체 디스크 공간과 사용 가능한 메모리를 사용합니다. 이러한 리소스를 제한하려면 `policymap`의 `policy.xml` 섹션 내에서 다음 구성을 변경합니다.
