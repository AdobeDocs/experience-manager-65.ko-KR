...
description: 자산을 로 가져오는 방법을 설명합니다. [!DNL Adobe Experience Manager]를 클릭하고, 메타데이터를 적용하고, 렌디션을 생성한 후 이를 게시 인스턴스에 활성화합니다.
contentOwner: AG
...
자산 마이그레이션 시 [!DNL Adobe Experience Manager]고려해야 할 몇 가지 단계가 있습니다. 현재 홈에서 자산 및 메타데이터 추출은 구현 마다 다양하므로 이 문서의 범위를 벗어나지만 이 문서에서는 이러한 자산을 로 가져오는 방법을 설명합니다 [!DNL Experience Manager]를 채울 때 메타데이터를 적용하고 렌디션을 생성한 다음 인스턴스에 게시하도록 활성화합니다.

## 사전 요구 사항 {#prerequisites}

이 방법론의 단계를 실제로 수행하기 전에 의 지침을 검토하고 구현합니다 [자산 성능 조정 팁](performance-tuning-guidelines.md). 최대 동시 작업 구성과 같은 많은 단계는 로드 중인 서버의 안정성과 성능을 크게 향상시킵니다. 시스템이 assets로 로드되면 파일 데이터 저장소 구성과 같은 다른 단계를 수행하는 것이 훨씬 어렵습니다.

>[!NOTE]
>
>다음 자산 마이그레이션 도구는 의 일부가 아닙니다 [!DNL Experience Manager] 및 는 Adobe에서 지원되지 않습니다.
>
...
마이그레이션을 시작하기 전에 [!UICONTROL DAM 자산 업데이트] 워크플로우. 모든 자산을 시스템으로 수집한 다음 워크플로우를 일괄적으로 실행하는 것이 좋습니다. 마이그레이션이 진행되는 동안 이미 라이브 상태인 경우 이러한 활동을 소시간에 실행하도록 예약할 수 있습니다.

### 태그 로드 {#loading-tags}

이미지에 적용하는 태그 분류법이 이미 있을 수 있습니다. 반면에 CSV 자산 가져오기 및 [!DNL Experience Manager] 메타데이터 프로필 지원을 사용하면 자산에 태그를 적용하는 프로세스를 자동화할 수 있으며, 태그를 시스템에 로드해야 합니다. 다음 [ACS AEM Tools Tag Maker](https://adobe-consulting-services.github.io/acs-aem-tools/features/tag-maker/index.html) 기능을 사용하면 시스템에 로드되는 Microsoft Excel 스프레드시트를 사용하여 태그를 채울 수 있습니다.

### 자산 수집 {#ingesting-assets}
...
Adobe의 Managed Services 팀은 Glutton이라는 도구를 사용하여 고객 환경에 데이터를 로드합니다. Glutton은 한 디렉토리의 모든 자산을 한 디렉토리에 있는 다른 디렉토리에 로드하는 작은 Java 애플리케이션입니다 [!DNL Experience Manager] 배포. Glutton 대신 Perl 스크립트와 같은 도구를 사용하여 자산을 저장소에 게시할 수도 있습니다.
...
다음 [ACS AEM 도구 CSV 자산 가져오기](https://adobe-consulting-services.github.io/acs-aem-tools/features/csv-asset-importer/index.html) 파일 시스템 및 자산 메타데이터에서 자산을 가져와 CSV 파일로 가져옵니다. Experience Manager 자산 관리자 API는 자산을 시스템으로 가져오고 구성된 메타데이터 속성을 적용하는 데 사용됩니다. 자산은 네트워크 파일 마운트 또는 외부 드라이브를 통해 서버에 마운트되는 것이 좋습니다.
...
### 표현물 처리 {#processing-renditions}

자산을 시스템에 로드한 후 다음을 통해 자산을 처리해야 합니다 [!UICONTROL DAM 자산 업데이트] 메타데이터를 추출하고 표현물을 생성하는 워크플로우입니다. 이 단계를 수행하기 전에 을 복제하고 수정해야 합니다 [!UICONTROL DAM 자산 업데이트] 워크플로우에 따라 유연하게 작업할 수 있습니다. 기본 워크플로우에는 Dynamic Media PTIFF 생성 또는 다음과 같이 필요하지 않은 많은 단계가 포함되어 있습니다 [!DNL InDesign Server] 통합.
...
1. 아래의 파일 시스템 검색 수행 `crx-quickstart/launchpad/felix` 대상 `sling.id`. Delete this file.
...
마이그레이션을 완료하면 [!UICONTROL DAM 자산 업데이트] 일상적인 시스템 사용을 위해 렌디션 생성 및 메타데이터 추출을 지원하도록 워크플로우를 다시 활성화해야 합니다.

## 마이그레이션 [!DNL Experience Manager] 배포 {#migrating-between-aem-instances}

거의 일반적이지는 않지만 많은 양의 데이터를 한 곳에서 마이그레이션해야 하는 경우가 있습니다 [!DNL Experience Manager] 다른 사용자에게 배포 예를 들어 [!DNL Experience Manager] AMS 마이그레이션과 같이 하드웨어를 업그레이드, 업그레이드하거나 새로운 데이터 센터로 마이그레이션합니다.

이 경우 자산이 이미 메타데이터로 채워지고 렌디션이 이미 생성됩니다. 한 인스턴스에서 다른 인스턴스로 자산을 이동하는 데 집중할 수 있습니다. 사이에 마이그레이션할 때 [!DNL Experience Manager] 배포하려면 다음 단계를 수행합니다.

1. 워크플로우 비활성화: Adobe Assets와 함께 표현물을 마이그레이션하고 있으므로 [!UICONTROL DAM 자산 업데이트] 워크플로우.

1. 태그 마이그레이션: 소스에 태그가 이미 로드되어 있으므로 [!DNL Experience Manager] 배포 시 컨텐츠 패키지에서 패키지를 빌드하고 대상 인스턴스에 패키지를 설치할 수 있습니다.

1. 자산 마이그레이션: 한 도구에서 자산을 이동하는 데 권장되는 두 가지 도구가 있습니다 [!DNL Experience Manager] 다른 사용자에게 배포:

   * **Vault 원격 복사** 또는 vlt rcp를 사용하면 네트워크에서 vlt를 사용할 수 있습니다. 소스 및 대상 디렉토리를 지정하고 한 인스턴스에서 모든 저장소 데이터를 다운로드하여 다른 인스턴스로 로드할 수 있습니다. Vlt rcp는에 설명되어 있습니다. [https://jackrabbit.apache.org/filevault/rcp.html](https://jackrabbit.apache.org/filevault/rcp.html)
...
1. 자산 활성화: 다음 지침을 따르십시오. [자산 활성화](#activating-assets) 에 대한 초기 마이그레이션에 대해 문서화됨 [!DNL Experience Manager].

1. 복제 게시: 새 마이그레이션과 마찬가지로, 단일 게시 인스턴스를 로드하고 복제하는 것이 두 노드에서 컨텐츠를 활성화하는 것보다 더 효율적입니다. 자세한 내용은 [게시 복제.](#cloning-publish)

1. 워크플로우 활성화: 마이그레이션을 완료한 후, [!UICONTROL DAM 자산 업데이트] 지속적인 시스템 사용을 위해 렌디션 생성 및 메타데이터 추출을 지원하는 워크플로우입니다.
