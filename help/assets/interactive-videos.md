   ...
   <table> 
    <tbody> 
      ...
      <td><p>작업 유형에서 <strong>경험 조각</strong>을 선택한 후 다음을 수행합니다.<p> 
       <ul> 
       <li>Experience Manager Sites 고객인 경우 검색 아이콘(돋보기)을 선택하여 경험 조각 페이지를 엽니다. 사용할 경험 조각을 선택한 다음 페이지의 오른쪽 상단에서 <strong>이전 페이지의 [액션] 패널로 돌아갈 수 있도록 </strong>을(를) 선택합니다.<br /> <a href="/help/sites-authoring/experience-fragments.md">경험 조각</a>을 참조하세요.</li> 
      </ul> 
       <ul> 
...
샘플 웹 사이트를 사용하는 경우 Quickview 모달 대화 상자는 `BODY` 문서에 직접 첨부된 Quickview 모달 ID를 사용하여 `DIV`(으)로 구현됩니다. 따라서 이 대화 상자를 뷰어의 컨테이너로 이동하는 코드는 다음과 같이 간단합니다.
