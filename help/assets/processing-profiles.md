...
선택적으로 재처리 워크플로우의 배치 크기를 최대 1000개의 자산인 기본값 50개에서 조정할 수 있습니다. 를 실행할 때 _Scene7: 자산 재처리_ 폴더의 워크플로우에서 자산이 일괄적으로 그룹화된 다음 처리를 위해 Dynamic Media 서버로 전송됩니다. 처리 후, 전체 배치 세트에 있는 각 자산의 메타데이터는 Experience Manager 시 업데이트됩니다. 일괄 처리 크기가 큰 경우 처리가 지연될 수 있습니다. 또는 일괄 처리 크기가 너무 작으면 Dynamic Media 서버로 라운드 트립이 너무 많이 발생할 수 있습니다.
...
1. Experience Manager의 자산 페이지에서 처리 프로필이 할당된 자산 및 적용할 자산의 폴더로 이동합니다 **[!UICONTROL Scene7: 자산 재처리]** 워크플로우,
...
1. 선택 **[!UICONTROL 시작]**&#x200B;를 선택하고 을 선택합니다. **[!UICONTROL 확인]**.
   ...
   워크플로우를 모니터링하거나 진행 상태를 확인하려면 Experience Manager 기본 콘솔 페이지에서 **[!UICONTROL 도구]** > **[!UICONTROL 워크플로우]**. 워크플로우 인스턴스 페이지에서 워크플로우를 선택합니다. 메뉴 모음에서 를 선택합니다 **[!UICONTROL 기록 열기]**. 동일한 [워크플로우 인스턴스] 페이지에서 선택한 워크플로우를 종료, 일시 중단 또는 이름을 변경할 수도 있습니다.
...
1. Experience Manager에서 **[!UICONTROL Adobe Experience Manager]** 전역 탐색 콘솔에 액세스하려면 **[!UICONTROL 도구]** (망치) 아이콘 > **[!UICONTROL 워크플로우]** > **[!UICONTROL 모델]**.
...
1. 설정 **[!UICONTROL Scene7에 일괄 업로드 - 단계 속성]** 대화 상자에서 다음을 설정합니다.
   ...
   * 선택 **[!UICONTROL 시간 제한 시 고급]** 시간 초과에 도달했을 때 진행하려는 경우 시간이 초과되면 받은 편지함으로 진행하려면 선택 취소 를 클릭하십시오.
