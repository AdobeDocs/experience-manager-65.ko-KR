...
디지털 자산 관리는 점점 더 많은 시나리오에서 사용되고 있습니다. 전문 사용자를 위한 강력한 유저 인터페이스(예: 사진 작가 또는 택소노믹스 전문가)는 소규모 전문가 수준의 사용자 그룹에 적합한 소규모 솔루션(예: 사진 작가 또는 분류 전문가)에서 점점 더 다양한 사용자 그룹으로 이동할 때 너무 많은 정보를 제공할 수 있고 이해 관계자는 특정 사용자 인터페이스 또는 애플리케이션을 요청하여 고객과 연관성이 높은 디지털 에셋에 액세스할 수 있도록 할 수 있습니다. [!DNL Adobe Experience Manager Assets]
...
에셋 중심의 애플리케이션을 만드는 것은 코딩 작업이 필요 없는 구성 프로세스, 사용 중인 메타데이터에 대한 지식 및 사용자 그룹 및 요구 사항에 대한 지식만 있을 뿐만 아니라 다음을 통해 생성된 자산 중심의 애플리케이션 [!DNL Assets] 은 확장 가능합니다. 에셋 검색, 보기 및 수정에 사용할 수 있는 재사용 가능한 구성 요소를 중재하여 만들 수 있습니다.

자산 중심의 응용 프로그램은 자산 편집기 페이지로 [!DNL Experience Manager] 구성되며, 이를 사용하여 특정 자산을 자세히 볼 수 있습니다. 자산 편집기 페이지에서는 메타데이터의 편집도 허용할 수 있습니다. 단, 자산에 액세스하는 사용자에게 필요한 권한이 있어야 합니다.
...
여러 자산을 한 번에 변경할 [!DNL Experience Manager Assets] 수 있습니다. 자산을 선택한 후 해당 자산을 동시에 변경할 수 있습니다.
