...
Digital Asset Management 는 점점 더 많은 시나리오에서 사용되고 있습니다. 전문 교육을 받은 소규모 사용자 그룹(예: 사진 작가 또는 분류학자)을 위한 소규모 솔루션에서 비즈니스 사용자, WCM 작성자, 기자 등 보다 크고 다양한 사용자 그룹으로 이동할 때 의 강력한 사용자 인터페이스 [!DNL Adobe Experience Manager Assets] 전문 사용자에게는 너무 많은 정보가 제공될 수 있으며 이해 당사자는 특정 사용자 인터페이스 또는 애플리케이션에 관련성이 있는 디지털 에셋에 액세스하도록 요청할 수 있습니다.
...
에셋 중심 애플리케이션을 만들면 코딩이 필요 없고 사용 중인 메타데이터에 대한 지식뿐만 아니라 사용자 그룹 및 해당 요구 사항에 대한 지식만 필요한 구성 프로세스가 상당 부분 활성화됩니다. 을 사용하여 만든 자산 중심 애플리케이션 [!DNL Assets] 확장 가능: 중간 정도의 코딩 노력을 사용하여 자산을 검색, 보기 및 수정하기 위한 재사용 가능한 구성 요소를 만들 수 있습니다.
...
에셋 편집기를 사용자 정의하여 사용자가 디지털 에셋을 보고 편집할 수 있는 방법을 결정합니다. 이렇게 하려면 새 자산 편집기 페이지를 만든 다음 사용자가 해당 페이지에서 수행할 수 있는 보기 및 작업을 사용자 지정합니다.
...
사용자 지정된 자산 편집기 페이지를 만든 후에는 자산을 두 번 클릭할 때 만든 사용자 지정된 자산 공유가 사용자 지정된 편집기 페이지에서 자산을 열도록 해야 합니다.
...
1. 다음에서 **자산 편집기 경로** 필드에서 에셋 공유 페이지에서 에셋을 열 에셋 편집기의 경로를 입력하고 를 클릭합니다 **확인**.
...
1. 사이드 킥에서 자산 편집기로 구성 요소를 드래그하고 구성 요소 대화 상자에서 모든 수정 작업을 수행합니다. 구성 요소는 다음 표에 설명되어 있으며 다음의 세부 지침에 설명되어 있습니다.

>[!NOTE]
>
>에셋 편집기 페이지를 디자인할 때 읽기 전용이거나 편집 가능한 구성 요소를 만듭니다. 사용자는 연필 이미지가 구성 요소에 나타나는 경우 필드를 편집할 수 있다는 것을 알고 있습니다. 기본적으로 대부분의 구성 요소는 읽기 전용으로 설정됩니다.
...
1. 다음을 클릭하여 시작 작업 만들기 **편집** 를 입력합니다. 원하는 경우 상자 제목을 입력할 수 있습니다. 기본적으로 상자 제목은 입니다. **메타데이터**. 검증용 Java-script 클라이언트 코드를 생성하려면 클라이언트 검증(Client Validation) 확인란을 선택합니다.
...
1. 필드 이름을 두 번 클릭합니다. 예: **제목** 메타데이터 필드를 열고 변경합니다. 다음에서 **일반** 의 탭 **구성 요소 편집** 창에서는 네임스페이스와 필드 레이블 및 유형(예: )을 정의합니다. `dc:title`.
...
하위 에셋 구성 요소를 더블 클릭하여 기본 에셋 및 모든 하위 에셋의 제목을 변경할 수 있는 하위 에셋 대화 상자를 엽니다. 기본값은 해당 필드 아래에 나타납니다.
...
태그 구성 요소를 두 번 클릭하여 태그 대화 상자를 엽니다. 이 대화 상자에서 원할 경우 태그에서 제목을 변경할 수 있고 할당된 네임스페이스를 선택할 수 있습니다. 이 필드를 편집할 수 있게 하려면 **[!UICONTROL 편집 숨기기]** 확인란. 기본적으로 태그는 편집할 수 있습니다.
...
썸네일 구성 요소를 두 번 클릭하여 대체 텍스트를 변경할 수 있는 썸네일 대화 상자를 엽니다. 기본적으로 썸네일 대체 텍스트는 다음과 같습니다. **다운로드하려면 클릭** 에셋.
...
| [!UICONTROL 잠금] | 사용자가 자산을 잠글 수 있습니다. 이 기능은 기본적으로 활성화되어 있지 않으며 구성 요소 목록에서 활성화해야 합니다. |
...
포함 [!DNL Experience Manager Assets] 여러 에셋을 한 번에 변경할 수 있습니다. 선택한 에셋을 선택한 후 다음 항목을 동시에 변경할 수 있습니다.
