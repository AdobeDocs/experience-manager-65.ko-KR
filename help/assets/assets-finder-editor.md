...
디지털 자산 관리가 점점 더 많은 시나리오에서 사용되고 있습니다. 전문 사용자용 [!DNL Adobe Experience Manager Assets]의 강력한 유저 인터페이스는 전문 전문 지식을 가진 소규모 사용자 그룹(예: 사진 작가 또는 택소노믹스 전문가)에서 비즈니스 사용자, WCM 작성자, 저널리스트 등 다양한 사용자 그룹으로 전환하는 경우 많은 정보를 제공할 수 있고 이해 관계자는 특정 유저 인터페이스 또는 애플리케이션에 해당 사용자에게 연관성이 높은 디지털 에셋에 액세스하도록 요청할 수 있습니다.
