---
title: 메타데이터 프로필을 사용하여 자산의 메타데이터 요구 사항을 사용자 정의
...
1. 샘플 메타데이터와 같은 메타데이터 프로필의 제목을 입력하고 만들기를 **[!UICONTROL 탭합니다]**. 메타데이터 [!UICONTROL 프로필에] 대한 양식 편집이 표시됩니다.
...
1. 구성 요소를 클릭하고 설정 탭에서 해당 속성을 **[!UICONTROL 구성합니다]** . 예를 들어 설명 구성 **[!UICONTROL 요소를]** 클릭하고 해당 속성을 편집합니다.
   ...
   * **[!UICONTROL 기본값]**:이 속성을 사용하여 메타데이터 구성 요소의 기본값을 추가합니다. 예를 들어 &quot;My description&quot;을 지정하면 이 값이 자산의 메타데이터 `dc:desc` 노드의 속성에 할당됩니다.
   ...
   >[!NOTE]
   >
   >새 메타데이터 속성에 기본값 추가(에 이미 존재하지 않음) `/jcr:content/metadata` node) does not display the property and its value on the asset&#39;s Properties page by default. 자산의 속성 페이지에서 새 속성을 보려면 [!UICONTROL 해당] 스키마 양식을 수정합니다.
...
1. 완료를 탭/ **[!UICONTROL 클릭합니다]**. 메타데이터 프로필은 메타데이터 프로필 **[!UICONTROL 페이지의 프로필 목록에 추가됩니다]** .<br>
...
1. 메타데이터 **[!UICONTROL 프로필]** 페이지에서 복사본을 만들 메타데이터 프로필을 선택합니다.
...
1. 도구[] 모음에서 **[!UICONTROL 메타데이터]** 프로필 삭제
...
나중에 변경한 기존 메타데이터 프로필이 이미 있는 폴더의 자산을 다시 처리할 수 있습니다. [폴더의 자산에 대한 처리 프로필을 편집한 후 재처리](processing-profiles.md#reprocessing-assets)를 참조하십시오.
...
1. 폴더에서 확인 표시를 탭하거나 클릭하여 선택한 다음 속성을 탭하거나 **[!UICONTROL 클릭합니다]**.

1. 메타데이터 **[!UICONTROL 프로필]** 탭을 선택하고 드롭다운 메뉴에서 프로필을 선택하고 저장을 **[!UICONTROL 누릅니다]**.
...
* 해당 프로필로 `https://[aem_server]:[port]/mnt/overlay/dam/gui/content/assets/foldersharewizard.html/content/dam` 이동하여 적용하고 저장을 **[!UICONTROL 누릅니다]**.
...
>* [처리 프로필을 사용하기 위해 디지털 자산을 구성하는 모범 사례](/help/assets/organize-assets.md)
