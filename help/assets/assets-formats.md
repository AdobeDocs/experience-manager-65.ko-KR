...
description: 파일 형식 및 MIME 형식은  [!DNL Assets] and [!DNL Dynamic Media] 에서 지원하며 각 형식에 지원되는 기능입니다.
contentOwner: AG
...
[!DNL Experience Manager Assets] 는 다양한 파일 형식을 지원하며 각 기능은 서로 다른 MIME 유형을 다양하게 지원합니다. [!DNL Assets]을(를) 다른 표준을 준수하는 DAM(디지털 자산 관리) 솔루션 및 데스크탑 소프트웨어와 통합하려면 Adobe [!DNL Extensible Metadata Platform](XMP)을 사용하십시오.
...
| 형식 | 업로드<br>(입력 형식) | <br> 이미지<br> 사전 설정<br> 만들기(출력 형식) | Preview<br> dynamic<br> rendition | <br> dynamic<br> 변환 전달 | <br> dynamic<br> 변환 다운로드 |
|---|:---:|:---:|:---:|:---:|:---:|
| PNG | ✓ | ✓ | ✓ | ✓ | ✓ |
| GIF | ✓ | ✓ | ✓ | ✓ | ✓ |
| TIFF | ✓ | ✓ | ✓ | ✓ | ✓ |
...
* EPS 파일에 대한 지원은 래스터 이미지에만 적용됩니다. 예를 들어 EPS 벡터 이미지에 대한 축소판 생성은 기본적으로 지원되지 않습니다. 지원을 추가하려면 [ImageMagick](best-practices-for-imagemagick.md)을 구성하십시오. 타사 도구를 통합하여 추가 기능을 활성화하려면 [명령줄 기반 미디어 처리기](media-handlers.md#command-line-based-media-handler)을 참조하십시오.
...
[다이내믹 미디어에서 3D 자산을 사용한 작업을 참조하십시오.](/help/assets/assets-3d.md)
...
Adobe PDF 래스터라이저 라이브러리는 컨텐츠를 많이 사용하는 대용량 [!DNL Adobe Illustrator] 및 PDF 파일에 대한 고품질 축소판과 미리 보기를 생성합니다. Adobe은 다음과 같은 경우 PDF 래스터라이저 라이브러리를 사용하는 것이 좋습니다.
...
[이미징 트랜스코딩 라이브러리](imaging-transcoding-library.md)를 참조하십시오.

## 지원되는 Camera Raw {#supported-camera-raw}

[!DNL Adobe Camera Raw] 라이브러리는 [!DNL Assets]에서 원시 이미지를 인제스트할 수 있도록 합니다. [Camera Raw 지원](camera-raw.md)을 참조하십시오.
...
## 다이내믹 미디어 {#supported-document-formats-dynamic-media}에서 지원되는 문서 형식
...
* Dynamic Media를 사용하여 INDD 파일에 대한 동적 변환을 생성하려면 [InDesign(INDD) 파일 포맷](../assets/managing-image-presets.md#indesign-indd-file-format)을 참조하십시오.
...
## Dynamic Media에서 {#supported-input-video-formats-for-dynamic-media-transcoding} 트랜스코딩에 지원되는 입력 비디오 포맷
...
## 지원되는 MIME 형식 {#supported-mime-types}

기본적으로 [!DNL Experience Manager]은 파일 확장자를 사용하여 파일 유형을 감지합니다. [!DNL Experience Manager] 파일의 내용에서 검색할 수 있습니다. 후자의 경우 [!DNL Experience Manager] 웹 콘솔에서 [!UICONTROL 일 CQ DAM MIME 유형 서비스]의 [!UICONTROL 내용] 옵션에서 MIME 감지를 선택합니다.

지원되는 MIME 형식 목록은 `/conf/global/settings/cloudconfigs/dmscene7/jcr:content/mimeTypes`의 CRXDE Lite에서 사용할 수 있습니다.
