...
썸네일 크기 조정은 다음 형식으로 정의됩니다. **`width:height:center`**, 예: `80:80:false`. 폭과 높이는 축소판의 크기를 픽셀 단위로 결정합니다. 가운데 값은 false 또는 true이고, true로 설정하면 썸네일 이미지의 크기가 구성에 지정된 것과 정확히 일치함을 나타냅니다. 크기가 조정된 이미지가 더 작으면 축소판 내에서 가운데에 표시됩니다.
