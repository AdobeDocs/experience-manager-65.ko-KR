...
매크로와 마찬가지로 이미지 사전 설정은 이름 아래에 저장된 크기 및 형식 명령의 사전 정의된 컬렉션입니다. 이미지 사전 설정 작동 방식을 이해하려면 웹 사이트에서 각 제품 이미지가 데스크탑 및 모바일 게재에 대해 서로 다른 크기, 서로 다른 형식 및 압축률을 나타내야 한다고 가정합니다.
...
[AEM Assets과 InDesign Server](/help/assets/indesign.md)의 통합을 참조하십시오.
...
ExtendScript은 [!UICONTROL DAM 자산 업데이트] 워크플로우의 미디어 추출 프로세스 구성 요소의 인수에 경로를 지정합니다.
...
축소판 크기 조절은 다음 형식으로 정의됩니다.**[!UICONTROL width:height:center]**(예: *80:80:false*) 너비와 높이는 축소판의 크기를 픽셀 단위로 결정합니다.center 값은 false 또는 true이고, true로 설정하면 축소판 이미지의 크기가 구성에 지정된 크기와 정확히 일치함을 나타냅니다. 크기가 조정된 이미지가 더 작으면 축소판 내에서 가운데에 표시됩니다.
