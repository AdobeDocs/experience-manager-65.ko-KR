...
필터 **[!UICONTROL 패널을 사용하면]** 파일 유형, 파일 크기, 마지막 수정 날짜, 자산 상태, 인사이트 데이터 및 Adobe Stock 라이선스와 같은 다양한 옵션(예측)을 기반으로 검색 결과를 필터링하여 검색 범위를 좁힐 수 있습니다. 관리자는 필터 패널을 사용자 정의하고 검색 패싯을 사용하여 검색 조건자를 추가하거나 제거할 수 있습니다. 필터 [!UICONTROL 패널의 파일] 유형 [!UICONTROL 필터에는] 혼합 상태 확인란이있습니다. 따라서 모든 중첩 설명(또는 형식)을 선택하지 않으면 첫 번째 수준 확인란은 선택되지 않고 대신 통과됩니다.
...
* 모두 [!UICONTROL 선택] 확인란을 사용하여 카드 보기에서 검색된 처음 100개의 자산과 목록 보기에서 검색된 처음 200개의 자산만 선택할 수 있습니다. 사용자 인터페이스에서 더 많은 자산을 스크롤하고 로드하는 경우 모두 선택 [!UICONTROL 옵션을 사용하여 더 많은 자산을 선택할 수] 있습니다.
