...
+++## 스마트 이미징에서 계정을 활성화하려면 언제 해야 합니까? {#when-can-i-expect-my-account-to-be-enabled-with-smart-imaging}
...
+++### &quot;tuning&quot;은 무엇입니까? 정의할 수 있는 설정 또는 동작이 있습니까? {#tuning-settings}
...
+++## 스마트 이미징에서 자동으로 품질 백분율 출력 설정을 조정합니까? 또는 수동으로 조정되는 설정이며 모든 이미지에 적용됩니까? 어떤 범위내에서? {#percent-quality}
...
+++### 장치 픽셀 비율(dpr) 기능이 복합 이미지에서 예상대로 작동하지 않는 이유는 무엇입니까? {#composite-images}
...
+++### 스마트 이미징 PNG가 무손실 WebP/AVIF로 변환되는 이유는 무엇입니까? {#convert-to-lossless}
