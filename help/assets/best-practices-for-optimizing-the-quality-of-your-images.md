...
AEM에는 이미지 조정 및 최적화 및 렌더링 결과를 위한 100개 이상의 Dynamic Media 이미지 제공 명령이 포함되어 있습니다. 다음 지침을 통해 프로세스를 간소화하고 몇 가지 필수 명령과 모범 사례를 사용하여 신속하게 좋은 결과를 얻을 수 있습니다.
...
이미지 선명하게 하기는 웹 사이트에서 이미지를 제어하는 가장 복잡한 측면이며 오류가 많은 경우입니다. 다음 유용한 리소스를 참조하여 AEM에서 선명하게 하고 언샵 마스킹이 작동하는 방식에 대해 자세히 알아보십시오.

우수 사례 백서 [Adobe Dynamic Media Classic에서 이미지 선명하게 하기](/help/assets/assets/sharpening_images.pdf)도 AEM에 적용됩니다.
...
* 언샵 마스킹( `&op_USM`) - 언샵 마스킹은 업계 표준 선명도 필터입니다. 가장 좋은 방법은 아래 지침에 따라 언샵 마스킹으로 이미지를 선명하게 하는 것입니다. 언샵 마스킹을 사용하면 다음 세 가지 매개변수를 제어할 수 있습니다.
   ...
   * `&op_sharpen=`amount,radius,임계값
      ...
      * **[!UICONTROL 임계값]** (0-255, 효과 민감도)
      ...
      이미지 선명하게 하기 에 대한 AEM 도움말 항목입니다.
