...
* `&jpegSize=` jpg 압축 매개 변수와 상호 작용합니다 `&qlt=`. 지정된 JPG 압축 매개 변수(`&qlt=`)의 JPG 응답이 jpegSize 값을 초과하지 않으면 `&qlt=` 정의된 대로 이미지가 반환됩니다. 그렇지 `&qlt=` 않으면 이미지가 허용되는 최대 크기로 맞춰질 때까지 또는 시스템에서 이미지가 맞지 않는다고 판단하고 오류를 반환할 때까지 점진적으로 감소합니다.
