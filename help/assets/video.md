...
1. **다음을 수행하여 Dynamic Media** 비디오를 업로드합니다.
   ...
   * 고유한 비디오 인코딩 프로필을 만듭니다. 또는 다이내믹 미디어와 함께 제공되는 사전 정의된 _응용 비디오 인코딩_ 프로필을 사용하면 됩니다.
      ...
      * [비디오 인코딩 프로필](/help/assets/video-profiles.md#creating-a-video-encoding-profile-for-adaptive-streaming) 만들기
   ...
   * 기본 소스 비디오를 업로드할 하나 이상의 폴더에 비디오 처리 프로필을 연결합니다.
      ...
      * 처리 프로필[을 사용하기 위해 디지털 자산을 구성하는 모범 사례에 대해 자세히 알아보십시오.](/help/assets/organize-assets.md)
   ...
   * 기본 소스 비디오를 폴더에 업로드합니다. 최대 15GB의 비디오 파일을 업로드할 수 있습니다. 폴더에 비디오를 추가하면 폴더에 할당된 비디오 처리 프로필에 따라 인코딩됩니다.
      ...
      * [지원되는 입력 파일 포맷](/help/assets/assets-formats.md#supported-multimedia-formats)에 대해 자세히 알아보십시오.
...
자산 구성처리 프로필 사용을  [위한 디지털 자산을 구성하기 위한 모범 사례에 대한 자세한 내용](organize-assets.md)
   ...
   * 비디오 검토, 승인 및 주석 달기, 버전 관리
      ...
      * [비디오에 주석 ](managing-video-assets.md#annotate-video-assets) 달기 또는  [자산에 주석 달기](manage-assets.md#annotating)
...
적응형 비디오 스트리밍은 다양한 iOS 플랫폼에서 지원됩니다. [다이내믹 미디어 뷰어 참조 안내서](https://experienceleague.adobe.com/docs/dynamic-media-developer-resources/library/viewers-aem-assets-dmc/video/c-html5-video-reference.html#video)를 참조하십시오.
...
[HTML5 뷰어 정보](https://experienceleague.adobe.com/docs/dynamic-media-developer-resources/library/viewers-for-aem-assets-only/c-html5-aem-asset-viewers.html#viewers-for-aem-assets-only)도 참조하십시오.
...
비디오에 전체 비디오 전체에서 유사한 모션 레벨이 포함된 경우 CBR을 사용합니다. CBR은 비디오 컨텐츠를 스트리밍하는 데 가장 일반적으로 사용됩니다. [사용자 정의 비디오 인코딩 매개 변수 사용](/help/assets/video-profiles.md#using-custom-added-video-encoding-parameters)을 참조하십시오.
...
비디오 처리 프로필을 무시하고 YouTube에 직접 게시하는 경우 간단히 말해 AEM Asset의 비디오 에셋에 볼 수 있는 축소판이 없을 수 있습니다. 또한 동적 미디어 또는 dynamicmedia_scene7 실행 모드에서 실행되는 경우 인코딩되지 않은 비디오는 어떤 다이내믹 미디어 자산 유형에서도 작동하지 않습니다.
...
1. 오른쪽 위 모서리 근처의 Google 클라우드 페이지에서 **[!UICONTROL 콘솔을 클릭합니다.]**
...
1. 다음 중 하나를 수행합니다.
   ...
   * 프로젝트의 대시보드의 시작하기 카드에서 **[!UICONTROL 탐색을 탭하고 API를 활성화합니다.]**
   * 프로젝트의 대시보드, APIs 카드에서 **[!UICONTROL API로 이동을 누릅니다.]**

   ![6_5_googleaccount-api-enable2](assets/6_5_googleaccount-apis-enable2.png)

1. API 및 서비스 페이지의 맨 위에 있는 **[!UICONTROL API 및 서비스 활성화를 탭합니다.]**
1. API 라이브러리 페이지의 왼쪽에 있는 **[!UICONTROL 범주]**&#x200B;에서 **[!UICONTROL YouTube를 누릅니다.]** 페이지 오른쪽에서  **[!UICONTROL YouTube 데이터 API를 누릅니다.]**
...
1. 프로젝트 페이지에 자격 증명 추가 페이지의 4단계에서 **[!UICONTROL 자격 증명 다운로드]** 머리글 아래에서 **[!UICONTROL 다운로드를 누릅니다.]**
...
YouTube에 비디오를 게시하려면 하나 이상의 채널이 있어야 합니다. YouTube 채널을 이미 만든 경우 이 작업을 건너뛰고 [게시를 위한 태그 추가](/help/assets/video.md#adding-tags-for-publishing)로 이동할 수 있습니다.
...
1. 개요 페이지의 추가 기능 머리글 아래에서 **[!UICONTROL 내 모든 채널을 보거나 새 채널을 만듭니다.]**
1. 채널 페이지에서 **[!UICONTROL 새 채널 만들기를 클릭합니다.]**
1. 브랜드 계정 페이지의 브랜드 계정 이름 필드에 비디오 자산을 게시할 위치를 선택한 회사 이름이나 다른 채널 이름을 입력한 다음 **[!UICONTROL 만들기를 클릭합니다.]**
...
또는 AEM에서 기본 태그를 사용하려는 경우 이 작업을 건너뛰고 [Enabling the YouTube Publish replication agent](#enabling-the-youtube-publish-replication-agent)로 이동할 수 있습니다.

### YouTube 게시 복제 에이전트 {#enabling-the-youtube-publish-replication-agent} 활성화

YouTube 게시 복제 에이전트를 활성화한 후 Google 클라우드 계정에 대한 연결을 테스트하려면 **[!UICONTROL 연결 테스트를 누릅니다.]** 브라우저 탭에 연결 결과가 표시됩니다. YouTube 채널을 추가한 경우 해당 채널의 목록이 테스트의 일부로 표시됩니다.

1. AEM의 왼쪽 위 모서리에서 AEM 로고를 클릭하고 왼쪽 레일에서 **[!UICONTROL 도구]** > **[!UICONTROL 배포]** > **[!UICONTROL 복제]** > **[!UICONTROL 작성자 에이전트(]**)를 클릭합니다.
1. 작성자 페이지에서 **[!UICONTROL YouTube 게시(youtube)를 클릭합니다.]**
1. 도구 모음에서 설정 오른쪽에 있는 **[!UICONTROL 편집을 클릭합니다.]**
...
1. 글로벌 페이지의 오른쪽 위 모서리 근처에 있는 **[!UICONTROL 만들기를 누릅니다.]**
...
1. 일반 텍스트 편집기를 사용하여 [Google 클라우드 설정 구성](/help/assets/video.md#configuring-google-cloud-settings) 작업에서 이전에 다운로드하고 저장한 JSON 파일을 엽니다.
1. 전체 JSON 텍스트를 선택하고 복사합니다.
1. YouTube 계정 설정 대화 상자로 돌아갑니다. **[!UICONTROL JSON 구성]** 필드에서 JSON 텍스트를 붙여 넣습니다.
1. 페이지의 오른쪽 위 모서리 근처에 있는 **[!UICONTROL 저장을 탭합니다.]**
...
1. 원하는 태그 추가가 완료되면 **[!UICONTROL 저장을 탭합니다.]**
...
1. 타사 서비스 머리글 아래의 YouTube에서 **[!UICONTROL 지금 구성을 누릅니다.]**
...
1. YouTube 계정 설정 대화 상자의 **[!UICONTROL 응용 프로그램 이름]** 필드에 Google 프로젝트 ID를 입력합니다.
   ...
   처음에 [에서 Google 클라우드 설정](/help/assets/video.md#configuring-google-cloud-settings)을(를) 구성할 때 프로젝트 ID를 지정했습니다.
...
1. 원하는 태그 추가가 완료되면 **[!UICONTROL OK를 누릅니다.]**
...
1. **[!UICONTROL 기본값을 클릭합니다.]** (선택 상자의 왼쪽에 &quot;기본값&quot;을 표시하지 마십시오.)
1. **[!UICONTROL default]** 페이지에서 **[!UICONTROL video]**&#x200B;의 왼쪽에 있는 상자를 선택한 다음 **[!UICONTROL 편집을 클릭합니다.]**
...
1. 메타데이터 프로필 추가 대화 상자의 **[!UICONTROL 프로필 제목]** 텍스트 필드에 이름 `YouTube Video`을 입력한 다음 **[!UICONTROL 만들기를 클릭합니다.]**
...
>[!NOTE]
>
>다이내믹 미디어 - Scene7 모드에서 실행 중인 경우 즉시 게시가 YouTube에 자동으로 게시되지는 않습니다. 다이내믹 미디어 - Scene7 모드를 설정하면 두 가지 게시 옵션을 선택할 수 있습니다.**[!UICONTROL 즉시]** 또는 **[!UICONTROL 활성화 시.]**
>
...
1. 페이지의 오른쪽 위 모서리에서 **[!UICONTROL 선택을 클릭합니다.]**
1. 비디오 속성 페이지의 오른쪽 위 모서리에서 **[!UICONTROL 저장 후 닫기를 클릭합니다.]**
...
1. 도구 모음에서 **[!UICONTROL 게시 관리를 클릭합니다.]** 세 점 아이콘(.)을 눌러야 할 수도 있습니다...) 도구 모음에서 **[!UICONTROL 게시 관리.]**
1. 게시 관리 페이지에서 **[!UICONTROL 게시 취소를 누릅니다.]**
1. 페이지의 오른쪽 위 모서리에서 **[!UICONTROL Next.]**&#x200B;를 누릅니다.
1. 페이지의 오른쪽 위 모서리에서 **[!UICONTROL 게시 취소를 누릅니다.]**
...
1. 자산 폴더에서 비디오 인코딩 진행 상황 보기:
   ...
   * 목록 보기에서는 비디오 인코딩 진행률이 **[!UICONTROL 처리 상태]** 열에 표시됩니다. 오류가 있으면 이 메시지가 동일한 열에 표시됩니다.
   ...
   이 열은 기본적으로 표시되지 않습니다. 열을 활성화하려면 보기 드롭다운 메뉴에서 **[!UICONTROL 설정 보기]**&#x200B;를 선택하고 **[!UICONTROL 처리 상태]** 열을 추가하고 **[!UICONTROL 업데이트를 탭하거나 클릭합니다.]**
...
1. 진행 중인 워크플로우의 경우 **[!UICONTROL 도구]** > **[!UICONTROL 워크플로]** > **[!UICONTROL 인스턴스로부터 사용 가능한 워크플로우 인스턴스를 참조하십시오.]**
   ...
   인스턴스를 선택하고 **[!UICONTROL 작업 내역 열기를 탭하거나 클릭합니다.]**

   ![chlimage_1-434](assets/chlimage_1-434.png)

   워크플로우 인스턴스 영역에서 워크플로우를 일시 중단하거나 종료하거나 이름을 변경할 수도 있습니다. 자세한 내용은 [워크플로우 관리](/help/sites-administering/workflows-administering.md)를 참조하십시오.

1. 실패한 작업의 경우 **[!UICONTROL 도구]** > **[!UICONTROL 워크플로]** > **[!UICONTROL 실패에서 사용할 수 있는 워크플로 실패를 참조하십시오.]** Workflow  **[!UICONTROL Failure는]** 모든 실패한 워크플로우 활동을 방지합니다.
...
1. 완료된 워크플로우의 경우 **[!UICONTROL 도구]** > **[!UICONTROL 워크플로우]** > **[!UICONTROL 보관에서 사용할 수 있는 워크플로우 아카이브를 참조하십시오.]** 워크플로우  **** 아카이브 로그 항목은 모두 완료된 워크플로우 활동입니다.
...
* 인코딩 작업의 경우, **[!UICONTROL 일 CQ Workflow 이메일 알림 서비스를 검색하여**[!UICONTROL &#x200B;도구&#x200B;]**>**[!UICONTROL &#x200B;작업&#x200B;]**>**[!UICONTROL &#x200B;웹 콘솔&#x200B;]**에서 모든 AEM 워크플로우 이메일 알림의 구성 페이지에 액세스할 수 있습니다.]** AEM [에서 이메일 알림 구성을 참조하십시오](/help/sites-administering/notification.md). **[!UICONTROL Notify on Abort]** 또는 **[!UICONTROL Notify on Complete]**&#x200B;에 대한 확인란을 선택하거나 취소할 수 있습니다.

* YouTube 게시 작업의 경우 다음을 수행합니다.

1. AEM에서 **[!UICONTROL 도구]** > **[!UICONTROL 워크플로]** > **[!UICONTROL 모델을 누릅니다.]**
1. 워크플로우 모델 페이지에서 **[!UICONTROL YouTube에 게시]**&#x200B;를 선택한 다음 도구 모음에서 **[!UICONTROL 편집]**&#x200B;을 누릅니다.
...
1. 인라인 도구 모음에서 구성 아이콘(공구 모양)을 누릅니다. **[!UICONTROL 인수]** 탭을 클릭합니다.

   ![6_5_publishtoyoutubeworkflow-configurationicon](assets/6_5_publishtoyoutubeworkflow-configurationicon.png)

1. YouTube 업로드 프로세스 - 단계 속성 대화 상자에서 **[!UICONTROL 인수]** 탭을 누릅니다.
...
1. 다음 확인란을 선택하거나 취소할 수 있습니다.
   ...
   * 게시 완료 - 채널 및 URL에 대한 정보 포함
   ...
   >[!NOTE]
   >
   >이러한 이메일은 YouTube에만 해당되며 일반 워크플로우 이메일 알림에도 추가됩니다. 그 결과, 두 개의 이메일 알림 세트(CQ Workflow 이메일 알림 서비스&#x200B;]**에서 사용할 수 있는 일반 알림, 구성 설정에 따라 YouTube에서만 받을 수 있습니다.**[!UICONTROL 

1. 완료되면 대화 상자의 오른쪽 위 모서리 근처에 있는 **[!UICONTROL Done]** 아이콘(확인 표시)을 누릅니다.
1. YouTube에 게시 워크플로우 페이지의 오른쪽 위 모퉁이에서 **[!UICONTROL 동기화를 누릅니다.]**
...
[다이내믹 미디어 Cloud Services 구성](/help/assets/config-dynamic.md)을 참조하십시오.
...
비디오 보고서가 올바르게 작동하려면 다이내믹 미디어 Cloud Services이 구성되면 보고서 세트 ID가 자동으로 생성됩니다. 동시에 보고서 세트 ID가 게시 서버로 푸시되어 자산을 미리 볼 때 URL 복사 기능을 사용할 수 있습니다. 그러나 게시 서버가 이미 설정되어 있어야 합니다. 게시 서버가 설정되어 있지 않으면 여전히 게시하여 비디오 보고서를 볼 수 있지만 Dynamic Media Cloud 구성으로 돌아가 **[!UICONTROL OK를 탭해야 합니다.]**

비디오 보고서를 보려면:

1. AEM의 왼쪽 위 모서리에서 AEM 로고를 누른 다음 왼쪽 레일에서 **[!UICONTROL 도구]**(망치 아이콘) > **[!UICONTROL 자산]** > **[!UICONTROL 비디오 보고서를 누릅니다.]**
...
비디오 데이터를 저장할 시작 및 종료 날짜 범위를 지정한 다음 **[!UICONTROL 보고서 실행을 누릅니다.]**

   상위 지표 그룹 상자는 사이트에서 게시된 모든 *개 비디오에 대한 다양한 집계 측정을 식별합니다.*
...
다이내믹 미디어 뷰어 참조 및 Scene7 HTML5 뷰어 SDK를 사용하여 고유한 비디오 뷰어를 만듭니다.

[다이내믹 미디어 뷰어 참조 안내서](https://experienceleague.adobe.com/docs/dynamic-media-developer-resources/library/home.html)를 참조하십시오.
...
1. 포함 코드에서 다음 줄을 찾습니다.
   ...
   config2 매개 변수의 올바른 값은 **포함 코드 **와 복사본 **URL **함수에 모두 있습니다. 복사 **URL **명령의 URL에서 찾을 매개 변수는 `&config2=<value>`입니다. 이 값은 거의 항상 `companypreset`이지만 일부 경우 `companypreset-1`, `companypreset-2` 등이 될 수도 있습니다.
...
1. 타사 애플리케이션 또는 서비스를 사용하여 비디오 캡션/자막 파일을 만듭니다.
   ...
   도구에서 **[!UICONTROL 비디오 파일]** 필드의 URL을 입력하고 비디오 파일의 복사한 URL을 붙여 넣은 다음 **[!UICONTROL 로드]**&#x200B;를 클릭합니다. 비디오 파일 자체에 URL을 가져온 다음 비디오 파일의 **[!UICONTROL URL 입력 필드에 붙여 넣을 수 있는 에셋](/help/assets/linking-urls-to-yourwebapplication.md#obtaining-a-url-for-an-asset)에 대한 URL 얻기를 참조하십시오.[]** 그러면 Internet Explorer, Chrome 또는 Safari에서 비디오를 기본적으로 재생할 수 있습니다.
   ...
   <table>
     <tbody>
      <tr>
       ...
       <td>
       <ol>
       <li>업로드한 장 파일과 연결할 <i>게시된 </i>비디오 자산으로 이동합니다. URL은 </i> 이후에 <i>만 복사할 수 있습니다. 먼저 자산을 <i>게시된</i>합니다. <a href="/help/assets/publishing-dynamicmedia-assets.md">자산 게시를 참조하십시오.</a></i></li>
...
## 다이내믹 미디어의 비디오 축소판 정보 - Scene7 모드 {#about-video-thumbnails-in-dynamic-media-scene-mode}

비디오 축소판은 비디오 프레임 또는 고객에게 비디오를 나타내는 이미지 자산의 축소판입니다. 이 축소판은 고객이 비디오를 클릭하도록 권장해야 합니다.

AEM의 모든 비디오에는 관련 축소판이 있어야 합니다.축소판은 교체하지 않으면 삭제할 수 없습니다. 기본적으로 비디오를 AEM에 업로드할 때 첫 번째 프레임이 축소판으로 사용됩니다. 하지만 예를 들어 브랜딩 또는 시각적 검색을 위해 축소판을 사용자 정의할 수 있습니다. 비디오 축소판을 사용자 정의할 때 사용할 프레임에서 비디오를 재생하거나 이미 업로드한 이미지 자산과 디지털 자산 관리자에서 *게시된*&#x200B;을 선택할 수 있습니다.

비디오에서 선택한 사용자 정의 비디오 축소판 이미지는 DAM에 별도의 에셋으로 추출되어 저장되지 않습니다. 그러나 기존 이미지 자산에서 선택하는 사용자 정의 비디오 축소판은 JCR에 저장됩니다. 선택한 자산의 경로는 다음 예제 경로로 비디오 자산의 노드 아래에 저장됩니다.

`/content/dam/*<folder_name*>/<*video_name*>/jcr:content/manualThumbnail`

비디오 축소판을 사용자 지정하는 기능은 비디오가 있는 폴더에 비디오 프로필을 적용한 후에만 사용할 수 있습니다.

[Dynamic Media의 비디오 축소판 정보 - 하이브리드 모드](#about-video-thumbnails-in-dynamic-media-hybrid-mode)를 참조하십시오.

### 사용자 정의 비디오 축소판 {#adding-a-custom-video-thumbnail} 추가

이러한 단계는 &quot;Dynamicmedia_Scene7&quot; 모드에서 실행되는 다이내믹 미디어에만 적용됩니다.
...
1. 비디오의 속성 페이지에서 **[!UICONTROL 축소판 변경을 누릅니다.]**
1. 축소판 변경 페이지에서 다음 중 하나를 수행합니다.

   * 비디오의 프레임을 새로운 축소판으로 사용하려면:

      * 도구 모음에서 **[!UICONTROL 비디오의 프레임 선택을 누릅니다.]**
      * 재생 버튼을 누른 다음 비디오의 새로운 축소판으로 캡처할 프레임에 있는 일시 중지 버튼을 누릅니다.
   * 이미지 자산을 새 축소판으로 사용하려면 다음을 수행하십시오.

      * 도구 모음에서 **[!UICONTROL 자산에서 축소판 선택을 누릅니다.]**
      * **[!UICONTROL 축소판 선택을 누릅니다.]**
      * 사용할 이전에 업로드되고 게시된 이미지 자산으로 이동합니다. 비디오의 축소판 이미지로 사용할 수 있도록 에셋 크기가 자동으로 조정됩니다.
      * 이미지 자산을 선택한 다음 **[!UICONTROL 선택을 누릅니다.]**


1. 축소판 변경 페이지에서 **[!UICONTROL 변경 내용 저장을 탭합니다.]**
1. 비디오의 속성 페이지의 오른쪽 맨 위에서 **[!UICONTROL 저장 및 닫기를 누릅니다.]**

## 다이내믹 미디어의 비디오 축소판 정보 - 하이브리드 모드 {#about-video-thumbnails-in-dynamic-media-hybrid-mode}

Dynamic Media에서 자동으로 생성된 10개의 축소판 이미지 중 하나를 선택하여 비디오에 추가할 수 있습니다. 비디오 플레이어는 AEM Sites, AEM Mobile 또는 AEM Screens의 작성 환경에서 비디오 에셋이 Dynamic Media 구성 요소와 함께 사용될 때 선택한 축소판을 표시합니다. 이 축소판은 전체 비디오의 내용을 가장 잘 나타내는 정적 사진 역할을 하며 사용자는 [재생] 단추를 클릭하도록 합니다.
...
[Dynamic Media의 비디오 축소판 정보 - Scene7 모드](/help/assets/video.md#about-video-thumbnails-in-dynamic-media-scene-mode)를 참조하십시오.

### 비디오 축소판 {#adding-a-video-thumbnail} 추가

이러한 단계는 하이브리드 모드에서 실행되는 다이내믹 미디어에만 적용됩니다.
...
1. 비디오의 속성 페이지에서 **[!UICONTROL 축소판 변경을 누릅니다.]**
1. 축소판 변경 페이지의 도구 모음에서 **[!UICONTROL 프레임 선택을 누릅니다.]**

   Dynamic Media는 사용자가 지정한 기본 시간 간격 또는 시간 간격에 따라 비디오에서 일련의 축소판 이미지를 생성합니다.

1. 생성된 축소판 이미지를 미리 본 다음 비디오에 추가할 이미지를 선택합니다.
1. **[!UICONTROL 변경 내용 저장.]**&#x200B;을 탭합니다.

   선택한 축소판을 사용하도록 비디오의 축소판 이미지가 업데이트됩니다. 나중에 축소판 이미지를 변경하려는 경우 **[!UICONTROL 축소판 변경]** 페이지로 돌아가서 새 이미지를 선택할 수 있습니다.
...
1. AEM에서 **[!UICONTROL 도구]** > **[!UICONTROL 일반]** > **[!UICONTROL CRXDE Lite을 누릅니다.]**
...
1. **[!UICONTROL OK]**&#x200B;을 눌러 속성 탭으로 돌아갑니다.
1. CRXDE Lite 페이지의 왼쪽 위 모서리 근처에 있는 **[!UICONTROL 모두 저장]**&#x200B;을 탭한 다음 왼쪽 위 모서리의 뒤로 홈 아이콘을 눌러 AEM으로 돌아갑니다.

   [비디오 축소판 추가를 참조하십시오.](#adding-a-video-thumbnail)

### 사용자 정의 비디오 축소판 {#adding-a-custom-video-thumbnail-1} 추가
...
1. 도구 모음에서 **[!UICONTROL 속성 보기]** 아이콘(&quot;i&quot;가 포함된 원)을 누릅니다.
1. 비디오의 속성 페이지에서 **[!UICONTROL 축소판 변경을 누릅니다.]**
1. 축소판 변경 페이지의 도구 모음에서 **[!UICONTROL 새 축소판 업로드를 누릅니다.]**
1. 사용할 축소판 이미지로 이동하고 선택한 다음 **[!UICONTROL 열기]**&#x200B;를 눌러 AEM에 이미지를 업로드합니다. 업로드에 따라 이미지를 게시해야 합니다.
