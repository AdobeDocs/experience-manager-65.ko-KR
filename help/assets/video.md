...
1. Experience Manager의 왼쪽 상단 모서리에서 Experience Manager 로고를 선택한 다음 왼쪽 레일에서 을 선택합니다. **[!UICONTROL 도구]** (망치 아이콘) > **[!UICONTROL 에셋]** > **[!UICONTROL 비디오 보고서]**.
1. 비디오 보고서 페이지에서 다음 중 하나를 수행합니다.

   * 오른쪽 상단 모서리 근처에서 **비디오 보고서 새로 고침** 아이콘.
보고서 종료 날짜가 현재 날짜인 경우에만 새로 고침 을 사용합니다. 이렇게 하면 보고서를 마지막으로 실행한 이후 발생한 비디오 추적이 표시됩니다.

   * 오른쪽 상단 모서리 근처에서 **날짜 선택** 아이콘.
...
1. 포함 코드에서 다음을 사용하여 줄을 찾습니다.
   ...
   config2 매개 변수에 대한 올바른 값이 **[!UICONTROL 포함 코드]** 및 사본 **[!UICONTROL URL]** 함수. 사본의 URL에서 **[!UICONTROL URL]** command, 찾을 매개 변수는 다음과 같습니다. `&config2=<value>` . The value is almost always `companypreset`, but in some instances it can also be `companypreset-1`, `companypreset-2`, and so forth.
...
![비디오에 대해 업로드된 .VTT 캡션 파일 및 업로드된 .MP3 오디오 트랙 파일을 보여 주는 표와 함께 Dynamic Media의 캡션 및 오디오 트랙 탭입니다.](assets-dm/msma-subtitle-audiotracks-tab2.png)
...
1. (선택 사항) 비디오에 캡션 파일을 하나 이상 추가하려면 다음을 수행합니다.
   ...
   * 미디어 플레이어에 표시할 캡션의 경우 *필수* 다음에 대한 필수 세부 정보(메타데이터) 추가 *각각* 업로드한 캡션 파일입니다. 캡션 파일 이름 오른쪽에 있는 연필 아이콘을 선택합니다. 다음에서 **캡션 편집** 대화 상자에서 파일에 대한 다음과 같은 필수 세부 정보를 입력한 다음 을 선택합니다 **[!UICONTROL 저장]**. 업로드한 각 캡션 파일에 대해 이 프로세스를 반복합니다.
     ...
     | 레이블 | 에서 캡션 이름에 대해 표시되는 텍스트입니다. **[!UICONTROL 오디오 또는 자막 선택]** 미디어 플레이어의 팝업 목록입니다. 레이블은 자막 또는 캡션 트랙에 해당하는 것으로, 고객에게 표시됩니다. 예: `English (CC)` |
...
1. (선택 사항) 비디오에 하나 이상의 오디오 트랙을 추가하려면 다음을 수행하십시오.
   ...
   * 오디오 트랙이 **[!UICONTROL 오디오 또는 캡션 선택]** 미디어 플레이어의 팝업 목록, *필수* 에 대한 필수 세부 정보 추가 *각각* 추가한 오디오 트랙 파일입니다. 오디오 트랙 파일 이름 오른쪽에 있는 연필 아이콘을 선택합니다. 다음에서 **오디오 트랙 편집** 대화 상자에서 다음과 같은 필수 세부 정보를 입력한 다음 을 선택합니다 **[!UICONTROL 저장]**. 업로드한 각 오디오 트랙 파일에 대해 이 프로세스를 반복합니다.
     ...
     | 레이블 | 오디오 트랙의 이름으로 표시되는 텍스트입니다 **[!UICONTROL 오디오 또는 자막 선택]** 미디어 플레이어의 팝업 목록입니다. 레이블은 오디오 트랙에 해당하는 것으로, 고객에게 표시됩니다. 예, `English [Original]`. 비디오에 첨부된 오디오의 레이블이으로 설정됩니다. `[Original]` 기본적으로. |
...
>[!CAUTION]
>
>게시 또는 게시 취소된 비디오에 캡션 파일 또는 오디오 파일을 업로드할 때 다음과 같은 경우 파일이 삭제됩니다. [*재처리*](/help/assets/processing-profiles.md#reprocessing-assets) 비디오입니다. 비디오의 원본 오디오만 그대로 유지됩니다. 이러한 경우 캡션 파일 및 오디오 트랙 파일을 비디오에 다시 업로드해야 합니다.
...
![캡션 및 오디오 트랙 필드에 강조 표시된 상태 열입니다.](assets-dm/msma-lifecycle-status2.png)*업로드된 각 캡션 및 오디오 추적 파일의 라이프사이클 상태입니다.*
...
다음에서 **기본값으로 설정** 대화 상자에서 **[!UICONTROL 바꾸기]**.
   ...
   ![선택한 오디오 트랙 파일 이름이 있고 &quot;기본값으로 설정&quot; 버튼이 강조 표시된 오디오 트랙 제목입니다.](assets-dm/msma-defaultaudiotrack2.png)*비디오에 대한 기본 오디오 트랙 설정.*
...
1. 오른쪽 하단 모서리 근처에서 볼륨 아이콘 왼쪽에 있는 말풍선 아이콘을 선택한 다음 들으려는 오디오나 캡션을 선택하거나 또는 둘 다 표시합니다. 원하는 경우 캡션에서 다음을 선택할 수 있습니다. **[!UICONTROL 끔]** 캡션을 표시하지 않습니다.

   ![비디오 뷰어의 오디오 및 캡션 팝업 목록입니다.](assets-dm/msma-selectaudiosubtitle.png)*비디오 재생을 위해 오디오 및 캡션을 선택하는 사용자 시뮬레이션.*
...
**비디오에 폐쇄 캡션을 추가하려면 다음 작업을 수행하십시오.**
...
1. 서드파티 애플리케이션 또는 서비스를 사용하여 비디오 캡션 파일을 만듭니다.
   ...
   사이트의 화면 지침을 따라 WebVTT 파일을 작성하고 저장합니다. 완료되면 캡션 파일 내용을 복사하여 일반 텍스트 편집기에 붙여 넣은 다음 `.vtt` 파일 이름 확장명.
   ...
   <table>
     <tbody>
      <tr>
       ...
       <li>드롭다운 메뉴에서 다음을 클릭합니다. <strong>뷰어</strong>.</li>
...
1. 비디오의 속성 페이지에서 을 선택합니다. **[!UICONTROL 썸네일 변경]**.
1. 썸네일 변경 페이지의 도구 모음에서 를 선택합니다 **[!UICONTROL 프레임 선택]**.
...
1. 오른쪽 아래 패널의 속성 탭에서 을 두 번 선택합니다. `thumbnailtime`.
...
1. 선택 **[!UICONTROL 확인]** 속성 탭으로 돌아갑니다.
1. CRXDE Lite 페이지의 왼쪽 상단 모서리 근처에서 을 선택합니다. **[!UICONTROL 모두 저장]**&#x200B;을 클릭한 다음 왼쪽 상단의 Back Home 아이콘을 선택하여 Experience Manager 로 돌아갑니다.
...
1. 도구 모음에서 **[!UICONTROL 속성 보기]** 아이콘(안에 &quot;i&quot;가 있는 원)
1. 비디오의 속성 페이지에서 을 선택합니다. **[!UICONTROL 썸네일 변경]**.
1. 썸네일 변경 페이지의 도구 모음에서 를 선택합니다 **[!UICONTROL 새 썸네일 업로드]**.
1. 사용할 썸네일 이미지로 이동하여 선택한 다음 를 선택합니다 **[!UICONTROL 열기]** Experience Manager에 이미지 업로드를 시작합니다. 업로드 후 이미지를 게시하십시오.
1. 이미지를 성공적으로 업로드하고 게시한 후 [썸네일 변경] 페이지에서 을 선택합니다. **[!UICONTROL 변경 내용 저장]**.
...
| `onlyIfPublished` | 선택 사항입니다. 전달된 경우 `manifestUrl` 비디오가 게시된 경우에만 가 반환됩니다. |
