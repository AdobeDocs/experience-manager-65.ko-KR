...
>[!NOTE]
>
>Dynamic Media에서 비디오를 사용하여 작업하기 전에 AEM 관리자가 다이내믹 미디어 - Scene7 모드 또는 다이내믹 미디어 - 하이브리드 모드에서 Dynamic Media Cloud 서비스를 이미 사용하고 구성했는지 확인하십시오.
>
...
>[!NOTE]
>
>다이내믹 미디어 - Scene7 모드에서 실행될 때는 즉시 게시가 YouTube에 자동으로 게시되지 않습니다. 다이내믹 미디어 - Scene7 모드가 설정된 경우 두 가지 게시 옵션을 선택할 수 있습니다. **[!UICONTROL 즉시]** 또는 활성화 **[!UICONTROL 시]**.
>
...
AEM의 모든 비디오에는 연결된 축소판이 있어야 합니다. 축소판은 교체하지 않으면 삭제할 수 없습니다. 기본적으로 비디오를 AEM에 업로드할 때 첫 번째 프레임이 축소판으로 사용됩니다. 하지만 예를 들어 브랜딩 또는 시각적 검색을 위해 축소판을 사용자 정의할 수 있습니다. 비디오 축소판을 사용자 정의할 때 사용할 프레임에서 비디오를 재생하고 일시 중지하거나 디지털 자산 관리자에서 이미 업로드하여 *게시한* 이미지 자산을 선택할 수 있습니다.
...
Dynamic Media에서 자동으로 생성된 10개의 축소판 이미지 중 하나를 선택하여 비디오에 추가할 수 있습니다. 비디오 플레이어는 AEM Sites, AEM Mobile 또는 AEM Screens의 작성 환경에서 Dynamic Media 구성 요소와 함께 비디오 자산을 사용할 때 선택한 축소판을 표시합니다. 이 축소판은 전체 비디오의 내용을 가장 잘 나타내는 정적 사진 역할을 하며 사용자는 [재생] 단추를 클릭하도록 합니다.
...
다이내믹 미디어 [- Scene7 모드의 비디오 축소판 정보도 참조하십시오](/help/assets/video.md#about-video-thumbnails-in-dynamic-media-scene-mode).
...
1. CRXDE Lite 페이지의 왼쪽 위 모서리 근처에 있는 **[!UICONTROL 모두]**&#x200B;저장을 탭한 다음 왼쪽 위 모서리의 뒤로 홈 아이콘을 탭하여 AEM으로 돌아갑니다.
