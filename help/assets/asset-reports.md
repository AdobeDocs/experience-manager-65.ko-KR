...
1. 보고서가 저장된 CRX 저장소에서 제목, 설명, 썸네일 및 폴더 경로 등 보고서 세부 정보를 구성합니다. 기본적으로 폴더 경로는 `/content/dam`입니다. 다른 경로를 지정할 수 있습니다.
   ...
   **[!UICONTROL 링크 공유]** 보고서에 [!DNL Assets] 내에서 외부 사용자와 공유되는 자산의 URL이 표시됩니다. It includes email ids of the user who shared the assets, emails ids of users with which the assets are shared, share date, and expiration date for the link. The columns are not customizable.
