...
# 메타데이터 기능의 구성 및 관리 [!DNL Assets] {#config-metadata}
...
## 일괄 메타데이터 업데이트에 대한 제한 구성 {#bulk-metadata-update-limit}

서비스 거부(DOS)와 같은 상황을 방지하기 위해 Sling 요청에서 지원되는 매개 변수의 수를 [!DNL Enterprise Manager] 제한합니다. 한 번에 많은 자산의 메타데이터를 업데이트할 때 한도에 도달해도 메타데이터가 더 많은 자산에 대해 업데이트되지 않습니다. Enterprise Manager는 로그에 다음 경고를 생성합니다.

`org.apache.sling.engine.impl.parameters.Util Too many name/value pairs, stopped processing after 10000 entries`

제한을 변경하려면 [ **[!UICONTROL 도구] > [작업]** ] > [ **[!UICONTROL 웹 콘솔]** ] **[!UICONTROL 에 액세스한 다음 [최대 POST 매개 변수]]** 값을 변경하고 Apache Apache **** **** Sling 요청 매개 변수 처리OSGi 구성에서 최대 매개 변수 값을 변경합니다.
...
1. 예를 들어 프로필의 제목을 입력하고 만들기 `Sample Metadata`를 **[!UICONTROL 클릭합니다]**. 메타데이터 [!UICONTROL 프로필에] 대한 양식 편집이 표시됩니다.
...
1. 구성 요소를 클릭하고 설정 **[!UICONTROL 탭에서 해당 속성을]** 구성합니다. 예를 들어 설명 구성 **[!UICONTROL 요소를]** 클릭하고 해당 속성을 편집합니다.
   ...
   * **[!UICONTROL 속성에 매핑]**:이 속성의 값은 저장소에 저장된 자산 노드에 대한 상대 경로 또는 이름을 제공합니다. 이 값은 경로가 자산의 노드 아래에 있음을 `./` 나타내므로 항상 다음으로 시작해야 합니다.

   ![메타데이터 프로필의 속성 설정에 매핑](assets/metadata-profile-setting-map-property.png)

   속성에 **[!UICONTROL 매핑에 지정하는]** 값은 자산의 메타데이터 노드 아래에 속성으로 저장됩니다. 예를 들어 속성 `./jcr:content/metadata/dc:desc` 에 **[!UICONTROL 맵]**&#x200B;의 이름으로 지정하는 경우 [!DNL Assets] 이 값을 자산의 메타데이터 노드 `dc:desc` 에 저장합니다.
...
* 6.5로 업그레이드하기 전에 기존 메타데이터 프로필이 있을 수 있습니다. 업그레이드 후 [ [!DNL Experience Manager] 메타데이터 프로필 ] 탭의 [!UICONTROL 속성] 폴더에 이러한 프로필을 적용하면 메타데이터 양식 필드가 표시되지 않습니다. 그러나 새로 만든 메타데이터 프로필을 적용하면 양식 필드가 표시되지만 예상대로 사용할 수 없습니다. 기능 손실은 없지만 (사용할 수 없음) 양식 필드를 보려면 기존 메타데이터 프로필을 편집하고 저장합니다.
