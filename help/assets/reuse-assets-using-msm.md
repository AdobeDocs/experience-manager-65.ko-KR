...
[!DNL Adobe Experience Manager]의 MSM(Multi Site Manager) 기능을 사용하면 한 번 작성되고 여러 웹 위치에서 다시 사용되는 컨텐츠를 재사용할 수 있습니다. [!DNL Assets] 기능에 대해 MSM과 동일한 디지털 자산에 사용할 수 있습니다. [!DNL Assets]에 MSM을 사용하면 다음을 수행할 수 있습니다.
...
### MSM의 용어 [!DNL Assets] 용어 {#glossary}

**소스:** 원래 자산 또는 폴더입니다. Live Copy가 파생되는 기본 복사본입니다.

**Live Copy:** 소스 자산/폴더의 복사본입니다. Live Copy는 추가 Live Copy의 소스가 될 수 있습니다. LC를 만드는 방법을 확인하십시오.
...
**롤아웃 구성:** 동기화할 속성, 방법 및 시기를 결정하는 규칙입니다. 이러한 구성은 Live Copy를 만들 때 적용됩니다.나중에 편집할 수 있습니다.하위 요소는 상위 자산에서 롤아웃 구성을 상속할 수 있습니다. [!DNL Assets]에 대한 MSM의 경우 표준 롤아웃 구성만 사용하십시오. 다른 롤아웃 구성은 [!DNL Assets]에 대해 MSM에 사용할 수 없습니다.

**동기화:** 출시에 추가하여 소스 및 Live Copy 간에 업데이트를 소스에서 Live Copy로 전송하여 패리티 기능을 제공하는 다른 작업을 수행할 수 있습니다. 특정 Live Copy에 대한 동기화가 시작되어 소스에서 변경 내용을 가져옵니다. 이 작업을 사용하면 Live Copy 중 하나만 업데이트할 수 있습니다. 동기화 작업을 참조하십시오.

**일시 중단:** Live Copy와 소스 에셋/폴더 사이의 라이브 관계를 일시적으로 제거합니다. 관계를 다시 시작할 수 있습니다. 일시 중단 작업을 참조하십시오.

**다시 시작:** Live Copy가 소스로부터 업데이트를 다시 받기 시작하도록 라이브 관계를 다시 시작합니다. 다시 시작 동작을 참조하십시오.
...
1. 소스 자산 또는 폴더를 선택합니다. 도구 모음에서 **[!UICONTROL 만들기]** > **[!UICONTROL Live Copy]**&#x200B;를 클릭합니다.
   ...
   *그림:인터페이스에서 Live Copy  [!DNL Experience Manager] 만들기*
...
## 소스 및 Live Copy {#properties}의 다양한 속성과 상태 보기
...
* 소스 폴더를 선택하고 [!UICONTROL Live Copy 콘솔]에서 각 Live Copy에 대한 세부 정보를 찾습니다.

>[!TIP]
>
>몇 개의 개별 Live Copy의 상태를 확인하려면 속성 페이지에 있는 첫 번째 방법을 사용합니다. 많은 Live Copy의 상태를 확인하려면 두 번째 방법을 사용하십시오. 즉, **[!UICONTROL 관계 상태]** 페이지를 참조하십시오.
...
### 폴더 {#statuslcfolder}의 모든 Live Copy에 대한 정보 및 상태

[!DNL Experience Manager] 소스 폴더의 모든 Live Copy의 동체를 확인할 수 있는 콘솔을 제공합니다. 이 콘솔에는 모든 하위 자산의 상태가 표시됩니다.
...
소스 에셋 또는 폴더를 선택하고 왼쪽 레일을 열고 **[!UICONTROL 참조]**&#x200B;를 클릭합니다. 또는 에셋 또는 폴더를 선택하고 키보드 단축키 `Alt + 4`을 사용합니다.
...
Live Copy 자산 또는 폴더를 선택하고 왼쪽 레일을 연 다음 **[!UICONTROL 참조]**&#x200B;를 클릭합니다. 또는 에셋 또는 폴더를 선택하고 키보드 단축키 `Alt + 4`을 사용합니다.
...
또는 특정 Live Copy를 선택한 후 참조 레일에서 롤아웃 작업을 시작할 수 있습니다. 자세한 내용은 Live Copy[에 대한 참조 레일의 빠른 작업을 참조하십시오. ](#refraillc) 이 롤아웃 방식에서는 선택한 Live Copy 및 선택적으로 해당 하위 복사본만 업데이트됩니다.
...
|---|---|---|
|  참조레일 | 소스를 이미 선택한 경우 빠르게 동기화합니다. | 소스[에 대한 참조 레일의 빠른 작업 참조](#refrailsource) |
| [!UICONTROL 속성] 페이지의 도구 모음 | Live Copy 속성이 이미 열려 있으면 동기화를 시작합니다. | [Live Copy 동기화](#synclc)를 참조하십시오. |
| [!UICONTROL Live Copy ] Overviewconsole | 소스 폴더를 선택하거나 [!UICONTROL Live Copy 개요] 콘솔이 이미 열려 있을 때 여러 자산을 신속하게 동기화할 수 있습니다(반드시 모든 것이 아닙니다). 한 번에 하나의 자산에 대해 동기화 작업이 시작되지만 한 번에 여러 자산에 대해 보다 빠르게 동기화할 수 있습니다. | Live Copy 폴더[의 많은 자산에 대한 작업을 참조하십시오.](#bulkactions) |

### Live Copy 동기화 {#synclc}

동기화 작업을 시작하려면 Live Copy의 **[!UICONTROL 속성]** 페이지를 열고 **[!UICONTROL Live Copy]**&#x200B;를 클릭한 다음 도구 모음에서 원하는 작업을 클릭합니다.

동기화 작업과 관련된 상태 및 정보를 보려면 [Live Copy](#statuslcasset) 및 [폴더의 모든 Live Copy의 정보 및 상태](#statuslcfolder)를 참조하십시오.
...
>[!NOTE]
>
>관계가 일시 중단된 경우, 동기화 작업은 도구 모음에서 사용할 수 없습니다. 동기화 작업을 참조 레일에서 사용할 수 있지만, 성공적으로 롤아웃되더라도 수정 내용이 전파되지 않습니다.
...
또는 **[!UICONTROL Live Copy 개요]** 콘솔에서 Live Copy 폴더에 있는 여러 자산의 관계를 빠르게 일시 중단하거나 다시 시작할 수 있습니다. [Live copy 폴더](#bulkactions)의 많은 자산에 대해 작업을 수행합니다.

## Live Copy {#localmods}에 대한 로컬 수정
...
분리 작업을 사용하여 소스와 Live Copy 간의 관계를 완전히 제거할 수 있습니다. Live Copy가 분리된 후 독립형 에셋 또는 폴더가 됩니다. 분리 후 즉시 [!DNL Experience Manager] 인터페이스에 새 자산으로 표시됩니다. Live Copy를 소스에서 분리하려면 다음 단계를 따르십시오.
...
또는 **[!UICONTROL Live Copy 개요]** 콘솔에서 Live Copy 폴더의 여러 자산을 신속하게 분리할 수 있습니다. [Live copy 폴더](#bulkactions)의 많은 자산에 대해 작업을 수행합니다.

## Live Copy 폴더 {#bulkactions}의 많은 자산에 대해 작업 수행

하나의 Live Copy 폴더에 여러 개의 자산이 있는 경우 각 자산에 대해 작업을 시작하는 것은 지루할 수 있습니다. [!UICONTROL Live Copy 콘솔]에서 많은 자산에 대한 기본 작업을 빠르게 시작할 수 있습니다. 위의 방법은 개별 자산에 대해 계속 작동합니다.

1. 소스 폴더를 선택합니다. 도구 모음에서 **[!UICONTROL 속성]**&#x200B;을 클릭합니다. 또는 키보드 단축키 `p`를 사용합니다.
1. **[!UICONTROL Live Copy 소스]**&#x200B;를 클릭합니다. 콘솔을 열려면 **[!UICONTROL Live Copy 개요]**&#x200B;를 클릭합니다.
1. 이 대시보드에서 live copy 폴더에서 Live Copy 자산을 선택합니다. 도구 모음에서 원하는 작업을 클릭합니다. 사용 가능한 작업은 **[!UICONTROL 동기화]**, **[!UICONTROL 재설정]**, **[!UICONTROL 일시 중단]** 및 **[!UICONTROL 분리]**&#x200B;입니다. 선택한 소스 폴더와 라이브 관계에 있는 모든 Live Copy 폴더의 자산에 대해 이러한 작업을 신속하게 시작할 수 있습니다.

   ![Live Copy 개요 콘솔에서 Live Copy 폴더의 많은 에셋을 손쉽게 업데이트](assets/livecopyconsole_update_many_assets.png)

   *그림:Live Copy  [!UICONTROL Overview 콘솔에서 Live Copy 폴더의 많은 에셋을 손쉽게 ] 업데이트할 수 있습니다.*

## [!DNL Assets] {#extendapi}에 대한 MSM 확장

[!DNL Experience Manager] MSM Java API를 사용하여 기능을 확장할 수 있습니다. [!DNL Assets]의 경우, 확장은 [!DNL Sites]에 대해 MSM과 작동하는 것과 동일하게 작동합니다. 특정 작업에 대한 자세한 내용은 [MSM](/help/sites-developing/extending-msm.md) 확장 및 다음을 참조하십시오.
...
>[!NOTE]
>
>* [!DNL Sites]에 대한 MSM의 블루프린트는 [!DNL Assets]에 대해 MSM의 Live Copy 소스라고 합니다.
>* 사이트 만들기 마법사에서 장 단계를 제거하는 것은 [!DNL Assets]에 대한 MSM에서 지원되지 않습니다.
>* 페이지 속성(터치 활성화 UI)에서 MSM 잠금 구성은 [!DNL Assets]에 대한 MSM에서 지원되지 않습니다.
...
* Live Copy 자산에 대해 편집 작업이 작동하지 않습니다. Live Copy의 소스가 Live Copy 자체인 경우 편집 작업은 해당 Live Copy에 대해 작동하지 않습니다.
...
더 많은 시나리오에서 [!DNL Assets]에 대한 MSM은 사이트에 대한 MSM 기능과 일치합니다. 주목할 만한 주요 차이점:

* [!DNL Sites]에 대한 MSM의 블루프린트는 [!DNL Assets]에 대해 MSM의 Live Copy 소스라고 합니다.
* 사이트에서 블루프린트와 Live Copy를 비교할 수는 있지만 [!DNL Assets]에서는 소스를 Live Copy와 비교할 수 없습니다.
* [!DNL Assets]에서는 Live Copy를 편집할 수 없습니다.
* 사이트에는 대개 하위 항목이 있지만 [!DNL Assets]은 그렇지 않습니다. 개별 자산의 Live Copy를 만들 때는 하위 항목을 포함하거나 제외하는 옵션이 없습니다.
* 사이트 만들기 마법사에서 장 단계를 제거하는 것은 [!DNL Assets]에 대한 MSM에서 지원되지 않습니다.
* 페이지 속성에서 MSM 잠금 구성(터치 활성화 UI)은 [!DNL Assets]에 대한 MSM에서 지원되지 않습니다.
...
## [!DNL Assets] {#limitations}에 대한 MSM의 제한 및 알려진 문제
