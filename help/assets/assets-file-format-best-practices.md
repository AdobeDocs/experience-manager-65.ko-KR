...
description: '[!DNL Experience Manager Assets]를 사용하여 지원되는 다양한 파일 유형을 처리하는 모범 사례'
contentOwner: AG
...
[!DNL Adobe Experience Manager Assets] 사용자의 다양한 파일 지원 요구 사항을 충족하기 위해 많은 독점적 및 타사 파일 포맷 라이브러리를 지원합니다. 지원되는 Adobe 라이브러리에는 [!DNL Adobe Camera Raw]깁슨, Adobe PDF 래스터라이저 등이 [!DNL Adobe InDesign Server]있습니다. 또한 [!DNL Experience Manager Assets] 타사 라이브러리를 [!DNL ImageMagick]비롯하여 [!DNL TwelveMonkeys]다양한 서비스를 지원합니다.
...
[!DNL Adobe Camera Raw] 라이브러리는 CMYK 색상 프로파일을 입력으로 지원합니다. 그러나 RGB 색상 공간에서 출력을 생성하고 JPEG 포맷에서만 출력을 지원합니다. 소스 파일 색상 공간(예: CMYK)은 축소판에 유지되지 않습니다.
...
[!DNL Dynamic Media] 글로벌, 확장 및 성능 최적화 네트워크를 통해 다양한 유형의 리치 컨텐츠를 실시간으로 생성하고 전달할 수 있습니다. 인터랙티브한 보기 경험을 제공하고 디지털 캠페인 관리 프로세스를 간소화합니다. 활성화에 대한 자세한 [!DNL Dynamic Media]내용은 다이내믹 미디어 [구성을 참조하십시오](/help/assets/config-dynamic.md).

현재, 파일당 최대 20GB의 컨텐츠를 지원할 [!DNL Dynamic Media] 수 있습니다.
...
에서 [!DNL ImageMagick] 라이브러리를 설정하는 방법을 알아보려면 ImageMagick 사용을 [!DNL Experience Manager]참조하십시오 [](/help/assets/media-handlers.md#an-example-using-imagemagick). 최적 사용을 보려면 ImageMagick [구성을 위한 우수 사례를 참조하십시오](/help/assets/best-practices-for-imagemagick.md).
