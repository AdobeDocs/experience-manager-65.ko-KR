...
1. **만들기**&#x200B;를 사용하여 마법사를 엽니다.
   ...
   >[!CAUTION]
   >
   >다음과 같은 경우 [콘텐츠 조각 모델 사용이 활성화되지 않았습니다.](/help/assets/content-fragments/content-fragments-configuration-browser.md), **만들기** 옵션을 사용할 수 없습니다.

1. **모델 제목**&#x200B;을 지정합니다. 다음을 추가할 수도 있습니다. **태그**, a **설명**, 및 선택 **모델 활성화** 끝 [모델 활성화](#enabling-disabling-a-content-fragment-model) 필요한 경우.
...
1. **필드를 추가하려면**
   ...
   * 모델에 필드가 추가되면 오른쪽 패널에 그 특정 데이터 유형에 대해 정의할 수 있는 **속성**&#x200B;이 표시됩니다. 여기에서 해당 필드에 필요한 사항을 정의할 수 있습니다.
      ...
      * **필드 레이블**&#x200B;을 입력하면 **속성 이름**&#x200B;이 자동으로 채워집니다. 비어 있는 경우 이후에 수동으로 업데이트할 수 있습니다.
        ...
        >[!CAUTION]
        >
        >속성을 수동으로 업데이트할 때 **속성 이름** 데이터 형식의 경우 이름에는 A-Z, a-z, 0-9 및 밑줄(_)만 포함해야 합니다.
        >
...
* **속성 이름**
  ...
  데이터 형식에 대해 이 속성을 수동으로 업데이트할 때 **필수** contain *전용* A-Z, a-z, 0-9 및 밑줄(_)을 특수 문자로 사용할 수 있습니다.
...
* 데이터 유형 **여러 줄 텍스트**&#x200B;의 경우 **기본 유형**&#x200B;을 다음 중 하나로 정의할 수 있습니다.
  ...
  변경 **기본 유형** 콘텐츠 조각 모델의 경우 해당 조각을 편집기에서 열고 저장한 후에 기존 관련 콘텐츠 조각에만 적용됩니다.
...
* **[조각 참조](#fragment-reference-nested-fragments)**(중첩된 조각)
   ...
   * 구조화된 데이터를 포함/검색할 수 있습니다.
     ...
     >[!NOTE]
     >
     >이 방법은 특히 다음 경우에 유용합니다. [GraphQL에서 콘텐츠 조각을 사용하여 Headless 콘텐츠 게재](/help/assets/content-fragments/content-fragments-graphql.md).
   * 최종 조각에서 하나의 참조 또는 여러 참조에 대해 구성할 수 있습니다.
...
>[!NOTE]
>
>이는 특히 다음의 경우에 관심이 있다. [GraphQL에서 콘텐츠 조각을 사용하여 Headless 콘텐츠 게재](/help/assets/content-fragments/content-fragments-graphql.md).
...
콘텐츠 거버넌스를 구현하기 위해 다음을 구성할 수 있습니다 **정책** 에셋 폴더에서 해당 폴더에서 조각 생성에 허용되는 콘텐츠 조각 모델을 제어합니다.
