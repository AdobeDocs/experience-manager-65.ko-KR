...
또한 을 사용하여 기본 구성에서 사전 정의된 세트 또는 게시자에서 테스트를 제거할 수도 있습니다. `exclude` 매개 변수. 또한 테스트 C가 아닌 테스트 세트 이름과 실제 이름도 지정해야 합니다 `lass` name). 테스트 이름은 `name` 테스트 클래스의 속성입니다. 아래 예에서는 `CreatePageTreeTest` (명명된) `UploadAsset`) 테스트를 toughday suite에서 제거합니다.
