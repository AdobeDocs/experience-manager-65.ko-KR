...
* **`RolloutConfig`** 트리거할 때 사용할 목록 `RolloutConfig` `LiveActions`이 들어 있습니다. 는 `LiveCopy` 상속을 `RolloutConfig` 상속하고 결과는 에 있습니다 `LiveRelationship`.
...
* 작업을 수행하는 [ 인터페이스의 `com.day.cq.wcm.msm.api.LiveAction`](https://helpx.adobe.com/experience-manager/6-5/sites/developing/using/reference-materials/javadoc/com/day/cq/wcm/msm/api/LiveAction.html) 구현입니다.
...
* Live Copy의 소스를 나타내는 [ `Resource`](https://helpx.adobe.com/experience-manager/6-5/sites/developing/using/reference-materials/javadoc/org/apache/sling/api/resource/Resource.html) 개체입니다.
* Live Copy의 대상을 나타내는 `Resource` 개체입니다.
* Live Copy [ 의 `LiveRelationship`](https://helpx.adobe.com/experience-manager/6-5/sites/developing/using/reference-materials/javadoc/com/day/cq/wcm/msm/api/LiveRelationship.html) 개체입니다.
...
>[!NOTE]
>
>인수 `Resource` 는 개체 `null` 와 같은 개체 `Resources` 에 적응하지 않는 `Node` 개체일 수 [ `NonExistingResource`](https://helpx.adobe.com/experience-manager/6-5/sites/developing/using/reference-materials/javadoc/org/apache/sling/api/resource/NonExistingResource.html) 있습니다.
...
1. Open CRXDE Lite; 예: https://localhost:4502/crx/de [](https://localhost:4502/crx/de).
...
1. 이전 단계를 반복하여 `/apps/wcm/core` 폴더 트리를 만듭니다. 라는 형식 노드 `sling:Folder` 를 `core` 만듭니다 `resources`. <!-- ![chlimage_1-77](assets/chlimage_1-77.png) -->
