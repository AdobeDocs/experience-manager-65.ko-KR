...
>* 엔드 투 엔드 커뮤니티 문서에 대해서는 [Adobe Experience Manager 워크플로우를 사용하여 디지털 자산 수정.](https://experienceleague.adobe.com/docs/experience-manager-65/assets/using/assets-workflow.html?lang=ko)
>* 자세한 내용은 [워크플로우에 대한 AEM 전문가 웨비나에 문의하십시오](https://communities.adobeconnect.com/p5s33iburd54/).
>* 정보 위치 변경 내용은 [AEM 6.5의 저장소 구조 변경](/help/sites-deploying/repository-restructuring.md) 및 [워크플로우 우수 사례 - 위치](/help/sites-developing/workflows-best-practices.md#locations).
>
...
워크플로우 모델의 버전이 지정됩니다. 워크플로우 인스턴스를 실행하면 워크플로우가 시작될 때 사용할 수 있는 대로 워크플로우의 런타임 모델을 사용하고 유지합니다.
...
발생하는 워크플로우 모델 또는 생성된 런타임 모델 또는 둘 다에 대한 편집 *after* 특정 인스턴스가 시작되었지만 해당 인스턴스에 적용되지 않습니다.

>[!CAUTION]
>
>수행되는 단계는 [런타임 모델](/help/sites-developing/workflows-models.md#sync-your-workflow-generate-a-runtime-model)생성된 **동기화** 작업이 워크플로우 모델 편집기에서 트리거됩니다.
>
>이 시점 이후에 워크플로우 모델이 변경되는 경우(없이) **동기화** 트리거되는) 런타임 인스턴스에서 이러한 변경 사항을 반영하지 않습니다. 업데이트 후에 생성된 런타임 모델만 변경 사항을 반영합니다. 예외는 기본 ECMA 스크립트입니다. 스크립트는 한 번만 유지되므로 이러한 변경 사항이 적용됩니다.
...
페이로드 구현은 저장소의 리소스(경로, UUID 또는 URL별)나 직렬화된 Java™ 개체를 참조합니다. 저장소에서 리소스를 참조하는 것은 유연하며 sling 생산적입니다. 예를 들어 참조된 노드를 양식으로 렌더링할 수 있습니다.
...
* **기본값**
   ...
   이러한 유형은 표준 AEM 인스턴스에 포함된 기본 제공 워크플로우입니다.

* 사용자 지정 워크플로우(콘솔에 표시기 없음)

   이러한 워크플로우는 사용자 지정으로 오버레이된 기본 제공 워크플로우나 새로운 워크플로우로 만들었습니다.

* **기존**

   이전 버전의 AEM에서 만든 워크플로우입니다. 이러한 워크플로우는 업그레이드 중에 유지되거나 이전 버전에서 워크플로우 패키지로 내보낸 다음 새 버전으로 가져올 수 있습니다.

### 임시 워크플로 {#transient-workflows}

표준 워크플로우는 실행 중에 런타임(기록) 정보를 저장합니다. 워크플로우 모델을 **임시** 이러한 기록이 지속되지 않도록 하기 위해 이 워크플로우는 정보를 유지하는 데 사용되는 시간과 리소스를 절약하기 때문에 성능 조정에 사용됩니다.
...
자산 정보가 중요하지만 워크플로우 런타임 기록은 아닌 많은 자산을 로드하기 위해 임시 워크플로우가 도입되었습니다.
...
>[!CAUTION]
>
>워크플로우 모델이 임시 로 플래그가 지정되는 경우 런타임 정보가 계속 유지되어야 하는 몇 가지 시나리오가 있습니다.
>
>* 페이로드 유형(예: 비디오)에는 처리를 위한 외부 단계가 필요합니다. 이러한 경우 상태 확인에 런타임 기록이 필요합니다.
>* 워크플로우가 **AND 분할**. 이러한 경우 상태 확인을 위해 런타임 기록이 필요합니다.
>* 임시 워크플로우가 참가자 단계에 들어가면 런타임 시 모드가 비임시 모드로 변경됩니다. 작업이 사용자에게 전달되기 때문에 기록을 유지해야 합니다.
>
...
>임시 워크플로우 내에서는 **이동 단계**.
>
>이유는 **이동 단계** 에서 워크플로우를 계속하기 위해 sling 작업을 만듭니다. `goto` 점. 이 보고서는 워크플로우를 일시화하는 목적을 무효화하고 로그 파일에 오류를 생성합니다.
>
>사용 **또는 분할** 임시 워크플로우 내에서 선택할 수 있습니다.
...
활성화 **다중 리소스 지원** 워크플로우 모델의 경우 여러 리소스를 선택하더라도 단일 워크플로우 인스턴스가 시작됩니다. 각 패키지가 패키지로 첨부됩니다.

If **다중 리소스 지원** 은 워크플로우 모델에 대해 활성화되지 않으며 여러 리소스를 선택한 다음 각 리소스에 대해 개별 워크플로우 인스턴스가 시작됩니다.
...
워크플로우 단계는 작업을 처리할 때 워크플로우의 진행 상황을 시각화하는 데 도움이 됩니다. 이 매개 변수를 사용하여 처리를 통해 워크플로우가 얼마나 멀리 있는지 개요를 제공할 수 있습니다. 워크플로우가 실행되면 사용자는 다음에 설명된 진행률을 볼 수 있습니다. **단계** (개별 단계와 반대됨).
...
1. 워크플로우를 실행하면 단계 이름 대신 스테이지 이름에 따라 진행 상황을 볼 수 있습니다. 워크플로우 진행 상태는 [워크플로우 항목의 작업 세부 사항 창의 워크플로우 정보 탭](/help/sites-authoring/workflows-participating.md#opening-a-workflow-item-to-view-details-and-take-actions) 다음 목록에 추가됨 [받은 편지함](/help/sites-authoring/inbox.md).

### 워크플로우 및 Forms {#workflows-and-forms}

일반적으로 워크플로우는 AEM에서 양식 제출을 처리하는 데 사용됩니다. 이 ID는 [핵심 구성 요소 양식 구성 요소](https://experienceleague.adobe.com/docs/experience-manager-core-components/using/wcm-components/forms/form-container.html?lang=en) 표준 AEM 인스턴스 또는 [AEM Forms 솔루션](/help/forms/using/aem-forms-workflow.md).

양식을 만들 때 양식 제출을 워크플로우 모델과 쉽게 연결할 수 있습니다. 예를 들어 컨텐츠를 저장소의 특정 위치에 저장하거나 사용자에게 양식 제출 및 해당 컨텐츠에 대해 알리는 작업을 수행할 수 있습니다.
