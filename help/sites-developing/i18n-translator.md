...
Translator 도구와 관리하는 사전은 구성 요소 UI를 다양한 언어로 제공하기 위한 것입니다. 페이지 또는 사용자 생성 콘텐츠를 번역하려면 [다국어 사이트를 위한 콘텐츠 번역](/help/sites-administering/translation.md) 및 [사용자 생성 콘텐츠 번역](/help/communities/translate-ugc.md)을 참조하세요.

>[!CAUTION]
>
>프로젝트에 대해 만들어지고 `/apps` 아래에 있는 사전만 편집하십시오.
>
>이 도구에서는 AEM 시스템 사전도 사용할 수 있습니다. AEM UI에 문제가 발생할 수 있으므로 AEM 시스템 사전을 변경하지 마십시오. 또한 업그레이드 시 변경 사항이 손실될 수 있습니다. AEM 시스템 사전이 `/libs` 아래에 있습니다.
...
1. 이제 **확인**&#x200B;을 클릭하거나 대상 사전을 볼 수 있습니다.
...
1. 번역기를 다시 로드하면 사전 경로(예: `/apps/myProject/i18n`)를 도구 모음의 드롭다운 선택기에서 사용할 수 있습니다. 문자열 및 해당 번역을 추가하려면 선택하십시오.

   >[!NOTE]
   >
   >변환기는 실제로 경로 아래에 있는 언어의 번역만 저장합니다(예: `/apps/myProject/i18n`).
   >
...
>프로젝트에 대해 만들어지고 `/apps` 아래에 있는 사전만 편집하십시오.
>
>AEM UI에 문제가 발생할 수 있으므로 AEM 시스템 사전을 변경하지 마십시오. 또한 업그레이드 시 변경 사항이 손실될 수 있습니다. AEM 시스템 사전이 `/libs` 아래에 있습니다.
...
[변환된 문자열을 요청하는 코드](/help/sites-developing/i18n-dev.md#including-variables-in-localized-sentences)은(는) 컨텍스트에 따라 인덱싱된 변수에 대한 값을 제공합니다.
...
* **텍스트별로 필터링:** 영어 문자열, 댓글 또는 번역과 일치하는 패턴입니다. 패턴 전체 또는 일부와 일치하는 항목만 테이블에 나타납니다.
* **변경 내용: 모두, 수정됨, 새로 만들기, 삭제됨:** 변경되고 저장되지 않은 항목을 표시합니다.
...
* **댓글이 있음**: 번역자에 대한 댓글이 있는 항목을 표시합니다.
...
사전에 영어 문자열을 추가한 후 문자열의 번역을 추가할 수 있습니다. [사전을 내보내기](/help/sites-developing/i18n-translator.md#exporting-a-dictionary)하여 타사에서 번역할 수도 있습니다.

1. [프로젝트 특정 사전](#creating-a-dictionary)을(를) 선택하십시오. 번역이 들어 있는 저장소의 경로를 지정합니다. 예를 들어 다음과 같이 **사전**&#x200B;을 선택합니다.
...
1. 문자열 중 하나에 대한 번역된 텍스트를 편집하려면 다음 중 하나를 수행할 수 있습니다.
   ...
   * 필요한 문자열의 **문자열** 또는 **댓글** 필드를 두 번 클릭하여 **문자열 편집** 대화 상자를 열고 필요에 따라 번역을 편집한 다음 **확인**&#x200B;을 클릭하여 대화 상자를 닫습니다.

   ![chlimage_1-217](assets/chlimage_1-217.png)

1. 변경 내용을 커밋하려면 도구 모음에서 **저장**&#x200B;을 클릭합니다.

   >[!NOTE]
   >
   >**저장** 대신 **다시 설정 및 새로 고침**&#x200B;을 클릭하면 이전 텍스트로 변경 내용이 모두 되돌아갑니다.
...
XLIFF 파일을 내보내고 언어를 포함할 때 저장소에 있는 사전의 노드 구조에 해당 언어가 포함되어야 합니다. 언어가 포함되지 않으면 오류가 발생합니다. 예를 들어 프랑스어 XLIFF 파일을 내보내려면 사전 폴더에 이름이 `fr`인 `mix:language` 자식 노드가 포함되어야 합니다. ([사전 만들기](/help/sites-developing/i18n-translator.md#creating-a-dictionary)를 참조하십시오.)
...
1. 내보내기 > 전체 *XX* Xliff 옵션 내보내기를 클릭합니다. 여기서 *XX*&#x200B;은(는) DE 또는 FR과 같은 두 문자 언어 코드입니다.
...
1. CRXDE Lite에서 **모두 저장**&#x200B;을 클릭하고 번역기를 다시 로드합니다. 격자가 업데이트되어 정의된 언어를 표시합니다.

   >[!NOTE]
   >
   >번역자는 사전에 실제로 [존재하는](#creating-a-dictionary) 언어(즉, `/apps/myProject/i18n`과 같은 사전 경로 아래)에 대한 번역만 저장합니다.
   >
   >이러한 언어가 격자에 표시된 언어에 해당하는지 확인하십시오.

### 작성자가 사용할 수 있는 언어 만들기 {#making-languages-available-to-authors}

AEM 인스턴스의 새로운 언어에 대한 사전을 정의한 후 작성자가 이를 선택할 수 있도록 해야 합니다(예: **환경 설정**&#x200B;에서 사용).
...
1. **웹 사이트** 콘솔에서 **기본 설정**&#x200B;의 언어를 사용하려면 응용 프로그램에서 다음 사항을 변경해야 합니다.
...
미국, 영국, 호주 등이 모두 영어를 사용하는 등 다양한 국가에서 동일한 언어를 사용하고 있다. 이는 `en_US`, `en_GB` 및 `en_AU`과(와) 같이 언어와 국가를 모두 나타내는 코드로 표시됩니다.

기본 국가는 플래그를 표시할 때(예: 언어 복사 대화 상자에서) 언어 코드의 국가를 확인하는 데 사용됩니다.

>[!NOTE]
>
>위의 번역자가 관리하는 현지화의 경우 정확한 언어만 작동합니다. 언어 환경 설정 드롭다운에서 `en_uk`을(를) 사용하는 경우 저장소에 `en_uk` 사전이 있어야 합니다.
...
1. 언어 목록은 다음 아래에 저장됩니다.
   ...
   그런 다음 목록을 변경하거나 확장합니다. 언어 노드(예: `ja`)의 `defaultCountry` 속성에는 `jp`을(를) 언어 `ja`의 기본 국가로 정의하는 `ja_jp`과(와) 같은 전체 코드가 포함되어야 합니다.
