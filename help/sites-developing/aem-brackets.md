...
AEM Brackets Extension은 AEM 구성 요소와 클라이언트 라이브러리를 편집하는 매끄러운 워크플로우를 제공하고 코드 편집기 내에서 Photoshop 파일과 레이어에 액세스할 수 있는 [Brackets](https://brackets.io/) 코드 편집기의 기능을 활용합니다. 익스텐션에서 제공하는 간편한 동기화(Maven 또는 File Vault 필요 없음)는 개발자의 효율성을 높이고 AEM에 대한 지식이 제한된 프런트 엔드 개발자도 프로젝트에 참여할 수 있도록 지원합니다. 또한 이 익스텐션은 구성 요소 개발을 쉽고 보다 안전하게 하기 위해 JSP의 복잡성을 덜어주는 [HTML 템플릿 언어(HTL)](https://docs.adobe.com/content/help/ko-KR/experience-manager-htl/using/overview.html)에 대한 일부 지원을 제공합니다.
...
1. 대괄호를 엽니다. **파일**&#x200B;에서 **Extension Manager...**
1. 검색 바에 **AEM**&#x200B;을 입력하고 **AEM Brackets Extension**&#x200B;을 찾습니다.
...
>[!NOTE]
>
>컨텐츠 패키지가 있는 자체 프로젝트가 없는 경우 [HTL TodoMVC 예제](https://github.com/Adobe-Marketing-Cloud/aem-sightly-sample-todomvc)를 사용해 볼 수 있습니다. GitHub에서 **ZIP 다운로드**&#x200B;를 클릭하고 로컬에서 파일을 추출합니다. 위의 지시에 따라 대괄호로 `jcr_root` 폴더를 엽니다. 그런 다음 아래 절차에 따라 **프로젝트 설정**&#x200B;을 설정하고, 전체 컨텐츠 패키지&#x200B;**를 내보내면서 전체 패키지를 AEM 개발 인스턴스에 업로드합니다. 자세한 내용은 전체 컨텐츠 패키지 동기화 섹션에 설명되어 있습니다.**
>
>이러한 단계 후에 AEM 개발 인스턴스의 `/content/todo.html` URL에 액세스할 수 있어야 하며 Brackets의 코드 수정 작업을 시작하고 웹 브라우저에서 새로 고친 후 변경 사항이 AEM 서버에 즉시 동기화되는 방법을 확인할 수 있습니다.
...
AEM Brackets Extension은 `filter.xml`에 정의된 필터링 규칙에서 허용하는 파일 및 폴더에 대해 다음 유형의 컨텐츠 동기화를 제공합니다.
...
프로젝트 탐색기에서 파일 또는 폴더를 마우스 오른쪽 단추로 클릭하여 컨텍스트 메뉴를 열고 **서버로 내보내기** 또는 **서버에서 가져오기** 옵션에 액세스할 수 있습니다.

![chlimage_1-56](assets/chlimage_1-56a.png)

>[!NOTE]
>
>선택한 항목이 `jcr_root` 폴더 외부에 있으면 **Export to Server** 및 **Import from Server**&#x200B;컨텍스트 메뉴 항목이 비활성화됩니다.
...
>[!NOTE]
>
>사용된 동기화 방법에 관계없이 `filter.xml`의 필터링 규칙에 의해 포함된 것으로 표시된 컨텐츠만 동기화됩니다.
>
>또한 `.vltignore` 파일은 저장소에서 컨텐츠를 동기화하거나 동기화하지 않도록 제외하기 위해 지원됩니다.
...
1. HTML 특성에 `sly`을 입력합니다. 특성이 `data-sly-`(으)로 자동 완료되었습니다.
1. 드롭다운 목록에서 HTL 속성을 선택합니다.

### 식 자동 완성 {#expression-auto-completion}

표현식 `${}` 내에서 일반적인 변수 이름은 자동으로 완료됩니다.

## 추가 정보 {#more-information}

AEM Brackets Extension은 Apache License 버전 2.0에서 [Adobe Marketing Cloud](https://github.com/Adobe-Marketing-Cloud) 조직이 GitHub에서 호스팅하는 오픈 소스 프로젝트입니다.
...
Brackets 코드 편집기는 [Adobe Systems Incorporated](https://github.com/adobe) 조직에서 GitHub에서 호스팅하는 오픈 소스 프로젝트이기도 합니다.
