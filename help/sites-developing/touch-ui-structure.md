...
description: AEM에서 구현된 것처럼 터치에 적합한 UI는 몇 가지 기본 원칙이 있으며 몇 가지 주요 요소로 구성됩니다
...
AEM 터치 지원 UI에는 몇 가지 기본 원칙이 있으며 몇 가지 주요 요소로 구성됩니다.
...
이 값은 아래 페이지에서 보기 또는 에셋 제어와 관련된 위치 및 표면 도구에 대한 상황입니다. 도구 모음은 제품마다 다르지만 요소에 공통점이 있습니다.
...
여기에는 에셋 및 구성 요소를 선택할 수 있는 두 개의 기본 탭이 포함되어 있습니다. 여기서 드래그하여 페이지로 놓을 수 있습니다.

기본적으로 사이드 패널은 숨겨져 있습니다. 이 옵션을 선택하면 왼쪽에 표시되거나, 창 크기가 1024px보다 작은 경우(예: 모바일 장치에서)에 전체 창을 덮도록 미끄러져 가로로 표시됩니다.
...
터치 지원 UI와 관련된 개념에 대한 자세한 내용은 이 문서에서 확인하십시오 [AEM 터치 지원 UI의 개념](/help/sites-developing/touch-ui-concepts.md).
