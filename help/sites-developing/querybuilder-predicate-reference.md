...
JCR 부울 속성에서 일치합니다. 값만 수락 `true`&quot; 및 &quot; `false`&quot;. 의 경우 `false`&quot;, 속성에 값이 있으면 일치합니다.&quot; `false`또는 이 링크가 전혀 없는 경우입니다. 이 기능은 활성화된 경우에만 설정되는 부울 플래그를 확인하는 데 유용합니다.
...
패싯 추출을 지원합니다. 은 &quot;오늘&quot;, &quot;이번 주&quot;, &quot;이번 달&quot;, &quot;지난 3개월&quot;, &quot;올해&quot;, &quot;지난해&quot; 및 &quot;지난해보다 이전&quot;의 버킷을 제공합니다.
...
* **하한**
   ...
   check 속성에 바인딩된 하한 날짜(예: ) `2014-10-01`
...
* **excludepaths**
   ...
   정규 표현식이 결과 경로에 대해 일치합니다. 단, 일치하는 표현식은 결과에서 제외합니다.
...
중첩된 조건을 작성할 수 있습니다. 그룹은 중첩 그룹을 포함할 수 있습니다. QueryBuilder 쿼리의 모든 항목은 암시적으로 루트 그룹에 있으며 `p.or` 및 `p.not` 매개 변수도 마찬가지입니다.
...
* **N_&lt;predicate>**
   ...
   다음과 같이 동시에 여러 중첩된 술어를 추가합니다. `1_property, 2_property, ...`

### hasPermission {#haspermission}

현재 세션에 지정된 항목으로 결과를 제한합니다. [JCR 권한.](https://www.adobe.io/experience-manager/reference-materials/spec/jcr/2.0/16_Access_Control_Management.html#16.2.3%20Standard%20Privileges)
...
특정 언어로 CQ 페이지를 찾습니다. 이는 페이지 언어 속성과 종종 최상위 사이트 구조에 언어 또는 로케일을 포함하는 페이지 경로를 모두 살펴봅니다.

이는 필터링 전용 술어이며 검색 색인을 활용할 수 없습니다.

패싯 추출을 지원합니다. 는 각 고유 언어 코드에 대한 버킷을 제공합니다.
...
노드가 DAM 주 자산이고 하위 자산이 아닌지 확인합니다. 이는 기본적으로 &quot;하위 에셋&quot; 노드 내에 없는 모든 노드입니다. 이 작업은 를 확인하지 않습니다 `dam:Asset` 노드 유형. 이 술어를 사용하려면 &quot; `mainasset=true`&quot; 또는 &quot; `mainasset=false`&quot;, 더 이상의 속성이 없습니다.

이는 필터링 전용 술어이며 검색 색인을 활용할 수 없습니다.

패싯 추출을 지원합니다. 는 기본 및 하위 에셋에 대해 2개의 버킷을 제공합니다.

#### 속성 {#properties-9}

* **주요 자산**

   부울, &quot; `true`기본 자산의 경우, &quot; `false`하위 자산의 경우 &quot;

### memberOf {#memberof}

특정 항목의 멤버인 항목 검색 [sling 리소스 컬렉션](https://helpx.adobe.com/experience-manager/6-5/sites/developing/using/reference-materials/javadoc/org/apache/sling/resource/collection/ResourceCollection.html).
...
패싯 추출을 지원합니다. 각 고유한 노드 이름(파일 이름)에 대해 버킷을 제공합니다.

#### 속성 {#properties-11}

* **nodename**

   와일드카드를 허용하는 노드 이름 패턴: `*` = any 또는 no char, `?` = 모든 문자, `[abc]` = 대괄호 안에 있는 문자만
...
* **가공되지 않음**
   ...
   부울, &quot; `true`&quot;아직 만료되지 않은 경우(미래 또는 같은 날짜),&quot; `false`만료 날짜(과거 날짜)에 대해 &quot;&quot;(필수)

* **속성**

   상대 경로 `DATE` 확인할 속성(필수)
...
* **orderby**
   ...
   앞에 @이 있는 JCR 속성 이름(예: ) `@jcr:lastModified` 또는 `@jcr:content/jcr:title`또는 쿼리의 다른 조건자(예: ) `2_property`, 정렬 대상

* **정렬**

   정렬 방향, 다음 중 하나 `desc`내림차순 또는 &quot; `asc`오름차순의 경우 &quot;(기본값)

* **사례**

   로 설정된 경우 `ignore`&quot;은(는) 정렬 대소문자를 구분하지 않게 합니다. 즉, &quot;a&quot;가 &quot;B&quot; 앞에 옵니다. 비어 있거나 누락된 경우 정렬은 대소문자를 구분하며, 즉 &quot;B&quot;가 &quot;a&quot; 앞에 옵니다.
...
* **경로**
   ...
   경로 패턴. 정확히 말하면 전체 하위 트리가 일치합니다(예: 추가). `//*` xpath에서 기본 경로는 포함되지 않지만(exact=false, default), 와일드카드()를 포함할 수 있는 정확한 경로만 일치합니다. `*`); self 가 설정되면, 기본 노드를 포함하는 전체 하위 트리가 검색됩니다.
...
* **평평해**
   ...
   직접 하위만 검색합니다(&quot; 추가 등). `/*`xpath의 &quot;&quot;(다음과 같은 경우에만 사용됨) `exact`&#39;은(는) true가 아닙니다(선택 사항).
...
패싯 추출을 지원합니다. 결과에 있는 각 고유 속성 값에 대한 버킷을 제공합니다.
...
* **정렬 단추**
   ...
   속성을 확인할 값. JCR 속성 유형을 따라 문자열 변환을 수행합니다.
...
* **및**
   ...
   여러 값을 결합하려면 true로 설정합니다( `N_value`) 및 (5.3 이후)
...
* **깊이**
   ...
   속성/상대 경로가 존재할 수 있는 와일드카드 수준의 수입니다(예: `property=size depth=2` 은(는) node/size, node/&amp;ast;/size 및 node/&amp;ast;/&amp;ast;/size)를 확인합니다.

### rangeproperty {#rangeproperty}

간격에 대해 JCR 속성과 일치합니다. 이는 다음과 같은 선형 유형이 있는 속성에 적용됩니다. `LONG`, `DOUBLE` 및 `DECIMAL`. 대상 `DATE` 최적화된 날짜 형식 입력이 있는 daterange 술어를 참조하십시오.

하한과 상한을 정의하거나 둘 중 하나만 정의할 수 있습니다. 작업(예: &quot;보다 작음&quot; 또는 &quot;작거나 같음&quot;)은 하한과 상한에 대해 개별적으로 지정할 수도 있습니다.
...
일치 `JCR DATE` 현재 서버 시간을 기준으로 한 시간 오프셋을 사용하는 날짜/시간 간격에 대한 등록 정보입니다. 다음을 지정할 수 있습니다. `lowerBound` 및 `upperBound` 밀리초 값 또는 bugzilla 구문 사용 `1s 2m 3h 4d 5w 6M 7y` (1초, 2분, 3시간, 4일, 5주, 6개월, 7년) 접두사로 &quot; &quot;를 사용합니다. `-`현재 시간 이전의 음수 오프셋을 나타냅니다. 다음을 지정하는 경우 `lowerBound` 또는 `upperBound`, 다른 하나는 현재 시간을 의미하는 0으로 기본값이 설정됩니다.
...
* **상한**
   ...
   상한 날짜(밀리초) 또는 `1s 2m 3h 4d 5w 6M 7y` 현재 서버 시간과 관련하여 1초, 2분, 3시간, 4일, 5주, 6개월, 7년) 음수 오프셋에 &quot;-&quot;를 사용합니다.

* **하한**

   밀리초 단위의 하한 날짜 또는 `1s 2m 3h 4d 5w 6M 7y` 현재 서버 시간과 관련하여 1초, 2분, 3시간, 4일, 5주, 6개월, 7년) 음수 오프셋에 &quot;-&quot;를 사용합니다.
...
* **p.offset**
   ...
   결과 페이지의 시작, 즉 건너뛸 항목의 수를 나타내는 숫자
...
* **p.guessTotal**
   ...
   권장: 비용이 많이 들 수 있는 전체 결과 합계를 계산하지 마십시오. 계산할 최대 합계를 나타내는 숫자(예: 1000, 대략적인 크기에 대해 사용자에게 충분한 피드백을 제공하고 더 작은 결과에 대한 정확한 숫자) 또는 &quot; `true`필요한 최소값까지만 계산하려는 경우 `p.offset` + `p.limit`

* **발췌**

   로 설정된 경우 `true`&quot;, 결과에 전체 텍스트 발췌 포함
...
* **p.hits**
   ...
   * **전체**:
      ...
      노드의 sling JSON 렌더링, `jcr:path` 히트의 경로 표시: 기본적으로 노드의 직접 속성만 나열하므로 다음과 같은 심층 트리 포함 `p.nodedepth=N`를 0으로 지정하면 전체, 무한 하위 트리를 의미합니다. `p.acls=true` 주어진 결과 항목에 현재 세션의 JCR 권한을 포함시키려면 다음을 수행합니다(매핑: `create` = `add_node`, `modify` = `set_property`, `delete` = `remove`)

   * **선택적**:

      에 지정된 속성만 `p.properties`: 상대 경로의 공백으로 구분된(URL에 &quot;+&quot; 사용) 목록입니다. 상대 경로의 깊이가 1보다 크면 이 경로는 하위 객체로 표시됩니다. 특수 jcr:path 속성은 히트의 경로를 포함합니다

### savedquery {#savedquery}

지속 QueryBuilder 쿼리의 모든 술어를 현재 쿼리에 하위 그룹 술어로 포함합니다.
...
키워드를 지정하여 하나 이상의 태그로 태그가 지정된 콘텐츠를 검색합니다. 이렇게 하면 먼저 제목에서 이러한 키워드가 포함된 태그를 검색한 다음 태그가 지정된 항목으로만 결과를 제한합니다.
...
* **lang**
   ...
   현지화된 특정 태그 제목에서만 검색 (예: &quot; `de`&quot;)

* **모두**

   (bool) 전체 태그, 즉 모든 제목, 설명 등을 검색합니다. (l보다 우선함) `ang`&quot;)

### 유형 {#type}

결과를 기본 노드 유형 또는 믹스인 유형인 특정 JCR 노드 유형으로 제한합니다. 이렇게 하면 해당 노드 유형의 하위 유형도 검색됩니다. 저장소 검색 인덱스는 효율적인 실행을 위해 노드 유형을 포함해야 합니다.

패싯 추출을 지원합니다. 결과에 각 고유 유형에 대한 버킷을 제공합니다.
