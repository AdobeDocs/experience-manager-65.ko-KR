...
필터를 만든 후 [그룹 구성](/help/sites-developing/mobile.md#creating-a-device-group)에서 사용할 수 있습니다.
...
`matches` 함수는 장치 기능이 모든 필터 조건을 충족하면 `true`을(를) 반환합니다. 메서드 인수에 제공된 정보를 평가하여 장치가 그룹에 속하는지 확인합니다. 다음 값은 인수로 제공됩니다.
...
>[!NOTE]
>
>AEM에 배포되는 WURFL™ 데이터베이스에 필터 기준으로 사용하는 기능이 포함되어 있는지 확인합니다. ([장치 검색](/help/sites-developing/mobile.md#server-side-device-detection)을 참조하세요.)
...
작성 시 5.5.2는 AEM 최신 릴리스에 있는 WCM Mobile API 번들 버전입니다. Adobe 웹 콘솔([https://localhost:4502/system/console/bundles](https://localhost:4502/system/console/bundles))을 사용하여 환경에 배포된 번들 버전인지 확인하십시오.

**POM:**(POM에서 다른 groupId 및 버전을 사용합니다.)
