...
AEM에서는 워크플로우 모델을 만드는 데 사용할 수 있는 몇 가지 프로세스 단계를 제공합니다. 사용자 지정 프로세스 단계는 기본 제공 단계에 포함되지 않은 작업에도 추가할 수 있습니다([워크플로우 모델 만들기](/help/sites-developing/workflows-models.md) 참조).
...
예를 들어, 워크플로우가 AEM 페이지 *P*&#x200B;에 적용되는 경우, 워크플로우가 진행됨에 따라 다음 단계별로 *P*&#x200B;이 전달됩니다. 각 단계는 경우에 따라 선택적으로 *P*&#x200B;에 따라 달라집니다.
...
`WorkflowProcess`에 전달된 세션은 저장소의 루트에 다음 권한이 있는 워크플로우 프로세스 서비스에 대해 서비스 사용자가 백업합니다.
...
>
>이제 권한이 위의 ([권한](#permissions))으로 정의됩니다. 구현을 업데이트하는 데 권장되는 방법입니다.
>
...
>
>
이렇게 하면 관리 세션을 `WorkflowProcess` 구현에 제공하는 이전 동작으로 되돌릴 수 있으며 저장소 전체에 대한 무제한 액세스를 다시 한 번 제공합니다.
...
`AbsoluteTimeAutoAdvancer`(절대 시간 자동 진행) 프로세스는 지정된 시간 이후에 아닌 지정된 시간 및 날짜에 시간 초과된다는 점을 제외하고 **자동 진행**&#x200B;과 동일하게 동작합니다.
...
`AutoAdvancer` 프로세스는 워크플로우를 다음 단계로 자동으로 이동합니다. 다음 단계가 두 개 이상 있을 경우(예: OR 분할이 있는 경우) 이 프로세스는 *기본 경로*&#x200B;를 따라 워크플로우를 진행하며, 지정된 경우 워크플로우를 진행하지 않습니다.
...
`ProcessAssembler` 프로세스는 단일 워크플로우 단계에서 여러 하위 프로세스를 순차적으로 실행합니다. `ProcessAssembler`을(를) 사용하려면 작업 과정에서 이 유형의 단일 단계를 만들고 해당 인수를 설정하여 실행할 하위 프로세스의 이름과 인수를 지정합니다.
