...
>
>이전 버전에서는 관리 세션이 `WorkflowProcess` 를 구현하면 특정 ACL을 정의하지 않고도 가 저장소에 대한 전체 액세스 권한을 가질 수 있습니다.
>
...
>
>에 관리 세션을 제공하는 이전 동작으로 되돌아갑니다. `WorkflowProcess` 를 구현하고 다시 한 번 전체 저장소에 무제한 액세스를 제공합니다.
