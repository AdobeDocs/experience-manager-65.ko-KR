...
* [GitHub에서 사용 가능한 샘플 WKND SPA Project 앱](https://github.com/adobe/aem-guides-wknd-spa)
   * React 앱의 [최신 릴리스를 다운로드합니다.](https://github.com/adobe/aem-guides-wknd-spa/releases) 이름이 `wknd-spa-react.all.classic-X.Y.Z-SNAPSHOT.zip`과(와) 비슷하게 지정됩니다.
   * 앱에 대한 [최신 샘플 이미지](https://github.com/adobe/aem-guides-wknd-spa/releases)를 다운로드합니다. 이름은 `wknd-spa-sample-images-X.Y.Z.zip`과(와) 비슷하게 지정됩니다.
   * [패키지 관리자](/help/sites-administering/package-manager.md)를 사용하여 AEM의 다른 패키지와 마찬가지로 패키지를 설치하십시오.
   * 이 워크스루를 위해 Maven을 사용하여 앱을 설치할 필요는 없습니다.

>[!CAUTION]
>
>이 문서에서는 데모용으로만 [WKND Spa 프로젝트 앱](https://github.com/adobe/aem-guides-wknd-spa)을 사용합니다. 어떤 프로젝트 작업에도 를 사용하지 마십시오.
>
>모든 AEM 프로젝트는 React 또는 Angular을 사용하여 SPA 프로젝트를 지원하고 SPA SDK을 사용하는 [AEM 프로젝트 원형,](https://experienceleague.adobe.com/docs/experience-manager-core-components/using/developing/archetype/overview.html?lang=ko)을 사용해야 합니다.
...
* 깔끔한 분리로 인해 시스템이 더욱 확장되고 독립적인 프런트 엔드 개발이 가능합니다.

### SPA는 어떻게 작동합니까? {#how-does-a-spa-work}

SPA의 기본 개념은 서버 호출로 인한 지연을 최소화하여 SPA가 기본 애플리케이션의 응답성에 근접하도록 서버 호출 및 종속성을 줄이는 것입니다.
...
방문자가 모바일 기본 앱에서 기대하는 것과 접근하는 보다 유연한 경험의 경우 SPA는 첫 번째 로드 시 방문자에 필요한 모든 데이터를 로드합니다. 처음에는 시간이 다소 걸릴 수 있지만 서버 호출을 추가할 필요가 없습니다.

클라이언트측에서 렌더링하면 페이지 요소가 더 빨리 반응하고 방문자에 의한 페이지와의 상호 작용이 즉시 이루어집니다. 필요한 추가 데이터는 페이지 속도를 최대화하기 위해 비동기적으로 호출됩니다.

>[!NOTE]
>
>AEM에서 SPA가 작동하는 방법에 대한 기술 정보는 문서 [AEM에서 SPA 시작하기](/help/sites-developing/spa-getting-started-react.md)를 참조하십시오.
>
>SPA 편집기의 디자인, 아키텍처 및 기술 워크플로우에 대한 자세한 내용은 [SPA 편집기 개요](/help/sites-developing/spa-overview.md) 문서를 참조하십시오.

## SPA를 통한 콘텐츠 편집 경험 {#content-editing-experience-with-spa}

AEM SPA 편집기를 사용하도록 SPA를 빌드하면 컨텐츠 작성자가 컨텐츠를 편집하고 만들 때 차이를 알지 못합니다. 일반적인 AEM 기능을 사용할 수 있고 작성자의 워크플로를 변경할 수 없습니다.
...
1. 제목 구성 요소를 선택하고 도구 모음이 다른 구성 요소에 대해 과 같이 표시되는지 확인합니다. **편집**&#x200B;을 선택합니다.

   ![2단계](assets/spa-walkthrough-step-2.png)

1. AEM 내에서 컨텐츠를 정상적으로 편집합니다. 변경 사항은 유지됩니다.

   ![3단계](assets/spa-walkthrough-step-3.png)

   >[!NOTE]
   >
   >현재 위치 텍스트 편집기 및 SPA에 대한 자세한 내용은 [SPA 편집기 개요](spa-overview.md#requirements-limitations)를 참조하십시오.
...
페이지에서 추가 구성 요소를 드래그 앤 드롭하고, 구성 요소를 다시 정렬하고, 레이아웃을 수정하는 등의 추가 작성 도구는 비 SPA 애플리케이션에서와 같이 지원됩니다.
...
SPA가 최종 사용자에 대해 작동하는 방식을 경험한 다음 SPA 페이지를 검사하면 SAP 앱이 AEM의 SPA 편집기에서 작동하는 방식을 더 잘 이해할 수 있습니다.
...
1. 게시 서버에서 또는 페이지 편집기의 **페이지 정보** 메뉴에서 **게시됨으로 보기** 옵션을 사용하여 WKND SPA 프로젝트 애플리케이션을 편집기에 로드합니다.
   ...
   하위 페이지, 날씨 위젯 및 문서에 대한 탐색을 포함하여 페이지 구조에 주목하십시오.
...
다음 섹션인 [SPA 응용 프로그램 로드](#loading-an-spa-application)에서는 SPA를 로드하는 방식과 컨텐츠를 동기적으로 및 비동기적으로 로드하는 방식을 자세히 알아봅니다.
...
1. 개발자 도구의 **네트워크** 탭으로 전환하고 페이지를 다시 로드합니다.

   이미지 요청을 무시하면서 페이지에 대해 로드되는 기본 리소스는 페이지 자체, CSS, React JavaScript, 해당 종속성 및 페이지에 대한 JSON 데이터입니다.
...
1. 새 탭에 `react.model.json`을 로드합니다.
   ...
   ![단계 6](assets/spa-walkthrough-step-1-6.png)
   ...
   각 구성 요소는 표시되는 내용과 렌더링 방법을 선택합니다(HTL로 서버측 또는 React로 클라이언트측). 이 문서는 React를 통해 클라이언트측 렌더링에 중점을 둡니다.
...
샘플 WKND SPA 프로젝트 애플리케이션을 사용하면 JSON 콘텐츠 전달 및 리소스의 비동기 로드를 위해 콘텐츠 서비스를 사용하여 앱이 어떻게 동작하며 게시될 때 로드되는지 명확합니다.
...
1. 브라우저의 기본 제공 개발자 도구를 사용하여 페이지 콘텐츠를 검사합니다. 선택 도구를 사용하여 페이지에서 편집 가능한 구성 요소를 선택하고 요소 세부 정보를 조회합니다.
   ...
   이는 편집기가 SPA 내에서 편집 가능한 구성 요소로 인식하는 데 필요한 유일한 마크업 속성입니다. 이 속성을 기반으로 SPA 편집기는 편집 가능한 구성이 구성 요소와 연관되어 올바른 프레임, 도구 모음 등이 로드되도록 결정합니다.

   일부 특정 클래스 이름이 플레이스홀더 표기 및 자산 드래그 앤 드롭 기능에 추가되기도 합니다.

   >[!NOTE]
   >
   >이것은 편집 가능한 각 구성 요소에 대해 삽입된 `cq` 요소가 있는 AEM에서 서버측 렌더링된 페이지의 동작 변경입니다.
   >
   >
   >SPA의 이 접근 방식을 사용하면 추가 데이터 속성만 사용하여 사용자 지정 요소를 삽입하지 않아도 되므로 프런트 엔드 개발자의 마크업이 더 간단해집니다.

## 다음 단계 {#next-steps}

이제 AEM의 SPA 편집 환경과 SPA가 SPA 편집기와 관련되는 방식을 이해했으므로 SPA의 빌드 방법에 대해 자세히 알아볼 수 있습니다.

* [AEM에서 SPA 시작하기](/help/sites-developing/spa-getting-started-react.md)는 AEM에서 SPA 편집기와 함께 작동하도록 기본 SPA를 빌드하는 방법을 보여 줍니다.
