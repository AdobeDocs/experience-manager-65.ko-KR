...
PageInfo 서블릿은 저장소의 리소스에 대한 정보를 반환합니다. 서블릿은 URL에 바인딩되며 `https://<server>:<port>/libs/wcm/core/content/pageinfo.json` 이 `path` 매개 변수를 사용하여 리소스를 식별합니다. 다음 URL은 `/content/we-retail/us/en` 노드에 대한 정보를 반환합니다.
...
다음 Java 클래스는 PageInfoProvider [를](https://helpx.adobe.com/experience-manager/6-5/sites/developing/using/reference-materials/javadoc/index.html) 구현하고 현재 페이지 리소스의 게시된 URL을 반환합니다.
