...
1. **워크플로우 모델** 콘솔을 엽니다.**도구**, **워크플로**, **모델** 또는 다음 예를 통해[https://localhost:4502/aem/workflow](https://localhost:4502/aem/workflow)
1. **만들기**&#x200B;를 선택하고 **모델 만들기**&#x200B;를 선택합니다.
1. **워크플로우 모델 추가** 대화 상자가 나타납니다. **완료**&#x200B;를 선택하기 전에 **제목** 및 **이름**(선택 사항)을 입력합니다.
1. 새 모델은 **워크플로우 모델** 콘솔에 나열됩니다.
1. 새 작업 흐름을 선택한 다음 [**편집**&#x200B;을 사용하여 구성](#editinganexistingworkflow)에 대해 엽니다.
...
* [스테이지](#configuring-workflow-stages-that-show-workflow-progress), [워크플로우가 일시적](#creatingatransientworkflow-) 및/또는 [여러 리소스를 사용하는지 여부](#configuring-a-workflow-for-multi-resource-support)를 비롯한 워크플로우 속성 구성

[**기본값 및/또는 레거시**(기본) 작업 과정](#editing-a-default-or-legacy-workflow-for-the-first-time)을 편집하면 변경 전에 [안전 복사본](/help/sites-developing/workflows-best-practices.md#locations-workflow-models)이 수행되도록 추가 단계가 있습니다.

작업 과정 업데이트가 완료되면 **동기화**&#x200B;를 사용하여 **런타임 모델 생성**&#x200B;해야 합니다. 자세한 내용은 [워크플로우 동기화](#sync-your-workflow-generate-a-runtime-model)를 참조하십시오.
...
모델을 편집할 때 사용 가능한 단계가 **단계 브라우저**&#x200B;의 다양한 그룹에 표시됩니다. 예:
...
1. 단계 브라우저를 엽니다.상단 도구 모음의 맨 왼쪽에 있는 **사이드 패널 켜기/끄기** 를 사용합니다. 여기에서 다음을 수행할 수 있습니다.
...
**Configure** 및 **단계 속성** 대화 상자를 사용하여 워크플로우 단계의 동작을 사용자 정의할 수 있습니다.
...
1. 필요에 따라 **단계 속성**&#x200B;을 구성합니다.사용 가능한 속성은 단계 유형에 따라 다르며, 사용 가능한 여러 탭이 있을 수도 있습니다. 예를 들어, 기본 **참가자 단계**&#x200B;는 `Step 1` 새 워크플로우에 있습니다.
...
1. **저장 및 닫기**;로 변경 내용을 확인합니다.그 뒤에 **동기화**(편집기 도구 모음)가 나타나 런타임 모델을 생성합니다.
...
## Touch UI {#classic2touchui}에서 워크플로우 모델을 사용할 수 있도록 설정
...
1. **[!UICONTROL 편집]**&#x200B;을 선택하고 **[!UICONTROL 페이지 정보]** 메뉴를 연 다음 **[!UICONTROL 속성 열기]**&#x200B;를 선택하고 **[!UICONTROL 기본]** 탭(아직 열지 않은 경우)으로 이동합니다.
...
1. 페이지에 **양식** 구성 요소를 추가합니다.
1. **페이지** 에  **** 표시된 양식 시작 구성 요소를 구성합니다.
...
로그 파일에 **DEBUG** 메시지를 표시하도록 AEM을 구성할 수도 있습니다. 자세한 내용은 [로깅](/help/sites-deploying/configure-logging.md)을 참조하고 개발이 완료되면 **로그 수준**&#x200B;을 다시 **정보**&#x200B;로 설정합니다.
...
1. 단계 브라우저의 **워크플로** 선택에서 **참가자 단계**&#x200B;을 워크플로로 드래그하고 **흐름 시작**&#x200B;과 **흐름 끝** 사이에 배치합니다.
...
1. **Or Split**&#x200B;을 워크플로우로 드래그하여 `Validate Content`와 **흐름 끝** 사이에 배치합니다.
...
1. **참가자 단계**&#x200B;를 왼쪽 분기로 드래그하여 속성을 열고 다음 값을 지정한 다음 변경 사항을 확인합니다.
...
1. 사용자가 **완료****콘텐츠 유효성 검사** 단계로 이동할 때 **페이지를 요청된 상태로 게시** 또는 **게시 요청 거부**&#x200B;를 선택할 수 있도록 이 워크플로우를 페이지에 적용합니다.

   ![chlimage_1-72](assets/chlimage_1-72.png)

### 예:ECMA 스크립트 {#defineruleecmascript}을 사용하여 OR 분할에 대한 규칙 정의
...
예: **활성화 요청**. 이 워크플로우는 **사이트** 내에 페이지를 게시하는 데 사용되며 컨텐츠 작성자에게 적절한 복제 권한이 없을 때 자동으로 트리거됩니다. 자세한 내용은 [페이지 작성 사용자 정의 - 활성화 요청 사용자 정의](/help/sites-developing/customizing-page-authoring-touch.md#customizing-the-request-for-activation-workflow)를 참조하십시오.
