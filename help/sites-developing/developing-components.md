...
* When [페이지 작성](/help/sites-authoring/default-components.md)로 지정하는 경우 작성자가 컨텐츠를 편집하고 구성할 수 있도록 구성 요소를 사용할 수 있습니다.
   ...
   * 구성 시 [상거래](/help/commerce/cif-classic/administering/ecommerce.md) 예를 들어 구성 요소가 카탈로그에서 정보를 수집하고 렌더링할 수 있는 사이트입니다.
자세한 내용은 [eCommerce 개발](/help/commerce/cif-classic/developing/ecommerce.md) 추가 정보.
...
구성 요소 대화 상자에 대해서만 클라이언트 라이브러리를 로드하도록 하려면(즉, 다른 구성 요소에 대해 로드되지 않음) 속성을 설정해야 합니다 `extraClientlibs`** **대화 상자의 카테고리 이름을 방금 만든 클라이언트 라이브러리의 카테고리 이름으로 지정합니다. 클라이언트 라이브러리가 매우 크고 필드가 해당 대화 상자에만 특화된 경우 다른 대화 상자에서는 필요하지 않습니다.
...
이제 대화 상자 필드에서 이벤트 처리 방법이 [사용자 지정 클라이언트 라이브러리의 리스너](#listeners-in-a-custom-client-library). 이것은 이전의 방법에서 바뀐 것이다 [컨텐츠 구조의 청취자](#listenersinthecontentstructureclassicui).
...
AEM에서는 페이지에 단락 시스템을 구성할 수 있는 기능을 제공합니다 [사용자가 (적절한) 자산을 해당 페이지의 인스턴스로 드래그하면 새 구성 요소의 인스턴스가 자동으로 생성됩니다](/help/sites-authoring/editing-content.md#insertingacomponenttouchoptimizedui) (항상 빈 구성 요소를 페이지로 드래그하지 않고),
...
지원되는 Markdown은 과 동일합니다 [콘텐츠 조각](/help/assets/content-fragments/content-fragments-markdown.md).
