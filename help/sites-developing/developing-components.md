...
* [작성 페이지](/help/sites-authoring/default-components.md)에서는 작성자가 컨텐츠를 편집하고 구성할 수 있습니다.
...
>[!CAUTION]
>
>`/libs/cq/gui/components/authoring/dialog` 아래의 구성 요소는 편집기에서만 사용됩니다(작성의 구성 요소 대화 상자). 마법사 대화 상에서와 같이 다른 곳에서 사용할 경우 예상대로 작동하지는 않을 수 있습니다.
...
구성 요소의 기본 구조는 [AEM 구성 요소 - 기본 사항](/help/sites-developing/components-basics.md#structure) 페이지에서 다룹니다. 이 문서는 터치 지원 UI와 클래식 UI를 모두 다룹니다. 새 구성 요소에서 클래식 설정을 사용할 필요가 없어도 기존 구성 요소에서 상속할 때 이를 파악하는 데 도움이 됩니다.

## 기존 구성 요소 및 대화 상자 확장 {#extending-existing-components-and-dialogs}

구현하려는 구성 요소에 따라 전체 [structure](#structure)를 처음부터 정의하고 개발하는 대신 기존 인스턴스를 확장하거나 사용자 지정할 수 있습니다.
...
기존 구성 요소를 확장하려면 [리소스 유형 계층 구조](/help/sites-developing/components-basics.md#component-hierarchy-and-inheritance) 및 관련 상속 메커니즘으로 수행할 수 있습니다.

>[!NOTE]
>
>구성 요소를 검색 경로 로직을 기반으로 오버레이로 재정의할 수도 있습니다. 그러나 이 경우 [Sling 리소스 합병](/help/sites-developing/sling-resource-merger.md)은 트리거되지 않고 `/apps`는 전체 오버레이를 정의해야 합니다.

>[!NOTE]
>
>[컨텐츠 조각 구성 요소](/help/sites-developing/customizing-content-fragments.md)도 사용자 정의하고 확장할 수 있지만, 자산과의 전체 구조와 관계를 고려해야 합니다.

### 기존 구성 요소 대화 상자 사용자 지정 {#customizing-a-existing-component-dialog}

또한 [Sling Resource Combination](/help/sites-developing/sling-resource-merger.md)을(를) 사용하여 *구성 요소 대화 상자*&#x200B;를 무시하고 `sling:resourceSuperType` 속성을 정의할 수도 있습니다.

즉, 필요한 차이만 재정의하면 됩니다(전체 대화 상자 재정의( `sling:resourceSuperType` 사용). 구성 요소 대화 상자를 확장하는 데 권장되는 방법입니다.

자세한 내용은 [Sling 리소스 합병](/help/sites-developing/sling-resource-merger.md)을 참조하십시오.

## 마크업 {#defining-the-markup} 정의

구성 요소는 [HTML](https://www.w3schools.com/htmL/html_intro.asp)으로 렌더링됩니다. 구성 요소는 필요한 컨텐츠를 가져온 다음 필요에 따라 작성자 및 게시 환경 모두에서 렌더링하는 데 필요한 HTML을 정의해야 합니다.

### HTML 템플릿 언어 사용 {#using-the-html-template-language}

AEM 6.0과 함께 도입된 [HTML 템플릿 언어(HTL)](https://docs.adobe.com/content/help/ko-KR/experience-manager-htl/using/overview.html)는 HTML에 대해 선호되고 권장되는 서버측 템플릿 시스템으로 JSP(JavaServer Pages)를 대체합니다. 강력한 엔터프라이즈 웹 사이트를 구축해야 하는 웹 개발자를 위해 HTL을 사용하면 향상된 보안 및 개발 효율성을 얻을 수 있습니다.
...
>[!NOTE]
>
>터치가 활성화된 UI에서는 [WCM 모드](https://helpx.adobe.com/experience-manager/6-5/sites/developing/using/reference-materials/javadoc/com/day/cq/wcm/api/WCMMode.html) 쿠키에는 `EDIT` 및 `PREVIEW` 값만 사용됩니다.
...
[[MOCK] Granite UI provides a large range of the basic components (widgets)](https://helpx.adobe.com/experience-manager/6-5/sites/developing/using/reference-materials/granite-ui/api/index.html) needed to create your dialog on the authoring environment. 필요한 경우 이 선택 항목을 확장하고 [자신만의 위젯](#creatinganewwidget)을 만들 수 있습니다.
...
>[!NOTE]
>
>[Granite UI] 구성 요소의 특성(및 ExtJS 위젯의 차이)으로 인해 구성 요소가 터치 지원 UI와 상호 작용하는 방법 및 [클래식 UI](/help/sites-developing/developing-components-classic.md)에는 몇 가지 차이가 있습니다.

### 새 대화 상자 만들기 {#creating-a-new-dialog}

터치 지원 UI에 대한 대화 상자:

* 은 `cq:dialog`으로 지정되어 있습니다.
* 은 `sling:resourceType` 속성이 설정된 `nt:unstructured` 노드로 정의됩니다.

* 는 해당 `cq:Component` 노드 아래에 있으며 해당 구성 요소 정의 옆에 있습니다.
* 는 컨텐츠 구조 및 `sling:resourceType` 속성을 기반으로 서버측(Sling components)에서 렌더링됩니다.
...
>[!NOTE]
>
>구성 요소에 터치 지원 UI에 대해 정의된 대화 상자가 없는 경우, 클래식 UI 대화 상자는 호환성 레이어 내부의 대비책으로 사용됩니다. 이러한 대화 상자를 사용자 정의하려면 클래식 UI 대화 상자를 사용자 정의해야 합니다. 클래식 UI](/help/sites-developing/developing-components-classic.md)에 대한 [AEM 구성 요소를 참조하십시오.
...
터치 지원 UI에 대한 구성 요소 대화 상자에서 사용할 새 위젯을 만들려면 [새 [UI] 필드 구성 요소](/help/sites-developing/granite-ui-component.md)을(를) 만들어야 합니다.
...
구성 요소의 스타일 및 동작을 정의하려면 사용자 지정 CSS/LESS 및 JS를 정의하는 전용 [클라이언트 라이브러리](/help/sites-developing/clientlibs.md)를 만들 수 있습니다.

클라이언트 라이브러리를 구성 요소 대화 상자에만 로드하도록 하려면(즉, 다른 구성 요소에 대해 로드되지 않음) 대화 상자의 속성 `extraClientlibs`** **을 방금 만든 클라이언트 라이브러리의 범주 이름으로 설정해야 합니다. 클라이언트 라이브러리가 꽤 크고/또는 필드가 해당 대화 상자에 고유하므로 다른 대화 상자에서는 필요하지 않을 경우 권장됩니다.

모든 대화 상자에 대해 클라이언트 라이브러리를 불러오려면 클라이언트 라이브러리의 category 속성을 `cq.authoring.dialog`으로 설정합니다. 모든 대화 상자를 렌더링할 때 기본적으로 포함되는 클라이언트 라이브러리의 범주 이름입니다. 클라이언트 라이브러리가 작고/또는 필드가 일반적이고 다른 대화 상자에서 다시 사용할 수 있는 경우 이 작업을 수행하려고 합니다.
...
#### {#extending-inheriting-from-a-field} 필드 확장(상속)
...
이제 대화 상자 필드에서 이벤트를 처리하는 방법이 사용자 지정 클라이언트 라이브러리](#listeners-in-a-custom-client-library)의 [리스너로 수행됩니다. 콘텐츠 구조](#listenersinthecontentstructureclassicui)에 [리스너를 갖는 이전 방식과는 다릅니다.
...
이를 위해서는 상호 작용하려는 기본 위젯 라이브러리에 대해 알아야 합니다. 반응할 이벤트를 확인하려면 [Coral UI 설명서](https://helpx.adobe.com/experience-manager/6-5/sites/developing/using/reference-materials/coral-ui/coralui3/index.html)를 참조하십시오. 이는 이전에 ExtJS를 사용하여 수행해야 했던 프로세스와 매우 유사합니다.해당 위젯의 설명서 페이지를 찾은 다음 해당 이벤트 API의 세부 사항을 확인합니다.
...
#### 콘텐츠 구조의 청취자 {#listeners-in-the-content-structure}
...
디자인 대화 상자는 구성 요소에 [디자인 모드](/help/sites-authoring/default-components-designmode.md)에서 편집할 수 있는 디자인 세부 사항이 있을 때 제공됩니다.

이 정의는 컨텐트](#creating-a-new-dialog)을 편집하는 데 사용되는 [대화 상자의 정의와 매우 유사하며, 노드가
...
[구성 요소 도구 모음](/help/sites-developing/touch-ui-structure.md#component-toolbar)은 사용자가 편집, 구성, 복사 및 삭제와 같은 구성 요소의 다양한 작업에 액세스할 수 있도록 해줍니다.
...
## 참조 레일에 대한 구성 요소 구성(대출/대여) {#configuring-a-component-for-the-references-rail-borrowed-lent}
...
기본 AEM에서는 참조 구성 요소만 확인합니다. 구성 요소를 추가하려면 OSGi 번들 **WCM 작성 내용 참조 구성**&#x200B;을 구성해야 합니다.
...
>[!NOTE]
>
>AEM에서 작업할 때는 이러한 서비스에 대한 구성 설정을 관리하는 방법이 몇 가지 있습니다.자세한 내용 및 권장 방법은 [OSGi](/help/sites-deploying/configuring-osgi.md) 구성을 참조하십시오.
...
* [템플릿의 단락 시스템에서  `components` 속성 정의](/help/sites-developing/components-basics.md#adding-your-component-to-the-paragraph-system).

## 자산을 드래그하면 구성 요소 인스턴스 {#configuring-a-paragraph-system-so-that-dragging-an-asset-creates-a-component-instance}가 생성되도록 단락 시스템 구성

AEM은 사용자가 (적절한) 자산을 해당 페이지](/help/sites-authoring/editing-content.md#insertingacomponenttouchoptimizedui)의 인스턴스로 드래그할 때(항상 빈 구성 요소를 페이지로 드래그할 필요 없이) 페이지에 [새 구성 요소의 인스턴스가 자동으로 생성되도록 페이지에 단락 시스템을 구성할 수 있는 가능성을 제공합니다.
...
* 프로젝트를 ZIP 파일[으로 다운로드합니다.](https://github.com/Adobe-Marketing-Cloud/aem-project-archetype/archive/master.zip)

>[!NOTE]
>
>구성 요소 인스턴스 자동 만들기는 이제 [핵심 구성 요소](https://docs.adobe.com/content/help/ko-KR/experience-manager-core-components/using/introduction.html) 및 편집 가능한 템플릿을 사용할 때 UI 내에서 쉽게 구성할 수 있습니다. 지정된 미디어 유형과 자동으로 연결된 구성 요소를 정의하는 방법에 대한 자세한 내용은 [페이지 템플릿 만들기](/help/sites-authoring/templates.md#editing-a-template-structure-template-author)를 참조하십시오.

## AEM Brackets Extension {#using-the-aem-brackets-extension} 사용

[AEM Brackets Extension](/help/sites-developing/aem-brackets.md)은 AEM 구성 요소와 클라이언트 라이브러리를 편집하는 매끄러운 워크플로우를 제공합니다. 이것은 [Brackets](https://brackets.io/) 코드 편집기를 기반으로 합니다.

확장:

* 간편한 동기화(Maven 또는 File Vault 필요 없음)를 통해 개발자의 효율성을 높이고 AEM에 대한 지식이 제한적으로 제공되므로 프런트 엔드 개발자는 프로젝트에 참여할 수 있습니다.
* 구성 요소 개발을 단순화하고 보안을 강화하기 위해 고안된 템플릿 언어인 일부 [HTL](https://docs.adobe.com/content/help/en/experience-manager-htl/using/overview.html) 지원을 제공합니다.
...
* HTL
   ...
   * [HTL](https://docs.adobe.com/content/help/en/experience-manager-htl/using/overview.html)의 사용은 필수적이지 않지만 구성 요소를 업데이트해야 하는 경우 [JSP에서 HTL](/help/sites-developing/components-basics.md#htl-vs-jsp)으로 마이그레이션하는 것을 고려하는 것이 이상적인 시간입니다.
...
* 구성 요소
   ...
   * [클래식  `cq:listener` ](#migrating-cq-listener-code) UI에만 해당되는 함수를 사용하는 코드 마이그레이션

* 대화 상자

   * 터치가 활성화된 UI에서 사용할 새 대화 상자를 만들어야 합니다. 하지만 호환성을 위해 터치 지원 UI에 대해 대화 상자가 정의되지 않은 경우 터치 지원 UI는 클래식 UI 대화 상자의 정의를 사용할 수 있습니다.
   * 기존 구성 요소를 확장하는 데 도움이 되도록 [대화 상자 전환 도구](/help/sites-developing/dialog-conversion.md)가 제공됩니다.
   * [[ExtJS를 [Granite UI 구성 요소]에 매핑](/help/sites-developing/touch-ui-concepts.md#extjs-and-corresponding-granite-ui-components) 은 [ExtJS xtype] 및 [노드 유형]에 상응하는 [Granite UI 리소스 유형]을 갖는 편리한 개요를 제공합니다.
   ...
   * vtypes에서 [Granite UI 유효성 검사](https://helpx.adobe.com/experience-manager/6-5/sites/developing/using/reference-materials/granite-ui/api/jcr_root/libs/granite/ui/components/foundation/clientlibs/foundation/js/validation/index.html)로 마이그레이션
...
클래식 UI용으로 설계된 프로젝트를 마이그레이션하는 경우 `cq:listener` 코드(및 구성 요소 관련 clientlibs)는 클래식 UI에 고유한 기능(예: `CQ.wcm.*`)을 사용할 수 있습니다. 마이그레이션의 경우 터치를 사용하는 UI의 동일한 개체/함수를 사용하여 이러한 코드를 업데이트해야 합니다.
