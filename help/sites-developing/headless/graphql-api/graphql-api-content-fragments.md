...
1. 해당 GraphQL 스키마(GraphiQL 자동 문서에서 출력):
   ...
   이 이미지는 생성된 형식 `ArticleModel`에 여러 개의 [필드](#fields)이(가) 포함되어 있음을 보여 줍니다.
...
사용자 생성 필드의 데이터 형식 외에도 GraphQL for AEM에서는 콘텐츠 조각을 식별하거나 콘텐츠 조각에 대한 추가 정보를 제공하기 위해 여러 *도우미* 필드도 생성합니다.
