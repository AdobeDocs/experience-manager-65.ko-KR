...
태그는 간단한 단어일 수 있습니다(예: `sky`) 또는 계층 분류(예: `fruit/apple`: 제네릭을 모두 의미합니다 `fruit` 및 보다 구체적임 `apple`).
...
네임스페이스를 사용하면 항목을 그룹화할 수 있습니다. 가장 일반적인 사용 사례는 사이트당 네임스페이스(예: 공개, 내부 및 포털) 또는 더 큰 애플리케이션(예: WCM, 에셋, 커뮤니티)입니다. 그러나 네임스페이스는 다양한 다른 요구 사항에 사용할 수 있습니다. 네임스페이스는 사용자 인터페이스에서 현재 콘텐츠에 적용 가능한 태그의 하위 집합(즉, 특정 네임스페이스의 태그)만 표시하기 위해 사용됩니다.
