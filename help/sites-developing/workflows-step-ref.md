...
다음 속성은 다음 사이트의 여러 워크플로우 단계 구성 요소에 사용할 수 있습니다 **사용자/그룹** 속성 대화 상자의 탭:
...
다음 **이동 단계** 워크플로우 모델에서 실행할 다음 단계를 지정할 수 있습니다. 규칙 정의, 외부 스크립트 또는 ECMA 스크립트를 라우팅 표현식으로 지정하여 워크플로우 모델의 다음 단계를 평가할 수 있습니다.

* 지정하는 조건이 true인 경우 **이동 단계** 완료되고 워크플로우 엔진이 지정된 단계를 실행합니다.
* 지정하는 조건이 true를 갖지 않으면 **이동 단계** 완료 및 일반 라우팅 로직이 실행할 다음 단계를 결정합니다.

다음 **이동 단계** 워크플로우 모델에서 고급 공정순서 구조를 구현할 수 있습니다. 예를 들어 루프를 구현하려면 **이동 단계** 루프 조건을 평가하는 라우팅 표현식을 사용하여 워크플로우에서 이전 단계를 실행하도록 정의할 수 있습니다.
...
* **프로세스**
   ...
   * **라우팅 표현식**: 규칙 정의, 외부 스크립트 또는 실행 여부를 결정하는 ECMA 스크립트를 선택합니다 **Target 단계**.

      * **규칙 정의:** 를 사용하십시오 [표현식 편집기](/help/forms/using/variable-in-aem-workflows.md#use-expression-editor) 을 눌러 규칙을 정의합니다.
      * **외부 스크립트:** 외부 스크립트의 경로입니다.
      * **ECMA Script**: 실행 여부를 결정하는 스크립트입니다 **이동 단계**.
...
규칙 정의 를 라우팅 표현식으로 사용하여 for 루프를 시뮬레이션할 수도 있습니다. [만들기 **count** 변수](/help/forms/using/variable-in-aem-workflows.md#create-a-variable) 긴 데이터 유형입니다. 사용 **표현식** 을 매핑 모드에서 **[변수 설정](/help/sites-developing/using-variables-in-aem-workflows.md#set-a-variable)** 의 값을 설정하는 단계 **count** 변수까지 **count + 1** 각 실행 시 **변수 설정** 단계.

![for 루프 시뮬레이션](assets/variable_use_case_count_new.png)

에서 **이동 단계**, 사용 **변수 설정** 로서의 **Target 단계** 및 **5개 미만** 를 라우팅 표현식으로 사용합니다.
...
* 편집 **또는 속성 분할**:
   ...
   * **분기 (*x)***
   ...
   >[!NOTE]
   >
   >자세한 내용은 [OR 분할에 대한 규칙 정의](/help/sites-developing/workflows-models.md#defineruleecmascript).
...
다음 작업 **대화 상자 참가자 단계** 작업 항목이 지정된 사용자로부터 정보를 수집할 수 있습니다. 이 단계는 나중에 워크플로우에서 사용되는 소량의 데이터를 수집하는 데 유용합니다.
...
* **대화 상자**
   ...
   * **대화 상자 경로**: 대화 상자 노드의 경로입니다 [만들기 대화 상자](#dialog-participant-step-creating-a-dialog).
...
1. **대화 상자 경로 속성**
   ...
   다음 **대화 상자 참가자 단계** 있음 **대화 상자 경로** 속성(속성 [참가자 단계](#participant-step)). 의 값 **대화 상자 경로** 속성은 `dialog` 노드 아래에 있어야 합니다.
...
다음 **동적 참가자 단계** 구성 요소는 다음과 유사합니다 **[참가자 단계](#participant-step)** 실행 시 참여자가 자동으로 선택된다는 차이점과 함께

단계를 구성하려면 다음을 선택합니다 **참가자 선택기** 대화 상자와 함께 작업 항목을 지정할 참가자를 식별합니다.
...
* **참가자 선택기**
   ...
   * **참가자 선택기**: 의 이름 [생성할 참가자 선택](#developingtheparticipantchooser).
   * **인수**: 필요한 인수
   * **이메일**: 사용자에게 이메일 알림을 전송해야 하는지 여부.

* **대화 상자**

   * **대화 상자 경로**: 대화 상자 노드의 경로입니다 [만드는 대화 상자(과 같이) **대화 상자 참가자 단계**)](#dialog-participant-step-creating-a-dialog).
...
* **ECMAscript**
   ...
   >[!NOTE]
   >
   >다음 **워크플로우 개시자 참가자 선택기** 구성 요소 확장 **동적 참가자 단계** 는 이 스크립트를 단계 구현으로 사용합니다.
...
* **OSGi 서비스**
   ...
   * `getParticipant` 메서드: 동적으로 확인된 주체 ID를 `String` 값.
   ...
   >[!NOTE]
   >
   >**임의 참가자 선택기** 은 임의 사용자를 선택하는 샘플 서비스입니다( `com.day.cq.workflow.impl.process.RandomParticipantChooser`). 다음 **임의 참가자 선택** r 단계 구성 요소 샘플은 **동적 참가자 단계** 및 은 이 서비스를 단계 구현으로 사용합니다.
...
에서 **동적 참가자 단계** 속성 대화 상자, **참가자 선택기** 목록에 항목이 포함됩니다. `Workflow Initiator Participant Chooser (script)`: 이 서비스를 나타냅니다.

워크플로우 모델이 시작되면, 로그는 워크플로우를 시작한 사용자의 ID와 작업 항목이 지정된 사용자를 나타냅니다. 이 예에서 `admin` 사용자가 워크플로우를 시작했습니다.

`13.09.2015 15:48:53.037 *INFO* [10.176.129.223 [1347565733037] POST /etc/workflow/instances HTTP/1.1] com.adobe.example.InitiatorParticipantChooser Assigning Dynamic Participant Step work item to admin`

### 양식 참가자 단계 {#form-participant-step}

다음 **양식 참가자 단계** 작업 항목이 열릴 때 양식을 표시합니다. 사용자가 양식을 작성하고 제출하면 필드 데이터가 워크플로우 페이로드의 노드에 저장됩니다.
...
* **양식**
   ...
   * **양식 경로**: 경로 [양식 만들기](#form-participant-step-creating-the-form).

#### 양식 참가자 단계 - 양식 만들기 {#form-participant-step-creating-the-form}

와 함께 사용할 양식 만들기 **양식 참가자 단계** 정상입니다. 그러나 양식 참가자 단계의 양식에는 다음 구성이 있어야 합니다.

* 다음 **양식 시작** 구성 요소에는 **작업 유형** 속성 설정 `Edit Workflow Controlled Resource(s)`.
* 다음 **양식 시작** 구성 요소에는 `Form Identifier` 속성을 사용합니다.
* 양식 구성 요소에는 **요소 이름** 필드 데이터가 저장되는 노드의 경로로 설정됩니다. 경로는 워크플로우 페이로드 컨텐츠에서 노드를 찾아야 합니다. 값은 다음 형식을 사용합니다.

   `./jcr:content/path_to_node`

* 양식에 **워크플로우 제출 단추** 구성 요소. 구성 요소의 속성은 구성하지 않습니다.

워크플로우의 요구 사항은 필드 데이터를 저장할 위치를 결정합니다. 예를 들어 필드 데이터를 사용하여 페이지 컨텐츠의 속성을 구성할 수 있습니다. 다음 값 **요소 이름** 속성은 필드 데이터를 `redirectTarget` 속성 `jcr:content` 노드:

`./jcr:content/redirectTarget`

다음 예에서는 필드 데이터가 **텍스트** 페이로드 페이지의 구성 요소:
...
* **인수**
   ...
   * **참가자**: 선택할 수 있는 사용자 목록을 지정합니다. 사용자를 목록에 추가하려면 **항목 추가** 및 사용자 노드의 홈 경로 또는 사용자 ID를 입력합니다. 사용자 순서는 작업 항목이 할당될 가능성에 영향을 주지 않습니다.

### 워크플로우 개시자 참가자 선택기 {#workflow-initiator-participant-chooser}

다음 **워크플로우 개시자 참가자 선택기** 단계는 생성된 작업 항목을 워크플로우를 시작한 사용자에게 지정하는 참가자 선택기입니다. 를 구성하는 것 외에는 속성이 없습니다 **공통** 속성을 사용합니다.
