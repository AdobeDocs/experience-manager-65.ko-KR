...
* **시간 초과**
   ...
   단계가 &quot;시간 초과&quot;될 기간.
다음 중에서 선택할 수 있습니다. **Off**, **** 1h **,** 1h **,** 6h **, 12h****** immediately24h ImmediatelyCools입니다.
...
규칙 정의를 라우팅 표현식으로 사용하여 for 루프를 시뮬레이션할 수도 있습니다. [긴 데이터 유형의 **카운트** 변수를](/help/forms/using/variable-in-aem-workflows.md#create-a-variable) 만듭니다. Set **변수** 단계의 매핑 모드로 **[](/help/sites-developing/using-variables-in-aem-workflows.md#set-a-variable)**표현식을&#x200B;**사용하여**Count **변수**수 + 1의 값을 설정하여 Set 변수Set의 각&#x200B;**실행에 대한 각 실행**횟수를 설정합니다.

![for 루프 시뮬레이션](assets/variable_use_case_count_new.png)

단계에서 **변수****설정을** 타겟 단계로 **사용하고** 5 **를** 경로 지정 표현식으로계산합니다.
...
* OR **분할 속성을 편집합니다**.
   ...
   * **분기(*x)***
   ...
   >[!NOTE]
   >
   >OR [분할에 대한 규칙 정의를 참조하십시오](/help/sites-developing/workflows-models.md#defineruleecmascript).
...
>[!NOTE]
>
>이메일 알림을 활성화하려면 일부 속성을 구성해야 합니다. 이메일 템플릿을 사용자 정의하거나 새 언어의 이메일 템플릿을 추가할 수도 있습니다. See [Configuring Email Notification](/help/sites-administering/notification.md#configuringemailnotification) to configure email notifications in AEM.
...
* **대화 상자**
   ...
   * **대화 상자 경로**:사용자가 만드는 [대화 상자의 대화 상자 노드에 대한](#dialog-participant-step-creating-a-dialog)경로입니다.
...
1. **대화 상자 경로 속성**
   ...
   대화 **상자 참가자 단계에는** **대화 상자 경로** 속성이 있으며 참가자 단계의 [속성은](#participant-step)함께표시됩니다. Dialog Path **속성** 값은 대화 상자 `dialog` 노드의 경로입니다.
...
1. **대화 상자 정의 예**
   ...
   다음 XML 코드 조각은 페이로드 컨텐츠의 `String` `watchEmail` 노드에 값을 저장하는 대화 상자를 나타냅니다. title 노드는 TextField [구성 요소를](https://helpx.adobe.com/experience-manager/6-5/sites/developing/using/reference-materials/granite-ui/api/jcr_root/libs/granite/ui/components/coral/foundation/form/textfield/index.html) 나타냅니다.
...
동적 **참가자 단계** 구성 요소는 **[참가자 단계와 비슷하며](#participant-step)**, 참가자가 런타임 시 자동으로 선택된다는 차이가 있습니다.
...
* **참가자 선택기**
   ...
   * **참가자 선택기**:만든 [참가자 선택기의](#developingtheparticipantchooser)이름입니다.
   * **인수**:필요한 인수.
   * **이메일**:사용자에게 이메일 알림을 전송할지 여부.

* **대화 상자**

   * **대화 상자 경로**:대화 상자 참가자 단계와 같이 [대화 상자의 대화 상자 노드에 **대한**&#x200B;경로입니다](#dialog-participant-step-creating-a-dialog).
...
* **OSGi 서비스**
   ...
   서비스는 com.day.cq.workflow.exec. [ParticipantStepChooser 인터페이스를 구현해야](https://helpx.adobe.com/experience-manager/6-5/sites/developing/using/reference-materials/javadoc/com/day/cq/workflow/exec/ParticipantStepChooser.html) 합니다. 인터페이스는 다음 멤버를 정의합니다.
...
[ **동적 참가자 단계** 속성] 대화 상자의 [참가자 선택기 **]** 목록에는 `Workflow Initiator Participant Chooser (script)`이 서비스를나타내는 항목이 포함됩니다.
...
* **양식**
   ...
   * **양식 경로**:만든 [양식의](#form-participant-step-creating-the-form)경로입니다.
...
* **프로세스**
   ...
   * **프로세스**:실행할 프로세스 구현입니다. 드롭다운 메뉴를 사용하여 ECMAScript 또는 OSGi 서비스를 선택합니다. 자세한 내용:

      * 표준 ECMAScripts 및 OSGi 서비스는 프로세스 [단계에 대한 기본 제공 프로세스를 참조하십시오](/help/sites-developing/workflows-process-ref.md).
      * 프로세스 단계에 대한 ECMAScripts 만들기를 참조하십시오. [ECMAScript를 사용한 프로세스 단계 구현을 참조하십시오](/help/sites-developing/workflows-customizing-extending.md#using-ecmascript).
      * 프로세스 단계에 대한 OSGi 서비스 만들기를 참조하십시오. [Java 클래스를 사용한 프로세스 단계 구현을 참조하십시오](/help/sites-developing/workflows-customizing-extending.md#implementing-a-process-step-with-a-java-class).
   * **처리기 고급**:실행 후 워크플로우를 다음 단계로 자동 진행하려면 이 옵션을 선택합니다. 선택하지 않으면 구현 스크립트가 워크플로우 진행률을 처리해야 합니다.
   * **인수**:프로세스에 전달할 인수입니다.
