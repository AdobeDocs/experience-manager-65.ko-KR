...
기본적으로, 노드 `cq:ClientLibraryFolder` 는 저장소의 `/apps`및 하위 트리 내 `/libs` 의 어느 곳에서나 삽입할 수 있으며, 이러한 기본값 및 기타 설정은 시스템 콘솔 `/etc` 의 **Adobe Granite HTML Library Manager** 패널을 통해 제어할 수 [](https://localhost:4502/system/console/configMgr)있습니다.
...
>[!CAUTION]
>
>`<cq:includeClientLib>`이전에는 클라이언트 라이브러리를 포함하는 데 일반적으로 사용되었던 AEM 5.6 이후 더 이상 사용되지 않습니다. 대신 위에 설명된 대로 사용해야 [ `<ui:includeClientLib>`](/help/sites-developing/taglib.md#lt-ui-includeclientlib) 합니다.
