...
JS, CSS 또는 테마 라이브러리 필터링에 대한 특성을 비롯한 자세한 내용은 [ui:includeClientLib](/help/sites-developing/taglib.md#lt-ui-includeclientlib)을 참조하십시오.

>[!CAUTION]
>
>`<cq:includeClientLib>`이전에는 클라이언트 라이브러리를 포함하는 데 일반적으로 사용되었던 이전 버전은 AEM 5.6 이후 더 이상 사용되지 않습니다. 대신 위에 설명된 대로  [ `<ui:includeClientLib>`](/help/sites-developing/taglib.md#lt-ui-includeclientlib) 사용해야 합니다.
...
>[!NOTE]
>
>클라이언트 라이브러리 폴더 아래의 정적 리소스는 *resources*&#x200B;이라는 폴더에 있어야 합니다. 이미지와 같은 정적 리소스가 없는 경우 폴더 *resources*&#x200B;에 게시 인스턴스에서 참조할 수 없습니다. 다음은 한 예입니다.https://localhost:4503/etc.clientlibs/geometrixx/components/clientlibs/resources/example.gif
...
1. 파일을 열고 다음 텍스트를 입력하여 소스 파일 경로의 루트를 식별합니다.
   ...
   * `[root]`*를 TXT 파일을 기준으로 소스 파일이 포함된 폴더의 경로로 바꿉니다. 예를 들어 소스 파일이 TXT 파일과 동일한 폴더에 있는 경우 다음 텍스트를 사용합니다.
