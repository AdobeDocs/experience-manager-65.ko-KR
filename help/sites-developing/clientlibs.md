...
기본적으로 `cq:ClientLibraryFolder` 노드는 내부의 어느 곳에든 배치할 수 있습니다 `/apps`, `/libs` 및 `/etc` 저장소의 하위 트리(이러한 기본값 및 기타 설정은 **Adobe Granite HTML 라이브러리 관리자** 패널의 [시스템 콘솔](https://localhost:4502/system/console/configMgr)).
...
JS, CSS 또는 테마 라이브러리 필터링을 위한 특성을 포함한 전체 정보는 를 참조하십시오 [ui:includeClientLib](/help/sites-developing/taglib.md#lt-ui-includeclientlib).

>[!CAUTION]
>
>`<cq:includeClientLib>`이전에는 클라이언트 라이브러리를 포함하는 데 일반적으로 사용되던 가 AEM 5.6 이후로 더 이상 사용되지 않습니다. [ `<ui:includeClientLib>`](/help/sites-developing/taglib.md#lt-ui-includeclientlib) 는 위에 자세히 설명된 대로 대신 사용해야 합니다.
...
>[!NOTE]
>
>클라이언트 라이브러리 폴더 아래의 정적 리소스는 *리소스*. 폴더 아래에 이미지와 같은 정적 리소스가 없는 경우 *리소스*&#x200B;게시 인스턴스에서 참조할 수 없습니다. 다음은 한 예입니다. https://localhost:4503/etc.clientlibs/geometrixx/components/clientlibs/resources/example.gif
