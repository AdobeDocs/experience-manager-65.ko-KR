...
템플릿은 유형의 페이지를 만드는 데 사용됩니다 `cq:Page` 앞에서 언급했듯이 페이지는 특별한 유형의 구성 요소입니다. 각 AEM 페이지에는 구조화된 노드가 있습니다 `jcr:content`. 이 기능은
...
1. 기존 템플릿(가급적 원하는 내용에 가까운 정의가 있는 경우)을 새 노드에 복사합니다.
   ...
   >[!NOTE]
   >
   >사용 가능한 템플릿 목록은 새 페이지의 위치와 각 템플릿에 지정된 배치 제한에 따라 다릅니다. 자세한 내용은 [템플릿 가용성](#templateavailibility).
