...
웹 사이트에 대한 기존 템플릿을 활성화하고 **웹 사이트** 콘솔에서 **웹 사이트** 아래에 페이지를 만들 때 **페이지 만들기** 대화 상자에 표시하려면 템플릿 노드의 allowedPaths 속성을 다음으로 설정합니다.**/content(/)*)?**
...
1. 기존 템플릿(가능한 한 가까이 정의하면 원하는 대로 가능)을 새 노드에 복사합니다.
   ...
   >[!NOTE]
   >
   >사용 가능한 템플릿 목록은 새 페이지의 위치 및 각 템플릿에 지정된 배치 제한에 따라 달라집니다. [템플릿 가용성](#templateavailibility)을 참조하십시오.
