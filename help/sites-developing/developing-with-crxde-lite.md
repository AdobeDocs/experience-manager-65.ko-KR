...
>[!NOTE]
>
>AEM 6.5.5.0부터 CRXDE Lite에 대한 익명 액세스가 더 이상 불가능합니다.
...
<table>
 <tbody>
  ...
  <tr>
   <td>탐색기 창</td>
   <td><p>저장소의 모든 노드의 트리를 표시합니다.</p> <p>속성을 표시하려면 노드를 <strong>클릭합니다</strong> . 노드를 클릭하면 도구 모음에서 작업을 선택할 수 있습니다. 노드를 다시 클릭하여 이름을 변경합니다.</p> <p>트리 탐색 필터(빈시계 아이콘): 저장소에 있는 이름에 입력 텍스트가 포함된 노드를 필터링할 수 있습니다. 로컬에 로드된 노드에만 적용됩니다.<br /> </p> </td>
  </tr>
  <tr>
   <td>편집 창</td>
...
1. 이 단계는 선택 사항입니다. 구성 요소 속성 **이 컨테이너임,** 데코레이션 **없음,****셀 이름** 대화 상자 경로 **를**&#x200B;설정합니다. **다음**&#x200B;을 클릭합니다.
...
CRXDE Lite를 사용하면 CND(Compact Namespace 및 [Node Type Definition) 표기법으로 노드 유형 정의를 가져오거나 내보낼 수 있습니다](https://jackrabbit.apache.org/jcr/node-type-notation.html).
