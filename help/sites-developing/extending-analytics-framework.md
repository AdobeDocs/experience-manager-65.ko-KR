...
페이지에 있는 JavaScript는 `s_gi`s_code.js Adobe Analytics 라이브러리에서 정의하는 개체와 해당 속성에 값을 할당합니다. 개체 인스턴스의 이름은 입니다. `s`. 이 섹션에 나와 있는 코드 예제는 이 단원에 대한 몇 가지 참조를 만듭니다 `s` 변수를 채우는 방법에 따라 페이지를 순서대로 표시합니다.
...
Adobe Analytics 내에는 프레임워크에서 구성할 수 있는 다양한 사전 정의된 변수가 있습니다. 다음 **charset**, **cookieLife**, **currencyCode** 및 **trackInlineStats** 변수는에 포함됩니다. **일반 분석 설정** 기본적으로 나열됩니다.
...
>[!CAUTION]
>
>이 목록에 있는 변수 중 일부는 **필수** Adobe Analytics 호출이 올바르게 작동하도록 하려면, (예: **currencyCode**, **charSet**)
>
...
1. 에 Javascript 코드 추가 **s_doPlugins** 함수.
...
모든 Adobe Analytics 프레임워크에 통합된 사용자 지정 JavaScript 코드를 제공합니다. 페이지의 Adobe Analytics 프레임워크에 사용자 지정 항목이 없는 경우 [무형식 javascript](/help/sites-administering/adobeanalytics.md), /libs/cq/analytics/components/sitecatalyst/config.js.jsp 스크립트가 생성하는 javascript가 [analytics.sitecatalyst.js](/help/sites-administering/adobeanalytics.md) 파일. 기본적으로 스크립트는 주석 처리되어 있어서 영향을 주지 않습니다. 이 코드는 을 `s.usePlugins` 끝 `false`:
...
따라서 javascript가 설정되어야 합니다 `s.usePlugins` 끝 `true` 에 있는 모든 코드가 `s_doPlugins` 함수가 실행됩니다. 코드를 사용자 정의하려면 config.js.jsp 파일을 고유한 JavaScript를 사용하는 파일과 오버레이합니다. Javascript에서 Adobe Analytics 플러그인을 사용해야 하는 경우 [통합](/help/sites-administering/adobeanalytics.md) AEM으로.
...
>[!NOTE]
>
>플러그인에 대한 새 클라이언트 라이브러리 폴더를 만듭니다. 플러그인을 목록에 추가하지 마십시오. `/libs/cq/analytics/clientlibs/sitecatalyst/plugins` 폴더를 삭제합니다. 이 방법을 사용하면 `sitecatalyst.plugins` 범주는 AEM 재설치 또는 업그레이드 작업 중에 덮어쓰지 않습니다.
...
1. 방금 만든 클라이언트 라이브러리 폴더를 선택하고 오른쪽 아래 속성 막대를 사용하여 다음 속성을 추가합니다.
...
1. 방금 만든 클라이언트 라이브러리 폴더를 마우스 오른쪽 단추로 클릭하고 만들기 > 파일 만들기를 클릭합니다. 파일 이름에 js.txt를 입력한 다음 확인을 누릅니다.

1. 모두 저장을 클릭합니다.

다음 절차를 사용하여 플러그인 코드를 가져와 AEM 저장소에 코드를 저장하고 클라이언트 라이브러리 폴더에 코드를 추가합니다.

1. 에 로그인 [sc.omniture.com](https://sc.omniture.com) Adobe Analytics 계정을 사용합니다.
...
>[!NOTE]
>
>플러그인을 사용할 때 지원 플러그인도 통합해야 합니다. 그렇지 않으면 플러그인 javascript는 지원 플러그인의 함수에 수행하는 호출을 인식하지 못합니다. 예를 들어 getPreviousValue() 플러그인이 제대로 작동하려면 split() 플러그인이 필요합니다.
>
