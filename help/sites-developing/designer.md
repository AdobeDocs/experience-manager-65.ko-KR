...
>[!CAUTION]
>
>이 문서에서는 클래식 UI를 기반으로 웹 사이트를 만드는 방법에 대해 설명합니다. Adobe은 문서 [AEM Sites 개발 시작](/help/sites-developing/getting-started.md)에 자세히 설명된 대로 웹 사이트에 최신 AEM 기술을 활용할 것을 권장합니다.
...
디자인은 **도구** 탭의 **designs** 섹션에서 정의할 수 있습니다.
...
## {#what-you-will-need}이 필요한 항목

디자인을 실현하려면 다음을 수행해야 합니다.

**CSS**  - CSS 스타일 시트는 페이지의 특정 영역의 형식을 정의합니다.
**이미지**  - 배경, 버튼 등의 기능에 사용하는 이미지입니다.

### 웹 사이트 {#considerations-when-designing-your-website} 디자인 시 고려 사항

웹 사이트를 개발할 때는 이미지 및 CSS 파일을 `/etc/design/<project>` 아래에 저장하는 것이 좋습니다. 그러면 다음 코드 단편에 설명된 것처럼 현재 디자인을 기준으로 리소스를 참조할 수 있습니다.
...
* 웹 사이트의 재설계는 사이트 루트의 디자인 경로를 `design/v1`에서 `design/v2.`(까지)로 가리키면 간단하게 수행할 수 있습니다

* `/etc/designs` 그리고 브라우저 `/content` 가 외부 사용자가 트리 아래에 무엇이 있는지 궁금해하도록 사용자를 보호하는 것을 보는 유일한 외부  `/apps` URL입니다. 위의 URL 혜택은 자산의 노출을 몇 개의 서로 다른 위치로 제한하기 때문에 시스템 관리자가 더 나은 보안을 설정할 수 있도록 도와줍니다.
