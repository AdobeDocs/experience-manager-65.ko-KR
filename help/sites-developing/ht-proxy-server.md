...
각 활성 연결은 가독성을 높이기 위해 들여쓰기됩니다. 기본값은 16 레벨입니다. 이 기능은 `proxy.jar version 1.16`과 함께 도입되었습니다.
...
이것은 6번째 연결에 있는 클라이언트( `C`)와 서버( `S`) 사이에 평균 속도로 전달된 바이트 수를 보여줍니다.
...
AEM 인스턴스는 `test.html` 파일의 컨텐츠와 응답합니다.
