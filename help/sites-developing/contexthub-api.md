...
* **** 옵션:(선택 사항) 쿠키 속성을 구성하는 다음 속성이 들어 있는 개체입니다.
   ...
   * 만료:쿠키가 만료되는 시기를 지정하는 `date` 또는 `number` 값. 날짜 값은 절대 만료 시간을 지정합니다. 숫자(일 단위)는 만료 시간을 현재 시간과 숫자로 설정합니다. 기본값은 `undefined`입니다.
   * 보안:쿠키의 `boolean` 속성을 지정하는 `Secure` 값입니다. 기본값은 `false`입니다.
   * 경로:쿠키의 `String` `Path` 속성으로 사용할 값입니다. 기본값은 `undefined`입니다.
