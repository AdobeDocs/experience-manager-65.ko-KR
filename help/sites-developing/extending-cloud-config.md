...
* 종속성 지원(예:[Adobe Analytics](/help/sites-administering/marketing-cloud.md#integrating-with-adobe-analytics) 플러그인은 [Adobe Analytics](/help/sites-administering/marketing-cloud.md#integrating-with-adobe-analytics) 구성이 필요합니다.
...
및 사용자 지정 구성 요소를 가리키는 `resourceType`을 정의합니다.
...
콘텐츠 모델은 다음 위치에 `cq:Page`으로 저장됩니다.
...
API에 대한 참조 문서는 [com.day.cq.wcm.webservicessupport](https://helpx.adobe.com/experience-manager/6-5/sites/developing/using/reference-materials/javadoc/com/day/cq/wcm/webservicesupport/package-summary.html)을 참조하십시오.
...
숨겨진 양식 필드를 추가하여 이를 실현할 수 있습니다. 이 필드에는 속성 이름에 `@Encrypted` 주석이 있어야 합니다.즉, `password` 필드의 이름은 다음과 같이 작성됩니다.
...
<table>
 <tbody>
  ...
  <tr>
   ...
   <td>페이지에 자동으로 포함되는 구성 요소에 대한 참조 경로입니다.<br /> 이것은 추가 기능 및 JS 포함에 사용됩니다.<br /> 여기에는 포함된 페이지의 구성 요소<br /> <code> cq/cloudserviceconfigs/components/servicecomponents</code><br /> 가 포함됩니다(일반적으로  <code>body</code> 태그 앞).<br /> Analytics 및 Target의 경우 방문자 행동을 추적하기 위한 JavaScript 호출과 같은 추가 기능을 포함하도록 이 기능을 사용합니다.</td>
  </tr>
  <tr>
   <td>설명</td>
