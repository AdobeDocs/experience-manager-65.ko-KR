...
AEM에는 심각도별로 나열된 세 가지 주요 슬로우 쿼리 분류가 있습니다.
...
1. **큰 결과 집합 쿼리**
   ...
   * 매우 많은 수의 결과를 반환하는 쿼리

처음 두 분류의 쿼리(색인 없는 쿼리와 제한 수준이 낮은 쿼리)는 Oak 쿼리 엔진이 각각 검사하도록 하기 때문에 속도가 느립니다 **잠재적** 에 속한 항목을 식별하는 결과(콘텐츠 노드 또는 인덱스 항목) **실제** 결과 집합.
...
AEM 6.3에서는 기본적으로 100,000개 순회에 도달하면 쿼리가 실패하고 예외가 발생합니다. 이 제한은 기본적으로 AEM 6.3 이전 AEM 버전에는 없지만 Apache Jackrabbit Query Engine 설정 OSGi 구성 및 QueryEngineSettings JMX 빈(속성 LimitReads)을 통해 설정할 수 있습니다.

### 인덱스 없는 쿼리 검색 {#detecting-index-less-queries}

#### 개발 중 {#during-development}

설명 **모두** 쿼리 및 쿼리 계획에 **/&amp;ast; 트래버스** 설명. 쿼리 계획 트래버스 예제:
...
이 쿼리는 다음으로 확인됩니다. `cqPageLucene` 색인이지만 다음에 대한 속성 색인 규칙이 없으므로 `jcr:content` 또는 `cq:tags`, 이 제한이 평가되면 의 모든 레코드가 `cqPageLucene` 일치를 결정하기 위해 색인을 선택합니다. 이는 지수가 100만 개를 포함하고 있는 경우 을 의미한다 `cq:Page` 노드를 만든 다음 100만 개의 레코드를 확인하여 결과 세트를 결정합니다.
...
물론, 추가적인 쿼리 제한은 적격한 결과 세트를 감소시키고 쿼리 최적화를 더욱 최적화한다.
...
사후 인덱스 필터링의 또 다른 원인은 개발 중에 종종 누락되는 액세스 제어 목록입니다. 쿼리가 사용자가 액세스할 수 없는 경로를 반환하지 않는지 확인하십시오. 일반적으로 쿼리에 대한 관련 경로 제한을 제공하는 것과 함께 더 나은 콘텐츠 구조를 통해 이 작업을 수행할 수 있습니다.

Lucene 색인이 많은 결과를 반환하여 쿼리 결과로 매우 작은 하위 집합을 반환하는지 식별하는 유용한 방법은 DEBUG 로그를 활성화하는 것입니다. `org.apache.jackrabbit.oak.plugins.index.lucene.LucenePropertyIndex` 색인에서 로드되는 문서 수를 확인합니다. 최종 결과 수와 로드된 문서 수가 불균형하게 달라져서는 안 됩니다. 자세한 내용은 [로깅](/help/sites-deploying/configure-logging.md).
...
oak.queryLimitInMemory에 대해 낮은 임계값을 설정합니다(예: 10000) 및 oak.queryLimitReads(예: 5000) &quot;쿼리가 x개 이상의 노드를 읽음...&quot;이라는 UnsupportedOperationException에 도달할 때 값비싼 쿼리를 최적화합니다.

이렇게 하면 리소스 집약적인 쿼리(예: 어떤 색인에서도 지원하지 않거나 덜 포함되는 색인에서 지원하지 않습니다. 예를 들어, 1M 노드를 읽는 쿼리는 많은 IO를 유발하고 전체 애플리케이션 성능에 부정적인 영향을 줍니다. 따라서 초과 제한으로 인해 실패한 모든 쿼리를 분석하고 최적화해야 합니다.
...
다음 예제에서는 AEM 개발자가 사용하는 가장 일반적인 쿼리 언어인 Query Builder를 사용하지만 JCR-SQL2 및 XPath에도 동일한 원칙이 적용됩니다.
...
* **최적화된 쿼리**
   ...
   `nt:hierarchyNode` 은(는) 의 상위 노드 유형입니다. `cq:Page`, 및 가정 `jcr:content/contentType=article-page` 다음에만 적용됩니다. `cq:Page` 사용자 지정 응용 프로그램을 통한 노드, 이 쿼리는 `cq:Page` 노드 위치 `jcr:content/contentType=article-page`. 하지만 이는 다음과 같은 이유로 인해 차선적인 제한입니다.
...
* **최적화된 쿼리**
   ...
   속성 제한 변경 `jcr:content/contentType` (사용자 지정 값)을 잘 알려진 속성으로 `sling:resourceType` 쿼리가 속성 인덱스로 확인되도록 합니다. `slingResourceType` 모든 콘텐츠 인덱싱 기준 `sling:resourceType`.
...
* **최적화된 쿼리**
   ...
   에서 경로 제한 범위 지정 `path=/content`끝 `path=/content/my-site/us/en` 인덱스를 통해 검사해야 하는 인덱스 항목 수를 줄일 수 있습니다. 쿼리가 경로를 매우 잘 제한할 수 있는 경우 그 이상은 `/content` 또는 `/content/dam`: 인덱스에 다음을 포함하는지 확인합니다. `evaluatePathRestrictions=true`.

   메모 사용 `evaluatePathRestrictions` 색인 크기를 늘립니다.

1. 가능하면 다음과 같은 쿼리 함수/작업을 피하십시오. `LIKE` 및 `fn:XXXX` 제한 기반 결과 수에 따라 비용이 증가합니다.
...
1. ***이 조정은 Query Builder에만 해당되며 JCR-SQL2 또는 XPath에는 적용되지 않습니다.***
   ...
   * **최적화된 쿼리**
   ...
   `p.guessTotal=100` query Builder에서 처음 100개의 결과만 수집하고, 결과가 하나 이상 있는지(하지만 이 숫자를 카운트하면 속도가 느려지므로 몇 개가 더 있는지 아님) 나타내는 부울 플래그를 설정하도록 지시합니다. 이 최적화는 결과의 하위 집합만 점진적으로 표시되는 페이지 매김 또는 무한 로드 사용 사례에 적합합니다.
...
1. XPath(또는 JCR-SQL2)를에 제공합니다 [Oak 색인 정의 생성기](https://oakutils.appspot.com/generate/index) 를 클릭하여 최적화된 Lucene 속성 인덱스 정의를 생성합니다.
...
1. 생성된 정의를 추가 방식으로 기존 Lucene 속성 인덱스에 수동으로 병합합니다. 기존 구성은 다른 쿼리를 충족하는 데 사용될 수 있으므로 제거하지 않도록 주의하십시오.
   ...
   1. AEM 색인 재지정 우수 사례에 따라 기존 컨텐츠가 이 색인 구성 변경에 의해 영향을 받는지 여부에 따라 새로 고침 또는 색인 재지정을 순서대로 수행합니다.
...
1. 생성된 Lucene 속성 인덱스 정의를 배포합니다.
   ...
   이 색인의 초기 배포 시 AEM은 색인을 필수 데이터로 채웁니다.

## 인덱스 없는 쿼리와 트래버스 쿼리는 언제 정상입니까? {#when-index-less-and-traversal-queries-are-ok}

AEM의 유연한 콘텐츠 아키텍처로 인해, 콘텐츠 구조의 순회가 시간이 지남에 따라 용납할 수 없을 정도로 커지지 않도록 예측하고 보장하는 것은 어렵습니다.

따라서 경로 제한과 노드 유형 제한의 조합이 다음을 보장하는 경우를 제외하고 색인이 쿼리를 충족하는지 확인합니다. **20개 미만의 노드가 트래버스됩니다.**
...
* **Query Builder 디버거**
   ...
   * AEM에 위치 [/libs/cq/search/content/querydebug.html](http://localhost:4502/libs/cq/search/content/querydebug.html)

* **CRXDE Lite - 쿼리 도구**

   * XPath 및 JCR-SQL2 쿼리를 실행하기 위한 WebUI.
   * AEM에 위치 [/crx/de/index.jsp](http://localhost:4502/crx/de/index.jsp) > 도구 > 쿼리...
...
* **[색인 관리자](/help/sites-administering/operations-dashboard.md#the-index-manager)**
   ...
   * AEM 인스턴스에 인덱스를 표시하는 AEM Operations WebUI를 통해 어떤 인덱스가 이미 있는지 쉽게 이해하고 타깃팅하거나 늘릴 수 있습니다.
