...
>* 각 행은 다음과 같은 노드 또는 속성을 정의합니다.
   >  `N:<name> [<nodeType>]` : 이름이 인 노드를 설명합니다. `<*name*>` 및 노드 유형 `<*nodeType*>`*.*
   >  `P:<name> [<propertyType]` : 이름이 `<*name*>` 및 속성 유형 `<*propertyType*>`.
   >  `P:<name> = <value>` : 속성에 대해 설명합니다. `<name>` 값을 `<value>`.
>
>* 들여쓰기는 노드 간의 계층 구조를 보여 줍니다.
>* 항목 구분 | 가능한 항목의 목록을 나타냅니다. 예를 들어 유형 또는 이름이 있습니다. 예 `String|String[]` 는 속성이 String 또는 String일 수 있음을 의미합니다.[].
>
...
* 는 [CQ QueryBuilder](https://helpx.adobe.com/experience-manager/6-5/sites/developing/using/reference-materials/javadoc/com/day/cq/search/QueryBuilder.html).
...
이 대화 상자는 표준 CQ입니다 [대화 상자](/help/sites-developing/components-basics.md#dialogs) 및 는 다음과 같이 구성할 수 있습니다( [CQ.Dialog](https://helpx.adobe.com/experience-manager/6-5/sites/developing/using/reference-materials/widgets-api/index.html?class=CQ.Dialog) 추가 정보).
