...
David은 2010년 Adobe이 인수한 글로벌 컨텐츠 관리 및 컨텐츠 인프라 소프트웨어의 선두 공급업체인 Day Software AG의 공동 설립자이자 CTO였습니다. 현재 Adobe에서 엔터프라이즈 기술 담당 부사장으로 근무하고 있으며, 컨텐츠 관리를 위한 기술 표준인 Java™ JCR(Content Repository) API(Application Programming Interface) 인 JSR-170의 개발도 주도하고 있습니다.
...
관계형 세계에서 소프트웨어 산업은 데이터를 모델링하는 방법에 대한 많은 경험을 가지고 있지만 컨텐츠 리포지토리 공간은 아직 초기 단계입니다.

콘텐츠를 어떻게 모델링해야 하는지에 대한 의견을 표현하는 것으로 이 공백을 메우기 시작하고 싶은데, 이는 언젠가는 개발자 커뮤니티에 더 의미 있는 것으로 졸업할 수 있기를 바랍니다. 이는 단순히 &quot;내 의견&quot;이 아니라 더 일반적으로 적용할 수 있는 것입니다. 그러니 빨리 진화하는 제 첫 번째 시도에 대해 생각해 보세요.
...
콘텐츠 계층 구조는 중요한 자산입니다. 그러니까 그냥 그렇게 되도록 내버려두지 말고 디자인하세요. 사람이 인식할 수 있는 &quot;좋은&quot; 이름이 없는 노드의 경우 다시 고려해야 할 사항입니다. 임의의 숫자는 결코 &quot;좋은 이름&quot;이 아니다.

기존의 관계적 모델을 위계적 모델에 신속하게 투입하는 것이 수월할 수 있지만, 그 과정에서 어떤 사고를 넣어야 한다.

내 경험에 따르면, 액세스 제어 및 컨테인먼트에 대해 고려한다면 일반적으로 콘텐츠 계층 구조에 적합한 드라이버입니다. 파일 시스템인 것처럼 생각하십시오. 파일 및 폴더를 사용하여 로컬 디스크에서 모델링할 수도 있습니다.

개인적으로는 처음에는 노드 타이핑 시스템보다 계층 규칙을 선호하고 나중에 타이핑을 소개한다.
...
XML을 통해 디자인되고 표현되므로 JCR에게 유용한 데이터 구조와의 호환성을 위해 SNS(Same Name Sidelines)가 사양에 도입된 반면, SNS는 저장소에 대한 상당한 오버헤드와 복잡성을 제공합니다.
...
참조는 참조 무결성을 의미합니다. 참조는 참조 무결성을 관리하는 저장소에 추가 비용을 추가할 뿐만 아니라 콘텐츠 유연성의 관점에서 비용이 많이 든다는 점을 이해하는 것이 중요합니다.

개인적으로 저는 매달려 있는 참조를 처리할 수 없을 때에만 참조를 사용하고, 그렇지 않으면 경로, 이름 또는 문자열 UUID를 사용하여 다른 노드를 참조합니다.
...
예를 들어 &quot;Document&quot; 유형의 노드를 잠재적으로 참조할 수 있도록 하려는 이유만으로 &quot;Document&quot; 노드 유형이 &quot;Document&quot;의 모든 인스턴스에 동적으로 추가될 수 있으므로 정적 방식으로 mix:referenceable에서 확장되어야 한다는 의미는 아닙니다.
