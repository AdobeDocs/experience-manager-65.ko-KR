...
또한 ContextHub 도구 모음이 미리 보기 모드에 표시되는지를 구성해야 합니다. [ContextHub UI](ch-configuring.md#showing-and-hiding-the-contexthub-ui) 표시 및 숨기기 를 참조하십시오.
...
모든 저장소 유형은 [`ContextHub.Store.Core`](contexthub-api.md#contexthub-store-core) 클래스의 확장입니다. 새 저장소 유형을 만드는 방법에 대한 내용은 [사용자 지정 저장소 만들기](ch-extend.md#creating-custom-store-candidates)를 참조하십시오. 샘플 저장소 유형에 대한 자세한 내용은 [샘플 ContextHub 저장소 후보](ch-samplestores.md)를 참조하십시오.
...
* 먼저 [지리적 위치](ch-samplestores.md#contexthub-geolocation-sample-store-candidate) 스토어를 사용하여 사용자의 위도를 결정합니다.
...
로그 생성을 포함하여 ContextHub를 디버깅하는 여러 가지 옵션이 있습니다. 자세한 내용은 [ContextHub 구성 을 참조하십시오.](ch-configuring.md#logging-debug-messages-for-contexthub)
