...
다음을 사용하는 것도 고려해야 합니다. [반응형 디자인](/help/sites-developing/responsive.md), 여러 화면 크기를 수용하는 단일 사이트 생성.
...
MSM(Multi Site Manager)을 사용하여 표준 사이트에서 모바일 라이브 카피를 만듭니다. 표준 사이트는 자동으로 모바일 사이트로 변환됩니다. 모바일 사이트에는 모바일 사이트의 모든 기능(예: 에뮬레이터 내의 에디션)이 있으며 표준 사이트와 동기화하여 관리할 수 있습니다. 섹션을 참조하십시오 [다른 채널을 위한 라이브 카피 만들기](/help/sites-administering/msm.md) 다중 사이트 관리자 페이지에서 참조할 수 있습니다.

## 서버측 모바일 API {#server-side-mobile-api}

모바일 클래스가 포함된 Java 패키지는 다음과 같습니다.

* [com.day.cq.wcm.mobile.api](https://helpx.adobe.com/experience-manager/6-5/sites/developing/using/reference-materials/javadoc/com/day/cq/wcm/mobile/api/device/capability/package-summary.html) - MobileConstant를 정의합니다.
...
* [com.day.cq.wcm.mobile.core](https://helpx.adobe.com/experience-manager/6-5/sites/developing/using/reference-materials/javadoc/com/day/cq/wcm/mobile/core/package-summary.html) - WCM Mobile을 중심으로 순환하는 다양한 유틸리티 메서드를 제공하는 MobileUtil을 정의합니다.
...
모바일 장치 에뮬레이터는 일반 에뮬레이터 프레임워크를 기반으로 합니다. 자세한 내용은 다음을 참조하십시오 [에뮬레이터](/help/sites-developing/emulators.md) 페이지를 가리키도록 업데이트하는 중입니다.

디바이스 에뮬레이터는 모바일 디바이스를 페이지에 표시하는 반면 일반적인 편집(parsys, components)은 디바이스의 화면 내에서 발생합니다. 장치 에뮬레이터는 사이트에 대해 구성된 장치 그룹에 따라 다릅니다. 여러 에뮬레이터를 장치 그룹에 할당할 수 있습니다. 그런 다음 콘텐츠 페이지에서 모든 에뮬레이터를 사용할 수 있습니다. 기본적으로 사이트에 할당된 첫 번째 장치 그룹에 할당된 첫 번째 에뮬레이터가 표시됩니다. 에뮬레이터는 페이지 상단의 에뮬레이터 회전 메뉴 또는 사이드 킥의 편집 버튼을 통해 전환할 수 있습니다.

**에뮬레이터 만들기**

에뮬레이터를 만들려면 다음을 참조하십시오. [사용자 지정 모바일 에뮬레이터 만들기](/help/sites-developing/emulators.md) 섹션에 자세히 설명되어 있습니다.

**모바일 에뮬레이터의 주요 특성**

* 장치 그룹은 하나 이상의 에뮬레이터로 구성됩니다. 장치 그룹 구성 페이지(예: /etc/mobile/groups/touch)에는 다음이 포함됩니다. `emulators` 아래 속성 `jcr:content` 노드.
...
* **스마트** 기본 HTML 및 이미지를 지원하지만 JavaScript는 지원하지 않는 Blackberry와 같은 장치의 전화기.
...
1. 브라우저에서 **도구** 콘솔.
1. 아래에 새 페이지 만들기 **도구** > **모바일** > **장치 그룹**. 다음에서 **페이지 만들기** 대화 상자:
...
앞에서 설명한 대로 사용자 지정 CSS를 디자인 페이지의 CSS와 마찬가지로 장치 그룹 페이지와 연결할 수 있습니다.이 CSS는 작성자 및 게시자에 대한 페이지 콘텐츠의 장치 그룹별 렌더링에 영향을 주는 데 사용됩니다.그러면 이 CSS가 자동으로 포함됩니다.

* 이 장치 그룹에서 사용하는 모든 에뮬레이터에 대해 작성자 인스턴스의 페이지에서 를 클릭합니다.
* 요청의 사용자 에이전트가 이 특정 장치 그룹의 모바일 장치와 일치하는 경우 게시 인스턴스의 페이지에서 입니다.
...
이 섹션에서는 페이지 렌더링을 최적화하거나 클라이언트에 대체 웹 사이트 버전을 제공하기 위해 AEM의 장치 클라이언트측 감지를 사용하는 방법에 대해 설명합니다.

AEM은 다음을 기반으로 장치 클라이언트측 감지 지원 `BrowserMap`. `BrowserMap` AEM에서 클라이언트 라이브러리로 제공됩니다. `/etc/clientlibs/browsermap`.

`BrowserMap` 은 고객에게 대체 웹 사이트를 제공하는 데 사용할 수 있는 세 가지 전략을 제공하며, 이 전략은 다음 순서로 적용됩니다.
...
>[!NOTE]
>
>클라이언트 라이브러리 통합에 대한 자세한 내용은 [클라이언트측 HTML 라이브러리 사용](/help/sites-developing/clientlibs.md) 섹션.

### 대체 링크 제공 {#providing-alternate-links}

다음 `PageVariantsProvider` OSGi 서비스는 동일한 패밀리에 속하는 사이트에 대한 대체 링크를 생성할 수 있습니다. 서비스에서 고려할 사이트를 구성하려면 다음을 수행하십시오. `cq:siteVariant` 노드를 다음에 추가해야 함 `jcr:content` 사이트 루트의 노드입니다.

다음 `cq:siteVariant` 노드에는 다음 속성이 있어야 합니다.

* `cq:childNodesMapTo` - 하위 노드가 매핑될 링크 요소의 속성을 결정합니다. 루트 노드의 하위 노드가 글로벌 웹 사이트의 언어 변형에 대한 루트를 나타내도록 웹 사이트의 콘텐츠를 구성하는 것이 좋습니다(예: `/content/mysite/en`, `/content/mysite/de`). 이 경우 의 값은 `cq:childNodesMapTo` 다음이어야 함: `hreflang`;
...
대체 링크를 사용하지 않으려면 각각에 대해 글로벌 URL을 구성할 수 있습니다 `DeviceGroup`. 를 임베드하는 고유한 클라이언트 라이브러리를 만드는 것이 좋습니다. `browsermap.standard` 클라이언트 라이브러리이지만 장치 그룹을 재정의합니다.

BrowserMap은 이름이 같은 새 장치 그룹을 만들고 추가함으로써 장치 그룹 정의를 재정의할 수 있도록 설계되었습니다. `BrowserMap` 사용자 지정된 클라이언트 라이브러리의 객체입니다.

>[!NOTE]
>
>자세한 내용은 다음을 참조하십시오. [사용자 지정된 브라우저 맵](#creatingacustomisedbrowsermap) 섹션.

### 선택기 기반 URL 정의 {#defining-selector-based-urls}

에 대한 대체 사이트를 표시하기 위해 이전 메커니즘 중 어느 것도 사용되지 않는 경우 `BrowserMap`을 클릭한 다음 의 이름을 사용할 선택기 `DeviceGroups` 이(가)에 추가됩니다. `URL`s. 이 경우 요청을 처리할 고유한 서블릿을 제공해야 합니다.
...
페이지에서 표준 BrowserMap 클라이언트 라이브러리를 사용하려면 `/libs/wcm/core/browsermap/browsermap.jsp` 를 사용하는 파일 `cq:include`페이지의 `head` 섹션.
...
맞춤화된 항목을 만들면 `BrowserMap` 클라이언트 라이브러리에서는 다음과 같은 방법을 권장합니다.
...
를 추가하여 장치가 웹 사이트의 특정 버전을 테스트하도록 강제할 수 있습니다. `device` 매개 변수를 URL에 추가합니다. 다음 URL은 Geometrixx Outdoors 웹 사이트의 모바일 버전을 렌더링합니다.
...
재정의된 장치 값은 쿠키에 저장되므로 을 추가하지 않고 웹 사이트를 탐색할 수 있습니다. `device` 매개 변수 `URL`.

따라서 동일한 을 호출해야 합니다. `URL` (으)로 `device` 을 로 설정 `browser` 웹 사이트의 데스크탑 버전으로 돌아가려면

>[!NOTE]
>
>BrowserMap은 재정의된 장치 값을 라는 쿠키에 저장합니다. `BMAP_device`. 이 쿠키를 삭제하면 CQ가 현재 장치(예: 데스크탑 또는 모바일)에 따라 적절한 웹 사이트 버전을 제공합니다.
...
1. 작성 모드를 비활성화하여 작성자 인스턴스의 모바일 페이지를 요청합니다. 예:
...
모바일 페이지는 일반적으로 Dispatcher에서 캐시할 수 있는데, 이는 장치 그룹에 대해 렌더링되는 페이지가 장치 그룹 선택기에 의해 페이지 URL에서 구분되기 때문입니다 `/content/mobilepage.touch.html`. 선택기가 없는 모바일 페이지에 대한 요청은 캐시되지 않습니다. 이 경우 디바이스 감지가 작동하고 최종적으로 일치하는 디바이스 그룹(또는 해당 문제에 대한 &quot;nomatch&quot;)으로 리디렉션됩니다. 장치 그룹 선택기로 렌더링된 모바일 페이지는 링크 재작성기에 의해 처리되며, 링크 재작성기는 페이지 내의 모든 링크를 장치 그룹 선택기를 포함하도록 재작성하여 이미 자격이 있는 페이지를 클릭할 때마다 장치 감지를 다시 수행하지 않도록 합니다.
...
이 문제를 해결하려면 페이지에 간단한 선택 UI를 포함할 수 있습니다. 이 UI에서 최종 사용자는 AEM에서 선택한 장치 그룹을 무시할 수 있습니다. 위의 예에서 페이지의 링크(또는 아이콘)는 최종 사용자가 `coolpage.touch.html` 만약 그가 자신의 장치가 그것에 충분하다고 생각한다면.
