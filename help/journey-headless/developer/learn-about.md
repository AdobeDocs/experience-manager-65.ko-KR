...
일반적으로 클라이언트측에서 콘텐츠를 렌더링합니다. 일반적으로 이는 누군가 모바일 장치에서 콘텐츠를 호출하고, CMS가 콘텐츠를 제공한 다음, 모바일 장치(클라이언트)가 사용자가 제공한 콘텐츠를 렌더링할 책임이 있음을 의미합니다. 디바이스가 이전 버전이거나 속도가 느린 경우 디지털 경험도 마찬가지로 속도가 느립니다.
...
이 지식을 기반으로 다음 문서인 [AEM Headless 시작하기](getting-started.md)를 검토하여 AEM Headless 번역 여정을 계속하십시오. 여기에서는 필요한 도구를 설정하는 방법과 Headless 콘텐츠 게재 및 사전 요구 사항에 접근하는 방법에 대해 어떻게 생각하고 있는지에 대해 알아보게 됩니다.
