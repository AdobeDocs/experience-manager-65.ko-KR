...
1. **탐색** > **프로젝트**&#x200B;로 이동합니다.
1. 클릭 **만들기** > **폴더** 을 클릭하여 프로젝트용 폴더를 만듭니다.
   * 이 작업은 옵션이지만 번역 작업을 구성하는 데 도움이 됩니다.
1. 다음에서 **폴더 만들기** 창, 추가 **제목** 을 클릭하여 폴더를 찾은 다음 **만들기**.

   ![프로젝트 폴더 만들기](assets/create-project-folder.png)

1. 폴더를 클릭하여 폴더를 엽니다.
1. 새 프로젝트 폴더에서 **만들기** > **프로젝트**.
1. 프로젝트는 템플릿을 기반으로 합니다. 다음을 클릭합니다. **번역 프로젝트** 템플릿을 선택하고 **다음**.
...
1. 다음에서 **고급** 탭에서 **타겟 언어** 드롭다운에서 콘텐츠를 번역할 언어를 선택합니다. **만들기**&#x200B;를 클릭합니다.

   ![프로젝트 고급 탭](assets/project-advanced-tab.png)

1. 클릭 **열기** 확인 대화 상자에서 확인할 수 있습니다.
...
* **번역 작업** - 이 카드 또는 이 카드는 상태, 에셋 수 등을 포함하여 실제 번역 작업에 대한 개요를 제공합니다. 일반적으로 언어당 작업 이름에 ISO-2 코드가 추가된 하나의 작업이 있습니다.
...
1. 작업에서 라인 항목을 삭제하거나 콘텐츠 조각 또는 에셋 콘솔에서 볼 수 있는 옵션과 같은 추가 옵션을 보려면 라인 항목의 선택 확인란을 클릭하십시오.
   ![번역 작업 옵션](assets/translation-job-options.png)

일반적으로 번역 작업의 콘텐츠는 **번역 작업** 창의 **상태** 열에 표시된 **초안** 상태에서 시작합니다.

번역 작업을 시작하려면 번역 프로젝트 개요로 돌아가 의 맨 위에서 V자 버튼을 클릭합니다. **번역 작업** 카드 및 선택 **시작**.
...
1. 다음 중 하나의 아래에 있는 줄임표 버튼을 클릭합니다. **번역 작업** 카드.
1. 작업에 콘텐츠가 없는지 확인합니다. 다음을 클릭합니다. **추가** 창 맨 위에 있는 단추 **에셋/페이지** 드롭다운에서 을 클릭합니다.
...
1. 클릭 **선택** 을 눌러 선택한 컨텐츠를 작업에 추가합니다.
...
>[!TIP]
>
>경로 브라우저는 콘텐츠를 검색하고, 필터링하고, 탐색할 수 있는 강력한 도구입니다. 다음을 클릭합니다. **컨텐츠 전용/필터** 단추를 사용하여 사이드 패널을 토글하고 다음과 같은 고급 필터를 표시합니다. **수정한 날짜** 또는 **번역 상태**.
>
...
[이전에 본 바와 같이,](#using-translation-project) 기계 번역된 콘텐츠가 상태로 AEM으로 다시 흐릅니다. **승인됨** 기계번역이 활용되고 있기 때문에 사람의 개입이 필요 없다는 가정이다. 그러나 번역된 콘텐츠를 계속 검토할 수 있습니다.
