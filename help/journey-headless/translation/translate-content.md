...
AEM Headless 번역 여정의 이전 문서인 [번역 규칙 구성](translation-rules.md)에서는 AEM의 번역 규칙을 사용하여 번역 콘텐츠를 식별하는 방법에 대해 알아보았습니다. 이제
...
이 문서는 커넥터 및 번역 규칙과 함께 AEM의 번역 프로젝트를 사용하여 콘텐츠를 번역하는 방법을 이해하는 데 도움이 됩니다. 이 문서를 읽고 나면 다음을 수행해야 합니다.
...
두 방법 모두 번역을 수행하는 담당자에 따라 다른 올바른 접근 방식입니다.
...
1. **탐색** > **프로젝트**&#x200B;로 이동합니다.
1. 프로젝트 폴더를 만들려면 **만들기** > **폴더**&#x200B;를 클릭합니다.
   * 이 작업은 옵션이지만 번역 작업을 구성하는 데 도움이 됩니다.
1. **폴더 만들기** 창에서 폴더에 대한 **제목**&#x200B;을 추가한 다음 **만들기**&#x200B;를 클릭합니다.

   ![프로젝트 폴더 만들기](assets/create-project-folder.png)

1. 폴더를 클릭하여 폴더를 엽니다.
1. 새 프로젝트 폴더에서 **만들기** > **프로젝트**&#x200B;을(를) 클릭합니다.
1. 프로젝트는 템플릿을 기반으로 합니다. **번역 프로젝트** 템플릿을 클릭하여 선택한 다음 **다음**&#x200B;을 클릭합니다.
...
1. **고급** 탭에서 **타겟 언어** 드롭다운을 사용하여 콘텐츠를 번역할 언어를 선택합니다. **만들기**&#x200B;를 클릭합니다.

   ![프로젝트 고급 탭](assets/project-advanced-tab.png)

1. 확인 대화 상자에서 **열기**&#x200B;를 클릭합니다.
...
* **번역 작업** - 이 카드는 상태, 에셋 수 등을 포함하여 실제 번역 작업에 대한 개요를 제공합니다. 일반적으로 언어당 작업 이름에 ISO-2 코드가 추가된 하나의 작업이 있습니다.
...
1. **번역 작업** 카드 하단의 생략 부호를 클릭합니다.
1. **번역 작업** 창에 작업에 있는 모든 항목이 나열됩니다.
   ![번역 작업 세부 정보](assets/translation-job-detail.png)
1. 한 라인이 번역할 여러 콘텐츠 항목을 나타낼 수 있다는 점을 염두에 두고 해당 라인의 세부 정보를 보려면 라인을 클릭합니다.
1. 작업에서 라인 항목을 삭제하거나 컨텐츠 조각 또는 Assets 콘솔에서 볼 수 있는 옵션과 같은 추가 옵션을 보려면 라인 항목의 선택 확인란을 클릭하십시오.
   ![번역 작업 옵션](assets/translation-job-options.png)

일반적으로 번역 작업의 콘텐츠는 **번역 작업** 창의 **상태** 열에 표시된 **초안** 상태에서 시작합니다.

번역 작업을 시작하려면 번역 프로젝트 개요로 돌아가 **번역 작업** 카드 상단의 V자 버튼을 클릭하고 **시작**&#x200B;을 선택합니다.
...
번역 프로젝트를 수동으로 생성할 때, AEM에서는 필요한 작업이 생성되지만 포함될 콘텐츠는 자동으로 선택되지 않습니다. 이렇게 하면 번역 프로젝트 관리자가 번역할 콘텐츠를 유연하게 선택할 수 있습니다.

번역 작업에 콘텐츠를 추가하려면 다음 작업을 수행하십시오.

1. **번역 작업** 카드 중 하나에서 하단의 생략 부호를 클릭합니다.
1. 작업에 콘텐츠가 없는지 확인합니다. 창 맨 위에 있는 **추가** 단추를 클릭한 다음 드롭다운에서 **Assets/페이지**&#x200B;를 클릭합니다.

   ![빈 번역 작업](assets/empty-translation-job.png)

1. 경로 브라우저가 열려 추가할 콘텐츠를 구체적으로 선택할 수 있습니다. 콘텐츠를 찾은 다음 을(를) 클릭하여 선택합니다.

   ![경로 브라우저](assets/path-browser.png)

1. 선택한 콘텐츠를 작업에 추가하려면 **선택**&#x200B;을 클릭합니다.
...
>[!TIP]
>
>경로 브라우저는 콘텐츠를 검색하고, 필터링하고, 탐색할 수 있는 강력한 도구입니다. **콘텐츠 전용/필터** 단추를 클릭하여 측면 패널을 전환하고 **수정된 날짜** 또는 **번역 상태**&#x200B;와 같은 고급 필터를 표시합니다.
>
>[추가 리소스 섹션](#additional-resources)에서 경로 브라우저에 대해 자세히 알아볼 수 있습니다.

이전 단계를 사용하여 프로젝트의 모든 언어(작업)에 필요한 콘텐츠를 추가할 수 있습니다. 모든 콘텐츠를 선택한 후에는 번역을 시작할 수 있습니다.
...
해당 아이콘을 클릭하면 편집기 콘솔에서 번역된 콘텐츠 조각을 열어 번역된 콘텐츠의 세부 정보를 볼 수 있습니다.
...
**탐색** > **Assets** > **파일**&#x200B;을 통해 이 폴더로 이동하면 번역된 콘텐츠를 볼 수 있습니다.
...
이제 Headless 번역 여정의 이 부분을 완료했으므로 다음 작업을 수행할 수 있습니다.
