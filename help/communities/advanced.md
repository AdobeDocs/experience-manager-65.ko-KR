...
* 기본 및 고급 점수 및 배지 규칙은 다음과 같습니다 [컨텐츠에 적용됨](/help/communities/implementing-scoring.md#apply-rules-to-content) 같은 방식으로

   * 기본 및 고급 점수 및 배지 규칙은 동일한 콘텐츠에 적용할 수 있습니다.

* [구성 요소에 대한 배지 활성화](/help/communities/implementing-scoring.md#enable-badges-for-component) 는 일반적입니다.
...
* 고급 배지 규칙:
   ...
   * `badgingLevels` 설정 **포상에 대한 전문가 레벨 수**
   * 필요한 경우 `badgingPaths` 임계값 배열 매핑 대신 배지에 대한 배지의 배열입니다.
...
* **점수 가중치**
   ...
   항목의 경우 점수를 계산할 때 가장 높은 우선 순위를 지정해야 하는 동사를 지정합니다. 하나 이상의 항목을 입력할 수 있지만 **주제 당 하나의 동사**. 자세한 내용은 [항목 및 동사](/help/communities/implementing-scoring.md#topics-and-verbs).
...
* **점수 증가율**
성장 속도가 느려져 전문가 수를 제한하게 되는 0과 점수 범위 사이의 점수를 지정합니다.
...
자세한 내용은 [점수 하위 규칙](/help/communities/implementing-scoring.md#scoring-sub-rules).
...
고급 배지 규칙 속성은 [기본 배지 규칙 속성](/help/communities/implementing-scoring.md#badging-rules).
...
<table>
 <tbody>
  ...
  <tr>
   <td>badgingPath</td>
   <td>문자열[]</td>
   <td><em>(필수)</em> 배지 이미지의 최대 개수까지 배지 이미지의 다중 값 문자열입니다. 배지 이미지 경로는 먼저 가장 높은 전문가에게 부여되도록 정렬해야 합니다. 배지 수준이 나타내는 것보다 적은 배지가 있는 경우 배열의 마지막 배지가 나머지 배열에 채워집니다. 항목 예:<br /> <code>/libs/settings/community/badging/images/expert-badge/jcr:content/expert.png</code></td>
  </tr>
  <tr>
   <td>badgingLevels</td>
   <td>긴</td>
   <td><em>(선택 사항)</em> 받을 전문 지식 수준을 지정합니다. 예를 들어 <code>expert </code>그리고 <code>almost expert</code> (두 개의 배지)를 선택한 경우 값을 2로 설정해야 합니다. badgingLevel은 badgingPath 속성에 대해 나열된 전문가 관련 배지 이미지 수와 일치해야 합니다. 기본값은 1입니다.</td>
  </tr>
  <tr>
   <td>badgingType</td>
   <td>문자열</td>
  ...
  <tr>
   <td>scoringRules</td>
   <td>문자열[]</td>
   <td><em>(선택 사항)</em> 나열된 점수 규칙으로 식별된 점수 이벤트로 배지 규칙을 제한하는 다중 값 문자열입니다.<br /> 항목 예:<br /> <code>/libs/settings/community/scoring/rules/adv-comments-scoring</code><br /> 기본값은 제한이 없습니다.</td>
  </tr>
 </tbody>
</table>
...
베타 릴리스에 포함된 는 [포럼 함수](/help/communities/functions.md#forum-function) (포럼 기능과 포럼 기능의 댓글 구성 요소에 대해 각각 하나씩):
