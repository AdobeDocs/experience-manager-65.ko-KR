...
* 파일 편집 `/etc/my.cnf`
* 에서 `[mysqld]` 섹션에서 다음 줄을 추가합니다. `lower_case_table_names = 1`
...
* 파일 편집 `/etc/my.cnf`
* 에서 `[client]` 섹션, 추가: `default-character-set=utf8`
* 에서 `[mysqld]` 섹션, 추가: `character-set-server=utf8`
...
1. 대화 상자에서 `Setup New Connection`를 채울 때는 동일한 서버에 작성자 AEM 인스턴스와 MySQL을 사용하여 데모 목적으로 플랫폼에 적합한 값을 입력합니다.
   ...
   * 기본 스키마: `leave blank`
1. 선택 `Test Connection` 를 눌러 실행 중인 MySQL 서비스에 대한 연결을 확인합니다.
...
* 을(를) 선택합니다 `jcr:content` sql 파일에 대한 노드입니다.
...
* 다음 값을 입력합니다.
   ...
   * **[!UICONTROL 데이터 소스 이름]**: 에 입력한 이름 [MySQL 연결](#new-connection-settings)예: &#39;활성화&#39;.
...
* 을(를) 선택합니다 `[+]` 안전한 사용자 에이전트를 추가하는 아이콘
* Enter 키 `Mozilla/*`.
* **[!UICONTROL 저장]**&#x200B;을 선택합니다.
