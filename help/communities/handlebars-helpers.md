...
# SCF 핸들막대 도움말 도우미 {#scf-handlebars-helpers}
...
AEM Communities과 함께 제공되는 사용자 지정 SCF 도우미는 [클라이언트 라이브러리](../../help/sites-developing/clientlibs.md)에서 정의됩니다.
...
문자열 구분 모드 목록에 대해 [WCM mode](https://helpx.adobe.com/experience-manager/6-5/sites/developing/using/reference-materials/javadoc/com/day/cq/wcm/api/WCMMode.html)의 현재 값을 테스트하는 블록 도우미입니다.
...
* **모드**:문자열
   ...
   (선택 사항) 설정되면 테스트할 [WCM 모드](https://helpx.adobe.com/experience-manager/6-5/sites/developing/using/reference-materials/javadoc/com/day/cq/wcm/api/WCMMode.html)의 쉼표로 구분된 목록입니다.
...
이를 통해 JCR 노드로 추가된 리소스에 대해 가능한 것보다 프로그래밍 방식으로 보다 손쉽게 리소스를 사용자 지정할 수 있습니다. [커뮤니티 구성 요소 추가 또는 포함](scf.md#add-or-include-a-communities-component)을 참조하십시오.

일부 Communities 구성 요소만 포함할 수 있습니다. AEM 6.1의 경우, 포함할 수 있는 댓글은 [comments](essentials-comments.md), [등급](rating-basics.md), [평가](reviews-basics.md) 및 [투표](essentials-voting.md)입니다.

서버 쪽에만 해당하는 이 도우미는 JSP 스크립트용 [cq:include](../../help/sites-developing/taglib.md)와 유사한 기능을 제공합니다.
...
* **컨텍스트**:문자열 또는 개체
   ...
   `this.id`을(를) 사용하여 `id`에서 리소스를 가져와 요청된 resourceType을 렌더링합니다.
...
서버 쪽에만 해당하는 이 도우미는 JSP 스크립트용 [ui:includeClientLib](../../help/sites-developing/taglib.md)과 유사한 기능을 제공합니다.
...
* Handlebars 도우미 호출은 0개 이상의 공백으로 구분된 매개 변수가 뒤에 오는 간단한 식별자(도우미의 *name*)입니다.
...
* 현재 컨텍스트인 `this`은 항상 핸들바 도우미에게 사용할 수 있습니다.
* 컨텍스트는 문자열, 숫자, 부울 또는 JSON 데이터 개체일 수 있습니다.
* 현재 컨텍스트 내에 중첩된 개체를 컨텍스트로 전달할 수 있습니다(예: `this.url` 또는 `this.id`)(간단한 및 블록 도움말의 다음 예 참조).

* 블록 도우미는 템플릿의 모든 위치에서 호출할 수 있는 함수입니다. 매번 다른 컨텍스트로 템플릿의 블록을 0회 이상 호출할 수 있습니다. 여기에는 {{#*name*}}과 {{/*name*} 사이의 컨텍스트가 포함됩니다.
...
## 사용자 지정 SCF helpers {#custom-scf-helpers}
...
사용자 지정 SCF 도우미를 서버측에서 구현하고 등록하려면 Java 인터페이스 [TemplateHelper](https://helpx.adobe.com/experience-manager/6-5/sites/developing/using/reference-materials/javadoc/com/adobe/cq/social/handlebars/api/TemplateHelper.html)을 구현하고 [OSGi 서비스](../../help/sites-developing/the-basics.md#osgi)로 만들고 OSGi 번들의 일부로 설치합니다.
...
클라이언트측 도움말은 `Handlebars.registerHelper()`을(를) 호출하여 등록된 핸들막대 스크립트입니다.
예:
...
* `cq.social.scf`에 대한 종속성을 포함합니다.
* Handlebars가 로드된 후 로드합니다.
* [included](clientlibs.md)이어야 합니다.

참고:SCF 도우미는 `/etc/clientlibs/social/commons/scf/helpers.js`에 정의되어 있습니다.
