...
/home/users **에서*****/rep:cache **node가 생성됩니다. 노드의 주체 이름에 쿼리하면 이 캐시를 직접 사용할 수 있음을 찾는 데 사용되는 캐시입니다.
...
1. **구성을**&#x200B;확인합니다. 모든 [](/help/communities/sync.md#bestpractices)구성이 우수 사례에 언급된 대로 AEM 작성자 인스턴스에서 적절히 설정되어 있는지 [확인합니다](/help/communities/sync.md#main-pars-header-863110628).
...
1. **비교 옵서버 팩토리 구성을**&#x200B;확인합니다. 게시 팜 간에 특정 노드만 동기화되지 않는 경우(예: 그룹 구성원이 동기화되지 않은 경우) Adobe Granite Distribution - Diff Observer [Factory 구성이](/help/sites-administering/sync.md#diffobserver) 활성화되고 **rep:멤버는** look 속성 이름으로 ****&#x200B;설정됩니다.
1. **AEM Communities 사용자 동기화 리스너 구성을 확인합니다.** 생성된 사용자가 동기화되지만 구독과 팔로잉이 작동하지 않는 경우 AEM Communities 사용자 동기화 리스너 구성이 다음을 수행했는지 확인하십시오.
   ...
   * 노드 유형 - **rep:User, nt:unstructured**, **nt:resource**, **:ACL**, ******sling Folder:Sling Folder, and OrderedFolder:를 설정합니다.**
