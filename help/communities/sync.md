...
기본적으로 게시 환경에서 만든 사용자 및 프로필(예: 자가 등록, 소셜 로그인, LDAP 인증)은 작성 환경에 표시되지 않습니다.

토폴로지가 [팜 게시](/help/communities/topologies.md) 및 사용자 동기화가 올바르게 구성되었습니다. *사용자* 및 *사용자 프로필* 는 Sling 배포를 사용하여 게시 팜 전체에서 동기화됩니다.

### 게시할 때 새 커뮤니티 그룹이 생성됨 {#new-community-group-is-created-on-publish}

게시 인스턴스에서 시작되지만 새 사이트 페이지 및 새 사용자 그룹을 생성하는 커뮤니티 그룹 생성이 실제로 작성자 인스턴스에서 발생합니다.

프로세스의 일부로 새 사이트 페이지가 모든 게시 인스턴스에 복제됩니다. 동적으로 생성된 커뮤니티 사용자 그룹과 해당 멤버십은 모든 게시 인스턴스에 배포되는 Sling입니다.
...
작성자 인스턴스가 게시자의 내보내기 끝점을 ping합니다. 특정 게시자(n)에서 사용자가 만들어지거나 업데이트될 때마다 작성자는 내보내기 종단점에서 콘텐츠를 가져오고 [콘텐츠 푸시](/help/communities/sync.md#main-pars-image-1413756164) 다른 게시자(콘텐츠를 가져오는 게시자와 다른 n-1)에게 보냅니다.

Apache Sling 동기화 에이전트 구성을 구성하려면 다음 작업을 수행하십시오.

1. AEM 작성자 인스턴스에 대한 관리자 권한으로 로그인합니다.
1. 액세스 [웹 콘솔](/help/sites-deploying/configuring-osgi.md). 예를 들어, [https://localhost:4502/system/console/configMgr](https://localhost:4502/system/console/configMgr).
1. 찾기 **Apache Sling 배포 에이전트 - 동기화 에이전트 팩토리**.
   ...
   * **활성화** 확인란을 선택합니다.
   * 선택 **여러 대기열을 사용합니다.**
   * 지정 **내보내기 종단점** 및 **가져오기 종단점** (내보내기 및 가져오기 끝점을 더 추가할 수 있습니다.)
...
다음 [승인된 사용자 생성됨](/help/sites-administering/sync.md#createauthuser) 모든 게시 인스턴스에서 게시자는 작성자와 연결하고 작성자에서 Sling 배포를 구성할 수 있습니다. 이 승인된 사용자는 모든 필수 사항을 갖습니다. [ACL](/help/sites-administering/sync.md#howtoaddacl).
...
1. AEM 작성자 인스턴스에 대한 관리자 권한으로 로그인합니다.
1. 액세스 [웹 콘솔](/help/sites-deploying/configuring-osgi.md).

   예를 들어, [https://localhost:4502/system/console/configMgr](https://localhost:4502/system/console/configMgr).
1. 찾기 **Adobe Granite 배포 - 암호화된 전송 비밀 공급자.**
1. 편집을 위해 열 기존 구성을 선택합니다(연필 아이콘).

   속성 확인 **socialpubsync** - **publishUser.**

1. 사용자 이름과 암호를 다음으로 설정 [승인된 사용자](/help/sites-administering/sync.md#createauthorizeduser).
...
이 구성은 게시자 간에 동기화할 데이터를 구성하는 데 사용됩니다. 에 지정된 경로에서 데이터가 생성/업데이트될 때 **허용된 루트**, &quot;var/community/distribution/diff&quot;가 활성화되고 생성된 복제기가 게시자의 데이터를 가져와 다른 게시자에 설치합니다.
...
1. 찾기 **Apache Sling 배포 에이전트 - 큐 에이전트 팩토리**.
1. 편집을 위해 열 기존 구성을 선택합니다(연필 아이콘).

   이름 확인: **socialpubsync -reverse**

1. 다음 항목 선택 **활성화됨** 확인란을 선택하고 저장합니다.
1. 복제할 노드 경로 지정 **허용된 루트**.
1. 각각에 대해 반복 **게시** 인스턴스.
...
한 게시자에서 그룹의 멤버십을 변경해도 다른 게시자의 멤버십이 업데이트되지 않는 경우 다음을 확인합니다. **ref:members** 추가됨 **조회 속성 이름**.
...
1. 찾기 **Adobe Granite 배포 - 차이점 옵저버 팩토리**.
1. 편집을 위해 열 기존 구성을 선택합니다(연필 아이콘).

   확인 **에이전트 이름: socialpubsync -reverse**.

1. **활성화** 확인란을 선택합니다.
1. 지정 **rep:members** 에서 propertyName에 대한 설명으로 **조회 속성 이름**, 저장 순으로 선택합니다.
...
1. AEM 작성자 인스턴스에 대한 관리자 권한으로 로그인합니다.
1. 액세스 [웹 콘솔](/help/sites-deploying/configuring-osgi.md), 예: [https://localhost:4502/system/console/configMgr](https://localhost:4502/system/console/configMgr)
1. 찾기 **Apache Sling 배포 트리거 - 예약된 트리거 팩토리**
...
구독과 다음에 차이가 있는 Sling 배포의 문제에 대해서는 다음에서 다음 속성을 확인하십시오. **AEM Communities 사용자 동기화 수신기** 구성이 설정됩니다.
...
1. 관리자 권한으로 로그인합니다.
1. 액세스 [웹 콘솔](/help/sites-deploying/configuring-osgi.md). 예를 들어, [https://localhost:4503/system/console/configMgr](https://localhost:4503/system/console/configMgr).
...
1. 다음으로 이동 [https://_host:port_/system/console/status-slingsettings](https://localhost:4503/system/console/status-slingsettings).
1. 값 확인 **Sling ID**.

   ![새총](assets/slingid.png)

   게시 인스턴스의 Sling ID가 다른 게시 인스턴스의 Sling ID와 일치하는 경우:

1. Sling ID가 일치하는 게시 인스턴스 중 하나를 중지합니다.
1. 다음에서 `crx-quickstart/launchpad/felix` 디렉터리, 다음 파일을 검색하고 삭제합니다. *sling.id.file.*
   ...
   Windows 탐색기를 사용하여 다음을 검색합니다. `sling.id.file`

1. 게시 인스턴스를 시작합니다. 시작 시 새 Sling ID가 할당됩니다.
1. 유효성 검사 **Sling ID** 은(는) 이제 고유합니다.
...
위치 `/home/users`, a `*/rep:cache` 노드가 생성됩니다. 노드의 주체 이름을 쿼리하는 경우 이 캐시를 직접 사용할 수 있음을 찾는 데 사용되는 캐시입니다.

다음과 같은 경우 사용자 동기화를 중지할 수 있습니다. `rep :cache` 노드는 게시자 간에 동기화됩니다.

게시자 간에 각 AEM 게시 인스턴스에서 업데이트가 제대로 동기화되도록 하려면 다음을 수행하십시오.

1. 액세스 [웹 콘솔](/help/sites-deploying/configuring-osgi.md)

   예를 들어, [https://localhost:4503/system/console/configMgr](https://localhost:4503/system/console/configMgr).
1. 를 찾습니다. **Apache Sling 배포 패키징 - Vault 패키지 빌더 팩토리**
...
1. **인증된 사용자 권한 확인**
   ...
   패키지가 제대로 설치되지 않은 경우 [승인된 사용자](/help/sites-administering/sync.md#createauthuser) 첫 번째 게시 인스턴스에 올바른 ACL이 있습니다.

   유효성을 검사하려면 대신 [승인된 사용자 생성됨](/help/sites-administering/sync.md#createauthuser) 변경 [Adobe Granite 배포 - 암호화된 암호 전송 비밀 공급자](/help/sites-administering/sync.md#adobegraniteencpasswrd) 관리자 자격 증명을 사용하도록 작성자 인스턴스에 대한 구성 이제 패키지를 다시 설치해 보십시오. 사용자 동기화가 관리자 자격 증명으로 제대로 작동하는 경우 생성된 게시 사용자에게 적절한 ACL이 없음을 의미합니다.

1. **비교 옵저버 팩토리 구성 확인**

   게시 팜 전체에서 특정 노드만 동기화되지 않는 경우(예: 그룹 멤버가 동기화되지 않음) [Adobe Granite 배포 - 차이점 옵저버 팩토리](/help/sites-administering/sync.md#diffobserver) 구성이 활성화되었으며 **rep: members** 다음에 설정됨 **조회 속성 이름**.
