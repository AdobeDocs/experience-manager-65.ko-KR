...
Adobe Experience Manager(AEM) 커뮤니티에서는 게시 환경(구성된 권한에 따라)에서 *사이트 방문자*&#x200B;가 *구성원*&#x200B;이 되고 *사용자 그룹*&#x200B;을 만들고 *구성원 프로필*&#x200B;을 편집할 수 있습니다.

*사용자 데이터*&#x200B;이(가) *사용자*, *사용자 프로필* 및 *사용자 그룹*&#x200B;을(를) 참조합니다.

*구성원*&#x200B;은(는) 작성 환경에 등록된 사용자와 반대로 게시 환경에 등록된 *사용자*&#x200B;을(를) 참조합니다.
...
[토폴로지](/help/communities/topologies.md)이(가) [게시 팜](/help/sites-deploying/recommended-deploys.md#tarmk-farm)인 경우 한 게시 인스턴스에 대해 수행한 등록 및 수정 사항은 다른 게시 인스턴스와 동기화되어야 합니다. 구성원은 모든 게시 노드에 로그인하고 해당 데이터를 볼 수 있어야 합니다.
...
1. **인증된 사용자 권한 확인**
   ...
   패키지가 제대로 설치되지 않은 경우 첫 번째 게시 인스턴스에서 만든 [승인된 사용자](/help/sites-administering/sync.md#createauthuser)에 올바른 ACL이 있는지 확인합니다.

   이 유효성을 검사하려면 [생성된 권한 있는 사용자](/help/sites-administering/sync.md#createauthuser) 대신 관리자 사용자 자격 증명을 사용하도록 작성자 인스턴스에서 [Adobe Granite 배포 - 암호화된 암호 전송 비밀 공급자](/help/sites-administering/sync.md#adobegraniteencpasswrd) 구성을 변경합니다. 이제 패키지를 다시 설치해 보십시오. 사용자 동기화가 관리자 자격 증명으로 제대로 작동하는 경우 생성된 게시 사용자에게 적절한 ACL이 없음을 의미합니다.
...
1. **AEM Communities 사용자 동기화 수신기 구성을 확인하십시오.** 생성된 사용자가 동기화되지만 구독 및 다음이 작동하지 않는 경우 AEM Communities 사용자 동기화 리스너 구성에 다음이 있는지 확인합니다.
   ...
   * Ignorable 노드가 **.tokens**, **system** 및 **rep:cache**(으)로 설정되었습니다.
   * 분산 폴더 - 분산할 폴더로 설정합니다.
