...
* 에서 MongoDB 다운로드 [https://www.mongodb.org/](https://www.mongodb.org/)
...
* 기본 구성
   ...
   * 설치된 MongoDB 폴더는 &lt;mongo-install>.
   * 정의된 데이터 디렉토리 경로는 &lt;mongo-dbpath>.
...
기본 포트 27017을 사용하여 MongoDB 서버가 시작됩니다.
...
기본 포트 8983을 사용하여 Solr HTTP 서버를 시작합니다. Solr 콘솔로 이동하여 테스트할 Solr 콘솔을 가져올 수 있습니다.

* 기본 Solr 콘솔 [http://localhost:8983/solr/](http://localhost:8983/solr/)

>[!NOTE]
>
>Solr Console을 사용할 수 없는 경우 &lt;solrinstall>/example/logs. SOLR이 해결할 수 없는 특정 호스트 이름(예: &quot;user-macbook-pro&quot;)에 바인딩하려고 하는지 확인합니다.
이 경우 etc/hosts 파일을 이 호스트 이름에 대한 새 항목으로 업데이트하십시오(예: 127.0.0.1 user-macbook-pro). 그러면 Solr이 제대로 시작됩니다.

### SolrCloud {#solrcloud}

프로덕션이 아닌 매우 기본적인 solrCloud 설정을 실행하려면 다음 명령을 사용하여 Solr을 시작하십시오.
...
MongoDB가 시작되기 전에 AEM이 실행 중이었던 경우 AEM 인스턴스를 다시 시작해야 합니다.
...
1. 간단하게 다음에 대한 댓글 보기 [작성자 인스턴스](http://localhost:4502/content/community-components/en/comments.html) ( 관리자 / 관리자로 로그인되어 있을 수 있음).
   ...
   참고: 아래에 JCR 노드가 있는 동안 *asipath* 작성자의 경우 SCF 프레임워크를 위한 것입니다. 실제 UGC는 JCR에 있지 않습니다. MongoDB에 있습니다.
