...
이 검색 기능은 AEM Communities의 필수적인 기능입니다. AEM Communities은 [AEM 플랫폼 검색](../../help/sites-deploying/queries-and-indexing.md) 기능 외에 UGC(사용자 생성 컨텐트)를 검색할 목적으로 [UGC 검색 API](#ugc-search-api)를 제공합니다. UGC는 다른 AEM 컨텐츠 및 사용자 데이터와 별도로 입력되어 저장될 때 고유한 속성을 갖습니다.
...
[MSRP](msrp.md)의 경우 UGC는 검색용으로 Solr를 사용하도록 구성된 MongoDB에 저장됩니다. UGC는 CRX에 표시되지 않지만, 작성자 및 게시 환경 모두에서 [moderation](moderate-ugc.md)을(를) 사용할 수 있습니다.
...
[JSRP](jsrp.md)의 경우 UGC는 [Oak](../../help/sites-deploying/platform.md)에 저장되고 입력된 AEM 작성자 또는 게시 인스턴스의 보관소에만 표시됩니다.

UGC는 일반적으로 게시 환경에 입력되기 때문에 다중 게시자 프로덕션 시스템의 경우, 입력한 컨텐츠가 모든 게시자에게 표시되도록 게시 팜이 아닌 [게시 클러스터](topologies.md)를 구성해야 합니다.

JSRP의 경우 게시 환경에 입력된 UGC는 작성 환경에서 보이지 않습니다. 따라서 모든 [moderation](moderate-ugc.md) 작업이 게시 환경에서 수행됩니다.
...
사용자 지정 속성이 사용 중이고 검색이 느린 경우 사용자 지정 속성을 더 성능으로 만들려면 추가 색인을 만들어야 합니다. 이식성을 유지하려면 검색 가능한 사용자 지정 속성을 만들 때 [이름 지정 요구 사항](#naming-of-custom-properties)을 따르십시오.

기존 색인을 수정하거나 사용자 지정 색인을 만들려면 [Oak 쿼리 및 색인](../../help/sites-deploying/queries-and-indexing.md)을 참조하십시오.
...
기존 Oak 색인을 [CRXDE Lite](../../help/sites-developing/developing-with-crxde-lite.md)에서 보려면 위치가 다음과 같습니다.
...
사용자 지정 속성을 추가할 때 [UGC 검색 API](#ugc-search-api)로 만든 정렬 및 검색에 이러한 속성이 표시되도록 하려면 속성 이름에 접미어를 추가하려면 *required*&#x200B;이 필요합니다.
...
* *텍스트* 는 토큰화된 문자열이고,  ** 문자열은 그렇지 않습니다. 퍼지(다음과 같은) 검색에 *Text*&#x200B;을 사용합니다.
...
[comment system](essentials-comments.md)이 포함된 구성 요소는 끝점에 필터 매개 변수를 추가할 수 있습니다.
...
[검색 구성 요소](search.md)의 기본 구현에서는 이 구문을 사용합니다. [커뮤니티 구성 요소 안내서](components-guide.md)에서 검색 결과 페이지를 여는 URL에서 볼 수 있습니다. 실험해 보려면 [http://localhost:4503/content/community-components/en/search.html](http://localhost:4503/content/community-components/en/search.html)로 이동하십시오.
...
`q` 매개 변수의 값은 쿼리입니다. URL 인코딩이 디코딩되면 추가 디버깅을 위해 쿼리를 Solr Admin Query 툴로 전달할 수 있습니다.

## 관련 리소스 {#related-resources}

* [커뮤니티 컨텐츠 저장소](working-with-srp.md)  - UGC 공용 저장소에 대해 사용할 수 있는 SRP 선택 사항에 대해 설명합니다.
