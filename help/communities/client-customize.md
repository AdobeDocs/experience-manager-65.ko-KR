...
[구성 요소를 오버레이합니다](#overlays) 기본 구성 요소를 변경하고 구성 요소에 대한 모든 참조에 영향을 줍니다.

[구성 요소 확장](#extensions)은(는) 고유한 이름으로, 변경 범위를 제한합니다. &#39;extend&#39;라는 용어는 &#39;override&#39;와 혼용하여 사용됩니다.

## 오버레이 {#overlays}

구성 요소 오버레이는 기본 구성 요소를 수정하고 기본값을 사용하는 모든 인스턴스에 영향을 주는 방법입니다.

오버레이는 /**libs** 디렉터리에서 원래 구성 요소를 수정하는 대신 /**apps** 디렉터리에서 기본 구성 요소의 복사본을 수정하여 수행됩니다. 구성 요소는 &#39;libs&#39;를 &#39;apps&#39;로 대체한다는 점을 제외하고 동일한 상대 경로로 구성됩니다.
...
이는 특정 용도로 수정하여 구성 요소에 고유한 경로를 만들고 /libs 디렉터리의 원래 기본 구성 요소를 슈퍼 리소스 유형으로 참조하려는 기본 구성 요소인 [확장](#extensions)과 다릅니다.

댓글 구성 요소를 오버레이하는 간단한 예를 보려면 [댓글 오버레이 구성 요소 튜토리얼](overlay-comments.md)을 사용해 보십시오.
...
`data-scf-component` 특성 값은 **`social/tally/components/hbs/rating`**&#x200B;과(와) 같은 기본값이거나 **weretail/components/hbs/rating**&#x200B;과(와) 같은 사용자 지정된 기능에 대한 확장(사용자 지정된) 구성 요소일 수 있습니다.

구성 요소를 바인딩하려면 전체 구성 요소 스크립트를 다음 속성으로 &lt;div> 요소 내에 묶어야 합니다.
...
예를 들어 `/apps/weretail/components/hbs/rating/rating.hbs`부터:
...
1. 사이트의 클라이언트 라이브러리 폴더([clientlibs](#clientlibs-for-scf))에 스타일시트를 포함하고 [ui:includeClientLib](../../help/sites-developing/clientlibs.md)이(가) 있는 템플릿과 페이지에서 스타일시트가 포함되어 있는지 확인하십시오.
...
>[!CAUTION]
>
>접두사가 `scf-js`인 모든 CSS 클래스 이름은 JavaScript 코드에서 특정 용도로 사용됩니다. 이러한 클래스는 구성 요소의 상태에 영향을 주며(예: 숨김에서 표시로 전환), 재정의하거나 제거해서는 안 됩니다.
>
>`scf-js` 클래스는 스타일에 영향을 주지 않지만 스타일 시트에서 클래스 이름을 사용할 경우 요소의 상태를 제어하므로 부작용이 발생할 수 있습니다.
...
[클라이언트측 라이브러리](../../help/sites-developing/clientlibs.md)(clientlibs)를 사용하면 클라이언트에서 콘텐츠를 렌더링하는 데 사용되는 JavaScript 및 CSS를 구성하고 최적화할 수 있습니다.

SCF용 clientlib은 범주 이름에 &#39;작성자&#39;가 있는 경우에만 달라지는 두 변형에 대한 매우 구체적인 이름 지정 패턴을 따릅니다.

| Clientlib 변형 | 카테고리 속성에 대한 패턴 |
|--- |--- |
| clientlib 완료 | cq.social.hbs.&lt;구성 요소 이름> |
| 작성자 clientlib | cq.social.author.hbs.&lt;구성 요소 이름> |
...
[커뮤니티 구성 요소 안내서](components-guide.md)에는 각 SCF 구성 요소에 필요한 전체 clientlib이 나열됩니다.

[커뮤니티 구성 요소에 대한 Clientlibs](clientlibs.md)에서는 페이지에 clientlibs를 추가하는 방법에 대해 설명합니다.

### 작성자 Clientlibs {#author-clientlibs}

작성자 버전 clientlib은 구성 요소를 구현하는 데 필요한 최소 JavaScript으로 제거됩니다.
...
[커뮤니티 구성 요소 안내서](components-guide.md)에서 각 SCF 구성 요소에 대해 나열된 clientlib에 &quot;author&quot;를 삽입하여 필요한 작성자 clientlib을 식별할 수 있습니다.

### 사용 고려 사항 {#usage-considerations}

모든 사이트는 클라이언트 라이브러리를 관리하는 방식이 다릅니다. 다양한 요인은 다음과 같습니다.

* 전체 속도: 사이트가 응답형 사이트일 수 있지만 첫 번째 페이지를 로드하는 데 약간 느릴 수 있습니다. 많은 페이지가 동일한 JavaScript을 사용하는 경우 다양한 JavaScript을 하나의 clientlib에 임베드하고 첫 페이지에서 참조하여 로드할 수 있습니다. 이 단일 다운로드의 JavaScript은 캐시된 상태로 유지되므로 후속 페이지를 위해 다운로드할 데이터의 양이 최소화됩니다.
* 첫 페이지까지 짧은 시간: 첫 페이지를 빠르게 로드하고자 할 수 있습니다. 이 경우 JavaScript은 필요한 경우에만 참조할 수 있도록 여러 개의 작은 파일에 있습니다.
