...
>[!CAUTION]
>
>접두사가 있는 모든 CSS 클래스 이름 `scf-js` 는 javascript 코드에서 특정 사용을 합니다. 이러한 클래스는 구성 요소의 상태에 영향을 주며(예: 숨김에서 가시기로 전환) 무시하거나 제거할 수 없습니다.
>
>반면에 `scf-js` 클래스는 스타일에 영향을 주지 않으며, 클래스 이름은 요소의 상태를 제어하므로 부작용이 있을 수 있는 경고가 있는 스타일시트에서 사용할 수 있습니다.
...
구성 요소를 오버레이하거나 재정의할 때 SCF 스크립트의 스크립트 태그를 제거해서는 안 됩니다. HTML에서 JSON을 주입하기 위해 자동으로 생성된 SCF 스크립트 태그는 속성으로 식별됩니다 `data-scf-json=true`.
