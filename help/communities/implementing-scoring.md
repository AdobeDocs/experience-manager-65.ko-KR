...
* [참가를 장려하기 위해 회원에게 배지](#enable-scoring) 기본 제공(콘텐츠 제작 수량)
* [멤버를 전문가로 식별할 수 있는 배지](/help/communities/advanced.md) 제공(컨텐츠의 품질)
...
커뮤니티 [배지 콘솔에서는](/help/communities/badges.md) 획득(수상)시 또는 커뮤니티에서 특정 역할을 수행할 때(할당됨) 회원에게 표시할 수 있는 사용자 지정 배지를 추가할 수 있습니다.
...
* 배지가 기능 구성 요소에 대해 [활성화되어](#enableforcomponent) 있어야 합니다.
* 점수 및 배지 규칙은 구성 요소가 배치된 페이지(또는 상위)에 [적용되어야](#applytopage) 합니다.
...
* 점수 규칙당 누적된 포인트에 대해 [배지](#badges) 지정([배지 규칙](#badging-rules))

* [채점 및 배지 규칙을 커뮤니티 사이트에 적용](#apply-rules-to-content)
...
* page2에 점수 지정 규칙2가 포함되어 있고 해당 상위 페이지1에 점수 지정 규칙1이 들어 있는 경우
* page2 구성 요소의 작업은 rule1과 rule2를 모두 호출합니다.
* if both rules contained subrules for same `topic/verb` :
...
하위 규칙의 이름은 일반적으로 *subject, object *와 *동사를 사용하는 패턴을 따릅니다.*&#x200B;예를 들어,
...
1. /etc/community/scoring/rules/forums-scoring
   ...
   * subRules[] =/etc/community/scoring/rules/sub-rules/member-forum-create/etc/community/scoring/rules/sub-rules/member-receive-vote/etc/community/scoring/rules/sub-give-vote/etc/community/scoring/rules/sub-rules/member-is-adjusticated
 ...
 <tbody>
  ...
  <tr>
   ...
   <td><em>(필수)</em> 'number|path' 형식의 다중 값 문자열
    ...
    </ul> 숫자가 값을 늘리고 숫자와 경로 사이에 공백이 없도록 문자열을 정렬해야 합니다.<br /><br /> 시작 예: <code>80|/etc/community/badging/images/gold-badge/jcr:content/gold.png</code></td>
  </tr>
  <tr>
   <td>badgingType</td>
   <td>문자열</td>
...
>* 의 불명확하고 무작위 이름 - 권한 있는 ID와 [관련된 보안](/help/sites-administering/security-checklist.md#verify-that-you-are-not-disclosing-personally-identifiable-information-in-the-users-home-path) 검사 목록 참조
>
...
그런 다음 포럼 및 주석 구성 요소를 사용하여 배지가 표시되도록 하십시오.
