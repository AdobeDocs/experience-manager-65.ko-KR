...
* [커뮤니티에서 구성원의 역할을 ](#assign-and-revoke-badges) 식별하는 배지(badgestget)를 지정합니다.

* [참가를 장려하기 위해 ](#enable-scoring) 배지 구성원의 기본 제공(만든 콘텐츠의 양)

* [고급 ](/help/communities/advanced.md) 배지 수여로 구성원을 전문가(콘텐츠 품질)로 식별할 수 있습니다.
...
커뮤니티 [배지 콘솔](/help/communities/badges.md)에서는 획득(수상) 시 또는 커뮤니티(지정)에서 특정 역할을 수행할 때 멤버에 대해 표시될 수 있는 사용자 지정 배지를 추가할 수 있습니다.

### 지정된 배지 {#assigned-badges}

역할 기반 배지는 커뮤니티 내에서 자신의 역할에 따라 관리자가 커뮤니티 구성원에게 할당됩니다.

할당(및 awared) 배지는 선택한 [SRP](/help/communities/srp.md)에 저장되며 직접 액세스할 수 없습니다. GUI를 사용할 수 있을 때까지 역할 기반 배지를 할당하는 유일한 방법은 코드 또는 cURL을 사용하여 지정하는 것입니다. cURL 지침은 [배지 할당 및 폐지](#assign-and-revoke-badges)라는 섹션을 참조하십시오.
...
* 배지 지정은 기능 구성 요소에 대해 [enabled](#enableforcomponent)여야 합니다.
* 점수 지정 및 배지 규칙은 구성 요소가 배치된 페이지(또는 상위 페이지)에 [적용되어야 합니다.](#applytopage)
...
>
>배지는 지정된 배지와 동일한 방식으로 취소될 수 있습니다. [배지 지정 및 취소](#assign-and-revoke-badges) 섹션을 참조하십시오. UI가 개선되어 멤버의 배지를 관리할 수 있게 됩니다.
...
점수 지정 및 배지를 활성화하려면 속성 `scoringRules` 및 `badgingRules`을 사이트의 컨텐츠 트리의 노드에 추가합니다.
...
| 점수 지정 규칙 | 문자열 | [점수 규칙](#scoring-rules)의 배열 목록 |

>[!NOTE]
>
>점수 지정 규칙이 배지 수여에 영향을 주지 않는 것으로 나타나면 배지 규칙의 scoringRules 속성으로 점수 지정 규칙이 차단되지 않았는지 확인합니다. [배지 규칙](#badging-rules)이라는 제목이 있는 섹션을 참조하십시오.

### 구성 요소 {#enable-badges-for-component} 배지 활성화

점수 및 배지 규칙은 [작성 모드](/help/communities/author-communities.md)에서 구성 요소 구성을 편집하여 배지 지정을 활성화한 구성 요소의 인스턴스에만 적용됩니다.

구성 요소 인스턴스에 대한 배지 표시를 활성화/비활성화합니다. `allowBadges` 포럼, QnA 및 주석 구성 요소에 대해 [구성 요소 편집 대화 상자](/help/communities/author-communities.md)에서 구성 가능합니다. **배지 표시**&#x200B;라는 확인란을 통해 구성 요소를 구성할 수 있습니다.
...
점수 지정 규칙은 `jcr:content` 노드에 속성이 있는 `cq:Page` 유형의 노드로서 이를 정의하는 하위 규칙 목록을 지정합니다.
...
기본적으로 포인트는 하위 규칙이 컨텐츠 소유자를 포인트( `forOwner`)를 수신함에 따라 지정하지 않는 한 조치 수행 멤버에게 부여됩니다.
...
/libs/settings/community/scoring/rules/sub-rules/member-receive-voit
...
배지 규칙은 점수 규칙을 점수 및 배지에 상관 관계를 표시하는 해당 `jcr:content` 노드의 속성과 함께 `cq:Page` 유형의 노드입니다.

배지 규칙은 배지에 매핑된 점수의 순서 목록인 필수 `thresholds` 속성으로 구성됩니다. 점수는 높은 값으로 순서가 정해져야 한다. 예:
...
배지 규칙은 포인트가 누적되는 방식을 결정하는 점수 지정 규칙과 일치합니다. [콘텐츠에 규칙 적용](#apply-rules-to-content)이라는 섹션을 참조하십시오.
...
<table>
 <tbody>
  ...
  <tr>
   <td>badgingType</td>
   <td>문자열</td>
   <td><em>(선택 사항)</em> 점수 엔진을 "기본" 또는 "고급"으로 식별합니다. 고급 점수 지정 엔진을 원하는 경우 <a href="/help/communities/advanced.md">고급 점수 지정 및 배지</a>를 참조하십시오. 기본값은 "basic"입니다.</td>
  </tr>
  <tr>
   <td>점수 지정 규칙</td>
   <td>문자열</td>
   <td>(<em>optional</em>) 점수 지정 규칙에 의해 식별된 점수 이벤트로 배지 규칙을 제한하는 다중 값 문자열</td>
  </tr>
 </tbody>
</table>
...
릴리스에는 [포럼 및 댓글 점수 지정 규칙](#includedscoringrules)에 해당하는 두 개의 배지 규칙이 포함됩니다.
...
[멤버 콘솔](/help/communities/members.md#badges-tab)을 사용하거나 cURL 명령을 사용하여 프로그래밍 방식으로 멤버에 배지가 할당될 수 있습니다.
...
*signin* = administrator-id:password for example:관리:관리자
...
>* 잘 알려지지 않은 임의 이름일 수 있습니다. 권한 부여 가능한 ID에 대해서는 [보안 검사 목록](/help/sites-administering/security-checklist.md#verify-that-you-are-not-disclosing-personally-identifiable-information-in-the-users-home-path)을 참조하십시오.
...
## 사용자 지정 구성 요소의 점수 및 배지 {#scoring-and-badges-for-custom-components}
...
>[!NOTE]
>
>구성 요소 인스턴스에 대한 배지 표시를 활성화/비활성화합니다. `allowBadges` **Display Badges**&#x200B;라는 레이블이 지정된 확인란을 통해 업데이트된 [구성 요소 편집 대화 상자](/help/communities/author-communities.md)에서 구성 가능합니다.
...
**[평가](/help/communities/reviews.md)**
...
구성 요소SocialEvent  `topic`= com/adobe/cq/social/tally/rating
...
사용자 지정 구성 요소의 경우, SocialEvent가 인스턴스화되고 `topic`에 대해 발생하는 `actions`으로 구성 요소의 이벤트를 기록합니다.

점수를 지원하려면 SocialEvent가 `getVerb()` 메서드를 재정의하여 각 `action`에 대해 적절한 `verb`이 반환됩니다. 작업에 대해 반환되는 `verb`은 일반적으로 사용되는 하나(예: `POST`) 또는 구성 요소에 특화된 하나(예: `ADD RATING`)일 수 있습니다. `actions`과 `verbs` 사이에 *n-1* 관계가 있습니다.
...
### 점수 지정 규칙은 {#scoring-rule-has-no-effect} 효과가 없습니다.
...
[시작하기 자습서](/help/communities/getting-started.md)(참여) 사이트를 사용하여 점수 매기와 배지 재지정을 신속하게 시도할 수 있습니다.
...
개발자를 위한 [점수 지정 및 배지 필수 패키지](/help/communities/configure-scoring.md) 페이지에서 자세한 내용을 확인할 수 있습니다.

고급 점수 지정 엔진에 대한 자세한 내용은 [고급 점수 지정 및 배지](/help/communities/advanced.md)를 참조하십시오.

구성 가능한 리더보드 [component](/help/communities/enabling-leaderboard.md) 및 [function](/help/communities/functions.md#leaderboard-function)은 커뮤니티 사이트에서 멤버 및 해당 점수의 표시를 단순화합니다.
