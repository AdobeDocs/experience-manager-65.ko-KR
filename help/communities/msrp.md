...
* [몽고 DB](https://www.mongodb.org/):

   * 버전 2.6 이상
   * 몽고 또는 분할을 구성할 필요가 없음
   * [복제본 집합](#mongoreplicaset)을(를) 사용하는 것이 좋습니다
...
* **[!UICONTROL MongoDB 저장소 리소스 공급자(MSRP) 선택]**
...
>[!NOTE]
>
>기본적으로 `communities` 이름으로 설정되어 있는 mongoDB 데이터베이스는 [노드 저장소 또는 데이터(이진) 저장소](../../help/sites-deploying/data-store-config.md)에 사용 중인 데이터베이스 이름으로 설정해서는 안 됩니다. [AEM 6.5의 저장소 요소](../../help/sites-deploying/storage-elements-in-aem-6.md)도 참조하세요.

### MongoDB 복제본 집합 {#mongodb-replica-set}

운영 환경의 경우 주-보조 복제 및 자동 장애 조치를 구현하는 MongoDB 서버의 클러스터인 복제본 세트를 설정하는 것이 좋습니다.

복제본 집합에 대한 자세한 내용은 MongoDB의 [복제](https://docs.mongodb.org/manual/replication/) 설명서를 참조하십시오.

복제본 집합을 사용하여 작업하고 응용 프로그램과 MongoDB 인스턴스 간의 연결을 정의하는 방법을 알아보려면 MongoDB의 [연결 문자열 URI 형식](https://docs.mongodb.org/manual/reference/connection-string/) 설명서를 참조하십시오.
...
프로덕션 환경의 경우 [SolrCloud 모드](solr.md#solrcloud-mode)는 독립 실행형 모드(단일 로컬 Solr 설정)보다 향상된 성능을 제공합니다.

구성에 대한 자세한 내용은 [SRP에 대한 Solr 구성](solr.md)을 참조하십시오.
...
1. 새 Solr 구성 파일 설치
   * [표준 MLS](solr.md#installing-standard-mls)용
   * [고급 MLS](solr.md#installing-advanced-mls)용
1. MSRP 색인 재지정
[MSRP 색인 재지정 도구](#msrp-reindex-tool) 섹션 참조
...
* 기본 메뉴에서 **[!UICONTROL 도구]** > **[!UICONTROL 작업]** > **[!UICONTROL 복제]**(으)로 이동합니다.
* **[!UICONTROL 트리 활성화]** 선택
* **[!UICONTROL 시작 경로]**:
...
이 도구를 사용하면 MongoDB는 MSRP에 대한 *truth*&#x200B;의 소스입니다. 백업은 MongoDB에서만 수행하면 됩니다.
...
* 모든 UGC를 다시 인덱싱하려면 의 `asipath` 속성 값을 지정하십시오.
  `/etc/socialconfig/srpc/defaultconfiguration`
* 인덱스를 일부 UGC로 제한하려면 `asipath`의 하위 트리를 지정하십시오.

*reindex-url* = SRP의 리인덱싱을 위한 끝점
`http://localhost:4503/services/social/datastore/mongo/reindex`

>[!NOTE]
>
>[DSRP Solr을 다시 인덱싱](dsrp.md)하는 경우 URL은 **/services/social/datastore/rdb/reindex**&#x200B;입니다.
...
데모 또는 개발 환경에 대한 MSRP를 설정하려면 [데모용 MongoDB를 설정하는 방법](demo-mongo.md)을 참조하세요.
...
* JCR에서, [/etc/socialconfig](http://localhost:4502/crx/de/index.jsp#/etc/socialconfig/)인 경우
   ...
   * [srpc](http://localhost:4502/crx/de/index.jsp#/etc/socialconfig/srpc) 노드를 포함하지 않습니다. 이는 저장소 공급자가 JSRP임을 의미합니다.
