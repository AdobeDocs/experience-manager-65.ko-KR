...
>[!NOTE]
>
>MongoDB 데이터베이스(기본값은 이름) `communities`에 사용되는 데이터베이스 이름으로 설정하면 안 됩니다 [노드 저장소 또는 데이터(이진) 저장소](../../help/sites-deploying/data-store-config.md). 참조 - [AEM 6.5의 저장소 요소](../../help/sites-deploying/storage-elements-in-aem-6.md).
...
* JCR에서 [/etc/socialconfig](http://localhost:4502/crx/de/index.jsp#/etc/socialconfig/)
   ...
   * 다음을 포함하지 않음 [srpc](http://localhost:4502/crx/de/index.jsp#/etc/socialconfig/srpc) node이면 스토리지 공급자가 JSRP임을 의미합니다.
...
오류를 해결하려면 [표준 MLS 설치](solr.md#installing-standard-mls), 다음을 확인합니다.
