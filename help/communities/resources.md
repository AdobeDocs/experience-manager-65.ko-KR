...
AEM Communities의 경우 리소스 콘솔은 [활성 관리자](users.md)가 활성 커뮤니티 사이트의 구성원을 만들고, 관리하고, 할당하는 곳입니다.
...
>[!NOTE]
>
>[AEM 6.3](deploy-communities.md#latestfeaturepack)의 릴리스 및 상응하는 커뮤니티 기능 팩이 [AEM 6.2 FP3](deploy-communities.md#latestfeaturepack) 및 [AEM 6.1 FP7](https://docs.adobe.com/content/docs/en/aem/6-1/deploy/communities.html#Latest 기능 팩)인 경우 지원 기능에 [MySQL 데이터베이스](mysql.md)이 더 이상 필요하지 않습니다.

## 용어 {#terminology}

### 리소스 {#resource}

리소스는 [활성 커뮤니티](overview.md#enablement-community)에 필수적입니다. 멤버들은 그들의 기술을 향상시킬 수 있는 소재이다.
...
커뮤니티 사이트가 만들어지면 URL에 대해 사이트에 지정된 이름이 다양한 역할에 대한 다양한 권한을 사용하여 구성된 [사이트별 사용자 그룹](users.md)을 만드는 데 사용됩니다. 이러한 자동 생성 그룹의 모든 접두사는 `Community <site-name>`입니다.

이러한 사용자 그룹 중 하나는 게시 환경에서 등록된 사용자를 커뮤니티 구성원으로 식별하는 `Community <site-name> Members` 그룹입니다. 예를 보려면 [Getting Started with AEM Communities for Enablement](getting-started-enablement.md) 자습서를 참조하십시오.
...
[활성 커뮤니티](overview.md#enablement-community)의 경우, 사이트를 비공개로 만든 후 관리자가 사용자를 구성원 그룹에 추가하는 것이 좋습니다.
...
* **[!UICONTROL 추가]**
   ...
   (*옵션*) 멤버 할당 페이지 및 리소스 콘솔에서 활성 리소스에 대한 카드에 표시할 이미지입니다. 서버의 로컬 파일 시스템에서 이미지가 선택됩니다. 이미지가 제공되지 않으면 업로드된 리소스에 대해 축소판이 생성됩니다.
...
* **[!UICONTROL 태그]**
   ...
   (*선택적*) 활성 리소스를 하나 이상의 카탈로그에 연결하는 하나 이상의 태그를 선택할 수 있습니다. [태깅 지원 리소스](tag-resources.md)를 참조하십시오.

* **[!UICONTROL 카탈로그에 표시]**

   선택하지 않으면 활성 리소스가 카탈로그에 표시되지 않습니다. 이 확인란을 선택하면 활성 리소스는 [사전 필터링된](catalog-developer-essentials.md#pre-filters) 또는 UI의 멤버 필터가 없으면 모든 카탈로그에 표시됩니다. 기본값은 선택 취소입니다.
...
* **[!UICONTROL 작은 자산]**
   ...
   (*선택적*) AEM Assets에서 선택됨 카탈로그 등의 게시 환경에서 리소스를 나타내는 축소판 이미지입니다.

* **[!UICONTROL 큰 자산]**

   (*선택적*) AEM Assets에서 선택됨 리소스의 기본 페이지와 같이 게시 환경에서 리소스를 나타내는 큰 이미지입니다.

* **[!UICONTROL 컨텐츠 조각 자산]**

   (*선택적*) AEM Assets에서 선택됨 게시 환경에서 참조될 수 있지만 기본적으로 사용되지 않는 컨텐츠 조각.
...
* **[!UICONTROL 외부 리소스 정의]**
   ...
   재료를 표시할 위치를 입력합니다. 성공 상태 및 점수에 대한 값은 수동으로 입력됩니다( [보고서](reports.md) 참조). 업로드된 표지 이미지를 사용하여 추가 정보를 제공할 수 있습니다.
...
* **[!UICONTROL 리소스 표지 이미지]**
   ...
   표지 이미지는 활성 리소스를 처음 볼 때 표시되는 이미지입니다. 예를 들어 비디오 리소스가 아직 재생되지 않은 경우 표지 이미지가 표시됩니다. 사용자 지정 이미지가 업로드되지 않으면 기본 이미지가 표시됩니다. 비디오 리소스의 경우 [축소판 생성](enablement.md#ffmpeg)이 가능하지만 업로드된 경우에만 가능하며 비디오가 URL로 참조될 때는 해당되지 않습니다. 위치 리소스의 경우 이미지를 사용하여 추가 정보를 제공할 수 있습니다.
...
* **[!UICONTROL 리소스 작성자]**
   ...
   *(선택 사항)* 활성 리소스의 작성자입니다. 풀다운 메뉴를 사용하여 [구성원 그룹](#members-group)의 구성원인 사용자 중에서 선택합니다.

* **[!UICONTROL 리소스 연락처(&amp;A);]**

   *(필수)* 회원이 활성 리소스에 대해 연락할 수 있는 사람입니다. 풀다운 메뉴를 사용하여 [구성원 그룹](#members-group)의 구성원인 사용자 중에서 선택합니다.

* **[!UICONTROL 리소스 전문가]**

   *(선택 사항)* 회원이 활성 리소스에 대한 전문성을 가진 사람에게 연락할 수 있는 사람입니다. 풀다운 메뉴를 사용하여 [구성원 그룹](#members-group)의 구성원인 사용자 중에서 선택합니다.
...
* **[!UICONTROL 할당자 추가]**
   ...
   풀다운 메뉴를 사용하여 수강생으로 등록할 사용자 및 사용자 그룹(굵게 표시된 부분에 나열됨)에서 선택합니다. [](#members-group) 구성원이 커뮤니티 사이트에 로그인하면 해당 구성원이 등록된 활성 리소스(및 학습 경로)가 해당 [할당](functions.md#assignments-function) 페이지에 표시됩니다.
...
* **[!UICONTROL 추가]**
   ...
   (*선택적*) 구성원의 할당 페이지 및 리소스 콘솔에서 학습 경로의 카드에 표시할 이미지입니다. 서버의 로컬 파일 시스템에서 이미지가 선택됩니다. 이미지가 제공되지 않으면 업로드된 리소스에 대해 축소판이 생성됩니다.
...
* **[!UICONTROL 태그]**
   ...
   (*선택적*) 학습 경로를 하나 이상의 카탈로그에 연결하는 하나 이상의 태그를 선택할 수 있습니다. [태깅 지원 리소스](tag-resources.md)를 참조하십시오.

* **[!UICONTROL 카탈로그에 표시]**

   선택하지 않으면 학습 경로가 카탈로그에 표시되지 않습니다. 이 확인란을 선택하면 [사전 필터링된](catalog-developer-essentials.md#pre-filters) 또는 UI에서 멤버 필터가 없으면 학습 경로가 모든 카탈로그에 표시됩니다. 카탈로그에 학습 경로를 표시하면 포함된 모든 리소스에 대한 읽기 액세스 권한이 간접적으로 부여됩니다. 기본값은 선택 취소입니다.
...
* **[!UICONTROL 학습 경로에서 순서 적용]**
   ...
   (*선택적*) 켜기로 설정하면 학습 경로를 진행하는 데 수강생이 필요한 활성 리소스가 추가되는 순서가 됩니다. 기본값은 해제입니다.

* **[!UICONTROL 리소스]**

   현재 커뮤니티 사이트에 대해 만들어진 *게시된* 활성 리소스 중에서 하나 이상의 리소스를 선택합니다.
...
* **[!UICONTROL 등록 추가]**
   ...
   풀다운 메뉴를 사용하여 커뮤니티 사이트의 [구성원 그룹](#members-group)의 구성원인 구성원 및 구성원 그룹(굵은체 목록에 나열됨)에서 선택합니다. 처음 학습 경로를 만들 때는 할당을 추가할 필요가 없습니다. 학습 경로 속성을 수정하여 나중에 학습자를 추가할 수 있습니다.

* **[!UICONTROL 학습 경로 연락처(&amp;A);]**

   *(필수)* 회원이 학습 경로와 관련하여 연락할 수 있는 사람입니다. 풀다운 메뉴를 사용하여 커뮤니티 사이트의 [구성원 그룹](#members-group)의 구성원인 사용자 중에서 선택합니다.
...
trashcan 아이콘을 선택하면 지원 리소스는 확인 후 `Deleted`이 됩니다.
...
게시 환경의 구성원이 해당 리소스를 삭제하지 않고 활성 리소스에 액세스할 수 없도록 하려면 월드 아이콘을 사용하여 리소스를 `Unpublish`합니다.
...
이 보고서는 현재 활성 리소스 또는 학습 경로를 위한 것입니다. 제공되는 보고 깊이는 [Adobe Analytics](analytics.md)에 대한 라이선스가 부여되고 커뮤니티 사이트에 대해 활성화되었는지 여부에 따라 다릅니다. [타임라인](#timeline), [뷰어 참여](#viewer-engagement) 및 [장치별 참여 보고서는 [폴링 간격](analytics.md#report-importer)을 기준으로 Adobe Analytics에서 가져옵니다.](#engagement-by-device)

Adobe Analytics 활성화 여부에 관계없이 모든 활성 리소스의 경우 [Assignee Status](#assignee-status) 및 [Ratings](#ratings)과 [보고서 요약](#report-summary) 테이블에 대한 보고서가 있습니다.
...
* 활성 리소스의 경우:`Download Resource Report as CSV` 단추
* 학습 경로:`Download Learning Path Report as CSV` 단추
