...
* [https://&lt;서버>:&lt;포트>/content/community-components/en.html](http://localhost:4502/content/community-components/en.html)
...
* 사이트 방문자의 로그인 여부입니다.
* 로그인한 경우 멤버에게 할당된 권한.
* 기본 SRP [JSRP](jsrp.md)이(가) 사용 중인지 여부입니다.

작성자의 경우 편집 모드로 전환하려면 서버 이름 뒤에 `editor.html` 또는 `cf#`을(를) 첫 번째 경로 세그먼트로 삽입합니다.

* 표준 UI:

  [https://&lt;서버>:&lt;포트>/editor.html/content/community-components/en.html](http://localhost:4502/editor.html/content/community-components/en.html)

* 클래식 UI:

  [https://&lt;서버>:&lt;포트>/cf#/content/community-components/en.html](http://localhost:4502/cf#/content/community-components/en.html)
...
1. 제목: 선택한 구성 요소의 이름
1. [클라이언트측 라이브러리](#client-side-libraries): 하나 이상의 필수 범주 목록
...
작성자 인스턴스에서 안내서를 사용할 때 대화 상자를 열어 구성 요소를 구성하는 경험을 할 수 있습니다. 개발자에 대한 정보는 설명서의 [구성 요소 및 기능 기본 사항](essentials.md) 섹션에 있으며, 대화 상자 설정은 작성자를 위한 [커뮤니티 구성 요소](author-communities.md) 섹션에 설명되어 있습니다.

커뮤니티 구성 요소 안내서의 경우 일부 구성 요소 대화 상자 설정이 [포함 가능](scf.md#add-or-include-a-communities-component) 전환 상태로 오버레이됩니다. 기존 리소스를 사용하거나 동적으로 포함된 리소스를 사용하려면 편집 모드에서 구성 요소와 포함 가능한 텍스트를 모두 선택하고 두 번 클릭하여 편집 대화 상자를 엽니다.
...
>[!NOTE]
>
>SRP를 [JSRP](jsrp.md)(으)로 기본 설정한 상태로 두면 게시 인스턴스에 입력한 UGC는 게시 시에만 표시되고 작성자 인스턴스의 [중재](moderate-ugc.md) 콘솔에서는 *표시되지*&#x200B;않습니다.

## 클라이언트측 라이브러리 {#client-side-libraries}

각 구성 요소에 대해 나열된 클라이언트측 라이브러리(clientlibs)는 구성 요소를 페이지에 배치할 때 참조되는 *필수*&#x200B;입니다. clientlib은 브라우저에서 구성 요소를 렌더링하는 데 사용되는 JavaScript 및 CSS의 다운로드를 관리하고 최적화하는 수단을 제공합니다.

자세한 내용은 [Clientlibs for Communities 구성 요소](clientlibs.md)를 참조하십시오.

## 가장 {#impersonation}

관리자 또는 개발자로 로그인한 경우가 많은 작성자 인스턴스에서 다른 사용자로 로그인한 구성 요소를 경험하려면 **[!UICONTROL 가장]** 단추 왼쪽에 있는 텍스트 상자를 사용하여 사용자 이름을 입력하거나 풀다운 목록에서 선택한 다음 단추를 클릭합니다. 로그아웃하고 가장을 종료하려면 되돌리기 를 클릭합니다.

게시 인스턴스는 가장할 필요가 없습니다. 로그인/로그아웃 링크를 사용하여 [데모 사용자](tutorials.md#demo-users)와 같은 다양한 사용자를 가장하기만 하면 됩니다.
...
사용자 지정을 빠르게 실험하려면 구성 요소 페이지의 콘텐츠 JCR 노드에 `scg:showIde` 속성을 추가하고 true로 설정해야 합니다.
