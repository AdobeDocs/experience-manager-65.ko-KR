...
이 섹션에서는 [초기 응용 프로그램](initial-app.md) 섹션에서 템플릿이 설정되었고, [초기 컨텐츠](initial-content.md) 섹션에 설정된 초기 페이지가 있으므로 커뮤니티 구성 요소를 사용하여 작성을 활성화하는 기능을 포함하는 기초 스크립트를 사용하여 응용 프로그램을 개발할 수 있습니다. 이 섹션의 마지막에 웹 사이트가 작동합니다.
...
첫 번째 단계는 리소스 슈퍼 유형 속성을 `/apps/an-scf-sandbox/components/playpage` 노드에 추가하여 슈퍼 유형의 스크립트와 속성을 상속합니다.
...
1. 스크립트 태그가 열려 있거나 닫혀 있는 경우 &quot; // TODO ...&quot;을 대체합니다. 의 헤드 및 본문 부분에 대한 스크립트가 포함되어 있습니다. &lt;html>.

   `foundation/components/page`의 슈퍼 유형이 있는 경우, 동일한 폴더에 정의되지 않은 스크립트는 `/apps/foundation/components/page` 폴더(있는 경우)의 스크립트와 다른 스크립트는 `/libs/foundation/components/page` 폴더의 스크립트로 결정됩니다.
...
1. Foundation script `head.jsp`은(는) 오버레이할 필요가 없지만 Foundation 스크립트 `body.jsp`은(는) 비어 있습니다.
   ...
   `/apps/an-scf-sandbox/components/playpage/body.jsp`을(를) 열고 다음 텍스트로 붙여 넣습니다.
