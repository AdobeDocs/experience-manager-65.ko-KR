...
1. [암호화 키를 복제합니다](#replicate-the-crypto-key). 따라서 모든 AEM 인스턴스에서 암호화/암호 해독이 올바르게 수행되는지 확인할 수 있습니다.
...
Adobe Analytics에 대한 기본 연결이 성공적으로 구성되면 커뮤니티 사이트에 대한 프레임워크를 만들거나 편집해야 합니다. 프레임워크의 목적은 커뮤니티 기능(AEM) 변수를 Analytics(보고서 세트) 변수에 매핑하는 것입니다.
...
Communities 구성 요소가 포함되어 있지만 [사이트 만들기 마법사](/help/communities/sites-console.md)를 통해 만들어지지 않은 **일반 AEM 사이트**&#x200B;의 경우 요소를 추가해야 합니다. href 값은 사이트 경로여야 합니다. 예를 들어 사이트 경로가 `/content/my/company/en`이면 다음을 사용합니다.
...
커뮤니티 eVar 및 이벤트가 보고서 세트에 이미 존재하는 매핑을 덮어쓰는 경우 데이터 손실을 방지하려면 다른 Analytics eVar 또는 이벤트에 커뮤니티 기능에 대한 AEM 변수를 할당하고 원래 매핑을 복원하십시오.
...
### Analytics와 AEM 변수 매핑 확인 {#verify-analytics-to-aem-variable-mapping}
...
>* **`event1`** ~ **`event7`**
>
>**커뮤니티 사이트를 게시하기 전에** 기존 매핑을 복원합니다. 자동으로 매핑된(커뮤니티 사이트에 대해 Analytics가 활성화된 경우) 커뮤니티 AEM 변수를 다른 Analytics 변수로 이동합니다. 이 재매핑은 모든 커뮤니티 구성 요소에서 일관되어야 합니다.
>
>이를 수행하지 않으면 복구할 수 없는 데이터 손실이 발생할 수 있습니다.
...
커뮤니티 사이트에 대해 Analytics Cloud 서비스를 사용하도록 설정하고 필요한 경우 AEM 변수에 대한 Analytics [매핑을 조정](#mapped-analytics-to-aem-variables)한 후 [(다시)커뮤니티 사이트를 게시](/help/communities/sites-console.md#publishing-the-site)하여 게시 환경에 구성을 복제하십시오.
