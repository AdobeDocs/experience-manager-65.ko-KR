...
알려진 제한은 사이트의 컨텐츠에 대해 **익명 액세스**&#x200B;가 비활성화되어 있지만 jcr 컨텐츠 및 json을 통해 커뮤니티 사이트 컨텐츠를 익명의 방문자에게 노출합니다. 그러나 이 동작은 해결 방법으로 Sling 제한을 사용하여 제어할 수 있습니다.
...
이 경험에서는 [Aaron McDonald](/help/communities/tutorials.md#demo-users)이 [커뮤니티 관리자 및 중재자](/help/communities/create-site.md#roles)의 역할을 할당받았다고 가정합니다. 그렇지 않은 경우 작성 환경으로 돌아가 사이트 설정[을 수정하고 커뮤니티 관리자 및 중재자로 Aaron McDonald를 선택합니다.](/help/communities/sites-console.md#modifying-site-properties)

오른쪽 상단 모서리에서 `Log in`을 선택하고 사용자 이름(aaron.mcdonald@mailinator.com)과 암호(암호)로 로그인합니다. Twitter 또는 Facebook 자격 증명으로 로그인할 수 있습니다.
...
* [메시지 ](/help/communities/configure-messaging.md) 옵션을 선택하면 쪽지 섹션으로 안내되며 여기서 다음을 수행할 수 있습니다.

   1. 받은 쪽지(받은 편지함), 전송(보낸 편지함), 삭제된 메시지(휴지통)를 봅니다.
   1. 새 쪽지를 작성하여 개인 및 그룹에 보냅니다.

* [알림 ](/help/communities/notifications.md) 옵션은 관심 있는 이벤트를 보고 알림 설정을 편집할 수 있는 알림 섹션으로 연결됩니다.
* [중재 ](/help/communities/published-site.md#moderationlink) 권한이 있는 경우 관리자가 AEM Communities 중재 페이지로 이동합니다.
...
작성자 환경에서도 그룹을 만들고 작성 환경의 커뮤니티 사이트([커뮤니티 그룹 콘솔](/help/communities/groups.md))에서도 관리할 수 있습니다. 이 자습서에서는 [작성자](/help/communities/nested-groups.md)에 그룹을 만드는 경험의 다음에 있습니다.
...
1. **템플릿 탭**
   ...
   * `Reference Group` 선택(중첩된 그룹을 허용하기 위해 해당 구조에 그룹 함수 포함)

1. **그룹 만들기**&#x200B;를 선택합니다.

   ![creategroup](assets/creategroup.png)

새 그룹을 만든 후, **새 스포츠 그룹**&#x200B;을 선택하여 그 안에 중첩된 두 그룹을 만듭니다. 사이트 구조는 그룹 기능으로 시작할 수 없으므로 스포츠 그룹을 연 후에는 그룹 링크를 선택해야 합니다.

![grouplink1](assets/grouplink1.png)

`Blog`으로 시작하는 두 번째 링크 집합은 현재 선택된 그룹인 `Sports` 그룹에 속합니다. Sports&#39; `Groups` 링크를 선택하면 두 그룹을 Sports 그룹 내에 중첩할 수 있습니다.

예를 들어 두 개의 `new groups`을 추가합니다.

* 이름 `Baseball`

   * `Open Group`(필수 멤버십)으로 설정하면 됩니다.
   * 템플릿 탭에서 `Conversational Group`을 선택합니다.

* 이름 `Gymnastics`

   * 설정을 `Member Only Group`(제한된 구성원)으로 변경합니다.
   * 템플릿 탭에서 `Conversational Group`을 선택합니다.
...
* 작성자의 경우 [그룹 콘솔](/help/communities/groups.md)은 세 번째 선택 사항(옵션 멤버십)을 제공합니다.`Public Group`
...
작성자 - 관리자 권한으로 [커뮤니티 그룹 콘솔](/help/communities/members.md)로 이동하고 Weston McCall을 `Community Engage Gymnastics <uid> Members` 그룹에 추가합니다.
...
Weston McCall(weston.mccall@dodgit.com / 암호)로 로그인한 다음 동일한 위치로 이동합니다. Weston은 `Join` 여는 `Baseball` 그룹과 `enter or Leave` 비공개 `Gymnastics` 그룹 중 하나를 사용할 수 있습니다.
...
예를 들어 **author** 인스턴스로 이동하고 [커뮤니티 사이트 콘솔](/help/communities/sites-console.md)에서 `engage` 폴더를 열고 **사이트 열기** 아이콘을 선택하여 작성 편집 모드로 전환합니다. 그런 다음 미리 보기 모드를 선택하여 `Web Page` 링크를 선택한 다음 편집 모드를 선택하여 제목 및 텍스트 구성 요소를 추가합니다. 마지막으로, 페이지나 전체 사이트를 다시 게시합니다.

![webpagelink](assets/webpagelink.png)

### 중재 링크 {#moderationlink}

커뮤니티 구성원이 중재 권한이 있으면 중재 링크가 표시되고 이 링크를 선택하면 게시된 커뮤니티 컨텐츠가 표시되고, 이 링크를 선택하면 작성 환경의 [중재 콘솔](/help/communities/moderation.md)과 유사한 방식으로 [중재된](/help/communities/moderate-ugc.md)이 됩니다.
...
사용자의 이름, 이메일 및 암호를 지정한 후 `Sign Up`을 선택하면 사용자가 만들어지고 해당 사용자가 서명할 수 있습니다.
