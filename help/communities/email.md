...
* **[!UICONTROL SMTP 사용 SSL]**
   ...
   선택하면 보안 이메일이 전송됩니다. 포트가 465로 설정되었는지 또는 SMTP 서버의 필요에 따라 설정되었는지 확인합니다.
* **[!UICONTROL 이메일 디버그]**
...
* 확인 **회신 이메일에서 게시물 만들기** 은(는) `unchecked`.
...
* **[!UICONTROL 제목의 최대 길이]**
   ...
   추적기 ID가 제목 줄에 추가된 경우, 이는 추적된 ID를 제외한 제목의 최대 길이이며, 이 길이를 초과하는 경우 잘립니다. 이 크기는 추적된 ID 정보가 손실되지 않도록 가능한 한 작아야 합니다. 기본값은 200입니다.
...
* **[!UICONTROL 전자 메일을 HTML으로]**: 선택하면 이메일의 콘텐츠 유형이 (으)로 설정됩니다. `"text/html;charset=utf-8"`. 기본값은 선택되어 있습니다.
...
전자 메일을 저장소로 가져오려면 폴링 가져오기를 구성하고 저장소에서 해당 속성을 수동으로 구성해야 합니다.
...
새 폴링 구성이 저장되면 프로토콜을에서 변경하려면 구독 이메일 가져오기의 속성을 추가로 수정해야 합니다. `POP3` 끝 `emailreply`.
