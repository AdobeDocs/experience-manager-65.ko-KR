...
assignments 함수는 [활성화를 위한 커뮤니티 사이트](/help/communities/overview.md#enablement-community). 커뮤니티 구성원에게 지원 리소스를 할당할 수 있습니다. 참조: [할당 기본 사항](/help/communities/essentials-assignments.md) 개발자용.

이 함수는 의 기능으로 사용할 수 있습니다. [지원 추가 기능](/help/communities/enablement.md). 지원 추가 기능을 사용하려면 프로덕션 환경에서 사용할 추가 라이선스가 필요합니다.
...
카탈로그 함수는 다음을 위한 기능을 제공합니다 [지원 커뮤니티](/help/communities/overview.md#enablement-community) 구성원이 할당되지 않은 지원 리소스를 검색할 수 있습니다. 다음을 참조하십시오 [태그 지정 지원 리소스](/help/communities/tag-resources.md) 및 [카탈로그 기본 사항](/help/communities/catalog-developer-essentials.md) 개발자용.

커뮤니티 사이트의 모든 지원 리소스 및 학습 경로는 해당 속성이 다음과 같은 경우 모든 카탈로그에 표시됩니다. ` [Show in Catalog](/help/communities/resources.md)`가 true로 설정되어 있는 경우 리소스 및 학습 경로를 명시적으로 포함하려면 을(를) 적용해야 합니다. [사전 필터링](/help/communities/catalog-developer-essentials.md#pre-filters) 카탈로그로.

템플릿에 추가될 때, 구성을 통해 사이트 방문자에게 제공되는 태그 필터를 구성하는 데 사용되는 태그 네임스페이스를 지정할 수 있습니다.

![카탈로그 기능](assets/catalog-function.png)

* [제목 및 URL 설정](#title-and-url-settings)

* **[모든 네임스페이스] 선택**

   선택한 태그 네임스페이스는 카탈로그에 나열된 지원 리소스 목록을 필터링하기 위해 방문자가 선택할 수 있는 태그를 정의합니다.
선택하면 커뮤니티 사이트에 허용된 모든 태그 네임스페이스를 사용할 수 있습니다.
선택을 취소하면 커뮤니티 사이트에 허용된 하나 이상의 네임스페이스를 선택할 수 있습니다.
기본값이 선택되어 있습니다.
