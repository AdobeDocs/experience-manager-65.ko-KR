...
구성 요소 확장의 예에서는 실제로 두 개의 구성 요소로 구성된 주석 시스템을 사용합니다
...
이 클라이언트 라이브러리를 명시적으로 포함하지 않도록 기본 주석 시스템의 clientlib에 대한 카테고리 값을 사용할 수 있습니다( `cq.social.author.hbs.comments`)를 반환하지만 이 clientlib은 기본 구성 요소의 모든 인스턴스에도 포함됩니다.
...
* 선택 **[!UICONTROL 모두 저장]**
* 포함 `/apps/custom/components/comments/clientlib`s 노드를 선택하고 3개의 파일을 만듭니다.
