...
* [중재](moderation.md) UGC(사용자 생성 콘텐츠).

* Be [특권](#privileged-members-group) 블로그, 달력, QnA 및 포럼의 항목을 만들려면

게시 환경에 등록된 사용자는 일반적으로 다음과 같습니다 *커뮤니티 구성원(구성원)* 그들을 구별하다 *사용자* 작성 환경에서 을 선택합니다.
...
* 자세한 내용은 [구성 지침](deploy-communities.md#tunnel-service-on-author) 배포 페이지에서 을 참조하십시오.
...
게시 환경에서, [설정](sites-console.md#user-management) 커뮤니티 사이트에서 사이트 방문자는 다음과 같은 커뮤니티 멤버가 될 수 있습니다.
...
사용자를 만들고 활성화 관리자의 역할에 할당할 때와 같은 단계를 수행하고 c를 추가합니다 `ommunity-administrators` 그룹의 이름을 지정합니다.
...
사용자 동기화를 통한 사용자 그룹 동기화는 향후 릴리스에 포함되지만 현재 *멤버십* 사용자 동기화가 실행되면 사용자 그룹이 동기화됩니다.
