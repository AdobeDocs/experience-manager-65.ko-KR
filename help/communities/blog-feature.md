...
* 초안 모드
* 예약된 게시
* 대신 작성([권한이 있는 구성원](/help/communities/users.md#privileged-members-group)은(는) 다른 커뮤니티 구성원을 대신하여 컨텐츠를 만들 수 있음)
...
## 페이지 {#adding-blog-components-to-a-page}에 블로그 구성 요소 추가
...
[필수 클라이언트측 라이브러리](/help/communities/blog-developer-basics.md#essentials-for-client-side)가 포함될 때 다음과 같이 `Blog` 구성 요소가 표시됩니다.
...
* **최대 썸네일 크기**
   ...
   인라인 이미지의 축소판 이미지의 최대 크기(픽셀 단위). 기본값은 800 x 800입니다.
...
**태그 필드** 탭 아래에서 **설정** 탭에서 **태그 지정 허용**&#x200B;이(가) 확인되면 적용할 태그를 지정합니다.
...
개발자를 위한 [Blog Essentials](/help/communities/blog-developer-basics.md) 페이지에서 자세한 내용을 확인할 수 있습니다.

블로그 항목 및 댓글의 중재를 보려면 [사용자 생성 콘텐츠 중재](/help/communities/moderate-ugc.md)를 참조하십시오.

블로그 항목 및 댓글에 태깅하는 방법은 [사용자 생성 콘텐츠 태그 지정](/help/communities/tag-ugc.md)을 참조하십시오.

블로그 항목 및 댓글의 번역에 대해서는 [사용자 생성 콘텐츠 번역](/help/communities/translate-ugc.md)을 참조하십시오.
