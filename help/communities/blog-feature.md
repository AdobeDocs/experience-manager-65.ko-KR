...
* 초안 모드
* 예약된 게시
* 대신 작성([권한이 있는 구성원](/help/communities/users.md#privileged-members-group)이 다른 커뮤니티 구성원을 대신하여 콘텐츠를 만들 수 있음)
* 블로그 기사 및 댓글의 [컨텍스트 내 및 벌크 중재](/help/communities/moderate-ugc.md)
...
>[!NOTE]
>
>`Journal` 및 `Journal Sidebar` 구성 요소의 제목이 `Blog` 및 `Blog Sidebar`입니다.
>
...
[필수 클라이언트측 라이브러리](/help/communities/blog-developer-basics.md#essentials-for-client-side)가 포함된 경우 `Blog` 구성 요소는 다음과 같이 표시됩니다.

![블로그 구성 요소 추가](assets/add-blog-component.png)

### 블로그 구성 {#configuring-blog}

배치된 `Blog` 구성 요소를 선택하여 편집 대화 상자를 여는 `Configure` 아이콘에 액세스하고 선택합니다.
...
* **추천 콘텐츠 허용**
  ...
  선택하면 아이디어가 [추천 콘텐츠](/help/communities/featured.md)(으)로 식별됩니다. 기본값은 선택 취소되어 있습니다.
...
**태그 필드** 탭에서 **설정** 탭에서 **태그 지정 허용**&#x200B;을 선택한 경우 적용할 태그를 지정합니다.
...
* **블로그 구성 요소 경로**
  ...
  *(선택 사항)* 블로그 기사를 나열할 블로그 리소스의 위치입니다. 비워 두면 같은 페이지에 나타나는 resourceType `social/journal/components/hbs/journal`의 구성 요소가 사용됩니다.
...
로그인한 사용자에게 중재자 또는 관리자 권한이 있는 경우 블로그에 게시된 모든 블로그 기사 및 댓글에 대해 [중재 작업](/help/communities/moderate-ugc.md)(구성 요소의 구성에서 허용된 경우)을 수행할 수 있습니다.

![중재자-홈 페이지](assets/moderator-homepage.png)

#### 구성원 {#members}

로그인한 사용자가 커뮤니티 구성원 또는 [권한이 있는 구성원](/help/communities/users.md#privileged-members-group)(구성에 따라 다름)인 경우 `New Article`을(를) 선택하여 새 블로그 문서를 만들고 게시할 수 있습니다.
...
자세한 내용은 개발자를 위한 [Blog Essentials](/help/communities/blog-developer-basics.md) 페이지에서 확인할 수 있습니다.

블로그 게시물 및 댓글의 중재에 대해서는 [사용자 생성 콘텐츠 중재](/help/communities/moderate-ugc.md)를 참조하십시오.

블로그 항목과 댓글에 태그를 지정하려면 [사용자 생성 콘텐츠에 태그 지정](/help/communities/tag-ugc.md)을 참조하세요.

블로그 게시물 및 댓글의 번역은 [사용자 생성 콘텐츠 번역](/help/communities/translate-ugc.md)을 참조하세요.
