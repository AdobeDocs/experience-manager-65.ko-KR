...
그룹 콘솔에서는 커뮤니티 사이트의 [템플릿 구조](/help/communities/sites-console.md#step1) include [그룹 함수](/help/communities/functions.md#groups-function).

* AEM Communities에서는 다른 그룹 내에 그룹 중첩을 지원합니다. 그룹 중첩은 [새 그룹의 구조](/help/communities/tools-groups.md) 그룹 함수를 포함합니다.
...
>[!NOTE]
>
>커뮤니티 사이트 콘솔에서만 액세스할 수 있는 이 그룹 콘솔은 구성원과 혼동하지 않도록 합니다 [그룹 콘솔](/help/communities/members.md) 구성원 그룹을 관리하기 위한 것입니다.
>
>구성원 그룹은 게시 환경에 등록되고 작성 환경에서 [터널 서비스](/help/communities/deploy-communities.md#tunnel-service-on-author).
...
* 폴더 내에서 커뮤니티 사이트의 인스턴스를 선택합니다.
   ...
   * 이러한 스크린샷은 다음 시작하기 자습서에서 확인할 수 있습니다 [게시할 때 그룹 만들기](/help/communities/published-site.md).
...
* **커뮤니티 그룹 이름**
   ...
   URL에 나타나는 그룹의 루트 페이지의 이름입니다. 그룹 이름에 밑줄 문자(_)와 리소스 및 구성과 같은 키워드를 사용하지 마십시오.
...
커뮤니티 사이트가 게시되면 다음을 수행할 수 있습니다 [속성 편집](#modifyinggroupproperties) 다른 테마를 선택합니다.
...
* **필수 멤버십**
   ...
   선택된 경우 커뮤니티 그룹은 열린 그룹입니다. 커뮤니티 사이트 구성원은 그룹의 콘텐츠를 볼 수 있지만 컨텐츠를 게시하려면 그룹에 가입해야 합니다. 구성원은 `Join` 게시 환경의 단추. 기본값이 선택되어 있지 않습니다.

* **제한된 멤버십**

   선택된 경우 커뮤니티 그룹은 비밀 그룹입니다. 커뮤니티 구성원은 명시적으로 초대되어야 합니다. 초대된 멤버가 검색 상자에 입력됩니다. 구성원은 나중에 [구성원 및 그룹 콘솔](/help/communities/members.md) 작성 환경. 기본값이 선택되어 있지 않습니다.
...
>[!NOTE]
>
>그룹은 지정된 대로 모든 언어로 생성됩니다 [1단계: 커뮤니티 그룹 템플릿](/help/communities/groups.md#step-community-group-template) 추가 사용 가능한 커뮤니티 그룹 언어 의 각 커뮤니티 사이트의 커뮤니티 그룹 콘솔에서 있을 수 있습니다.
...
다음 속성에 대한 세부 사항은 [그룹 만들기](#group-creation) 섹션을 참조하십시오. 게시 환경에서 만들든 작성 환경에서 만들든 모든 중첩 그룹을 수정할 수 있습니다.
...
* 사이트 구조의 커뮤니티 함수 인스턴스에서 다음을 수행합니다.
   ...
   * **`Gear icon`**
표시 제목, URL 및 [권한 있는 구성원 그룹](/help/communities/users.md#privilegedmembersgroups).
