...
게시 콘텐츠(UGC)에서 수행할 수 있는 작업은 사용자 ID 및 환경에 따라 달라집니다. 아래 표는 다음 용어를 사용하여 사용자 ID에 따른 다양한 역할을 설명합니다.
...
* [포럼](/help/communities/forum.md), [관념화](/help/communities/ideation-feature.md), [QnA](/help/communities/working-with-qna.md) 및 [일정](/help/communities/calendar.md)
**[!UICONTROL 설정]** > **[!UICONTROL 중재]**&#x200B;에서.
...
스팸 단어가 포함된 사용자 생성 게시물(블로그, 포럼, 댓글 등 모든 콘텐츠 유형)은 게시물 위에 &quot;이 게시물이 스팸으로 분류되었습니다&quot;라는 텍스트로 표시됩니다.

중재자는 이러한 게시물을 보고 사이트에 나타나지 않도록 표시할 수 있습니다. 이러한 게시물에 대한 중재 작업은 컨텍스트 내 또는 벌크 중재 UI를 통해 수행할 수 있습니다.
...
감정은 게시물(UGC)에 있는 양수 및 음수 키워드([watchwords](#configuringwatchwords)) 수를 기반으로 계산됩니다.

감정 분석은 사전 구성된 규칙 세트를 사용하며 UGC의 감정을 계산합니다. 기본 규칙은 `/libs/cq/workflow/components/workflow/social/sentiments/rules`에 있습니다.

규칙이 생성하는 값은 1(모두 음수, 양수 단어 없음)에서 10(모두 양수, 음수 단어 없음)까지입니다. 감정 값 5는 중립 감정으로, 기본값입니다.
...
분석되면 감정은 UGC와 함께 저장됩니다.

[일괄 중재 콘솔](/help/communities/moderation.md)에서 감정이 음수, 중립 또는 양수인지 여부를 기준으로 UGC를 필터링하고 볼 수 있습니다.

#### Watchwords {#watchwords}

AEM Communities은 [감정](#sentiment)을(를) 평가하는 프로세스의 단계로 *watchword 분석기*&#x200B;를 제공합니다. watchwords가 제공하는 감정 값에 대한 기여는 게시된 콘텐츠에 사용된 음수 및 양수 watchwords와 금칙어의 비교에 기인한다.

#### 감정 및 감시 단어 구성 {#configure-sentiment-and-watchwords}

긍정적 및 부정적 관찰어 목록은 감정 규칙이 될 수 있는 대로 사용자 지정할 수 있습니다.
...
* **긍정적인 Watchwords**
  ...
  기본값을 재정의하는 양수 감정에 기여하는 단어를 쉼표로 구분한 목록입니다. 기본값은 빈 목록입니다.

* **음성 Watchwords**

  기본값을 재정의하는 음수 감정에 기여하는 단어를 쉼표로 구분한 목록입니다. 기본값은 빈 목록입니다.
...
* **감정 규칙**
  ...
  양수 및 음수 watchwords를 기반으로 감정을 계산하는 규칙의 저장소 위치입니다. 기본값은 `/libs/cq/workflow/components/workflow/social/sentiments/rules`입니다(하지만 더 이상 관련된 워크플로가 없음).
