...
[활성 커뮤니티](/help/communities/overview.md#enablement-community)은(는) 비공개로 설정하는 것이 좋습니다.

익명의 사이트 방문자가 액세스가 거부되면 커뮤니티 사이트는 비공개 사이트이고, 자가 등록을 할 수 없으며, 소셜 로그인을 사용할 수 없습니다.

[사용자 관리](/help/communities/sites-console.md#user-management)에 대해 대부분의 확인란은 선택 해제되어 있는지 확인합니다.
...
커뮤니티 컨텐츠에 적용할 수 있는 태그는 이전에 [Tagging Console](/help/sites-administering/tags.md#tagging-console)(예: [Tutorial 네임스페이스](/help/communities/enablement-setup.md#create-tutorial-tags))을 통해 정의된 AEM 네임스페이스를 선택하여 제어합니다.
...
[커뮤니티 구성원 ](/help/communities/users.md) 롤이 역할 섹션의 설정을 통해 할당됩니다.
...
* 유형 `q`
* [퀸 하퍼](/help/communities/enablement-setup.md#publishcreateenablementmembers)을 선택합니다.

>[!NOTE]
>
>[터널 ](/help/communities/deploy-communities.md#tunnel-service-on-author) 서비스를 통해 게시 환경에만 존재하는 구성원 및 그룹을 선택할 수 있습니다.
...
스크린샷에 표시되는 `Communities` 선택 항목은 [구성 설명서의 프레임워크 예입니다.](/help/communities/analytics.md#aem-analytics-framework-configuration)

![chlimage_1-454](assets/chlimage_1-454.png)

#### 번역 {#translation}

[번역 설정](/help/communities/sites-console.md#translation)은 UGC를 변환할 수 있는지 여부와 어떤 언어로 변환할지 여부를 지정합니다(있는 경우).
...
(선택 사항)  [활성화 보고에서 비디오 하트비트 ](/help/communities/analytics.md#video-heartbeat-analytics) 분석을 포함하는 데 필요한 Adobe Analytics 계정의 ID입니다.
...
새 커뮤니티 사이트와 함께 다양한 관리 기능에 적합한 권한이 설정된 새 사용자 그룹이 만들어집니다. 자세한 내용은 [커뮤니티 사이트에 대한 사용자 그룹](/help/communities/users.md#usergroupsforcommunitysites)을 방문하십시오.

이 새 커뮤니티 사이트의 경우 1단계의 사이트 이름이 &quot;활성화&quot;인 경우 게시 환경에 존재하는 새 사용자 그룹은 [커뮤니티 구성원 및 그룹 콘솔](/help/communities/members.md#groups-console)에서 볼 수 있습니다.
...
작성자의 경우, 터널 서비스가 활성화되어 있으면, 새로 만든 커뮤니티 사이트의 커뮤니티 구성원 그룹에 [초기 설정 중에 만든 사용자를 할당할 수 있습니다.](/help/communities/enablement-setup.md#publishcreateenablementmembers)

커뮤니티 그룹 콘솔을 사용하여 구성원을 개별적으로 추가하거나 그룹의 멤버십을 통해 추가할 수 있습니다.

이 예제에서 그룹 `Community Ski Class`은 구성원 `Quinn Harper`뿐만 아니라 그룹 `Community Enable Members`의 구성원으로 추가됩니다.
...
사이트가 구성되고 게시되도록 푸시되면, 게시 인스턴스의 로그인 매핑[( ](/help/communities/sites-console.md#configure-for-authentication-error))을 구성합니다. `Adobe Granite Login Selector Authentication Handler` 로그인 자격 증명이 올바르게 입력되지 않으면 인증 오류가 오류 메시지와 함께 커뮤니티 사이트의 로그인 페이지가 다시 표시됩니다.

`Login Page Mapping`을(를) 다음과 같이 추가합니다.
...
이렇게 하려면 [CRX|DE](https://localhost:4503/crx/de) Lite를 사용하여 게시에서 [리소스 매핑](/help/sites-deploying/resource-mapping.md) 테이블을 편집해야 합니다.

시작하려면 다음을 수행하십시오.

1. 게시 시 CRXDE에 액세스하고 관리자 권한으로 로그인

   * 예를 들어 [https://localhost:4503/crx/de](https://localhost:4503/crx/de)로 이동하고 `admin/admin`로 로그인합니다.
...
>[!NOTE]
>
>비활성화하려면 `sling:match` 속성 값을 &#39;x&#39; - `xlocalhost.4503/$` - 및 **모두 저장**&#x200B;과(와) 함께 미리 보내기만 하면 됩니다.

![chlimage_1-364](assets/chlimage_1-364.png)

#### 문제 해결:맵 {#troubleshooting-error-saving-map} 저장 중 오류 발생

변경 사항을 저장할 수 없는 경우 노드 이름이 `localhost.4503`이고 &#39;점&#39; 구분 기호가 있고 &#39;콜론&#39; 구분 기호가 있는 `localhost:4503`이 아닌 `localhost`인지 확인하십시오.

![chlimage_1-365](assets/chlimage_1-365.png)

#### 문제 해결:{#troubleshooting-fail-to-redirect} 리디렉션 실패

정규 표현식 `sling:match` 문자열의 끝에 있는 &#39;**$**&#39;은(는) 매우 중요하므로 정확히 `https://localhost:4503/`만 매핑되고, 그렇지 않으면 리디렉션 값이 URL의 server:port 다음에 있을 수 있는 경로에 프리펜됩니다. 따라서 AEM에서 로그인 페이지로 리디렉션하려고 하면 실패합니다.

## 커뮤니티 사이트 수정 {#modifying-the-community-site}

사이트를 처음 만든 후 작성자는 [사이트 열기 아이콘](/help/communities/sites-console.md#authoring-site-content)을 사용하여 표준 AEM 작성 활동을 수행할 수 있습니다.

또한 관리자는 [사이트 편집 아이콘](/help/communities/sites-console.md#modifying-site-properties)을 사용하여 사이트 속성(예: 제목)을 수정할 수 있습니다.

수정 후에는 **Save** And rere-**Publish**&#x200B;사이트를 참조하십시오.
...
사이트의 구조를 수정하려면 **[!UICONTROL Communities]** **[!UICONTROL Sites]** 콘솔로 이동하고 `enable` 폴더를 열고 **Edit Site** 아이콘을 선택하여 `Enablement Tutorial`의 속성에 액세스하십시오.
...
커뮤니티 사이트에 대한 변경 사항을 저장하려면 오른쪽 위 모서리에 있는 **저장**&#x200B;을 선택합니다.

그런 다음 사이트를 다시-**게시**&#x200B;합니다.
