...
1. &#39;https:// `/content/communities/scorm/RecordResults` 에서 CSRF 필터의 제외된 경로 속성에 추가<hostname>:<port>게시자의 /system/console/configMgr&#39;
...
선택한 배포가 [게시 팜인](/help/communities/topologies.md#tarmk-publish-farm)경우, **notifications **나 **`primary publisher`** Adobe Analytics에 의존하는 기능과 같이 모든 인스턴스에서 발생해서는 안 되는 활동의 **대상으로 하나의 AEM 게시 인스턴스가 식별되어야 합니다**.
...
* for each target AEM instance
   ...
   * 이전에 복사한 2개 파일 붙여넣기
   * 대상 AEM 인스턴스가 현재 실행 중인 경우 Granite Crypto 번들을 [새로](#refresh-the-granite-crypto-bundle) 고쳐야 합니다.
