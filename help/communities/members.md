...
AEM Communities 기능을 사용하려면 게시 환경에서 커뮤니티에 참여하기 전에 사이트 방문자가 등록 및 로그인해야 하는 경우가 많습니다. 사용자 등록은 게시 환경에만 있어야 하며 작성 환경에 등록된 *사용자*&#x200B;와 구분하기 위해 일반적으로 멤버&#x200B;*라고 합니다.*

### 게시 {#members-users-on-publish}의 구성원(사용자)

커뮤니티 구성원 및 그룹 콘솔을 사용하면 *publish* 환경에 등록된 구성원 및 구성원 그룹이 *작성자* 환경에서 만들고 관리할 수 있습니다. 이는 [터널 서비스](deploy-communities.md#tunnel-service-on-author)가 활성화된 경우에만 가능합니다.
...
>[!NOTE]
>
>샘플 컨텐츠가 배포되고 활성화되어 있으면 많은 샘플 사용자가 작성자 및 게시 환경 모두에 존재합니다. 이러한 사용자는 [nosampleconcontent runmode](../../help/sites-administering/production-ready.md)로 실행할 때 존재하지 않습니다.
...
>[!CAUTION]
>
>[터널 서비스](deploy-communities.md#tunnel-service-on-author)가 활성화되지 않은 경우 멤버 콘솔을 사용할 수 없습니다.

![member-console1](assets/member-console1.png)

### 검색 {#search-features}

`Members` 헤더 왼쪽에 있는 사이드 패널 아이콘을 선택하여 검색 사이드 패널 열기를 전환합니다.
...
`Members` 헤더 왼쪽의 검색 아이콘을 선택하여 닫힌 검색 사이드 패널을 전환합니다.

### 멤버 통계 {#member-statistics}

`Views`, `Posts`, `Follows` 및 `Likes`을(를) 표시하는 열은 사용자가 Adobe Analytics [이(가) 활성화된 하나 이상의 커뮤니티 사이트의 구성원이면 업데이트됩니다.](sites-console.md#analytics)

### CSV 내보내기 {#export-csv}

`Export CSV` 링크를 선택하면 모든 멤버가 쉼표로 구분된 값 목록으로 다운로드됩니다. 이는 스프레드시트로 가져오는 데 적합합니다.
...
(*필수*) 구성원의 이메일 주소입니다.
멤버는 프로필을 업데이트할 때 이메일 주소를 변경할 수 있습니다.I
...
멤버에 대한 아바타를 제공하려면 **[!UICONTROL 이미지 업로드]**&#x200B;를 선택하고 .jpg, .png, .tif 또는 .gif 형식의 이미지를 선택합니다. 이미지의 기본 크기는 72dpi에서 240 x 240픽셀입니다.
...
`BADGES` 패널에서는 배지를 수동으로 할당하고 취소할 수 있습니다. 배지는 일반적으로 획득된 배지는 물론 지정된 역할에 해당될 수 있습니다.
...
* **[!UICONTROL 배지 추가]**
   * [사용 가능한 배지](badges.md)에서 선택하려면 입력을 시작합니다. 배지가 선택되면 각 사이트 또는 회원의 아바타와 함께 배지가 표시될 모든 사이트를 선택합니다.
...
* **[!UICONTROL 그룹에 구성원 추가]**
   ...
   (*선택적*) 그룹의 초기 구성원으로 포함시킬 게시 측 멤버를 선택합니다.

* **[!UICONTROL 저장]**&#x200B;을 선택합니다

## 인가된 관리자 {#authorized-administrators}

커뮤니티 구성원 콘솔에서 구성원을 사용하여 작업할 때는 적절한 권한이 있는 사용자로 로그인하고 [터널 서비스](deploy-communities.md#tunnel-service-on-author)에서 사용하는 복제 에이전트가 올바르게 구성되어야 합니다.

`admin`으로 로그인하지 않은 경우 로그인한 사용자는 `administrators` 사용자 그룹의 구성원이어야 합니다.

작성자[의 복제 에이전트도 참조하십시오.](deploy-communities.md#replication-agents-on-author)
