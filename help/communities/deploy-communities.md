...
* 라이선스 옵션:
   ...
   * [ASRP용 Adobe Cloud](/help/communities/asrp.md)
...
* 세계화를 지원하는 경우 자동 번역 [설정](/help/sites-administering/translation.md)(개발을 위해 샘플 설정이 제공됨)
...
* 공유 UGC 스토리지 [(SRP)에 대한 선택](/help/communities/working-with-srp.md) 사항&#x200B;****&#x200B;확인
   ...
   * 관계형 데이터베이스 SRP [(DSRP)의 경우](/help/communities/dsrp.md)
   ...
   * Adobe SRP [(ASRP)의 경우](/help/communities/asrp.md)

      * 프로비전을 위해 계정 담당자에게 문의
      * [ASRP 선택](/help/communities/srp-config.md)
   * JCR SRP [(JSRP)의 경우](/help/communities/jsrp.md)
...
* DSRP [의](/help/communities/dsrp.md) 경우:사용자 생성 컨텐츠 저장(UGC)
...
번들 설치에 대한 자세한 내용은 웹 콘솔 [페이지에서 확인할 수](/help/sites-deploying/web-console.md) 있습니다.
...
AEM Communities SCORM 엔진은 [활성화](/help/communities/overview.md#enablement-community) 기능을 위해 필요합니다. AEM 6.5 Communities에서 지원되는 Scorm 패키지:
...
자세한 내용 및 설치 정보는 SRP [용](/help/communities/solr.md) 솔루션 구성을 참조하십시오.

### 패키지 공유 링크 정보 {#about-links-to-package-share}

**Adobe AEM Cloud에 표시되는 패키지**

이 페이지에서 패키지에 대한 링크는 패키지 공유를 위해 AEM의 실행 인스턴스가 필요하지 않습니다 `adobeaemcloud.com`. 패키지를 볼 수 있지만 이 `Install` 단추는 Adobe 호스팅 사이트에 패키지를 설치하는 것입니다. 로컬 AEM 인스턴스에 설치하려면 이 옵션을 선택하면 오류가 `Install` 발생합니다.
...
AEM Communities에서 공용 스토어는 사용자 생성 콘텐츠(UGC)를 저장하는 데 사용되며 종종 [저장소 리소스 공급자(SRP)](/help/communities/working-with-srp.md)라고도 합니다. 권장되는 배포 센터는 일반 스토어에 대한 SRP 옵션을 선택하는 것입니다.

공용 저장소는 게시 환경에서 UGC를 중재 및 분석하면서 UGC를 [복제할](/help/communities/sync.md) 필요성을 제거합니다.

* [커뮤니티 콘텐츠 스토어](/help/communities/working-with-srp.md) :aem 커뮤니티의 SRP 스토리지 옵션에 대해 설명합니다.

* [권장 토폴로지](/help/communities/topologies.md) :사용 사례 및 SRP 선택에 따라 사용할 토폴로지에 대해 설명합니다.

## 업그레이드 {#upgrading}

이전 버전의 AEM에서 AEM 6.5 플랫폼으로 업그레이드할 때 AEM 6.5 [로 업그레이드를 읽어야 합니다](/help/sites-deploying/upgrade.md).

플랫폼 업그레이드 외에 AEM Communities [6.5로](/help/communities/upgrade.md) 업그레이드를 참조하여 커뮤니티 변경에 대해 알아보십시오.

## 구성 {#configurations}

### 기본 게시자 {#primary-publisher}

선택한 배포가 [게시 팜인](/help/communities/topologies.md#tarmk-publish-farm)경우, 하나의 AEM 게시 인스턴스가 **`primary publisher`** 알림 **또는 Adobe Analytics를 사용하는 기능과 같이 모든 인스턴스에서 발생해서는 안 되는** 활동의 대상으로 식별되어야 **합니다**.
...
* 두 에이전트 모두에 대해 동일한 절차를 따르십시오.
   ...
   * **역방향 복제 에이전트(역 게시)**
      ...
      1. 포트가 없는 `4503`경우 URI **를** 편집하여 올바른 포트를 지정하십시오.

      1. 사용자가 아닌 `admin`경우 사용자 **및** 암호를 편집하여 **** `administrators` 사용자 그룹의 구성원을 지정합니다.
...
작성 환경을 사용하여 사이트를 [만들고, 사이트 속성을](/help/communities/sites-console.md)[](/help/communities/sites-console.md#modifying-site-properties) 수정하거나, 커뮤니티 구성원을 [](/help/communities/members.md)관리하는 경우, 작성자에 등록된 사용자가 아니라 게시 환경에 등록된 구성원(사용자)에 액세스해야 합니다.
...
* On **author**
* 관리자 권한으로 로그인
* 게시자가 localhost:4503이 아니거나 전송 사용자가 아닌 `admin`경우 복제 에이전트를 [구성합니다.](#replication-agents-on-author)
...
모든 AEM 서버 인스턴스가 동일한 암호화 키를 사용해야 하는 AEM Communities의 두 가지 기능이 있습니다. Analytics [및](/help/communities/analytics.md) ASRP입니다 [](/help/communities/asrp.md).
...
* [Dispatcher for Communities 구성](/help/communities/dispatcher.md)
* [알려진 문제](/help/communities/troubleshooting.md#dispatcher-refetch-fails)

## 관련 커뮤니티 설명서 {#related-communities-documentation}

* 커뮤니티 사이트 [관리를](/help/communities/administer-landing.md) 방문하여 커뮤니티 사이트 만들기, 커뮤니티 사이트 템플릿 구성, 커뮤니티 콘텐츠 중재, 구성원 관리 및 메시지 구성에 대해 알아보십시오.
