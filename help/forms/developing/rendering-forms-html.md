...
# HTML로 양식 렌더링 {#rendering-forms-as-html}
...
페이지 탐색 기능과 양식 스크립트를 모두 포함하는 HTML 양식을 렌더링할 때(예: 스크립트가 양식을 렌더링할 때마다 데이터베이스에서 필드 데이터를 검색한다고 가정) 양식 스크립트가 양식:readyevent이 아닌 양식:calculate 이벤트에 있는지 확인하십시오.

양식 :ready 이벤트에 있는 양식 스크립트는 양식의 초기 렌더링 동안 한 번만 실행되며 후속 페이지 검색 시 실행되지 않습니다. 반대로 양식 :calculate 이벤트는 양식이 렌더링되는 각 페이지 탐색에 대해 실행됩니다.
