...
* 서명 서비스는 PDF 문서에 디지털 서명을 하는 등의 작업을 수행하는 동안 인증서와 자격 증명을 액세스합니다. ([PDF 문서 디지털 서명](/help/forms/developing/digitally-signing-certifying-documents.md#digitally-signing-pdf-documents)을 참조하십시오.)
...
## 트러스트 관리자 API {#importing-credentials-by-using-the-trust-manager-api}를 사용하여 자격 증명 가져오기
...
자격 증명을 가져올 때는 자격 증명을 위한 별칭을 지정합니다. 별칭은 자격 증명이 필요한 Forms 작업을 수행하는 데 사용됩니다. 자격 증명을 가져온 후에는 다음 그림과 같이 관리 콘솔에서 볼 수 있습니다. 자격 증명의 별칭은 *Secure*&#x200B;입니다.
...
1. 가져오기 작업 수행
   ...
   * 하나의 요소를 포함하는 문자열 배열을 만듭니다. `truststore.usage.type.sign` 값을 요소에 지정합니다.
   * `CredentialServiceClient` 개체의 `importCredential` 메서드를 호출하고 다음 값을 전달합니다.
      ...
      * 사용 값을 포함하는 문자열 배열. 예를 들어 이 값 `truststore.usage.type.sign`을 지정할 수 있습니다. Reader 확장 자격 증명을 가져오려면 `truststore.usage.type.lcre`을 지정합니다.
...
## 트러스트 관리자 API {#deleting-credentials-by-using-the-trust-manager-api}를 사용하여 자격 증명 삭제
...
자격 증명을 프로그램 방식으로 삭제하려면 먼저 데이터 통합 서비스 클라이언트를 만드십시오. 서비스 클라이언트를 만들 때 서비스를 호출하는 데 필요한 연결 설정을 정의합니다. 자세한 내용은 [연결 속성 설정](/help/forms/developing/invoking-aem-forms-using-java.md#setting-connection-properties)을 참조하십시오.
