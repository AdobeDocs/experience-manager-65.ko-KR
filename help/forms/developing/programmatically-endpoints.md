...
* **filePattern**: 공급자가 허용하는 들어오는 파일 첨부 패턴을 지정합니다. 여기에는 특정 파일 이름 확장명(&amp;ast;.dat, &amp;ast;.xml)이 있는 파일, 특정 이름(데이터)이 있는 파일 및 이름 및 확장명(&amp;ast;)에 복합 표현식이 있는 파일이 포함됩니다.[dD][aA]&#39;port&#39;) 기본값은 `*`입니다.
