...
# 파일 포맷과 PDF {#converting-between-file-formatsand-pdf} 간 변환

**PDF 서비스 생성 정보**

PDF 생성 서비스는 기본 파일 형식을 PDF로 변환합니다. 또한 PDF를 다른 파일 포맷으로 변환하고 PDF 문서의 크기를 최적화합니다.

PDF 생성 서비스는 기본 애플리케이션을 사용하여 다음 파일 포맷을 PDF로 변환합니다. 별도로 명시되어 있지 않는 한 이러한 애플리케이션의 독일어, 프랑스어, 영어 및 일본어 버전만 지원됩니다. *Windows* 는 Windows Server® 2003 및 Windows Server 2008만 지원합니다.
...
>[!NOTE]
>
>PDF 생성 서비스에 대한 자세한 내용은 [AEM Forms에 대한 서비스 참조](https://www.adobe.com/go/learn_aemforms_services_63)를 참조하십시오.
...
>[!NOTE]
>
>추가 파일 형식에 대한 자세한 내용은 [추가 기본 파일 형식 지원 추가](converting-file-formats-pdf.md#adding-support-for-additional-native-file-formats)를 참조하십시오.
...
PDF 서비스 클라이언트를 생성한 후 `createPDF2` 메서드를 호출할 수 있습니다. 이 방법에는 파일 확장자를 포함하여 변환할 문서에 대한 정보가 필요합니다.
...
1. 파일을 PDF 문서로 변환
   ...
   * 변환에 사용할 파일 형식 설정을 포함하는 `java.lang.String` 개체입니다. 파일 유형 설정은 .doc 또는 .xls와 같은 다양한 파일 유형에 대한 변환 설정을 제공합니다.
   ...
   * PDF 문서에 적용할 메타데이터 정보를 포함하는 선택적 `com.adobe.idp.Document` 개체입니다.
...
1. 결과 검색
   ...
   * `CreatePDFResult` 개체의 `getCreatedDocument` 메서드를 호출하여 `com.adobe.idp.Document` 개체를 반환합니다.
   * `com.adobe.idp.Document` 개체의 `copyToFile` 메서드를 호출하여 이전 단계에서 만든 개체에서 PDF 문서를 추출합니다.

   `createPDF2` 메서드를 사용하여 로그 문서를 얻은 경우(HTML 변환에 해당되지 않음) 다음 작업을 수행하십시오.
...
1. PDF 클라이언트 생성
   ...
   * `System.ServiceModel.EndpointAddress` 생성자를 사용하여 `GeneratePDFServiceClient.Endpoint.Address` 개체를 만듭니다. WSDL을 지정하는 문자열 값을 AEM Forms 서비스에 전달합니다(예: `http://localhost:8080/soap/services/GeneratePDFService?blob=mtom`). `lc_version` 특성을 사용할 필요는 없습니다. 하지만 `?blob=mtom`을 지정합니다.
   * `GeneratePDFServiceClient.Endpoint.Binding` 필드의 값을 가져와 `System.ServiceModel.BasicHttpBinding` 개체를 만듭니다. 반환 값을 `BasicHttpBinding`으로 캐스팅합니다.
...
1. PDF 문서로 변환할 파일을 검색합니다.
   ...
   * 바이트 배열의 내용을 `MTOM` 속성에 할당하여 `BLOB` 개체를 채웁니다.
...
1. 파일을 PDF 문서로 변환
   ...
   * 파일 확장자를 포함하는 문자열.
   * 변환에 사용할 파일 형식 설정을 포함하는 `java.lang.String` 개체입니다. 파일 유형 설정은 .doc 또는 .xls와 같은 다양한 파일 유형에 대한 변환 설정을 제공합니다.
   * 사용할 PDF 설정을 포함하는 문자열 오브젝트입니다. `Standard`을(를) 지정할 수 있습니다.
   * 사용할 보안 설정을 포함하는 문자열 개체입니다. `No Security`을(를) 지정할 수 있습니다.
   * PDF 문서를 생성하는 동안 적용할 설정이 포함된 선택적 `BLOB` 개체입니다.
   * PDF 문서에 적용할 메타데이터 정보를 포함하는 선택적 `BLOB` 개체입니다.
   * `CreatePDF2` 메서드로 채워진 `BLOB` 유형의 출력 매개 변수입니다. `CreatePDF2` 메서드는 이 개체를 변환된 문서로 채웁니다. 이 매개 변수 값은 웹 서비스 호출에만 필요합니다.
   * `CreatePDF2` 메서드로 채워진 `BLOB` 유형의 출력 매개 변수입니다. `CreatePDF2` 메서드는 이 개체를 로그 문서로 채웁니다. 이 매개 변수 값은 웹 서비스 호출에만 필요합니다.

1. 결과 검색

   * 바이트 배열에 `BLOB` 개체의 `MTOM` 필드를 할당하여 변환된 PDF 문서를 검색합니다. 바이트 배열은 변환된 PDF 문서를 나타냅니다. `createPDF2` 메서드의 출력 매개 변수로 사용되는 `BLOB` 개체를 사용해야 합니다.
...
프로그래밍 방식으로 PDF 생성 작업을 수행하려면 먼저 PDF 생성 서비스 클라이언트를 만들어야 합니다. Java API를 사용하는 경우 `GeneratePdfServiceClient` 개체를 만듭니다. 웹 서비스 API를 사용하는 경우 `GeneratePDFServiceService`을(를) 만드십시오.
...
### Java API {#convert-html-content-to-a-pdf-document-using-the-java-api}를 사용하여 HTML 내용을 PDF 문서로 변환
...
1. PDF 클라이언트 생성
   ...
   생성자를 사용하여 `GeneratePdfServiceClient` 개체를 만들고 연결 속성이 포함된 `ServiceClientFactory` 개체를 전달합니다.
...
1. HTML 내용을 PDF 문서로 변환
   ...
   * 변환할 HTML 파일의 URL을 포함하는 `java.lang.String` 개체
   * 변환에 사용할 파일 형식 설정을 포함하는 `java.lang.String` 개체입니다. 파일 유형 설정에는 할당 수준이 포함될 수 있습니다.
   * 사용할 보안 설정의 이름을 포함하는 `java.lang.String` 개체
   * PDF 문서를 생성하는 동안 적용할 설정이 포함된 선택적 `com.adobe.idp.Document` 개체입니다. 이 정보가 제공되지 않으면 설정은 이전 세 개의 매개 변수를 기반으로 자동으로 선택됩니다.
   * PDF 문서에 적용할 메타데이터 정보를 포함하는 선택적 `com.adobe.idp.Document` 개체입니다.

1. 결과 검색

   `htmlToPDF2` 메서드는 생성된 새 PDF 문서를 포함하는 `HtmlToPdfResult` 개체를 반환합니다. 새로 만든 PDF 문서를 얻으려면 다음 작업을 수행하십시오.
...
1. 결과 검색
   ...
   * 바이트 배열에 `BLOB` 개체의 `MTOM` 필드를 할당하여 변환된 PDF 문서를 검색합니다. 바이트 배열은 변환된 PDF 문서를 나타냅니다. `HtmlToPDF2` 메서드의 출력 매개 변수로 사용되는 `BLOB` 개체를 사용해야 합니다.
   * 생성자를 호출하고 변환된 PDF 문서의 파일 위치를 나타내는 문자열 값을 전달하여 `System.IO.FileStream` 개체를 만듭니다.
   * 생성자를 호출하고 `System.IO.FileStream` 개체를 전달하여 `System.IO.BinaryWriter` 개체를 만듭니다.
   * `System.IO.BinaryWriter` 개체의 `Write` 메서드를 호출하고 바이트 배열을 전달하여 바이트 배열의 내용을 PDF 파일에 기록합니다.
...
1. PDF 문서 변환
   ...
   * 파일 위치를 지정하는 `java.lang.String` 개체
   * 변환의 대상 파일 유형을 지정하는 문자열 개체 지정 `RTF`.
   * PDF 문서를 생성하는 동안 적용할 설정이 포함된 선택적 `BLOB` 개체입니다.
   * `ExportPDF2` 메서드로 채워진 `BLOB` 유형의 출력 매개 변수입니다. `ExportPDF2` 메서드는 이 개체를 변환된 문서로 채웁니다. 이 매개 변수 값은 웹 서비스 호출에만 필요합니다.
...
*AppMonis* 는 사용자가 해당 애플리케이션에서 제공하는 대화 상자를 탐색하는 것과 동일한 방식으로 기본 애플리케이션과 상호 작용하는 기업 구성 요소입니다. AppMon이 Microsoft Word와 같은 응용 프로그램을 열어 인쇄하도록 지시하는 데 사용하는 XML 그램에는 다음과 같은 순차적 작업이 포함됩니다.
...
부동 메뉴 막대를 시각적으로 손쉽게 식별할 수 있습니다.하지만 시각적인 조사만으로 특수한 유형의 대화를 구별할 수는 없을 것이다. 대화 상자를 검사하여 AppMon이 표준 Win32 API를 사용하여 AppMon과 상호 작용할 수 있는지 확인하기 위해 Microsoft Spy++(Microsoft Visual C++ 개발 환경의 일부) 또는 이와 동등한 WinID([https://www.dennisbabkin.com/php/download.php?what=WinID](https://www.dennisbabkin.com/php/download.php?what=WinID)에서 무료로 다운로드할 수 있음)와 같은 타사 응용 프로그램이 필요합니다.
...
<table>
 ...
 <tbody>
  ...
  <tr>
   ...
   <td><p>이 유형의 파일은 XML 편집 응용 프로그램을 사용하여 만들고 수정할 수 있습니다. 기본 응용 프로그램<a href="converting-file-formats-pdf.md#creating-or-modifying-an-additional-dialog-xml-file-for-a-native-application">에 대한 추가 대화 상자 XML 파일 만들기 또는 수정을 참조하십시오.</a> </p><p><strong>중요</strong>:서버가 지원할 각 기본 응용 프로그램에 대한 추가 응용 프로그램별 대화 상자 지침을 만들어야 합니다. </p></td>
  </tr>
 </tbody>
</table>
...
이 섹션 및 다음 섹션에서 사용자의 관점에서 대화 상자와 해당 구성 요소를 설명하는 경우 *대화 상자*, *button*, *field* 및 *콤보 상자*&#x200B;와 같은 용어가 사용됩니다.

이 섹션과 다음 섹션에서 내부 표현의 관점에서 대화 상자와 해당 구성 요소를 설명하는 경우 *window 요소* 용어가 사용됩니다. 창 요소의 내부 표시는 각 창 요소 인스턴스가 레이블로 식별되는 계층입니다. 또한 창 요소 인스턴스는 물리적인 특성과 동작을 설명합니다.
...
이 섹션에서는 AppMon에 지침을 제공하는 XML 요소에 대해 설명합니다. 이러한 요소에는 `dialog` 요소 및 `window` 요소와 같은 이름이 있습니다. 이 문서에서는 고정 폭 글꼴을 사용하여 XML 요소를 구별합니다. `dialog` 요소는 XML 스크립트 파일이 의도적이거나 의도치 않게 표시될 수 있는 대화 상자를 식별합니다. `window` 요소는 창 요소(대화 상자 또는 대화 상자의 구성 요소)를 식별합니다.
...
*스크립트 XML 파일*&#x200B;은 기본 응용 프로그램이 특정 창 요소로 이동한 다음 해당 요소에 대한 응답을 제공하도록 지시하는 일련의 단계를 지정합니다. 대부분의 응답은 사용자가 해당 대화 상자의 필드, 콤보 상자 또는 단추에 제공하는 입력에 해당하는 텍스트 또는 키 입력입니다.
...
*대화 상자 XML 파일*&#x200B;은 PDF 생성 서비스가 시스템 또는 기본 응용 프로그램 대화 상자에 응답하는 방식을 지정합니다. 대화 상자 XML 파일을 통해 PDF 생성 서비스가 메시지가 표시되지 않는 대화 상자에 손쉽게 응답하여 변환 프로세스를 진행할 수 있습니다.
...
특성이 캡션을 식별하지 못하는 경우 대신 상위에 대한 색인을 사용하여 창 요소를 식별할 수 있습니다. *index*&#x200B;는 동위 창 요소를 기준으로 창 요소의 위치를 지정합니다. 콤보 상자를 식별할 수 있는 유일한 방법은 자주 색인입니다.

다음 문제에 주의하십시오.

* Microsoft Spy++는 앰퍼샌드(&amp;)를 사용하여 캡션의 핫키를 식별하여 캡션을 표시합니다. 예를 들어 Spy++는 하나의 인쇄 대화 상자에 대한 캡션을 `Pri&nt`로 보여 줍니다. 이는 핫키가 *n*&#x200B;임을 나타냅니다. 스크립트 및 대화 상자 XML 파일의 캡션 제목에는 앰퍼샌드를 생략해야 합니다.
* 일부 캡션에는 줄 바꿈이 포함됩니다. PDF 생성 서비스는 줄바꿈을 식별할 수 없습니다. 캡션에 줄 바꿈이 포함되어 있는 경우 캡션을 충분히 포함시켜 다른 메뉴 항목과 구분한 다음 누락된 부분에 정규 표현식을 사용합니다. 예로는 ( `^Long caption title$`)이 있습니다. 캡션 특성에서 [정규 표현식 사용](converting-file-formats-pdf.md#using-regular-expressions-in-caption-attributes)을 참조하십시오.
* 예약된 XML 문자에 문자 엔티티(이스케이프 시퀀스라고도 함)를 사용합니다. 예를 들어, 앰퍼맨드에는 `&`, 기호보다 작거나 큰 경우 `<` 및 `>`, 아포스트로피에는 `&apos;`, 따옴표는 `&quot;`를 사용하십시오.
...
1. 대화 상자 및 스크립트 XML 파일을 appmondata.jar 파일에 추가하거나 이 파일에서 기존 XML 파일을 수정합니다. (기본 응용 프로그램[및 ](converting-file-formats-pdf.md#creating-or-modifying-a-script-xml-file-for-a-native-application)기본 응용 프로그램에 대한 스크립트 XML 파일 만들기 또는 수정 참조[](converting-file-formats-pdf.md#creating-or-modifying-an-additional-dialog-xml-file-for-a-native-application)
1. WinZip 또는 WinRAR와 같은 도구를 사용하여 adobe-generatepdf-dsc.jar > adobe-appmondata.jar를 엽니다.
1. 대화 상자 및 스크립트 XML 파일을 appmondata.jar 파일에 추가하거나 이 파일에서 기존 XML 파일을 수정합니다. (기본 응용 프로그램[및 ](converting-file-formats-pdf.md#creating-or-modifying-a-script-xml-file-for-a-native-application)기본 응용 프로그램에 대한 스크립트 XML 파일 만들기 또는 수정 참조[ ](converting-file-formats-pdf.md#creating-or-modifying-an-additional-dialog-xml-file-for-a-native-application) adobe-appmondata.jar 파일에 XML 파일을 추가한 후 새로운 adobe-appmondata.jar 파일을 adobe-generatepdf-dsc.jar 파일에 넣습니다.
1. 추가 기본 파일 형식에 대한 지원을 추가한 경우 응용 프로그램의 경로를 제공하는 시스템 환경 변수를 만드십시오(기본 응용 프로그램[을 찾으려면 환경 변수 만들기를 참조하십시오.)](converting-file-formats-pdf.md#creating-an-environment-variable-to-locate-the-native-application)
...
스크립트에는 기본 응용 프로그램의 창 요소를 탐색하고 해당 요소에 대한 특정 응답을 제공하는 지침이 들어 있습니다. 이 정보를 포함하는 파일은 `appmon.`[appname]&quot;`.script.`[locale]`.xml`입니다. 예: appmon.notepad.script.en_US.xml

#### 스크립트가 {#identifying-steps-the-script-must-execute}을(를) 실행해야 하는 단계 식별
...
#### 캡션 속성 {#identifying-the-dialogs-specified-in-caption-attributes}에 지정된 대화 상자 식별
...
#### 창 및 windowList 요소 {#ordering-the-window-and-windowlist-elements} 순서 지정

다음과 같이 `window` 및 `windowList` 요소를 주문해야 합니다.

* `window` 요소가 `windowList` 또는 `dialog` 요소에서 자식으로 표시되는 경우 순서에 있는 위치를 나타내는 `caption` 이름의 길이와 함께 이러한 `window` 요소를 내림차순으로 정렬합니다.
* `window` 요소에 여러 `windowList` 요소가 나타나는 경우 해당 `windowList` 요소를 내림차순으로 정렬하고 첫 번째 `indexes/` 요소의 길이가 순서의 위치를 나타냅니다.`caption`
...
이전에 지원되지 않았던 기본 응용 프로그램의 스크립트를 만드는 경우 해당 응용 프로그램에 대한 추가 대화 상자 XML 파일도 만들어야 합니다. AppMon에서 사용하는 모든 기본 응용 프로그램에는 대화 상자 XML 파일이 하나만 있어야 합니다. 요청되지 않은 대화 상자가 없어도 추가 대화 상자 XML 파일이 필요합니다. `window` 요소가 자리 표시자일 경우에도 추가 대화 상자에는 `window` 요소가 하나 이상 있어야 합니다.
...
추가 dialogXML 파일을 식별하는 파일 이름은 `appmon.[appname].addition.[locale].xml`입니다. 예: appmon.excel.addition.en_US.xml
...
>[!NOTE]
>
>native2pdfconfig.xml 구성 파일에 지정된 일반 응용 프로그램에는 기본 대화 상자 XML 파일이 없습니다. 기본 파일 형식](converting-file-formats-pdf.md#adding-or-modifying-support-for-a-native-file-format)에 대한 지원 추가 또는 수정 섹션에서는 이러한 사양에 대해 설명합니다.[

`window` 요소에 자식으로 표시되는 `windowList` 요소를 주문해야 합니다. ([창 및 windowList 요소 순서 지정](converting-file-formats-pdf.md#ordering-the-window-and-windowlist-elements)을 참조하십시오.)
...
1. **홈** > **서비스** > **Adobe PDF 생성기** > **구성 파일**&#x200B;을 선택한 다음 **내보내기 구성**&#x200B;을 선택합니다.
1. 필요에 따라 native2pdfconfig.xml 파일의 `filetype-settings` 요소를 수정합니다.
1. **홈** > **서비스** > **Adobe PDF 생성기** >**구성 파일**&#x200B;을 선택한 다음 **구성 가져오기**&#x200B;를 선택합니다. 구성 데이터를 PDF 생성 서비스로 가져와 이전 설정을 대체합니다.

>[!NOTE]
>
>응용 프로그램의 이름은 `GenericApp` 요소의 `name` 특성 값으로 지정됩니다. 이 값은 해당 응용 프로그램에 대해 개발한 스크립트에 지정된 해당 이름과 정확히 일치해야 합니다. 마찬가지로 `GenericApp` 요소의 `displayName` 속성은 해당 스크립트의 `expectedWindow` 창 캡션과 정확히 일치해야 합니다. 이러한 동등성은 `displayName` 또는 `caption` 속성에 나타나는 일반 표현식을 해결한 후에 평가됩니다.
...
기본 응용 프로그램 실행 파일의 위치를 지정하는 환경 변수를 만듭니다. 이 변수는 `[applicationname]_PATH` 형식을 사용해야 합니다. 여기서 *applicationname*&#x200B;은 XML 구성 파일 및 스크립트에서 사용되는 응용 프로그램 이름과 정확히 일치해야 하며, 경로에 실행 파일의 경로가 큰따옴표로 묶여있습니다. 이러한 환경 변수의 예는 `Photoshop_PATH`입니다.
