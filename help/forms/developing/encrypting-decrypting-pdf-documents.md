...
암호화할 PDF 문서 리소스를 지정해야 합니다. 전체 PDF 문서를 암호화할 수 있습니다. 문서의 메타데이터를 제외한 모든 항목이나 문서의 첨부 파일만 암호화할 수 있습니다. 문서의 첨부 파일만 암호화하면 사용자가 첨부 파일에 액세스하려고 할 때 암호를 입력하라는 메시지가 표시됩니다.
...
1. 암호화 런타임 옵션을 설정합니다.
   ...
   * 다음을 호출하여 권한 지정 `java.util.List` 개체 &#39;s `add` 설정할 권한에 해당하는 열거형 값을 전달합니다. 예를 들어 사용자가 PDF 문서에 있는 데이터를 복사할 수 있도록 하는 권한을 설정하려면 다음을 지정합니다 `PasswordEncryptionPermission.PASSWORD_EDIT_COPY`. 설정할 각 권한에 대해 이 단계를 반복합니다.
...
PDF 문서를 보호해야 하는 암호화 유형을 결정할 수 있습니다.
