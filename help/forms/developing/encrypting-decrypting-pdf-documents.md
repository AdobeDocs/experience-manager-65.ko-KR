...
* 암호로 PDF 문서를 암호화합니다. ([암호](encrypting-decrypting-pdf-documents.md#encrypting-pdf-documents-with-a-password)를 사용하여 PDF 문서 암호화 참조)
* 인증서를 사용하여 PDF 문서를 암호화합니다. (인증서[를 사용하여 PDF 문서 암호화 참조)](encrypting-decrypting-pdf-documents.md#encrypting-pdf-documents-with-certificates)
* PDF 문서에서 암호 기반의 암호화 제거 ([암호 암호화 제거](encrypting-decrypting-pdf-documents.md#removing-password-encryption)를 참조하십시오.)
* PDF 문서에서 인증서 기반의 암호화 제거 ([인증서 기반 암호화 제거](encrypting-decrypting-pdf-documents.md#removing-certificate-based-encryption)를 참조하십시오.)
* 다른 서비스 작업을 수행할 수 있도록 PDF 문서의 잠금을 해제합니다. 예를 들어 암호로 암호화된 PDF 문서의 잠금을 해제하면 디지털 서명을 적용할 수 있습니다. ([암호화된 PDF 문서 잠금 해제 참조](encrypting-decrypting-pdf-documents.md#unlocking-encrypted-pdf-documents))
* 보안 PDF 문서의 암호화 유형을 확인합니다. ([암호화 유형 결정 참조](encrypting-decrypting-pdf-documents.md#determining-encryption-type))

>[!NOTE]
>
>암호화 서비스에 대한 자세한 내용은 [AEM Forms에 대한 서비스 참조](https://www.adobe.com/go/learn_aemforms_services_63)를 참조하십시오.
...
>[!NOTE]
>
>암호화된 PDF 문서를 AEM Forms 저장소에 업로드하면 PDF 문서의 암호를 해독하여 XDP 컨텐츠를 추출할 수 없습니다. AEM Forms 저장소에 업로드하기 전에 문서를 암호화하지 않는 것이 좋습니다. ([리소스 쓰기](/help/forms/developing/aem-forms-repository.md#writing-resources)를 참조하십시오.)
...
1. 암호화 런타임 옵션을 설정합니다.
   ...
   * `java.util.List` 개체 &#39;s `add` 메서드를 호출하고 설정할 권한에 해당하는 열거형 값을 전달하여 권한을 지정합니다. 예를 들어 사용자가 PDF 문서에 있는 데이터를 복사할 수 있는 권한을 설정하려면 `PasswordEncryptionPermission.PASSWORD_EDIT_COPY`을 지정합니다. 설정할 각 권한에 대해 이 단계를 반복합니다.
...
1. 암호를 추가합니다.
   ...
   * 암호로 암호화할 PDF 문서를 포함하는 `com.adobe.idp.Document` 개체
...
1. 암호화 클라이언트 API 개체를 만듭니다.
   ...
   * `System.ServiceModel.EndpointAddress` 생성자를 사용하여 `EncryptionServiceClient.Endpoint.Address` 개체를 만듭니다. WSDL을 지정하는 문자열 값을 AEM Forms 서비스에 전달합니다(예: `http://localhost:8080/soap/services/EncryptionService?WSDL`). `lc_version` 특성을 사용할 필요는 없습니다. 이 속성은 서비스 참조를 만들 때 사용됩니다.)
   * `EncryptionServiceClient.Endpoint.Binding` 필드의 값을 가져와 `System.ServiceModel.BasicHttpBinding` 개체를 만듭니다. 반환 값을 `BasicHttpBinding`으로 캐스팅합니다.
...
1. 암호화 런타임 옵션을 설정합니다.
   ...
   * 사용자가 `PasswordEncryptionOptionSpec` 개체의 `documentOpenPassword` 데이터 멤버에 열기 암호를 나타내는 문자열 값을 할당하여 암호화된 PDF 문서를 열 수 있도록 하는 암호 값을 지정합니다.
   * 마스터 암호를 나타내는 문자열 값을 `PasswordEncryptionOptionSpec` 개체의 `permissionPassword` 데이터 멤버에 할당하여 사용자가 PDF 문서에서 암호화를 제거할 수 있는 암호 값을 지정합니다.

1. 암호를 추가합니다.

   `EncryptionServiceClient` 개체의 `encryptPDFUsingPassword` 메서드를 호출하고 다음 값을 전달하여 PDF 문서를 암호화합니다.

   * 암호로 암호화할 PDF 문서를 포함하는 `BLOB` 개체
   * 암호화 런타임 옵션이 포함된 `PasswordEncryptionOptionSpec` 개체입니다.
...
1. 암호화된 PDF 문서를 PDF 파일로 저장할 수 있습니다.
   ...
   * `encryptPDFUsingPassword` 메서드에서 반환된 `BLOB` 개체의 데이터 내용을 저장하는 바이트 배열을 만듭니다. `BLOB` 개체의 `MTOM` 데이터 멤버의 값을 가져와 바이트 배열을 채웁니다.
...
인증서 기반의 암호화를 사용하면 공개 키 기술을 통해 특정 수신자의 문서를 암호화할 수 있습니다. 문서에 대해 다양한 권한을 받는 사람에게 부여할 수 있습니다. 암호화의 많은 측면들은 공개 키 기술에 의해 가능하다. 알고리즘은 다음 속성을 가진 *keys*&#x200B;이라는 큰 숫자를 생성하는 데 사용됩니다.
...
>[!NOTE]
>
>인증서를 사용하여 PDF 문서를 암호화하기 전에 AEM Forms에 인증서를 추가해야 합니다. 관리 콘솔을 사용하거나 Trust Manager API를 사용하여 프로그래밍 방식으로 인증서를 추가합니다. (Trust Manager API[를 사용하여 자격 증명 가져오기를 참조하십시오.)](/help/forms/developing/credentials.md#importing-credentials-by-using-the-trust-manager-api)
...
1. 인증서를 참조합니다.
   ...
   * `Recipient` 개체의 `setX509Cert` 메서드를 호출하고 인증서가 포함된 `com.adobe.idp.Document` 개체를 전달합니다. 또한 `Recipient`개체는 Truststore 인증서 별칭 또는 LDAP URL을 인증서 소스로 사용할 수 있습니다.
   ...
   * `java.util.List` 개체의 add 메서드를 호출하고 `CertificateEncryptionIdentity` 개체를 전달합니다. 이 `java.util.List` 개체는 매개 변수로 `encryptPDFUsingCertificates` 메서드에 전달됩니다.
...
1. 인증서로 암호화된 PDF 문서를 만듭니다.
   ...
   * 암호화할 PDF 문서를 포함하는 `com.adobe.idp.Document` 개체
...
### 웹 서비스 API {#encrypt-a-pdf-document-with-a-certificate-using-the-web-service-api}를 사용하여 인증서로 PDF 문서 암호화
...
1. 암호화된 PDF 문서를 PDF 파일로 저장할 수 있습니다.
   ...
   * 생성자를 호출하고 보안 PDF 문서의 파일 위치를 나타내는 문자열 값을 전달하여 `System.IO.FileStream` 개체를 만듭니다.
   * `encryptPDFUsingCertificates` 메서드에서 반환된 `BLOB` 개체의 데이터 내용을 저장하는 바이트 배열을 만듭니다. `BLOB` 개체의 `binaryData` 데이터 멤버의 값을 가져와 바이트 배열을 채웁니다.
   * 생성자를 호출하고 `System.IO.FileStream` 개체를 전달하여 `System.IO.BinaryWriter` 개체를 만듭니다.
   * `System.IO.BinaryWriter` 개체의 `Write` 메서드를 호출하고 바이트 배열을 전달하여 바이트 배열의 내용을 PDF 파일에 기록합니다.
...
암호화된 PDF 문서에서 인증서 기반 암호화를 제거하려면 암호화된 PDF 문서와 PDF 문서를 암호화하는 데 사용한 키에 해당하는 개인 키가 모두 필요합니다. 암호화된 PDF 문서에서 인증서 기반 암호화를 제거할 때 개인 키의 별칭 값이 지정됩니다. 공개 키에 대한 자세한 내용은 [인증서를 사용하여 PDF 문서 암호화](encrypting-decrypting-pdf-documents.md#encrypting-pdf-documents-with-certificates)를 참조하십시오.
...
1. 암호화 제거
   ...
   * 암호화된 PDF 문서를 포함하는 `com.adobe.idp.Document` 개체
...
1. 암호화 제거
   ...
   * 암호화된 PDF 문서를 나타내는 파일 스트림 데이터를 포함하는 `BLOB` 개체
...
1. PDF 문서를 저장합니다.
   ...
   * `removePDFPasswordSecurity` 메서드에서 반환된 `BLOB` 개체의 내용을 저장하는 바이트 배열을 만듭니다. `BLOB` 개체의 `MTOM` 데이터 멤버의 값을 가져와 바이트 배열을 채웁니다.
   * 생성자를 호출하고 `System.IO.FileStream` 개체를 전달하여 `System.IO.BinaryWriter` 개체를 만듭니다.
   * `System.IO.BinaryWriter` 개체의 `Write` 메서드를 호출하고 바이트 배열을 전달하여 바이트 배열의 내용을 PDF 파일에 기록합니다.
...
암호화된 PDF 문서에서 암호 기반 암호화를 제거하려면 암호화된 PDF 문서와 PDF 문서에서 암호화를 제거하는 데 사용되는 마스터 암호 값이 모두 필요합니다. 암호로 암호화된 PDF 문서를 여는 데 사용되는 암호를 사용하여 암호화를 제거할 수 없습니다. PDF 문서가 암호로 암호화될 때 마스터 암호가 지정됩니다. ([암호](encrypting-decrypting-pdf-documents.md#encrypting-pdf-documents-with-a-password)를 사용하여 PDF 문서 암호화 참조)
...
암호로 암호화된 PDF 문서의 잠금을 해제하려면 암호화된 PDF 문서와 암호로 암호화된 PDF 문서를 여는 데 사용되는 암호 값이 모두 필요합니다. 이 값은 암호로 PDF 문서를 암호화할 때 지정됩니다. ([암호](encrypting-decrypting-pdf-documents.md#encrypting-pdf-documents-with-a-password)를 사용하여 PDF 문서 암호화 참조)
...
1. 문서의 잠금을 해제합니다.
   ...
   * PDF 문서를 암호화하는 데 사용되는 개인 키에 해당하는 공개 키의 별칭 이름을 지정하는 문자열 값.
   ...
   `unlockPDFUsingPassword` 및 `unlockPDFUsingCredential` 메서드 모두 작업을 수행하기 위해 다른 AEM Forms Java 메서드에 전달하는 `com.adobe.idp.Document` 개체를 반환합니다.

1. AEM Forms 작업을 수행합니다.

   잠금 해제된 PDF 문서에서 AEM Forms 작업을 수행하여 비즈니스 요구 사항을 충족할 수 있습니다. 예를 들어 잠금 해제된 PDF 문서에 사용 권한을 적용하려는 경우 `unlockPDFUsingPassword` 또는 `unlockPDFUsingCredential` 메서드에서 반환한 `com.adobe.idp.Document` 개체를 `ReaderExtensionsServiceClient` 개체의 `applyUsageRights` 메서드에 전달하십시오.
...
1. 문서의 잠금을 해제합니다.
   ...
   * PDFf 문서를 암호화하는 데 사용되는 개인 키에 해당하는 공개 키의 별칭 이름을 지정하는 문자열 값.
   ...
   `unlockPDFUsingPassword` 및 `unlockPDFUsingCredential` 메서드 모두 작업을 수행하기 위해 다른 AEM Forms 메서드에 전달하는 `com.adobe.idp.Document` 개체를 반환합니다.

1. AEM Forms 작업을 수행합니다.

   잠금 해제된 PDF 문서에서 AEM Forms 작업을 수행하여 비즈니스 요구 사항을 충족할 수 있습니다. 예를 들어, 잠금 해제된 PDF 문서에 사용 권한을 적용하려면 `unlockPDFUsingPassword` 또는 `unlockPDFUsingCredential` 메서드에서 반환한 `BLOB` 개체를 `ReaderExtensionsServiceClient` 개체의 `applyUsageRights` 메서드에 전달하십시오.
...
1. 암호화 유형을 결정합니다.
   ...
   * `EncryptionServiceClient` 개체의 `getPDFEncryption` 메서드를 호출하고 PDF 문서를 포함하는 `com.adobe.idp.Document` 개체를 전달하여 암호화 유형을 결정합니다. 이 메서드는 `EncryptionTypeResult` 개체를 반환합니다.
   * `EncryptionTypeResult` 개체의 `getEncryptionType` 메서드를 호출합니다. 이 메서드는 암호화 유형을 지정하는 `EncryptionType` 열거형 값을 반환합니다. 예를 들어 PDF 문서가 암호 기반 암호화로 보호되는 경우 이 메서드는 `EncryptionType.PASSWORD`을 반환합니다.
...
1. 암호화 유형을 결정합니다.
   ...
   * `EncryptionServiceClient` 개체의 `getPDFEncryption` 메서드를 호출하고 PDF 문서를 포함하는 `BLOB` 개체를 전달합니다. 이 메서드는 `EncryptionTypeResult` 개체를 반환합니다.
   * `EncryptionTypeResult` 개체의 `encryptionType` 데이터 메서드 값을 가져옵니다. 예를 들어 PDF 문서가 암호 기반 암호화를 통해 보호되는 경우 이 데이터 멤버의 값은 `EncryptionType.PASSWORD`입니다.
