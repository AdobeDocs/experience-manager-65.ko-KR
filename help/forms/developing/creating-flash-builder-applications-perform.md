...
HTTP 토큰을 사용하여 SSO(Single Sign On) 인증을 수행하는 Flash Builder를 사용하여 클라이언트 애플리케이션을 만들 수 있습니다. 예를 들어 Flash Builder를 사용하여 웹 기반 응용 프로그램을 만든다고 가정합니다. 응용 프로그램에 서로 다른 보기가 있으며 각 보기가 서로 다른 AEM Forms 작업을 불러온다고 가정합니다. 각 양식 작업에 대해 사용자를 인증하는 대신 사용자가 한 번 인증할 수 있는 로그인 페이지를 만들 수 있습니다. 인증된 사용자는 다시 인증하지 않고도 여러 작업을 호출할 수 있습니다. 예를 들어 사용자가 작업 공간(또는 다른 Forms 애플리케이션)에 로그인한 경우에는 사용자가 다시 인증할 필요가 없습니다.

클라이언트 응용 프로그램에 SSO 인증을 수행하는 데 필요한 응용 프로그램 로직이 포함되어 있지만 AEM Forms 사용자 관리는 실제 사용자 인증을 수행합니다. HTTP 토큰을 사용하여 사용자를 인증하기 위해 클라이언트 응용 프로그램은 Authentication Manager 서비스의 작업을 `authenticateWithHTTPToken` 호출합니다. 사용자 관리는 HTTP 토큰을 사용하여 사용자를 인증할 수 있습니다. 이후 AEM Forms에 대한 원격 또는 웹 서비스 호출의 경우 인증을 위해 자격 증명을 전달할 필요가 없습니다.

>[!NOTE]
>
>이 섹션을 읽기 전에 Remoting을 사용하여 AEM Forms을 호출하는 것이 좋습니다. AEM Forms [Remoting을 사용하여 AEM Forms 호출을 참조하십시오](/help/forms/developing/invoking-aem-forms-using-remoting.md#invoking-aem-forms-using-remoting).

SSO를 사용하여 사용자를 인증한 후 이름이 지정된 다음 AEM Forms `MyApplication/EncryptDocument`이 호출됩니다. 입력 및 출력 값과 같은 이 프로세스에 대한 자세한 내용은 [짧은 기간 프로세스 예를 참조하십시오](/help/forms/developing/aem-forms-processes.md).
...
Flash Builder를 사용하여 구축한 클라이언트 애플리케이션은 및 에서 구성된 사용자 관리자 `/um/login` 의 보안 서블릿과 상호 작용합니다 `/um/logout`. 즉, 클라이언트 응용 프로그램은 시작 중에 사용자의 상태를 확인하기 위해 요청을 `/um/login` URL로 보냅니다. 그러면 사용자 관리자가 사용자 상태에 응답합니다. 클라이언트 응용 프로그램 및 User Manager 보안 서블릿은 HTTP를 사용하여 통신합니다.
...
>[!NOTE]
>
>i18n을 제대로 처리하려면 이 값이 POST 양식에 있는지 확인합니다.
...
>[!NOTE]
>
>그러나 클라이언트 `authstate=CREDENTIAL_CHALLENGE`는 가져온 자격 증명을 POST 양식의 보안 서블릿으로 보내는 것이 좋습니다.
...
1. 보안 서블릿이 사용자를 인증하면 `ISSOManager` 전달됩니다 `SSOEvent.AUTHENTICATION_SUCCESS`. 응답으로 클라이언트 응용 프로그램은 기본 페이지를 표시합니다. 이 예에서 기본 페이지는 MyApplication/EncryptDocument라는 짧은 기간 AEM Forms 프로세스를 호출합니다.
...
* `SSOStandalone.mxml`: 클라이언트 응용 프로그램을 나타내는 기본 MXML 파일입니다. (SOStandalone.mxml 파일 [만들기를 참조하십시오](creating-flash-builder-applications-perform.md#creating-the-ssostandalone-mxml-file).)
...
* `views/remoting.mxml`: Remoting을 사용하여 MyApplication/EncryptDocument라는 짧은 기간 AEM Forms 프로세스를 호출하는 보기를 나타냅니다. remoting.mxml 파일 [만들기를 참조하십시오](creating-flash-builder-applications-perform.md#creating-the-remoting-mxml-file).
...
이 경우 사용자는 클라이언트 응용 프로그램에서 AEM Forms으로 처음으로 로그인을 시도합니다. (사용자와 관련된 이전 세션이 없습니다.) 이 `applicationComplete` 이벤트에서 사용자 `SSOManager.singleSignOn` 관리자에게 요청을 보내는 메서드가 호출됩니다.
