...
1. 바코드 양식 클라이언트 API 개체 만들기
   ...
   생성자를 사용하여 `BarcodedFormsServiceClient` 개체를 만들고 연결 속성이 포함된 `ServiceClientFactory` 개체를 전달합니다.

1. 바코드 데이터가 포함된 PDF 양식 가져오기

   * 생성자를 사용하고 PDF 문서의 위치를 지정하는 문자열 값을 전달하여 바코드된 데이터가 포함된 PDF 양식을 나타내는 `java.io.FileInputStream` 개체를 만듭니다.
...
1. PDF 양식의 데이터 디코딩
   ...
   * 데이터 매트릭스 바코드를 디코딩할지 여부를 지정하는 `java.lang.Boolean` 개체
   * QR 코드 바코드를 디코딩할지 여부를 지정하는 `java.lang.Boolean` 개체
   * 코드 바코드를 디코딩할지 여부를 지정하는 `java.lang.Boolean` 개체
   * 코드 128 바코드를 디코딩할지 여부를 지정하는 `java.lang.Boolean` 개체
   * 코드 39 바코드를 디코딩할지 여부를 지정하는 `java.lang.Boolean` 개체
   * EAN-13 바코드를 디코딩할지 여부를 지정하는 `java.lang.Boolean` 개체
   * EAN-8 바코드를 디코딩할지 여부를 지정하는 `java.lang.Boolean` 개체
   * 바코드에 사용되는 문자 집합 인코딩 값을 지정하는 `com.adobe.livecycle.barcodedforms.CharSet` 열거형 값.

   `decode` 메서드는 디코딩된 양식 데이터를 포함하는 `org.w3c.dom.Document` 개체를 반환합니다.

1. 데이터를 XML 데이터 소스로 변환

   `BarcodedFormsServiceClient` 개체의 `extractToXML` 메서드를 호출하고 다음 값을 전달하여 디코딩된 데이터를 XDP 또는 XFDF 데이터로 변환합니다.

   * 디코딩된 데이터를 포함하는 `org.w3c.dom.Document` 개체입니다. `decode` 메서드의 반환 값을 사용해야 합니다.
   * 줄 구분 기호를 지정하는 `com.adobe.livecycle.barcodedforms.Delimiter` 열거형 값. `Delimiter.Carriage_Return`을 지정하는 것이 좋습니다.
   * 필드 구분 기호를 지정하는 `com.adobe.livecycle.barcodedforms.Delimiter` 열거형 값. 예를 들어 `Delimiter.Tab`을 지정합니다.
   * 바코드 데이터를 XDP 또는 XFDF XML 데이터로 변환할지 여부를 지정하는 `com.adobe.livecycle.barcodedforms.XMLFormat` 열거형 값입니다. 예를 들어 데이터를 XDP 데이터로 변환하려면 `XMLFormat.XDP`을 지정합니다.
   ...
   `extractToXML` 메서드는 각 요소가 `org.w3c.dom.Document` 개체인 `java.util.List` 개체를 반환합니다. 양식에 있는 각 바코드에 별도의 요소가 있습니다. 즉, 양식에 4개의 바코드가 있는 경우 반환된 `java.util.List` 개체에 4개의 요소가 있습니다.

1. 디코딩된 데이터 처리

   * 목록에 있는 각 `org.w3c.dom.Document` 개체를 가져오려면 `java.util.List` 개체를 반복하십시오.
...
1. 프로젝트 파일 포함
   ...
   * Microsoft .NET 클라이언트 어셈블리를 참조하십시오. 자세한 내용은 [Base64 인코딩](/help/forms/developing/invoking-aem-forms-using-web.md#invoking-aem-forms-using-base64-encoding)을 사용하여 AEM Forms 호출에서 &quot;.NET 클라이언트 어셈블리 참조&quot;를 참조하십시오.
...
1. PDF 양식의 데이터 디코딩
   ...
   * 데이터 매트릭스 바코드를 디코딩할지 여부를 지정하는 `Boolean` 개체
   * QR 코드 바코드를 디코딩할지 여부를 지정하는 `Boolean` 개체
   * 코드 바코드를 디코딩할지 여부를 지정하는 `Boolean` 개체
   * 코드 128 바코드를 디코딩할지 여부를 지정하는 `Boolean` 개체
   * 코드 39 바코드를 디코딩할지 여부를 지정하는 `Bolean` 개체
   * EAN-13 바코드를 디코딩할지 여부를 지정하는 `Boolean` 개체
   * EAN-8 바코드를 디코딩할지 여부를 지정하는 `Boolean` 개체
   * 바코드에 사용되는 문자 집합 인코딩 값을 지정하는 `CharSet` 열거형 값.
...
1. 데이터를 XML 데이터 소스로 변환
   ...
   * 줄 구분 기호를 지정하는 `Delimiter` 열거형 값. `Delimiter.Carriage_Return`을 지정하는 것이 좋습니다.
   * 필드 구분 기호를 지정하는 `Delimiter` 열거형 값. 예를 들어 `Delimiter.Tab`을 지정합니다.
   * 바코드 데이터를 XDP 또는 XFDF XML 데이터로 변환할지 여부를 지정하는 `XMLFormat` 열거형 값입니다. 예를 들어 데이터를 XDP 데이터로 변환하려면 `XMLFormat.XDP`을 지정합니다.
   ...
   `extractToXML` 메서드는 각 요소가 `BLOB` 인스턴스인 `Object` 배열을 반환합니다. 양식에 있는 각 바코드에 별도의 요소가 있습니다. 즉, 양식에 4개의 바코드가 있는 경우 반환된 `Object` 배열에 4개의 요소가 있습니다.

1. 디코딩된 데이터 처리

   * 생성자를 호출하고 보안 PDF 문서의 파일 위치를 나타내는 문자열 값을 전달하여 `System.IO.FileStream` 개체를 만듭니다.
   * `encryptPDFUsingPassword` 메서드에서 반환된 `BLOB` 개체의 데이터 내용을 저장하는 바이트 배열을 만듭니다. `BLOB` 개체의 `binaryData` 데이터 멤버의 값을 가져와 바이트 배열을 채웁니다.
