...
클라이언트에서 양식을 렌더링하려면 클라이언트 렌더링 런타임 옵션을 설정해야 합니다. `RenderAtClient` 런타임 옵션 대상 `true`. 이렇게 하면 양식이 렌더링되는 클라이언트 장치로 전달됩니다. If `RenderAtClient` 은(는) `auto` (기본값) 양식 디자인은 양식이 클라이언트에서 렌더링되는지 여부를 결정합니다. 양식 디자인은 흐름 가능한 레이어가 있는 양식 디자인이어야 합니다.

설정할 수 있는 선택적 런타임 옵션은 `SeedPDF` 옵션을 선택합니다. 다음 `SeedPDF` 옵션은 PDF 컨테이너(시드 PDF 문서)를 양식 디자인 및 XML 데이터와 결합합니다. 양식 디자인과 XML 데이터는 모두 Acrobat 또는 Adobe Reader으로 전달되며, 여기서 양식이 렌더링됩니다. 다음 `SeedPDF` 최종 사용자에게 양식 소유자에게 사용 허가된 글꼴을 사용할 수 있는 권한이 없는 경우와 같이 클라이언트 컴퓨터에 양식에 사용되는 글꼴이 없는 경우 옵션을 사용할 수 있습니다.

디자이너를 사용하여 시드 PDF 파일로 사용할 간단한 동적 PDF 파일을 만들 수 있습니다. 이 작업을 수행하려면 다음 단계가 필요합니다.

1. 시드 PDF 파일 내에 글꼴을 임베드해야 할지 여부를 결정합니다. 시드 PDF 파일에는 렌더링 중인 양식에 필요한 추가 글꼴이 포함되어야 합니다. 시드 PDF 파일에 글꼴을 포함할 때 글꼴 라이센스 계약을 위반하지 않아야 합니다. 디자이너에서 글꼴을 합법적으로 포함할 수 있는지 여부를 결정할 수 있습니다. 저장할 때 양식에 포함할 수 없는 글꼴이 있으면 포함할 수 없는 글꼴이 나열된 메시지가 표시됩니다. 이 메시지는 정적 PDF 문서의 디자이너에 표시되지 않습니다.
1. 디자이너에서 시드 PDF 파일을 만드는 경우 최소한 메시지가 포함된 텍스트 필드를 추가하는 것이 좋습니다. 이 메시지는 이전 버전의 Adobe Reader 사용자에게 문서를 보려면 Acrobat 7.0 이상 또는 Adobe Reader 7.0 이상이 필요하다는 것을 표시해야 합니다.
...
1. 클라이언트 렌더링 런타임 옵션 설정
   ...
   * 설정 `RenderAtClient` 런타임 옵션을 사용하려면 `PDFFormRenderSpec` 개체 `setRenderAtClient` 열거형 값 전달 방법 및 `RenderAtClient.Yes`.

1. 클라이언트에서 양식 렌더링

   호출 `FormsServiceClient` 개체 `renderPDFForm` 메서드를 실행하고 다음 값을 전달합니다.

   * 파일 이름 확장명을 포함하여 양식 디자인 이름을 지정하는 문자열 값입니다. AEM Forms 애플리케이션의 일부인 양식 디자인을 참조하는 경우 다음과 같은 전체 경로를 지정해야 합니다. `Applications/FormsApplication/1.0/FormsFolder/Loan.xdp`.
   ...
   * A `URLSpec` Forms 서비스에서 양식을 렌더링하는 데 필요한 URI 값을 포함하는 개체입니다.
...
1. 클라이언트 렌더링 런타임 옵션 설정
   ...
   * 만들기 `PDFFormRenderSpec` 개체를 만들 때 사용됩니다.
   * 설정 `RenderAtClient` 런타임 옵션을 사용하려면 `PDFFormRenderSpec` 개체 `setRenderAtClient` 메서드 및 문자열 값 전달 `RenderAtClient.Yes`.
...
1. 클라이언트에서 양식 렌더링
   ...
   * A `java.util.HashMap` 첨부 파일을 저장하는 객체입니다. 이는 선택적 매개 변수이며 다음을 지정할 수 있습니다 `null` 양식에 파일을 첨부하지 않으려면.
   * 비어 있음 `com.adobe.idp.services.holders.BLOBHolder` 메서드에서 채운 개체입니다. 이 매개 변수는 렌더링된 PDF 양식을 저장하는 데 사용됩니다.
