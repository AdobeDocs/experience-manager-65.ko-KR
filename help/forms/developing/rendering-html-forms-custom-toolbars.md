---
title: CustomToolbar를 사용하여 HTML 양식 렌더링
seo-title: CustomToolbar를 사용하여 HTML 양식 렌더링
description: 'null'
...
# CustomToolbar를 사용하여 HTML 양식 렌더링 {#rendering-html-forms-with-customtoolbars}

## 사용자 지정 도구 모음을 사용하여 HTML 양식 렌더링 {#rendering-html-forms-with-custom-toolbars}

양식 서비스를 사용하면 HTML 양식으로 렌더링되는 도구 모음을 사용자 정의할 수 있습니다. 도구 모음은 기본 CSS 스타일을 재정의하여 모양을 변경하고 Java 스크립트를 재정의하여 동적 동작을 추가할 수 있습니다. 도구 모음은 fscmenu.xml이라는 XML 파일을 사용하여 사용자 정의합니다. 기본적으로 양식 서비스는 내부적으로 지정된 URI 위치에서 이 파일을 검색합니다.
...
이 위치에서 fscmenu.xml을 복사하여 요구 사항에 맞게 수정한 다음 사용자 정의 URI 위치에 배치할 수 있습니다. 그런 다음 Forms Service API를 사용하여 지정된 위치에서 fscmenu.xml 파일을 사용하여 Forms 서비스에 표시되는 런타임 옵션을 설정합니다. 이러한 작업을 수행하면 Forms 서비스가 사용자 정의 도구 모음이 있는 HTML 양식을 렌더링합니다.
...
도구 모음을 사용자 정의하려면 외부 키워드를 사용하여 키워드를 바꿉니다 `FSToolBarURI`. 이 키워드는 런타임에 Forms 서비스로 전달된 URI를 나타냅니다(이 방법은 이 섹션 후반부에 표시됨).
...
또한 객체의 메서드를 호출하고 로케일 값을 지정하는 문자열 값을 전달하여 유효한 로케일 값 `HTMLRenderSpec` `setLocale` 을 지정합니다. 예를 들어, 전달을 `fr_FR` 사용하여 프랑스어를 지정합니다. 양식 서비스는 지역화된 도구 모음과 번들로 제공됩니다.

>[!NOTE]
>
>사용자 정의 도구 모음을 사용하는 HTML 양식을 렌더링하기 전에 HTML 양식이 렌더링되는 방법을 알고 있어야 합니다. 양식 [렌더링을 HTML로 참조하십시오](/help/forms/developing/rendering-forms-html.md).
...
Forms Service API(Java)를 사용하여 사용자 정의 도구 모음이 포함된 HTML Form을 렌더링합니다.
...
1. HTML 양식 렌더링
   ...
   * 메서드로 채워지는 빈 `com.adobe.idp.services.holders.BLOBHolder` `renderHTMLForm` 개체 이 매개 변수 값은 렌더링된 양식을 저장합니다.
