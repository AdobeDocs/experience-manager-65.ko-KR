...
<table>
 ...
 <tbody>
  ...
  <tr>
   <td><p>3</p></td>
   <td><p><code>HandleData</code> Java 서블릿은 데이터를 검색할 응용 프로그램 논리를 포함합니다.</p></td>
  </tr>
 </tbody>
</table>
...
>[!NOTE]
>
>데이터를 XML 데이터로 전송하려면 양식 디자인을 Designer에서 올바르게 구성해야 합니다. 양식 디자인이 XML 데이터를 전송하도록 제대로 구성하려면 양식 디자인에 있는 전송 단추가 XML 데이터를 전송하도록 설정되어 있는지 확인하십시오. 제출 단추를 설정하여 XML 데이터를 제출하는 방법에 대한 자세한 내용은 [AEM Forms 디자이너](https://www.adobe.com/go/learn_aemforms_designer_63)를 참조하십시오.
...
양식 데이터가 URL UTF-16 데이터로 제출되면 클라이언트 컴퓨터에 Adobe Reader 또는 Acrobat 8.1 이상이 필요합니다. 또한 양식 디자인에 URL 인코딩 데이터(HTTP 게시물)가 있는 전송 단추가 있고 데이터 인코딩 옵션이 UTF-16인 경우 메모장과 같은 텍스트 편집기에서 양식 디자인을 수정해야 합니다. 전송 단추에 대해 인코딩 옵션을 `UTF-16LE` 또는 `UTF-16BE` 중 하나로 설정할 수 있습니다. 디자이너는 이 기능을 제공하지 않습니다.
...
* 다음 값을 콘텐트 유형 매개 변수로 `processFormSubmission` 메서드에 전달합니다.`CONTENT_TYPE=application/pdf`.
* `RenderOptionsSpec` 개체의 `PDFToXDP` 값을 `true`로 설정합니다.
* `RenderOptionsSpec` 개체의 `ExportDataFormat` 값을 `XMLData`로 설정합니다.

`processFormSubmission` 메서드를 호출할 때 제출된 양식의 콘텐츠 유형을 지정합니다. 다음 목록은 적용 가능한 컨텐츠 유형 값을 지정합니다.
...
>[!NOTE]
>
>제출된 Forms 처리 섹션과 관련된 세 개의 해당 빠른 시작이 있습니다. Java API 빠른 시작을 사용하여 PDF로 제출된 처리 PDF forms은 제출된 PDF 데이터를 처리하는 방법을 보여 줍니다. 이 빠른 시작 부분에 지정된 콘텐츠 유형은 `application/pdf`입니다. Java API 빠른 시작을 사용하여 XML로 제출된 처리 PDF forms은 PDF 양식에서 제출한 제출된 XML 데이터를 처리하는 방법을 보여 줍니다. 이 빠른 시작 부분에 지정된 콘텐츠 유형은 `text/xml`입니다. 마찬가지로, Java API 빠른 시작을 사용하여 XML로 제출된 HTML 양식 처리는 HTML 양식에서 전송된 XML 데이터를 처리하는 방법을 보여줍니다. 이 빠른 시작 부분에 지정된 컨텐츠 유형은 application/x-www-form-urlencoded입니다.
...
* **2(유효성 확인):** Forms 서비스가 양식 데이터의 유효성을 검사하여 결과를 사용자에게 다시 렌더링해야 합니다.
* **3(다음):** 현재 페이지가 클라이언트 응용 프로그램에 작성해야 하는 결과로 변경되었습니다.
* **4(이전**):현재 페이지가 클라이언트 응용 프로그램에 작성해야 하는 결과로 변경되었습니다.

>[!NOTE]
>
>계산 및 유효성 검사는 사용자에게 다시 렌더링해야 합니다. ([양식 데이터 계산](/help/forms/developing/calculating-form-data.md#calculating-form-data)을 참조하십시오.)
...
제출된 데이터의 컨텐츠 유형에 따라 제출된 XML 데이터에서 개별 양식 필드 값을 추출하거나 제출된 PDF 데이터를 PDF 파일로 저장(또는 다른 서비스로 전송)할 수 있습니다. 개별 양식 필드를 추출하려면 제출된 XML 데이터를 XML 데이터 소스로 변환한 다음 `org.w3c.dom` 클래스를 사용하여 XML 데이터 소스 값을 검색합니다.
...
1. 양식 데이터 검색
   ...
   * 생성자를 사용하여 `RenderOptionsSpec` 개체를 만듭니다. `RenderOptionsSpec` 개체의 `setLocale` 메서드를 호출하고 로케일 값을 지정하는 문자열 값을 전달하여 로케일 값을 설정합니다.
   ...
   >[!NOTE]
   >
   >`RenderOptionsSpec` 개체의 `setPDF2XDP` 메서드를 호출하고 `true`를 전달하고 `setXMLData`을(를) 호출하고 `true`을(를) 전달하여 제출된 PDF 내용에서 XDP 또는 XML 데이터를 만들도록 Forms 서비스에 지시할 수 있습니다. 그런 다음 `FormsResult` 개체의 `getOutputXML` 메서드를 호출하여 XDP/XML 데이터에 해당하는 XML 데이터를 검색할 수 있습니다. `FormsResult` 개체는 다음 하위 단계에서 설명하는 `processFormSubmission` 메서드에서 반환됩니다.
   ...
   * `FormsServiceClient` 개체의 `processFormSubmission` 메서드를 호출하고 다음 값을 전달합니다.
      ...
      * 런타임 옵션을 저장하는 `RenderOptionsSpec` 개체입니다.
      ...
      `processFormSubmission` 메서드는 양식 제출 결과를 포함하는 `FormsResult` 개체를 반환합니다.

   * Forms 서비스가 `FormsResult` 개체의 `getAction` 메서드를 호출하여 양식 데이터 처리를 완료했는지 확인합니다. 이 메서드가 `0` 값을 반환하면 데이터를 처리할 준비가 됩니다.
...
1. 제출된 데이터 처리
   ...
   * 데이터 내용 유형이 `application/vnd.adobe.xdp+xml` 또는 `text/xml`이면 응용 프로그램 논리를 만들어 XML 데이터 값을 검색합니다.
      ...
      * XML 문서 내에서 각 노드의 값을 검색합니다. 이 작업을 수행하는 한 가지 방법은 두 개의 매개 변수를 허용하는 사용자 지정 메서드를 만드는 것입니다.`org.w3c.dom.Document` 개체 및 값을 검색할 노드의 이름입니다. 이 메서드는 노드의 값을 나타내는 문자열 값을 반환합니다. 이 프로세스를 따르는 코드 예제에서 이 사용자 지정 메서드는 `getNodeText`이라고 합니다. 이 메서드의 본문이 표시됩니다.
...
1. 양식 데이터 검색
   ...
   * 생성자를 사용하여 `java.io.ByteArrayOutputStream` 개체를 만들고 `java.io.InputStream` 개체의 길이를 전달합니다.
   ...
   * `FormsService` 개체의 `processFormSubmission` 메서드를 호출하고 다음 값을 전달합니다.
      ...
      * 메서드로 채워진 빈 `MyArrayOf_xsd_anyTypeHolder` 개체입니다. 이 매개 변수는 양식과 함께 제출된 첨부 파일을 저장하는 데 사용됩니다.
      * 제출된 폼으로 메서드에 의해 채워진 빈 `FormsResultHolder` 개체입니다.

      `processFormSubmission` 메서드는 `FormsResultHolder` 매개 변수를 양식 제출 결과로 채웁니다.

   * Forms 서비스가 `FormsResult` 개체의 `getAction` 메서드를 호출하여 양식 데이터 처리를 완료했는지 확인합니다. 이 메서드가 `0` 값을 반환하면 양식 데이터를 처리할 준비가 됩니다. `FormsResultHolder` 개체의 `value` 데이터 멤버의 값을 가져와 `FormsResult` 개체를 가져올 수 있습니다.


1. 양식 제출 시 첨부 파일이 포함되어 있는지 확인

   `MyArrayOf_xsd_anyTypeHolder` 개체의 `value` 데이터 멤버의 값을 가져옵니다(`MyArrayOf_xsd_anyTypeHolder` 개체가 `processFormSubmission` 메서드에 전달됨). 이 데이터 멤버는 `Objects` 배열을 반환합니다. `Object` 배열 내의 각 요소는 양식과 함께 제출된 파일에 해당하는 `Object`입니다. 배열 내의 각 요소를 가져와 `BLOB` 개체에 캐스팅할 수 있습니다.
