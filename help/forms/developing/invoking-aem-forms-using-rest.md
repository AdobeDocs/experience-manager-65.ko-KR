...
REST에 보안 전송을 제공하려면 AEM 양식 관리자가 AEM Forms을 호스팅하는 J2EE 응용 프로그램 서버에서 HTTPS 프로토콜을 활성화할 수 있습니다. 이 구성은 J2EE 응용 프로그램 서버에만 적용됩니다. 양식 서버 구성의 일부가 아닙니다.
...
다음 HTML 예는 날짜 값을 명명된 AEM Forms 프로세스로 전달합니다 `SOAPEchoService`. 호출 메서드의 이름이 입니다 `echoCalendar`. HTML `Post` 메서드가 사용된다는 점을 참고하십시오.
...
REST를 사용하여 *MyApplication/EncryptDocument라는 짧은 AEM Forms* 프로세스를 호출할 수 있습니다.
