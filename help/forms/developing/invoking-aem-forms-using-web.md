...
1. BAT 파일을 만들어 Ant 빌드 스크립트를 실행합니다. 다음 명령은 Ant 빌드 스크립트 실행을 담당하는 BAT 파일 내에서 찾을 수 있습니다.
   ...
   C:\Program Files\Java\jaxws-ri\bin 디렉터리에 ANT 빌드 스크립트를 배치합니다. 스크립트는 JAVA 파일을 ./classes 폴더에 기록합니다. 스크립트는 서비스를 호출할 수 있는 JAVA 파일을 생성합니다.
...
1. `EncryptDocumentServiceWse` 개체의 `Credentials` 데이터 구성원을 AEM Forms 사용자 이름 및 암호 값을 지정하는 `System.Net.NetworkCredential` 값으로 설정하십시오.
...
Apache Axis WSDL2Java 도구는 SOAP 요청을 서비스로 전송하는 데 사용되는 메서드가 포함된 JAVA 파일을 생성합니다. 서비스에서 받은 SOAP 요청은 축에서 생성한 라이브러리에서 디코딩되고 메서드 및 인수로 다시 변환됩니다.
...
1. `EncryptDocumentSoapBindingStub` 개체의 `setUsername` 및 `setPassword` 메서드를 호출하여 AEM Forms 사용자 이름 및 암호 값을 설정하십시오.
...
1. 생성자를 사용하고 `javax.activation.FileDataSource` 개체를 전달하여 `javax.activation.DataHandler` 개체를 만듭니다. `javax.activation.FileDataSource` 개체는 생성자를 사용하여 만든 다음 PDF 문서를 나타내는 `java.io.File` 개체를 전달하면 만들 수 있습니다.
...
* `AuthenticationManager.getAuthResultOnBehalfOfUser` 메서드를 사용합니다. 이 메서드를 사용하면 클라이언트 응용 프로그램에서 모든 AEM 양식 사용자에 대한 `AuthResult` 개체를 가져올 수 있습니다.
