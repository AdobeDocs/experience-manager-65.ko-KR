...
* PDF 문서를 PDF/A 문서로 변환 ([문서를 PDF/A 문서로 변환](pdf-a-documents.md#converting-documents-to-pdf-a-documents)을 참조하십시오.)
...
>[!NOTE]
>
>DocConverter 서비스에 대한 자세한 내용은 [AEM Forms에 대한 서비스 참조](https://www.adobe.com/go/learn_aemforms_services_63)를 참조하십시오.
...
1. 추적 정보 설정
   ...
   * `PDFAConversionOptionSpec` 개체의 `setLogLevel` 메서드를 호출하고 추적 수준을 지정하는 문자열 값을 전달하여 정보 추적 수준을 설정합니다. 예를 들어 값 `FINE`을 전달합니다. 다른 값에 대한 자세한 내용은 [AEM Forms API 참조](https://www.adobe.com/go/learn_aemforms_javadocs_63_en)의 `setLogLevel` 메서드를 참조하십시오.
...
1. DocConvert 클라이언트 만들기
   ...
   * 사용자 이름과 암호 값을 지정하는 `System.Net.NetworkCredential` 값으로 `DocConverterServiceService` 개체의 `Credentials` 데이터 멤버를 설정합니다.
...
1. PDF/A 문서 저장
   ...
   * `PDFAConversionResult` 개체를 사용하여 반환된 `BLOB` 개체의 내용을 저장하는 바이트 배열을 만듭니다. `BLOB` 개체의 `binaryData` 데이터 멤버의 값을 가져와 바이트 배열을 채웁니다.
...
DocConverter 서비스를 사용하여 PDF 문서가 PDF/A 규격인지 확인할 수 있습니다. PDF/A 문서와 PDF 문서를 PDF/A 문서로 변환하는 방법에 대한 자세한 내용은 [문서를 PDF/A 문서로 변환](pdf-a-documents.md#converting-documents-to-pdf-a-documents)을 참조하십시오.
...
1. 런타임 옵션 설정
   ...
   * `PDFAValidationOptionSpec` 개체의 `setLogLevel` 메서드를 호출하고 추적 수준을 지정하는 문자열 값을 전달하여 정보 추적 수준을 설정합니다. 예를 들어 값 `FINE`을 전달합니다. 다른 값에 대한 자세한 내용은 [AEM Forms API 참조](https://www.adobe.com/go/learn_aemforms_javadocs_63_en)의 `setLogLevel` 메서드를 참조하십시오.

1. PDF 문서에 대한 정보 검색

   `DocConverterServiceClient` 개체의 `isPDFA` 메서드를 호출하고 다음 값을 전달하여 PDF/A 규정을 결정합니다.

   * PDF 문서를 포함하는 `com.adobe.idp.Document` 개체
   * 런타임 옵션을 지정하는 `PDFAValidationOptionSpec` 개체
...
### 웹 서비스 API {#determine-pdf-a-compliancy-using-the-web-service-api}를 사용하여 PDF/A 규정 준수 확인
...
1. 런타임 옵션 설정
   ...
   * 생성자를 사용하여 `PDFAValidationOptionSpec` 개체를 만듭니다.
   * `PDFAValidationOptionSpec` 개체의 `compliance` 데이터 멤버를 값 `PDFAConversionOptionSpec_Compliance.PDFA_1B`에 할당하여 준수 수준을 설정합니다.
   * `PDFAValidationOptionSpec` 개체의 `resultLevel` 데이터 멤버를 값 `PDFAValidationOptionSpec_ResultLevel.DETAILED`에 할당하여 정보 추적 수준을 설정합니다.

1. PDF 문서에 대한 정보 검색

   `DocConverterServiceService` 개체의 `isPDFA` 메서드를 호출하고 다음 값을 전달하여 PDF/A 규정을 결정합니다.

   * PDF 문서를 포함하는 `BLOB` 개체
   * 런타임 옵션이 포함된 `PDFAValidationOptionSpec` 개체
