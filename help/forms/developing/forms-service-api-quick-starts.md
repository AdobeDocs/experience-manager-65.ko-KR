...
>[!NOTE]
>
>v로 프로그래밍에 있는 빠른 시작은 Unix와 같은 다른 운영 체제를 사용하고 있는 양식 서버를 기반으로 하며, Windows 특정 경로를 해당 운영 체제에서 지원되는 경로로 바꿉니다. 마찬가지로 다른 J2EE 응용 프로그램 서버를 사용하는 경우 올바른 연결 속성을 지정해야 합니다. [연결 속성 설정](/help/forms/developing/invoking-aem-forms-using-java.md#setting-connection-properties)을 참조하십시오.
...
다음 코드 예제에서는 *Loan.xdp*&#x200B;라는 대화형 PDF 양식을 클라이언트 웹 브라우저에 렌더링합니다. 파일이 양식에 첨부됩니다. 양식 디자인이 응용 프로그램의 일부이며 컨텐츠 루트 URI 값 `repository:///`을(를) 사용하여 참조됩니다. ([대화형 PDF forms 렌더링](/help/forms/developing/rendering-forms.md#rendering-interactive-pdf-forms)을 참조하십시오.)
...
다음 코드 예는 Forms 서비스 Java API를 사용하여 클라이언트에서 *Loan.xdp*&#x200B;라는 양식을 렌더링합니다. 양식 디자인이 응용 프로그램의 일부이며 컨텐츠 루트 URI 값 `repository:///`을(를) 사용하여 참조됩니다. (클라이언트[에서 Forms 렌더링 참조)](/help/forms/developing/rendering-forms.md#rendering-forms-at-the-client)
...
다음 코드 예제는 *TLALifeClaim.xdp*&#x200B;라는 이름의 안내서를 클라이언트 웹 브라우저에 렌더링합니다.
...
다음 코드 예제에서는 조각을 기반으로 하는 양식을 렌더링합니다. 양식 디자인의 이름은 *PurchaseOrderDynamic.xdp*&#x200B;이며 AEM Forms 저장소에 있습니다. XDP 파일은 저장소의 FormsFolder라는 폴더에 저장됩니다. 또한 POFragment 양식 참조에서 참조되는 조각도 저장소에 있어야 합니다. ([조각에 따라 Forms 렌더링](/help/forms/developing/rendering-forms.md#rendering-forms-based-on-fragments)을 참조하십시오.)
...
다음 코드 예제에서는 권한이 활성화된 양식을 클라이언트 웹 브라우저에 렌더링합니다. 이 코드 예제의 사용 권한 세트를 사용하면 사용자가 양식에 주석을 추가하고 양식 데이터를 저장할 수 있습니다. ([렌더링 권한 사용 Forms](/help/forms/developing/rendering-forms.md#rendering-rights-enabled-forms)을 참조하십시오.)
...
다음 코드 예제에서는 Forms 서비스 Java API를 사용하여 HTML 양식을 렌더링합니다. 도구 모음이 HTML 양식과 두 개의 첨부 파일에 추가됩니다. 또한 사용자 에이전트 값은 `HttpServletRequest` 개체에서 가져옵니다. (HTML[으로 Forms 렌더링 참조)](/help/forms/developing/rendering-forms.md#rendering-forms-as-html)
...
## 빠른 시작(SOAP 모드):Java API {#quick-start-soap-mode-rendering-an-html-form-with-a-custom-toolbar-using-the-java-api}를 사용하여 사용자 정의 도구 모음을 사용하여 HTML 양식 렌더링

다음 코드 예제에서는 툴바가 프랑스어로 표시된 HTML 양식을 렌더링합니다. fscmenu.xml의 위치는 C:\Adobe (이 폴더는 AEM Forms을 호스팅하는 서버에 있어야 함)입니다. 로케일 값은 `fr_FR`입니다. 사용자 정의 도구 모음을 사용하여 HTML 양식을 렌더링하는 방법에 대해 설명하는 섹션은 이 빠른 시작 부분에 사용되는 fscmenu.xml 파일의 구문을 보여줍니다. (사용자 지정 도구 모음[을(를) 사용하여 HTML Forms 렌더링 참조)](/help/forms/developing/rendering-forms.md#rendering-html-forms-with-custom-toolbars)
...
다음 코드 예제에서는 XML로 전송된 양식을 처리합니다. `processFormSubmission` 메서드에 전달된 콘텐츠 형식 값은 `CONTENT_TYPE=text/xml`입니다. `mortgageAmount`, `lastName` 및 `firstName`이라는 필드에 해당하는 값이 표시됩니다. 이 빠른 시작 시 `getNodeText`이라는 사용자 정의 메서드가 사용됩니다. 노드 이름을 지정하는 문자열 값과 `org.w3c.dom.Document` 인스턴스를 수락합니다. 이 메서드는 노드의 값을 나타내는 문자열 값을 반환합니다. ([제출된 Forms 처리](/help/forms/developing/rendering-forms.md#handling-submitted-forms)를 참조하십시오.)
...
>[!NOTE]
>
>동일한 응용 프로그램에서 `com.adobe.idp.Document` 개체와 `org.w3c.dom.Document`을(를) 사용하는 경우 `org.w3c.dom.Document`의 자격을 모두 만족하십시오.
...
다음 코드 예제에서는 PDF 데이터로 제출된 양식을 처리합니다. `processFormSubmission` 메서드에 전달된 콘텐츠 형식 값은 `CONTENT_TYPE=application/pdf`입니다. 제출된 양식이 *tempPDF.pdf*&#x200B;라는 PDF 파일로 저장됩니다. 또한 양식이 PDF로 제출되므로 첨부 파일을 검색할 수 있습니다. 모든 파일 첨부 파일은 JPEG 파일로 저장됩니다. ([제출된 Forms 처리](/help/forms/developing/rendering-forms.md#handling-submitted-forms)를 참조하십시오.)
...
다음 코드 예제에서는 XML 데이터로 전송된 HTML 양식을 처리합니다. `processFormSubmission` 메서드에 전달된 컨텐츠 유형 값은 `CONTENT_TYPE=application/x-www-form-urlencoded`입니다. `mortgageAmount`, `lastName` 및 `firstName`이라는 필드에 해당하는 값이 표시됩니다. 이 빠른 시작 시 `getNodeText`이라는 사용자 정의 메서드가 사용됩니다. 노드 이름을 지정하는 문자열 값과 `org.w3c.dom.Document` 인스턴스를 수락합니다. 이 메서드는 노드의 값을 나타내는 문자열 값을 반환합니다. ([제출된 Forms 처리](/help/forms/developing/rendering-forms.md#handling-submitted-forms)를 참조하십시오.)
...
다음 Java 코드 예제에서는 XML로 전송된 양식 데이터를 처리합니다. 양식 데이터는 Forms API를 사용하여 양식 제출 시 검색되고 출력 서비스로 전송됩니다. 양식 데이터와 양식 디자인을 사용하여 비대화형 PDF 문서를 만듭니다. 비대화형 PDF 문서는 `/Company Home/Test Directory`이라는 컨텐츠 서비스 노드에 저장됩니다. 양식 이름이 동적으로 만들어집니다. 즉, 사용자의 이름과 성은 PDF 파일의 이름을 지정하는 데 사용됩니다. 새 컨텐츠의 리소스 식별자는 클라이언트 웹 브라우저에 작성됩니다. ([제출된 XML 데이터를 사용하여 PDF 문서 만들기](/help/forms/developing/rendering-forms.md#creating-pdf-documents-with-submitted-xml-data)를 참조하십시오.)
...
## 빠른 시작(SOAP 모드):Java API {#quick-start-soap-mode-prepopulating-forms-with-flowable-layouts-using-the-java-api}를 사용하여 Forms을 플로우 가능한 레이아웃으로 미리 채우기
...
* `renderPOForm`:Forms 서비스 Java API를 사용하여 동적 구매 발주 양식을 렌더링합니다. `convertDataSource` 메서드에서 반환된 `com.adobe.idp.Document` 개체는 양식을 채울 때 사용됩니다.

   이러한 모든 메서드는 Java 서블릿의 `doPost` 메서드 내에서 호출됩니다. ([Forms을 플로우 가능한 레이아웃으로 미리 채우기](/help/forms/developing/rendering-forms.md#prepopulating-forms-with-flowable-layouts)를 참조하십시오.)
...
다음 코드 예제에서는 캐싱, 독립형 및 선형화된 옵션을 설정하여 성능을 최적화합니다. 선형화된 파일은 웹상에서 전달하도록 최적화되어 있습니다. ([Forms 서비스의 성능 최적화](/help/forms/developing/rendering-forms.md#optimizing-the-performance-of-the-forms-service)를 참조하십시오.)
...
## 빠른 시작(SOAP 모드):Java API {#quick-start-soap-mode-rendering-by-value-using-the-java-api}를 사용하여 값별 렌더링

다음 Java 빠른 시작은 값별로 *Loan.xdp*&#x200B;라는 양식 디자인을 기반으로 하는 대화형 PDF 양식을 렌더링합니다. 양식 디자인은 *inputXDP*&#x200B;라는 이름의 `com.adobe.idp.Document` 개체를 채우는 데 사용됩니다. (값[으로 Forms 렌더링 참조)](/help/forms/developing/rendering-forms.md#rendering-forms-by-value)
...
다음 Java 빠른 시작은 콘텐츠 서비스에서 Loan.xdp 파일을 검색합니다(더 이상 사용되지 않음). 이 XDP 파일은 `/Company Home/Form Designs` 공간에 있습니다. XDP 파일이 `com.adobe.idp.Document` 인스턴스에서 반환됩니다. `com.adobe.idp.Document` 인스턴스가 Forms 서비스로 전달됩니다. 대화형 양식은 클라이언트 웹 브라우저에 작성됩니다. ([Forms 서비스에 문서 전달](/help/forms/developing/passing-documents-forms-service.md)을 참조하십시오.)
