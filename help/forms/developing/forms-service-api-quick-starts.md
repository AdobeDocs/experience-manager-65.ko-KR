...
다음 코드 예제에서는 Forms 서비스 Java™ API를 사용하여 HTML 양식을 렌더링합니다. HTML 양식과 두 개의 첨부 파일에 도구 모음이 추가됩니다. 또한 `HttpServletRequest` 개체에서 사용자 에이전트 값을 가져옵니다. ([HTML으로 Forms 렌더링](/help/forms/developing/rendering-forms.md#rendering-forms-as-html)을 참조하십시오.)
...
다음 코드 예제에서는 PDF 데이터로 제출된 양식을 처리합니다. `processFormSubmission` 메서드에 전달된 콘텐츠 형식 값은 `CONTENT_TYPE=application/pdf`입니다. 제출된 양식은 *tempPDF.pdf*(이)라는 PDF 파일로 저장됩니다. 또한 양식이 PDF으로 제출되므로 첨부 파일을 검색할 수 있습니다. 모든 첨부 파일은 JPEG 파일로 저장됩니다. ([제출된 Forms 처리](/help/forms/developing/rendering-forms.md#handling-submitted-forms)를 참조하십시오.)
