...
이 DDX 문서 내에서 `DocumentInformation` 요소는 어셈블러 서비스에 입력 PDF 문서에 대한 정보를 반환하도록 지시합니다. `DocumentInformation` 요소 내에서 `PDFAValidation` 요소는 입력 PDF 문서가 PDF/A 규격인지 여부를 나타내는 데 어셈블러 서비스에 지시합니다.

어셈블러 서비스는 입력 PDF 문서가 `PDFAConformance` 요소가 포함된 XML 문서 내에서 PDF/A와 호환되는지 여부를 지정하는 정보를 반환합니다. 입력 PDF 문서가 PDF/A와 호환되는 경우 `PDFAConformance` 요소의 `isCompliant` 속성 값은 `true`입니다. PDF 문서가 PDF/A를 준수하지 않을 경우 `PDFAConformance` 요소의 `isCompliant` 속성 값은 `false`입니다.
...
어셈블러 서비스 작업을 수행하려면 DCX 문서를 참조해야 합니다. 입력 PDF 문서가 PDF/A와 호환되는지 확인하려면 DCX 문서에 `DocumentInformation` 요소 내에 `PDFAValidation` 요소가 포함되어 있는지 확인합니다. `PDFAValidation` 요소는 입력 PDF 문서가 PDF/A와 호환되는지 여부를 지정하는 XML 문서를 반환하는 어셈블러 서비스에 지시합니다.
...
어셈블러 서비스 클라이언트를 만들고 DDX 문서를 참조하고 대화형 PDF 문서를 참조하고 런타임 옵션을 설정한 후 `invokeDDX` 작업을 호출할 수 있습니다. DCX 문서에 `DocumentInformation` 요소가 포함되어 있으므로 어셈블러 서비스는 PDF 문서 대신 XML 데이터를 반환합니다.
...
1. 기존 DDX 문서를 참조합니다.
   ...
   * 생성자를 사용하고 DDX 파일의 위치를 지정하는 문자열 값을 전달하여 DDX 문서를 나타내는 `java.io.FileInputStream` 개체를 만듭니다. PDF 문서가 PDF/A와 호환되는지 확인하려면 DCX 문서에 `DocumentInformation` 요소 내에 포함된 `PDFAValidation` 요소가 포함되어 있는지 확인합니다.
   * 생성자를 사용하여 `com.adobe.idp.Document` 개체를 만들고 `java.io.FileInputStream` 개체를 전달합니다.

1. PDF/A 규정 준수를 결정하는 데 사용되는 PDF 문서를 참조합니다.

   * 생성자를 사용하여 `java.io.FileInputStream` 개체를 만들고 PDF/A 규정 준수를 확인하는 데 사용되는 PDF 문서의 위치를 전달하면 됩니다.
...
1. PDF 문서에 대한 정보 검색
   ...
   * 런타임 옵션을 지정하는 `com.adobe.livecycle.assembler.client.AssemblerOptionSpec` 개체
   ...
   `invokeDDX` 메서드는 입력 PDF 문서가 PDF/A 규격인지 여부를 지정하는 XML 데이터가 포함된 `com.adobe.livecycle.assembler.client.AssemblerResult` 개체를 반환합니다.
...
## 웹 서비스 API {#determine-whether-a-document-is-pdf-a-compliant-using-the-web-service-api}를 사용하여 문서가 PDF/A 규격인지 확인
...
1. PDF/A 규정 준수를 결정하는 데 사용되는 PDF 문서를 참조합니다.
   ...
   * `MyMapOf_xsd_string_To_xsd_anyType_Item` 개체를 `MyMapOf_xsd_string_To_xsd_anyType` 개체에 추가합니다. `MyMapOf_xsd_string_To_xsd_anyType` 개체 `Add` 메서드를 호출하고 `MyMapOf_xsd_string_To_xsd_anyType` 개체를 전달합니다.
...
1. PDF 문서에 대한 정보 검색
   ...
   * 입력 PDF 문서를 포함하는 `MyMapOf_xsd_string_To_xsd_anyType` 개체 해당 키는 PDF 소스 파일의 이름과 일치해야 하며 해당 값은 입력 PDF 파일에 해당하는 `BLOB` 개체여야 합니다.
...
1. 반환된 XML 문서를 저장합니다.
   ...
   * 입력 PDF 문서가 PDF/A 문서인지 여부를 지정하는 XML 데이터가 포함된 `AssemblerResult` 개체의 `documents` 필드에 액세스합니다.`Map`
   * 각 결과 문서를 얻으려면 `Map` 개체를 반복합니다. 그런 다음 해당 배열 멤버의 값을 `BLOB`으로 캐스팅합니다.
   * `BLOB` 개체의 `MTOM` 필드에 액세스하여 XML 데이터를 나타내는 이진 데이터를 추출합니다. 이 필드는 XML 파일로 기록할 수 있는 바이트 배열을 저장합니다.
