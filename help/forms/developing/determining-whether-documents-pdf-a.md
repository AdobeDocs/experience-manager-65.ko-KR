...
PDF/A-1 사양은 두 가지 적합성 수준, 즉 A와 B로 구성됩니다. 두 수준의 주요 차이점은 적합성 수준 B에는 필요하지 않은 논리 구조(접근성) 지원입니다. PDF/A-1은 적합성 수준에 관계없이 모든 글꼴이 생성된 PDF/A 문서 내에 임베드됨을 지시합니다. 현재 유효성 검사(및 전환)에서는 PDF/A-1b만 지원됩니다.
...
이 DDX 문서 내에서 `DocumentInformation` 요소는 어셈블러 서비스에 입력 PDF 문서에 대한 정보를 반환하도록 지시합니다. `DocumentInformation` 요소 내에서 `PDFAValidation` 요소는 어셈블러 서비스에 입력 PDF 문서가 PDF/A 규격인지 여부를 표시하도록 지시합니다.

어셈블러 서비스는 `PDFAConformance` 요소가 포함된 XML 문서 내에서 입력 PDF 문서가 PDF/A와 호환되는지 여부를 지정하는 정보를 반환합니다. 입력 PDF 문서가 PDF/A와 호환되는 경우 `PDFAConformance` 요소의 `isCompliant` 특성 값은 `true`입니다. PDF 문서가 PDF/A를 준수하지 않는 경우 `PDFAConformance` 요소의 `isCompliant` 특성 값은 `false`입니다.

>[!NOTE]
>
>이 섹션에 지정된 DDX 문서에 `DocumentInformation` 요소가 포함되어 있으므로 어셈블러 서비스는 PDF 문서 대신 XML 데이터를 반환합니다. 즉, 어셈블러 서비스는 PDF 문서를 어셈블하거나 디스어셈블하지 않으며 XML 문서 내의 입력 PDF 문서에 대한 정보를 반환합니다.
...
어셈블러 서비스 작업을 수행하려면 DDX 문서를 참조해야 합니다. 입력 PDF 문서가 PDF/A를 준수하는지 확인하려면 DDX 문서에 `DocumentInformation` 요소 내에 `PDFAValidation` 요소가 포함되어 있는지 확인하십시오. `PDFAValidation` 요소는 어셈블러 서비스에 입력 PDF 문서가 PDF/A 규격 문서인지 여부를 지정하는 XML 문서를 반환하도록 지시합니다.

**PDF/A 준수 여부를 확인하는 데 사용되는 PDF 문서를 참조하십시오**

PDF 문서를 참조하고 어셈블러 서비스에 전달하여 PDF 문서가 PDF/A를 준수하는지 여부를 확인해야 합니다.
...
어셈블러 서비스가 반환하는 XML 문서는 입력 PDF 문서가 PDF/A를 준수하는지 여부를 지정합니다. 예를 들어 입력 PDF 문서가 PDF/A와 호환되지 않으면 어셈블러 서비스는 다음 요소가 포함된 XML 문서를 반환합니다.
...
[웹 서비스 API를 사용하여 문서가 PDF/A를 준수하는지 확인](/help/forms/developing/determining-whether-documents-pdf-a.md#determine-whether-a-document-is-pdf-a-compliant-using-the-web-service-api)
...
1. 기존 DDX 문서를 참조합니다.
   ...
   * 생성자를 사용하고 DDX 파일의 위치를 지정하는 문자열 값을 전달하여 DDX 문서를 나타내는 `java.io.FileInputStream` 개체를 만듭니다. PDF 문서가 PDF/A를 준수하는지 확인하려면 DDX 문서에 `DocumentInformation` 요소 내에 포함된 `PDFAValidation` 요소가 포함되어 있는지 확인하십시오.
   * 생성자를 사용하고 `java.io.FileInputStream` 개체를 전달하여 `com.adobe.idp.Document` 개체를 만듭니다.

1. PDF/A 준수 여부를 확인하는 데 사용되는 PDF 문서를 참조하십시오.

   * 생성자를 사용하고 PDF/A 준수 여부를 확인하는 데 사용되는 PDF 문서의 위치를 전달하여 `java.io.FileInputStream` 개체를 만듭니다.
...
1. PDF 문서에 대한 정보를 검색합니다.
   ...
   * PDF/A 규정 준수를 확인하는 데 사용되는 입력 PDF 파일이 포함된 `java.util.Map` 개체입니다.
   * 런타임 옵션을 지정하는 `com.adobe.livecycle.assembler.client.AssemblerOptionSpec` 개체

   `invokeDDX` 메서드는 입력 PDF 문서가 PDF/A 규격 문서인지 여부를 지정하는 XML 데이터를 포함하는 `com.adobe.livecycle.assembler.client.AssemblerResult` 개체를 반환합니다.
...
[빠른 시작(SOAP 모드): Java API를 사용하여 문서가 PDF/A와 호환되는지 확인](/help/forms/developing/assembler-service-java-api-quick.md#quick-start-soap-mode-determining-whether-a-document-is-pdf-a-compliant-using-the-java-api)(SOAP 모드)

[AEM Forms Java 라이브러리 파일 포함](/help/forms/developing/invoking-aem-forms-using-java.md#including-aem-forms-java-library-files)

[연결 속성 설정](/help/forms/developing/invoking-aem-forms-using-java.md#setting-connection-properties)

## 웹 서비스 API를 사용하여 문서가 PDF/A를 준수하는지 확인 {#determine-whether-a-document-is-pdf-a-compliant-using-the-web-service-api}
...
1. PDF 문서에 대한 정보를 검색합니다.
   ...
   * 입력 PDF 문서가 포함된 `MyMapOf_xsd_string_To_xsd_anyType` 개체입니다. 키는 PDF 소스 파일의 이름과 일치해야 하며, 값은 입력 PDF 파일에 해당하는 `BLOB` 개체여야 합니다.
   * 런타임 옵션을 지정하는 `AssemblerOptionSpec` 개체입니다.

   `invoke` 메서드는 입력 PDF 문서가 PDF/A 문서인지 여부를 지정하는 XML 데이터를 포함하는 `AssemblerResult` 개체를 반환합니다.
