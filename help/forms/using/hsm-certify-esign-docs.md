---
title: HSM을 사용하여 문서 디지털 서명 또는 인증
...
1. **HSM 자격 증명 구성 서비스**&#x200B;를 열고 다음 필드에 대한 값을 지정하십시오.
   ...
   * **DLL 경로**: 서버에 있는 HSM 또는 etoken 클라이언트 라이브러리의 경로를 지정하십시오. 예를 들어, `C:\Program Files\LunaSA\cryptoki.dll`과 같이 입력합니다. 클러스터된 환경에서는 클러스터의 모든 서버가 동일한 경로를 사용해야 합니다.
   ...
   * **인증서 SHA1**: 사용 중인 자격 증명에 대한 공개 키(.cer) 파일의 SHA1 값(지문)을 지정하십시오. SHA1 값에 공백이 포함되지 않았는지 확인하십시오.
