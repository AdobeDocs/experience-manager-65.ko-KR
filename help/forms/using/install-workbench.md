...
이 문서는 Workbench 설치, 구성, 관리 또는 배포를 담당하는 관리자 또는 개발자를 위한 것입니다. 업그레이드된 AEM Forms 프로세스를 지원하도록 시스템을 구성하는 정보도 포함되어 있습니다. 제공된 정보는 이 문서를 읽는 사람이 Microsoft® Windows® 운영 체제를 잘 알고 있다는 가정을 기반으로 합니다.
...
>[!NOTE]
>
>AEM Forms 서버에는 별도의 설치 프로그램이 필요합니다. 자세한 내용은 JEE의 AEM Forms 설치 설명서 를 참조하십시오.
...
>[!NOTE]
>
>Workbench를 설치하려면 관리 권한이 있어야 합니다. 관리자가 아닌 계정을 사용하여 설치하는 경우 설치 관리자가 적절한 계정에 대한 자격 증명을 묻는 메시지를 표시합니다.

### 지원되는 플랫폼 {#supported-platforms}

Workbench에 대해 지원되는 플랫폼 전체 목록을 참조 하십시오. [AEM Forms 지원 플랫폼](https://www.adobe.com/go/learn_aemforms_supportedplatforms_65).
...
아래 표에는 Workbench 설치 시 발생할 수 있는 Designer 설치 시나리오 및 수행해야 하는 작업에 대한 전체 목록이 나와 있습니다.
...
1. 다음 작업 중 하나를 수행합니다.
   ...
   * 워크벤치를 다운로드하여 파일 시스템에 압축을 해제합니다. 다운로드한 후 \workbench 디렉토리로 이동하고 run_windows_installer.bat 파일을 두 번 클릭합니다.
   ...
   >[!NOTE]
   >
   >&quot;Java™ 가상 컴퓨터를 만들 수 없습니다.&quot; 오류가 발생하면 -Xmx512M 값으로 _JAVA_OPTIONS라는 환경 변수를 만들고 설치 관리자를 실행합니다.
...
1. 이제 컴퓨터에 다음 항목이 설치됩니다.
   ...
   * **AEM Forms SDK**: SDK 사용에 대한 자세한 내용은 <a href="https://helpx.adobe.com/pdf/aem-forms/6-3/programming-with-aem-forms.pdf">AEM Forms을 사용한 프로그래밍</a>.
...
Workbench를 사용하려면 일반적으로 별도의 컴퓨터에서 실행되는 AEM Forms 인스턴스가 있어야 합니다. AEM Forms에 로그온하려면 사용자 이름 및 암호와 서버 위치에 대한 세부 정보가 있어야 합니다.
...
위치:
   * `changeit` 은 캐시 키 저장소에 대한 기본 암호입니다.
   * certname은 1단계에서 선택한 인증서입니다.
   * 예제는 인증서에 대해 선택하는 별칭입니다. 이 값을 변경할 수 있습니다.

1. 인증서를 신뢰하라는 메시지가 표시되면 예 를 입력하고 Enter 키를 누릅니다. 키 도구가 계속 진행되어 캐시 파일을 로 가져옵니다 [Workbench_HOME]/workbench/jre/lib/security 디렉터리입니다.
