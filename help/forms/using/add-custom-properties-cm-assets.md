...
# 서신 관리 자산에 사용자 정의 속성 추가{#add-custom-properties-to-correspondence-management-assets}
...
1. 새로 만든 항목 폴더 아래에 다음 단계를 사용하여 모든 에셋의 사용자 지정 속성에 대한 노드를 추가합니다(예: GeoLocation).
   ...
   1. 노드 만들기 대화 상자에 다음 값이 있는지 확인하고 **확인**&#x200B;을 클릭합니다.
      ...
      **이름:** 지리적 위치(또는 이 속성에 지정할 이름)

      **유형:** nt:unstructured

      ![노드 만들기: 지리적 위치](assets/geographicallocationcreatenode.png)

   1. 만든 새 노드(여기서는 GeoLocation)를 클릭합니다. CRX은 노드의 속성을 표시합니다.
   1. 노드에 다음 속성을 추가합니다(여기서는 GeoLocation).

      | **이름** | **유형** | **값** |
      |---|---|---|
      | fieldLabel | 문자열 | 필드/속성을 지정할 이름입니다. (여기서: 수신자 위치) |
      | 이름 | 문자열 | `./extendedproperties/GeoLocation`(이 값은 항목 노드 아래에 만든 필드 이름과 동일하게 유지) |
      | renderReadOnly | 부울 | true |
      | sling:resourceType | 문자열 | `granite/ui/components/coral/foundation/form/textfield` |

   1. **모두 저장**&#x200B;을 클릭합니다.

1. 사용자 지정을 보려면 에셋(텍스트, 목록, 조건 또는 레이아웃 조각) 또는 편지 위로 마우스를 가져간 후 **속성 보기**&#x200B;를 클릭하고 **편집**&#x200B;을 클릭합니다. 에셋/편지 속성의 기본 탭에 새 필드(수신자 위치)가 표시됩니다.

   >[!NOTE]
   >
   >사용자 지정이 UI에 표시되기 전에 브라우저 캐시를 지워야 할 수 있습니다.

   ![모든 자산에 사용자 지정 속성이 추가됨](assets/lcoationofrecipientsui-1.png)

   >[!NOTE]
   >
   >추가하는 모든 에셋에 대한 일반 속성은 에셋 속성의 기본 탭에 표시됩니다. 기본적으로 모든 에셋에 대해 추가된 공통 속성은 속성 페이지와 에셋 생성 페이지에 표시됩니다. 일반 속성을 숨기려면 <!--link to show / hide properties]-->해야 합니다.
...
1. 새로 생성된 오버레이 노드(/apps/fd/cm/ma/gui/content/cmmetadataproperties/commonproperties/col1/items) 아래
nt:unstructured 유형의 드롭다운(여기서는 `geographicallocation`)을 만들어야 하는 각 속성(필드)에 대한 노드를 만듭니다.
1. 다음 속성을 노드(여기서는 geographicallocation)에 추가하고 **모두 저장**&#x200B;을 클릭합니다.
   ...
   <tr>
      <td>이름</td>
      <td>문자열</td>
      <td>./extendproperties/geographicallocation (값을 항목 노드 아래에 생성한 필드 이름과 동일하게 유지)</td>
   </tr>
   <tr>
...
1. 속성 노드(여기서는 geographicallocation) 아래에 이름이 `items`인 새 노드를 추가합니다. 항목 노드 아래에 드롭다운의 값에 대해 각각 노드를 추가합니다. 첫 번째 노드를 공백으로 추가하여 드롭다운의 기본값으로 사용하고 사용자가 필드에 값을 지정하지 않는 옵션을 제공하는 것이 좋습니다. 여러 옵션/드롭다운 값을 추가하려면 다음 단계를 반복합니다.

   1. 속성 노드(여기서는 geographicallocation)를 마우스 오른쪽 단추로 클릭하고 **만들기** > **노드 만들기**&#x200B;를 선택합니다.
   1. 필드 이름을 `item1,`(으)로 입력하고 **확인**&#x200B;을(를) 클릭합니다.:unstructured
   1. 새로 만든 노드(여기 item1)에 다음 속성을 추가한 다음 **모두 저장**&#x200B;을 클릭합니다.
      ...
      <table>
         ...
         <tr>
          <td>text</td>
          <td>문자열</td>
          <td>사용자가 볼 수 있는 드롭다운 옵션의 값입니다. 빈(기본값) 값에 대해 비워 두거나 <strong>International</strong> 또는 <strong>Within US</strong>.<br /> 등의 값을 입력하십시오. </td>
         </tr>
         <tr>
          <td>값</td>
          <td>문자열</td>
...
![모든 자산 유형에 대해 사용자 지정 탭이 추가됨](assets/recipientstab.png)
...
1. 앱 폴더에서 다음 단계를 사용하여 cmmetadataproperties 폴더(컨텐츠 폴더)와 유사한 경로/구조로 cmmetadataproperties라는 폴더를 만듭니다.
   ...
   1. **확인**&#x200B;을 클릭합니다. 폴더 구조는 앱 폴더에 생성됩니다.
      ...
      ![CRX에서 만들어진 오버레이 폴더 구조](assets/cmmetadatapropertiesappsfolder.png)
...
1. cmmetadataproperties 폴더 아래에 다음 단계를 사용하여 모든 에셋에 대한 사용자 정의 탭을 만들기 위한 노드(예: commontab)를 추가합니다.
   ...
   1. 생성한 새 노드(여기 commontab)를 클릭합니다. CRX은 노드의 속성을 표시합니다.
...
1. 마지막 단계(여기서는 commontab)에서 생성된 탭 노드의 경우, 다음 단계를 사용하여 item이라는 노드를 생성합니다.
   ...
   1. 관련 노드(여기서는 commontab)를 마우스 오른쪽 단추로 클릭하고 **만들기** > **노드 만들기**&#x200B;를 선택합니다.
   1. 노드 만들기 대화 상자에 다음 값이 있는지 확인하고 **확인**&#x200B;을 클릭합니다.
...
1. 이전 단계(commontab 아래)에서 생성한 항목 노드의 다음 단계를 사용하여 사용자 정의 탭(commontab)에 열(여기서는 Column1)을 생성하기 위한 노드를 추가합니다(열을 더 추가하려면 이 단계를 반복합니다).
   ...
   1. 노드 만들기 대화 상자에 다음 값이 있는지 확인하고 **확인**&#x200B;을 클릭합니다.
      ...
      **이름:** 열1(또는 노드에 지정할 이름 - 이 이름은 사용자 인터페이스에 표시되지 않습니다.)

      **유형:** nt:unstructured

   1. 다음 속성을 노드(여기 Column1)에 추가한 다음 **모두 저장**&#x200B;을 클릭합니다.
...
1. 이전 단계(여기서는 Column1)에서 만든 노드에서 다음 단계를 사용하여 items라는 노드를 추가합니다.
   ...
   1. 노드(여기 Column1)를 마우스 오른쪽 단추로 클릭하고 **만들기** > **노드 만들기**&#x200B;를 선택합니다.
   1. 노드 만들기 대화 상자에 다음 값이 있는지 확인하고 **확인**&#x200B;을 클릭합니다.
...
1. 사용자 지정 탭(여기서는 수신자)에 필드를 만들려면 노드(여기서는 GeographyLocation)를 추가합니다. 이 속성은 사용자가 만든 열에 해당합니다. 필드를 만들려면 다음 단계를 사용하십시오(더 많은 필드/노드를 만들려면 이 단계를 반복하십시오).
   ...
   1. 필드 노드(여기서는 GeographicLocation)에 다음 속성을 추가하고 **모두 저장**&#x200B;을 클릭합니다.
...
1. 편지에 대해 이 탭을 추가하려면 다음 경로의 다음 항목 폴더와 유사한 경로/구조로 오버레이 폴더를 만드십시오.
   ...
   문자 또는 다른 에셋에 대한 오버레이를 만들려면 [assettype]을(를) 텍스트, 조건, 목록, 데이터 사전 또는 조각으로 바꾸어 다음 경로를 사용하십시오.
...
1. 새로 만든 항목 폴더에서 다음 단계를 사용하여 에셋의 사용자 지정 탭에 대한 노드를 추가합니다(여기서 mytab - 이 이름은 사용자 인터페이스에 표시되지 않음).
   ...
   1. 만든 새 노드(여기 mytab)를 클릭합니다. CRX은 노드의 속성을 표시합니다.
...
1. 사용자 지정을 보려면 관련 에셋(여기에 있는 편지) 위로 마우스를 가져간 후 속성 보기를 클릭하고 **편집**&#x200B;을 클릭합니다. 사용자 인터페이스에 새 탭(수신자)과 필드(수신자 위치)가 나타납니다.

   >[!NOTE]
   >
   >사용자 지정이 UI에 표시되기 전에 브라우저 캐시를 지워야 할 수 있습니다.

   ![사용자 지정 탭이 편지에 추가됨](assets/recipientstab-1.png)
...
1. 새로 만든 항목 폴더에서 다음 단계를 사용하여 에셋의 사용자 지정 탭에 대한 노드를 추가합니다(예: customtab).
   ...
   1. 만든 새 노드(여기 customtab)를 클릭합니다. CRX은 노드의 속성을 표시합니다.
   1. 노드에 다음 두 가지 속성을 추가합니다(여기서 customtab).
...
1. 이전 단계에서 만든 노드(여기서는 customtab)에 다음 단계를 사용하여 items라는 노드를 추가합니다.
   ...
   1. 노드(여기서는 customtab)를 마우스 오른쪽 단추로 클릭하고 **만들기** > **노드 만들기**&#x200B;를 선택합니다.
   1. 노드 만들기 대화 상자에 다음 값이 있는지 확인하고 **확인**&#x200B;을 클릭합니다.
...
1. 생성된 각 열에 대해 사용자 인터페이스의 새 탭에서 필드를 만들기 위해 항목 노드 아래에 노드를 만듭니다. 열에 필드를 더 만들려면 이 단계를 반복합니다.
   ...
   1. 관련 노드(여기 Column1 아래의 항목)를 마우스 오른쪽 단추로 클릭하고 **만들기** > **노드 만들기**&#x200B;를 선택합니다.
   1. 노드 만들기 대화 상자에 다음 값이 있는지 확인하고 **확인**&#x200B;을 클릭합니다.

      **이름:** 선택한 이름(여기서 지리적 위치)
...
1. 사용자 지정을 보려면 관련 에셋(여기에 있는 텍스트) 위로 마우스를 가져간 후 속성 보기를 클릭하고 **편집**&#x200B;을 클릭합니다. 사용자 인터페이스에 새 탭과 필드(수신자의 위치)가 나타납니다.
...
1. 만든 오버레이 항목 노드 아래에 col4(또는 다른 이름)라는 이름의 노드를 만들고 **모두 저장**&#x200B;을 클릭합니다.
...
1. childlistpage.jsp 파일(/apps/fd/cm/ma/gui/components/admin/childpagerenderer/childlistpage/childlistpage.jsp)을 열고 다음 사항을 변경합니다.
   ...
   1. &lt;tr> 태그(&lt;tr &lt;%= attrs.build() %>) 시작 전에 다음을 추가하십시오.
      ...
      코드에서 GeoLocation은 사용자 지정 노드/필드를 만드는 동안 이름 속성에 설정한 값입니다. 사용자 지정 노드/필드를 만드는 동안 ./extendproperties/ 접두사가 있는 속성 이름을 지정했습니다: ./extendproperties/GeoLocation. 코드에서 접두사는 필요하지 않습니다.
...
sling:orderBefore 속성을 추가하여 열 위치를 지정할 때 이 절차의 6.4단계에서 지정된 해당 &lt;td> 태그의 순서도 업데이트해야 합니다. 예를 들어 이 경우 지리적 위치의 &lt;td> 태그가 버전 열의 &lt;td> 태그 앞에 오도록 해야 합니다.
...
1. `https://'[server]:[port]'/[ContextPath]/crx/de`(으)로 이동하여 관리자로 로그인합니다.
1. `/oak:index/cmLucene`(으)로 이동하여 **집계** 노드를 추가하십시오.
...
1. 새로 만든 집계 폴더 아래에 cm:resource 노드를 추가합니다. cm:resource 아래에 include0이라는 노드를 추가합니다.
   ...
   1. 만든 새 노드(여기에 포함0)를 클릭합니다. CRX은 노드의 속성을 표시합니다.
...
1. 다음 위치의 속성으로 이동한 다음 그 아래에 노드 위치를 추가하십시오. `/oak:index/cmLucene/indexRules/cm:resource/properties`
   ...
   1. 노드 만들기 대화 상자에 다음 값이 있는지 확인하고 **확인**&#x200B;을 클릭합니다.
      ...
      **이름:** 위치(또는 검색에 추가하려는 사용자 지정 속성 이름)

      **유형:** nt:unstructured

   1. 만든 새 노드(여기 위치)를 클릭합니다. CRX은 노드의 속성을 표시합니다.
...
1. 새로 만든 노드의 목록에서 다음 속성을 추가하고 **모두 저장**&#x200B;을 클릭합니다.
...
1. 사용자 지정은 Forms 및 문서, Assets 및 사이트를 비롯한 모든 콘솔에 대한 검색 결과를 목록 보기에 표시합니다.

## 에셋 페이지의 기본 보기 변경 {#change-default-view-of-the-assets-page}

>[!NOTE]
>
>이러한 단계는 Forms 및 Documents, Assets 및 Sites와 같은 모든 콘솔의 기본 보기를 변경합니다.
...
1. geographicallocation과 같은 사용자 지정 속성 노드 아래에 &quot;nt:unstructured&quot; 유형의 &quot;granite:rendercondition&quot;(이)라는 이름의 노드를 만듭니다.
1. 다음 속성을 노드에 추가하고 **모두 저장**&#x200B;을 클릭합니다.
...
1. 자산 만들기 페이지에서 이 속성을 숨기려면 다음 속성을 추가하고 **모두 저장**&#x200B;을 클릭합니다.
...
1. 자산의 속성 페이지에서 사용자 지정 속성을 숨기려면 다음 속성을 추가하고 **모두 저장**&#x200B;을 클릭합니다.
   ...
   값을 다시 표시하려면 속성 값을 `false`(으)로 다시 설정하거나 속성 항목을 삭제하십시오.
