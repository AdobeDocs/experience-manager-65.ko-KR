...
>[!NOTE]
>
>* AEM 6.4 Forms에서 crx-repository의 구조 및 경로가 변경되었습니다. 이전 버전에서 AEM 6.4 Forms으로 에셋을 가져오고 양식에 이전 구조에 대한 일부 종속성이 있는 경우 종속성을 수동으로 내보내야 합니다. 저장소의 구조 및 경로 변경 사항에 대한 자세한 내용은 [AEM의 저장소 재구성](/help/sites-deploying/repository-restructuring.md)을 참조하십시오.
>

## Forms 및 문서 에셋 다운로드 또는 업로드 {#download-or-upload-forms-amp-documents-assets}

AEM Forms 사용자 인터페이스를 사용하면 AEM CRX 패키지 또는 바이너리 파일로 다운로드하여 AEM 인스턴스에서 자산을 내보낼 수 있습니다. 그런 다음 다운로드한 AEM CRX 패키지 또는 이진 파일을 다른 AEM 인스턴스로 가져올 수 있습니다.
...
이러한 에셋 유형의 경우 AEM 패키지 관리자를 사용하여 소스 AEM 서버에 CRX 패키지를 만들고 대상 서버에 패키지를 설치해야 합니다. 패키지를 만들고 설치하는 방법에 대한 자세한 내용은 [패키지 작업](/help/sites-administering/package-manager.md)을 참조하십시오.
...
1. Experience Manager ![adobeexperiencemanager](assets/adobeexperiencemanager.png) 아이콘 > 탐색 ![나침반](assets/compass.png) 아이콘 > Forms > Forms 및 문서를 선택합니다.
1. 양식 자산을 선택하고 **다운로드** 아이콘을 선택합니다.
...
1. AEM Forms 인스턴스에 로그인합니다.
1. Experience Manager ![adobeexperiencemanager](assets/adobeexperiencemanager.png) 아이콘 > 탐색 ![나침반](assets/compass.png) 아이콘> Forms> Forms 및 문서 를 선택합니다.
1. **만들기** >**파일 업로드**&#x200B;를 선택합니다. 양식 또는 패키지 업로드 대화 상자가 나타납니다.
1. 대화 상자에서 가져올 패키지 또는 아카이브를 찾아 선택합니다. PDF 문서, XSD, 이미지, 스타일시트 및 XDP 양식을 선택할 수도 있습니다. **열기**&#x200B;를 선택합니다. 선택한 폴더 또는 파일 이름에는 특수 문자가 포함되지 않아야 합니다.

   대화 상자에서 업로드할 자산의 세부 정보를 확인하고 **업로드**&#x200B;를 선택합니다.
...
AEM Forms을 사용하여 테마를 만들거나, 다운로드하거나, 업로드할 수 있습니다. 테마는 양식, 문서 및 편지와 같은 다른 에셋처럼 만들어집니다. 테마를 만들고, 다운로드한 다음 별도의 인스턴스에 업로드하여 재사용할 수 있습니다. 테마에 대한 자세한 내용은 [AEM Forms의 테마](../../forms/using/themes.md)를 참조하십시오.
...
1. AEM Forms 인스턴스에 로그인합니다.
1. Experience Manager ![adobeexperiencemanager](assets/adobeexperiencemanager.png) 아이콘 > 탐색 ![나침반](assets/compass.png) 아이콘> Forms> 테마를 선택합니다.
1. 테마를 선택하고 **다운로드**&#x200B;를 선택합니다. 테마는 보관 파일(.zip 파일)로 다운로드됩니다.
...
1. Experience Manager에서 **Forms > 테마**(으)로 이동합니다.
...
1. AEM Forms 인스턴스에 로그인합니다.
1. Experience Manager ![adobeexperiencemanager](assets/adobeexperiencemanager.png) 아이콘 > 탐색 ![나침반](assets/compass.png) 아이콘> Forms> 테마를 선택합니다.
1. **만들기** > **파일 업로드**&#x200B;를 클릭합니다. 파일 업로드 프롬프트에서 컴퓨터에서 테마 패키지를 찾아 선택하고 **업로드**&#x200B;를 클릭합니다. 테마가 업로드되었습니다.
...
예를 들어 서신 관리에 이미지와 텍스트를 사용하는 문자가 있는 경우 다운로드한 패키지에는 문자와 관련된 이미지와 텍스트도 포함됩니다. 자산과 연결된 모든 메타데이터 속성(사용자 지정 속성 포함)도 다운로드됩니다. 패키지(.cmp)를 다운로드하면 [다른 AEM Forms 인스턴스로 패키지를 가져올 수 있습니다](../../forms/using/import-export-forms-templates.md#p-upload-forms-documents-assets-p).

모든 서신 관리 에셋 및 관련 종속성을 패키지로 다운로드하려면 다음 단계를 완료하십시오.

1. AEM Forms 서버에 양식 사용자로 로그인합니다.
1. 전역 탐색 모음에서 **Adobe Experience Manager**&#x200B;을(를) 선택합니다.
1. 도구(![도구](assets/tools.png))를 선택한 다음 **Forms**&#x200B;을 선택합니다.
1. **서신 관리 Assets 내보내기**&#x200B;를 선택합니다.

   ![publish-cmp-assets-1](assets/publish-cmp-assets-1.png)

   ( &quot;모든 응답 관리 내보내기 Assets 페이지가 나타나고 내보내기 프로세스를 마지막으로 시도한 시간에 대한 정보와 마지막으로 성공적으로 내보낸 패키지를 다운로드하는 링크가 표시됩니다.

   ![export-last-run-details](assets/export-last-run-details.png)

1. **내보내기**&#x200B;를 선택하고 확인 메시지에서 **확인**&#x200B;을 선택합니다.
   ...
   >[!NOTE]
   >
   >한 번 시작되면 모든 Assets 내보내기 프로세스를 취소할 수 없습니다. 또한 모두 내보내기 작업이 진행 중인 동안에는 에셋을 만들거나, 삭제하거나, 수정하거나, 게시하거나, Publish 모든 Assets 프로세스를 시작하지 마십시오.a

1. 패키지 파일을 다운로드하려면 **내보낸 패키지 다운로드** 링크를 선택하십시오.

   패키지의 자산을 서신 관리의 다른 인스턴스에 추가하려면 [패키지를 AEM Forms 인스턴스로 가져오기](../../forms/using/import-export-forms-templates.md#p-upload-forms-documents-assets-p)하십시오.
...
>[!NOTE]
>
>마이그레이션을 위해 이전 서신 관리 자산을 가져오는 동안 관리자 계정을 사용하여 로그인합니다. 이전 서신 관리 에셋을 마이그레이션하는 방법에 대한 자세한 내용은 [서신 관리 에셋을 AEM 6.1 양식으로 마이그레이션](/help/forms/using/migration-utility.md)을 참조하십시오.

1. 데이터 사전, 편지 또는 문서 단편 페이지에서 **만들기 > 파일 업로드**&#x200B;를 선택하고 .cmp 파일을 선택합니다.
1. 서신 관리에는 가져온 에셋 목록과 함께 Assets 가져오기 대화 상자가 표시됩니다. **가져오기**&#x200B;를 선택합니다.
...
1. 관련 Assets 또는 편지 페이지에서 하나 이상의 에셋/편지를 선택합니다. UI에 복사 아이콘이 표시됩니다.
1. 복사를 선택합니다. UI에 붙여넣기 아이콘이 표시됩니다. 붙여넣기 전에 폴더 내부로 이동하거나 탐색할 수도 있습니다. 서로 다른 폴더에 동일한 이름의 자산이 포함될 수 있습니다. 폴더에 대한 자세한 내용은 [폴더 및 자산 구성](#folders-and-organizing-assets)을 참조하십시오.
...
AEM Forms UI를 통해 콘텐츠를 검색할 수 있습니다. 상단 표시줄을 사용하여 **[A]** 검색을 선택하여 자산과 문서 등의 리소스에 대한 콘텐츠를 검색할 수 있습니다.

에셋을 검색할 때 AEM Forms에 사이드 패널이 표시됩니다. ![assets-browser-content-only](assets/assets-browser-content-only.png) > 필터 **[B]**&#x200B;를 선택하여 사이드 패널을 호출할 수도 있습니다. 사이드 패널의 다양한 필터를 사용하여 검색 범위를 좁힐 수 있습니다. 사이드 패널에서는 검색을 저장할 수도 있습니다.
...
검색, 필터, 저장된 검색 및 사이드 패널 사용에 대한 자세한 내용과 지침은 [검색](/help/sites-authoring/search.md)을 참조하세요.
