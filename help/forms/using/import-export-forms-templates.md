...
1. Experience Manager 선택 ![adobeexperiencemanger](assets/adobeexperiencemanager.png) 아이콘 > 탐색 ![나침반](assets/compass.png) icon> Forms > Forms 및 문서.
1. 양식 자산을 선택하고 **다운로드** 아이콘.
1. 에셋 다운로드에서 다음 옵션 중 하나를 선택하고 을 선택합니다 **다운로드**.
...
1. 대화 상자에서 가져올 패키지 또는 아카이브를 찾아 선택합니다. PDF 문서, XSD, 이미지, 스타일시트 및 XDP 양식을 선택할 수도 있습니다. 선택 **열기**. 선택한 폴더 또는 파일 이름에는 특수 문자가 포함되지 않아야 합니다.

   대화 상자에서 업로드할 에셋의 세부 정보를 확인하고 을 선택합니다 **업로드**.
...
1. AEM Forms 인스턴스에 로그인합니다.
1. Experience Manager 선택 ![adobeexperiencemanger](assets/adobeexperiencemanager.png) 아이콘 > 탐색 ![나침반](assets/compass.png) icon> Forms> 테마.
1. 테마를 선택하고 선택 **다운로드**. 테마는 보관 파일(.zip 파일)로 다운로드됩니다.
...
1. 편지, 문서 단편 또는 데이터 사전 페이지에서 단일 패키지로 내보낼 자산을 선택하고 다운로드 큐를 선택합니다. 자산을 내보내기 위해 정렬합니다.
...
1. 내보낸 종속성을 보려면 해결을 선택합니다. 또는 다음 단계로 건너뜁니다. 해결을 선택하지 않더라도 종속성은 계속 내보내집니다.
1. .cmp 파일을 다운로드하려면 를 선택합니다. **확인**.
...
1. 선택 **Adobe Experience Manager** 전역 탐색 모음에서 를 클릭합니다.
1. 도구 선택( ![도구](assets/tools.png)) 그런 다음 을 선택합니다 **Forms**.
1. 선택 **응답 관리 에셋 내보내기**.
...
1. 선택 **내보내기** 그리고 확인 메시지에서 다음을 선택합니다. **확인**.
...
1. 다음 항목 선택 **내보낸 패키지 다운로드** 패키지 파일 다운로드 링크.
...
1. 서신 관리는 가져온 에셋 목록과 함께 에셋 가져오기 대화 상자를 표시합니다. 선택 **가져오기**.
...
1. 정의한 제목이 있는 새 폴더가 자산 목록의 현재 위치에 표시됩니다.
   ...
   새로 만든 폴더를 선택하여 폴더 내부로 이동하고 폴더 내에 에셋 또는 폴더를 만들 수 있습니다. 또한 폴더를 선택하여 다운로드 대기열에 추가하거나 삭제하거나 이름을 편집할 수 있습니다.
...
1. 복사를 선택합니다. UI에 붙여넣기 아이콘이 표시됩니다. 붙여넣기 전에 폴더 내부로 이동하거나 탐색할 수도 있습니다. 서로 다른 폴더에 동일한 이름의 자산이 포함될 수 있습니다. 폴더에 대한 자세한 내용은 [폴더 및 자산 구성](#folders-and-organizing-assets).
1. 붙여넣기를 선택합니다. 붙여넣기 대화 상자가 나타납니다. 시스템에서는 에셋/문자의 새 복사본에 이름과 제목을 자동으로 생성하지만 에셋/문자의 제목과 이름을 편집할 수 있습니다.
...
1. 붙여넣기를 선택합니다. 복사된 에셋의 새 복사본이 생성됩니다.

## 검색 {#search-forms}

AEM Forms UI를 통해 콘텐츠를 검색할 수 있습니다. 상단 막대를 사용하여 검색을 선택할 수 있습니다 **[A]** 에셋, 문서 등의 리소스를 콘텐츠에서 검색할 수 있습니다.

에셋을 검색할 때 AEM Forms에 사이드 패널이 표시됩니다. 다음을 선택할 수도 있습니다. ![assets-browser-content-only](assets/assets-browser-content-only.png) > 필터 **[B]** 을 클릭하여 사이드 패널을 호출하십시오. 사이드 패널의 다양한 필터를 사용하여 검색 범위를 좁힐 수 있습니다. 사이드 패널에서는 검색을 저장할 수도 있습니다.
