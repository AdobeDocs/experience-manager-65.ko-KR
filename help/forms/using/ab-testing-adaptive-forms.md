...
1. 선택 **Target 프레임워크**.
1. 다음에서 **TARGET URL** 필드, A/B 테스트가 실행될 모든 URL을 지정합니다. 예: https://&lt;*호스트 이름*>:&lt;*포트*>/ OSGi 또는 https://의 AEM Forms 서버용&lt;*호스트 이름*>:&lt;*포트*JEE의 AEM Forms 서버용 >/lc/
게시 인스턴스에 대한 Target URL을 구성할 때 고객이 호스트 이름 또는 IP 주소를 사용하여 액세스할 수 있으면 두 가지를 모두 Target URL로 구성해야 합니다(호스트 이름과 IP 주소를 사용). URL 중 하나만 구성하는 경우, 다른 URL에서 온 고객에 대해서는 A/B 테스트가 실행되지 않습니다. 클릭 **+** 를 사용하여 여러 URL을 지정할 수 있습니다.
...
1. 대상 페이지에서 을 누릅니다. **대상 만들기 > Target 대상 만들기**.
