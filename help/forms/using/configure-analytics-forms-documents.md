...
적응형 양식, HTML5 양식 및 인터랙티브한 커뮤니케이션을 사용하는 동안 사용자가 직면하는 인터랙션 패턴과 문제를 발견하기 위해 Adobe Analytics을 사용할 수 있습니다. 즉시 Adobe 분석은 다음 매개 변수에 대한 정보를 추적하고 저장합니다.
...
1. **Go Live 날짜**&#x200B;가 오늘로 설정된 상태로 두십시오. 이동 날짜는 보고서 세트가 활성화된 날짜를 결정합니다.
...
1. **보고서 만들기** 세트를 클릭합니다. 보고서 세트가 성공적으로 만들어졌다는 메시지와 함께 페이지 새로 고침이 표시됩니다.
...
1. 왼쪽 위 모서리에서 **Adobe Experience Manager** > **도구** ![](/help/forms/using/assets/tools.png) > **배포** > **Cloud Services**&#x200B;을 클릭합니다.
...
1. **Analytics에 연결**&#x200B;을 클릭합니다. 연결이 성공했다는 대화 상자가 나타납니다. **확인**&#x200B;을 클릭합니다.
...
1. AEM 클라우드 서비스 콘솔에서 Adobe Analytics 아래에 있는 **구성 표시**&#x200B;를 클릭합니다.
...
1. 사이드킥의 **페이지 탭**&#x200B;을 사용하여 프레임워크를 활성화하고 **프레임워크 활성화**&#x200B;를 클릭합니다.

## AEM Forms 분석 구성 서비스 {#configuring-aem-forms-analytics-configuration-service} 구성

1. 작성자 인스턴스의 경우 `https://<server>:<port>;/system/console/configMgr`에서 AEM 웹 콘솔 구성 관리자를 엽니다.
...
1. 다음 필드에 적절한 값을 지정하고 **저장**&#x200B;을 클릭합니다.
   ...
   * **보고서 가져오기 시간 초과:** 서버가 분석 보고서에 응답할 때까지 대기할 기간(초)을 지정합니다. 기본 시간은 120초입니다.
   >[!NOTE]
   >
...
## 양식 또는 문서 {#enabling-analytics-for-a-form-or-document}에 대한 분석 활성화

1. `https://[hostname]:'port'`에 AEM 포털에 로그인합니다.
1. **Forms > Forms 및 문서**&#x200B;를 클릭하고 양식 또는 문서를 선택한 다음 **Analytics 활성화**&#x200B;를 클릭합니다. 분석이 활성화되어 있습니다.

   ![양식 또는 문서에 대한 분석 활성화](assets/enable-analytics-1.png)

   양식에 대한 분석 활성화

   **A.** Analytics 사용 단추  **B.** 선택한 양식
