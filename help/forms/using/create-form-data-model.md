...
이 자습서는 [첫 번째 적응형 양식 만들기](../../forms/using/create-your-first-adaptive-form.md) 시리즈의 한 단계입니다. 전체 자습서 사용 사례를 이해하고, 실행하고, 시연하려면 연순의 순서로 시리즈를 따르는 것이 좋습니다.
...
* [!DNL MySQL]JDBC 드라이버에 대한 OSGi 번들([ JDBC 데이터베이스 드라이버 번들로 제공)에 설명된 대로](/help/sites-developing/jdbc.md#bundling-the-jdbc-database-driver)
* 첫 번째 자습서 [적응형 양식 만들기](/help/forms/using/create-adaptive-form.md)에 설명된 적응형 양식

## 1단계:MySQL 데이터베이스를 데이터 소스 {#config-database}로 구성

양식 데이터 모델을 만들도록 여러 유형의 데이터 소스를 구성할 수 있습니다. 이 자습서의 경우 샘플 데이터로 구성되고 채워진 MySQL 데이터베이스를 구성합니다. 지원되는 다른 데이터 소스 및 구성 방법에 대한 자세한 내용은 [AEM Forms 데이터 통합](../../forms/using/data-integration.md)을 참조하십시오.
...
1. [!DNL MySQL] 데이터베이스의 JDBC 드라이버를 OSGi 번들로 설치합니다.
   ...
   1. **[!UICONTROL 파일]**&#x200B;을 눌러 찾아보고 [!DNL MySQL] JDBC 드라이버 OSGi 번들을 선택합니다. **[!UICONTROL 번들 시작]** 및 **[!UICONTROL 패키지 새로 고침]**&#x200B;을 선택하고 **[!UICONTROL 설치 또는 업데이트]**&#x200B;를 누릅니다. [!DNL MySQL]에 대한 [!DNL Oracle Corporation's] JDBC 드라이버가 활성화되어 있는지 확인합니다. 드라이버가 설치되어 있습니다.
...
1. [!DNL MySQL] 데이터베이스를 데이터 소스로 구성:
   ...
   1. 구성 대화 상자에서 다음 세부 정보를 지정합니다.
      ...
      * **데이터 소스 이름:** 이름을 지정할 수 있습니다. 예를 들어 **WeRetailMySQL**&#x200B;을 지정합니다.
      ...
      * **JDBC 연결 URI**:데이터베이스의 연결 URL을 지정합니다. 포트 3306 및 스키마 웹 사이트에서 실행 중인 [!DNL MySQL] 데이터베이스의 경우 URL은 다음과 같습니다.`jdbc:mysql://'server':3306/weretail?autoReconnect=true&useUnicode=true&characterEncoding=utf-8`
      * **사용자 이름:** 데이터베이스의 사용자 이름입니다. 데이터베이스와의 연결을 설정하려면 JDBC 드라이버를 활성화해야 합니다.
      * **암호:** 데이터베이스의 암호입니다. 데이터베이스와의 연결을 설정하려면 JDBC 드라이버를 활성화해야 합니다.
      * **차입 시 테스트:** 차입시  **[!UICONTROL 테스트 옵션을]** 활성화합니다.
      * **반환 시 테스트:** 반환 시  **[!UICONTROL 테스트를]** 활성화합니다.
      * **유효성 검사 쿼리:** SQL SELECT 쿼리를 지정하여 풀에서 연결을 확인합니다. 쿼리는 하나 이상의 행을 반환해야 합니다. 예를 들어 **customerdetails**&#x200B;에서 *를 선택합니다.
      * **트랜잭션 격리**:값을 READ_ **COMMITTED로 설정합니다**.

         다른 속성에 기본값 [값](https://tomcat.apache.org/tomcat-7.0-doc/jdbc-pool.html)을 두고 **[!UICONTROL 저장]**&#x200B;을 누릅니다.
...
1. 데이터 소스 선택 화면에는 구성된 모든 데이터 소스가 나열됩니다. **WeRetailMySQL** 데이터 소스를 선택하고 **[!UICONTROL 만들기]**&#x200B;를 누릅니다.
...
1. AEM 작성자 인스턴스의 경우 **[!UICONTROL Forms]** > **[!UICONTROL 데이터 통합]**&#x200B;으로 이동합니다. 기본 URL은 [https://localhost:4502/aem/forms.html/content/dam/formsanddocuments-fdm](https://localhost:4502/aem/forms.html/content/dam/formsanddocuments-fdm)입니다.
1. 이전에 만든 **고객-배송-청구-세부 정보** 양식 데이터 모델이 여기에 나와 있습니다. 편집 모드에서 엽니다.

   선택한 데이터 원본 **WeRetailMySQL**&#x200B;이 양식 데이터 모델에서 구성됩니다.

   ![default-fdm](assets/default-fdm.png)

1. WeRailMySQL 데이터 소스 트리를 확장합니다. 데이터 모델을 형성하려면 **웹 소매** > **고객 세부 정보** 스키마에서 다음 데이터 모델 개체 및 서비스를 선택하십시오.
...
1. 데이터 모델 개체에 대한 읽기 및 쓰기 서비스를 구성합니다.
   ...
   1. **customerdetails** 데이터 모델 개체를 선택하고 **[!UICONTROL 속성 편집]**&#x200B;을 누릅니다.
   1. 서비스 읽기 드롭다운에서 **[!UICONTROL get]**&#x200B;을 선택합니다. 사용자 지정 세부 정보 데이터 모델 개체의 기본 키인 **id** 인수가 자동으로 추가됩니다. ![aem_6_3_edit](assets/aem_6_3_edit.png)을 탭하고 인수를 다음과 같이 구성합니다.

      ![읽기 기본값](assets/read-default.png)

   1. 마찬가지로 쓰기 서비스로 **[!UICONTROL 업데이트]**&#x200B;를 선택합니다. **customerdetails** 개체는 자동으로 인수로 추가됩니다. 인수는 다음과 같이 구성됩니다.
   ...
   1. **[!UICONTROL Done]**&#x200B;을 눌러 데이터 모델 개체 속성을 저장합니다. 그런 다음 **[!UICONTROL 저장]**&#x200B;을 눌러 양식 데이터 모델을 저장합니다.

      데이터 모델 개체의 기본 서비스로 **[!UICONTROL get]** 및 **[!UICONTROL update]** 서비스가 추가됩니다.
...
1. **[!UICONTROL 서비스]** 탭으로 이동하여 **[!UICONTROL get]** 및 **[!UICONTROL update]** 서비스를 구성합니다.
   ...
   1. **[!UICONTROL update]** 서비스를 선택하고 **[!UICONTROL 속성 편집]**&#x200B;을 누릅니다. 속성 대화 상자가 열립니다.
   ...
   1. [!UICONTROL 속성 편집] 대화 상자에서 다음을 지정합니다.
      ...
      * **인수**:ID 및  **** 사용자 지정  **세부 정보**&#x200B;라는 인수를 선택합니다.
...
1. **[!UICONTROL 모델]** 탭으로 이동한 후 **고객 세부 정보** 데이터 모델 개체를 선택하고 **[!UICONTROL 테스트 모델 개체]**&#x200B;를 누릅니다.
1. [!UICONTROL 테스트 모델/서비스] 창의 **[!UICONTROL 모델/서비스 선택]** 드롭다운에서 **[!UICONTROL 모델 개체 읽기]**&#x200B;를 선택합니다.
1. **customerdetails** 섹션에서 구성된 [!DNL MySQL] 데이터베이스에 있는 **id** 인수에 대한 값을 지정하고 **[!UICONTROL 테스트]**&#x200B;을 누릅니다.

   지정된 ID와 연결된 고객 세부 사항을 가져와서 아래와 같이 **[!UICONTROL 출력]** 섹션에 표시합니다.
