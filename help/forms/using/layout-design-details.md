...
>[!NOTE]
>
>XDP 템플릿의 하위 양식 구성 요소에 콘텐츠를 추가하는 대신 대화형 커뮤니케이션의 인쇄 채널에 콘텐츠를 추가합니다. 를 사용하여 인쇄 채널의 대상 영역에 컨텐츠를 추가합니다 [문서 조각, 차트, 이미지](create-interactive-communication.md#step2), 및 레이아웃 조각.

XDP 템플릿에서 하위 양식을 사용하려면 다음 단계를 수행하십시오.

1. Forms 디자이너를 열고 을 선택합니다. **파일** > **새로 만들기** > **빈 양식 사용**, 탭 **다음**&#x200B;를 누른 다음 탭합니다. **완료** 을 클릭하여 템플릿을 만들 양식을 엽니다.
...
1. 을(를) 선택합니다 **하위 양식** 탭을 선택하고 **물결** 에서 **컨텐츠** 드롭다운 목록. 에서 **바인딩** 탭:

   1. 하위 폼의 이름을 **이름** 필드.
   1. 선택 **데이터 바인딩 없음** 에서 **데이터 바인딩** 드롭다운 목록.

   2~5단계를 반복하여 XDP 템플릿에 하위 양식을 더 추가합니다. 추가 [텍스트, 문서 조각, 이미지 및 차트](create-interactive-communication.md#step2) Interactive Communication을 작성하는 동안에만 대상 영역에 매핑합니다.
...
>[!NOTE]
>
>XDP 템플릿의 이미지 필드 또는 하위 양식 구성 요소에 이미지를 추가하는 대신 대화형 커뮤니케이션의 인쇄 채널에 이미지를 추가합니다. 자세한 내용은 [대화형 커뮤니케이션에 컨텐츠 추가](../../forms/using/create-interactive-communication.md#step2).

XDP 템플릿에서 이미지 필드 구성 요소를 사용하려면 다음 단계를 수행하십시오.

1. 드래그 앤 드롭 **이미지 필드** 구성 요소 **개체 라이브러리** 추가 정보.
1. 하위 폼을 선택하여 **개체** 오른쪽 창의 창.
1. 에서 **바인딩** 탭:

   1. 에서 이미지 필드의 이름을 지정합니다 **이름** 필드.
   1. 선택 **데이터 바인딩 없음** 에서 **데이터 바인딩** 드롭다운 목록.
...
>[!NOTE]
>
>표 셀에 텍스트를 입력하거나 [양식 데이터 모델 개체로 바인딩 만들기](create-interactive-communication.md#step2) 대화형 커뮤니케이션을 작성하는 동안에만 해당됩니다.
...
1. 에서 **표 삽입** 대화 상자:
   ...
   1. 을(를) 선택합니다 **테이블에 머리글 행 포함** 테이블 머리글에 대한 행을 포함하는 확인란
   1. 탭 **확인**.

1. 탭 **+** 테이블 이름 옆에 있는 왼쪽 창에서 헤더와 다른 행에 포함된 셀 이름을 마우스 오른쪽 단추로 클릭하고 을 선택합니다 **개체 이름 바꾸기** 표 셀의 이름을 변경하려면 다음을 수행하십시오.
1. 에서 테이블 머리글 텍스트 필드를 클릭합니다. **디자인 보기** 이름을 변경할 수 있습니다.
1. 드래그 앤 드롭 **텍스트 필드** 구성 요소 **개체 라이브러리** 의 **디자인 보기**. 대화형 커뮤니케이션을 작성하는 동안 테이블 셀을 양식 데이터 모델 개체에 바인딩하려면 이 단계를 수행하십시오.

   ![테이블의 텍스트 필드](assets/text_fields_table_new.png)

1. 왼쪽 창에서 행 이름을 선택하고 을 선택합니다 **개체** > **바인딩** > **각 데이터 항목에 대해 행 반복**. 이 단계를 수행하여 이 행의 테이블 셀 사이에 컬렉션 유형의 양식 데이터 모델 개체가 있는 바인딩이 만들어지면 데이터베이스에서 사용할 수 있는 각 데이터 항목에 대해 테이블 행이 자동으로 반복되도록 합니다.
...
1. 탭 **만들기** > **파일 업로드**.
1. 로컬 파일 시스템에서 XDP 템플릿의 위치로 이동하여 를 누릅니다 **열기** xdp 템플릿을 AEM Forms 서버로 가져오려면 다음을 수행하십시오.
...
* 디자이너에서 **파일** > **양식 속성** > **기본값**&#x200B;및 PDF 렌더링/형식 설정이 동적 XML 양식으로 설정되어 있는지 확인합니다.
