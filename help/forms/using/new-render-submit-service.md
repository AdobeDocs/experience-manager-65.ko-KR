...
이 메서드는 NewHTMLFormsService의 `renderHTMLForm` API의 정확한 동작을 시뮬레이션합니다. 양식의 HTML 변환에 대한 URL을 가져오려면 `generateFormURL` API를 호출합니다. 그런 다음 runtimeMap을 다음 키 또는 값으로 채웁니다.
...
이 프로세스는 `taskContext`에서 검색된 `inputDocument`로 `outputDocument`을 설정합니다.
...
이 프로세스는 여러 플랫폼에서 XDP 양식을 매끄럽게 렌더링합니다. 프로세스는 `taskContext`에서 사용자 에이전트를 검색하고 데이터를 사용하여 프로세스를 호출하여 HTML 또는 PDF를 렌더링합니다.
...
![gen_question_b_20](assets/gen_question_b_20.png) **어떤 작업 프로필을 기본적으로 사용할 수 있습니까?**

XDP Forms:

* 기본값(새로운 &#39;기본 렌더링/제출&#39; 프로세스를 사용하여 렌더링/제출)

![gen_question_b_20](assets/gen_question_b_20.png) **양식 렌더링이 장치의 HTML과 데스크탑의 PDF로 이루어지도록 하려면 프로세스 디자이너가 해야 할 작업은 무엇입니까?**
...
![gen_question_b_20](assets/gen_question_b_20.png) **는 기본 작업 프로필 동작을 변경하는 데 업그레이드에 영향을 줍니까?**
