...
1. 저장된 프로세스/태스크 검색 질의를 보거나 질의를 생성하려면 **임시 쿼리** 그런 다음 을 클릭합니다. **프로세스/작업 검색**.

   ![search_node](assets/search_nodes.png)

   다음 **내 필터** [트리 보기] 오른쪽에 패널이 표시됩니다.

   다음에서 **내 필터** 패널에서 새 임시 쿼리를 만들고 을 클릭하여 이전에 저장한 쿼리를 실행할 수 있습니다.
...
1. 쿼리를 만들려면 **추가** (+).
   ...
   **필터를 정의하려면**
   ...
   1. 쿼리에 다른 필터를 추가하려면 **추가(+)** 필터 행의 오른쪽에 있습니다.

      쿼리에서 필터를 제거하려면 **삭제 (-)** 필터 행의 오른쪽에 있습니다.

      ![filter_add_del](assets/filter_add_del.png)

쿼리를 만든 후 오른쪽 위의 옵션을 사용합니다. **필터 만들기** 패널 위치:

* **취소**: 변경 사항을 취소하고 다음으로 돌아가기 **내 필터** 패널.
* **실행**: 현재 쿼리를 실행하여 결과를 확인 및/또는 확인합니다. 이 경우 쿼리를 실행하기 전에 쿼리를 저장할 필요가 없습니다. 결과를 확인하고 필요한 경우 변경한 다음, 출력이 만족스러우면 쿼리를 저장할 수 있습니다.
* **저장**: 필터를 저장합니다. 그런 다음 필터를 다음에서 보고 실행할 수 있습니다. **내 필터** 패널.
...
1. 쿼리를 실행하려면 **내 필터** 패널 또는 클릭 **실행** 버튼을 클릭하여 필터를 만들거나 편집합니다.
1. 쿼리 결과는 **보고서** 패널 **프로세스 보고** 창.
   ...
   다음에서 **표시** 드롭다운 목록에서 페이지당 표시할 결과 수를 선택합니다.

   다음에서 **페이지** 텍스트 상자에 해당 페이지로 직접 이동할 페이지 번호를 입력합니다.

1. 프로세스 검색 결과에는 다음 필드가 표시됩니다.

   * **프로세스 ID**: 프로세스 ID입니다. 필드가 하이퍼링크됩니다. 이 필드에서 프로세스 ID를 클릭하면 로 리디렉션됩니다. **[!UICONTROL 프로세스 세부 정보]** 패널입니다.
   * **개시자**: 프로세스 인스턴스를 시작한 AEM Forms 사용자
   * **생성 시간**: 프로세스 인스턴스가 시작된 날짜 및 시간입니다
   ...
   * **작업 ID**: 작업의 ID입니다. 필드가 하이퍼링크됩니다. 이 필드에서 작업 ID를 클릭하면 로 리디렉션됩니다. **[!UICONTROL 작업 세부 정보]** 패널 을 참조하십시오.
   * **개시자**: 프로세스 인스턴스를 시작한 AEM Forms 사용자
   * **생성 시간**: 프로세스 인스턴스가 시작된 날짜 및 시간입니다
   * **완료 시간**: 프로세스 인스턴스가 완료된 날짜 및 시간입니다
   * **기간**: 프로세스 인스턴스의 시작부터 완료까지의 기간
...
1. 레일 단추 클릭 ![lc_pr_rail_button](assets/lc_pr_rail_button.png) 을(를) 축소하려면 왼쪽 상단에 **내 필터** 창에서 사용 가능한 공간을 확장합니다. **보고서** 패널.
...
다음을 사용합니다. **프로세스 세부 정보** 특정 프로세스의 세부 정보를 볼 수 있는 패널

마찬가지로 **작업 세부 정보** 특정 작업의 세부 정보를 보는 패널입니다.
...
* **프로세스/작업 검색 결과에서**
* **프로세스/작업 세부 정보 패널에 프로세스/작업 ID 입력**

#### 프로세스/작업 검색 결과에서 {#from-a-process-task-search-result}

1. 프로세스/작업 검색을 실행합니다. 자세한 내용은 [프로세스 검색 질의를 실행하려면](#to-execute-a-search-query).
...
1. 목록에서 프로세스 ID를 클릭하여 **프로세스 세부 정보** 패널.
   ...
   다음 **프로세스/작업 세부 정보** 쿼리 결과에 프로세스/작업에 포함된 작업/양식의 세부 정보가 표시됩니다.
   ...
   **오른쪽 하단 패널:** 선택한 프로세스의 프로세스 내역에 대한 다음 세부 정보를 표시합니다.
   ...
   **오른쪽 하단 패널:** 선택한 작업의 프로세스 내역에 대한 다음 세부 정보를 표시합니다.
...
1. 클릭 **프로세스/작업 검색으로 돌아가기** 프로세스/작업 세부 사항이 드릴다운된 검색 결과로 돌아갑니다.
   ...
   그러나 특정 프로세스/작업 ID를 입력하여 프로세스/작업 세부 정보를 찾은 경우 프로세스/작업 검색으로 돌아가기 를 클릭하면 **프로세스/작업 검색**&#x200B;검색 결과를 표시하지 않습니다.

#### 프로세스/작업 세부 정보 패널에 프로세스/작업 ID 입력 {#by-entering-the-process-task-id-in-the-process-task-details-panel-br}

1. 로 이동 **프로세스/작업 세부 정보** 패널.
...
1. 프로세스/작업 ID 텍스트 상자에 프로세스/작업 ID를 입력합니다.
   ...
   의 필드 **프로세스/작업 세부 정보** 쿼리 결과는 AEM Forms 프로세스/작업과 관련된 필드입니다.
