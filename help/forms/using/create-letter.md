...
<table> 
 <tbody> 
  ...
  <tr> 
   ...
   <td>로고 및 서명 이미지와 같은 이미지. 기업 로고와 같은 이미지는 서신의 대부분 또는 전체에 나타납니다. 서명 이미지는 서신과 서신을 대신하여 발송되는 사람에게 해당됩니다.</td> 
...
1. 레이아웃을 탭하고 탭하여 선택합니다. **다음**.

   ![편지를 만들려면 레이아웃을 선택하십시오.](assets/selectlayout.png)

1. 서신에 대한 속성을 입력하고 을 누릅니다 **저장:**
...
1. &quot;편지가 성공적으로 생성되었습니다.&quot;라는 메시지가 표시됩니다. (경고 메시지에서) 탭 **열기** 를 사용하여 데이터 모듈 및 레이아웃 조각을 구성합니다. 또는 탭 **완료** 이전 페이지로 돌아갑니다.

   ![경고 메시지: 편지가 성공적으로 생성됨](assets/createcorrespondencecreated.png)

   **다음**: 를 누를 때 **열기**, 서신 관리에서는 레이아웃(XDP)의 모든 구성 요소가 나열된 레이아웃 표현을 표시합니다. 을(를) 삽입하십시오. [데이터 모듈 및 레이아웃 단편 및 구성](/help/forms/using/create-letter.md#p-insert-data-modules-and-layout-fragments-in-a-letter-and-configure-them-p).

### 편지에 데이터 모듈 및 레이아웃 단편 삽입 및 구성 {#insert-data-modules-and-layout-fragments-in-a-letter-and-configure-them}

서신을 만든 후 열기를 탭하면 레이아웃(XDP)의 모든 하위 양식/대상 영역이 나열된 레이아웃 표현이 서신 관리에 표시됩니다. 각 대상 영역에서 데이터 모듈 또는 레이아웃 조각을 삽입하도록 선택할 수 있습니다(그런 다음 레이아웃 조각에 데이터 모듈 삽입).

>[!NOTE]
>
>편지 페이지에서 편지에 대한 편집 아이콘을 탭하여 편지에 데이터 모듈 및 레이아웃 조각을 삽입하고 구성하도록 선택할 수도 있습니다.

1. 누르기 **삽입** 각 하위 양식에 대해 각 하위 양식에 삽입할 데이터 모듈 또는 레이아웃 단편 을 선택합니다.
...
1. 모듈을 편집하려면 모듈 옆에 있는 편집 아이콘을 탭합니다. 모듈을 편집한 후 을 누릅니다. **저장**.

   이 페이지에서는 하위 양식에 대해 다음 작업을 수행할 수도 있습니다.

   1. **자유 텍스트 허용**: 자유 텍스트 허용 이 활성화되면 사용자는 CCR 보기에서 편지에 인라인 텍스트를 추가할 수 있습니다. CCR 보기에서 자유 텍스트 허용이 활성화된 대상 영역에 대해 &#39;T&#39; 작업이 활성화되어 있으며, 사용자가 이 작업을 탭하면 텍스트 이름과 설명을 요청한 다음 확인을 탭하면 사용자가 텍스트를 추가할 수 있는 편집 모드에서 해당 텍스트가 열립니다. 따라서 다른 텍스트 모듈과 유사하게 작동합니다
...
1. 연결을 지정한 후 을 누릅니다 **다음**. 서신 관리는 첨부 화면을 표시합니다.
...
1. 누르기 **저장**. 편지가 만들어지고 편지 페이지에 나열됩니다.
...
1. 붙여넣기를 누릅니다. 붙여넣기 대화 상자가 나타납니다. 동일한 위치에 문자를 복사하고 붙여넣는 경우 새 편지 사본에 이름과 제목을 자동으로 지정하지만 글자의 제목과 이름을 편집할 수 있습니다.
1. 필요한 경우 편지 사본을 저장할 제목 및 이름 을 편집합니다.
1. 붙여넣기를 누릅니다. 편지의 사본이 작성됩니다. 이제 새로 만든 편지에서 필요한 사항을 변경할 수 있습니다.
