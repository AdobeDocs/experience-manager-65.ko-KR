...
차트 또는 그래프는 데이터를 시각적으로 표현한 것입니다. 많은 양의 정보를 이해하기 쉽고 시각적인 형식으로 압축할 수 있으므로 복잡한 데이터를 보다 효과적으로 시각화, 해석 및 분석할 수 있습니다.
AEM Forms 추가 기능 패키지는 기본 차트 구성 요소를 제공합니다. 적응형 양식 및 문서에서 반복 가능한 패널 및 테이블에서 2D 데이터의 시각적 표현을 사용할 수 있습니다. 차트 구성 요소를 사용하여 다음 유형의 차트를 추가하고 구성할 수 있습니다.
...
차트를 구성하려면 차트 구성 요소를 마우스 오른쪽 단추로 클릭하고 편집을 클릭하여 차트 편집 대화 상자를 엽니다. 이 대화 상자에는 차트를 구성할 수 있는 제목 및 텍스트, 구성, 고급 옵션 및 스타일 지정 탭이 포함되어 있습니다.
...
* **데이터 소스에 대해 행 이름 반복**: 데이터를 소싱할 테이블 행 또는 반복 가능 패널의 요소 이름을 지정합니다. 예제에서 statementDetails는 Statement Details 테이블에서 반복 가능한 행의 요소 이름입니다.
...
이 예에 사용된 신용 카드 명령문은 적응형 문서이고 명령문 세부 사항 섹션은 작성 모드에서 다음과 같이 표시되는 테이블입니다.
...
* 행의 요소 이름은 statementDetails입니다. 표 행 속성에서 구성할 수 있습니다.
...
차트에서 플로팅할 소스 데이터의 값을 계산하기 위해 통계 함수를 사용하도록 차트를 구성할 수 있습니다. 차트 구성 요소에는 몇 가지 내장 함수가 함께 제공되지만, 고유한 함수를 작성하여 차트 구성에서 사용할 수 있도록 할 수 있습니다.
...
차트에서 기본 함수를 사용할 수 있을 뿐만 아니라 JavaScript로 사용자 지정 함수를 작성하여 차트 구성 요소의 함수 목록에서 사용할 수 있도록 할 수 있습니다.
