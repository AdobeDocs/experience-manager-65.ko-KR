...
**파일 패턴:** 공급자가 허용하는 들어오는 첨부 파일 패턴입니다. 여기에는 특정 확장명(&amp;ast;.dat, &amp;ast;.xml), 특정 이름(데이터) 및 이름과 확장명(.``[dD][aA]``&#39;port&#39;)에 복합 표현식이 있는 파일이 포함됩니다. 기본값은 &amp;ast;.&amp;ast;입니다.
...
**파일 패턴:** 공급자가 허용하는 들어오는 첨부 파일 패턴을 지정합니다. 여기에는 특정 확장명(&amp;ast;.dat, &amp;ast;.xml), 특정 이름(데이터) 또는 이름과 확장명(&amp;ast;.`[dD][aA]`&#39;port&#39;)에 복합 표현식이 있는 파일이 포함됩니다.
