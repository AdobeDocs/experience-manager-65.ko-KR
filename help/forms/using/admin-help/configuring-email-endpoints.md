...
**파일 패턴:** 공급자가 허용하는 들어오는 파일 첨부 패턴입니다. 여기에는 특정 확장명(&amp;ast;.dat, &amp;ast;.xml)과 특정 이름(데이터) 및 이름과 확장명(.)의 복합 표현식이 있는 파일이 포함됩니다.[dD][aA]&#39;port&#39;) 기본값은 &amp;ast;입니다.ast;(&amp;A)
...
**파일 패턴:** 공급자가 허용하는 들어오는 파일 첨부 패턴을 지정합니다. 여기에는 이름 및 확장(&amp;ast;.dat, &amp;ast;.xml)에 특정 확장명(&amp;ast;.dat), 특정 이름(데이터) 또는 복합 표현식이 있는 파일이 포함됩니다.[dD][aA]&#39;port&#39;)
