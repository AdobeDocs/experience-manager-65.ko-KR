...
1. (Linux 및 Solaris) 명령 프롬프트에서 `pwconv`(매개 변수 없이)를 입력하여 섀도 암호 파일을 생성합니다.
   ...
   >[!NOTE]
   >
   >(Linux 및 Solaris) WebSphere Application Server 로컬 OS 보안 레지스트리가 작동하려면 섀도 암호 파일이 있어야 합니다. 섀도 암호 파일의 이름은 보통 **/etc/shadow**&#x200B;로 지정되며 /etc/passwd 파일을 기반으로 합니다. 섀도 암호 파일이 없으면 전역 보안을 설정하고 사용자 레지스트리를 로컬 OS로 구성한 후 오류가 발생합니다.
