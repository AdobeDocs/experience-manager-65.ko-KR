...
**PDF 변경:** 사용자는 페이지를 추가 및 제거하고 페이지에서 콘텐츠를 편집할 수 있습니다.

**Fill &amp; Sign:** 사용자는 PDF에서 양식 필드를 채우고 서명할 수 있습니다.

**복사:** 사용자가 이 정책으로 보호된 문서의 텍스트를 복사할 수 있도록 허용합니다.

**화면 Reader:** PDF에 대한 사용자 지정 사용 권한 표시 확인란을 선택하면 이 사용 권한이 표시됩니다. 이 옵션을 선택하면 Adobe AcrobatPDF 에 임시 태그를 추가하여 화면 판독기로 가독성을 높일 수 있는 권한이 있습니다.
...
**PDF에 인증된 Acrobat 플러그인만 사용:** 정책에 대해 이 옵션을 선택하면 정책으로 보호된 문서를 열 때 Acrobat 8.0 이상이 인증된 모드로 실행되도록 지정됩니다. Acrobat이 인증 모드에서 실행되면 서드파티 플러그인이 로드되지 않습니다.
...
* 첨부 파일 및 메타데이터를 포함한 전체 문서. *메타데이터*&#x200B;은(는) 문서 속성 대화 상자나 Acrobat 고급 메뉴를 통해 볼 수 있는 문서 및 문서에 대한 정보입니다. Acrobat에서 텍스트, 오디오 및 비디오 파일과 같이 서로 다른 유형의 파일을 PDF 문서에 첨부할 수 있습니다.
