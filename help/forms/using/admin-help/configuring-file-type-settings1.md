...
**모든 텍스트를 윤곽선으로 변환** 모든 문자 객체(점 유형, 영역 유형 및 경로 유형)를 투명도가 포함된 페이지의 모든 문자 모양 정보를 윤곽선으로 변환하고 버립니다. 이 옵션을 사용하면 병합하는 동안 텍스트 너비가 일관되게 유지됩니다. 이 옵션을 활성화하면 Acrobat에서 보거나 저해상도 데스크탑 프린터에서 인쇄할 때 작은 글꼴이 약간 더 두껍게 표시됩니다. 고해상도 프린터 또는 이미지세터에 인쇄된 유형의 품질에는 영향을 주지 않습니다.
