...
* 기본적으로 사용할 파일 형식 설정을 지정합니다([PDF Generator 구성 파일 가져오기 및 내보내기](https://helpx.adobe.com/aem-forms/6-2/admin-help/importing-exporting-pdf-generator-configuration.html) 참조).
...
* [파일 형식 설정 삭제](https://helpx.adobe.com/aem-forms/6-2/admin-help/enable-pdf-a-support.html)
...
**PDF 출력 스타일:** 전경에 있는 페이지의 비트맵 이미지와 아래에 보이지 않는 레이어에 있는 스캔한 텍스트를 포함하려면 검색 가능한 이미지를 선택하십시오. 페이지의 모양은 변경되지 않지만 텍스트는 선택 가능하고 읽기 가능해집니다. [서식이 지정된 텍스트 및 그래픽]을 선택하여 인식된 텍스트, 글꼴, 그림 및 기타 그래픽 요소를 사용하여 원래 페이지를 재구성합니다. 기본값은 검색 가능한 이미지(정확)입니다.
...
다양한 파일 유형에 대한 지원을 사용자 지정할 수 있습니다. ([AEM Forms를 사용한 프로그래밍](https://www.adobe.com/go/learn_lc_programming_11)에서 &quot;[추가 기본 파일 형식에 대한 지원 추가](https://help.adobe.com/en_US/AEMForms/6.1/ProgramLC/WS624e3cba99b79e12e69a9941333732bac8-7756.2.html)&quot;을(를) 참조하십시오.)
...
1. 관리 콘솔에서 **[!UICONTROL 서비스 > PDF Generator]**&#x200B;을 클릭합니다.
...
**PDF 출력 스타일:** 생성할 PDF 형식을 결정합니다. 모든 형식은 OCR과 글꼴 및 페이지 인식을 텍스트 이미지에 적용하여 일반 텍스트로 변환합니다.
...
**선택한 인코딩 강제 적용:** HTML 원본 파일에 지정된 인코딩을 무시하고 [기본 인코딩] 옵션에 표시된 선택 항목을 사용합니다.

### 스파이더 설정 {#spidering-settings}

*스파이더*&#x200B;은(는) 웹 페이지에서 다른 웹 페이지로 연결되는 링크를 검색합니다. 다른 웹 페이지에 대한 링크가 발생하면 대상 페이지를 가져와서 생성된 PDF 문서에 포함됩니다. 가져올 레벨 및 PDF으로 변환할 레벨 수를 설정하려면 다음 옵션을 활성화합니다.
...
**여백:** 생성된 PDF 문서에서 여백(위쪽, 아래쪽, 왼쪽, 오른쪽)을 지정합니다.

**PDF에 책갈피 추가:** PDF 문서에 책갈피를 추가합니다.

**태그가 지정된 PDF 사용:**&#x200B;이(가) PDF 문서에 태그를 포함합니다.
...
**확대/축소:** PDF 문서의 초기 보기에 대해 사전 설정된 확대/축소를 선택하거나 사용자 지정 값을 선택합니다. 기본 설정을 선택하면 기본 Acrobat 배율이 사용됨을 나타냅니다.

**페이지 번호 열기:** PDF이 열 페이지 번호를 지정합니다.
...
PDF Generator은 Adobe Flash을 위한 비디오(SWF 또는 FLV 파일)를 제출하고 Adobe Flash을 위한 비디오가 포함된 PDF 파일을 만드는 기능을 지원합니다. 이 변환에서는 Forms 서버에 Adobe Flash Player을 설치할 필요가 없습니다. 이 옵션에 액세스하는 방법에 대한 지침은 [파일 형식 설정 만들기 또는 편집](configuring-file-type-settings.md#create-or-edit-file-type-settings)을 참조하십시오.
...
1. 포함된 글꼴의 사용 중인 하위 집합만 포함하려면 **포함된 모든 글꼴 하위 집합**&#x200B;을 선택합니다.
   ...
   >[!NOTE]
   >
   >*이 옵션을&#x200B;**일부 글꼴 임베드 취소**와 함께 사용하는 경우&#x200B;**A**임베드 취소할 글꼴 목록에 있는 글꼴은 여전히 완전히 임베드 해제됩니다.*
...
* **모든 대체 이미지 버리기**: 화면에 표시할 버전을 제외한 이미지의 모든 버전을 제거합니다. 일부 PDF은 저해상도 온스크린 보기 및 고해상도 인쇄와 같은 다양한 목적을 위해 동일한 이미지의 여러 버전을 포함합니다.
...
* **숨겨진 레이어 내용을 삭제하고 표시되는 레이어를 병합합니다**: 파일 크기를 줄입니다. 최적화된 문서는 원래 PDF 모양이지만 레이어 정보를 포함하지 않습니다.
* **문서 정보 및 메타데이터 버리기**: 문서 정보 사전 및 모든 메타데이터 스트림의 정보를 제거합니다. (다른 이름으로 저장 명령을 사용하여 메타데이터 스트림을 PDF 사본으로 복원합니다.)
* **첨부 파일 버리기**: PDF에 메모로 추가된 첨부 파일을 포함하여 모든 첨부 파일을 제거합니다. (PDF 최적화 도구에서 첨부 파일을 최적화하지 않습니다.)
...
**Adobe PDF에 Source 파일 첨부**: 이 옵션을 선택하면 원래 Excel 스프레드시트가 생성된 PDF 문서 내에 첨부 파일로 삽입됩니다.

**태그 지정된 Adobe PDF으로 접근성 및 Reflow 사용**: 접근성 및 리플로우를 위해 PDF 문서 내부에 태그를 포함합니다.
...
이러한 옵션은 Microsoft PowerPoint 파일의 변환 방법을 결정합니다. 이러한 옵션에 액세스하는 방법에 대한 지침은 [파일 형식 설정 만들기 또는 편집](/help/forms/using/admin-help/configuring-file-type-settings1.md#create-or-edit-file-type-settings)을 참조하십시오.

**[!UICONTROL OpenOffice를 대체 변환기로 사용]**: 이 옵션이 선택되어 있고 Microsoft PowerPoint를 사용한 변환이 실패하거나 지정된 시간 초과 제한에 도달하면 PDF Generator은 OpenOffice를 사용하여 변환을 시도합니다. OpenOffice를 사용한 변환이 실패하거나 지정된 시간 초과 제한에 도달하면 로그 파일에 예외가 기록됩니다.
...
**[!UICONTROL Adobe PDF에 Source 파일 첨부]**: 원본 파일을 PDF 파일에 첨부 파일로 추가합니다. 이 옵션은 기본적으로 선택 해제되어 있습니다.

**[!UICONTROL 태그가 지정된 Adobe PDF으로 접근성 및 Reflow 사용]**: 태그를 PDF 파일에 임베드합니다. 이 옵션은 기본적으로 선택 해제되어 있습니다.
...
1. **[!UICONTROL Adobe PDF에 Source 파일 첨부]**: 원본 파일을 PDF 파일에 첨부 파일로 추가합니다.
...
**[!UICONTROL OpenOffice를 대체 변환기로 사용]**: 이 옵션을 선택하고 Microsoft Word를 사용한 변환이 실패하거나 지정된 시간 초과 제한에 도달하면 PDF Generator이 OpenOffice를 사용하여 변환을 시도합니다. OpenOffice를 사용한 변환이 실패하거나 지정된 시간 초과 제한에 도달하면 로그 파일에 예외가 기록됩니다.
...
**[!UICONTROL Adobe PDF에 Source 파일 첨부]**: 원본 파일을 PDF 파일에 첨부 파일로 추가합니다.

**[!UICONTROL 상호 참조와 목차를 링크로 변환]**: 모든 상호 참조와 목차 항목을 링크로 변환합니다. 이 옵션은 기본적으로 선택되어 있습니다.

**[!UICONTROL 태그가 지정된 Adobe PDF으로 접근성 및 Reflow 사용]**: 태그를 PDF 파일에 임베드합니다. 이 옵션은 기본적으로 선택되어 있습니다.
...
**Adobe PDF에 Source 파일 첨부**: 원본 파일을 PDF 파일에 첨부 파일로 추가합니다.
...
이러한 옵션은 AutoCAD 파일의 변환 방법을 결정합니다. 이러한 옵션에 액세스하는 방법에 대한 지침은 [파일 형식 설정 만들기 또는 편집](/help/forms/using/admin-help/configuring-file-type-settings1.md#create-or-edit-file-type-settings)을 참조하십시오.
...
**[!UICONTROL PDF/E-1 호환 파일 만들기]**: PDF/E-1 호환 파일을 만듭니다. PDF/E는 엔지니어링 및 기술 문서 교환을 위한 ISO 표준입니다. PDF/E-1에 대한 자세한 내용은 [Adobe 및 업계 표준](https://www.adobe.com/enterprise/standards/index.html)을 참조하세요.
...
이러한 옵션은 OpenOffice 파일의 변환 방법을 결정합니다. 이러한 옵션에 액세스하는 방법에 대한 지침은 [파일 형식 설정 만들기 또는 편집](/help/forms/using/admin-help/configuring-file-type-settings1.md#create-or-edit-file-type-settings)을 참조하십시오.
...
**페이지 방향**: 일반 텍스트 파일만 변환되는 PDF 문서에 대해 세로 또는 가로 중 하나를 선택하십시오.
...
**일반**: 태그가 지정된 PDF을 변환하거나 작성기 및 FormCalc 문서 메모, 슬라이드 전환 효과 또는 빈 페이지를 PDF으로 내보내는 옵션을 사용할 수 있습니다. 태그를 내보낼 때 파일 크기가 많이 증가할 수 있습니다. 내보내는 태그 중에는 목차, 하이퍼링크 및 컨트롤이 있습니다.
...
**태그가 지정된 PDF**: OpenOffice 문서에서 태그가 지정된 PDF을 만들 수 있습니다. 태그가 지정된 PDF은 문서 내용 구조에 대한 정보를 포함합니다. 이렇게 하면 다른 화면이 있는 장치에 문서를 표시할 때와 화면 판독기 소프트웨어를 사용할 때 도움이 될 수 있습니다. 또한 접근성 소프트웨어가 PDF 문서의 내용을 소리내어 읽는 것과 같이 PDF 문서와 함께 다양한 유용한 작업을 수행하는 데 도움이 됩니다.
