...
PDFG 네트워크 프린터를 사용하면 인쇄를 지원하는 모든 애플리케이션에서 PDF 문서를 생성할 수 있습니다. 사용자가 PDFG 네트워크 프린터를 설치한 후 Windows Campaign 컨트롤 패널의 프린터 섹션에 *PDF 생성기*&#x200B;라는 새 프린터가 나타납니다. 이름이 같은 프린터가 이미 있는 경우 다른 이름을 입력하라는 메시지가 표시됩니다.
...
1. (PDFG 관리자) 관리 콘솔에서 서비스 > PDF 생성기 > PDFG 네트워크 프린터를 클릭합니다.
   ...
   (PDFG 사용자) `http(s)://[host]:'port'/pdfgui`으로 이동하고 PDFG 네트워크 프린터 설치 아래의 링크를 클릭합니다.
