...
## HSM 디바이스가 온라인 인 경우 HSM 자격 증명의 별칭을 생성합니다. {#create-an-alias-for-an-hsm-credential-when-the-hsm-device-is-online}
...
1. 토큰 이름, 슬롯 ID 또는 슬롯 목록 색인을 사용하여 자격 증명이 HSM에 저장된 위치를 식별합니다.
   ...
   * **슬롯 ID:** 슬롯 ID는 데이터 형식 길이가 긴 유형의 슬롯 식별자입니다.
   * **슬롯 목록 색인:** 슬롯 목록 색인을 선택하는 경우 슬롯 정보를 슬롯에 해당하는 정수로 설정합니다. 이것은 클라이언트가 HSMPART1 파티션에 먼저 등록되어 있는 경우 HSMPART1이 SlotListIndex 값 0을 사용하는 것으로 참조된다는 것을 의미합니다.
...
양식 서버와 HSM 디바이스 간의 네트워크 세션이 중단된 후 HSM 디바이스에 대한 오픈 연결을 재설정합니다. 예를 들어, 네트워크 중단이나 소프트웨어 업데이트를 위해 HSM 디바이스가 오프라인으로 전환되는 경우 문제가 발생할 수 있습니다. 서비스 중단 후 기존 연결은 오래되고 이러한 연결에 대한 모든 서명 요청이 실패합니다. 모든 HSM 연결 재설정 옵션을 사용하여 이전 연결을 지웁니다.
...
AEM 양식은 웹 서비스 기반 IPC/RPC 메커니즘을 사용합니다. 이 메커니즘을 통해 AEM 양식은 원격 컴퓨터에 설치된 HSM을 사용할 수 있습니다. 이 기능을 사용하려면 HSM이 설치된 원격 컴퓨터에 웹 서비스를 설치합니다. 자세한 내용은 Windows 64비트 플랫폼[에서 Sun JDK를 사용하여 AEM forms ES에 대한 HSM 지원 구성을 참조하십시오.](https://kb2.adobe.com/cps/808/cpsid_80835.html)
