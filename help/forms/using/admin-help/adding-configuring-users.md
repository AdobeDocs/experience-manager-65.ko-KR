...
MySQL을 사용하는 경우 사용자 ID에 확장 문자가 포함될 수 있습니다. 그러나 abcde와 &quot;봉&quot;과 &quot;백&quot;과 같은 두 문자열 사이에 비교가 될 때는 같은 것으로 간주됩니다. 예를 들어 동기화 시 새 사용자가 데이터베이스에 추가된 경우 동일한 사용자 ID를 가진 사용자가 데이터베이스에 있는지 여부를 확인하기 위해 비교가 수행됩니다. 새 사용자 *를 추가할* 때 사용자 *가 이미 데이터베이스에* 존재하는 경우 비교 시 두 이름을 구별할 수 없습니다. 사용자가 데이터베이스에 이미 있으며 새 사용자는 무시되고 추가되지 않는다고 가정합니다.
