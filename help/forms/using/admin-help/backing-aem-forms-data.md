...
description: 이 문서에서는 Adobe Experience Manager(AEM) Forms 데이터베이스, GDS 및 콘텐츠 스토리지 루트 디렉터리의 핫 백업 또는 온라인 백업을 완료하는 데 필요한 단계를 설명합니다.
contentOwner: admin
...
* 롤링 모드 백업 또는 스냅샷 모드 백업을 사용하는 경우 데이터베이스를 올바른 로그 설정으로 구성하여 데이터베이스의 핫 백업을 허용해야 합니다. ([AEM Forms 데이터베이스](/help/forms/using/admin-help/files-back-recover.md#aem-forms-database)를 참조하십시오.)
...
관리 콘솔, LCBackupMode 명령 또는 AEM Forms 설치 시 함께 제공되는 API를 사용하여 백업 모드를 시작하거나 종료할 수 있습니다. 롤링 백업(연속 적용)의 경우 관리 콘솔 옵션을 사용할 수 없습니다. 명령줄 옵션이나 API를 사용해야 합니다. <!-- Fix broken link For information about using the API to enter and leave backup modes, see AEM Forms API Reference on Help and Tutorials page. -->
...
1. 안전 백업 모드에서 작동을 선택하고 확인을 클릭합니다.
...
1. 명령 프롬프트에서 다음 명령을 한 줄로 실행합니다.
   ...
   * (Linux®, UNIX®) `LCBackupMode.sh enter [-host=`*hostname* `] [-port=`*portnumber* `] [-user=`*username* `] [-password=`*password* `] [-label=`*labelname* `]`
   ...
   `Host`는 AEM Forms가 실행되는 호스트 이름입니다.

   `port`는 AEM Forms가 실행되는 애플리케이션 서버의 WebServices 포트입니다.

   `user`는 AEM Forms 관리자의 사용자 이름입니다.

   `password`는 AEM Forms 관리자의 암호입니다.

   `label`은 이 백업에 대한 텍스트 레이블이며 모든 문자열일 수 있습니다.
...
1. 관리 콘솔에 로그인합니다.
1. 설정 > 핵심 시스템 설정 > 백업 유틸리티를 클릭합니다.
1. 안전 백업 모드에서 작동을 선택 취소하고 확인을 클릭합니다.
...
1. 운영 체제에 따라 `LCBackupMode.cmd` 또는 `LCBackupMode.sh` 스크립트를 편집하여 시스템에 적합한 기본값을 제공합니다.
   ...
   >[!NOTE]
   >
   >[AEM Forms 설치 준비](https://www.adobe.com/go/learn_aemforms_prepareInstallsingle_63)에서 사용 중인 애플리케이션 서버에 해당하는 장에 설명된 대로 JAVA_HOME 디렉터리를 설정하십시오&#x200B;*.*
...
1. 다음 명령을 한 줄로 실행합니다.
   ...
   * (Linux®, UNIX®) `LCBackupMode.sh leaveContinuousCoverage [-Host=`*hostname* `] [-port=`*portnumber* `] [-user=`*username* `] [-password=`*password* `]`
     ...
     `port`는 AEM Forms가 애플리케이션 서버에서 실행되는 포트입니다.
