...
1. 노드에서 `ServerSettings` 다음 항목을 추가한 다음 파일을 저장합니다.
   ...
   여기서 *time은* 자동 백그라운드 동기화 사이의 초 수입니다. 이 값을 다음으로 보낸 경우 동기화 `0`가 항상 발생합니다. 기본값은 `14400` 초(4시간마다)입니다.
...
1. 노드에서 `ServerSettings` 다음 항목을 추가한 다음 파일을 저장합니다. `<entry key="maximumSizeOfWatermarkElement" value="max filesize in KB"/> <entry key="maximumWatermarkElementsPerWatermark" value="max elements"/>`
   ...
   첫 번째 항목인 *최대 파일 크기* (KB)는 PDF 워터마크 요소에 대해 허용되는 최대 파일 크기입니다. 기본값은 100KB입니다.
