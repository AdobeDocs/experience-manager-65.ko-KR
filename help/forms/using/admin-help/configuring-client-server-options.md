...
**클라이언트 세션 시간 초과:** 클라이언트 응용 프로그램을 통해 로그인한 사용자가 문서 보안과 상호 작용하지 않을 경우 문서 보안 연결이 끊어지는 시간(분)입니다.
...
* 워터마크 요소로 사용할 최대 PDF 파일 크기를 변경할 수 있습니다. 그러나 워터마크로 사용되는 큰 PDF 문서는 이러한 워터마크가 적용된 문서를 오프라인으로 동기화하는 동안 성능이 저하됩니다. [워터마크 구성 매개 변수 변경](configuring-client-server-options.md#change-the-watermark-configuration-parameters)을 참조하십시오.
...
1. 리치 워터마크 옵션을 위해 PDF을 선택한 경우 워터마크 선택 PDF 옆의 **찾아보기**&#x200B;를 클릭하여 워터마크로 사용할 PDF 문서를 선택합니다.
...
**메시지:** 메시지 본문에 나타나는 텍스트에는 받는 사람의 외부 사용자 암호가 다시 설정되었음을 나타내는 메시지가 표시됩니다.
...
1. `ServerSettings` 노드에서 다음 항목을 추가한 다음 파일을 저장합니다.
   ...
   여기서 *time*&#x200B;은(는) 자동 백그라운드 동기화 사이의 시간(초)입니다. 이 값을 `0`(으)로 보낸 경우 항상 동기화가 발생합니다. 기본값은 `14400`초(4시간마다)입니다.
...
* **통합자 버전:** 8.2
* Microsoft Office용 **응용 프로그램 제품군:** Acrobat Reader DC 확장
* **응용 프로그램 이름:** Microsoft Word, Microsoft Excel 또는 Microsoft PowerPoint
* **응용 프로그램 버전:** 2003 또는 2007

**특정 응용 프로그램에 대한 서비스를 거부하도록 문서 보안을 구성합니다**
...
1. `ServerSettings` 노드에서 다음 항목을 추가한 다음 파일을 저장합니다. `<entry key="maximumSizeOfWatermarkElement" value="max filesize in KB"/> <entry key="maximumWatermarkElementsPerWatermark" value="max elements"/>`
   ...
   첫 번째 항목 *최대 파일 크기*&#x200B;은(는) PDF 워터마크 요소에 허용되는 최대 파일 크기(KB)입니다. 기본값은 100KB입니다.

   두 번째 항목 *max 요소*&#x200B;은(는) 워터마크에서 허용되는 최대 요소 수입니다. 기본값은 5입니다.
...
대부분의 Document Security 사용자는 올바른 관리 사용자 인터페이스를 사용하는 동안 **www.adobe.com**&#x200B;과(와) 같은 외부 링크에 액세스할 수 없습니다.
...
1. `ExternalUser` 노드의 `SmtpUseTls` 키 값을 **true**(으)로 설정합니다.
1. `ExternalUser` 노드의 `SmtpUseSsl` 키 값을 **false**(으)로 설정합니다.
1. `config.xml` 저장.
1. 구성 파일을 가져옵니다. ([문서 보안 구성 파일을 수동으로 편집](configuring-client-server-options.md#manually-editing-the-document-security-configuration-file)을(를) 참조하십시오.)

### Document Security 문서에 대한 SOAP 끝점 비활성화 {#disable-soap-endpoints-for-document-security-documents}

문서 보안 문서의 SOAP 끝점을 비활성화하기 위해 config.xml이 다음과 같이 변경되었습니다.
...
1. 문서 보안 문서에 대한 SOAP 끝점을 비활성화하려면 값 특성을 **false**(으)로 설정하십시오.
...
1. 문서 보안 구성 파일을 내보냅니다. ([문서 보안 구성 파일을 수동으로 편집](configuring-client-server-options.md#manually-editing-the-document-security-configuration-file)을(를) 참조하십시오.)
1. 편집기에서 구성 파일을 열고 ServerSettings 노드를 찾습니다.
1. ServerSettings 노드에서 `DisableGlobalOfflineSynchronizationData` 속성의 값을 `true`(으)로 설정합니다.
   ...
   >[!NOTE]
   >
   >기본적으로 `DisableGlobalOfflineSynchronizationData`키 값은 `false`(으)로 설정됩니다.

1. 구성 파일을 저장하고 가져옵니다. ([문서 보안 구성 파일을 수동으로 편집](/help/forms/using/admin-help/configuring-client-server-options.md#manually-editing-the-document-security-configuration-file)을(를) 참조하십시오.)
