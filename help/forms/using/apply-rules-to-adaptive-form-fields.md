...
1. 드래그 앤 드롭 **[!UICONTROL 배송 주소, 상태 및 우편 번호]** 필드의 필드 [!UICONTROL 양식 개체] 탭에서 해당 tablename.property(예: customerdetails.shippingAddress)로 이동합니다. **[!UICONTROL 개체를 끌어 놓거나 여기를 선택하십시오.]** 의 필드 **[!UICONTROL 입력]** 상자. 테이블 이름(예: 이 사용 사례의 customerdetails) 접두사가 있는 모든 필드는 업데이트 서비스에 대한 입력 데이터로 사용됩니다. 이러한 필드에 제공된 모든 컨텐츠는 데이터 소스에서 업데이트됩니다.
...
1. 드래그 앤 드롭 **[!UICONTROL 고객 ID]** 필드의 필드 [!UICONTROL 양식 개체] 탭의 **[!UICONTROL 입력]** 상자. 접두사가 없는 필드(예: 이 사용 사례의 customerdetails)는 업데이트 서비스의 검색 매개 변수로 사용됩니다. 다음 **[!UICONTROL id]** 이 사용 사례의 필드는  **customerdetails**  테이블.
...
다음 절차에서는 양식에 입력한 고객 ID가 데이터베이스에 없는 경우 오류 메시지를 표시하는 규칙을 만드는 방법을 보여 줍니다. 또한 이 규칙은 포커스를 로 가져다가 재설정합니다. **[!UICONTROL 고객 ID]** 필드. 규칙은 을 사용합니다 [양식 데이터 모델 서비스의 dataIntegrationUtils API](/help/forms/using/invoke-form-data-model-services.md) 고객 ID가 데이터베이스에 있는지 확인합니다.

1. 탭하기 **[!UICONTROL 고객 ID]** 필드 및 탭 `Edit Rules` 아이콘. 다음 [!UICONTROL 규칙 편집기] 창이 열립니다.
1. 탭하기 **[!UICONTROL + 만들기]** 아이콘을 클릭하여 규칙을 추가합니다. 시각적 편집기가 열립니다.
...
1. 바꾸기 `guidelib.dataIntegrationUtils.executeOperation (operationInfo, inputs, outputs)` 섹션을 참조하십시오.
