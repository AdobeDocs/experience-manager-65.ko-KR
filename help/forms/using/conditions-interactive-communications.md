...
1. 다음 정보를 지정합니다.
   ...
   * **[!UICONTROL 태그]**: 사용자 정의 태그를 생성하려면 텍스트 필드에 값을 입력하고 입력을 선택합니다(선택적). 이 조건을 저장하면 새로 추가된 태그가 만들어집니다.
...
1. 조건에 포함할 자산을 선택할 적절한 자산을 선택한 다음 선택합니다 **[!UICONTROL 완료]**.
   ...
   **[B] 변경 내용을 적용합니다.** 이 아이콘을 선택하여 해당 조건의 에셋 및 규칙에서 변경한 내용을 적용합니다.
   **[C] 중복 에셋.** 이 아이콘을 선택하여 조건에 적용된 규칙(있는 경우)과 함께 에셋의 사본을 만듭니다. 그런 다음 복제된 자산에 대한 규칙 및 자산 편집을 계속할 수 있습니다. 에셋을 복제하면 유사한 규칙을 만들어 특정 컨텍스트를 기반으로 대체 에셋을 표시하는 데 유용합니다.
   **[D] 미리 보기를 표시합니다.** 이 아이콘을 선택하면 Create\Edit Condition 페이지 내에 에셋 미리보기가 표시됩니다.
...
1. 선택 **[!UICONTROL 규칙 만들기]** 필요에 따라 에셋을 표시하거나 숨길 규칙을 추가합니다. 규칙에서 변수를 사용하려면 다음을 참조하십시오. [변수 만들기](#variables). 자세한 내용은 [조건에 규칙 추가](#ruleeditor).
...
1. 조건을 만들거나 편집할 때 다음을 선택합니다. ![ruleeditoricon](assets/ruleeditoricon.png) (규칙 편집기) 관련 에셋의 아이콘입니다.
...
1. 평가 표현식, 문자열, 데이터 모델 속성, 변수 또는 날짜를 삽입합니다.
   ...
   * 규칙을 만들거나 편집하는 동안 다음을 선택할 수도 있습니다. ![icon_resize](assets/icon_resize.png) (크기 조정) - [규칙 만들기/규칙 편집] 대화 상자를 확장합니다. 확장된 전체 창 대화 상자를 통해 다음을 만들 수 있습니다. [변수](#variables) 를 클릭하여 규칙을 만듭니다. 크기 조정 을 다시 선택하여 일반적인 규칙 만들기 대화 상자로 돌아갑니다.
...
조건에서 규칙을 만들거나 편집하는 동안 다음을 선택할 수 있습니다. ![icon_resize](assets/icon_resize.png) (크기 조정) - [규칙 만들기]\[규칙 편집] 대화 상자를 확장합니다. 전체 창 대화 상자를 확장하면 다음과 같은 작업을 수행할 수 있습니다.
...
1. 조건에서 규칙을 만들거나 편집하는 동안 다음을 선택할 수 있습니다. ![icon_resize](assets/icon_resize.png) (크기 조정) - [규칙 만들기]\[규칙 편집] 대화 상자를 확장합니다.
...
1. 유효한 규칙을 구성한 후 다음을 선택합니다. **[!UICONTROL 완료]**.
