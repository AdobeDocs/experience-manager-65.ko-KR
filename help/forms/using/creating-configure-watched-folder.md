...
1. 탭 **고급**. 고급 탭에는 더 많은 필드가 포함되어 있습니다. 이러한 필드 대부분은 기본값을 포함합니다.
   ...
   * **패턴이 있는 파일 포함:** 감시 폴더에서 검색하고 선택할 폴더와 파일을 결정하는 데 사용하는 세미콜론(;) 구분 패턴 목록을 지정합니다. 예를 들어 파일 패턴 포함 을 입력하면 입력과 일치하는 모든 파일과 폴더가 선택됩니다. 자세한 내용은 [관리 도움말](/help/forms/using/admin-help/configuring-watched-folder-endpoints.md)
