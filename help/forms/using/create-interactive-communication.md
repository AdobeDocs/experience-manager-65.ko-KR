...
1. 인쇄 채널을 선택한 상태에서 구성 요소 **[!UICONTROL 탭을]** 선택합니다. 다음 구성 요소는 인쇄 채널에서 사용할 수 있습니다.
   ...
   또한 인쇄 및 웹 채널에 대한 대화형 통신을 작성하는 동안 실행 취소 및 다시 실행 작업을 사용할 수 있습니다.

   실행 취소 작업을 사용하여 마지막으로 수행한 작업을 취소하고 다시 실행 작업을 사용하여 삭제된 작업을 다시 통합할 수 있습니다. 예를 들어, 이미지를 삽입하거나 Interactive Communication에 데이터 바인딩을 만든 경우 이를 취소해야 하는 경우 실행 취소 작업을 사용합니다.

   ![재실행 작업 실행 취소](assets/undo_redo_actions_new.png)

   실행 취소 및 재실행 옵션은 작성 UI 페이지 도구 모음에 표시됩니다. 실행 취소 옵션은 작업을 수행한 후에만 표시됩니다. 다시 실행 옵션은 실행 취소 작업을 수행한 후에만 페이지 도구 모음에 표시됩니다. 이러한 작업은 페이지를 새로 고칠 때 재설정됩니다.
...
작성 인터페이스를 사용하면 Interactive Communication 내에서 언바운드 필드와 바운드 필드 및 변수를 구별할 수 있습니다. 인터페이스는 언바운드 필드 및 변수를 주황색 테두리로 강조 표시합니다.
...
문서 조각에 사용되는 언바운드 변수는 작성 인터페이스에 표시되지 않을 수 있습니다. 문서 조각 내 또는 조건 조각의 경우 인라인 텍스트 규칙으로 인해 발생할 수 있습니다. 이 경우 파란색으로 강조 표시된 도구 설명이 문서 조각의 일부로 표시됩니다. 도구 설명에 문서 조각 내에 사용된 언바운드 변수의 수가 표시됩니다.
...
1. 웹 채널에 구성 요소를 추가하려면 웹 채널을 선택하고 구성 요소를 **[!UICONTROL 누릅니다]**. 필요에 따라 대화형 커뮤니케이션의 웹 채널에 구성 요소를 드래그하여 놓고 구성합니다.
   ...
   | 구성 요소 | 기능 |
   |---|---|
   | Chart | 양식 데이터 모델 컬렉션에서 검색한 2차원 데이터를 시각적으로 표시하기 위해 대화형 통신에 사용할 수 있는 차트를 추가합니다. 자세한 내용은 차트 구성 [요소](../../forms/using/chart-component-interactive-communications.md)사용을 참조하십시오. |
   ...
   | 단추 | 대화형 통신에 [단추를](../../forms/using/create-interactive-communication.md#add-button-component-to-the-web-channel) 추가할 수 있습니다. Button 구성 요소를 사용하여 다른 Interactive Communications, 적응형 양식, 이미지 또는 문서 조각과 같은 기타 에셋 또는 외부 URL로 이동할 수 있습니다. |
   | 분리자 | 대화형 통신 내에 가로선을 삽입할 수 있습니다. 이 구성 요소를 사용하여 서신의 섹션을 구분할 수 있습니다. 예를 들어 구분자 구성 요소를 사용하여 신용카드 명세서에서 고객 세부 정보와 신용 카드 세부 정보 섹션을 구분할 수 있습니다. |
...
미리 보기 옵션을 **사용하여** 대화형 통신의 모양을 평가할 수 있습니다. 또한 인터랙티브 커뮤니케이션의 웹 채널은 다양한 디바이스에 대한 인터랙티브 커뮤니케이션의 경험을 에뮬레이션하는 옵션을 제공합니다. 예: iPhone, iPad 및 Desktop. 미리 보기 **및 에뮬레이터** **눈금자** 옵션을 ![](assets/ruler.png) 함께 사용하여 화면 크기가 다른 장치에 대한 웹 출력을 미리 볼 수 있습니다. 미리 보기의 샘플 데이터는 지정된 양식 데이터 모델에서 채워집니다.
...
1. 인쇄 채널에서 문서 컨테이너를 선택하고 속성을 **누릅니다**.
...
1. 다음을 지정합니다.
   ...
   * **[!UICONTROL 바인딩 유형]**:필드에 대해 다음 바인딩 유형 중 하나를 선택합니다.
      ...
      * 없음:에이전트가 속성 값을 채웁니다.
      * 텍스트 조각:이 옵션을 선택한 경우 해당 컨텐츠가 필드에 렌더링되는 텍스트 문서 조각을 찾아 선택할 수 있습니다. 또는 텍스트 문서 조각을 필드 이름으로 드래그하여 놓아 둘 사이의 바인딩을 설정합니다. 텍스트 문서 조각에는 변수가 없어야 합니다.
      * 데이터 모델 개체:필드에 값이 채워지는 양식 데이터 모델 속성을 선택합니다. 또는 Data Sources **탭을** 선택하고 속성을 필드로 드래그하여 놓습니다.
   * **[!UICONTROL 기본값]**:기본값은 지정된 데이터 모델 개체 또는 텍스트 조각에서 제공하는 값이 없을 때 필드가 비어 있지 않도록 합니다. 데이터 바인딩 유형이 none이면 필드에 기본값이 미리 채워집니다.
   * **[!UICONTROL 표시 패턴]**:필드의 표시 형식을 정의할 수도 있습니다. 유형 드롭다운 목록에서 사전 정의된 옵션을 **선택하여** 표시 형식을 필드에 적용합니다. 목록에서 **사용할** 수 없는 표시 패턴을 정의하려면 사용자 지정을 선택합니다. 자세한 내용은 데이터 [표시 패턴을 참조하십시오.](../../forms/using/create-interactive-communication.md#datadisplaypatterns)
...
작성 인터페이스를 사용하면 인쇄 및 웹 채널에 대한 대화형 통신을 만들 때 사용할 수 있는 필드, 변수 및 양식 데이터 모델 요소에 대한 데이터 표시 패턴을 정의할 수 있습니다.

데이터 표시 패턴을 구성하려면 요소를 누르고 ![configure_icon](assets/configure_icon.png) (구성)을 선택하고 세로 막대의 속성 **[!UICONTROL 패널에서 표시 패턴을]** 설정합니다. 유형(Type) 드롭다운 **[!UICONTROL 목록에서]** 미리 정의된 옵션을 선택하여 선택한 유형과 연관된 패턴을 봅니다. 유형 **[!UICONTROL 드롭다운]** 목록에서 **[!UICONTROL 사용자]** 지정을 선택하여 목록에서 사용할 수 없는 패턴을 정의합니다. 패턴 필드에서 값을 **[!UICONTROL 편집하면]** 자동으로 유형이 사용자 **[!UICONTROL 지정으로 수정됩니다]**.

표시 패턴을 적용하려면 패턴 필드에 정의된 문자 또는 자릿수가 필드, 변수 및 양식 데이터 모델 요소의 값에 정의된 문자 또는 자릿수와 일치해야 합니다. 자세한 내용은 [예를](../../forms/using/create-interactive-communication.md#greaternumberofdigits)참조하십시오.

![data_display_pattern_example](assets/data_display_patterns_ssn_new.png)

인쇄 채널에서 웹 컨텐츠를 생성한 후 필드, 변수 또는 양식 데이터 모델 요소에 대한 표시 패턴을 다시 정의할 수 있습니다. 따라서 인쇄 및 웹 채널에 대해 정의된 표시 패턴이 요소에 달라질 수 있습니다. 인쇄 채널에서 요소에 대한 표시 패턴을 정의하지 않고 인쇄 채널을 사용하여 웹 컨텐츠를 자동 생성하는 경우 인쇄 채널에서 요소에 대해 정의된 데이터 바인딩은 유형 **[!UICONTROL 드롭다운 목록에서 사용할 수 있는 디스플레이 패턴 옵션을]** 정의합니다. 요소에 대해 정의된 바인딩이 없으면 요소의 데이터 유형이 사용 가능한 표시 패턴 옵션을 정의합니다. 예를 들어 인쇄 채널에서 요소에 대해 숫자 유형의 데이터 바인딩을 만드는 경우 유형 드롭다운 목록에서 사용할 수 **[!UICONTROL 있는]** 표시 패턴 옵션은 다양한 형식의 숫자 유형입니다.

미리 보기 **모드로 전환하거나** 에이전트 UI를 열어 이러한 요소에 적용된 표시 패턴을 봅니다.

다음 표에는 변수에 대한 데이터 표시 패턴을 설정한 결과로 표시되는 값의 예가 나와 있습니다.

| 유형 | 기본값 | 디스플레이 패턴 | 표시 값 | 설명 |
|---|---|---|---|---|
| 사회보장번호 | 123456789 | text{999-99-999} | 123-45-6789 | 기본값 필드의 자릿수는 패턴 필드의 자릿수와 일치합니다. 패턴을 기반으로 한 값이 성공적으로 표시됩니다. |
| 사회보장번호 | 1234567 | text{999-99-999} | 1-23-4567 | 기본값 필드의 자릿수가 패턴 필드의 자릿수보다 작습니다. 패턴은 사용 가능한 7자리 숫자에 적용됩니다. |
| 사회보장번호 | 1234567890 | text{999-99-999} | 1234567890 | 기본값 필드의 자릿수가 패턴 필드의 자릿수보다 큽니다. 따라서 표시 값은 변경되지 않습니다. |

변수 또는 양식 데이터 모델 요소에 대해 표시 패턴이 지정되지 않은 경우 기본적으로 [글로벌 문서 조각 구성이](https://helpx.adobe.com//experience-manager/6-5/forms/using/interactive-communication-configuration-properties.html) 사용됩니다.

숫자 데이터 유형의 변수에 표시 패턴을 적용하지 않는 경우 인쇄 미리 보기에는 전역 문서 조각 구성에 따라 패턴이 표시됩니다. 기본 글로벌 문서 조각 구성에 변경 사항을 적용하는 경우 에이전트 UI는 여전히 로케일에 대해 정의된 기본 구분 기호에 따라 패턴을 표시합니다.
...
또한 지정된 표시 패턴이 잘못되었거나 적용할 수 없는 경우 XFA 사양에 기반한 기본 패턴이 필드, 변수 또는 양식 데이터 모델 요소에 적용됩니다.
...
1. 표의 각 셀에 대한 데이터 바인딩을 지정합니다. 반복 가능한 행을 만들려면 공통 컬렉션 속성에 속하는 행에 양식 데이터 모델 속성을 삽입합니다.
   ...
   1. 속성을 구성합니다.
      ...
      * **[!UICONTROL 바인딩 유형]**:필드에 대해 다음 바인딩 유형 중 하나를 선택합니다.
         ...
         * **[!UICONTROL 없음]**
         * **[!UICONTROL 데이터 모델 개체]**:양식 데이터 모델 속성의 값이 필드에 채워집니다. 또는 Data Sources **탭을** 선택하고 속성을 필드로 드래그하여 놓습니다.
...
웹 템플릿의 루트 패널을 누르고 **+** 를 눌러 대화형 **통신에 표** 구성 요소를 추가합니다. 두 행이 포함된 표가 대화형 통신에 삽입됩니다. 표의 첫 번째 행은 표 머리글을 나타냅니다.
...
1. 표 **열을** 각각 추가하거나 삭제하려면 **** 열 추가 또는 열 삭제를 선택합니다.
...
1. 행 **추가** 또는 행 **삭제를 선택하여** 테이블 행을 각각 추가하거나 삭제합니다. 도구 모음에서 **사용할 수 있는 위로** 이동 및 **아래로** 이동 옵션을 사용하여 표의 행을 다시 정렬할 수 있습니다.
...
1. 표 셀에서 기본 텍스트 상자를 선택하고 ![](assets/edit.png) (편집)을 누릅니다.
1. 표 셀에 텍스트를 입력하고 을 눌러 ![](assets/done_icon.png) 저장합니다.

#### 테이블 셀과 데이터 모델 개체 요소 간 바인딩 만들기 {#createbindingtablecells}

1. 표 행에서 기본 텍스트 상자를 선택하고 ![](assets/edit.png) (편집)을 누릅니다.
1. 데이터 모델 개체 드롭다운 목록을 누르고 속성을 선택합니다.
1. 을 눌러 테이블 셀과 데이터 모델 객체 속성 간의 바인딩을 저장하고 만듭니다.
...
1. 컬렉션 속성을 하위 폼으로 드래그하여 놓습니다. 하위 폼에서 표를 만듭니다.
...
대화형 통신을 만들고 나면 나중에 해당 속성을 편집할 수 있습니다.

속성 **페이지에서** 다음 작업을 수행할 수 있습니다.

* 제목 및 설명과 같은 대화형 통신을 만들 때 지정한 필드의 값을 편집합니다.
...
다음 단계를 수행하여 기존 대화형 통신에 대한 웹 채널을 추가합니다.

1. 속성 **페이지에서** 채널 **탭을** 선택합니다.
1. 웹 **확인란을** 선택하고 웹 채널에 대한 템플릿을 선택합니다.
1. 웹 **채널과 인쇄 채널** 간의 동기화를 활성화하려면 [웹 채널에 대한 마스터로 인쇄 사용]을 선택합니다.
1. 저장 **및 닫기를** 눌러 변경 내용을 저장합니다.

   마찬가지로 채널 **탭의 웹** 확인란을 눌러 **** 대화형 통신에서 웹 채널을 삭제할 수 있습니다.

## 웹 채널에 단추 구성 요소 추가 {#add-button-component-to-the-web-channel}

단추를 대화형 통신의 웹 채널에 구성 요소로 추가할 수 있습니다. 규칙 편집기를 [](../../forms/using/rule-editor.md) 사용하여 규칙을 정의하여 다른 인터랙티브 커뮤니케이션, 적응형 양식, 이미지 또는 문서 조각과 같은 기타 자산 또는 단추 탭 시 외부 URL로 이동할 수 있습니다.

단추를 추가하고 이 단추에 대한 규칙을 정의하려면

1. 웹 템플릿에서 루트 패널을 누르고 **+** 를 눌러 대화형 **통신에** 단추 구성 요소를 추가합니다.
1. 단추 구성 요소를 탭하고 버튼을 탭하여 규칙을 ![](assets/edit-rules.png) 정의합니다.
1. [ **시기** ] 섹션의 단추 드롭다운 목록에서 **클릭한** 경우를 선택합니다.
1. Then **섹션에서** 다음을 수행합니다.

   1. 드롭다운 목록에서 작업을 선택합니다. 예를 들어 작업 **유형으로 [탐색]을** 선택합니다.
   ...
   마찬가지로 작업 유형 드롭다운 목록에서 서비스 호출 및 양식 제출 등의 사용 가능한 다른 옵션을 선택할 수 있습니다. 자세한 내용은 [규칙 편집기를](../../forms/using/rule-editor.md)참조하십시오.

1. 대화형 통신을 미리 보고 단추를 눌러 4단계에서 지정한 대화형 통신, 응용 양식, 자산 또는 웹 페이지를 봅니다.

## 웹 채널에 패널 구성 요소 추가 {#add-panel-component-to-the-web-channel}

패널 구성 요소는 다른 구성 요소를 함께 그룹화하기 위한 자리 표시자로, 아코디언 및 탭과 같은 구성 요소 그룹이 대화형 통신에 배치되는 방식을 제어합니다. 또한 패널 구성 요소를 사용하면 교육용 자격 증명을 채우는 데 필요한 여러 항목 등 최종 사용자를 위해 반복 가능한 구성 요소 그룹을 만들 수 있습니다.

웹 채널에 패널 구성 요소를 추가하려면 다음 단계를 수행하십시오.

1. 다음 **옵션을 사용하여** 웹 채널에 패널 구성 요소를 삽입합니다.

   * 구성 요소를 누르고 **+** 을 탭하고 패널 **구성 요소를** 선택합니다.

   * 구성 **요소** 브라우저 패널에서 대화형 **커뮤니케이션에서 패널** 구성 요소를 드래그하여 놓습니다.

   * 컨텐츠 **브라우저** 패널에서 패널을 **누르고** 하위 패널 **추가를**&#x200B;누릅니다. [하위 **패널 추가** ] 옵션을 선택하면 [하위 패널 **추가** ] 대화 상자가표시됩니다. 패널 구성 요소의 제목과 선택적 설명과 이름을 입력합니다.

1. 구성, 규칙 편집, **복사** , 삭제 및 구성 요소 삽입과 같은 추가 작업을 수행하려면 콘텐트 브라우저에서 패널을 누릅니다.

   또한 컨텐츠 브라우저 내에서 패널을 드래그 앤 드롭하여 오른쪽 **창에서** 대화형 통신 구조의 변경 사항을 반영할 수 있습니다.

## 인쇄 채널과 웹 채널 동기화 {#synchronize}

대화형 통신을 만들 때 웹 채널용 마스터로 인쇄를 선택하면 웹 채널이 인쇄 채널과 동기화되고 웹 채널의 컨텐츠 및 데이터 바인딩이 인쇄 채널에서 파생되며, 동기화를 탭하면 인쇄 채널에서 수행된 변경 사항이 웹 채널에 반영될 수 있습니다.

그러나 작성자는 필요에 따라 웹 채널의 구성 요소에 대한 상속을 중단할 수 있습니다.

![인쇄 마스터](assets/create_ic_print_master_new-1.png) 인쇄 ![마스터 웹 만들기](assets/create_ic_print_master_web_new-1.png)
...
>[!NOTE]
>
>채널을 동기화하면 문서 조각, 이미지, 조건, 목록 및 레이아웃 조각만 인쇄 채널에서 웹 채널에 동기화됩니다. 이러한 요소를 포함하는 하위 양식 또는 상위 노드는 동기화되지 않습니다.

### 채널 요소 인쇄를 선택하여 웹 채널 컨텐츠 제작 {#selectprintchannelelements}

대화형 통신을 만들 때 마스터로 인쇄를 선택하고 자동 동기화 옵션을 선택하지 않으면 인쇄 채널 요소를 웹 채널 작성 인터페이스로 드래그하여 놓을 수도 있습니다.

Data Sources **>** 마스터 **컨텐트로 이동하여** 인쇄 채널 요소를 봅니다. 대상 영역, 필드 또는 표를 웹 채널 작성 인터페이스로 드래그하여 놓습니다. 요소 이름 옆에 있는 파란색 원 아이콘은 인쇄 채널 요소가 이미 웹 채널에 포함되어 있음을 나타냅니다.

![마스터 컨텐츠](assets/master_content.png)

### 상속 취소 {#cancelinheritance}

웹 채널에서 구성 요소는 대상 영역에 포함됩니다.

웹 채널에서 관련 대상 영역 또는 변수 위로 마우스를 가져간 후 ![상속](assets/cancelinheritance.png) 취소(상속 취소)를 선택한 다음 상속 취소 대화 상자에서 예를 **[!UICONTROL 누릅니다]**.
...
웹 채널에 대해 마스터로 인쇄를 사용하고 인쇄 채널을 변경하는 경우 컨텐츠를 동기화하여 웹 채널에 새로 변경된 내용을 가져올 수 있습니다.
...
웹 채널 작성 인터페이스는 패널에서 구성 요소를 쉽게 그룹화하여 해당 구성 요소에 대한 작업을 동시에 수행할 수 있도록 합니다. [ **콘텐트** ] 탭에는 그룹화된 구성 요소가 컨텐츠 트리에서 패널의 하위 요소로 나열됩니다.

1. 구성 요소를 누르고 그룹( ![그룹](assets/group.jpg)) 작업을 선택합니다.
1. 여러 구성 요소를 선택하고 패널에서 **그룹 개체를 누릅니다**.

   ![그룹 개체](assets/component_toolbar_group_objects_new.png)

1. [ **패널의 개체 그룹** ] 대화 상자에서 패널 이름을 입력합니다.
1. 패널에 대한 제목 및 설명을 선택적으로 입력합니다.
1. bullet_ ![checkmark를 클릭합니다](assets/bullet_checkmark.png).
