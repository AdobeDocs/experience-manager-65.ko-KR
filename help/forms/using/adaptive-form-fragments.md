...
적응형 양식은 패널 또는 필드 그룹를 한 번만 좋아요 양식 세그먼트를 만들어 적응형 양식에서 재사용 하는 편리한 메커니즘을 제공 합니다. 재사용 가능 및 독립형 세그먼트를 적응형 양식 단편 이라고 합니다.
...
1. **확인**&#x200B;을 클릭합니다.
   ...
   패널은 지정 된 저장소 또는 기본 위치에 저장 됩니다. 적응형 양식에서는 패널이 조각의 스냅숏으로 바뀝니다. 아래 그림과 같이 일반 정보 패널과 해당 하위 패널인 개인 정보 및 주소는 조각으로 저장됩니다.
...
적응형 양식에 삽입 하는 모든 조각은 자리 표시자 이미지로 표시 됩니다. 자리 표시자는 조각에서 최대 10 개의 하위 패널에 대 한 제목을 표시 합니다. 자리 표시자 이미지 대신 전체 조각을 표시 하도록 AEM Forms를 구성할 수 있습니다.

양식에서 전체 조각을 표시 하려면 다음 단계를 수행 하십시오.

1. Https: [*호스트*] : [*포트*] /system/console/configMgr. 웹 콘솔 구성 페이지로 이동 AEM 합니다.

1. [적응형 양식]을 클릭 하 고 인터랙티브한 통신 웹 채널 구성을 ]**클릭**[!UICONTROL  하 여 편집 모드에서 엽니다. Search
...
DOR(Document of Record)을 사용하면 양식의 정보를 인쇄 또는 문서 형식으로 유지할 수 있습니다. 따라서 나중에 언제든지 고객에 대한 정보를 추적할 수 있으며, 기록 문서를 사용하여 양식과 컨텐츠를 PDF 형식으로 함께 보관할 수도 있습니다. [적응형 양식 단편을 위한 기록 문서 생성 방법](/help/forms/using/generate-document-of-record-for-non-xfa-based-adaptive-forms.md).
...
<table>
 <tbody>
  ...
  <tr>
   <td><p>미리보기</p> </td>
   <td><p>XML 파일의 데이터를 조각과 병합하여 조각을 HTML 또는 사용자 지정 미리 보기로 미리 보는 옵션을 제공합니다. 자세한 내용은 양식 </a> 미리 보기를 참조 <a href="/help/forms/using/previewing-forms.md" target="_blank"> 하십시오. <br /> <br /> </p> </td>
  </tr>
  <tr>
