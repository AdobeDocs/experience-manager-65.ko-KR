...
1. 을 클릭 하 여 **양식 모델** 탭 클릭 하 고 선택 위치 **드롭다운 메뉴에서** 조각에 대해 다음 모델 중 하나를 선택 합니다.
   ...
   * **양식 템플릿** : AEM Forms 업로드 템플릿를 사용 하 여 단편을 만들도록 지정 합니다. 조각의 양식 모델로 적절한 XDP 템플릿을 선택합니다.
...
편집 모드에서는 AEM 사이드 킥의 적응형 양식 구성 요소를 조각으로 끌어다 놓을 수 있습니다. 적응형 양식 구성 요소에 대 한 자세한 내용은 적응형 양식 ](../../forms/using/introduction-forms-authoring.md) 작성 소개를 참조 [ 하십시오.
...
1. 조각 생성 대화 상자에서 다음 정보를 지정합니다.
   ...
   * **대상 경로**: 조각이 저장될 저장소 경로. 경로를 지정 하지 않으면 해당 조각과 동일한 이름의 노드가 적응형 양식이 들어 있는 노드 다음에 만들어집니다. 이 노드에 조각이 저장 됩니다.

   * **양식 모델** : 적응형 양식의 양식 모델에 따라이 필드에 XML 스키마 **,** 양식 템플릿 **또는** 없음 **이 표시 됩니다** . 편집할 수 없는 필드입니다.

   * **조각 모델 루트** : XSD 기반 적응형 양식 에서만 나타납니다. 조각 모델의 루트를 지정 합니다. 다음을 선택할 수 있습니다. **/** 또는 드롭다운에서 XSD 복합 유형을 선택하십시오. 복합 유형을 조각 모델 루트로 선택 하는 경우에만 다른 적응형 양식에서 조각을 재사용할 수 있습니다.
...
AEM Forms UI를 사용 하 여 적응형 양식 조각에 대해 여러 작업을 수행할 수 있습니다.
...
<table>
 <tbody>
  ...
  <tr>
   <td><p>복사</p> </td>
   <td><p>선택한 조각을 복사합니다. 도구 모음에 붙여넣기 버튼 표시 됩니다. <br /> <br /> </p> </td>
  </tr>
  <tr>
   <td><p>다운로드</p> </td>
   <td><p>선택한 단편을 다운로드 합니다. <br /> <br /> </p> </td>
  </tr>
  <tr>
   <td><p>미리보기</p> </td>
