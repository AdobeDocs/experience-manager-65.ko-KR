...
1. 편지를 이메일로 보내려면 LiveCycle Forms Workflow을 사용할 수 있습니다. 다음과 같이 acmExtensionsConfig.xml의 modelExtension 태그 아래에 customAction 태그를 추가합니다.
   ...
   | 태그 이름에 cm_ 접두어가 포함된 매개변수 | customAction에 cm_(이름)로 시작하는 하위 태그가 포함된 경우 사후 프로세스(Letter Post 프로세스 또는 serviceName 태그로 표시되는 특수 프로세스)에서 cm_ 접두어가 제거된 관련 태그 아래의 입력 XML 코드에서 이러한 매개 변수를 사용할 수 있습니다. |
...
### <span class="acrolinxCursorMarker"> 동작을 사용하도록 설정하는 LiveCycle 프로세스를 추가하십시오.</code>처리 {#add-the-livecycle-process-to-enable-action-span-class-acrolinxcursormarker-span-handling}
...
1. `https:/[lc server]/:[lc port]/adminui`의 LiveCycle® 서버 인스턴스에 관리자로 로그인합니다.
...
1. `https:/[lc server]:[lc port]/adminui`에 LiveCycle 서버 관리자에게 관리자 자격 증명으로 로그인합니다.
...
DSCSample.jar 파일은 renderLetter API를 사용하여 DSC가 입력으로 제공하는 XML 데이터의 PDF 바이트로 문자를 렌더링합니다. renderLetter 및 기타 API에 대한 자세한 내용은 [Letter 렌더링 서비스](https://www.adobe.io/experience-manager/reference-materials/6-5/forms/javadocs/index.html?com/adobe/icc/ddg/api/LetterRenderService.html)를 참조하십시오.
