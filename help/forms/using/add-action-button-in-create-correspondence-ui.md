...
## 시나리오:Create Correspondence User Interface에서 버튼을 만들어 검토용 문자 {#scenario-create-the-button-in-the-create-correspondence-user-interface-to-send-a-letter-for-review} 보내기
...
1. 편지를 이메일로 보내려면 LiveCycle Forms Workflow을 사용할 수 있습니다. 다음과 같이 acmExtensionsConfig.xml의 modelExtension 태그 아래에 customAction 태그를 추가합니다.
   ...
   | permissionName | 해당 작업은 사용자에게 permissionName으로 지정된 권한이 있는 경우에만 표시됩니다. permissionName을 `forms-users`으로 지정하면 모든 사용자가 이 옵션에 액세스할 수 있습니다. |
...
#### /apps 분기 {#create-a-locale-folder-with-properties-file-in-the-apps-branch}에서 속성 파일이 있는 로케일 폴더를 만듭니다.
...
1. 다음 경로에서 **locale** 폴더를 마우스 오른쪽 단추로 클릭하고 **Overlay Node**&#x200B;을 선택합니다.
...
1. 새로 추가된 사용자 지정 작업/단추의 레이블을 현지화하려면 `/apps/fd/cm/config/defaultApp/locale/`에서 관련 로케일에 대한 ACMExtensionMessages.properties 파일을 만듭니다.
...
1. 앱 폴더에서 CRX의 /apps 분기에 다음 폴더와 유사한 구조를 가진 `js`이라는 폴더를 만듭니다.
...
#### LCA 프로세스 {#configure-the-livecycle-server-to-run-the-lca-process}을(를) 실행하도록 LiveCycle 서버 구성
...
1. `https://'[server]:[port]'/system/console/configMgr`으로 이동하고 관리자로 로그인합니다.
1. Adobe LiveCycle 클라이언트 SDK 구성을 찾아 **편집**(편집 아이콘)을 클릭합니다. 구성 패널이 열립니다.

1. 다음 세부 정보를 입력하고 **저장**&#x200B;을 클릭합니다.

   * **서버 URL**:작업 처리기 코드에서 사용하는 검토용으로 보내기 서비스의 LC 서버의 URL.
...
1. `https:/[lc server]/:[lc port]/adminui`에서 Livecycle Server 관리자로 로그인합니다.
...
1. 가져오기가 완료될 때 **런타임에 에셋 배포를 선택합니다**.

1. **가져오기**&#x200B;를 클릭합니다. 

#### ServiceName을 허용 목록에 추가하다 서비스 목록 {#adding-servicename-to-the-allowlist-service-list}에 추가
...
통신 관리 API를 사용하려면 DSCSample.jar(이 문서에 components.zip의 일부로 첨부)를 다운로드하고 LiveCycle 서버에 업로드합니다. DSCSample.jar 파일이 LiveCycle 서버로 업로드되면 AEM 서버는 DSCSample.jar 파일을 사용하여 renderLetter API에 액세스합니다.
...
1. 구성 파일에 다음 매개 변수를 제공합니다.
   ...
   * **crx.appRoot**=/content/apps/cm
   ...
   >[!NOTE]
   >
   >서버 사이드에서 변경할 때마다 LiveCycle 서버를 다시 시작합니다. 자체 LiveCycle 구성 요소를 만드는 방법에 대한 자세한 내용은 사용자 정의 DSC 개발[을 통해 LiveCycle ES 소프트웨어 확장을 참조하십시오.](https://www.adobe.com/devnet/livecycle/articles/dsc_development.html)
...
#### DSC를 LiveCyle {#import-dsc-to-livecyle}로 가져오기
...
1. 파일 브라우저를 통해 **DSCSample.jar** 파일을 선택하고 **Open**&#x200B;을 클릭합니다.
1. **RenderWrapper**&#x200B;을 마우스 오른쪽 단추로 클릭하고 **시작 구성 요소**&#x200B;을 선택합니다. 구성 요소가 시작되면 구성 요소 이름 옆에 녹색 화살표가 나타납니다.

## 검토용으로 문자 보내기 {#send-letter-for-review}
