...
1. 누르기 **만들기** 및 선택 **대화형 통신**. 다음 **대화형 통신 만들기** 마법사가 표시됩니다.
1. 지정 **create_first_ic** 다음에서 **제목** 및 **이름** 필드. 선택 **FDM_Create_First_IC** 양식 데이터 모델로 사용 및 탭 **다음**.
1. 다음에서 **채널** 마법사:

   1. 지정 **create_first_ic_print_template** 인쇄 템플릿으로 사용하고 **선택**. 다음을 확인합니다. **웹 채널에 대해 기본으로 인쇄 사용** 확인란이 선택되지 않았습니다.

   1. 지정 **Create_First_IC_templates** 폴더 > **Create_First_IC_Web_Template** 웹 템플릿으로 사용하고 탭합니다. **선택**.

   1. 누르기 **만들기**.

   대화형 통신이 성공적으로 생성되었다는 확인 메시지가 표시됩니다.

1. 누르기 **편집** 오른쪽 창에서 대화형 통신을 엽니다.
...
1. 누르기 **차트** 대상 영역 및 탭 **+** 추가 **차트** 구성 요소.
1. 차트 구성 요소를 탭하고 을 선택합니다. ![configure_icon](assets/configure_icon.png) (구성). 차트 등록 정보는 왼쪽 창에 표시됩니다.

   1. 차트의 이름을 지정합니다.
   1. 선택 **파이** 다음에서 **차트 유형** 드롭다운 목록입니다.
   1. 다음 항목 선택 **calltype** 의 속성 **호출** 의 데이터 모델 개체 유형 **X축** 섹션. 누르기 ![done_icon](assets/done_icon.png).
   1. 선택 **빈도** 다음에서 **함수** 드롭다운 목록입니다.
   1. 다음 항목 선택 **calltype** 의 속성 **호출** 의 데이터 모델 개체 유형 **Y축** 섹션. 누르기 ![done_icon](assets/done_icon.png).
...
1. 에서 텍스트 필드 선택 **날짜** 열 및 탭 ![configure_icon](assets/configure_icon.png) (구성).
1. 선택 **데이터 모델 개체** 다음에서 **바인딩 유형** 드롭다운 목록 및 **호출** > **calldate**. 누르기 ![done_icon](assets/done_icon.png) 속성을 저장하려면 두 번 클릭합니다.

   마찬가지로 를 사용하여 바인딩 만들기 **호출 시간**, **호출 번호**, **callduration**, 및 **호출 요금** 의 텍스트 필드에 대한 **시간**, **숫자**, **기간**, 및 **청구** 각 열.

1. 누르기 **PayNow** 대상 영역 및 탭 **+** 추가 **이미지** 구성 요소.
1. 이미지 구성 요소를 탭하고 을 선택합니다. ![configure_icon](assets/configure_icon.png) (구성). 이미지 속성은 왼쪽 창에 표시됩니다.

   1. 지정 **PayNow** 을(를) 의 이미지 이름으로 **이름** 필드.
   1. 누르기 **업로드**&#x200B;로컬 파일 시스템에 저장된 이미지를 선택하고 을 누릅니다 **열기**.
   1. 누르기 ![done_icon](assets/done_icon.png) 이미지 속성을 저장합니다.
...
1. 누르기 **편집** 오른쪽 창에서 대화형 통신을 엽니다.
1. 탭 **채널** 왼쪽 창에서 탭을 누른 다음 을 누릅니다 **웹**.
1. 로 이동 **에셋** 을 탭하고 필터를 적용하여 왼쪽 창에 문서 조각만 표시합니다.
1. 다음 문서 조각을 대화형 통신의 대상 영역으로 드래그 앤 드롭합니다.
...
1. 누르기 **청구 요약** 대상 영역 및 탭 **+** 추가 **차트** 구성 요소.
1. 차트 구성 요소를 탭하고 을 선택합니다. ![configure_icon](assets/configure_icon.png) (구성). 차트 등록 정보는 왼쪽 창에 표시됩니다.
...
1. 다음 항목 선택 **Calldate** 테이블 열 제목 및 탭 ![편집](assets/edit.png) (편집) 텍스트 이름을 로 변경합니다. **호출 날짜**. 마찬가지로 테이블에서 다른 열 머리글의 이름을 바꿉니다.
1. 사용 사례에 따라 **지금 결제** 단추를 클릭합니다. 버튼을 삽입하려면 다음 단계를 수행하십시오.

   1. 누르기 **지금 결제** 대상 영역 및 탭 **+** 추가 **텍스트** 구성 요소.

   1. 텍스트 구성 요소를 탭하고 을 누릅니다 ![편집](assets/edit.png) (편집).
...
1. 다음 단계를 사용하여 하이퍼링크 텍스트의 스타일을 Interactive Communication에서 단추로 표시합니다.
   ...
   1. 텍스트 구성 요소를 탭하고 선택합니다. ![편집](assets/edit.png) (편집).
   1. 다음에서 **테두리** 섹션, 지정 **1.5픽셀** 다음으로: **테두리 폭**, 선택 **단색** 다음으로: **테두리 스타일**, 및 지정 **46픽셀** 다음으로: **테두리 반경**.

   1. 다음에서 단추의 배경색으로 빨간색을 선택합니다. **배경** 섹션.
   1. 다음에서 **여백** 필드 **Dimension 및 위치** 섹션, 탭 **동시 편집** 아이콘, 설정 **오른쪽** 다음으로 여백 **450픽셀**. Top, Bottom 및 Left 필드는 공백으로 설정됩니다.

   ![대화형 통신에 하이퍼링크 삽입](assets/ic_web_hyperlink_new.png)

1. 누르기 **지금 결제** 대상 영역 및 탭 **+** 추가 **이미지** 구성 요소.
1. 이미지 구성 요소를 탭하고 을 선택합니다. ![configure_icon](assets/configure_icon.png) (구성). 이미지 속성은 왼쪽 창에 표시됩니다.

   1. 지정 **PayNow** 을(를) 의 이미지 이름으로 **이름** 필드.

   1. 누르기 **업로드**&#x200B;를 선택하고 **PayNowWeb** 이미지가 로컬 파일 시스템에 저장된 후 을 누릅니다 **열기**.
...
1. 다음에서 **채널** 마법사:
   ...
   1. 지정 **create_first_ic_print_template** 인쇄 템플릿으로 사용하고 **선택**.
...
1. 탭 **채널** 왼쪽 창에서 탭을 누른 다음 을 누릅니다 **웹** 인쇄 채널에서 웹 채널에 대한 컨텐츠를 자동으로 생성합니다.
1. 다음으로: **웹 채널에 대해 기본으로 인쇄 사용** 4단계에서 확인란을 선택하면 인쇄 채널의 웹 채널에 대해 컨텐츠 및 바인딩이 자동으로 생성됩니다.
   ...
   웹 채널에서 관련 대상 영역을 마우스로 가리킨 다음 을 선택합니다 ![상속 취소](assets/cancelinheritance.png) (상속 취소)를 클릭한 다음 **상속 취소** 대화 상자, 탭 **예**.

   ![상속 취소](assets/cancel_inheritance_web_channel_new.png)

   구성 요소의 상속을 취소한 경우 다시 활성화할 수 있습니다. 상속을 다시 활성화하려면 구성 요소를 포함하는 관련 대상 영역의 경계를 마우스로 가리킨 다음 을 누릅니다 ![상속 다시 활성화](assets/reenableinheritance.png).
