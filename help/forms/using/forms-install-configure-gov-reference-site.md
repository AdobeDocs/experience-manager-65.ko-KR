...
이 패키지는 **AEM Forms 6.4 OSGI 작성자**&#x200B;은 테스트되었으며, 따라서 다음 플랫폼 버전에서 지원됩니다.
...
다음 [AEM Forms We.Gov 데모 패키지](https://experience.adobe.com/#/downloads/content/software-distribution/en/aem.html?package=/content/software-distribution/en/details.html/content/dam/aem/public/adobe/packages/cq650/featurepack/we-gov-forms.pkg.all-2.0.2.zip) (**we-gov-forms.pkg.all-&lt;version>.zip**)은 다른 여러 하위 패키지 및 서비스를 포함하는 패키지로 제공됩니다. 패키지에는 다음 모듈이 포함되어 있습니다.
...
* **we-gov-forms.pkg.all-&lt;version>.zip** - *데모 패키지 완료*
   ...
   * **we-gov-forms.ui.apps-&lt;version>.zip** *- 모든 구성 요소, 클라이언트 라이브러리, 샘플 사용자, 워크플로우 모델 등을 포함합니다.*
      ...
      * **grid-aem.ui.apps-1.0-SNAPSHOT.zip** - *Sites 페이지 열 컨트롤에 대한 AEM Sites 그리드 레이아웃 패키지*
...
1. 다음으로 이동 *https://&lt;aemserver>:&lt;port>/crx/packmgr/index.jsp*
1. 이전 항목을 찾습니다. **we-gov-forms.pkg.all-&lt;version>.zip** 파일.
1. 자세히 옵션을 선택합니다.
...
1. &quot; 검색&#x200B;**워크플로우**&quot;.
1. &quot;**workflow-users**&quot; 그룹을 만들고 &quot;속성&quot;을 클릭합니다.
1. &quot;구성원&quot; 탭으로 이동합니다.
1. 입력 **webgov** 사용자 또는 그룹 선택 필드에서 다음을 수행합니다.
1. 드롭다운 &quot;**We.Gov Forms 사용자**&quot;.

   ![워크플로우 사용자에 대한 그룹 설정 편집](assets/edit_group_settings.jpg)

1. 메뉴 모음에서 &quot;저장 및 닫기&quot;를 클릭합니다.
1. 2-7단계를 반복하여 &quot;**analytics**&quot;, &quot; 선택&#x200B;**Analytics 관리자**&quot; 그룹 및 &quot;**We.Gov Forms 사용자**&quot;을 구성원으로 그룹화합니다.
1. 2-7단계를 반복하여 &quot;**forms 사용자**&quot;, &quot; 선택&#x200B;**forms-power-users**&quot; 그룹 및 &quot;**We.Gov Forms 사용자**&quot;을 구성원으로 그룹화합니다.
1. 2-7단계를 반복하여 &quot;**forms-users**&quot;, &quot; 선택&#x200B;**forms-users**&quot; 그룹 및 이번에는 &quot;**We.Gov 사용자**&quot;을 구성원으로 그룹화합니다.

### 전자 메일 서버 구성 {#email-server-configuration}

1. 설정 설명서 검토 [전자 메일 알림 구성](/help/sites-administering/notification.md)
1. 이 작업을 수행하려면 관리자로 로그인합니다.
1. 다음으로 이동 *https://&lt;aemserver>:&lt;port>/system/console/configMgr*
1. 을(를) 찾아 클릭합니다. **일 CQ 메일 서비스** 구성할 서비스입니다.
...
1. https://으로 이동합니다.&lt;aemserver>:&lt;port>위의 참조 설명서 링크에 설명된 프로세스를 완료할 수 있는 /aem/inbox.
1. 다음 `we-gov-forms.pkg.all-[version].zip` 패키지에는 를 추출하여 액세스할 수 있는 샘플 SSL 키 및 인증서가 포함되어 있습니다 `we-gov-forms.pkg.all-[version].zip/ssl` 패키지에 속하는 폴더입니다.
...
1. 전제 조건을 검토합니다. 자세한 내용은 [AEM SSL 구성](../../forms/using/forms-install-configure-gov-reference-site.md#aemsslconfig) 을 사용해야 합니다.
1. 다음으로 이동합니다.
...
1. 다음 세부 정보를 입력합니다.
   ...
   1. **인증 유형:** OAuth 2.0
   1. **인증 설정** 자세한 내용은 [MS Dynamics 클라우드 구성 설정](../../forms/using/forms-install-configure-gov-reference-site.md#dynamicsconfig) 이 정보를 수집하려면:
...
1. 다음으로 이동 [https://portal.azure.com/](https://portal.azure.com/) 및에 로그인하십시오.
1. 왼쪽 메뉴에서 &quot;모든 서비스&quot;를 선택합니다.
1. &quot;앱 등록&quot;을 검색하거나 해당 페이지로 이동합니다.
1. 기존 애플리케이션 등록을 생성하거나 선택합니다.
1. 를 복사합니다. **애플리케이션 ID** OAuth로 사용됩니다. **클라이언트 Id** AEM 클라우드 구성에서
...
1. 서비스를 테스트하여 구성된 데이터 소스에 성공적으로 연결되는지 확인하십시오.
   ...
   >[!NOTE]
   서비스를 테스트한 후 **취소** 비자발적 변경 내용이 양식 데이터 모델에 전파되지 않도록 합니다.
...
1. 을(를) 선택합니다 **We.gov 등록 FDM** 을(를) 선택합니다. **속성**.

   ![Dynamics CRM FDM 속성](assets/aftia-enrollment-fdm.jpg)

1. 로 이동합니다 **소스 업데이트** 탭.

1. 다음을 확인합니다. **컨텍스트 인식 구성** 가 로 설정되어 있습니다. `/conf/we-gov` 그리고 구성된 데이터 소스가 **We.Gov 더비 DS**.

   ![Dynamics CRM FDM 속성](assets/aftia-update-data-source.jpg)

1. 클릭 **저장 후 닫기**.

1. [서비스 테스트](work-with-form-data-model.md#test-data-model-objects-and-services) 구성된 데이터 소스에 성공적으로 연결되는지 확인

   * 연결을 테스트하려면 **CAUSORTGAGEACCOUNT** 그리고 서비스 받고 서비스 및 시스템 관리자를 테스트하면 데이터가 검색되는 것을 볼 수 있습니다.
...
1. 프레임워크 구성을 업데이트하려면 같은 페이지에서 &quot;We.Gov Adobe Analytics Framework(Analytics Framework)&quot;를 클릭합니다(참조) [AEM 작성 활성화](../../forms/using/forms-install-configure-gov-reference-site.md#enableauthoring) 작성 사용).
...
>[!NOTE]
AEM Forms Analytics 데이터는 오프라인 상태이거나, `we-gov-forms.ui.analytics-<version>.zip` 패키지가 설치되었지만 AEM Sites 데이터를 사용하려면 활성 클라우드 구성이 필요합니다.
...
1. &quot;Health Benefits를 위한 등록 애플리케이션&quot; 적응형 양식을 선택하고 &quot;Analytics 보고서&quot; 옵션을 선택합니다.
...
1. https://으로 이동합니다.&lt;aemserver>:&lt;port> > 왼쪽 상단에 있는 Adobe Experience Manager > 도구 > 보안 > Adobe IMS 구성 을 클릭합니다.
...
1. 구성 - 검토 섹션의 나머지 부분으로 진행하지 마십시오. [Adobe I/O에서 통합 만들기](#create-integration-adobeio)
...
1. 로 이동합니다 [Adobe I/O 콘솔](https://console.adobe.io/).
...
1. 전환이 완료되면 사용자에게 다음 화면이 표시됩니다.
   ...
   을(를) 클릭합니다. **출력** 폴더를 삭제하여 생성된 적응형 양식을 봅니다.
...
또한 이 사이트에는 Bootstrap 4( [https://getbootstrap.com/](https://getbootstrap.com/) ). 이 클라이언트 라이브러리는
...
다음을 의미할 수도 있습니다 [템플릿 사용자 지정 스타일 시스템](../../forms/using/forms-install-configure-gov-reference-site.md#customizetemplates) 지원되는 스타일에 대한 설명서입니다.
...
등록 적응형 양식은 처리를 위해 OSGI 워크플로우에 제출됩니다. 이 워크플로우는 다음 위치에서 찾을 수 있습니다. *https://&lt;aemserver>:&lt;port>/conf/we-gov/settings/models/we-gov-process.html*.
...
모든 워크플로우 단계 Java 코드는 **we-gov-forms.core-&lt;version>.jar** 번들입니다.
...
먼저 액세서빌러티 테스트를 수행하려면 Chrome 플러그인을 설치해야 합니다. 이 플러그인은 찾을 수 있습니다 [여기](https://chrome.google.com/webstore/detail/accessibility-developer-t/fpkknkljclfencbdbgkenhalefipecmb?hl=en).

설치되면 Chrome 브라우저 내에서 테스트할 페이지를 로드합니다(참고: 탭이 여러 개 있으면 점수가 달라질 수 있으므로 탭을 한 개만 여는 것이 좋습니다. 페이지가 로드되면
**마우스 오른쪽 버튼으로 클릭** 페이지에서 을(를) 선택하고 을(를) 선택합니다. **감사** 탭 . 개발자는 액세스 가능성 플러그인으로 수행할 감사 유형을 선택할 수 있습니다. 원하는 옵션을 모두 선택하면 사용자는 보고서 생성 버튼을 선택할 수 있습니다. 이렇게 하면 전체 액세스 가능성 등급을 표시하고 전체 액세스 가능성 등급을 높이는 데 사용할 수 있는 PDF 문서가 생성됩니다.
...
`we-gov-forms.pkg.all-<version>.zip` 설치 가능한 ZIP 파일입니다. CRX를 사용하여 이 패키지를 설치하면 됩니다.

패키지 관리자 를 사용하는 사용자는 로 이동하여 AEM Forms의 Ultramarine 테마에 액세스할 수 있습니다 **Forms** > **테마** > **참조 테마** > **울트라마린 액세스 가능**.
...
1. https://&#39;으로 이동합니다.[server]:[포트]&#39;/system/console/configMgr.
...
이제 모두 We.Gov 참조 사이트를 탐색하도록 설정되어 있습니다. We.Gov 참조 사이트 워크플로우 및 단계에 대한 자세한 내용은 [We.Gov 참조 사이트 안내](../../forms/using/forms-gov-reference-site-user-demo.md).
