...
description: JEE에 AEM Forms를 설치하는 데 필요한 및 지원되는 인프라 구성 요소 목록
seo-description: JEE에 AEM Forms를 설치하는 데 필요한 및 지원되는 인프라 구성 요소 목록
...
>* JEE의 AEM Forms는 지원되는 운영 체제 및 응용 프로그램의 영어, 프랑스어, 독일어 및 일본어 버전만 지원합니다.
>
...
>[!NOTE]
>
>AEM Forms 고객이 소유 비용을 절감하고 배포 아키텍처를 간소화하며 개발 스택을 현대화할 수 있도록 하기 위해 Adobe Experience Manager 엔터프라이즈 플랫폼은 독립 실행형 OSGi 기반 배포에 유리하도록 애플리케이션 서버 기반 배포에서 벗어납니다. Adobe는 인프라 구성 요소 축소가 감소된 AEM Forms JEE 스택을 계속 지원합니다.
>
...
>
>새로운 설치의 경우, AEM Forms를 최신 OSGi 스택에 배포하여 양식 데이터 모델을 사용한 응답형 모바일, 다중 채널 인터랙티브 커뮤니케이션 및 백엔드 데이터 통합에 대한 혁신적인 최신 기능을 활용하는 것이 좋습니다.
>
>기존 사용자가 JEE 스택에 AEM Forms를 계속 배포해야 한다는 점을 인식합니다. 이러한 시나리오에서 Adobe는 이 설명서에 설명된 대로 지원되는 인프라에 AEM Forms JEE를 배포해야 합니다. AEM 6.5 Forms로 업그레이드하고 이전 AEM Forms 릴리스에서 지원되지 않는 플랫폼을 사용하는 경우 Adobe 지원에 문의하여 지원되는 플랫폼으로 업그레이드하는 방법에 대한 도움말을 확인할 수 있습니다.

### JVM(Java Virtual Machines) {#java-virtual-machines-jvm}

Adobe Experience Manager Forms를 사용하려면 JDK(Java Development Kit) 배포에서 제공하는 Java Virtual Machine이 실행해야 합니다. Adobe Experience Manager는 다음과 같은 버전의 Java Virtual Machines로 작동합니다.
...
* Adobe 고객 지원 센터는 AEM에서 MongoDB 사용과 관련된 자격 있는 문제를 지원합니다. 자세한 내용은 Adobe Experience Manager [용 MongoDB 페이지를 참조하십시오](https://www.mongodb.com/lp/contact/mongodb-adobe-experience-manager).
...
* JEE의 AEM Forms는 RDBMK 지속성 대신 MySQL을 지원하지 않습니다.
...
* JEE의 AEM Forms는 AEM Repository(CRX-Repository)를 지속하기 위해 MySQL 사용을 지원하지 않습니다.
...
물리적 시스템 또는 가상 환경에서 JEE에서 AEM Forms를 실행할 수 있습니다. 그러나 가상 환경에서 AEM Forms에 문제가 발생하면 물리적 컴퓨터에서 해당 문제를 복제해 보십시오. 실제 시스템에서 문제가 계속되면 Adobe 지원 센터에 문의하여 해결 방법을 확인하십시오. 물리적 시스템에서 복제할 수 없는 문제는 가상 환경 공급업체에 문의하십시오.
...
JEE 서버에서 AEM Forms를 설정할 플랫폼을 선택할 때는 다음 예외를 고려하십시오.

1. JEE의 AEM Forms는 MySQL에서 IBM® WebSphere®를 지원하지 않습니다.
...
1. JEE의 AEM Forms는 Oracle Java™ SE가 아닌 JBoss®가 있는 JDK를 지원하지 않습니다.
1. JEE의 AEM Forms는 IBM® JDK가 아닌 IBM® WebSphere®가 포함된 JDK를 지원하지 않습니다.
1. CRX-repository는 TarMK, MongoDB 및 관계형 데이터베이스(RDBMK)의 지속성을 지원합니다. 응용 프로그램 서버와 CRX 저장소 사이에 두 개의 다른 데이터베이스 시스템이 있을 수 없습니다. 그러나 JEE 환경의 AEM Forms에서는 CRX 리포지토리와 애플리케이션 서버가 있는 지원되는 관계형 데이터베이스에 MongoMK를 사용할 수 있습니다.
1. JEE의 AEM Forms는 CentOS의 WebSphere 응용 프로그램 서버를 지원하지 않습니다.
1. JEE의 AEM Forms는 RBAC(JBoss 역할 기반 액세스 제어)를 지원하지 않습니다.

또한 JEE 배포에서 Adobe AEM Forms용 소프트웨어를 선택할 때는 다음 사항을 고려하십시오.

* JEE의 AEM Forms는 지원되는 주요 버전 및 보조 버전의 지정된 소프트웨어 위에 업데이트, 패치 및 수정 팩을 지원합니다. 하지만 다음 주 또는 부 버전 업데이트는 지정하지 않는 한 지원되지 않습니다.
...
* JEE의 AEM Forms는 Adobe의 [타사 소프트웨어 지원 정책에 따라 다양한 타사 소프트웨어를 지원합니다](../../forms/using/aem-forms-jee-supported-platforms.md#p-third-party-patch-support-policy-p).
* JEE의 AEM Forms는 타사 공급업체의 지원에 따라 플랫폼을 지원합니다. 일부 조합은 타사 공급업체에서 허용하지 않을 수 있습니다. 예를 들어, 많은 공급업체가 Oracle을 사용하여 애플리케이션 서버를 인증하지 않았습니다. 따라서 JEE의 AEM Forms도 이러한 조합을 지원하지 않습니다. 지원되는 소프트웨어 버전을 선택하려면 타사 공급업체의 지원 매트릭스를 확인하십시오.
...
이제 AEM Forms 앱에서 Apache Cordova를 지원합니다. 지원되는 플랫폼 전용 Cordova 버전은 다음과 같습니다.
...
## JEE에서 AEM Forms에 대한 시스템 요구 사항 {#system-requirements-for-aem-forms-on-jee}
...
<table>
 <tbody>
  ...
  <tr>
   ...
   <td>Intel® Xeon® E5-2680, 2.4GHz 프로세서 또는 동급<br /> VMWare ESX 5.1 이상<br /> RAM: 6GB(64비트 JVM이 있는 64비트 OS)<br /> 사용 가능한 디스크 공간: 15GB의 임시 공간 + JEE에서 AEM<br /> Forms용 22GB</td>
  </tr>
  <tr>
   <td>SUSE Linux Enterprise Server</td>
   <td>Intel Xeon E5-2670v2, 1vCPU, 2.5GHz 프로세서<br /> AWS m3.medium(3ECU)<br /> RAM: 6GB(64비트 JVM이 있는 64비트 OS)<br /> 사용 가능한 디스크 공간: 6GB의 임시 공간 및 JEE에서 AEM<br /> Forms를 위한 22GB</td>
  </tr>
  <tr>
   <td>Red Hat Enterprise Linux</td>
   <td>Intel Xeon E5-2670v2, 1vCPU, 2.5GHz 프로세서<br /> AWS m3.medium(3ECU)<br /> RAM: 6GB(64비트 JVM이 있는 64비트 OS)<br /> 사용 가능한 디스크 공간: 6GB의 임시 공간 및 JEE에서 AEM<br /> Forms를 위한 22GB<br /> </td>
  </tr>
  <tr>
...
* [JEE 배포 시 단일 서버 AEM Forms에 대한 시스템 요구 사항](https://www.adobe.com/go/learn_aemforms_sysreq_single_63)
...
>
>* 대부분의 최신 브라우저는 더 이상 NPAPI 기반 플러그인을 지원하지 않습니다. AEM Forms 애플리케이션 및 워크플로우에 영향을 주는 방법에 대한 자세한 내용은 NPAPI [브라우저 플러그인 폐지 및 그 효과를 참조하십시오](https://helpx.adobe.com/aem-forms/kb/discontinuation-of-npapi-plugins-impact-on-aem-forms.html).
...
| Google Android | Android 5.1 이상 AEM Forms 앱은 7-10인치 삼성 갤럭시태블릿과 인기 있는 스마트폰에서 인증되었습니다. |
...
JEE의 서드 파티 참조 플랫폼의 AEM Forms에는 JEE에서 AEM Forms를 개발 및 출시하는 동안, 그리고 해당 버전의 AEM Forms에서 지원하는 인프라의 최소 패치/서비스 팩 수준에서 현재 있었던 서드 파티 인프라의 특정 패치 수준이 표시됩니다.

Adobe는 타사 공급업체가 JEE에서 지원하는 버전과 역호환성을 보장한다고 가정했을 때, 출시와 동시에 서드파티 공급업체가 발행한 긴급 또는 권장 패치를 지원합니다. Adobe는 JEE 설명서의 AEM Forms에 명시된 최소 패치 수준 이후에 출시된 패치만 지원합니다.
...
Adobe의 통제 밖에 있는 상황에서 이전 버전과의 호환성을 주장하는 타사 패치는 Adobe 제품 또는 고객 환경에 부정적인 영향을 줄 수 있습니다. 이러한 경우 Adobe는 고객이 중요한 시스템에 적용하기 전에 제3자로부터 긴급 패치가 미치는 영향을 평가할 것을 권장합니다. Adobe는 일반적인 Adobe 지원 프로그램 또는 제3자가 패치에서 문제를 수정하도록 하는 등 적절한 사업 노력을 통해 타사와 협력할 것입니다. 이는 Adobe에서 지원할 새롭게 출시된 타사 패치는 공급업체 또는 JEE의 AEM Forms에 의해 설명된 대로 작동한다는 것을 보장하지 않습니다.

Adobe는 JEE 릴리스에서 AEM Forms가 지원하는 타사 참조 플랫폼과 특정 시점에 지원되는 패치 정의를 변경할 권한을 가집니다.
