...
1. MySQL 데이터베이스용 JDBC 드라이버를 OSGi 번들로 설치합니다.
   ...
   1. 선택 **설치/업데이트**. An **번들 업로드/설치** 대화 상자가 나타납니다.

   1. 선택 **파일 선택** MySQL JDBC 드라이버 OSGi 번들을 찾아 선택합니다. 선택 **번들 시작** 및 **패키지 새로 고침**, 및 선택 **설치** 또는 **업데이트**. MySQL에 대한 Oracle 공사의 JDBC 드라이버가 활성화되어 있는지 확인합니다. 드라이버가 설치되었습니다.
...
1. MySQL 데이터베이스를 데이터 소스로 구성:
   ...
   1. 찾기 **Apache Sling 연결의 풀링된 데이터 소스** 구성. 을(를) 선택하여 편집 모드로 구성을 엽니다.
   ...
   다른 속성을 기본값으로 유지 [값](https://tomcat.apache.org/tomcat-7.0-doc/jdbc-pool.html) 및 선택 **저장**.
...
1. 선택 **만들기** > **양식 데이터 모델**.
1. 양식 데이터 모델 만들기 마법사에서 **이름** (양식 데이터 모델) 예를 들어, **FDM_Create_First_IC**. **다음**&#x200B;을 선택합니다.
1. 데이터 소스 선택 화면에 구성된 모든 데이터 소스가 나열됩니다. 선택 **MySQL** 데이터 소스 및 선택 **만들기**.
...
1. 다음 **FDM_Create_First_IC** 이전에 만든 양식 데이터 모델이 여기에 나열됩니다. 해당 항목을 선택하고 **편집**.
...
1. 상단의 확인란을 선택합니다. **청구서** 데이터 모델 개체를 선택하고 **하위 속성 만들기**.
1. 다음에서 **하위 속성 만들기** 창:

   1. 입력 **usagecharges** 를 하위 속성의 이름으로 사용하십시오.
   1. 사용 **계산됨**.
   1. 선택 **부동** 을(를) 입력하고 를(를) 선택 **완료** 에 하위 속성을 추가하려면 **청구서** 데이터 모델 개체입니다.

   ![하위 속성 만들기](assets/create_child_property_new.png)

1. 선택 **규칙 편집** 규칙 편집기를 엽니다.
1. **만들기**&#x200B;를 선택합니다. 다음 **값 설정** 규칙 창이 열립니다.
1. Select Option 드롭다운에서 **수학 표현식**.

   ![사용 요금 규칙 편집기](assets/usage_charges_rule_editor_new.png)

1. 수학 표현식에서 을 선택합니다. **호출 요금** 및 **충돌 전하** 각각 첫 번째 및 두 번째 객체로 사용됩니다. 선택 **플러스** 를 연산자로 사용하십시오. 수학 표현식 내에서 를 선택하고 를 선택합니다. **표현식 확장** 추가하려면 **smschares**, **상호 전하**, **임신-**, **roamingintnl**, 및 **vas** 개체를 표현식에 추가합니다.

   다음 이미지는 규칙 편집기의 수학 표현식을 보여 줍니다.

   ![사용 요금 규칙](assets/usage_charges_rule_all_new.png)

1. 선택 **완료**. 규칙이 규칙 편집기에서 만들어집니다.
1. 선택 **닫기** 을 눌러 규칙 편집기 창을 닫습니다.
...
1. 상단의 확인란을 선택합니다. **고객** 데이터 모델 개체를 선택하고 **연결 추가**. 다음 **연결 추가** 속성 창이 열립니다.
1. 다음에서 **연결 추가** 창:
   ...
   * 선택 **추가** 을(를) 연결하려면 **고객** 데이터 모델 개체 대상 **호출** 속성을 사용하는 데이터 모델 개체입니다. 사용 사례에 따라 호출 데이터 모델 개체는 고객 데이터 모델 개체의 모바일 번호 속성에 연결되어 있어야 합니다. 다음 **인수 추가** 대화 상자가 열립니다.
...
1. 선택 **완료** 고객 및 호출 데이터 모델 개체 간에 1:n 연결을 만듭니다.
...
BOM 및 고객 데이터 모델 개체는 각각 BOM(청구서) 및 customerplan(고객) 속성을 사용하여 연결됩니다. 이러한 속성 사이에 바인딩을 만들어 MySQL 데이터베이스에서 사용 가능한 고객에 대한 계획 세부 정보를 검색합니다.
   ...
   * 선택 **완료** billplan 및 customerplan 속성 간에 바인딩을 만듭니다.
...
1. 상단의 확인란을 선택합니다. **고객** 데이터 모델 개체를 선택하고 **속성 편집**. 다음 **속성 편집** 창이 열립니다.
...
1. 상단의 확인란을 선택합니다. **호출** 데이터 모델 개체를 선택하고 **속성 편집**. 다음 **속성 편집** 창이 열립니다.
...
1. 로 이동 **서비스** 탭.
1. 다음 항목 선택 **get** 서비스 및 선택 **속성 편집**. 다음 **속성 편집** 창이 열립니다.
1. 다음에서 **속성 편집** 창:
...
1. 다음 항목 선택 **업데이트** 서비스 및 선택 **속성 편집**. 다음 **속성 편집** 창이 열립니다.
1. 다음에서 **속성 편집** 창:
...
1. 로 이동 **모델** 탭에서 **고객** 데이터 모델 개체 및 선택 **테스트 모델 개체**.
...
1. 로 이동 **서비스** 탭.
1. 다음 항목 선택 **get** 서비스 및 선택 **테스트 서비스.**
1. 다음에서 **입력** 섹션, 다음에 대한 값 지정 **mobileenum** 구성된 MySQL 데이터베이스에 있는 속성을 선택하고 **테스트**.
...
1. 위치 **샘플 데이터 편집** 창, 필요에 따라 데이터 편집 및 선택 **저장**. 창을 닫습니다.
