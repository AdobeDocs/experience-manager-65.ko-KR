...
* 데이터베이스 [](../../forms/using/create-form-data-model0.md#step-set-up-the-database) 설정 섹션에 설명된 샘플 데이터가 있는 MySQL 데이터베이스
* JDBC 데이터베이스 드라이버 번들로 묶음에 설명된 대로 MySQL JDBC 드라이버용 [OSGi 번들](https://helpx.adobe.com/experience-manager/6-3/help/sites-developing/jdbc.html#bundling-the-jdbc-database-driver)
...
1. MySQL 데이터베이스를 데이터 소스로 구성:
   ...
   1. 구성 대화 상자에서 다음 세부 정보를 지정합니다.
      ...
      * **JDBC 연결 URI**:데이터베이스의 연결 URL을 지정합니다. 포트 3306 및 스키마 텔레카에서 실행되는 MySQL 데이터베이스의 경우 URL은 다음과 같습니다.jdbc:mysql://[server]:3306/telca?autoReconnect=true&amp;useUnicode=true&amp;characterEncoding=utf-8
...
* 사용 요금 = 전화 요금 + 컨퍼런스 콜 요금 + SMS 요금 + 모바일 인터넷 요금 + 로밍 국가 + 로밍 국제 + VAS(모든 속성이 BOM 데이터 모델 개체에 있음) **사용** 하위 계산 속성에 대한 자세한 내용은 [대화형 통신 계획을 참조하십시오](/help/forms/using/planning-interactive-communications.md).
...
BOM 및 고객 데이터 모델 개체는 각각 청구 계획(BOM) 및 고객 계획(고객) 속성을 사용하여 연결됩니다. 이러한 속성 간의 바인딩을 만들어 MySQL 데이터베이스에서 사용할 수 있는 모든 고객에 대한 계획 세부 정보를 검색합니다.
   ...
   * 완료를 **눌러** billplan 및 customerplan 속성 간의 바인딩을 생성합니다.
   ![고객 청구서의 연관 추가](assets/add_association_customer_bills_new.png)
...
1. 입력 **섹션에서** 구성된 MySQL 데이터베이스에 있는 **mobilenum** 속성 값을 지정하고 테스트를 **누릅니다**.
   ...
   ![테스트 데이터 모델](assets/test_data_model_new.png)
