...
1. [새 로고 파일 업로드](#uploadlogo) crx에서 만든 폴더에서 입니다.
...
* 시스템 업데이트 시 파일이 안전한지 확인합니다. 업그레이드, 기능 팩 또는 핫픽스의 경우 /libs 분기가 업데이트되고 /libs 분기에 변경 사항을 호스팅하면 덮어쓰여집니다.
* 사용자 지정 파일을 저장하는 기본 위치를 사용하는 경우 실수로 해결할 수 있는 현재 시스템/분기를 방해하지 않도록 도와줍니다.
* AEM이 리소스를 검색할 때 리소스의 우선 순위가 높아지도록 지원합니다. AEM은 먼저 /apps 분기를 검색한 다음 /libs 분기를 검색하여 리소스를 찾도록 구성되어 있습니다. 이 메커니즘은 시스템이 오버레이(및 여기에 정의된 사용자 정의)를 사용함을 의미합니다.

다음 단계를 사용하여 /apps 분기에 필요한 폴더 구조를 생성합니다.

1. 다음으로 이동 `https://'[server]:[port]'/[ContextPath]/crx/de` 관리자로 로그인합니다.
1. 앱 폴더에서 이라는 폴더를 만듭니다. `css` css 폴더(ccrui 폴더에 있음)와 유사한 경로/구조를 갖습니다.
...
1. 앱 폴더에서 이라는 폴더를 만듭니다. `imgs` (ccrui 폴더에 있는) imgs 폴더와 유사한 경로/구조를 사용합니다.
...
* 로고 이미지의 높이가 50px를 넘는 경우, 서신 만들기 사용자 인터페이스는 헤더의 높이이므로 이미지를 최대 50px까지 축소합니다. 이미지 크기를 축소하는 동안 응답 만들기 사용자 인터페이스는 이미지의 종횡비를 유지합니다.
...
1. 를 두 번 클릭합니다. **jcr:data** 속성.
   ...
   이제 newlogo.png 폴더를 클릭하고 jcr:content(dim 옵션)를 두 번 클릭한 다음 nt:resource 유형을 설정합니다. 없는 경우 jcr:content라는 이름의 속성을 만듭니다.
