...
| § 1194.21 지침 | 지침 설명 | 규정 준수를 위한 LiveCycle Designer 우수 사례 | 메모 |
|-----------|-----------------------|-----------------------------------------------------------|-------|
...
| 나. | 애플리케이션은 접근성 기능으로 식별된 다른 제품의 활성화된 기능이 업계 표준에 따라 개발 및 문서화된 경우 이를 중단하거나 비활성화해서는 안 됩니다. 또한 응용 프로그램은 액세스 가능성 기능으로 식별된 운영 체제의 활성화된 기능을 중단하거나 비활성화해서는 안 됩니다. 이러한 액세스 가능성 기능에 대한 응용 프로그램 프로그래밍 인터페이스는 운영 체제의 제조업체에서 문서화했으며 제품 개발자가 사용할 수 있습니다. | LiveCycle Designer 관련 기법 없음 - 이 지침은 PDF forms용 Adobe Reader에서 처리합니다. | |
...
| 사. | 응용 프로그램은 사용자가 선택한 대비와 색상 선택 및 기타 개별 표시 속성을 재정의해서는 안 됩니다. | LiveCycle Designer 관련 기법 없음 | 가능하면 기본 시스템 색상을 사용합니다. |
...
| 11942 지침 | 지침 설명 | 규정 준수를 위한 LiveCycle Designer 우수 사례 | 메모 |
|-----------|-----------------------|-----------------------------------------------------------|-------|
...
| 체크포인트 | 체크포인트 설명 | 규정 준수를 위한 LiveCycle Designer 우수 사례 | 메모 |
|------------|------------------------|-----------------------------------------------------------|-------|
| [1.1](https://www.w3.org/TR/WCAG10/wai-pageauth.html#tech-text-equivalent) | 텍스트가 아닌 모든 요소에 해당하는 텍스트를 입력합니다(예: &quot;alt&quot;, &quot;longdesc&quot; 또는 요소 컨텐츠 참조). 여기에는 이미지, 텍스트 그래픽 표현(심볼 포함), 이미지 맵 영역, 애니메이션(예: 애니메이션 GIF), 애플릿 및 프로그래밍 객체, ASCII 아트, 프레임, 스크립트, 목록 글머리 기호로 사용되는 이미지, 스페이서, 그래픽 버튼, 사운드(사용자 상호 작용 없이 재생됨), 독립형 오디오 파일, 비디오 오디오 트랙 및 비디오가 포함됩니다. | <ul><li>2.4 이미지에 상응하는 텍스트 제공</li> <li>2.12 모든 멀티미디어 콘텐츠에 액세스할 수 있는지 확인합니다.</li> | |
...
| [6.1](https://www.w3.org/TR/WCAG10/wai-pageauth.html#tech-order-style-sheets) | 스타일시트 없이 읽을 수 있도록 문서를 구성합니다. 예를 들어, HTML 문서가 연결된 스타일 시트 없이 렌더링되는 경우에도 문서를 읽을 수 있어야 합니다. | 해당되지 않음 | |
...
| 우선 순위 2 체크포인트 | 체크포인트 설명 | 규정 준수를 위한 LiveCycle Best Practices | 메모 |
|------------|------------------------|-------------------------------------------------|-------|
...
| [3.2](https://www.w3.org/TR/WCAG10/wai-pageauth.html#tech-identify-grammar) | 게시된 형식 문법에 대해 유효성을 검사하는 문서를 만듭니다. | | PDF forms에서 렌더링하려면 Adobe Reader에서 게시된 PDF 사양과 일치해야 합니다. |
...
| [5.3](https://www.w3.org/TR/WCAG10/wai-pageauth.html#tech-avoid-table-for-layout) | 선형화할 때 표를 이해할 수 없는 경우 레이아웃에 표를 사용하지 마십시오. 그렇지 않으면 표가 적절하지 않은 경우 그에 해당하는 대체 버전(선형화된 버전일 수 있음)을 제공하십시오. | 특정 LiveCycle 기술 없음 | LiveCycle Forms에서 레이아웃에 표를 사용할 이유는 없습니다. 대신 레이아웃 팔레트를 사용하여 양식 필드를 그리드 패턴으로 배치합니다. 표 머리글과 같은 표별 기능을 사용할 때는 표만 사용하십시오. |
| [5.4](https://www.w3.org/TR/WCAG10/wai-pageauth.html#tech-table-layout) | 레이아웃에 표를 사용하는 경우 시각적 서식 지정을 위해 구조적 마크업을 사용하지 마십시오. | 특정 LiveCycle 기술 없음 | |
...
| 3.3.6 [오류 방지(모두)](https://www.w3.org/TR/UNDERSTANDING-WCAG20/minimize-error-reversible-all.html) | 특정 LiveCycle 기술 없음 | |
