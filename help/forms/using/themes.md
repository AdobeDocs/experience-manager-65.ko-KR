...
1. 클릭 **Adobe Experience Manager**&#x200B;를 클릭합니다. **Forms**&#x200B;를 클릭한 다음 **테마**.

1. 테마 페이지에서 **만들기 > 테마**.
테마를 만드는 마법사가 시작됩니다.

1. 테마 만들기 마법사의 기본 탭에서 다음을 제공합니다. **제목** 및 **이름** 주제입니다. 필수 필드입니다.
...
1. 클릭 **만들기** 을 클릭한 다음 **편집** 테마 편집기에서 테마를 열려면 **완료** 테마 페이지로 돌아가려면
...
1. 테마 페이지에서 **선택** 테마를 클릭하고 **다운로드**. 테마 세부 정보가 포함된 대화 상자가 표시됩니다.
...
* **캔버스** - 오른쪽에 나타납니다. 모든 스타일 변경 사항이 즉시 반영되는 샘플 적응형 양식 또는 대화형 커뮤니케이션을 보여줍니다. 캔버스에서 직접 개체를 선택하여 연결된 스타일을 찾고 이러한 스타일을 편집할 수도 있습니다. 맨 위에 있는 장치 해상도 눈금자는 캔버스를 제어합니다. 눈금자에서 해상도 중단점을 선택하면 각 해상도에 대한 샘플 양식 또는 대화형 커뮤니케이션의 미리 보기가 표시됩니다. 캔버스는 자세히 설명합니다 [아래](../../forms/using/themes.md#using-canvas).
...
필드 스타일을 지정하면 나이, 이름, 주소와 같은 모든 필드가 스타일이 지정됩니다.
   ...
   * **CSS 보기**: 선택한 구성 요소의 CSS를 볼 수 있습니다
   또한 사이드바의 맨 아래에 화살표가 있습니다. 화살표를 클릭하면 두 가지 더 선택할 수 있습니다. **성공 시뮬레이션** 및 **오류를 시뮬레이션합니다.** 위에서 설명한 옵션과 함께 이러한 옵션에 대해서는 자세히 설명합니다 [아래](../../forms/using/themes.md#using-rail).
...
자산 라이브러리에 스타일을 추가하면 다른 테마 및 [스타일 모드](../../forms/using/inline-style-adaptive-forms.md) 양식 편집기 또는 대화형 통신 편집기 UI에 대한 섹션을 참조하십시오. 마찬가지로 양식 편집기 또는 대화형 통신 편집기 UI의 스타일 모드를 사용하여 구성 요소의 스타일을 지정하면 스타일이 캐시되고 테마에서 사용할 수 있습니다.
...
1. 캔버스 도구 모음에서 **테마 옵션** ![테마 옵션](assets/theme-options.png) > **스타일 관리**.
...
스타일링에서 수정한 사항은 캔버스에 로드된 양식 또는 대화형 커뮤니케이션에 즉시 반영됩니다. 라이브 미리 보기를 사용하면 대화식으로 스타일링의 영향을 정의하고 볼 수 있습니다. 구성 요소의 스타일을 변경하면 **완료** 단추가 사이드바에서 활성화되어 있습니다. 변경 사항을 유지하려면 **완료** 버튼을 클릭합니다.
...
1. 테마 편집기 패널에서 **테마 옵션** ![테마 옵션](assets/theme-options.png) > **구성**.

1. 일반 탭에서 **적응형 양식/문서** 필드.
...
테마 편집기를 사용하면 만들거나 업로드한 테마를 편집할 수 있습니다. 다음으로 이동 **Forms 및 문서 > 테마**&#x200B;및 테마를 선택하고 엽니다. 테마가 테마 편집기에서 열립니다.
...
* **테마 옵션** ![테마 옵션](assets/theme-options.png): 다음 세 가지 옵션을 제공합니다
...
* **에뮬레이터** ![눈금자](assets/ruler.png): 테마 모양을 에뮬레이션하여 다양한 표시 크기를 지정합니다. 표시 크기는 에뮬레이터에서 중단점으로 처리됩니다. 중단점을 선택하고 스타일을 지정할 수 있습니다. 예를 들어, 데스크탑과 태블릿은 두 개의 중단점입니다. 각 중단점에 대해 다른 스타일을 지정할 수 있습니다.
...
사용 **Dimension 및 위치** 구성 요소의 크기를 사용자 지정하는 옵션 사이드바의 옵션. 상태가 **기본값**.

선택하는 대신 **숫자 상자 위젯**, 선택 **필드 위젯** 구성 요소 도구 모음에서 위의 단계를 수행합니다. 차원 선택 시 **필드 위젯** 옵션을 선택하면 숫자 상자를 제외한 모든 텍스트 상자의 크기가 동일합니다.
...
1. 다음과 같은 옵션 사용 **테두리, 배경** 사이드바에서 구성 요소의 모양을 사용자 지정합니다.
1. 를 사용하십시오 **오류 시뮬레이션** 옵션을 선택합니다. 옵션을 선택합니다.
...
오류 및 성공 이외의 상태의 동작을 시뮬레이션하려면 미리 보기 모드를 사용합니다. 미리 보기 모드를 사용하려면 **미리 보기** 를 클릭합니다.
...
이제 적응형 양식 또는 대화형 커뮤니케이션에서 웹 서비스에서 사용할 수 있는 글꼴을 사용할 수 있습니다. 즉시 사용 가능, [Adobe Fonts](https://fonts.adobe.com/)Adobe의 웹 글꼴 서비스인 를 구성으로 사용할 수 있습니다. Adobe Fonts을 사용하려면 키트를 만들고 여기에 글꼴을 추가하고 키트 ID를 가져옵니다 [Adobe Fonts](https://fonts.adobe.com/).

다음 단계를 수행하여 AEM에서 Adobe Fonts을 구성합니다.

1. 작성자 인스턴스에서 를 클릭합니다. ![adobeexperiencemanager](assets/adobeexperiencemanager.png)Adobe Experience Manager > 도구 ![망치](assets/hammer.png) > 배포 > Cloud Services.
1. 설정 **Cloud Services** 페이지로 이동하여 를 엽니다. **Adobe Fonts** 선택 사항입니다. 구성 폴더를 열고 **만들기**.
1. 설정 **구성 만들기** 대화 상자에서 구성에 대한 제목을 지정하고 **만들기**.
...
1. 테마 편집기에서 **테마 옵션** ![테마 옵션](assets/theme-options.png) > **구성**.
1. in **Adobe Fonts 구성** 필드에서 키트를 선택하고 **저장**.
...
1. 사이드바에서 **스타일링**.
1. 에서 테마를 선택합니다 **적응형 양식 테마** 드롭다운 및 클릭 **완료** ![확인 단추](assets/check-button.png).

대화형 커뮤니케이션에 테마를 적용하려면:

1. 편집 모드에서 대화형 커뮤니케이션을 엽니다. 편집 모드에서 대화형 커뮤니케이션을 열려면 양식을 선택하고 **열기**.
1. 편집 모드에서 구성 요소를 선택한 다음 ![필드 수준](assets/field-level.png) >**문서 컨테이너**&#x200B;를 클릭한 다음 ![cmppr](assets/cmppr.png).
...
1. 작성 인스턴스에서 **Adobe Experience Manager > Forms > 테마**.
1. 테마 라이브러리 폴더를 엽니다.
1. 테마 라이브러리 폴더에서 해당 기본 제공 테마 위에 포인터를 놓고 탭합니다 **복사**.
...
* **다른 테마에서 자산 방지**
   ...
   테마를 편집할 때 다른 주제의 자산(예: 이미지)을 찾아보고 추가할 수 있습니다. 예를 들어 페이지의 배경을 편집하는 경우 예를 들어 **페이지** ![편집 단추](assets/edit-button.png)> **배경** > **추가** > **이미지**&#x200B;다른 테마에서 이미지를 탐색하고 추가할 수 있는 대화 상자가 표시됩니다.
...
자세한 내용은 [테마를 사용하여 특정 모양 가져오기](#specific-af-appearance)
   ...
   * **인라인 스타일:**
      ...
      양식을 사용할 때 양식 또는 대화형 통신 다중 채널 편집기에서 스타일 모드를 사용하여 구성 요소의 스타일을 지정할 수 있습니다. 스타일 모드를 사용하여 양식 구성 요소 스타일링을 변경하면 테마에 지정된 스타일링을 무시합니다. 특정 양식의 특정 구성 요소에 대한 스타일을 변경하려면 다음을 참조하십시오 [구성 요소의 인라인 스타일](../../forms/using/inline-style-adaptive-forms.md).
