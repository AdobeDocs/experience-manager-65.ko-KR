  ...
  <tr>
   ...
   <td>
    <ul>
     ...
     <li>이 값이 변경되면 clientlib 노드 구조가 입력한 새 위치로 이동됩니다.<br /><em> 참고 <strong></strong>사항:기본 clientlib 위치를 변경하는 경우, CRXDE 보관소에서 <code>crx:replicate, rep:write, rep:glob:*, rep:itemNames:: js.txt, jcr:read </code>새 <code>forms-users</code> 위치에 및 <code>crx:replicate</code>에 <code>jcr:read </code>을 지정합니다 <code>fd-service</code> . 또한 <code>deny jcr:addChildNodes</code><code>forms-user</code></em></li>
    </ul> </td>
  </tr>
  <tr>
   <td>13.</td>
...
1. 사이드바 **아래쪽에 있는** [오류 시뮬레이션] 옵션을 사용하여 편집할 때 스타일링 모양을 확인합니다.
