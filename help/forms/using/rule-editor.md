...
Forms-power users 그룹에 추가 된 사용자는 새 스크립트를 작성 하 고 기존 스크립트를 편집할 수 있습니다. Forms 사용자 그룹의 사용자는 스크립트를 사용할 수 있지만 스크립트를 만들거나 편집할 수는 없습니다.
...
**조건-작업** 이 구문에서는 먼저 규칙이 조건이 정의된 후 트리거할 작업을 정의합니다. 이 구문은 프로그래밍 언어의 if then 문과 유사 합니다.
...
예를 들어 목록에는 빨강, 파랑, 녹색 및 노랑의 네 가지 옵션이 있습니다. 규칙 작성 하는 동안 옵션 (라디오 단추)은 자동으로 검색 되며 다음과 같이 규칙 작성자가 사용할 수 있게 됩니다.
...
>[!NOTE]
>
>또한 규칙의 값 집합을 사용 하 여 드롭다운 목록 구성 요소의 모든 값을 양식 데이터 모델 서비스 또는 웹 서비스의 출력에서 채울 수 있습니다. 그러나 선택 하는 출력 인수가 배열 유형 인지 확인 하십시오. 배열에서 반환된 모든 값은 지정된 드롭다운 목록에서 사용할 수 있습니다.
