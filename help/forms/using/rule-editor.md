...
규칙 편집기 AEM 6.1 Forms 및 이전 릴리스의 스크립팅 기능을 대체 합니다. 그러나 기존 스크립트는 새 규칙 편집기에 유지 됩니다. 규칙 편집기에서 기존 스크립트를 사용한 작업에 대한 자세한 내용은 [규칙 편집기가 기존 스크립트에 미치는 영향](#impact-of-rule-editor-on-existing-scripts).
...
다음 **날짜** 규칙 유형은 **condition-action-alternate action** 규칙 구문 또는 경우에 따라 **조건-작용** 구문입니다. 이 규칙 유형에 서 먼저 평가 조건을 지정 하 고 조건이 충족 되는 경우 트리거할 작업을 시작 합니다 ( `True` ). When 규칙 유형을 사용 하는 동안 여러 AND and OR 연산자를 사용 하 여 중첩 된 표현식 ](#nestedexpressions) 을 만들 [ 수 있습니다.
...
When 규칙을 작성하는 동안 값 지우기 작업을 트리거할 수 있습니다. 작업 값 지우기 지정 된 개체의 값을 지웁니다. When 문에서 옵션으로 명확한 값를 사용 하면 여러 필드를 사용 하 여 복잡 한 조건을 만들 수 있습니다.
...
규칙 유형의 값 설정 은 패널 및 도구 모음 버튼과 같은 일부 양식 객체에 사용할 수 없습니다. 표준 집합 값 규칙의 구조는 다음과 같습니다.
...
>[!NOTE]
>
>또한 규칙 값 설정 을 사용하여 양식 데이터 모델 서비스 또는 웹 서비스의 출력에서 드롭다운 목록 구성 요소의 모든 값을 채울 수 있습니다. 그러나 선택하는 출력 인수는 배열 형식이어야 합니다. 배열에 반환 되는 모든 값은 지정 된 드롭다운 목록에서 사용할 수 있게 됩니다.
...
양식 개체 탭에는 적응형 양식에 포함된 모든 개체의 계층 보기가 표시됩니다. 객체의 제목과 유형이 표시됩니다. 규칙을 작성할 때 양식 개체를 규칙 편집기로 드래그 앤 드롭할 수 있습니다. 개체 또는 함수를 자리 표시자로 끌어서 놓을 때 규칙를 만들거나 편집 하는 동안 자리 표시자는 자동으로 적절 한 값 유형을 사용 합니다.

하나 이상의 유효한 규칙이 적용 된 양식 객체는 녹색 점으로 표시 됩니다. 양식 객체에 적용 된 규칙 중 하나를 잘못된 하는 경우 양식 객체가 노란색 점으로 표시 됩니다.
