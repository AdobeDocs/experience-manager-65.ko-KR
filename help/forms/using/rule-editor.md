...
규칙 편집기는 AEM 6.1 Forms 및 이전 릴리스의 스크립팅 기능을 대체합니다. 그러나 기존 스크립트는 새 규칙 편집기에 보존됩니다. 규칙 편집기에서 기존 스크립트를 사용한 작업에 대한 자세한 내용은 [규칙 편집기가 기존 스크립트에 미치는 영향](#impact-of-rule-editor-on-existing-scripts).
...
다음 **날짜** 규칙 유형은 **condition-action-alternate action** 규칙 구문 또는 경우에 따라 **조건-작용** 구문입니다. 이 규칙 유형에서는 먼저 평가 조건을 지정한 다음 조건이 충족되면 트리거할 작업( `True`)을 지정합니다. When 규칙 유형을 사용하는 동안 여러 AND 및 OR 연산자를 사용하여 중첩 식을 만들 [수 있습니다](#nestedexpressions).
...
When 규칙을 작성하는 동안 값 지우기 작업을 트리거할 수 있습니다. Clear Value Of Action은 지정된 개체의 값을 지웁니다. When 문에서 Clear Value of를 옵션으로 사용하면 여러 필드가 있는 복잡한 조건을 만들 수 있습니다.
...
규칙 유형의 값 설정 은 패널 및 도구 모음 버튼과 같은 일부 양식 객체에 사용할 수 없습니다. 표준 Set Value Of 규칙의 구조는 다음과 같습니다.
...
>[!NOTE]
>
>또한 규칙 값 설정 을 사용하여 양식 데이터 모델 서비스 또는 웹 서비스의 출력에서 드롭다운 목록 구성 요소의 모든 값을 채울 수 있습니다. 그러나 선택하는 출력 인수는 배열 형식이어야 합니다. 배열에 반환된 모든 값은 지정된 드롭다운 목록에서 사용할 수 있게 됩니다.
...
양식 개체 탭에는 적응형 양식에 포함된 모든 개체의 계층 보기가 표시됩니다. 객체의 제목과 유형이 표시됩니다. 규칙을 작성할 때 양식 개체를 규칙 편집기로 드래그 앤 드롭할 수 있습니다. 규칙을 만들거나 편집하는 동안 개체 또는 함수를 자리 표시자에 드래그하여 놓으면 자리 표시자가 자동으로 적절한 값 유형을 사용합니다.

하나 이상의 유효한 규칙이 적용된 양식 오브젝트는 녹색 점으로 표시됩니다. 양식 오브젝트에 적용된 규칙 중 하나라도 유효하지 않은 경우 양식 오브젝트에 노란색 점이 표시됩니다.

함수 탭에는 Sum Of, Min Of, Max Of, Average Of, Number Of 및 Validate Form과 같은 기본 제공 함수 세트가 포함되어 있습니다. 이러한 함수를 사용하여 반복 가능한 패널과 테이블 행의 값을 계산하고 규칙을 작성할 때 작업 및 조건문에서 사용할 수 있습니다. 그러나 사용자 지정 함수도 만들 [수 있습니다](#custom-functions) .
