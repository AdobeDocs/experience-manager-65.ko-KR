...
# AEM Forms과 Adobe Sign 통합{#integrate-adobe-sign-with-aem-forms}
...
## AEM Forms으로 Adobe Sign 구성 {#configure-adobe-sign-with-aem-forms}

사전 요구 사항이 준비되면 다음 단계를 수행하여 작성자 인스턴스의 AEM Forms과 함께 Adobe Sign을 구성합니다.

1. AEM Forms 작성자 인스턴스의 경우 **도구** > ![일반](assets/hammer.png)********> 구성브라우저를 탐색합니다.
...
1. 도구 **망치**![> Cloud Service](assets/hammer.png)**** > **Adobe Sign** 으로 이동하고 위 단계에서 만든 구성 컨테이너를 선택합니다.
...
1. 구성 페이지에서 **[!UICONTROL 만들기를]** 눌러 AEM Forms에서 Adobe Sign 구성을 만듭니다.
1. Adobe Sign 구성 **** 만들기 페이지의 **[!UICONTROL 일반]****탭에서** 구성 **이름**&#x200B;을 지정하고Next를 누릅니다. 원하는 경우 제목을 지정하고 원하는 대로 이동하여 구성에 대한 축소판을 선택할 수 있습니다.

   현재 브라우저 창에서 URL을 복사합니다. AEM Forms으로 Adobe Sign 응용 프로그램을 구성해야 합니다.
...
1. Adobe Sign 응용 프로그램에 대한 OAuth 설정을 구성합니다.
   ...
   * workflow_read
   ...
   Adobe Sign 응용 프로그램에 대한 OAuth 설정을 구성하고 키를 획득하는 단계별 정보는 응용 프로그램 [개발자 설명서에 대한](https://www.adobe.io/apis/documentcloud/sign/docs.html#!adobeio/adobeio-documentation/master/sign/gstarted/configure_oauth.md) Configure oAuth settings를 참조하십시오.
...
1. 클라이언트 **ID** (응용 프로그램 ID라고도 함)와 클라이언트 암호를 **지정합니다**. 첨부 **파일에 대해 Adobe Sign 사용 옵션을 선택하여** 서명을 위해 전송된 해당 Adobe Sign 문서에 적응형 양식에 첨부된 파일을 추가합니다.
   ...
   Tap **[!UICONTROL Connect to Adobe Sign]**. 자격 증명을 묻는 메시지가 나타나면 Adobe Sign 응용 프로그램을 만드는 동안 사용된 계정의 사용자 이름과 암호를 입력합니다.
...
1. AEM Forms에 Adobe Sign을 구성하려면 1단계부터 12단계를 반복합니다. 구성에 동일한 제목(3단계에서 지정됨)과 동일한 이름(6단계에서 지정됨)을 사용하여 작성자 인스턴스에 구성된 설정을 복제할 수 있습니다.

   이제 Adobe Sign은 AEM Forms과 통합되어 적응형 양식에 사용할 수 있습니다. 응용 양식 [에서 Adobe Sign 서비스를](../../forms/using/working-with-adobe-sign.md#configure-adobe-sign-for-an-adaptive-form)사용하려면 응용 양식 속성에서 위에 만든 구성 컨테이너를 지정합니다.
