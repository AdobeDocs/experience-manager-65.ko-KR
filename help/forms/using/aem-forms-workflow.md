...
1. 새로 만든 워크플로우 모델을 선택하고 을(를) 선택합니다. **편집**. 이제 워크플로우 단계를 추가하여 비즈니스 논리를 구축할 수 있습니다. 처음 워크플로우 모델을 만들 때 다음과 같은 내용이 포함됩니다.
...
1. 선택 **[!UICONTROL 신규]**. 필드 목록이 표시됩니다. 워크플로우에 대한 감시 폴더를 구성하려면 다음 필드에 값을 지정하십시오.
...
1. 선택 **고급**. 다음 필드 및 탭에 대한 값을 지정하십시오. **만들기**. 감시 폴더는 워크플로우를 시작하도록 구성됩니다. 이제 파일을 감시 폴더의 입력 디렉터리에 배치할 때마다 지정된 워크플로가 트리거됩니다.
