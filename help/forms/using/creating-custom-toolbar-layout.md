...
첨부된 콘텐츠 패키지에는 아래에 설명된 전체 코드가 포함되어 있습니다. 콘텐츠 패키지를 설치한 후 `/content/forms/af/CustomLayoutDemo.html`을(를) 열어 사용자 지정 도구 모음 레이아웃 데모를 봅니다.
...
1. 폴더를 만들어 사용자 지정 도구 모음 레이아웃을 유지합니다. 예:
   ...
   >[!NOTE]
   >
   >사용자 지정 레이아웃을 유지하기 위해 만드는 폴더는 `apps` 폴더로 만들어야 합니다.
...
1. 복사된 노드 `mobileFixedToolbarLayout`의 이름을 `customToolbarLayout.`(으)로 변경합니다.
   ...
   새 사용자 지정 도구 모음 레이아웃이 **적응형 양식 도구 모음** 대화 상자 구성에 표시됩니다.
...
1. 이 사용자 지정 도구 모음 레이아웃을 선택하고 [확인]을 클릭합니다.
   ...
   `/etc/customlayout` 노드에 clientlib(javascript 및 css)를 추가하고 `customToolbarLayout.jsp`에 clientlib의 참조를 포함하십시오.
...
![사용자 지정 레이아웃 도구 모음의 데스크톱 보기](assets/toolbar_1.png)
