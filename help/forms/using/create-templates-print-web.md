...
* 고객 세부 정보: 문서 단편 포함
* 청구 요약: 문서 단편 포함
* 요약: 문서 조각(비용 하위 양식) 및 차트(차트 하위 양식)가 포함됩니다.
...
1. 2~5단계를 반복하여 다음 하위 양식을 만듭니다.
   ...
   * ValueAddedServices
   ...
   을(를) 이동하려면 **차트** [비용] 하위 양식 오른쪽에 있는 하위 양식을 선택하고 **차트** 왼쪽 창에서 하위 양식을 선택하고 **레이아웃** 탭을 누르고 값 지정 **앵커X** 필드. 값은 의 값보다 커야 합니다. **폭** 필드 **청구** 하위 양식. 다음 항목 선택 **청구** 하위 양식 및 선택 **레이아웃** 탭을 사용하여 값 보기 **폭** 필드.
...
1. 선택 **파일** > **다른 이름으로 저장** 파일을 로컬 파일 시스템에 저장하려면 다음을 수행합니다.
   ...
   1. 파일을 저장할 위치로 이동하고 이름을 로 지정합니다. **create_first_ic_print_template**.
...
1. 을(를) 드래그 앤 드롭합니다 **텍스트 필드** 의 구성 요소 **개체 라이브러리** (으)로 **디자이너 보기**.
   ...
   마찬가지로 드래그 앤 드롭 **텍스트 필드** 구성 요소를 **시간**, **숫자**, **기간**, 및 **청구** 행.

1. 선택 **파일** > **다른 이름으로 저장** 파일을 로컬 파일 시스템에 저장하려면 다음을 수행합니다.

   1. 파일을 저장할 위치로 이동하고 이름을 로 지정합니다. **table_lf**.
   1. 선택 **.xdp** 다음에서 **유형으로 저장** 드롭다운 목록입니다.
...
다음 작업을 사용하여 대화형 통신의 웹 채널에 대한 템플릿을 만들고 관리합니다.
...
다음 단계를 실행하여 편집 가능한 템플릿의 폴더를 생성합니다.
...
다음 단계를 실행하여 웹 채널용 템플릿을 **Create_First_IC_templates** 폴더:

1. 을 선택하여 해당 템플릿 폴더로 이동합니다. **도구** > **템플릿** > **Create_First_IC_templates** 폴더를 삭제합니다.
1. 누르기 **만들기**.
...
사용 사례를 기반으로 다음을 포함해야 합니다. **지금 결제** 및 **구독** 인터랙티브 통신의 버튼(적응형 양식 구성 요소). 대화형 통신에서 이러한 단추를 사용할 수 있도록 하려면 다음 단계를 수행하십시오.
...
1. 다음에서 **허용된 구성 요소** 탭 / **속성** 섹션, 선택 **단추** 다음에서 **적응형 양식** 구성 요소.
