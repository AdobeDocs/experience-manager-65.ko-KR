...
1. 다음 작업을 수행합니다.
   ...
   1. 계층 구조 복제:of `/libs/livecycle/core/content` at `/apps/livecycle/core/content`.
...
1. 다음 작업을 수행합니다.
   ...
   1. 계층 구조 복제:of `/libs/livecycle/core/components/login` at `/apps/livecycle/core/components/login`. 동일한(노드/폴더) 속성과 액세스 제어를 유지 관리합니다.

   1. 구성 요소 폴더 복사:`/libs/livecycle/core`에서 `/apps/livecycle/core`까지.
...
1. `i18n` 내에 있는 폴더를 모두 삭제합니다(단, `en`).

1. `en` 폴더에서 다음 작업을 수행합니다.

   1. 폴더의 이름을 지원할 로케일 이름으로 변경합니다. 예, `ar`.

   1. 속성 `jcr:language` 값을 `ar`(`ar` 폴더에 대해)으로 변경합니다.
   >[!NOTE]
   >
   >로케일이 언어 국가 코드 조합인 경우, `ar-DZ`, 폴더 이름과 속성 값을 `ar-DZ`로 변경합니다.
...
1. 이제 텍스트를 변경할 노드(원하는 로케일 코드 폴더 아래)의 속성 `sling:message`의 값을 수정합니다. 변환은 노드의 `sling:key` 속성 값에 언급된 키를 통해 수행됩니다.

1. 새 키-값 쌍을 추가하려면 다음 작업을 수행하십시오. 다음에 나오는 스크린샷의 예를 확인하십시오.

   1. `sling:MessageEntry` 유형의 노드를 만들거나 기존 노드를 복사하고 모든 로케일 폴더에서 이름을 바꿉니다.
   1. 복사 `login.jsp` :
...
1. `/apps/livecycle/core/components`의 `login.jsp`에 새 스타일을 사용하십시오.
