...
>[!NOTE]
>
>AEM Dispatcher를 사용하여 적응형 양식을 캐시하는 경우, 사전 채워진 데이터가 있는 양식이 포함된 적응형 양식도 캐시합니다. 이러한 양식이 AEM Dispatcher 캐시에서 제공되면 사용자에게 사전에 채워진 데이터나 오래된 데이터를 제공할 수 있습니다. 따라서 AEM Dispatcher를 사용하여 사전 채워진 데이터를 사용하지 않는 적응형 양식을 캐싱할 수 있습니다. 또한 디스패처 캐시는 캐시된 조각을 자동으로 무효화하지 않습니다. 따라서 양식 조각을 캐시하는 데 사용하지 마십시오. 이러한 양식 및 조각에 대해 [적응형 양식 캐시](../../forms/using/configure-adaptive-forms-cache.md)를 사용하십시오.
...
1. 압축 사용. `APACHE_HOME/conf/httpd.conf` 구성 파일에 다음 구성을 추가합니다.
...
>[!NOTE]
>
>* GDS 및 임시 디렉터리에 대해 다른 위치를 사용하는 경우 `https://'[server]:[port]'/adminui`에서 AdminUI를 열고 **홈 > 설정 > 핵심 시스템 설정 > 코어 구성**&#x200B;으로 이동하여 사용 중인 위치를 확인합니다.
