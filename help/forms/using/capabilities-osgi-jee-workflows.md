...
AEM 받은 편지함 을 사용하여 OSGi에서 Forms 중심의 AEM 워크플로우를 실행하고 모니터링할 수 있습니다. 반면 HTML Workspace을 사용하면 AEM Forms JEE 워크플로우를 실행하고 모니터링할 수 있습니다. 다음 표는 OSGi의 Forms 중심 AEM 워크플로용 AEM 받은 편지함 및 AEM Forms JEE 워크플로용 HTML Workspace에서 사용할 수 있는 다양한 중요한 작업을 이해하는 데 도움이 됩니다.
...
<table>
 <tbody>
  ...
  <tr>
   <td>프로세스, 작업 또는 양식 응용 프로그램을 시작하는 중<br /> </td>
   <td>지원됨<br /> </td>
   <td>지원됨<br /> </td>
  </tr>
  <tr>
...
<table>
 <tbody>
  <tr>
   <td>기능</td>
   <td>OSGi<br />의 양식 중심의 AEM 워크플로 </td>
...
1. OSGi에서 양식 중심의 AEM 워크플로 를 사용하여 채워진 적응형 양식에 서명할 수 있습니다. OSGi의 양식 중심 AEM 워크플로우는 양식 서명에서 지원합니다. [양식 내 서명](../../forms/using/working-with-adobe-sign.md#create-in-form-signing-experience) 환경은 지원되지 않습니다.

1. AEM Forms OSGi 및 HTML Workspace에서 양식 중심 워크플로우를 실행하고 모니터링하려면 AEM 받은 편지함에 대한 액세스 권한이 있어야 AEM Forms JEE 워크플로우를 실행하고 모니터링할 수 있습니다.
1. 기본 AEM Forms 문서 서비스는 OSGi의 양식 중심 AEM 워크플로와 JEE의 AEM Forms 워크플로 모두에서 사용할 수 있습니다. AEM Workflow는 OSGi 및 AEM Forms JEE(프로세스 관리) 워크플로의 양식 중심의 AEM 워크플로에 기본 문서 서비스를 사용합니다.
1. AEM Forms JEE 워크플로우는 적응형 양식만 렌더링할 수 있습니다. 적응형 양식을 PDF 문서로 렌더링하는 것은 지원되지 않습니다.
1. AEM forms JEE 워크플로에는 Adobe Sign에 대한 별도의 단계가 없습니다. AEM Forms JEE 워크플로우에 대한 Adobe Sign이 활성화된 적응형 양식이 필요합니다. 자세한 내용은 [Adobe Sign 설명서](../../forms/using/working-with-adobe-sign.md#add-and-configure-the-signature-step-component)를 참조하세요.
...
1. [전자 메일 보내기](../../forms/using/aem-forms-workflow-step-reference.md#send-email-step) 단계를 사용하여 전자 메일을 보낼 수 있습니다.

## AEM 받은 편지함 및 AEM Forms 앱 기능의 차이점 {#differences-between-aem-inbox-and-aem-forms-app-features}

Forms 중심 워크플로를 시작하는 두 가지 두드러진 방법에는 [AEM 받은 편지함](../../forms/using/manage-applications-inbox.md) 및 AEM Forms 앱이 있습니다. 그러나 AEM 받은 편지함 과 AEM Forms 앱의 기능은 다릅니다. AEM 받은 편지함은 [Forms 중심 워크플로](../../forms/using/aem-forms-workflow.md)에서만 작동하지만, AEM Forms 앱은 Forms 중심 워크플로와 프로세스 관리를 모두 사용합니다.
