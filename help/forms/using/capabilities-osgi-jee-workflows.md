...
AEM 받은 편지함을 사용하여 OSGi에서 Forms 중심의 AEM 워크플로우를 실행하고 모니터링할 수 있습니다. 반면 Workspace HTML을 사용하면 AEM Forms JEE 워크플로우를 실행하고 모니터링할 수 있습니다. 다음 표는 OSGi의 Forms 중심 AEM 워크플로우에 대한 AEM 받은 편지함 및 AEM Forms JEE 워크플로우의 Workspace HTML에서 사용할 수 있는 다양한 중요한 작업을 이해하는 데 도움이 됩니다.
...
1. AEM Forms OSGi에서 양식 중심 워크플로우를 실행 및 모니터링하려면 AEM 받은 편지함에 대한 액세스 권한이 필요하고, AEM Forms JEE 워크플로우를 실행 및 모니터링하려면 Workspace HTML을 사용해야 합니다.
...
1. AEM forms JEE 워크플로에는 Adobe Sign에 대한 별도의 단계가 없습니다. AEM Forms JEE 워크플로우에 대해 Adobe Sign이 활성화된 적응형 양식이 필요합니다. 자세한 내용은 [Adobe Sign 설명서](../../forms/using/working-with-adobe-sign.md#add-and-configure-the-signature-step-component)를 참조하세요.
...
Forms 중심 워크플로를 시작하는 두 가지 두드러진 방법에는 [AEM 받은 편지함](../../forms/using/manage-applications-inbox.md) 및 AEM Forms 앱이 있습니다. 그러나 AEM Inbox와 AEM Forms 앱의 기능은 다릅니다. AEM 받은 편지함은 [Forms 중심 워크플로](../../forms/using/aem-forms-workflow.md)에서만 작동하지만, AEM Forms 앱은 Forms 중심 워크플로와 프로세스 관리를 모두 사용합니다.
