...
AEM Forms은 AEM Forms 앱의 전체 소스 코드를 제공합니다. 소스에는 사용자 정의 작업 공간 응용 프로그램을 만드는 모든 구성 요소가 포함되어 있습니다. 소스 코드 아카이브 `adobe-lc-mobileworkspace-src-<version>.zip`는 소프트웨어 배포에서 `adobe-aemfd-forms-app-src-pkg-<version>.zip` 패키지의 일부입니다.
...
1. Visual Studio 2015가 설치 및 구성된 Windows 8.1 또는 Windows 10 장치의 `adobe-lc-mobileworkspace-src-<version>.zip` 보관 파일을 `%HOMEPATH%\Projects` 폴더로 복사합니다.
...
1. Visual Studio 2015를 사용하여 `CordovaApp.sln` 파일을 열고 AEM Forms 앱 빌드를 계속합니다.
...
1. Visual Studio 표준 도구 모음의 빌드 모드에 대한 드롭다운에서 **릴리스**&#x200B;를 선택합니다.
...
1. Visual Studio 솔루션 탐색기에서 프로젝트 **CordovaApp.Windows**&#x200B;을 마우스 오른쪽 단추로 클릭하고 **스토어 > AppPackages 만들기**&#x200B;를 선택합니다.
   ...
   `Retarget to windows 8.1 required` 오류가 발생하면 오류를 마우스 오른쪽 단추로 클릭하고 팝업 메뉴에서 **Windows 8.1**&#x200B;로 재타깃팅을 선택합니다.

   ![재타깃팅 솔루션](assets/retarget-solution.png)

1. 앱 패키지 만들기 마법사에서 앱을 Windows 스토어에 업로드할지 여부를 선택한 다음 **다음**&#x200B;을 클릭합니다.
...
### 개발자 라이센스 {#by-acquiring-the-developer-license}를 획득함으로써
...
### 기업 소유 장치 {#by-using-enterprise-owned-devices} 사용
...
## Visual Studio {#deploying-an-app-using-visual-studio}을(를) 사용하여 앱 배포
...
1. 앱이 Visual Studio에서 열려 있는 상태에서 솔루션 플랫폼 목록에서 Windows-x64, Windows-x86 또는 Windows-AnyCPU를 선택하고 **원격 컴퓨터**&#x200B;를 선택합니다.
1. 앱이 원격 컴퓨터에 배포됩니다.
