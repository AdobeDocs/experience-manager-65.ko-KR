...
* **호출**: 제공된 입력에 대해 입력 DDX에 지정된 작업을 호출합니다.
...
* **DDX**: DDX 호출 워크플로우 단계의 필수 입력이며 DDX 입력 드롭다운에서 다음 옵션 중 하나를 선택하여 지정할 수 있습니다.

   * *페이로드 관련*: DDX 입력 파일은 워크플로 항목의 페이로드 폴더에 상대적입니다.
   * *페이로드 사용*: 워크플로 항목의 페이로드가 입력 DDX 문서로 사용됩니다.
   * *절대 경로*: CRX 저장소의 DDX 문서에 대한 절대 경로입니다.

* **PayLoad에서 맵 만들기**: 이 옵션을 선택하면 페이로드 폴더의 모든 문서가 의 입력 문서 맵에 추가됩니다. `invoke` 어셈블러의 API입니다. 각 문서의 노드 이름은 맵에서 키로 사용됩니다.
...
1. *페이로드에 출력 저장*: 페이로드 폴더에 출력 문서를 저장하거나 페이로드가 파일인 경우 페이로드를 덮어씁니다.
1. *출력 문서 맵*: 출력 문서당 하나의 항목을 추가하여 각 출력 문서를 저장할 위치를 명시적으로 지정할 수 있습니다. 각 항목은 문서와 문서를 저장할 위치를 지정합니다. 출력 문서는 페이로드를 덮어쓰거나 페이로드 폴더에 저장할 수 있습니다. 출력 문서가 여러 개일 때 유용합니다.

1. *작업 로그*: 문제 해결에 유용한 작업 로그 문서를 저장할 위치를 지정합니다.
...
* *페이로드 관련*: 입력 문서는 워크플로 항목의 페이로드 폴더에 상대적입니다.
* *페이로드 사용*: 워크플로 항목의 페이로드가 입력 문서로 사용됩니다.
...
* *확인* 변환: 변환 후 변환된 PDF/A 문서에 대한 PDF/A 준수 여부를 확인할지 여부를 지정합니다.
...
PDF 양식 렌더링 워크플로우는 의 래퍼입니다 `renderPDFForm` XDP 템플릿 및 데이터 xml을 사용하여 PDF 양식을 만들기 위한 Forms 서비스 API입니다.
...
* *Url 제출*: 생성된 PDF 양식의 기본 제출 URL을 지정합니다.
...
비대화형 PDF 생성 워크플로우는 의 래퍼입니다 `generatePDFOutput` 출력 서비스 API. XDP 템플릿과 데이터 xml에서 비대화형 PDF 문서를 생성하는 데 사용됩니다.
