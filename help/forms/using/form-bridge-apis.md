...
# HTML 5 양식용 Form Bridge API {#form-bridge-apis-for-html-forms}
...
**connect(handler, context)** FormBridge에 연결하고 연결을 수행한 후 Form State가 초기화된 후 함수를 실행합니다.

* **입력**:

   * **핸들러**: Form Bridge가 연결된 후 실행할 함수
   * **컨텍스트**: 의 컨텍스트(this)가 있는 개체 *핸들러* 함수가 설정되었습니다.
...
* **입력:**
   ...
   * **옵션:** 다음 속성이 포함된 JavaScript 개체:
      ...
      * **오류**: 오류 처리기 함수
      * **성공**: 성공 핸들러 함수. 이 함수는에 XML이 포함된 개체에 전달됩니다. *데이터* 속성.
      * **컨텍스트**: 의 컨텍스트(this)가 있는 개체 *성공* 함수가 설정됨
      * **유효성 검사**: 서버에서 수신된 유효성 검사 오류를 확인하기 위해 호출하는 함수입니다. 유효성 검사 함수에 일련의 오류 문자열이 전달됩니다.
      * **formState**: 데이터 XML을 반환해야 하는 XFA 양식의 JSON 상태입니다. 지정하지 않으면 현재 렌더링된 양식에 대한 데이터 XML이 반환됩니다.

* **출력:** 없음
* **오류:** 없음

**registerConfig(configName, config)** FormBridge에 사용자/포털별 구성을 등록합니다. 이러한 구성은 기본 구성을 재정의합니다. 지원되는 구성은 구성 섹션에 지정됩니다.
...
**hideFields(fieldArray)** fieldArray에 Som 표현식이 제공되는 필드를 숨깁니다. 지정된 필드의 현재 상태 속성을 invisible로 설정합니다
...
**showFields(fieldArray)** fieldArray에 Som 표현식이 제공되는 필드를 표시합니다. 제공된 필드의 현재 상태 속성을 &quot;표시&quot;로 설정합니다.
...
**getFormState()** 양식 상태를 나타내는 JSON 반환

* **입력:** 없음
* **출력:** 의 현재 양식 상태를 나타내는 JSON이 포함된 개체 *데이터* 속성.
...
* **입력:**
   ...
   * **옵션:** 다음 속성이 포함된 JavaScript 개체:
      ...
      * **오류**: 오류 처리기 함수
      * **성공**: 성공 핸들러 함수
      * **컨텍스트**: 의 컨텍스트(this)가 있는 개체 *성공* 기능이 설정됨
...
* **입력:**
   ...
   * **som:** 필드의 일부 표현식이 포함된 배열입니다. 필드의 값을 설정하는 som 식입니다.
...
* **입력:** 값을 검색해야 하는 필드의 일부 표현식이 포함된 배열입니다.
* **출력:** 결과를에 배열로 포함하는 개체 **데이터** 속성.
...
* **입력:**
   ...
   * **som:** 값을 설정해야 하는 필드의 일부 식이 포함된 배열입니다.
   * **속성**: 값을 설정해야 하는 속성
   * **값:** Som 표현식에 지정된 필드에 대한 지정된 속성의 값을 포함하는 배열입니다.
