...
발송자에서 캐싱 응용 양식을 활성화하고 구성하려면 아래 단계를 수행하십시오.
...
1. [dispatcher.any 파일에 다음을 추가합니다](https://docs.adobe.com/content/help/en/experience-manager-dispatcher/using/configuring/dispatcher-configuration.html#automatically-invalidating-cached-files).
   ...
   * 업데이트된 버전의 양식이 게시되지 않을 때까지 응용 양식이 캐시에 남아 있습니다.
...
1. [아래 rules dispatcher.any 또는 custom rules 파일을 추가합니다](https://docs.adobe.com/content/help/en/experience-manager-dispatcher/using/configuring/dispatcher-configuration.html#specifying-the-documents-to-cache). 캐싱을 지원하지 않는 URL은 제외됩니다. 예: 대화형 통신.
...
컨텐츠 조각 또는 경험 조각을 적응형 양식에 추가하면 이러한 자산이 독립적으로 편집되고 게시되면 발송자 캐시에서 무효화되지 않은 이러한 자산을 포함하는 적응형 양식이 자동으로 포함됩니다.
...
1. 파일에 다음 코드를 추가하고 저장합니다. 사용자 환경에 맞게 수정하는 샘플 코드입니다.
