...
**이 섹션**: Camila the CX Lead 는 종이 기반 프로세스의 일부로 사용된 기존 PDF 기반 양식을 가지고 있습니다. 현대화 노력의 일환으로 이 PDF 양식을 사용하여 새로운 적응형 Forms을 자동으로 만들고 싶습니다.
...
1. Campaign은 PDF을 AEM Forms에 업로드합니다.

   ![양식 업로드](assets/aftia-upload-form.jpg)

1. 그런 다음 카밀라가 PDF 폼을 선택하고 를 클릭합니다 **자동 전환 시작** 변환 프로세스를 시작하려면 을(를) 클릭하여 **전환 덮어쓰기** 양식을 전환한 경우

   >[!NOTE]
   >
   >AFC의 설정은 최종 사용자에게 미리 구성되어 있으며, 이는 해당 설정을 변경하지 말아야 함을 의미합니다.

   * **선택 사항입니다**: Accessible Ultramarine 테마를 사용하려면 적응형 양식 테마 지정 을 클릭하고 옵션 목록에 나타나는 Accessible-Ultramarine 테마를 선택하면 됩니다.

   ![전환 시작](assets/aftia-start-conversion.jpg)

   ![초마린 테마](assets/aftia-upload-conversion-settings.jpg)

   전환 중에 완료율이 표시됩니다. 상태가 표시되면 **전환됨**&#x200B;를 클릭하고 **출력** 폴더에서 적응형 양식을 선택하고 **편집** 변환된 양식을 열려면 다음을 수행하십시오.

1. 카밀라는 이후 카밀라 카밀라 카밀라 카밀라 카밀라 카밀라 카밀라 카밀라 카밀라 카밀라 카밀라 카밀라 카밀라 카밀라 카밀라 카밀라 카밀라 카밀라 카밀라 카밀라 카밀라 카밀라 카밀라 카밀라 카밀라 카밀라 카밀라는 그 카밀라가 그 양식을 검토하고 모든 분야가 있다는 것을 확실히 한다
...
1. Camila에서 변환된 폼의 Properties를 엽니다. [automated forms conversion - We.Gov](#automated-forms-conversion-wegov).

1. 그런 다음 Camila는 옵션 목록에서 선택 드롭다운 > We.gov 등록 FDM을 선택하여 양식 모델 > 양식 데이터 모델 선택을 선택합니다.

1. 저장 및 닫기 단추를 클릭합니다.

   ![FDM 선택](assets/aftia-select-fdm.jpg)

1. Campaign이 **출력** 폴더를 선택하고 적응형 양식을 선택한 후 클릭합니다 **편집** 완료된 We.Gov 양식을 엽니다.
1. Campaign은 적응형 양식 필드를 선택하고 클릭 수 ![구성 아이콘](assets/configure-icon.svg). 를 사용하여 양식 데이터 모델 엔티티로 바인딩을 만듭니다. **바인딩 참조** 필드. 적응형 양식의 모든 필드에 대해 이 단계를 반복합니다.

### 양식 액세스 가능성 테스트(Camila) {#form-accessibility-testing}

또한 Campaign은 생성된 컨텐츠가 기업 표준에 따라 올바로 그리고 완벽하게 액세스할 수 있는지 확인합니다.

1. Campaign이 **출력** 폴더를 선택하고 적응형 양식을 선택한 후 클릭합니다 **미리 보기** 완료된 We.Gov 양식을 엽니다.
...
Aya는 그녀의 휴대폰에 짧은 적응형 양식을 작성함으로써 그녀의 자격을 확인하기 위해 몇 가지 질문에 대답합니다.
...
1. 을(를) 클릭합니다.**자격이 있습니까?**&quot; 단추.
1. 을(를) 클릭합니다.**지금 적용**&quot; 버튼을 눌러 진행합니다.
...
1. 다음 사용 **동적 논리** 를 사용하여 동적 기능 시연 **가족 상태** 드롭다운:
...
**이 섹션:** 다시 집으로 돌아와, Aya는 그녀가 필요한 정보를 발견했고 그녀의 데스크탑에서 신청서를 다시 열었습니다. Aya가 온라인 양식 포털로 이동하여 응용 프로그램을 다시 시작합니다. 일부 간단한 사용자 지정을 사용하는 경우 에이전시는 자동으로 링크를 생성 및 이메일로 전송하여 애플리케이션을 다시 시작할 수 있습니다.
...
**이 섹션:** 조지는 정부 기관 아야 의 경제 분석가인데, 정부로부터 서비스를 요청하고 있습니다. George는 검토를 위해 자신에게 할당된 모든 서비스 요청 애플리케이션을 볼 수 있는 하나의 대시보드를 제공합니다.
...
1. 다음으로 이동 *https://&lt;aemserver>:&lt;port>/aem/start.html*
1. 사용자 아이콘(오른쪽 상단 모서리)을 클릭하고 &quot;**로그아웃**&quot; 또는 &quot;**가장 대상**&#x200B;현재 관리 사용자로 로그인한 경우 메뉴 옵션 &quot;
...
1. 받은 편지함에서 최신 &quot;**Health Benefits 응용 프로그램 검토**&quot; 작업.
...
1. 을(를) 클릭합니다.**MS Dynamics 를 엽니다.**&quot; 단추를 클릭하여 새 창에서 MS Dynamics 레코드를 엽니다.
...
1. 왼쪽 상단 모서리로 이동하고 &quot;**승인**&quot;을 눌러 애플리케이션을 승인합니다.
...
**이 섹션:** Camila the CX Lead 는 Aya 와 환영 전화를 개설하여 Aya가 승인한 정부 서비스를 사용하는 방법을 설명합니다.
...
**이 섹션:** Aya는 전자 메일을 수신하면 자신의 이점을 요약하고 채울 양식 필드를 포함하는 대화형 커뮤니케이션에 대한 링크가 포함됩니다. PDF 혜택 문을 첨부하고 이메일의 대화형 커뮤니케이션 레터(대화형 커뮤니케이션과 동일한 테마/브랜딩 사용)에 연결합니다.
...
1. 를 클릭하여 PDF 첨부 파일을 엽니다.
1. 이메일 클라이언트에서 다시 스크롤하고 &quot;**온라인 시작 키트 보기**&quot;.

   1. 이렇게 하면 동일한 문서의 웹 채널 버전이 열립니다.

1. 직접 PDF에 대한 빠른 참조를 위해:
...
## 갱신 미리 알림 시민(Aya) {#renewal-reminder-citizen-aya}

**이 섹션:** 카밀라는 또한 1년 후에 통신 미리 알림을 예약합니다. (자동화/실행 및 이메일을 실행하는 워크플로우 단계)
...
1. 을(를) 클릭합니다.**새 응용 프로그램 제출**&#x200B;적응형 양식을 열려면 &quot; 버튼을 클릭합니다.
...
1. 사용자 경험이 완료되고 사용자 제출이 완료되면 Campaign이 AEM Forms 내의 데이터 소스 탭으로 이동합니다(**Forms** > **데이터 통합**)

1. 그런 다음 Camila가 AEM Forms을 선택합니다 **We.gov FDM** 그런 다음 **We.gov 등록 FDM**.

1. 그런 다음 Camila가 을(를) 선택합니다 **연락처** > **서비스 읽기** 테스트 대상
...
**이 섹션:** Campaign은 서비스 요청 양식 입력을 시작하고 포기한 국민 수, 요청 제출에서 승인/거부 응답에 이르는 평균 시간, 시민에게 보낸 혜택 핸드북에 대한 참여 통계 등 기관 KPI를 통해 볼 수 있는 대시보드로 이동합니다.
...
1. &quot;**AEM Forms We.Gov 사이트**&#x200B;사이트 페이지를 보려면 &quot;를 클릭하십시오.
1. 사이트 페이지(예: 홈) 중 하나를 선택하고 &quot;**Analytics 및 Recommendations**&quot;.
...
1. 페이지 보기 페이지(3단계에서 액세스)로 돌아가면 디스플레이 설정을 &quot;**목록 보기**&quot;.
1. &quot;**보기**&quot; 드롭다운 메뉴에서 &quot;**목록 보기**&quot;.

   ![보기 드롭다운 메뉴의 목록 보기](/help/forms/using/assets/list_view_view_dropdown.jpg)

1. 동일한 메뉴에서 &quot;**보기 설정**&quot; 및 &quot;**Analytics**&quot; 섹션을 참조하십시오.

   ![열 표시 구성](/help/forms/using/assets/view_setting_analytics.jpg)

1. 클릭 &quot;**업데이트**&quot;을 클릭하여 새 열을 사용할 수 있도록 합니다.
...
1. &quot;**건강 혜택을 위한 등록 신청**&quot; 적응형 양식을 선택하고 &quot;**Analytics 보고서**&quot; 옵션을 선택합니다.
