...
1. 다음으로 로그인:
   * **사용자**: 카밀라.산토스
   * **암호**: 암호
1. 메인 페이지에서 양식 > 양식 및 문서 > AEM Forms We.gov 양식 > AFC 를 선택합니다.
...
1. 그런 다음 카밀라는 자신이 선택한 양식을 편집하기 시작합니다. 루트 패널 > 편집(공구 모양) > 패널 레이아웃 드롭다운 메뉴에서 상단에서 탭 선택 > 확인란 선택
...
1. Camila는 자동화된 양식 변환 - We.Gov에서 변환된 양식의 [속성을 엽니다](#automated-forms-conversion-wegov).
...
1. &quot;양식 초안&quot; 패널에서 기존 &quot;건강 지원을 위한 등록 신청&quot;을 선택합니다.
...
1. 오른쪽 서클 CTA를 클릭하여 다음 섹션으로 이동합니다.
   ...
   Aya 제출 후 감사 페이지가 수신됩니다. 또한 Adobe Sign을 사용하여 전자 방식으로 기록 문서에 서명할 수 있도록 전자 메일을 받게 됩니다.
...
**이 섹션**: Camila는 AEM Forms 데이터 통합으로 이동하여 빠른 테스트를 실행하여 양식 데이터 모델 통합을 통해 외부 데이터 소스로 전송된 정보가 실제로 있는지 확인할 수 있습니다.

### 양식 데이터 모델(카메라) {#form-data-model-camila}

**이 섹션**: Camila가 Data Sources 페이지로 이동하여 서버가 Derby 데이터베이스 내에서 복제한 데이터의 유효성을 확인합니다.

1. 사용자 환경이 완료되고 사용자 제출이 완료되면 Camila는 AEM Forms 내의 데이터 소스 탭으로 이동합니다(**양식** > **데이터 통합**).
...
1. 그런 다음 Camila는 테스트 서비스에 연락처 ID를 제공한 다음 테스트 단추를 클릭합니다.
...
### Adobe Analytics 양식 보고(Camila) {#camila-reviews-forms-reporting-we-gov-adobe-analytics}
