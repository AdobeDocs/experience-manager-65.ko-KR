...
1. **모바일 보기**:We.Gov 사용자는 크기 조정된 브라우저로 모바일 보기를 복제합니다.
...
**이 섹션**: 문서가 변환되고 적응형 양식이 생성되면 Camila는 적응형 양식을 데이터 소스에 연결해야 합니다.
...
1. Camila가 **출력** 폴더, 적응형 양식 선택 및 클릭 수 **편집** 완성된 We.Gov 양식을 엽니다.
1. Camila가 적응형 양식 필드를 선택하고 클릭 ![구성 아이콘](assets/configure-icon.svg) 및 를 사용하여 양식 데이터 모델 엔터티로 바인딩을 **바인드 참조** 필드. Camila는 적응형 양식의 모든 필드에 대해 이 단계를 반복합니다.

### 양식 접근성 테스트(카밀라) {#form-accessibility-testing}

또한 Camila는 작성된 콘텐츠가 회사 표준에 따라 올바르게 구축되고 완전히 액세스할 수 있는지 확인합니다.

1. Camila가 **출력** 폴더, 적응형 양식 선택 및 클릭 수 **미리 보기** 완성된 We.Gov 양식을 엽니다.
...
**이 섹션**: Aya는 시민이며, 친구로부터 그녀가 정부 기관으로부터 서비스를 받을 자격이 있을 수 있다는 말을 듣습니다. Aya는 자격이 되는 서비스에 대한 자세한 내용을 알아보기 위해 휴대 전화에서 We.Gov 웹 사이트로 이동합니다.
...
**이 섹션:** 집으로 돌아온 Aya는 필요한 정보를 찾아 데스크톱에서 애플리케이션을 다시 시작합니다. Aya는 온라인 Forms 포털로 이동하여 애플리케이션을 다시 시작합니다. 간단한 사용자 지정을 통해 에이전시는 자동으로 링크를 생성하고 이메일로 애플리케이션을 재개할 수도 있습니다.
...
1. 다음으로 이동 *https://&lt;aemserver>:&lt;port>/aem/start.html*
1. 사용자 아이콘(오른쪽 상단)을 클릭하고 **로그아웃**&#x200B;또는 **다음 사용자로 가장** 현재 관리자 권한으로 로그인한 경우 메뉴 옵션을 선택합니다.
   ...
   1. 또는 가장:
      ...
      1. 유형 `George` 다음에서 **다음 사용자로 가장** 필드.

      1. 가장하려면 [확인]을 클릭하십시오.

1. 오른쪽 상단 모서리에서 알림(벨) 아이콘을 클릭합니다.
1. 클릭 **모두 보기** 받은 편지함으로 이동합니다.
1. 받은 편지함에서 최신 파일을 엽니다. **의료 보험 적용 검토** 작업.
...
1. 클릭 **MS® Dynamics 열기** 새 창에서 MS® Dynamics 레코드를 엽니다.
...
1. 왼쪽 상단 모서리로 이동하여 **승인** 을 눌러 애플리케이션을 승인합니다.
...
1. 사용자 아이콘(오른쪽 상단)을 클릭하고 **로그아웃**&#x200B;또는 **다음 사용자로 가장** 현재 관리자 권한으로 로그인한 경우 메뉴 옵션을 선택합니다.
   ...
   1. 또는 가장:
      ...
      1. 유형 `Camila` 다음에서 **다음 사용자로 가장** 필드.

      1. 가장하려면 [확인]을 클릭하십시오.

1. 오른쪽 상단 모서리에서 알림(벨) 아이콘을 클릭합니다.
1. 클릭 **모두 보기** 받은 편지함으로 이동합니다.
1. 받은 편지함에서 최신 파일을 엽니다. **새 연락처 승인** 작업.
...
1. AEM으로 돌아가서 왼쪽 상단 모서리로 이동한 다음 를 클릭합니다. **제출** 을 눌러 신청서를 제출하십시오.
...
1. 클릭 **새 신청서 제출** 적응형 양식을 열 수 있습니다.
...
1. 그런 다음 Camila가 AEM Forms We.gov FDM을 선택한 다음 **We.gov 등록 FDM**.

1. 그런 다음 Camila는 **연락처** > **읽기 서비스** 테스트를 받습니다.

   ![연락처 읽기 서비스](assets/aftia-contact-read-service.jpg)

1. 그런 다음 Camila는 연락처 ID를 사용하여 테스트 서비스를 제공한 다음 를 클릭합니다 **테스트**. 예를 들어 양식을 제출한 경우 1 또는 2입니다. 양식을 제출하지 않은 경우 데이터가 반환되지 않습니다.
...
**이 섹션:** Camila는 서비스 요청 양식을 작성하고 포기한 시민의 %, 요청 제출부터 승인/거부 응답까지의 평균 시간, 시민에게 보낸 혜택 안내서에 대한 참여 통계 등 기관 KPI에서 볼 수 있는 대시보드로 이동합니다.
...
1. 선택 **AEM Forms We.Gov 사이트** 를 클릭하여 사이트 페이지를 확인합니다.
...
1. 페이지 보기 페이지로 돌아가서(3단계에서 액세스) 표시 설정을 의 항목을 표시하도록 변경하여 페이지 보기 정보를 볼 수도 있습니다. **목록 보기**.
1. 를 찾습니다. **보기** 드롭다운 메뉴 및 선택 **목록 보기**.

   ![보기 드롭다운 메뉴의 목록 보기](/help/forms/using/assets/list_view_view_dropdown.jpg)

1. 동일한 메뉴에서 다음을 선택합니다. **설정 보기** 및 **분석** 섹션.
...
1. 다음 항목 선택 **건강 보험 등록 신청서** 적응형 양식 및 선택 **Analytics 보고서** 옵션을 선택합니다.
