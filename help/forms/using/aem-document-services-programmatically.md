...
디지털 서명은 서명의 그래픽 표현을 포함하는 양식 필드인 서명 필드에 나타납니다. 서명 필드는 표시되거나 표시되지 않을 수 있습니다. 서명자는 기존 서명 필드를 사용하거나 서명 필드를 프로그래밍 방식으로 추가할 수 있습니다. 두 경우 모두 서명 필드가 있어야 PDF 문서에 서명할 수 있습니다. 서명 서비스 Java API 또는 서명 웹 서비스 API를 사용하여 서명 필드를 프로그래밍 방식으로 추가할 수 있습니다. PDF 문서에 두 개 이상의 서명 필드를 추가할 수 있습니다. 그러나 각 서명 필드 이름은 고유해야 합니다.
...
<table>
 <tbody>
  ...
  <tr>
   ...
   <td>PDF 문서를 확장하는 Reader에 필요한 옵션을 포함합니다.</td>
  </tr>
  <tr>
   <td><code>unlockOptions</code></td>
...
<table>
 <tbody>
  ...
  <tr>
   <td><code>readerExtensionOptions</code></td>
   <td>PDF 문서 확장 Reader에 필요한 옵션을 포함합니다.</td>
  </tr>
  <tr>
   <td><code>unlockOptions</code></td>
   <td>암호화된 파일의 잠금을 해제하는 데 필요한 매개 변수를 포함합니다. 파일이 암호화된 경우에만 필요합니다.<br /> </td>
...
* **[generatePDFOutputBatch](/help/forms/using/aem-document-services-programmatically.md#p-generatepdfoutputbatch-p):** 양식 디자인을 데이터와 병합하여 PDF 문서를 만듭니다. 선택적으로, 는 각 레코드에 대한 메타데이터 파일을 생성하거나 출력을 PDF 파일에 저장합니다.
...
<table>
 <tbody>
  ...
  <tr>
   <td>uriOrFileName</td>
   <td>입력 파일의 경로와 이름을 지정합니다. 파일은 유형 PDF 또는 XDP일 수 있습니다. filename만 지정하면 옵션에 지정된 contentRoot와 관련하여 파일을 읽습니다.</td>
  </tr>
  <tr>
   <td>데이터</td>
...
<table>
 <tbody>
  ...
  <tr>
   ...
   <td>입력 파일의 경로와 이름을 지정합니다. 파일은 유형 PDF 또는 XDP일 수 있습니다. filename만 지정하면 옵션에 지정된 contentRoot와 관련하여 파일을 읽습니다. <br /> </td>
  </tr>
  <tr>
   <td>데이터</td>
   <td>PDF 문서와 병합되는 데이터가 들어 있는 XML 파일입니다.<br /> </td>
...
<table>
 <tbody>
  ...
  <tr>
   <td>pdfSettings</td>
   <td><p>변환된 문서에 대한 PDF 출력입니다. 다음 설정만 적용할 수 있습니다.</p>
    <ul>
  ...
  <tr>
   <td>settingsDoc</td>
   <td>파일에는 PDF 문서를 생성하는 동안 적용된 설정(예: 웹 보기에 대한 PDF 문서 최적화)과 PDF 문서를 만든 후 적용된 설정(예: 초기 보기 및 보안)이 포함되어 있습니다. 선택적 매개 변수입니다.<br /> </td>
  </tr>
  <tr>
   <td>xmpDoc </td>
