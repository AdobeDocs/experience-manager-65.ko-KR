...
* AEM Maven 프로젝트로 [AEM Forms 클라이언트 SDK](https://helpx.adobe.com/kr/aem-forms/kb/aem-forms-releases.html)을(를) 다운로드하고 구성합니다. AEM 문서 서비스를 사용하여 Maven 프로젝트를 작성하는 데 필요한 클라이언트 클래스는 [AEM Forms 클라이언트 SDK](https://helpx.adobe.com/kr/aem-forms/kb/aem-forms-releases.html)에서 사용할 수 있습니다.
...
<table>
 <tbody>
  ...
  <tr>
   <td><code>verificationTime</code></td>
   <td>서명의 유효성을 검사해야 하는 시간</td>
  </tr>
  <tr>
   <td><code>dssPrefs</code></td>
   <td>다양한 유효성 검사 구성을 제어하는 환경 설정입니다. 암호화된 문서의 경우 다음을 사용하여 잠금 해제 옵션을 설정합니다. <code>setUnlockOptions()</code></td>
  </tr>
  <tr>
   <td><code>resourceResolver</code></td>
   <td>Granite Trust Store에 대한 Resource Resolver</td>
  </tr>
 </tbody>
</table>
...
AEM을 사용하면 PDF 문서에서 디지털 서명을 확인할 수 있습니다. PDF 문서는 여러 서명자의 서명이 필요한 비즈니스 프로세스를 거치는 경우 여러 디지털 서명을 포함할 수 있습니다. 예를 들어, 금융거래는 대출 담당자와 관리자 모두의 서명을 필요로 한다. 서명 서비스 API를 사용하여 PDF 문서 내의 모든 서명을 확인할 수 있습니다. 여러 디지털 서명을 검증할 때 각 서명의 상태와 속성을 확인할 수 있습니다. 디지털 서명을 신뢰하기 전에 Adobe에서 확인하는 것이 좋습니다.
...
서명 또는 인증하려는 PDF 문서에 있는 모든 서명 필드의 이름을 검색할 수 있습니다. PDF 문서에 있는 서명 필드 이름을 잘 모르거나 이름을 확인하려는 경우 프로그래밍 방식으로 검색할 수 있습니다. Signature 서비스에서 서명 필드의 정규화된 이름(예: `asform1[0].grantApplication[0].page1[0].SignatureField1[0]`)을 반환합니다.
...
사용자가 암호를 지정하지 않고도 PDF Reader 또는 Acrobat에서 PDF 문서를 열 수 있도록 Adobe Reader 문서에서 암호 기반 암호화를 제거합니다. PDF 문서에서 암호 기반 암호화를 제거하면 문서가 더 이상 안전하지 않습니다.
...
사용자가 PDF Reader 또는 Acrobat에서 PDF 문서를 열 수 있도록 Adobe 문서에서 인증서 기반 암호화를 제거할 수 있습니다. 인증서로 암호화된 PDF 문서에서 암호화를 제거하려면 개인 키를 참조하십시오. PDF 문서에서 암호화를 제거하면 더 이상 안전하지 않습니다.
...
<table>
 <tbody>
  ...
  <tr>
   <td><code>ResourceResolver</code></td>
   <td>ResourceResolver가 특정 사용자의 키 저장소에 액세스하여 자격 증명을 가져올 수 있습니다.</td>
  </tr>
 </tbody>
</table>
...
* **[generatePDFOutput](/help/forms/using/aem-document-services-programmatically.md#p-generatepdfoutput-p):** 양식 디자인을 네트워크 위치, 로컬 파일 시스템 또는 HTTP 위치에 저장된 데이터와 리터럴 값으로 병합하여 PDF 문서를 생성합니다.
...
* **[generatePDFOutputBatch](/help/forms/using/aem-document-services-programmatically.md#p-generatepdfoutputbatch-p):** 양식 디자인을 데이터와 병합하여 PDF 문서를 만듭니다. 필요한 경우 는 각 레코드에 대한 메타데이터 파일을 생성하거나 출력을 PDF 파일에 저장합니다.
...
* **[generatePrintedOutput](/help/forms/using/aem-document-services-programmatically.md#p-generateprintedoutput-p):** 응용 프로그램에 저장된 양식 디자인 및 데이터 파일에서 PCL, PostScript 및 ZPL 출력을 생성합니다.

### generatePDFOutput {#generatepdfoutput}

generatePDFOutput API는 양식 디자인을 데이터와 병합하여 PDF 문서를 생성합니다. 필요한 경우 는 각 레코드에 대한 메타데이터 파일을 생성하거나 출력을 PDF 파일에 저장합니다. 네트워크 위치, 로컬 파일 시스템 또는 HTTP 위치에 저장된 양식 디자인 또는 데이터에 대해 generatePDFOutput API를 리터럴 값으로 사용합니다. 양식 디자인과 XML 데이터가 응용 프로그램에 저장되어 있는 경우 [generatePDFOutput](/help/forms/using/aem-document-services-programmatically.md#p-generatepdfoutput-p) API를 사용하십시오.
...
<table>
 <tbody>
  ...
  <tr>
   <td>uriOrFileName</td>
   <td>입력 파일의 경로와 이름을 지정합니다. 파일은 PDF 또는 XDP 유형일 수 있습니다. filename만 지정하면 옵션에 지정된 contentRoot와 관련하여 파일을 읽습니다.</td>
  </tr>
  <tr>
   <td>데이터</td>
   <td>PDF 문서와 병합되는 데이터가 들어 있는 XML 파일입니다.<br /> </td>
  </tr>
  <tr>
   <td>옵션</td>
   <td>contentRoot, locale, AcrobatVersion, linearizedPDF 및 taggedPDF 변수의 값을 지정합니다. options 매개 변수는 PDFOutputOptions 유형의 개체를 수락합니다. <br /> </td>
  </tr>
 </tbody>
</table>
...
<table>
 <tbody>
  ...
  <tr>
   ...
   <td>입력 파일의 경로와 이름을 지정합니다. 파일은 PDF 또는 XDP 유형일 수 있습니다. filename만 지정하면 옵션에 지정된 contentRoot와 관련하여 파일을 읽습니다. <br /> </td>
  </tr>
  <tr>
   <td>데이터</td>
   <td>PDF 문서와 병합되는 데이터가 들어 있는 XML 파일입니다.<br /> </td>
  ...
  <tr>
   <td>옵션</td>
   <td>contentRoot, locale, AcrobatVersion, linearizedPDF 및 taggedPDF 변수의 값을 지정합니다. options 매개 변수는 PDFOutputOptions 유형의 개체를 허용합니다.</td>
  </tr>
 </tbody>
</table>
...
양식 디자인을 데이터와 병합하여 PDF 문서를 만듭니다. 필요한 경우 는 각 레코드에 대한 메타데이터 파일을 생성하거나 출력을 PDF 파일에 저장합니다. 네트워크 위치, 로컬 파일 시스템 또는 HTTP 위치에 저장된 양식 디자인 또는 데이터에 대해 generatePDFOutputBatch API를 리터럴 값으로 사용합니다.
...
<table>
 <tbody>
  ...
  <tr>
   <td>데이터</td>
   <td>키 및 데이터 문서 맵을 지정합니다. 키가 null이 아니면 템플릿 맵에 지정된 해당 키에 대한 템플릿을 사용하여 데이터 문서가 렌더링됩니다. </td>
  </tr>
  <tr>
   <td>옵션</td>
   <td>contentRoot, locale, AcrobatVersion, linearizedPDF 및 taggedPDF 변수의 값을 지정합니다. options 매개 변수는 PDFOutputOptions 유형의 개체를 허용합니다.</td>
...
<table>
 <tbody>
  ...
  <tr>
   <td>Inputdocument<br /> </td>
   <td>입력 파일의 경로와 이름을 지정합니다. filename만 지정하면 옵션에 지정된 contentRoot와 관련하여 파일을 읽습니다. 파일은 XDP 유형일 수 있습니다. </td>
  </tr>
  <tr>
   <td>데이터</td>
   <td>PDF 문서와 병합되는 데이터가 들어 있는 XML 파일입니다.<br /> </td>
...
<table>
 <tbody>
  ...
  <tr>
   <td>옵션</td>
   <td>PrintedOutputOptions 형식의 개체를 지정합니다. 이 개체는 contentRoot, locale, printConfig, copies, paginationOverride의 값을 설정하는 데 사용됩니다.<br /> </td>
  </tr>
  <tr>
   <td>batchOptions</td>
...
<table>
 <tbody>
  ...
  <tr>
   <td>데이터<br /> </td>
   <td>XML 형식의 데이터가 포함된 XML 파일입니다.</td>
  </tr>
 </tbody>
</table>
...
createPDF API는 지원되는 파일 유형을 PDF 문서로 변환합니다. Microsoft Word, Microsoft PowerPoint, Microsoft Excel 및 Microsoft Project와 같은 다양한 파일 형식을 지원합니다. 이러한 애플리케이션 외에도 모든 서드파티 일반 PDF 생성 애플리케이션 유형을 API에 연결할 수도 있습니다.
...
<table>
 <tbody>
  ...
  <tr>
   <td>pdfSettings</td>
   <td><p>변환된 문서에 대한 PDF 출력 다음 설정만 적용할 수 있습니다.</p>
    <ul>
  ...
  <tr>
   <td>보안 설정</td>
   <td><p>변환된 문서에 대한 보안 설정입니다. 다음 설정을 적용할 수 있습니다.</p>
    <ul>
     <li>보안 없음</li>
     <li>암호 보안<br /> </li>
     <li>인증서 보안<br /> </li>
  ...
  <tr>
   <td>settingsDoc</td>
   <td>파일에는 PDF 문서를 생성하는 동안 적용된 설정(예: 웹 보기를 위해 PDF 문서 최적화)과 PDF 문서를 만든 후 적용된 설정(예: 초기 보기 및 보안)이 포함되어 있습니다. 선택적 매개 변수입니다.<br /> </td>
  </tr>
  <tr>
   <td>xmpDoc </td>
...
PDF 문서를 지원되는 파일 형식으로 변환합니다. 이 메서드는 PDF을 입력으로 수락하고 PDF의 콘텐츠를 지정된 파일 형식 형식으로 내보냅니다.
...
<table>
 <tbody>
  ...
  <tr>
   <td>settingsDoc </td>
   <td>파일에는 출력 문서를 생성하는 동안 적용할 구성이 포함되어 있습니다. 일반적으로 XML 파일입니다.</td>
  </tr>
 </tbody>
</table>
...
OptimizePDF API는 크기를 줄여 PDF 파일을 최적화합니다. 이 변환의 결과는 원본 버전보다 작을 수 있는 PDF 파일입니다. 이 작업은 또한 PDF 문서를 최적화 매개 변수에 지정된 PDF 버전으로 변환합니다. 최적화된 PDF이 포함된 OptimizePDFResult 개체를 반환합니다.
...
Distiller 서비스는 PostScript, 캡슐화된 PostScript(EPS) 및 프린터 텍스트 파일(PRN)을 PDF 파일로 변환합니다. Distiller 서비스는 대량의 인쇄 문서를 송장 및 명세서 등의 전자 문서로 변환하는 데 자주 사용됩니다. 문서를 PDF으로 변환하면 기업은 고객에게 종이 버전과 전자 버전의 문서를 보낼 수 있습니다. 지원되는 파일 형식은 .ps, .eps 및 .prn입니다. 이 서비스는 다음 API를 지원합니다.
...
<table>
 <tbody>
  ...
  <tr>
   <td>입력 파일 이름</td>
   <td>파일 확장명과 함께 입력 파일의 전체 이름을 지정합니다. 필수 매개 변수입니다.</td>
  </tr>
  <tr>
   <td>pdfSettings</td>
   <td><p>변환된 문서에 대한 PDF 출력 설정입니다. 다음 설정만 적용할 수 있습니다.</p>
    <ul>
     <li>High_Quality_Print<br /> </li>
     <li>PDFA1b_2005_RGB<br /> </li>
     <li>PDFA1b_2005_CMYK<br /> </li>
...
다음 Java 코드 샘플은 PostScript(PS), Encapsulated PostScript(EPS) 및 프린터 텍스트 파일(PRN) 유형의 입력 파일을 PDF 파일로 변환합니다.
...
자동 태그 PDF API는 문서에 태그를 추가하여 PDF 액세스 가능성을 향상시켜 액세스 가능성 표준을 준수하도록 합니다. 이렇게 하면 사용자 경험이 향상될 뿐만 아니라 문서 간에 정확도와 일관성이 유지됩니다. 자동 태그 API는 다음 요소에 태그를 지정할 수 있습니다.

* 텍스트 블록(단락)
* 한 연산자의 글머리 기호 목록
* 목차(목차)

![자동 태그가 지정된 PDF 문서](assets/auto-tag-api.png)

다음 Java 코드 샘플은 PDF 파일을 태그가 지정된 PDF 문서로 변환하는 방법을 보여 줍니다.

**구문**: `Document tag(final Document inDoc)`
...
<table>
 <tbody>
  ...
  <tr>
   <td><code>inDoc</code></td>
   <td>태깅할 입력으로 제공된 문서. 필수 매개 변수입니다.<br /> </td>
  </tr>
 </tbody>
</table>
