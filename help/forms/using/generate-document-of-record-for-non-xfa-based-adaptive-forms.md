...
* 없음 양식 모델 없이 적응형 양식을 만들 수 있습니다. 적응형 양식에 대해 레코드 문서가 자동으로 생성됩니다.
...
자세한 내용은 [레코드의 기본 서식 파일](#base-template-of-a-document-of-record)
...
1. 기본값을 선택하는지 사용자 지정 템플릿을 선택하는지 여부에 따라 다음 속성 중 일부 또는 전부가 레코드 문서 탭에 나타납니다. 다음 사항을 적절히 지정합니다.
   ...
   * **패널 설명 숨기기**
   ...
   선택하는 사용자 지정 XDP 템플릿에 여러 개의 마스터 페이지가 포함되는 경우 해당 페이지에 대한 속성이 **[!UICONTROL 콘텐츠]** 섹션 **[!UICONTROL 기록 문서]** 탭.

   ![마스터 페이지  속성](assets/master-page-properties.png)

   마스터 페이지 속성에는 로고 이미지, 헤더 텍스트, 양식 제목, 면책조항 레이블 및 면책조항 텍스트가 포함됩니다. 적응형 양식 또는 XDP 템플릿 속성을 레코드 문서에 적용할 수 있습니다. AEM Forms에서는 기본적으로 템플릿 속성을 레코드 문서에 적용합니다. 마스터 페이지 속성에 대한 사용자 지정 값을 정의할 수도 있습니다. 레코드 문서에서 여러 마스터 페이지를 적용하는 방법에 대한 자세한 내용은 [여러 마스터 페이지를 기록 문서에 적용](#apply-multiple-master-pages-dor).
...
* **[!UICONTROL 페이지 매김]** > **[!UICONTROL 장소]**: 패널을 배치할 위치를 결정합니다.
   * **[!UICONTROL 장소]** > **[!UICONTROL 이전]**: 상위 패널에서 이전 개체의 뒤에 패널을 배치합니다.
   * **[!UICONTROL 장소]** > **[!UICONTROL 콘텐츠 영역에서]** > 콘텐츠 영역의 이름: 패널을 지정된 콘텐츠 영역에 배치합니다.
   * **[!UICONTROL 장소]** > **[!UICONTROL 다음 컨텐츠 영역의 위쪽]**: 패널을 다음 컨텐츠 영역의 맨 위에 놓습니다.
   * **[!UICONTROL 장소]** > **[!UICONTROL 컨텐츠 영역 상단]** > 콘텐츠 영역의 이름: 패널을 지정된 컨텐츠 영역의 맨 위에 놓습니다.
   * **[!UICONTROL 장소]** > **[!UICONTROL 페이지에서]** > 마스터 페이지의 이름: 패널을 지정된 페이지에 배치합니다. 페이지 나누기가 자동으로 삽입되지 않으면 [!DNL AEM Forms] 페이지 나누기를 추가합니다.
   * **[!UICONTROL 장소]** > **[!UICONTROL 다음 페이지 상단]**: 다음 페이지의 맨 위에 패널을 배치합니다. 페이지 나누기가 자동으로 삽입되지 않으면 [!DNL AEM Forms] 페이지 나누기를 추가합니다.
   * **[!UICONTROL 장소]** > **[!UICONTROL 페이지 상단]** > 마스터 페이지의 이름: 지정된 페이지가 렌더링될 때 페이지 맨 위에 패널을 배치합니다. 페이지 나누기가 자동으로 삽입되지 않으면 [!DNL AEM Forms] 페이지 나누기를 추가합니다.
* **[!UICONTROL 페이지 매김]** > **[!UICONTROL 후]**: 패널을 배치한 후 채울 영역을 결정합니다.다음 필드는 **[!UICONTROL 후]** 섹션:
   * **[!UICONTROL 후]** > **[!UICONTROL 부모 채우기 계속]**: 부모 패널에 채워질 남아 있는 모든 개체의 데이터를 계속 병합합니다.
   * **[!UICONTROL 후]** > **[!UICONTROL 다음 컨텐츠 영역으로 이동]**: 패널을 배치한 후 다음 컨텐츠 영역 작성을 시작합니다.
   * **[!UICONTROL 후]** > **[!UICONTROL 콘텐츠 영역으로 이동]** > 콘텐츠 영역의 이름: 패널을 배치한 후 지정된 컨텐츠 영역 작성을 시작합니다.
   * **[!UICONTROL 후]** > **[!UICONTROL 다음 페이지로 이동]**: 패널을 배치한 후 다음 페이지 작성을 시작합니다.
   * **[!UICONTROL 후]** > **[!UICONTROL 페이지로 이동]** > 페이지 이름: 패널을 배치한 후 지정된 페이지 작성을 시작합니다.
* **[!UICONTROL 페이지 매김]** > **[!UICONTROL 오버플로]**: 패널이나 페이지에 걸쳐 있는 테이블에 대해 오버플로를 설정합니다. 다음 필드는 **[!UICONTROL 오버플로]** 섹션:
   * **[!UICONTROL 오버플로]** > **[!UICONTROL 없음]**: 다음 페이지 작성을 시작합니다. 페이지 나누기가 자동으로 삽입되지 않으면 [!DNL AEM Forms] 페이지 나누기를 추가합니다.
   * **[!UICONTROL 오버플로]** > **[!UICONTROL 콘텐츠 영역으로 이동]** > 콘텐츠 영역의 이름: 지정된 콘텐츠 영역 작성을 시작합니다.
   * **[!UICONTROL 오버플로]** > **[!UICONTROL 페이지로 이동]** > 페이지 이름: 지정된 페이지 작성을 시작합니다.

기록 문서에서 페이지 나누기를 적용하고 여러 마스터 페이지를 적용하는 방법에 대한 자세한 내용은 [기록 문서에 페이지 나누기 적용](#apply-page-breaks-in-dor) 및 [여러 마스터 페이지를 기록 문서에 적용](#apply-multiple-master-pages-dor).
...
선택하는 사용자 지정 XDP 템플릿에 여러 개의 마스터 페이지가 포함되는 경우 해당 페이지에 대한 속성이 [!UICONTROL 콘텐츠] 섹션 [!UICONTROL 기록 문서] 탭. 자세한 내용은 [기록 문서의 브랜딩 정보 사용자 정의](#customize-the-branding-information-in-document-of-record).

적응형 양식의 구성 요소에 다른 마스터 페이지를 적용하여 기록 문서에 여러 마스터 페이지를 적용할 수 있습니다. 를 사용하십시오 [페이지 매김](#document-of-record-settings) 여러 마스터 페이지를 적용할 레코드 등록 정보 문서의 섹션

다음은 여러 마스터 페이지를 기록 문서에 적용하는 방법의 예입니다. 4개의 마스터 페이지를 포함하는 XDP 템플릿을 [!DNL AEM Forms] server. [!DNL AEM Forms] 기본적으로 템플릿 속성을 레코드 문서에 적용합니다. [!DNL AEM Forms] 또한 템플릿의 첫 번째 마스터 페이지 속성을 레코드 문서에 적용합니다.
