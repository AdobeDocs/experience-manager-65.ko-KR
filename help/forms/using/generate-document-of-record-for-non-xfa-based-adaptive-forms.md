...
1. 기본값을 선택하는지 사용자 지정 템플릿을 선택하는지 여부에 따라 다음 속성 중 일부 또는 전부가 레코드 문서 탭에 나타납니다. 다음 사항을 적절히 지정합니다.
   ...
   * **패널 설명 숨기기**
   ...
   선택한 사용자 지정 XDP 템플릿에 여러 개의 마스터 페이지가 포함되어 있으면 해당 페이지의 속성이 **[!UICONTROL 레코드 문서]** 탭의 **[!UICONTROL content]** 섹션에 표시됩니다.

   ![기본 페이지 속성](assets/master-page-properties.png)

   마스터 페이지 속성에는 로고 이미지, 헤더 텍스트, 양식 제목, 면책조항 레이블 및 면책조항 텍스트가 포함됩니다. 적응형 양식 또는 XDP 템플릿 속성을 레코드 문서에 적용할 수 있습니다. AEM Forms에서는 기본적으로 템플릿 속성을 레코드 문서에 적용합니다. 마스터 페이지 속성에 대한 사용자 지정 값을 정의할 수도 있습니다. 레코드 문서에서 여러 마스터 페이지를 적용하는 방법에 대한 자세한 내용은 [레코드 문서에 여러 마스터 페이지 적용](#apply-multiple-master-pages-dor)을 참조하십시오.
...
* **[!UICONTROL 페이지 매김]**  >  **[!UICONTROL 위치]**: 패널을 배치할 위치를 결정합니다.
   * ****  >  **[!UICONTROL 다음 이전]** 배치: 상위 패널에서 이전 개체의 뒤에 패널을 배치합니다.
   * **[!UICONTROL 컨텐츠 영역]**  >  **[!UICONTROL 컨텐츠 영역에 배치]**  > 컨텐츠 영역 이름: 패널을 지정된 콘텐츠 영역에 배치합니다.
   * **[!UICONTROL 다음 컨텐츠 영역]**  >  **[!UICONTROL 위쪽 배치]**: 패널을 다음 컨텐츠 영역의 맨 위에 놓습니다.
   * **[!UICONTROL 컨텐츠 영역]**  >  **[!UICONTROL 위쪽]**  > 컨텐츠 영역 이름: 패널을 지정된 컨텐츠 영역의 맨 위에 놓습니다.
   * ****  >  **[!UICONTROL 페이지에 배치]**  > 마스터 페이지의 이름: 패널을 지정된 페이지에 배치합니다. 페이지 나누기가 자동으로 삽입되지 않으면 [!DNL AEM Forms]은 페이지 나누기를 추가합니다.
   * ****  >  **[!UICONTROL 다음 페이지의 상단]**: 다음 페이지의 맨 위에 패널을 배치합니다. 페이지 나누기가 자동으로 삽입되지 않으면 [!DNL AEM Forms]은 페이지 나누기를 추가합니다.
   * ****  >  **[!UICONTROL 페이지 상단]**  > 마스터 페이지의 이름: 지정된 페이지가 렌더링될 때 페이지 맨 위에 패널을 배치합니다. 페이지 나누기가 자동으로 삽입되지 않으면 [!DNL AEM Forms]은 페이지 나누기를 추가합니다.
* **[!UICONTROL 페이지 매김]**  >  **[!UICONTROL 후]**: 패널을 배치한 후 채울 영역을 결정합니다.다음 필드는 After 섹션에서 사용할 수  **** 있습니다.
   * **[!UICONTROL 이후]** >  **[!UICONTROL 계속 상위 채우기]**: 부모 패널에 채워질 남아 있는 모든 개체의 데이터를 계속 병합합니다.
   * **[!UICONTROL 후]** >  **[!UICONTROL 다음 컨텐츠 영역으로 이동]**: 패널을 배치한 후 다음 컨텐츠 영역 작성을 시작합니다.
   * **[!UICONTROL 다음]** >  **[!UICONTROL 컨텐츠 영역으로 이동]**  > 컨텐츠 영역 이름: 패널을 배치한 후 지정된 컨텐츠 영역 작성을 시작합니다.
   * **[!UICONTROL 다음]**  >  **[!UICONTROL 다음 페이지로 이동]**: 패널을 배치한 후 다음 페이지 작성을 시작합니다.
   * **[!UICONTROL 다음]** >  **[!UICONTROL 페이지로 이동]**  > 페이지 이름: 패널을 배치한 후 지정된 페이지 작성을 시작합니다.
* **[!UICONTROL 페이지 매김]**  >  **[!UICONTROL 오버플로]**: 패널이나 페이지에 걸쳐 있는 테이블에 대해 오버플로를 설정합니다. 다음 필드는 **[!UICONTROL 오버플로]** 섹션에서 사용할 수 있습니다.
   * **[!UICONTROL 오버플로]**  >  **[!UICONTROL 없음]**: 다음 페이지 작성을 시작합니다. 페이지 나누기가 자동으로 삽입되지 않으면 [!DNL AEM Forms]은 페이지 나누기를 추가합니다.
   * **[!UICONTROL 오버플로우]**  >  **[!UICONTROL 컨텐츠 영역으로 이동]**  > 컨텐츠 영역의 이름: 지정된 콘텐츠 영역 작성을 시작합니다.
   * **[!UICONTROL 오버플로우]**  >  **[!UICONTROL 페이지로 이동]**  > 페이지 이름: 지정된 페이지 작성을 시작합니다.

Document of Record에서 페이지 나누기를 적용하고 여러 마스터 페이지를 적용하는 방법에 대한 자세한 내용은 [레코드 문서에 페이지 나누기 적용](#apply-page-breaks-in-dor) 및 [레코드 문서에 여러 마스터 페이지 적용](#apply-multiple-master-pages-dor)을 참조하십시오.
...
여러 방법을 사용하여 기록 문서에 페이지 나누기를 적용할 수 있습니다.

기록 문서에 페이지 나누기를 적용하려면

1. 패널을 탭하고 ![구성](assets/configure-icon.svg)을 선택합니다.

1. **[!UICONTROL 레코드 문서]**&#x200B;를 확장하여 속성을 봅니다.

1. **[!UICONTROL 페이지 매김]** 섹션에서 **[!UICONTROL 위치]** 필드에서 ![폴더](assets/folder-icon.svg)를 탭합니다.
1. **[!UICONTROL 다음 페이지의 위쪽]**&#x200B;을 탭하고 **[!UICONTROL 선택]**&#x200B;을 탭합니다. **[!UICONTROL 페이지 상단]**&#x200B;을 탭하고 마스터 페이지를 선택한 다음, **[!UICONTROL 선택]**&#x200B;을 탭하여 페이지 나누기를 적용할 수도 있습니다.
1. ![저장](assets/save_icon.svg)을 눌러 속성을 저장합니다.

선택한 패널이 다음 페이지로 이동합니다.

## 여러 마스터 페이지를 기록 문서에 적용 {#apply-multiple-master-pages-dor}

선택한 사용자 지정 XDP 템플릿에 여러 개의 마스터 페이지가 포함되어 있으면 해당 페이지의 속성이 [!UICONTROL 레코드 문서] 탭의 [!UICONTROL content] 섹션에 표시됩니다. 자세한 내용은 [레코드](#customize-the-branding-information-in-document-of-record)의 문서에서 브랜딩 정보 사용자 지정을 참조하십시오.

적응형 양식의 구성 요소에 다른 마스터 페이지를 적용하여 기록 문서에 여러 마스터 페이지를 적용할 수 있습니다. 여러 마스터 페이지를 적용하려면 레코드 속성 문서의 [페이지 매김](#document-of-record-settings) 섹션을 사용합니다.

다음은 여러 마스터 페이지를 기록 문서에 적용하는 방법의 예입니다.
4개의 마스터 페이지를 포함하는 XDP 템플릿을 [!DNL AEM Forms] 서버에 업로드합니다. [!DNL AEM Forms] 기본적으로 템플릿 속성을 레코드 문서에 적용합니다. [!DNL AEM Forms] 또한 템플릿의 첫 번째 마스터 페이지 속성을 레코드 문서에 적용합니다.

두 번째 마스터 페이지 속성을 패널에 적용하고 다음에 오는 패널에 세 번째 마스터 페이지 속성을 적용하려면 다음 단계를 수행합니다.

1. 패널을 탭하여 두 번째 마스터 페이지를 적용하고 ![구성](assets/configure-icon.svg)을 선택합니다.
1. **[!UICONTROL 페이지 매김]** 섹션에서 **[!UICONTROL 위치]** 필드에서 ![폴더](assets/folder-icon.svg)를 탭합니다.
1. **[!UICONTROL 페이지]**&#x200B;에서 두 번째 마스터 페이지를 선택하고 **[!UICONTROL 선택]**을 누릅니다.
AEM Forms에서는 두 번째 마스터 페이지를 패널에 적용하고 적응형 양식의 모든 후속 패널을 적용합니다.
1. **[!UICONTROL 페이지 매김]** 섹션에서 **[!UICONTROL After]** 필드에서 ![폴더](assets/folder-icon.svg)를 탭합니다.
1. **[!UICONTROL 페이지로 이동]**&#x200B;을 탭하고 세 번째 마스터 페이지를 선택하고 **[!UICONTROL 선택]**&#x200B;을 누릅니다.
1. ![저장](assets/save_icon.svg)을 눌러 속성을 저장합니다.
