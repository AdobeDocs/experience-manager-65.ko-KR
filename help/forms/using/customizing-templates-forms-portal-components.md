...
## 사용자 지정 템플릿 {#creating-a-nbsp-custom-template} 만들기
...
반복 가능한 항목을 포함하려면 **data-repeable** 특성의 값을 **true**&#x200B;로 구성하십시오.

*설명한 예에서 사용자 지정 템플릿의 맨 위에 두 개의 Div 요소가 있습니다. 첫 번째 클래스는 &quot;__FP_boxes-container&quot; CSS 클래스를 사용하여 나열된 양식의 컨테이너 요소로 작동합니다. 두 번째 CSS 클래스는 &quot;__FP_boxes&quot; CSS 클래스를 사용하는 것으로, 이 경우에는 Form입니다. Div 요소에 있는&#x200B;**data-repeable**특성에 값&#x200B;**true**이 있습니다.*

각 자리 표시자에는 전용 OOTB 메타데이터 세트가 있습니다. 양식의 특정 위치에 사용자 지정 메타데이터를 표시하려면 위치에 **${metadata_prop} 속성**&#x200B;을 추가합니다.

*이 예제에서 메타데이터 속성은 여러 인스턴스에서 사용됩니다. 예를 들어&#x200B;**description**,**name**,**formUrl**,**htmlStyle**,**pdfUrl**,**pdfStyle**및 &lt;a111/>에서 사용됩니다. a12/>경로&#x200B;**에 지정된 방법***
...
* **htmlStyle**&amp;  **pdfStyle**:렌더링을 위해 각각 사용되는 HTML 및 PDF 아이콘의 스타일을 표시합니다. 유효한 값은 &quot;**__FP_display_none**&quot;이거나 비어 있습니다.
...
1. **로컬라이제이션 지원**:정적 텍스트를 현지화하려면 속성을 사용하고 현지화된 값 `${localize-YOUR_TEXT}` 을 사용할 수 있도록 하십시오(아직 존재하지 않음).
...
1. **구성 속성 사용**:검색 및 목록 구성 요소에는 사용자 인터페이스에서 사용할 수 있는 여러 구성이 있습니다. 예를 들어 편집 대화 상자를 통해 저장된 HTML 도구 설명 텍스트를 표시하려면 `${config-htmlLinkText}` 특성을 사용합니다. **마찬가지로 PDF 툴팁 텍스트의 경우 속성** `${config-pdfLinkText}` 을 사용하십시오.
...
1. 초안 및 제출 구성 요소 아래의 초안 섹션에 있는 삭제 옵션에 대해 CSS 클래스 이름 &quot;__FP_deleteDraft&quot;를 지정합니다. 또한 해당 초안의 초안 ID인 **${draftID}** 값과 함께 &quot;draftID&quot; 속성을 포함하십시오.

1. 초안 및 제출 서류를 열기 위한 링크를 만드는 동안 앵커 태그에 대한 **href** 특성의 값으로 **${path}.html**&#x200B;을(를) 지정할 수 있습니다.

![초안 및 제출 노드](assets/raw-image-with-index.png)

**A**. 컨테이너 요소

**각 양식에 대해 저장된 축소판을 가져오기 위해 고정 계층 구조를 갖는 B.** &quot;경로&quot; 메타데이터입니다.
...
1. **data-** repeatableone은  **** Linkcomponent에 아무런 의미가 없습니다. Adobe에서는 링크 구성 요소의 템플릿에서 이 속성을 사용하지 않는 것이 좋습니다.
