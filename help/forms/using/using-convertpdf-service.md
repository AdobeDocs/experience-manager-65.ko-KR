...
유일한 차이점은 JSP/Servlet에서 서비스를 실행하는 것뿐입니다. 문서 객체는 ResourceResolverHelper 객체에서 ResourceResolver 객체의 인스턴스를 자동으로 검색합니다. 이 자동 메커니즘은 워크플로우에서 코드를 호출할 때 작동하지 않습니다. 워크플로의 경우 ResourceResolver 개체의 인스턴스를 명시적으로 Document 클래스 생성자에 전달합니다. 그런 다음 문서 객체가 ResourceResolver 객체를 사용하여 보관소의 컨텐츠를 읽었습니다.
