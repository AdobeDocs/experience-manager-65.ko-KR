...
description: AEM Forms 작업 영역 사용자 인터페이스를 맞춤화하는 방법
...
맞춤화를 수행하는 일반 단계는 다음과 같습니다.

1. 에 액세스하여 CRXDE Lite에 로그인 `https://'[server]:[port]'/lc/crx/de/index.jsp`.
1. 만들기 `sling:Folder` 폴더 이름: `ws` 위치: `/apps`: 존재하지 않는 경우입니다. 을(를) 만들려면 `sling:Folder` 폴더를 마우스 오른쪽 단추로 클릭하고 `apps` 폴더 및 선택 **[!UICONTROL 만들기]** > **[!UICONTROL 노드 만들기]**. 이름을 다음으로 지정 `ws`, 다음으로 유형 선택 `sling:Folder` 및 클릭 **[!UICONTROL 확인]**. 클릭 **[!UICONTROL 모두 저장]**.
1. 다음으로 이동 `/apps/ws`로 이동한 다음 **[!UICONTROL 액세스 제어]** 탭.
1. 다음 항목 선택 **[!UICONTROL 저장소]** 옵션을 선택합니다. 다음에서 **[!UICONTROL 액세스 제어]** 목록, 클릭 **[!UICONTROL +]** 새 항목을 추가합니다. 클릭 **[!UICONTROL +]** 다시.
...
1. 다음을 복사합니다. `/libs/ws/locales` 폴더의 폴더 `/apps/ws` 폴더를 삭제합니다. 클릭 **[!UICONTROL 모두 저장]**.
...
1. CSS 사용자 정의에 대해 다음을 수행합니다.
   ...
   1. 다음 위치로 이동 `/apps/ws` 폴더 및 (이)라는 새 폴더 만들기 `css`.

   1. 다음에서 `css` 폴더, (이)라는 이름의 새 파일 만들기 `newStyle.css`.
...
1. 다음 작업을 수행합니다.
   ...
   1. 다음 이름의 폴더 만들기 `js` 위치: `/apps/ws`. 클릭 **[!UICONTROL 모두 저장]**.

   1. 다음 이름의 폴더 만들기 `libs` 위치: `/apps/ws/js`. 클릭 **[!UICONTROL 모두 저장]**.

   1. 복사 `/libs/ws/js/libs/jqueryui` 폴더 위치: `/apps/ws/js/libs`. 클릭 **[!UICONTROL 모두 저장]**.

1. HTML 사용자 정의에 대해 다음을 수행합니다.

   1. 아래 `/apps/ws/js`, 폴더 만들기 `runtime`. 클릭 **[!UICONTROL 모두 저장]**.

   1. 아래 `/apps/ws/js/runtime`, 폴더 만들기 `templates`. 클릭 **[!UICONTROL 모두 저장]**.

   1. 복사 `/libs/ws/js/main.js` 끝 `/apps/ws/js/main.js`.

   1. /libs/ws/js/registry.js을 다음으로 복사 `/apps/ws/js/registry.js`.

1. 클릭 **[!UICONTROL 모두 저장]**, 캐시 지우기 및 AEM Forms 작업 영역 새로 고침.
