...
# Content Sync를 사용하는 모바일{#mobile-with-content-sync}
...
컨텐츠 동기화를 사용하여 기본 모바일 애플리케이션에서 사용할 수 있도록 컨텐츠 패키지화합니다. Adobe Experience Manager(AEM)으로 작성된 페이지는 디바이스가 오프라인된 균일 앱 컨텐츠로 사용할 수 있습니다. 또한 AEM 페이지는 웹 표준을 기반으로 하므로 크로스 플랫폼 간에 작동하므로 모든 기본 래퍼에 포함할 수 있습니다. 이 전략은 개발 노력을 줄이고 앱 컨텐츠 콘텐츠를 쉽게 업데이트할 수 있도록 합니다.

컨텐츠 동기화 프레임워크는 웹 컨텐츠 파일을 포함하는 아카이브 파일을 만듭니다. 컨텐츠 형식은 간단한 페이지, 이미지, PDF 파일 또는 전체 웹 응용 프로그램 등 무엇이든 될 수 있습니다. Content Sync API는 모바일 앱 또는 빌드 프로세스에서 아카이브 파일에 대한 액세스를 제공하므로 컨텐츠를 검색하고 앱에 포함할 수 있습니다.

다음 단계 시퀀스 는 컨텐츠 동기화에 대한 일반적인 사용 사례를 보여줍니다.
...
1. 클라이언트가 포함된 브라우저에서 컨텐츠 열기 또는 다른 방법으로 이 컨텐츠를 사용합니다.
1. 나중에 클라이언트는 서버에서 업데이트된 컨텐츠 콘텐츠를 요청합니다. 컨텐츠 동기화 프레임워크는 다운로드 크기와 시간을 줄이기 위해 증분 업데이트를 제공하며, 이는 제한된 대역폭 또는 데이터 볼륨으로 인해 모바일 디바이스에 중요할 수 있습니다.

## 컨텐츠 동기화 처리기 개발 {#developing-the-content-sync-handlers}

컨텐츠 동기화 핸들러 개발에 대한 몇 가지 지침은 다음과 같습니다.
...
AEM이 설치되면 관리자 그룹의 구성원은 기본적으로 컨텐츠 동기화에서 다운로드 할 수 있습니다.
...
#### Content Sync 캐시에 대한 다운로드 액세스 재정의 {#overriding-download-access-for-a-content-sync-cache}

특정 컨텐츠 동기화 구성에 대한 다운로드 액세스를 구성하려면 노드에 다음 속성을 추가합니다.`cq:ContentSyncConfig`

* 이름: authorizable
* 유형: 문자열
* 값: 다운로드할 수 있는 사용자 또는 그룹의 이름입니다.

예를 들어 앱을 사용하면 사용자가 Content Sync에서 직접 업데이트를 설치할 수 있습니다. 모든 사용자가 업데이트를 다운로드 할 수 있도록하려면 승인 가능한 속성 `everyone`값을 로 설정합니다.

노드에 `cq:ContentSyncConfig` 승인 가능한 속성이 없는 경우 CQ Content Sync Manager 서비스의 대체 캐시 승인 가능 속성에 대해 구성된 기본 사용자 또는 그룹에 따라 다운로드할 수 있는 사용자가 결정됩니다.

### 컨텐츠 동기화 캐시를 업데이트하도록 사용자 구성 {#configuring-the-user-for-updating-a-content-sync-cache}
...
* 이름: `updateuser`
* 유형: `String`
* 값: 업데이트를 수행할 수 있는 사용자 또는 그룹 이름입니다.

노드에 `cq:ContentSyncConfig` 속성이 없는 `updateuser` 경우 기본 `anonymous` 사용자가 캐시를 업데이트합니다.

### 구성 유형 {#configuration-types}

처리는 간단한 JSON 렌더링에서 참조된 자산을 포함한 페이지의 본격적인 렌더링에 이르기까지 다양할 수 있습니다. 이 섹션에는 사용 가능한 구성 유형 및 특정 매개 변수가 나열되어 있습니다.

**복사** - 파일 및 폴더를 복사합니다.

* **path** - 경로가 단일 파일을 가리키는 경우 파일만 복사됩니다. 폴더(페이지 노드 포함)를 가리키면 아래의 모든 파일과 폴더가 복사됩니다.
...
* **path** - 클라이언트 라이브러리의 루트 경로입니다.
* **extension** - 클라이언트 라이브러리의 유형입니다. 현재 js *또는* css *로*&#x200B;설정해야 합니다.
...
* **includeImages** - 이미지를 포함해야 하는지 여부를 결정하는 선택적 부울 속성. 기본값은 입니다. *true*.
  ...
  기본적으로 리소스 유형이 foundation/components/image인 이미지 구성 요소만 포함하는 것으로 간주됩니다. 웹 콘솔에서 일 CQ WCM 페이지 업데이트 처리기&#x200B;**를**&#x200B;구성하여 리소스 유형을 더 추가할 수 있습니다.

**rewrite** - rewrite 노드는 내보낸 페이지에서 링크가 다시 작성되는 방법을 정의합니다. 다시 작성된 링크는 zip 파일에 포함된 파일 또는 서버의 리소스를 가리킬 수 있습니다.

노드는 `rewrite` 노드 아래에 `page` 있어야 합니다.
...
**etc.designs.default 및 etc.designs.mobile** - 구성의 처음 두 항목은 분명합니다. 여러 모바일 페이지를 포함하려면 /etc/designs 아래에 관련 디자인 파일이 필요합니다. 그리고 추가 처리가 필요하지 않기 때문에 복사로 충분합니다.

**events.plist** - 이 항목은 조금 특별합니다. 소개에서 언급했듯이 애플리케이션는 이벤트 위치의 마커가 있는 맵 보기 제공해야 합니다. 필요한 위치 정보는 PLIST 포맷 형식의 별도 파일로 제공됩니다. 이를 위해 인덱스 페이지에서 사용되는 이벤트 목록 구성 요소에는 plist.jsp라는 스크립트가 있습니다. 이 스크립트는 구성 요소의 리소스가 확장과 `.plist` 함께 요청될 때 실행됩니다. 평소와 같이 구성 요소 경로는 경로 속성에 지정되고 유형은 Sling 요청 처리를](/help/sites-developing/the-basics.md#sling-request-processing) 사용하기 [위해 컨텐츠로 설정됩니다.

**** events.touch.html - 다음 앱에 표시되는 실제 페이지가 표시됩니다. 경로 속성은 이벤트의 루트 페이지 로 설정됩니다. deep 속성 기본값이 true로 설정되므로 해당 페이지 아래의 모든 이벤트 페이지도 포함됩니다. 페이지를 구성 유형으로 사용하면 페이지의 이미지 또는 다운로드 구성 요소에서 참조할 수 있는 모든 이미지 또는 기타 파일이 포함됩니다. 또한 터치 선택기 설정을 통해 페이지의 모바일 버전을 얻을 수 있습니다. 기능 팩의 구성에는 이러한 종류의 항목이 더 많이 포함되어 있지만 여기서는 간단히 생략합니다.

**logo** - 로고 구성 유형은 지금까지 언급되지 않았으며 빌드 유형이 아닙니다. 그러나 컨텐츠 동기화 프레임워크는 어느 정도 확장 가능하며, 이는 다음 섹션에서 다룰 예제입니다.
...
구성이 설정되면 브라우저 또는 기타 HTTP 클라이언트를 사용하여 컨텐츠 콘텐츠를 다운로드하거나 iOS용으로 개발하는 경우 전용 WAppKitSync 클라이언트 라이브러리를 사용할 수 있습니다. 다운로드 위치는 구성의 경로와 *.zip* 확장으로 구성됩니다(예: 로컬 AEM 인스턴스로 작업할 때). *http://localhost:4502/content/weretail_go.zip*

### 컨텐츠 동기화 콘솔 {#the-content-sync-console}

컨텐츠 동기화 콘솔에는 저장소(유형의 `cq:ContentSyncConfig`모든 노드)에 있는 모든 컨텐츠 동기화 구성이 나열되며, 각 구성에 대해 다음을 수행할 수 있습니다.

* 캐시를 업데이트합니다.
* 캐시를 지웁니다.
* 전체 zip을 다운로드합니다.
* 지금과 특정 날짜 및 시간 사이의 diff zip을 다운로드합니다.
...
구성 옵션의 수는 이미 광범위하지만 특정 사용 사례의 모든 요구 사항을 다루지 않을 수 있습니다. 이 섹션에서는 컨텐츠 동기화 프레임워크의 확장 지점 및 사용자 지정 구성 유형을 만드는 방법에 대해 설명합니다.

각 구성 유형에는 해당 특정 유형에 대해 등록된 OSGi 구성 요소 팩토리인 컨텐츠 업데이트 처리기&#x200B;*가 있습니다*. 이러한 처리기는 컨텐츠 수집 및 처리, 컨텐츠 동기화 프레임워크에서 유지 관리하는 캐시에 추가합니다. 다음 인터페이스 또는 추상 기본 클래스를 구현합니다.
...
사용자 정의 핸들러를 구현하려면 먼저 구성 항목에 제공된 자원을 기반으로 이미지 클래스의 인스턴스 작성하십시오. 이것은 페이지의 실제 로고 구성 요소가 수행하는 것과 동일한 절차입니다. 이미지의 타겟 경로가 페이지에서 참조된 경로와 동일한지 확인합니다.

다음, 마지막 업데이트 이후 리소스가 수정되었는지 확인하십시오. 사용자 지정 구현은 불필요한 캐시 업데이트를 방지하고 변경 사항이 없으면 false를 반환해야 합니다. 리소스가 수정된 경우 캐시 루트를 기준으로 예상되는 타겟 위치에 이미지를 복사합니다. 마지막으로, `true` 는 캐시가 업데이트되었음을 프레임워크에 표시하기 위해 반환됩니다.
...
전송된 모든 데이터는 동일한 디렉터리 구조로 추출할 수 있으며 데이터를 추출할 때 추가 단계(예: 종속성 검사)가 필요하지 않습니다. iOS가 있는 경우 모든 데이터는 iOS 앱 문서의 문서 폴더 내 하위 폴더에 저장됩니다.

iOS 기반 AEM Mobile 앱의 일반적인 실행 경로는 다음과 같습니다.

* 사용자가 iOS 디바이스 장치에서 앱을 시작합니다.
* 앱이 AEM 백엔드에 연결을 시도하고 마지막 실행 이후 데이터 변경을 요청합니다.
* 서버는 해당 데이터를 검색하여 파일로 압축합니다.
* 데이터는 클라이언트 디바이스 서버로 반환되어 문서 폴더에 추출됩니다.
* UIWebView 구성 요소가 시작/새로 고쳐집니다.

이전에 연결할 수 없는 경우 다운로드한 데이터가 표시됩니다.

### 추가 리소스 {#additional-resources}

관리자와 작성자 역할과 책임에 대해 알아보려면 아래 리소스를 참조하십시오.
