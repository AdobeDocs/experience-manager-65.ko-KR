...
마지막으로 이 모듈은 자신을 포함하여 각 하위 페이지를 반복하고, angular-route-fragment.js 선택기 및 확장을 통해 Angular의 \$routeProvider에 대한 구성 항목으로 포함하여 각 페이지의 경로 조각 콘텐츠를 렌더링합니다. 즉, \$routeProvider는 지정된 경로가 요청될 때 렌더링할 콘텐츠를 앱에 알려줍니다.
...
문제를 종합하기 위해 PhoneGap 애플리케이션은 단일 페이지 앱(SPA) 패턴을 사용하여 기본 URI(해시 제외)가 변경되지 않도록 합니다. 따라서 **을(를) 참조하는 모든 에셋, 템플릿 또는 스크립트는 최상위 페이지를 기준으로 해야 합니다.** 최상위 페이지는 `*<name>*.angular-app-module.js` 및 `*<name>*.angular-app-controllers.js`을(를) 통해 Angular 라우팅 및 컨트롤러를 초기화합니다. 이 페이지는 sling:redirect를 *확장하지 *않는 저장소 루트와 가장 가까운 페이지여야 합니다.
