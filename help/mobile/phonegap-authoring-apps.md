...
앱 타일 관리 에는 애플리케이션 아이콘, 이름, 설명, 지원되는 플랫폼, 업데이트 URL 및 버전 정보를 위한 Call Home 이 표시됩니다. 이 타일로 드릴다운하여 PhoneGap 응용 프로그램 구성(config.xml)을 편집하고 유지 관리하고, 응용 프로그램을 배포용 다양한 응용 프로그램 저장소에 제출할 준비를 할 수 있습니다.
...
AEM Sites 내에서 동일한 방식으로 컨텐츠를 만들고, 업데이트하고, AEM Mobile에서 삭제할 수 있습니다. 다음 **페이지 컨텐츠 타일 관리** 관리되는 컨텐츠와 마지막으로 수정된 페이지의 수를 표시합니다. 타일에서 각 레코드를 클릭하여 컨텐츠를 드릴다운하여 페이지를 생성, 복사, 이동, 삭제 및 업데이트할 수 있습니다. 콘텐츠가 업데이트되면 을 통해 콘텐츠 업데이트를 고객에게 푸시할 수 있습니다. **콘텐츠 패키지 관리 타일.**
...
컨텐츠 패키지를 사용하면 AEM 앱 작성자가 AEM에서 페이지 컨텐츠를 관리하고 개발팀이 PhoneGap 셸 애플리케이션(즉, 앱 프레임워크 또는 인프라)을 변경한 다음 개발자를 등록하여 배포하기 위해 여러 저장소에 다시 제출하지 않고도 신속하게 고객에게 변경 사항을 푸시할 수 있습니다.

컨텐츠 패키지는 각 업데이트에 대해 컨텐츠 릴리스 패키지로 간주되는 ZIP 파일을 만듭니다. 이러한 패키지에는 앱을 렌더링하는 동안 생성된 html 리소스 및 html 페이지가 들어 있으며, 마지막 업데이트 이후 수정된 파일만 패키지할 수 있을 만큼 지능적입니다.

컨텐츠 패키지 관리 타일의 **유형** 열에는 애플리케이션 셸 컨텐츠를 나타내는 &#39;앱&#39;이 표시되며, 예를 들어 개발자가 관리하는 앱의 프레임워크 또는 인프라에서는 컨텐츠 작성자가 관리하는 페이지 컨텐츠를 나타내는 &#39;컨텐츠&#39;가 표시됩니다.

콘텐츠는 언어로 표현되거나 앱에서 여러 컨텐츠 릴리스 패키지를 사용하는 앱의 특정 부분으로 표시될 수 있습니다. 컨텐츠 번들 방법 중 선택은 유연하며 애플리케이션의 컨텐츠를 관리하는 방법에 전적으로 달려 있습니다.

다음 **수정됨** 열은 페이지가 가장 최근에 수정된 시기를 나타냅니다.

다음 **스테이징됨** 열에는 마지막 컨텐츠 업데이트가 생성된 시점이 표시됩니다. 컨텐츠 업데이트를 만들고 변경 사항을 스테이징하려면 타일에서 레코드를 열고 업데이트를 만듭니다.

다음 **게시됨** 열에는 마지막 컨텐츠 업데이트가 게시되어 고객이 소비할 수 있도록 된 시점이 표시됩니다. 컨텐츠를 게시하려면 먼저 해당 컨텐츠를 스테이징한 다음 이 타일로 드릴다운하고 컨텐츠 릴리스 세부 사항 콘솔에서 게시하여 업데이트를 게시해야 합니다.
...
다음 **PhoneGap Build 타일** 연결 대상 [https://build.phonegap.com](https://build.phonegap.com) 원격 빌드를 빌드 및 호스팅하려면 다음을 수행하십시오. 빌드되면 빌드를 다운로드로 사용하거나 QR 코드를 통해 장치에 직접 제공할 수 있습니다.
...
AEM Mobile은 를 통해 Adobe Analytics과 통합됩니다 [Adobe Mobile Services SDK](https://experienceleague.adobe.com/docs/mobile.html?lang=en) (AMS).
