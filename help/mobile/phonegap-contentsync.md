...
* **표현물**  - 유형은 사용자가 기본 이미지 대신 사용할 표현물을 지정할 수 있도록 해주는 문자열 배열입니다. 다음 목록에는 기본 제공 표현물이 요약되어 있지만 워크플로우에서 만든 렌디션을 사용할 수도 있습니다.
...
구성이 설정되면 바로 브라우저나 다른 HTTP 클라이언트로 컨텐츠를 다운로드할 수 있고 iOS용으로 개발하는 경우에는 전용 WAppKitSync 클라이언트 라이브러리를 사용할 수 있습니다. 다운로드 위치는 구성의 경로 및 *.zip* 확장으로 구성됩니다(예: 로컬 AEM 인스턴스 작업 시).*https://localhost:4502/content/weretail_go.zip*
