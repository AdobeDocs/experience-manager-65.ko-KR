...
cq:ContentSyncConfig 노드에 이 없는 경우 `updateuser` 기본 익명 사용자는 속성을 사용하여 캐시를 업데이트합니다.
...
**assets** - 에셋의 원본 렌디션을 수집합니다.

* **경로** - /content/dam 아래의 자산 폴더 경로.
* **렌디션** - Type은 사용자가 기본 이미지 대신 사용할 표현물을 지정할 수 있는 문자열 배열입니다. 다음 목록은 일부 기본 변환을 요약하지만, 워크플로우에서 만든 모든 변환을 사용할 수도 있습니다.
...
**events.plist** - 이 항목은 좀 특별합니다. 서론에서 언급했듯이 애플리케이션은 이벤트 위치의 마커가 포함된 맵 보기를 제공해야 합니다. 우리는 필요한 위치 정보를 PLIST 포맷의 별도의 파일로 제공할 것입니다. 이를 위해 인덱스 페이지에서 사용되는 이벤트 목록 구성 요소에 plist.jsp라는 스크립트가 있습니다. 이 스크립트는 구성 요소의 리소스를 `.plist` 확장명. 평소대로 구성 요소 경로는 경로 속성에 지정되며 유형은 Sling 요청 처리를 사용하려고 하므로 콘텐츠로 설정됩니다.
...
구성이 설정되면 콘텐츠는 브라우저나 다른 HTTP 클라이언트를 통해 다운로드할 수 있습니다. 또는 iOS용으로 개발하는 경우 전용 WAppKitSync 클라이언트 라이브러리를 사용할 수 있습니다. 다운로드 위치는 구성의 경로 및 *.zip* 확장(예: 로컬 AEM 인스턴스로 작업하는 경우): *https://localhost:4502/content/weretail_go.zip*
