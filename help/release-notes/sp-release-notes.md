...
* 이제 AEM [워크플로우에서 인쇄 가능한](../forms/using/aem-forms-workflow-step-reference.md) 출력 생성 단계를 사용할 수 있습니다.
...
* [태그 지정] 태그 지정 태그 지정 태그 검색 태그 필드에는 루트 경계 밖에 있는 태그가 표시되고 태그 필드의 `rootPath` 속성은 준수하지 않습니다(NPR-31895).

* [플랫폼 UI] 텍스트 필드에 잘못된 경로가 추가되면 경로 브라우저가 중단됩니다(NPR-31884).
...
* 디자이너:태그 지정 옵션을 활성화하면 생성된 PDF 출력(NPR-32547)에서 하위 폼 테두리가 사라집니다.

* 디자이너:태그 지정 옵션을 활성화하면 생성된 PDF 출력(NPR-31983, NPR-31950)에서 하위 폼 테두리가 사라집니다.
