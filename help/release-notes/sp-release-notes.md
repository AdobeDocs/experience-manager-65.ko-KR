...
* 페이지 편집기, 핵심 구성 요소, RTE 및 관리 사용자 인터페이스와 같은 Experience Manager 웹 콘텐츠 관리(WCM)의 다양한 영역에서 액세스 가능성이 개선되었습니다.

* 초안을 [!DNL Interactive Communication] 저장합니다.

* JEE에서 Experience Manager Forms [!DNL Oracle WebLogic 12] 에 대한 지원
...
* Adobe Experience Manager 내에서 패키지 공유 통합이 제거되었습니다.
...
* Adobe Experience Manager Sites는 해당 별칭에서 페이지를 게시 또는 게시 취소할 수 있는 옵션을 제공합니다. 옵션이 작동하지 않습니다(NPR-33415).
* 여러 템플릿이 들어 있는 템플릿에서 레이아웃 컨테이너를 삭제하면 템플릿이 올바르게 렌더링되지 않습니다(NPR-33347).
* Adobe Experience Manager Sites 페이지가 여러 Live Copy가 포함된 대규모 콘텐츠 집합의 일부인 경우 페이지 버전 내역 미리 보기가 로드되지 않습니다(NPR-33311).
* 이동 명령을 사용하여 Experience Manager Sites 페이지의 이름을 변경해도 페이지 제목이 업데이트되지 않습니다(NPR-33264).
...
* 페이지 리디렉션 서블릿은 ResourceType이 아닌 Experience Manager Sites URL에 .html `cq:Page` 을 추가합니다(NPR-33176).
...
* 기본 경험 조각 구성 요소를 사용하여 사용자 지정 구성 요소를 만들고 Experience Manager 사이트 페이지에서 사용하는 경우, Experience Manager는 사용자 지정 구성 요소의 참조(사용)를 표시하지 않습니다(NPR-32852).
...
* Experience Manager Sites 편집기 페이지를 로드하는 데 15초 이상 걸립니다(NPR-32413).
* 특정 필드에는 상속 취소 옵션이 표시되지 않습니다(NPR-32362).
* 경험 조각 구성 요소의 경로를 선택하고 선택 대화 상자 열기 확인란을 선택하면 경로 브라우저에서 선택한 경로로 이동하지 않습니다(NPR-32308).
* Experience Manager 6.2에서 Experience Manager 6.5로 업그레이드하면 정적 템플릿의 Parsys 구성 요소가 제대로 표시되지 않습니다. Parsys 구성 요소의 높이는 0으로 설정되며 구성 요소 내의 구성 요소는 표시되지 않습니다(NPR-33663).
...
* Dynamic Media Scene7 실행 모드의 Experience Manager에서 기본적으로 [!UICONTROL 다이내믹 미디어 동기화 모드를] 비활성화하면 이미지 사전 설정이 게시되지 않습니다(CQ-4294200).

* 벌크 업로드가 중단되는 동안 에셋 처리가 중단되고 워크플로우 인스턴스에 DAM 업데이트 에셋의 고정 인스턴스가 표시됩니다(CQ-4293916).

* Experience Manager에서 다이내믹 미디어 구성을 만드는 것은 작동하지만 사용자 인터페이스에서 저장을 선택하면 아무 일도 일어나지 않습니다(CQ-4292442).
...
* Experience Manager SDL 패키지(NPR-33175)를 설치하는 동안 오류 메시지가 표시됩니다.

### 플랫폼 {#platform-6550}

* 맵 항목 [!DNL Sling] 이 (NPR-33362)에서 만들어진 경우 `sling:match` 필터가 호출되지 `/etc/maps` 않습니다.
* Adobe Experience Manager가 세그먼테이션 장애로 인해 [!DNL Apache Lucene] (NPR-32988) 충돌이 발생했습니다.
...
* 예를 들어 7시간 동안 SQL 쿼리가 실행될 때 Experience Manager가 응답을 중지합니다(NPR-33044).
...
* 번역된 태그는 타사 번역 서비스에서 Experience Manager로 가져오지 않습니다(NPR-33154).
...
* Adobe Experience Manager Inbox에서 메시지를 액세스해도 메시지의 상태가 읽음으로 변경되지 않습니다(NPR-32764).
...
* Experience Manager 인스턴스를 다시 시작한 후 컬렉션에 대한 검토 작업에 대한 이메일에 잘못된 페이로드 링크가 포함되어 있습니다(NPR-32774).
...
두 가지 방법으로 작업 인스턴스에 Adobe Experience Manager 6.5.5.0을 자동으로 설치할 수 있습니다.
...
>[!NOTE]
>
>AEM Forms를 사용하지 않는 경우 건너뜁니다. Adobe Experience Manager Forms의 수정 사항은 별도의 추가 기능 패키지를 통해 제공됩니다.
...
>[!NOTE]
>
>JEE에서 AEM Forms를 사용하지 않는 경우 건너뜁니다. JEE에서 Adobe Experience Manager Forms에 적용된 수정 사항은 별도의 설치 관리자를 통해 제공됩니다.
...
| 영역 | 기능 | 대체 |
|---|---|---|
| 통합 | The **[!UICONTROL AEM Cloud Services Opt-In]** screen is deprecated. AEM 6.5에서 Adobe IMS 및 I/O를 통해 인증을 사용하는 Target 표준 API를 지원하고 분석 및 개인 설정을 위해 AEM 페이지를 계측하는 Adobe Launch의 늘어나는 역할을 지원하도록 AEM 및 Target 통합이 업데이트되어 옵트인 마법사가 기능상 무관해졌습니다. | 해당 AEM 클라우드 서비스를 통해 시스템 연결, Adobe IMS 인증 및 Adobe I/O 통합을 구성합니다. |
...
* AEM 6.5.x.x를 설치하는 동안 다음 오류 및 경고 메시지가 표시될 수 있습니다.
   ...
   * `com.adobe.granite.maintenance.impl.TaskScheduler` - granite/operations/maintenance에 유지 관리 창이 없습니다.
   * 쇼퍼블 배너 뷰어를 통해 자산을 미리 볼 때 다이내믹 미디어 대화형 이미지의 핫스팟이 표시되지 않습니다.
