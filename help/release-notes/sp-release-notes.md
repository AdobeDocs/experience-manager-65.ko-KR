...
* [!DNL AEM 6.5 Forms]을 설치하면 다음 타사 라이브러리가 자동으로 설치됩니다(CQDOC-18373).
...
* HTML 편지 미리 보기뿐만 아니라 [!UICONTROL 데이터] 탭에 문자 표시가 지연됩니다(NPR-37020).
...
* CRX DE 저장소에서 `mfAllowAttachments` 속성 값을 `True`로 설정하면 HTML5 양식 제출 시 `dataXml`이 손상되었습니다(NPR-37035).

* `dataXml`을 사용하여 XDP를 HTML으로 렌더링하면 [!DNL AEM Forms]에 `Page Unresponsive` 오류가 표시됩니다(NPR-36631).
...
JEE의 Experience Manager Forms용 누적 설치 프로그램 설치 및 배포 후 구성에 대한 자세한 내용은 [릴리스 노트](jee-patch-installer-65.md)를 참조하십시오.

>[!NOTE]
>
>JEE의 Experience Manager Forms용 누적 설치 프로그램을 설치한 후 최신 Forms 추가 기능 패키지를 설치하고 `crx-repository\install` 폴더에서 Forms 추가 기능 패키지를 삭제한 다음 서버를 다시 시작합니다.
...
| 영역 | 기능 | 대체 |
|---|---|---|
| 통합 | **[!UICONTROL AEM 클라우드 서비스 옵트인]** 화면은 [!DNL Experience Manager] 및 [!DNL Adobe Target] 통합이 Experience Manager 6.5에서 업데이트되므로 더 이상 사용되지 않습니다. 통합은 Adobe Target Standard API를 지원합니다. API는 Adobe IMS 및 [!DNL Adobe I/O]을 통해 인증을 사용하며, 분석 및 개인화를 위해 [!DNL Experience Manager] 페이지를 계측하는 Adobe Launch의 늘어나는 역할을 지원합니다. 옵트인 마법사는 기능상 무관합니다. | 각 [!DNL Experience Manager] 클라우드 서비스를 통해 시스템 연결, Adobe IMS 인증 및 [!DNL Adobe I/O] 통합을 구성합니다. |
...
* Adobe 고객 지원에 문의하는 방법](https://experienceleague.adobe.com/docs/customer-one/using/home.html)을 참조하십시오.[
