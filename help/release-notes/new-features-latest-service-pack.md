...
2020년 Adobe Experience Manager(AEM) 6.5의 경우 새로운 기능과 향상된 기능이 분기별 서비스 팩에 제공됩니다. 고객은 이러한 새로운 방식을 통해 혁신을 신속하게 채택할 수 있습니다.
...
* 사이트(contexthub.kernel.js)에서 ContextHub를 로드하고 초기화하는 데 걸리는 시간을 줄입니다. 사이트 방문 중에 첫 번째 페이지를 빠르게 로드합니다.

* 페이지 편집기에서 경험 조각을 페이지 캔버스로 드래그하여 놓은 후 페이지를 새로 고치는 시간을 줄입니다.

* Live Copy 개요에서 사이트에 Live Copy가 많이 있을 때 항목을 로드하는 데 걸리는 시간을 단축합니다(+200).

* 템플릿 편집기에서 템플릿 편집기의 속도가 느려질 수 있는 불완전하거나 잘못된 URL의 처리를 개선합니다.

또한 AEM 6.5 SP4부터 스타일 시스템이 향상되었으며, 이제 구성 요소 대화 상자 내에서 스타일을 선택할 수도 있습니다.
...
* 이제 선택적 컨텐츠 하위 트리만 모든 하위 트리가 아닌 Scene7에 동기화할 수 `content/dam`있습니다.
