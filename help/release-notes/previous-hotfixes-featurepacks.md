...
* 멀티 테넌트:폴더 속성을 저장하는 임차인은 성공 프롬프트 및 작업 설명 오류 메시지를 모두 확인합니다. &quot;속성을 편집할 수 없습니다. 권한이 부족합니다.&quot;를 모두 관찰하면 혼동됩니다. NPR-30545: CQ-4275333용 핫픽스
* [자산 선택기] 대화 상자에서는 자산을 선택할 수 없으므로 관련 소스 교체 기능을 사용하여 소스를 업데이트할 수 없습니다. NPR-30502: CQ-4275029용 핫픽스
* DAM 자산 업데이트 워크플로우 - 크기가 큰 mp4 파일을 업로드할 때 사용되지 않는 상태입니다. NPR-30480: CQ-4271352용 핫픽스
...
* 사용자를 선택하고 [추가]를 클릭하여 사용자 선택기의 자산 기여 폴더에 기여자를 추가할 수 없습니다. CQ-4277824, CQ-4278087용 핫픽스
...
* 뷰어 - VideoViewer:포스터=none이 사용되는 외부 비디오의 경우 제대로 작동하지 않습니다. CQ-4265536용 핫픽스
* IE11 및 MS Edge 브라우저에서 비디오 재생 중에 대기 아이콘이 표시됩니다. CQ-4251539용 핫픽스
* 3.8 SDK 및 5.13 뷰어 README 파일은 업데이트되지 않으며 이전 릴리스의 정보를 포함합니다. CQ-4273737용 핫픽스
...
* 텍스트 구성 요소의 RTE(Rich Text Editor) 플러그인은 일본어 및 한국어 텍스트의 왜곡된 문자를 표시합니다. (NPR-31331)
* RTE(Rich Text Editor)에서는 포함된 표를 목록 항목으로 삽입할 수 없습니다. (NPR-30879)
* RTE(Rich Text Editor)를 즉시 스캐폴딩하는 RTE(Rich Text Editor)가 예기치 않게 요소에 인라인 글꼴 크기를 적용합니다. (NPR-31284)
* 사용자가 왼쪽 레일 필드에 초점을 맞추고 키보드 단축키를 사용하여 컨텐츠를 붙여 넣으면 왼쪽 레일 필드에서 복사한 컨텐츠가 아니라 페이지 편집기 클립보드의 컨텐츠를 붙여넣습니다. (NPR-31172)
...
* 일부 ExperienceFragments가 들어 있는 페이지가 비편집기 모드( `editor.html` 접두사와 접두사가 없는 작성자 `wcmmode=disabled`또는 게시자에서)에서 열리면 요청이 HTTP 상태 오류 코드 500으로 끝납니다. (NPR-30743)

### WCM - 페이지 편집기 {#wcm-page-editor-6520}

**제품 개선 사항**

* EnhanceDocument 유형 필터를 더 많은 MIME 유형으로 사용하여 여러 값을 갖는 옵션을 지원합니다. CQ-4270694용 핫픽스
...
* WSDL(웹 서비스 정의 언어)을 사용하여 FDM(양식 데이터 모델)을 만드는 동안 오류 메시지가 `Caused by: com.adobe.aem.dermis.exception.DermisException: java.lang.Exception: Unable to handle content type` 반환됩니다.NPR-30477:CQ-4272921에 대한 핫픽스
...
* 사용자 지정 코드나 프로젝트가 두 번째로 배포된 경우 페이지가 렌더링되지 않고 다음 오류가 발생합니다.
...
* Parsing error when opening a Content Fragment which has characters dollar `($)` and open brace `({)`. CQ-4270266용 핫픽스
...
* User-provided data is not escaped on output in the `cq/replication/components/agent` component, resulting in a stored Cross-site scripting (XSS) vulnerability. CQ-4266263용 핫픽스
...
* OSGI에만 해당:Forms Service를 사용하여 정적 PDF 파일을 만들 수 있도록 지원합니다.
...
* 기본 문자열 출력에 콜론이 있을 때 잘못된 출력이 관찰됩니다. CQ-4260825용 핫픽스
...
* 차트의 선 색상이 삭제되는 경우 [정의되지 않음]으로 설정됩니다. CQ-4264762용 핫픽스
* 변경 내용을 동기화할 때 문서 조각에서 수행한 레이아웃 레이어 변경 사항이 사라집니다. CQ-4266054용 핫픽스
...
* 바인딩 유형이 텍스트 조각에서 문자열 필드/변수에 대한 없음/데이터 모델 개체로 변경될 때 에이전트 설정은 [에이전트로 편집 가능] 확인란 선택이 취소되어 표시되지 않습니다. CQ-4261953용 핫픽스
* 에이전트 UI를 제출하면 결과 웹 데이터 json 파일에는 상속이 취소된 언바운드 필드에 대한 정보가 저장됩니다. CQ-4265621용 핫픽스
...
* OSGi 전용:출력 및 양식 서비스에 새 PAGECOUNT 속성을 추가했습니다. NPR-28922: CQ-4270870용 핫픽스
* OSGi 전용:Forms Service를 사용하여 정적 PDF 파일을 만들 수 있도록 지원합니다. NPR-28572: CQ-4270869용 핫픽스
