...
* **FORMS-23802**(양식이 Sites 페이지에 있을 때 사용자 지정 함수가 로드되지 않음): 적응형 양식이 Sites 페이지에 임베드되어 있고 aem-forms-core-component 라이브러리의 버전이 1.1.76 미만인 경우 사용자 지정 함수가 미리 보기 또는 게시에서 작동하지 않습니다. 로그에 `InvalidFormContainerException: No form container found` 같은 오류가 표시될 수 있습니다. 이 문제를 해결하려면 AEM Forms SP24용 핫픽스를 [다운로드 및 설치](/help/release-notes/aem-forms-hotfix.md)하십시오(AddOn 6.0.1454).
