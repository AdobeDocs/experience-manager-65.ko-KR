...
* 이제 편집 버튼을 누르면 보조 도구 모음의 화면 판독기에 올바른 작업이 표시됩니다. 활성화하면 관련되지 않은 `Edit` 대신 `Preview button pressed,`이(가) 읽히므로 키보드 탐색 중 혼동이 제거됩니다. (SITES-25320)
...
* 시스템 개요에서 상태 집계를 수정했습니다. 이제 보기가 개별 검사가 실행된 후에 업데이트되고 올바른 카운트가 표시됩니다. 운영자는 보안 및 유지 관리 검사를 통과하면 잘못된 `OK` 배너 대신 `2 errors`을(를) 봅니다. (GRANITE-61482)
...
* **FORMS-23722** **을(를) 사용하는**&#x200B;첨부 파일`bindref` 필드가 포함된 양식을 **작업 할당** 단계를 통해 AEM Workflow에 제출하면 첨부 파일이 표시되지 않습니다. 따라서 작업이 받은 편지함에서 열리면 나타나지 않습니다. 파일이 저장소에 올바르게 저장되지만 작업 할당 단계 UI에 첨부 파일이 표시되지 않습니다.
...
* **FORMS-23789**(JEE의 AEM Forms만 해당): 사용자가 JEE SP24의 AEM Forms에서 Log4j에 문제가 발생하여 엔터프라이즈 고객의 로깅 및 모니터링에 차질이 발생했습니다. 이 문제를 해결하려면 JEE 서비스 팩 [에서 AEM Forms용 핫픽스를 &#x200B;](/help/release-notes/aem-forms-hotfix.md)다운로드하여 설치6.5.24.0하십시오.
...
로그 볼륨을 줄이려면 응용 프로그램 서버 구성에서 또는 JVM 인수 `com.adobe.util.XMLSecurityUtil`을(를) 통해 `ERROR`의 로깅 수준을 `-Dlogging.level.com.adobe.util.XMLSecurityUtil=ERROR`(으)로 설정합니다. 이 기능은 메시지를 숨기기만 하며 근본적인 원인은 해결하지 않습니다.
