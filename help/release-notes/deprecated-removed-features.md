...
| 통합 | **[!UICONTROL Experience Manager Cloud Service 옵트인]** 화면은 [!DNL Experience Manager] 6.5에서 [!DNL Experience Manager] 및 [!DNL Adobe Target] 통합이 업데이트되었으므로 더 이상 사용되지 않습니다. 통합은 Adobe Target Standard API를 지원합니다. API는 Adobe IMS 및 [!DNL Adobe I/O Runtime]을(를) 통해 인증을 사용합니다. Adobe 및 개인화를 위해 [!DNL Experience Manager] 페이지를 계측하는 Analytics Launch의 늘어나는 역할을 지원합니다. 옵트인 마법사는 기능적으로 관련이 없습니다. | 해당 [!DNL Experience Manager] 클라우드 서비스를 통해 시스템 연결, Adobe IMS 인증 및 [!DNL Adobe I/O Runtime] 통합을 구성하십시오. | 6.5.7.0 |
...
| 개발자 | Handlebars 클라이언트 라이브러리. Adobe은 배포(빠른 시작)의 일부로 제공되는 Handlebar 클라이언트 라이브러리를 유지 및 업데이트할 계획이 없습니다. | Adobe은 코드에 여전히 `Handlebars`이(가) 필요한 고객을 프로젝트 코드 베이스에 추가하도록 권장합니다. |  |
