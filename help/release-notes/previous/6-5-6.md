---
title: '[!DNL Adobe Experience Manager]'' 6.5.6 서비스 팩 릴리스 정보'
description: 6.5.6 서비스 팩에 대한 [!DNL Adobe Experience Manager] 릴리스 노트
contentOwner: AK
mini-toc-levels: 2
...
Adobe Experience Manager 6.5.6.0은 2019 **년 4월 6.5 릴리스**&#x200B;의 일반 공급 이후 릴리스된 새로운 기능, 주요 고객이 요청한 개선 사항, 성능, 안정성 및 보안 개선 사항이 포함된 중요한 업데이트입니다. Adobe Experience Manager 6.5 맨 위에 설치할 수 있습니다.
...
* [자동화된 Forms 전환 서비스에서](https://experienceleague.adobe.com/docs/aem-forms-automated-conversion-service/using/convert-existing-forms-to-adaptive-forms.html?lang=ko-KR) Adobe Sign 텍스트 태그를 지원합니다.

* [를 사용하여 [!DNL Automated Forms Conversion service]색상이 지정된 양식을 적응형 양식](https://experienceleague.adobe.com/docs/aem-forms-automated-conversion-service/using/convert-existing-forms-to-adaptive-forms.html?lang=ko-KR)으로 전환 지원.

* SMB 2 및 SMB 3 프로토콜 지원.

* 내장된 저장소(Apache Jackrabbit Oak)가 버전 1.22.4로 업데이트되었습니다.

다음은 [!DNL Experience Manager] 6.5.6.0 릴리스에서 제공된 수정 사항 목록입니다.

## [!DNL Sites] {#sites-6560}

* 또는 [!DNL Sites] [!DNL Screens]에서 프로젝트를 선택하고 관리 발행물] 을 클릭합니다[!UICONTROL . 사용자는 사용자 인터페이스 오류로 인해 게시] 관리 마법사에서 진행[!UICONTROL 할 수 없습니다. 특히 [!UICONTROL Publish] 옵션이 작동하지 않습니다(NPR-34099).
* 상속] 취소 또는 [!UICONTROL 상속] 비활성화 옵션을 선택 [!UICONTROL 취소한 후 iParsys(상속된 단락 시스템)의 위치가 원래 기본 위치로 되돌려지지 않습니다(NPR-34097).
* `RolloutConfigManagerFactoryImpl` 롤아웃 구성을 로드할 수 없는 경우 누락된 구성을 로드하지 않습니다. 캐시된 구성을 반환합니다(NPR-34092).
* 텍스트 코어 구성 요소에서 소스 HTML 편집 옵션을 사용한 후 태그 클래스 `em` 가 제거됩니다(NPR-34081).
* Experience Manager 6.3.3에서 Experience Manager 6.5.3으로 업그레이드한 후 롤아웃 프로세스가 훨씬 더 오래 걸리고 시간 초과 오류가 발생하여 롤아웃이 실패합니다(NPR-34049).
* `htmlwriter` 속성 값을 다시 인코딩하지 않습니다. XF 마크업에 있는 마크업은 디코딩된 속성 값(즉 `"` `&#34`, 대신)과 함께 내보내집니다. 이로 인해 XF 내보내기를 사용하는 시각적 경험 작성기의 Target 측에 문제가 발생합니다(NPR-34048).
* 페이지를 [!DNL Experience Manager Sites]이동할 때 로깅 개선을 통해 이유 있는 버전 생성 실패를 캡처합니다(NPR-34014).
* 에서 [!DNL Rich Text Editor] 모든 텍스트가 제거되면 단락 태그 도 제거됩니다(NPR-33976).
* `siteadmin` 페이지(클래식 UI)를 열거나 새로 고치면 메뉴의 `New` 옵션이 비활성화됩니다(NPR-33949).

  ![클래식 UI 메뉴의 누락 문제를 보여주는 스크린샷](/help/release-notes/assets/33949_missing_menu.png)

* A [!DNL Content Fragment] 는 실패 `ContentFragmentUsePojo` 하므로 A `TemplatedResource` 로 사용할 수 없습니다(NPR-33911).
* 동기 및 비동기 이동 작업은 동시 전송으로 인해 오류가 발생리드 수 있습니다. 페이지 이동 작업은 비동기 이동으로만 제한됩니다. 페이지의 동시 이동을 방지합니다(NPR-33875).
* [!UICONTROL 게시 관리 작성자에서 Publish 인스턴스로 컨텐츠 복제하는 작업이] 실패하고 JavaScript 오류가 생성됩니다(NPR-33872).
* 버전을 생성하기 위해 여러 페이지 또는 자산을 선택하면 마지막으로 선택한 페이지 또는 자산에 대해서만 새 버전이 생성됩니다(NPR-33866).
...
* [!DNL RTE] 레이아웃 컨테이너를 복사하여 페이지 위에 다시 붙여 넣으면 레이아웃 컨테이너의 구성 요소가 표시되지 않습니다. [!DNL RTE] 구성 요소는 편집할 수 없지만 페이지 새로 고침 시 표시됩니다(NPR-33662).
* 다른 중단점(중간 및 대형)에 대한 레이아웃 구성 요소의 크기를 조정할 때 레이아웃이 예상대로 작동하지 않습니다(NPR-33608).
* 의 인라인 편집 모드에서 [!DNL RTE]텍스트 구성 요소에 대해 이미지 드래그가 작동하지 않습니다(NPR-33602).
* 페이지 이름과 동일한 이름으로 블루프린트 페이지에 구성 요소를 만들 수 있습니다. 롤아웃 `_msm_moved` 중에 구성 요소의 이름을 바꾸기 위해 접미사가 붙습니다. 구성 요소가 단락 시스템의] 끝[!UICONTROL 으로 이동됩니다(NPR-33535).
* offTime 또는 onTime이 많은 페이지 또는 자산에 설정된 경우 리소스를 많이 사용하며 시작 및 종료 중에 시스템 속도가 느려집니다(NPR-33482).
* CRUD 권한이 있는 `/content/experience-fragment` 사용자 폴더는 삭제할 수 없습니다(NPR-33436).
* 섹션의 상위 폴더 [!DNL Experience Fragments] 에서 Adobe Target 내보내기 포맷] 옵션[!UICONTROL 으로 HTML 및 JSON]을 선택할 [!UICONTROL 수 있습니다. 이 상위 폴더의 하위 폴더에 대해 동일한 속성이 터치 지원 UI 페이지에 표시됩니다. 그러나 CRXDE에서는 를 `cq:adobeTargetExportFormat`위해 표시하는 `html,json` 대신 HTML만 표시합니다(NPR-33423).
* 페이지 별칭에서 Publish 또는 게시 취소는 지원되지 않습니다. 그렇지 않다고 주장하는 것으로 보이는 옵션을 제거합니다(NPR-33415).
* 의 특정 태그 위치를 의 한 위치에서 다른 [!DNL Experience Manager]위치로 이동할 수 있습니다. 이동 전후에 서로 다른 페이지에 적용할 수도 있습니다. 페이지의 속성을 편집할 때 태그가 동일하더라도 편집할 수 있도록 태그가 표시되지 않습니다(NPR-33353).
...
* 아래에 `/content/campaign` 브랜드 만들기는 캠페인을 만들 수 없는 구조가 됩니다. [!UICONTROL 브랜드] 만들기 옵션을 사용하면 새로 만든 브랜드에 만들기] 옵션이 없으므로 [!UICONTROL 오퍼 및 활동을] 만들 [!UICONTROL 수 없습니다(NPR-34113).
* 페이지 일시 중단 [!DNL Live Copy] 할 수 있으며 편집기 모드에서 볼 수 있듯이 상속이 끊어집니다. 페이지 속성에서 상속을 나타내는 아이콘이 상속이 존재하고 손상되지 않았음을 잘못 나타냅니다(NPR-34017).
* 참조가 많은 페이지는 비동기적으로 이동할 수 없으며 이동 작업이 실패하는 경우가 있습니다(CQ-4297969).
* URL에 문자가 있는 `/` 웹 페이지는 작성하는 동안 응답하지 않게 됩니다. 작성하는 동안 구성 요소가 추가되면 CPU 사용량이 증가하고 브라우저 응답이 중지됩니다(CQ-4295749).
...
* Experience Manager 받은 편지함에 대한 달력 보기의 교차 사이트 스크립팅(NPR-33545).
* 로 `childrenlist.html` 끝나는 URL은 404 응답 대신 HTML 페이지를 표시합니다. 이러한 URL은 사이트 간 스크립팅(NPR-33441)에 취약합니다.
...
* 이제 사용자는 키보드 키를 사용하여 자산 참조] 목록의 대화형 사용자 인터페이스 옵션[!UICONTROL 에 액세스하고 포커스할 수 있습니다(NPR-34115).

* 이제 화면 판독기에서 검색 페이지에 대한 조건자의 의도된 작업을 알립니다(NPR-34104).

* 이제 Search 페이지 및 검색 결과 페이지에는 화면 판독기 사용자를 더 잘 이해할 수 있도록 더 많은 정보를 제공하는 제목이 있습니다(NPR-34093).

* 이제 스크린 리더는 자산 [!UICONTROL 속성] 페이지의 기본] 탭에서 선택한 태그를 [!UICONTROL 삭제하는 옵션을 알립니다(NPR-33972).

* 이제 목록 보기의 각 행에 있는 요소가 화면 판독기에 의해 동일한 행의 요소로 발표됩니다(NPR-33932).

* 이제 키를 사용하여 `Tab` 탐색할 때 사용자 포커스 가 버전 미리 보기에서 닫기 옵션으로 이동합니다(NPR-33863).

* 이제 Omnisearch를 닫은 후 사용자 포커스가 검색 아이콘으로 이동합니다(NPR-33705).

* 실행 가능한 사용자 인터페이스 옵션은 이제 키보드 키를 사용하여 탐색할 때 향상된 대비와 함께 더 눈에 띄는 시각적 포커스 기능을 갖습니다. 키보드 사용자는 포커스 영역을 식별할 수 있습니다(NPR-33542).

* 이제 키보드를 사용한 드래그 기능이 화면 판독기의 찾아보기 모드에서 메타데이터 스키마 편집기] 편집기에서 작동합니다[!UICONTROL (CQ-4296326).

* 링크 공유 대화 상자에서 찾아보기 모드로 탐색할 때 스크린 리더,

   * 대화 상자가 로드되자마자 테이블 정보를 내레이션하지 않습니다.

   * 나열된 모든 자동 제안으로 이동할 수 있습니다.

   * 이메일 주소/Search] 추가(CQ-4294232)에 [!UICONTROL 대해 표시되는 자동 제안에 대해 설명합니다.

* 사용 `Esc` 카드 보기에서 빠른 작업 아이콘을 제거하는 키는 더 이상 마지막 포커스가 있는 항목에서 키보드 포커스를 제거하지 않습니다(CQ-4293554).

* 사용자 인터페이스의 대화형 옵션의 경우 이제 화면 판독기에서 아이콘의 리터럴 이름이 아닌 용도를 알립니다(CQ-4272943).

* 이제 키보드 포커스가 뷰어의 세부 사항 [!UICONTROL [!DNL Dynamic Media]] 에서 키보드 탭 자산키를 사용하여 탐색할 때 플라이아웃], [!UICONTROL InlineZoom], [!UICONTROL Shoppable_Banner], [!UICONTROL Zoom_dark], [!UICONTROL Zoom_light], [!UICONTROL ZoomVertical_dark][!UICONTROL  및 ZoomVertical_light] 옵션으로 성공적으로 이동합니다[!UICONTROL (CQ-4290605).

* [!UICONTROL 이제 키보드 키를 사용하여 자산 [!UICONTROL 속성] 페이지의 저장 및 닫기] 옵션에 액세스할 수 있습니다(NPR-34107).

* 이제 로그인 페이지의 잘못된 사용자 이름 및 암호 조합으로 인한 오류 메시지는 오류가 발생할 때마다 화면 판독기에서 알립니다(NPR-33722).
...
* 위치 [!DNL Experience Manager] 헤더 섹션, 검색 모드로 탐색할 때 화면 판독기에서 이제
  ...
  *그림: [!UICONTROL 도움말 메뉴의 [!UICONTROL 도움말]에 대한 Search].*

   * 사용자] 옵션 및 포커스 아래의 [!UICONTROL 필드로  가장 잘못된 값을 입력하면 오류 메시지가 텍스트 필드로 올바르게 이동합니다(NPR-33804).

  ![헤더의 사용자 메뉴](/help/release-notes/assets/User_aem_header.png)

  *그림: [!UICONTROL 헤더의 사용자] 메뉴에 있는 [!UICONTROL 필드로] 가장*

* 사용자는 이제 다음 내에서 키보드를 사용하여 포커스 변경 가능:

   * [!UICONTROL 이메일 주소 검색/추가] 의 필드 [!UICONTROL 링크 공유] 대화 상자.

   * [!UICONTROL 폴더 속성의 권한] 탭][!UICONTROL 에 닫힌 사용자 그룹] [!UICONTROL 아래의 [!UICONTROL 필드 사용자 또는 그룹] 추가(NPR-34452).
...
* 템플릿를 사용하여 [!DNL Adobe InDesign] 만든 마케팅 담보 자산(예: 브로셔, 전단지 및 비즈니스 카드)의 미리 보기 줄 바꿈 및 단락 나누기가 표시되지 않습니다(NPR-34268).

* 텍스트 추출 및 업로드된 PDF 파일에 대한 전체 텍스트 검색 기능이 작동하지 않습니다(NPR-34164). 이 문제를 해결하려면 서비스 팩 6을 [!DNL sAdobe Experience Manager] 설치한 후 배포 프로그램을 다시 시작하십시오.
...
* 게시된 자산사용자 새 위치로 이동하는 경우 [다시 게시]] 옵션을 선택 취소하면 [!UICONTROL 자산 균일 다시 게시됩니다. 이로 인해 많은 고아 자산이 게시 인스턴스에 놓이게 됩니다. 그러나 기본 동작은 게시된 자산 이동 작업이 자동으로 게시 취소하는 것입니다. 작성자가 자산을 이동할 때 재게시] 옵션을 선택하면 [!UICONTROL 이 자산이 다시 게시됩니다(NPR-33934).

* [!UICONTROL 컬렉션의 자산에 대한 이동 Assets] 페이지가 조정/다시 게시] 옵션과 같은 [!UICONTROL 모든 HTML 컨텐츠를 로드하지 않습니다. 따라서 사용자가 이동 작업을 완료할 수 없습니다(NPR-33860).

* 자산을 이동하고 이동된 자산의 이름과 제목에 특수 문자를 추가하면 자산의 새 위치에 추가 폴더(동일한 이름)가 만들어집니다(NPR-33826).

* [!UICONTROL 다운로드] 대화 상자에서 이메일] 옵션을 선택하면 자산 다운로드용 버튼 다운로드]가 [!UICONTROL 비활성화[!UICONTROL 됩니다(NPR-33730).
...
* 세부 사항 페이지페이지 측면 레일 옵션(예: [!UICONTROL 개요], 타임라인], [!UICONTROL [!UICONTROL 뷰어]) 간에 전환사용자 때 로드 시 Assets 깜박자산 있습니다(NPR-34235).

* 재처리 작업에서 다음과 같은 문제가 관찰됩니다.

   * 재처리 작업에서 반환된 작업 핸들에 작업 ID가 없습니다.

   * 비디오 로그에 대한 재처리 작업만 파일 이름이고 전체 경로는 재처리하지 않습니다.

   * 재처리 작업에 자산 유형을 정적으로 설정하는 옵션이 없습니다.

   * `ExcludeFromAVS` 옵션이 제공되지 않습니다(CQ-4298401).

* 이미지 프로필이 여러 종횡비(예: 11)가 있는 폴더에 추가되면 오류와 함께 스마트 자르기 기능이 실패합니다(NPR-34082).
...
* 가장&#x200B;**[!UICONTROL 할 사용자를 검색할 때 키를 누르면 `Esc` 사용자 목록 대신 사용자]** 대화 상자가 닫힙니다(NPR-34084).
...
* 상태 확인 및 패턴 감지기는 겹치는 기능을 가지고 있습니다. 그 결과 상태 검사가 제품에서 제거됩니다(NPR-33928).

## WCM {#wcm-6560}

* 기초 구성 요소 - 페이지 에 기초 이미지 구성 요소를 추가하고 이미지를 `Undo` 참조하면 작업이 작동하지 않습니다(NPR-34516).

* 페이지 이동 작업을 사용할 수 없습니다(CQ-4303028).

## [!DNL Communities] {#communities-6560}

* 소셜 미디어 상의 게시물 공유에 더 이상 사용되지 않는 옵션 Google+가 표시됩니다(NPR-33877).

* 커뮤니티 구성원이 그룹 템플릿 또는 기타 그룹 기능 설정을 수정할 수 없습니다(NPR-33530).

* 이미지의 하이퍼링크 태그가 포럼 게시물에서 제대로 생성되지 않습니다(NPR-33464).

* 액세스 가능성 오류는 커뮤니티 할당 기능에서 식별됩니다(NPR-33442).

* Admin Console을 통해 추가된 커뮤니티 그룹의 기존 사용자는 커뮤니티 그룹 콘솔에서 수정하면 사용자 목록에서 제거됩니다(NPR-34315).

* 잠재적으로 중요한 데이터가 유출됩니다 `TagFilterServlet` (NPR-33868).
...
* ` sling.bootdelegation.class.org.bouncycastle.jce.provider.BouncyCastleProvider` 파일에서 속성 `sling.properties` 삭제.

* 인스턴스 다시 시작 [!DNL Experience Manager Forms]
...
* 서비스 팩을 설치한 [!DNL Experience Manager] 후 PDF 파일에서 HSM 기반 인증서로 서명 작업을 실행할 수 없습니다(NPR-34310).

**디자이너**

* Designer 버전 6.5.x에서 XForms를 열 수 없습니다(CQ-4295322).

* 디자이너를 열면 시작 화면에 잘못된 연도가 표시됩니다(CQ-4295289).
...
### 변수로 양식 데이터 모델 서비스 출력 저장(6.5.6.0) {#save-fdm-service-to-variable}

양식 데이터 모델을 사용하면 양식 데이터 모델 서비스의 출력을 변수 변수에 저장할 수 있습니다. [!DNL Experience Manager Forms] 이제 양식 데이터 모델 서비스의 유형을 변수 유형에 자동으로 매핑합니다.

### 파일 첨부 파일 구성 요소에 대해 여러 파일 첨부(6.5.6.0) {#attach-multiple-files}

이제 다음을 수행할 수 있습니다. [여러 파일 첨부](/help/forms/using/introduction-forms-authoring.md) (으)로 [!UICONTROL 첨부 파일] 적응형 양식의 구성 요소입니다.

## UberJar {#uber-jar}

Experience Manager 6.5.6.0용 UberJar는 Maven Central 저장소에서 사용할 수 [있습니다](https://repo.maven.apache.org/maven2/com/adobe/aem/uber-jar/6.5.6/).
