...
* [!UICONTROL 빠른 게시] 또는 [!UICONTROL 게시 관리] 마법사를 사용하여 자산을 [!DNL Experience Manager] 또는 [!DNL Dynamic Media]에 선택적으로 게시하거나 게시 취소합니다.
...
* 사용자가 썸네일만 만들고 페이지 추출 및 키워드 추출을 건너뛸 수 있도록 `test` 및 `aiprocess`에 대한 기본 옵션이 이전 Dynamic Media의 `Rasterize`에서 `Thumbnail`(으)로 업데이트되었습니다.
...
* [!DNL Automated Forms Conversion service]을(를) 사용하여 [컬러 양식을 적응형 양식으로 변환](https://experienceleague.adobe.com/docs/aem-forms-automated-conversion-service/using/convert-existing-forms-to-adaptive-forms.html)할 수 있도록 지원합니다.
...
* [!DNL Experience Fragments] 섹션의 상위 폴더에서 [!UICONTROL HTML 내보내기 형식]에 대한 옵션으로 [!UICONTROL Adobe Target 및 JSON]을(를) 선택할 수 있습니다. 이 상위 폴더의 하위 폴더에 대해 터치 사용 UI에 동일한 속성이 표시됩니다. 그러나 CRXDE에서는 `cq:adobeTargetExportFormat`에 대해 `html,json`을(를) 표시하는 대신 HTML만 표시합니다(NPR-33423).
