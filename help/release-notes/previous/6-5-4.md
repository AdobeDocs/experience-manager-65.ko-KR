...
Adobe Experience Manager 6.5.4.0은(는) 새로운 기능, 주요 고객이 요청한 향상된 기능 및 성능, 안정성, 보안 개선 사항이 포함된 중요한 업데이트로, **2019년 4월**&#x200B;에 6.5 릴리스의 공식 출시 이후 릴리스되었습니다. Adobe Experience Manager 6.5 맨 위에 설치할 수 있습니다.

Adobe Experience Manager 6.5.4.0에 도입된 몇 가지 주요 기능 및 개선 사항은 다음과 같습니다.
...
* 기본 제공 저장소(Apache Jackrabbit Oak)가 버전 1.10.8으로 업데이트되었습니다.
...
* 사용자가 콘텐츠 조각을 편집하면 삭제된 콘텐츠 조각의 변형이 복원됩니다(NPR-32062).

* 사용자는 필수 필드에 정보를 제공하지 않고 콘텐츠 조각을 저장할 수 있습니다(NPR-31988).
...
* 기본 디자인 가져오기 구성 요소가 포함된 Adobe campaign에서 캠페인을 동기화할 수 없습니다(NPR-31728).
...
* ${extension}이(가) 있는 자산의 렌디션이 생성되지 않습니다(NPR-32294).
...
* Scene7의 배치 업로드 프로세스를 통해 자산을 처리하는 데 시간이 너무 오래 걸립니다(CQ-4286445).
...
* 슬라이드 편집기가 Experience Manager Dynamic Media 하이브리드 모드의 IE 11에서 열리지 않습니다(CQ-4255590). **Adobe Dynamic Media - 하이브리드 모드 고객:** Adobe에서 2022년 5월 이후에 Dynamic Media - 하이브리드 모드에서 Internet Explorer 11에 대한 지원을 종료합니다.
...
### [!DNL Experience Manager Assets]&#x200B;([!DNL Brand Portal])을(를) 사용하여 6.5.4.0 구성 {#configure-assets-bp}

[!DNL Experience Manager Assets]과 [!DNL Brand Portal] 사이의 인증 채널이 변경되었습니다. 이전에는 [!DNL Brand Portal]이 기존 OAuth 게이트웨이를 통해 클래식 UI에 구성되었으며, 이 게이트웨이는 인증을 위해 IMS 액세스 토큰을 가져오는 데 JWT 토큰 교환을 사용합니다. [!DNL Experience Manager Assets]이(가) [!DNL Brand Portal] 테넌트의 인증을 위해 IMS 토큰을 조달하는 [!DNL Adobe I/O]에서 [!DNL Brand Portal]&#x200B;(으)로 구성되었습니다.
...
* 적용형 양식: Experience Manager Sites 페이지에 임베드된 적용형 양식을 구성하는 동안 번역 문제가 발생했습니다(NPR-32168).
...
* 디자이너: 테이블에 병합된 셀이 있는 경우 출력 서비스를 사용하여 XDP 양식에서 변환된 출력 PDF 파일에 접근성 테스트를 수행하면 오류가 발생합니다(CQ-4285372).

* Foundation JEE: 클러스터에서 Experience Manager Forms 서버 연결이 끊어진 경우 캐싱 문제가 발생하여 서버에 다시 연결되지 않습니다(NPR-32412).

## UberJar {#uber-jar}

Experience Manager 6.5.4.0용 UberJar를 [Maven 중앙 저장소](https://repo.maven.apache.org/maven2/com/adobe/aem/uber-jar/6.5.4/)에서 사용할 수 있습니다.
