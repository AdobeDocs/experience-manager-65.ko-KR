---
title: '[!DNL Adobe Experience Manager]'' 6.5.4 서비스 팩 릴리스 정보'''
description: 6.5.4 서비스 팩에 대한 [!DNL Adobe Experience Manager] 릴리스 노트
contentOwner: AK
mini-toc-levels: 2
...
* 이제 Adobe Experience Manager Assets가 콘솔을 통해 [!DNL Adobe I/O] Brand Portal로 구성됩니다.
...
## 다양한 영역에서 성능 향상(6.5.4.0) {#performance-improvements}
...
* 선택하면 여기에 구성 요소 드래그 반응형 그리드의 자리 표시자에서 옵션을 선택하면 GET 요청이 전송되고 요청 결과 HTTP 403 오류가 발생합니다(NPR-31845).
...
* 기본 디자인 가져오기 구성 요소가 포함된 Adobe Systems 캠페인에서 캠페인을 동기화할 수 없습니다(NPR-31728).
...
* 보기 구성 대화 상자가 받은 편지함 사용자 지정 작업 과정 기능에 대해 제대로 작동하지 않습니다(NPR-32503 및 NPR-32492). 이 기능에 대한 자세한 내용은 [받은 편지함](/help/sites-authoring/inbox.md)을 참조하십시오.
...
* 3D 뷰어에서 미리 본 업로드된 3D 모델의 잘못된 컨테이너 높이가 자산의 세부 사항 페이지에서 관찰됩니다(CQ-4283309).

* 회전판 편집기가 하이브리드 모드의 IE 11 Experience Manager 다이내믹 미디어 열리지 않습니다(CQ-4255590). **Adobe Systems 다이내믹 미디어 - 하이브리드 모드 고객의 경우:** Adobe Systems는 2022년 5월 이후 다이내믹 미디어 - 하이브리드 모드에서 Internet Explorer 11에 대한 지원을 종료합니다.
...
* 화면 판독기는 선택한 태그(`X` 기호)와 선택한 태그의 번호를 제거하는 옵션을 알려 줍니다.
...
* 메타데이터 하위 스키마는 자산 속성의 MIME 유형을 기반으로 탭을 표시하지 않습니다(CQ-4283288).
...
>[!NOTE]
>
>Experience Manager 서비스 팩에는 Experience Manager Forms에 대한 수정 사항이 포함되어 있지 않습니다. 이러한 수정 사항은 별도의 Forms 추가 기능 패키지를 사용하여 전달됩니다. 또한, JEE의 Adobe Experience Manager Forms에 대한 수정 사항이 포함된 누적 설치 프로그램이 릴리스됩니다. 자세한 내용은 JEE](/help/release-notes/jee-patch-installer-65.md)에 Experience Manager Forms 설치를 참조하세요[.
...
Experience Manager 6.5.4.0용 UberJar는 Maven Central 저장소에서 사용할 수 [있습니다](https://repo.maven.apache.org/maven2/com/adobe/aem/uber-jar/6.5.4/).
