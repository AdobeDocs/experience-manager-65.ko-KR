...
| 유형 | 서비스 팩 릴리스 |
| 날짜 | 2023년 8월 24일 금요일 <!-- UPDATE FOR EACH NEW RELEASE --> |
...
[!DNL Experience Manager] 6.5.18.0에는 2019년 4월 6.5의 최초 출시 이후 릴리스된 새로운 기능, 주요 고객 요청 개선 사항, 버그 수정 사항 및 성능, 안정성, 보안 개선 사항이 포함되어 있습니다. [!DNL Experience Manager] 6.5에서 [이 서비스 팩을 설치](#install)합니다.
...
* AEM의 ActiveMQ는 더 이상 사용되지 않습니다. ActiveMQ는 두 AEM 게시 인스턴스 간의 통신에 사용되었습니다. 이제 고객은 로드 밸런서를 사용하는 것이 좋습니다.
...
* [구성 브라우저 설명서](https://experienceleague.adobe.com/docs/experience-manager-65/administering/introduction/configurations.html#using-configuration-browser)에 설명된 대로 _이름이 저장소의 노드 이름이 됩니다_. 그러나 구성 브라우저에서 구성 제목은 CRXDE Lite의 경로에 사용되며 구성 이름은 무시됩니다. (NPR-40607)
...
1. OSGi 번들 `org.apache.jackrabbit.oak-core`의 버전이 1.22.16 이상입니다(웹 콘솔 사용: `/system/console/bundles`).
...
문제를 해결하려면 구성 관리자를 실행하기 전에 `<AEM_Forms_Installation_dir>\jboss\bin\standalone.bat` 파일을 찾아 모든 항목에 대해 `Adobe_Adobe_JAVA_HOME`을(를) `Adobe_JAVA_HOME`(으)로 업데이트하십시오. (CQDOC-20803)
