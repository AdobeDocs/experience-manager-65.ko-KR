...
[!DNL Experience Manager] 6.5.16.0에는 2019년 4월 6.5의 최초 출시 이후 릴리스된 새로운 기능, 주요 고객 요청 개선 사항, 버그 수정 사항, 성능, 안정성 및 보안 개선 사항이 포함되어 있습니다. [!DNL Experience Manager] 6.5에 [이 서비스 팩 설치](#install). <!-- UPDATE FOR EACH NEW RELEASE -->
...
* JCR 속성이 `getString` 대신 `toString`(으)로 읽기 때문에 `getManifestURI`이(가) null을 반환합니다. (ASSETS-18674)
...
* AEM Forms JEE 서비스 팩에서 `Prepare Adobe Experience Manager Server For DSC deployment` 검사를 수행하는 동안 `admin`에서 관리자 사용자의 기본 암호를 변경하는 경우 오류가 발생합니다. (NPR-40062) 및 (NPR-39387)
...
* AEM 6.5.14.0으로 업그레이드하고 importData 서비스를 사용하여 PDF을 XML과 병합하는 경우 `Caused by: java.lang.NoSuchMethodError: com.adobe.xfa.form.FormModel.isXFABarcode(Lcom/adobe/xfa/Node;)Ljava/lang/Boolean` 오류가 발생합니다. (NPR-39807)
...
AEM 6.5.16.0 Forms 추가 기능 서비스 팩을 설치한 후 고객은 아래에 나열된 문제에 직면했습니다. 따라서 [AEM 6.5.16.0 Forms 추가 기능 서비스 팩 - 6.0.914](https://experienceleague.adobe.com/docs/experience-manager-release-information/aem-release-updates/forms-updates/aem-forms-releases.html)의 업데이트된 버전이 릴리스되었습니다. Adobe 업데이트된 서비스 팩을 사용하는 것이 좋습니다.
...
* Experience Manager 6.5에서 Adobe Search&amp;Promote 코드 및 종속성을 제거합니다. Adobe Search&amp;Promote은 2022년 9월에 서비스가 종료되었습니다. [Adobe Search&amp;Promote 서비스 종료 공지](https://experienceleague.adobe.com/docs/discontinued/using/search-promote.html)를 참조하십시오. (NPR-39706)
...
* [!DNL Experience Manager] 6.5.16.0에는 [!DNL Experience Manager] 6.5가 필요합니다. 자세한 지침은 [업그레이드 설명서](/help/sites-deploying/upgrade.md)를 참조하세요. <!-- UPDATE FOR EACH NEW RELEASE -->
...
* MongoDB 및 여러 인스턴스가 있는 배포에서 패키지 관리자를 사용하여 작성자 인스턴스 중 하나에 [!DNL Experience Manager] 6.5.16.0을 설치합니다.<!-- UPDATE FOR EACH NEW RELEASE -->

>[!IMPORTANT]
>
> Adobe은 [!DNL Experience Manager] 6.5.16.0 패키지를 제거하거나 제거하지 않는 것이 좋습니다. 따라서 팩을 설치하기 전에 `crx-repository`을(를) 롤백해야 하는 경우 백업을 만들어야 합니다. <!-- UPDATE FOR EACH NEW RELEASE -->
...
[!DNL Experience Manager] 6.5.16.0.<!-- UPDATE FOR EACH NEW RELEASE -->을(를) 자동으로 설치하는 데 사용할 수 있는 두 가지 방법이 있습니다.
...
1. 제품 정보 페이지(`/system/console/productinfo`)에는 [!UICONTROL 설치된 제품]에 업데이트된 버전 문자열 `Adobe Experience Manager (6.5.16.0)`이(가) 표시됩니다. <!-- UPDATE FOR EACH NEW RELEASE -->
...
[!DNL Experience Manager] 6.5.16.0용 UberJar를 [Maven 중앙 저장소](https://repo.maven.apache.org/maven2/com/adobe/aem/uber-jar/6.5.15/)에서 사용할 수 있습니다. <!-- CHECK FOR UPDATE EACH NEW RELEASE -->
...
* [Experience Manager 6.5.16.0에 포함된 콘텐츠 패키지 목록](/help/release-notes/assets/65160_packages.txt) <!-- UPDATE FOR EACH NEW RELEASE -->
