...
* **[AEM Forms Designer의 접근성 개선](https://helpx.adobe.com/kr/content/dam/help/en/experience-manager/6-5/forms/pdf/using-designer.pdf)**: AEM Forms Designer에서는 다음과 같은 접근성 개선 사항이 도입되었습니다.

   * AEM Forms Designer은 이제 새로운 `<TOC>` 및 `<TOCI>` 태그를 통해 액세스 가능한 목차를 지원하므로 작성자가 접근성 탭을 통해 하위 양식에 이러한 역할을 할당할 수 있습니다. 이 기능은 다중 레벨 목차 구조, 레이블이 지정된 항목, 번호가 매겨진 참조 및 스팬 하위 구조를 지원하므로 화면 판독기에서 목차 컨텐츠를 올바르게 식별하고 탐색할 수 있습니다.

   * AEM Forms Designer은 이제 범위, 행 범위 및 열 범위를 포함한 테이블 셀에 대한 액세스 가능성 속성 구성을 지원하므로 화면 판독기 및 보조 장치가 생성된 PDF의 테이블 구조를 올바르게 해석할 수 있습니다. 작성자는 셀 속성 대화 상자에서 직접 이러한 속성을 설정하고 XML Source 보기에서 확인할 수 있습니다. 구성된 속성은 태그가 지정된 PDF 출력에 정확하게 반영되며 Acrobat 테이블 편집기를 사용하여 유효성을 검사할 수 있습니다.
...
* Forms Designer의 태그 지정이 올바르더라도 태그 순서가 PDF에서 임의로 변경됩니다. (LC-3921313) 
...
1. OSGi 번들 `org.apache.jackrabbit.oak-core`의 버전이 1.22.17 이상입니다(웹 콘솔 사용: `/system/console/bundles`).
