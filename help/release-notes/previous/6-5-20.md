...
[!DNL Experience Manager] 6.5.20.0에는 2019년 4월 6.5의 최초 출시 이후 릴리스된 새로운 기능, 주요 고객 요청 개선 사항, 버그 수정 사항, 성능, 안정성 및 보안 개선 사항이 포함되어 있습니다. [!DNL Experience Manager] 6.5에서 [이 서비스 팩을 설치](#install)합니다.
...
* 이제 Dynamic Media은 Apple iOS/iPadOS에 대해 무손실 HEIC 이미지 형식을 지원합니다. Dynamic Media 이미지 제공 및 렌더링 API에서 [fmt](https://experienceleague.adobe.com/en/docs/dynamic-media-developer-resources/image-serving-api/image-serving-api/http-protocol-reference/command-reference/r-is-http-fmt)을(를) 참조하십시오.
...
* **ECDSA를 지원하는 향상된 보안**: 이제 AEM Forms은 JEE 및 OSGi 스택 모두에서 ECDSA(Elliptic Curve Digital Signature Algorithm)를 강력하게 지원합니다. 이제 사용자는 강화된 보안으로 PDF 문서에 서명, 인증 및 확인할 수 있습니다. 지원되는 EC 곡선 알고리즘은 다음과 같습니다.
...
* 컨텐츠 조각의 빠른 Publish을 사용하면 계속해서 로드되고 게시되지 않습니다. 즉, AEM 6.5.7에서 AEM 6.5.17로 서비스 팩을 업그레이드한 후 빠른 Publish이 컨텐츠 조각에 대해 작동하지 않습니다. 사용자가 관리 게시를 시도하면 제대로 작동합니다. 그러나 빠른 Publish을 시도했을 때 게시되지 않았습니다. 특히 `com.day.cq.wcm.core.impl.reference.ActivationReferenceSearchBuilder` 때문에 시스템이 스래시되었습니다. (SITES-17311)
...
* AEM이 취약성 보고서로 인해 `whitelist`이라고도 하는 허용 목록에 추가하다를 사용하고 있었습니다. 허용 목록에 추가하다으로 인해 고객이 필요한 기능을 사용할 수 없었습니다. (SITES-16822)
...
* 자산이 AEM에 업로드되면 `Update_asset` 워크플로우가 트리거됩니다. 그러나 워크플로우는 완료되지 않습니다. 워크플로우는 제품 업로드 단계까지만 완료됩니다. 다음 단계는 Scene7 일괄 업로드이지만 해당 프로세스를 AEM으로 가져오지는 않습니다. (ASSETS-30443)
* Dynamic Media 구성 요소에서 Dynamic Media이 아닌 비디오를 우아하게 처리하는 더 나은 방법이 필요합니다. 이 문제로 인해 `dynamicmedia_sly.js`을(를) 인스턴스화하는 동안 예외가 발생했습니다. (ASSETS-31301)
...
* 사용자가 `Home>Services>PDF Generator>Adobe PDF Settings` 경로 아래에서 ADMINUI의 글꼴을 선택하면 글꼴이 선택되지 않습니다. 또한 표준 또는 개인화된 프로필에서는 사용할 수 있는 글꼴 목록 상자가 비어 있습니다. 따라서 **항상 포함** 또는 **포함하지 않음**&#x200B;의 하위 목록을 개인화할 수 없습니다. PDF Generator PDF에 대한 글꼴을 구성할 수 없습니다. 로그에는 관련 오류 메시지가 표시되지 않습니다. (FORMS-12095)
...
* AEM에서 **[!UICONTROL 도구]** > **[!UICONTROL 일반]** > **[!UICONTROL 태그 지정]** > **[!UICONTROL 만들기]** > **[!UICONTROL 태그 만들기]**&#x200B;를 사용할 때 **제목** 필드에 라틴어가 아닌 문자를 입력하면 **이름** 필드에 하이픈 문자(`-` )로만 채워집니다. (NPR-41623)
...
* [!DNL Experience Manager] 6.5.20.0에는 [!DNL Experience Manager] 6.5가 필요합니다. 자세한 지침은 [업그레이드 설명서](/help/sites-deploying/upgrade.md)를 참조하세요. <!-- UPDATE FOR EACH NEW RELEASE -->
...
>[!IMPORTANT]
>
> Adobe은 [!DNL Experience Manager] 6.5.20.0 패키지를 제거하거나 제거하지 않는 것이 좋습니다. 따라서 팩을 설치하기 전에 `crx-repository`을(를) 롤백해야 하는 경우 백업을 만들어야 합니다. <!-- UPDATE FOR EACH NEW RELEASE -->
...
[!DNL Experience Manager] 6.5.20.0.<!-- UPDATE FOR EACH NEW RELEASE -->을(를) 자동으로 설치하는 데 사용할 수 있는 두 가지 방법이 있습니다.
...
1. 제품 정보 페이지(`/system/console/productinfo`)에는 [!UICONTROL 설치된 제품]에 업데이트된 버전 문자열 `Adobe Experience Manager (6.5.20.0)`이(가) 표시됩니다. <!-- UPDATE FOR EACH NEW RELEASE -->
...
[!DNL Experience Manager] 6.5.20.0용 UberJar를 [Maven 중앙 저장소](https://repo.maven.apache.org/maven2/com/adobe/aem/uber-jar/6.5.20/)에서 사용할 수 있습니다. <!-- CHECK FOR UPDATE EACH NEW RELEASE -->
...
* 대화형 통신 서비스에서 PDF 문서를 만들지만 사용자의 데이터가 양식 필드에 자동으로 채워지지 않습니다. 미리 채우기 서비스가 예상대로 작동하지 않습니다. [최신 AEM 6.5 양식 서비스 팩 20(6.5.20.0)](https://experienceleague.adobe.com/en/docs/experience-manager-release-information/aem-release-updates/forms-updates/aem-forms-releases)을 설치하여 문제를 해결할 수 있습니다. (FORMS-13413, FORMS-13493)
* automated forms conversion 서비스의 RnC(Review and Correct) 편집기를 로드하지 못했습니다. [최신 AEM 6.5 양식 서비스 팩 20(6.5.20.0)](https://experienceleague.adobe.com/en/docs/experience-manager-release-information/aem-release-updates/forms-updates/aem-forms-releases)을 설치하여 문제를 해결할 수 있습니다. (FORMS-13491)
...
* [Experience Manager 6.5.20.0에 포함된 콘텐츠 패키지 목록](/help/release-notes/assets/65200-packages.txt) <!-- UPDATE FOR EACH NEW RELEASE -->
