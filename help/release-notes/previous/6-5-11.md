...
* 내장된 저장소(Apache Jackrabbit Oak)가 1.22.9으로 업데이트되었습니다.

* 이제 JEE의 AEM Forms에서 캐싱 솔루션으로 Apache Geode를 지원합니다.
...
* 위치 [!DNL Dynamic Media]: 비디오 인코딩 프로필을 만드는 인터페이스에서 [!UICONTROL 스마트 자르기 비율] 옵션은 화면 판독기에서 적절하게 알려주도록 접근성에 대해 레이블이 지정됩니다.
...
* 대소문자를 혼용하여 도메인 이름을 사용하는 이메일을 가진 사용자가 에셋을 체크아웃할 때 에셋은에서 사용자의 체크아웃된 에셋에 표시되지 않습니다 [!DNL Asset Link] (CQ-4329266).
...
* PDF 처리 오류가 다음으로 전파되지 않음 [!DNL Experience Manager] 다음 위치에 오류가 있는 경우 [!DNL Scene7] (CQ-4331145).
...
* 포함된 컨테이너 워크플로가 있는 경우 을(를) 선택한 후에도 여러 워크플로 완료 이메일이 전송됩니다. `Notify on Complete of Container Workflow` 옵션(NPR-37280).
...
1. OSGi 번들 `org.apache.jackrabbit.oak-core` 는 버전 1.22.9 이상입니다(웹 콘솔 사용: `/system/console/bundles`).
