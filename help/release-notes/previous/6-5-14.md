...
[!DNL Experience Manager] 6.5.14.0에는 2019년 4월 6.5의 최초 출시 이후 릴리스된 새로운 기능, 주요 고객 요청 개선 사항, 버그 수정 사항, 성능, 안정성 및 보안 개선 사항이 포함되어 있습니다. [!DNL Experience Manager] 6.5에 [이 서비스 팩 설치](#install). <!-- UPDATE FOR EACH NEW RELEASE -->
...
* `role="combo box"`에 대한 필수 ARIA(Accessible Rich Internet Applications) 특성 `aria-expanded`이(가) 없습니다. (ASSETS-6928)
* 카드 보기의 기본 파일 탐색 영역에서 텍스트 콘텐츠 **[!UICONTROL 정렬 기준]**&#x200B;의 배경색에 대한 대비 비율이 4.5:1 이상입니다. (ASSETS-6926)
* Experience Manager이 워크플로 모델을 만드는 동안 **[!UICONTROL 워크플로 모델 선택]** 드롭다운 목록을 필수 필드로 식별하지 않습니다. (ASSETS-6871)
...
* 관리자가 아닌 사용자는 Dynamic Media **[!UICONTROL 일반 설정]** 및 **[!UICONTROL Publish 설정]**&#x200B;에서 옵션을 구성할 수 없습니다. (ASSETS-8628)
...
* **[!UICONTROL 검색할 유형]** 대화 상자에서 `<frame>` 및 `<iframe>`에 대해 제공되지 않는 `title` 특성으로 인해 화면 판독기 사용자가 영향을 받습니다. (ASSETS-5483)
* 화면 판독기에서 왼쪽 창의 **[!UICONTROL Assets]** 머리글 아래에 있는 여러 이미지에 대해 관련되고 의미 있는 `alt=` 값을 제공해야 합니다. (ASSETS-5644)
...
* Workspace HTML에서 워크플로우에 대한 탐색 링크를 선택할 때 탐색 링크가 선택되었음이 표시되지 않습니다. (NPR-37138)
...
* Forms 6.5 Designer으로 만든 정적 PDF forms의 경우 `Tab order entry in page with annotations not set to "S"` 오류로 인해 PDF 접근성에 실패합니다. (CQ-4343117)
...
* 화면 판독기는 액세스 가능한 이름(예: `Show description for Archive` 또는 `Show description for mini shopping cart`)을 알려주어야 합니다. 그러나 현재 액세스 가능한 이름은 _모두_&#x200B;의 도구 설명 정보 단추에 대해 `Info Circle button show description`(으)로 표시됩니다. (SITES-3104)
...
* `cq:actions` 속성을 사용자 지정한 후 편집 가능한 템플릿의 메뉴 표시줄에 **[!UICONTROL Delete]** 및 **[!UICONTROL Cut]** 단추가 없습니다. (NPR-38521)
* 구성 요소에 다른 구성 요소가 포함되어 있으면 메뉴 모음에 **[!UICONTROL 삭제]** 단추가 없기 때문에 템플릿 구조 내에서 구성 요소를 삭제할 수 없습니다. (NPR-38585)

## 슬링 {#sling-6514}

* `org.apache.sling.discovery.commons`, 버전 1.0.20의 `DiscoveryLiteDescriptor` 모듈에서 메모리 누수로 인해 프로덕션의 열린 파일 수가 증가했습니다. (NPR-38288)
* Experience Manager에서 **[!UICONTROL 작업]** > **[!UICONTROL 진단]**&#x200B;에서 **[!UICONTROL 상태 ZIP 다운로드]** > **[!UICONTROL 다운로드]**&#x200B;를 선택할 때 오류가 발생합니다. (NPR-38514)
...
* **[!UICONTROL 모두 선택]** > **[!UICONTROL 빠른 Publish]**&#x200B;을(를) 사용할 때 Experience Manager이 모든 자산을 게시하지 않았거나 **[!UICONTROL 카드]** 보기 또는 **[!UICONTROL 목록]** 보기에 게시되는 자산 수를 표시하지 않았습니다. (NPR-38546)
...
* [!DNL Experience Manager] 6.5.14.0에는 [!DNL Experience Manager] 6.5가 필요합니다. 자세한 지침은 [업그레이드 설명서](/help/sites-deploying/upgrade.md)를 참조하세요. <!-- UPDATE FOR EACH NEW RELEASE -->
...
>[!IMPORTANT]
>
>Adobe은 [!DNL Experience Manager] 6.5.14.0 패키지를 제거하거나 제거하지 않는 것이 좋습니다. 따라서 서비스 팩을 설치하기 전에 `crx-repository`을(를) 롤백해야 하는 경우 백업을 만들어야 합니다. <!-- UPDATE FOR EACH NEW RELEASE -->
...
[!DNL Experience Manager] 6.5.14.0.<!-- UPDATE FOR EACH NEW RELEASE -->을(를) 자동으로 설치하는 데 사용할 수 있는 두 가지 방법이 있습니다.
...
1. 제품 정보 페이지(`/system/console/productinfo`)에는 [!UICONTROL 설치된 제품]에 업데이트된 버전 문자열 `Adobe Experience Manager (6.5.14.0)`이(가) 표시됩니다. <!-- UPDATE FOR EACH NEW RELEASE -->
...
[!DNL Experience Manager] 6.5.13.0용 UberJar를 [Maven 중앙 저장소](https://repo.maven.apache.org/maven2/com/adobe/aem/uber-jar/6.5.13/)에서 사용할 수 있습니다. <!-- CHECK FOR UPDATE EACH NEW RELEASE -->
...
* [AEM 콘텐츠 조각(GraphQL 인덱스 패키지 1.0.5 포함](https://experience.adobe.com/#/downloads/content/software-distribution/en/aem.html?package=%2Fcontent%2Fsoftware-distribution%2Fen%2Fdetails.html%2Fcontent%2Fdam%2Faem%2Fpublic%2Fadobe%2Fpackages%2Fcq650%2Ffeaturepack%2Fcfm-graphql-index-def-1.0.5.zip)
...
* [Experience Manager 6.5.14.0에 포함된 콘텐츠 패키지 목록](/help/release-notes/assets/65140_packages.txt) <!-- UPDATE FOR EACH NEW RELEASE -->
