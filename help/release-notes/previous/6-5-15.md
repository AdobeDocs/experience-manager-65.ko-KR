...
[!DNL Experience Manager] 6.5.15.0에는 2019년 4월 6.5의 최초 출시 이후 릴리스된 새로운 기능, 주요 고객 요청 개선 사항, 버그 수정 사항, 성능, 안정성 및 보안 개선 사항이 포함되어 있습니다. [!DNL Experience Manager] 6.5에 [이 서비스 팩 설치](#install). <!-- UPDATE FOR EACH NEW RELEASE -->
...
* Experience Manager 파일에서 게시 관리의 [!UICONTROL 예약] 라디오 단추에서 옵션 중 하나를 선택하는 동안 라디오 단추 옵션 이름과 상태는 화면 판독기에 표시됩니다. 그러나 **예약** 레이블은 발표되지 않습니다. (ASSETS-6908, ASSETS-6906)
* 정렬 아이콘에 대한 대체 텍스트가 누락되었습니다. (ASSETS-6904)
* 자산 속성 페이지에서 IPTC 확장 탭 레이블의 필드 이름 `Person`이(가) 화면 판독기에 표시되지 않습니다. 화면 판독기는 편집 가능하고 현재 비어 있는 필드만 알려주고 레이블 이름은 알려주지 않습니다. (ASSETS-6903, ASSETS-6848)
...
* Experience Manager 컬렉션에서 [!UICONTROL 전자 메일] 필드가 필수 필드로 식별되지 않으며 값을 지정하지 않으면 오류 메시지가 표시되지 않습니다. (ASSETS-6877)
* Experience Manager 파일에서 `Add Email Address`에 **링크 공유** 화면에 오류 메시지가 표시되지 않습니다. 오류는에서 색상을 사용해서만 식별됩니다. (ASSETS-6876, ASSETS-6875)
...
* 키보드 포커스가 **도움말 표시** 메뉴에서 사용할 수 있는 `About Adobe Experience Manager` 옵션을 건너뜁니다. (ASSETS-6845)
...
* 복제 액세스 권한이 없는 사용자와 `ETC` 및 `VAR`에 대한 `READ` 및 `WRITE` 액세스 권한이 있는 사용자의 **게시 관리** 옵션이 사라집니다. (ASSETS-15007)
...
* **[!UICONTROL Copy]** 또는 **[!UICONTROL Livecopy]** 작업에서 `cq:targetMetadata` 정보가 잘못 중복되었습니다. 이 기능으로 인해 Experience Manager에 있는 두 개 이상의 경험 조각이 target에서 내보낸 동일한 오퍼를 가리킵니다. (NPR-38970)
...
* [!DNL Experience Manager] 6.5.15.0에는 [!DNL Experience Manager] 6.5가 필요합니다. 자세한 지침은 [업그레이드 설명서](/help/sites-deploying/upgrade.md)를 참조하세요. <!-- UPDATE FOR EACH NEW RELEASE -->
...
>[!IMPORTANT]
>
>Adobe은 [!DNL Experience Manager] 6.5.15.0 패키지를 제거하거나 제거하지 않는 것이 좋습니다. 따라서 팩을 설치하기 전에 `crx-repository`을(를) 롤백해야 하는 경우 백업을 만들어야 합니다. <!-- UPDATE FOR EACH NEW RELEASE -->
...
[!DNL Experience Manager] 6.5.15.0.<!-- UPDATE FOR EACH NEW RELEASE -->을(를) 자동으로 설치하는 데 사용할 수 있는 두 가지 방법이 있습니다.
...
1. 제품 정보 페이지(`/system/console/productinfo`)에는 [!UICONTROL 설치된 제품]에 업데이트된 버전 문자열 `Adobe Experience Manager (6.5.15.0)`이(가) 표시됩니다. <!-- UPDATE FOR EACH NEW RELEASE -->
...
[!DNL Experience Manager] 6.5.15.0용 UberJar를 [Maven 중앙 저장소](https://repo.maven.apache.org/maven2/com/adobe/aem/uber-jar/6.5.15/)에서 사용할 수 있습니다. <!-- CHECK FOR UPDATE EACH NEW RELEASE -->
...
* AEM Forms JEE 6.5.15.0 서비스 팩이 JBoss Linux 환경에 제대로 설치되지 않았습니다. `PatchInstallerProcessing[1-9*].log` 파일에서 로그 항목 `[AEM_Forms_JEE_DIR]/patch/AEMForms-6.5.0-0057/xml/RUP_BOM.xml not found! Assuming this component is not in the installation. Skipping Processing`을(를) 확인하여 AEM Forms JEE 6.5.15.0 서비스 팩의 올바른 설치를 확인합니다. 이 항목은 AEM Forms JEE 6.5.15.0 서비스 팩 설치가 실패했음을 나타냅니다. AEM Forms JEE 6.5.15.0 서비스 팩의 설치 문제를 해결하려면 수동으로 [RUP_BOM.xml 파일을 XML 디렉터리에 추가](/help/forms/using/linux-jboss-installation-issue-on-sp15.md)하십시오.
...
* [Experience Manager 6.5.15.0에 포함된 콘텐츠 패키지 목록](/help/release-notes/assets/65150_packages.txt) <!-- UPDATE FOR EACH NEW RELEASE -->
