...
description: 의 릴리스 정보 [!DNL Adobe Experience Manager] 6.5.4 서비스 팩
contentOwner: AK
mini-toc-levels: 2
...
* 이제 Adobe Experience Manager Assets는 다음을 통해 Brand Portal으로 구성됩니다. [!DNL Adobe I/O] 콘솔.
...
* 캐러셀 편집기가 Experience Manager Dynamic Media 하이브리드 모드로 IE 11에서 열리지 않습니다(CQ-425590). **Dynamic Media Adobe - 하이브리드 모드 고객의 경우:** Adobe은 2022년 5월 이후 Dynamic Media - 하이브리드 모드에서 Internet Explorer 11에 대한 지원을 종료합니다.
...
* 화면 판독기에는 선택한 태그를 제거하는 옵션이 표시됩니다(`X` 기호) 및 선택한 태그의 번호
...
[!DNL Experience Manager Assets]과 [!DNL Brand Portal] 사이의 인증 채널이 변경되었습니다. 이전에는 [!DNL Brand Portal]이 기존 OAuth 게이트웨이를 통해 클래식 UI에 구성되었으며, 이 게이트웨이는 인증을 위해 IMS 액세스 토큰을 가져오는 데 JWT 토큰 교환을 사용합니다. [!DNL Experience Manager Assets] 이제 다음으로 구성됨: [!DNL Brand Portal] 에서 [!DNL Adobe I/O]: 인증을 위한 IMS 토큰을 받습니다. [!DNL Brand Portal] 테넌트.
...
Experience Manager 6.5.4.0용 UberJar는에서 사용할 수 있습니다 [Maven Central 저장소](https://repo.maven.apache.org/maven2/com/adobe/aem/uber-jar/6.5.4/).
