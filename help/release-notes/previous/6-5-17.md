...
[!DNL Experience Manager] 6.5.17.0에는 2019년 4월 6.5의 최초 출시 이후 릴리스된 새로운 기능, 주요 고객 요청 개선 사항, 버그 수정 및 성능, 안정성, 보안 개선 사항이 포함되어 있습니다. [!DNL Experience Manager] 6.5에서 [이 서비스 팩을 설치](#install)합니다.
...
* **[Experience Manager Forms에서 정부용 Adobe Acrobat Sign 지원](/help/forms/using/adobe-sign-integration-adaptive-forms.md)**: 이제 AEM Forms이 정부용 Adobe Acrobat Sign과 통합됩니다(FedRAMP 규격). 이 통합은 공공기관 관련 계정(정부 부서 및 기관)에 대한 적응형 양식 제출과 함께 전자 서명의 고급 규정 준수 및 보안을 제공합니다. 공공기관용 Adobe Acrobat Sign과 통합하여 Adobe의 파트너와 공공기관 고객들은 가장 중요하고 민감한 비즈니스 라인에서 적응형 양식 전자 서명을 사용할 수 있습니다. 이 보안 계층이 추가되면 Adobe의 공공기관 고객들이 안심할 수 있도록 모든 전자 서명은 FedRAMP Moderate 규정을 완전히 준수해야 합니다.
...
* Adobe Sign 구성을 위해 `fd-cloudservice` 시스템 사용자에게 `modifyProperties` 권한을 추가하는 중입니다. (FORMS-6164)
...
1. OSGi 번들 `org.apache.jackrabbit.oak-core`의 버전이 1.22.15 이상입니다(웹 콘솔 사용: `/system/console/bundles`).
