...
description: 모바일 페이지를 작성할 때는 모바일 디바이스를 에뮬레이션하는 방식으로 페이지가 표시됩니다. 페이지 작성 시 여러 에뮬레이터 간을 전환하여 최종 사용자가 페이지에 액세스할 때 표시되는 내용을 확인할 수 있습니다.
...
1. 다른 에뮬레이터로 전환합니다. 이렇게 하려면 다음 중 하나를 수행할 수 있습니다.
   ...
   * 다음을 클릭합니다. **편집** 의 단추 **사이드 킥** 드롭다운 메뉴에서 디바이스를 선택합니다.
