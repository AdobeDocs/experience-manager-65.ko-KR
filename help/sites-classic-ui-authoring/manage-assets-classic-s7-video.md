...
Dynamic Media Classic 통합을 활성화하고 구성해도 바로 사용 가능한 두 가지 워크플로우 단계가 자동으로 제거되거나 비활성화되지는 않습니다 [!UICONTROL DAM 자산 업데이트] 수집 워크플로우. Adobe Experience Manager에서 이미 FFMPEG 기반 비디오 인코딩을 사용하고 있다면 작성 환경에 FFMPEG가 설치되어 있을 수 있습니다. 이 경우 Experience Manager Assets을 사용하여 수집된 새 비디오가 두 번 인코딩됩니다. FFMPEG 인코더에서 한 번, Dynamic Media Classic 통합에서 한 번,

Experience Manager의 FFMPEG 기반 비디오 인코딩이 구성되고 FFMPEG가 설치된 경우 Adobe에서 두 개의 FFMPEG 워크플로우를 제거하는 것이 좋습니다 [!UICONTROL DAM 자산 업데이트] 워크플로우.
...
1. Dynamic Media Classic 데스크탑 앱에서, [Dynamic Media Classic에 예약된 FTP 업로드 및 인코딩을 설정(시스템 자동화)](https://experienceleague.adobe.com/docs/dynamic-media-classic/using/upload-publish/uploading-files.html#upload-options).
1. Experience Manager에서 **[!UICONTROL Dynamic Media Classic]** 컨텐츠 파인더의 탭.
1. Dynamic Media Classic 비디오 구성 요소를 사용하여 작성합니다.

### Dynamic Media Classic 비디오와의 통합 구성 {#configuring-integration-with-scene-video}

1. in **[!UICONTROL Cloud Services]**&#x200B;로 이동하여 **[!UICONTROL Dynamic Media Classic]** 구성 및 선택 **[!UICONTROL 편집]**.
1. **[!UICONTROL 비디오]** 탭을 선택합니다.

   >[!NOTE]
   >
   >페이지에 클라우드 구성이 없는 경우 **[!UICONTROL 비디오]** 탭이 표시되지 않습니다. 자세한 내용은 [WCM용 Dynamic Media Classic 활성화](#enablingscene7forwcm).

1. 응용 비디오 인코딩 프로필, 곧바로 사용 가능한 단일 비디오 인코딩 프로필 또는 사용자 지정 비디오 인코딩 프로필을 선택합니다.

   >[!NOTE]
   >
   >비디오 사전 설정의 의미에 대한 자세한 내용은 [비디오 파일 인코딩을 위한 비디오 사전 설정](https://experienceleague.adobe.com/docs/dynamic-media-classic/using/setup/application-setup.html#video-presets-for-encoding-video-files).
   >
...
Dynamic Media Classic에서 사전 설정이 업데이트된 경우 Experience Manager에서 비디오에 대한 뷰어 및 인코딩 사전 설정을 업데이트합니다. 이 경우 클라우드 구성에서 Dynamic Media Classic 구성으로 이동한 후 을(를) 선택합니다 **뷰어 및 인코딩 사전 설정 업데이트**.
...
1. 선택 **[!UICONTROL 업로드]** 기본 소스 비디오를 업로드하려면 [!UICONTROL DAM 자산 업데이트] 워크플로우가 완료되고 **[!UICONTROL Dynamic Media Classic에 게시]**&#x200B;가 선택되면 비디오 업로드 및 인코딩이 완료됩니다.

   >[!NOTE]
   >
   >비디오 썸네일을 생성하는 데 시간이 다소 걸릴 수 있습니다.

   DAM 기본 소스 비디오를 비디오 구성 요소로 드래그하면 비디오 구성 요소에 액세스합니다 *모두* Dynamic Media Classic은 전달을 위해 프록시 렌디션을 인코딩했습니다.
...
>[!NOTE]
>
>곧바로 사용할 수 있는 Dynamic Media Classic 비디오 구성 요소는 범용 비디오 프로필을 사용합니다. 그러나 Experience Manager에서 사용할 HTML 5 기반 비디오 플레이어를 가져올 수 있습니다. Dynamic Media Classic에서 기본 제공 HTML5 비디오 플레이어의 포함 코드를 복사하여 Experience Manager 페이지에 넣습니다.

## Experience Manager 비디오 구성 요소 {#aem-video-component}

Dynamic Media Classic 비디오 구성 요소를 사용하는 것이 Dynamic Media Classic 비디오를 보는 데 권장되지만, 이 섹션에서는 [!UICONTROL 기본 비디오 구성 요소] 완전성을 위해 Experience Manager에서.
...
1. Experience Manager에서 **[!UICONTROL 도구]**&#x200B;를 선택하고 을 선택합니다. **[!UICONTROL 구성 콘솔]**.
1. 구성 콘솔에서 다음 위치로 이동합니다. **[!UICONTROL 도구]** > **[!UICONTROL 자산]** > **[!UICONTROL 비디오 프로필]** 탐색 트리에서
1. Dynamic Media Classic 비디오 프로필을 만듭니다. 에서 **[!UICONTROL 새로 만들기]** 메뉴, 선택 **[!UICONTROL 페이지 만들기]**.
1. Dynamic Media Classic 비디오 프로필 템플릿을 선택합니다. 새 비디오 프로필 페이지에 이름을 지정하고 을(를) 선택합니다 **[!UICONTROL 만들기]**.
