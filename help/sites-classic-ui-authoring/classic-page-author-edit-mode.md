...
다음 구성 요소는 표준 웹 페이지의 컨텐츠를 작성할 때 사용하기 위한 것입니다. 구성 요소는 AEM의 표준 설치에 기본적으로 사용할 수 있는 구성 요소의 하위 집합을 형성합니다.
...
컨텐츠 파인더에서 이미지를 끌어 놓거나 클릭하여 이미지를 로드할 수 있는 찾아보기 창을 엽니다. 이미지가 로드되면 이미지를 자르거나, 회전하거나, 삭제할 수 있습니다. 이미지를 확대/축소하려면 이미지 아래에서 확인 및 취소 단추 위에 있는 슬라이드 막대를 사용합니다.
   ...
   * **자르기**
이미지의 일부를 클립할 수 있습니다. 테두리를 끌어서 이미지를 자릅니다.
...
여기서는 회전판에 페이지가 포함되는 방법을 지정합니다.
   ...
   * **목록 작성 방법**
페이지 목록을 만드는 방법에는 하위 페이지, 고정 목록, 검색 또는 고급 검색(아래에 모두 설명됨)이 있습니다. 어떤 방법을 선택하더라도 목록에 포함된 각 페이지에는 이미지가 이미 연결되어 있어야 합니다. 회전판에 표시되는 이미지입니다. 페이지의 속성 아래에 페이지에 대한 이미지가 없는 경우 시작하기 전에 페이지에 이미지를 연결해야 합니다. 그렇지 않으면 회전판에 빈 페이지가 표시됩니다. 자세한 내용은 [페이지 속성 편집](/help/sites-classic-ui-authoring/classic-page-author-edit-page-properties.md).
...
>[!NOTE]
>
>AEM DAM에 디지털 자산을 표시하는 Adobe Experience Manager의 사용자 지정 회전판 구성 요소를 만들 수 있습니다.
...
다운로드 링크에 표시되는 짧은 설명입니다.
   ...
   * **파일**
결과 웹 페이지에서 다운로드할 수 있는 파일입니다. Content Finder에서 자산을 드래그하거나 영역을 선택하여 다운로드에 사용할 수 있도록 하려는 파일을 업로드할 수 있습니다.
...
* **자르기**
   ...
   이미지의 일부를 잘라낼 수 있도록 자르기를 선택합니다. 마우스를 사용하여 이미지를 자를 수 있습니다.
...
특정 단락을 참조하는 것 외에도 경로를 수정하여 전체 단락 시스템을 지정할 수도 있습니다. 경로를 다음과 같이 접미사로 붙입니다.
...
예를 들어 `Geometrixxe`를 입력하면 &quot;무슨 말씀이세요? Geometrixx&quot;.
   ...
   * **유사 페이지 텍스트**
      ...
      유사한 페이지의 결과 옆에 표시되는 텍스트입니다. 이미지의 일부를 잘라내려면 이 링크를 클릭합니다.
...
이 구성 요소를 사용하면 일련의 이미지를 순서대로 로드하여 페이지에 표시합니다. 이미지를 추가하거나 제거하고 각 이미지에 제목을 지정할 수 있습니다. 고급(Advanced)에서 표시 영역의 크기를 지정할 수도 있습니다.
...
* **슬라이드**
   ...
   * **제목**
      ...
      필요한 경우 제목을 지정합니다. 이 제목은 적절한 슬라이드에 오버레이됩니다.
...
>
>Adobe은 다음을 사용할 것을 권장합니다. **표** 구성 요소를 사용하여 구성할 수도 있지만 **텍스트** 구성 요소.

다음 **표** 구성 요소는 표를 만들고, 채우고, 형식을 지정할 수 있도록 미리 구성되어 있습니다. 대화 상자를 사용하여 테이블을 구성하고 내용을 처음부터 만들거나 Excel, OpenOffice 또는 메모장과 같은 외부 편집기에서 스프레드시트나 표를 복사하여 붙여넣을 수 있습니다.
...
그런 다음 서식이 지정된 텍스트가 페이지에 표시됩니다. 실제 디자인은 사이트 CSS에 따라 다릅니다.
...
열은 AEM에서 컨텐츠 레이아웃을 제어하는 메커니즘입니다. 표준 설치에서 2 및/또는 3개의 열을 만드는 구성 요소가 제공됩니다.

다음 예제는 사용 중인 열 2개와 열 구성 요소 3개를 보여줍니다. 새 구성 요소에 대한 자리 표시자를 사용할 수 있습니다.
...
* **양식**
   ...
   양식 구성 요소는 페이지에서 새 양식의 시작 부분과 끝 부분을 정의합니다. 이러한 요소 사이에 표 및 다운로드와 같은 다른 구성 요소를 배치할 수 있습니다.
...
>[!NOTE]
>
>양식 개발 및 사용자 지정에 대한 정보는 [Forms 개발 페이지](/help/sites-developing/developing-forms.md). 이 양식 사용자 지정에는 작업 추가, 제한, 필드 미리 로드 및 스크립트를 사용하여 작업을 수행하는 작업이 포함됩니다.
...
* **제한**
   ...
   여기에서 필수 필드인지 여부 및 필드의 제한(예: 숫자 포함)을 지정할 수 있습니다.
...
이러한 탭에서는 필요한 매개 변수를 제공합니다. 탭은 개별 구성 요소 유형에 따라 다르지만, 다음을 포함할 수 있습니다.
...
이 필드는 필수 항목이며 다음 문자만 포함해야 합니다.
   ...
   * **설명**
      ...
      필요한 경우 사용자에 대한 추가 정보를 제공할 수 있습니다. 양식에서 제목보다 더 작은 글꼴로 필드 아래에 이 설명이 표시됩니다.
...
* **제한**
   ...
   * **필수 메시지**
      ...
      사용자에게 이 필드가 필수임을 알리는 메시지입니다. 필수 필드에 별표가 표시됩니다.
...
양식 구성 요소는 **양식 시작** 및 **양식 끝** 요소를 생성하지 않습니다. 시작 요소와 끝 요소는 항상 쌍을 이루어야 양식이 올바르게 정의됩니다.
...
필요한 이 구성 요소는 페이지에서 새 양식의 시작을 정의합니다. 다음을 구성할 수 있습니다.
...
이 필드는 선택 사항이며 저장소의 노드에 대한 경로를 지정합니다. 이 노드에 필드 이름과 일치하는 속성이 있으면 양식의 해당 필드에 해당 속성의 값이 미리 로드됩니다. 일치하는 항목이 없으면 필드에 기본값이 포함됩니다.
**로드 경로**&#x200B;를 사용하여 필수 필드에 값이 있는 양식을 미리 로드할 수 있습니다. [양식 값 미리 로드](/help/sites-developing/developing-forms.md#preloading-form-values)를 참조하십시오.
   ...
   * **클라이언트 유효성 검사**
      ...
      이 양식에 대해 클라이언트 유효성 검사가 필요한지 여부를 나타냅니다(서버 유효성 검사). *항상* 발생합니다. 유효성 검사는 **Forms Captcha** 구성 요소.
...
계정을 만들 때 사용되는 페이지입니다.
      * **컨텐츠 만들기**
         ...
         * 컨텐츠 경로 양식에 포함된 내용을 저장할 컨텐츠 경로입니다. 슬래시 `/`로 끝나는 경로를 입력합니다. 즉, 양식을 게시할 때마다 지정된 위치에 새 노드가 만들어집니다(예: /).
            `/forms/feedback/`
...
필요한 리드 목록을 지정합니다.
      * **계정 만들기 및 업데이트**
         ...
         * **데이터 보기...**
            ...
            벌크 편집기에서 양식 결과에 대한 정보에 액세스할 수 있도록 단추를 선택합니다. 여기에서 정보를 `.tsv`(탭으로 구분) 파일로 내보낼 수 있습니다(예를 들면 Excel 스프레드시트에서 사용하기 위해).
      * **메일**
...
이 구성 요소로 숨김 필드를 만들어 이러한 숨김 필드는 양식을 제출한 후 작업을 수행해야 할 때, 또는 사후 처리에서 데이터를 숨겨야 할 때와 같이 다양한 용도로 사용할 수 있습니다.
...
링크 필드를 사용하면 사용자가 URL을 지정할 수 있습니다.
...
특수 탭을 사용하여 사용할 수 있는 네임스페이스와 같은 다양한 매개 변수를 지정할 수 있습니다.
