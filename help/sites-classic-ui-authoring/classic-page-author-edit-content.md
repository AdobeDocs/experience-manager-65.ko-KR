...
* 돋보기 아이콘을 사용하면 페이지가 게시 환경에 표시될 상태로 표시되는 미리보기 모드를 선택할 수 있습니다(경우에 따라 페이지 새로 고침이 필요함).
  ...
  ![아래쪽 화살표 기호가 표시된 제목 오른쪽에 있는 편집 모드 아이콘과 제목이 AEM인 막대.](do-not-localize/chlimage_1-8.png)
...
AEM은 사용자가 수행한 작업 내역과 해당 작업을 수행한 시퀀스를 저장합니다. 따라서 수행한 순서대로 여러 작업을 실행 취소합니다. 그런 다음 다시 실행을 사용하여 하나 이상의 작업을 다시 적용할 수 있습니다.
