...
* 구성 요소 대화 상자가 있는 경우 지정된 콘텐츠를 렌더링하는 데 사용되는 구성 요소는 암시적이므로 `sling:resourceType` 단락이 만들어지면 단락의 속성이 자동으로 채워집니다. 스캐폴드를 사용하면 주어진 단락에 대해 콘텐츠와 지정된 구성 요소를 정의하는 모든 정보를 대화 상자 자체에서 제공해야 합니다. 스캐폴드 대화 상자에서 다음을 사용하여 이 정보를 제공해야 합니다. *숨김* 페이지 생성 시 이 정보를 제출하는 필드입니다.
