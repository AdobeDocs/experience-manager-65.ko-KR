...
* **필수 메시지**  - 원할 경우 필드가 필수임을 알리는 메시지를 추가합니다.
...
* **제한 - 제한**  - 드롭다운 -  **없음**,  **이메일**,  **이름** (모음 기호 없음)을 선택하여 이메일 주소, 이름 또는 제한 조건 없음을 추가할 수 있습니다. 이메일을 선택하는 경우, 사용자가 필드에 입력하는 응답의 형식은 이메일 주소여야 합니다. 이름을 선택하는 경우 이름이어야 합니다(모음 기호는 허용되지 않음).
