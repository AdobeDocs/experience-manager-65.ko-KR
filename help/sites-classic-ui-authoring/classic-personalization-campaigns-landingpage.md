...
>[!CAUTION]
>
>디자인 가져오기 프로그램을 사용하려면 `/apps`에 액세스해야 하므로 `/apps`을(를) 변경할 수 없는 컨테이너화된 클라우드 환경에서는 이 프로그램이 작동하지 않습니다.
...
페이지 가져오기에 HTML 준비, 확인 및 페이지 테스트가 포함되므로 랜딩 페이지 가져오기는 관리자 작업으로 사용됩니다. 관리자는 `/apps`에 대한 읽기, 쓰기, 생성 및 삭제 권한이 있어야 합니다. 사용자에게 이러한 권한이 없으면 가져오기에 실패합니다.

>[!NOTE]
>
>디자인 가져오기는 `/apps`에 대한 읽기, 쓰기, 만들기 및 삭제 권한이 필요한 관리 도구로 사용되므로 Adobe은 프로덕션 환경에서 디자인 가져오기를 사용하지 않는 것을 권장합니다.
