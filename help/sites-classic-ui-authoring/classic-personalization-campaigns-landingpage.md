...
>[!CAUTION]
>
>디자인 가져오기에 액세스 권한이 필요하므로 `/apps`인 경우 `/apps` 는 변경할 수 없습니다.
...
**레이블** 사용자에게 표시되는 텍스트. 서식은 리치 텍스트 편집기로 수정할 수 있습니다.
...
**레이블** 사용자가 그래픽에서 보는 텍스트입니다. 서식은 리치 텍스트 편집기로 수정할 수 있습니다.
...
리드 양식 구성 요소는 **양식 시작** 및 **양식 끝** 요소를 생성하지 않습니다. 이러한 단락이 항상 쌍을 이루어야 양식이 올바르게 정의됩니다.
...
페이지 가져오기에 HTML 준비, 확인 및 페이지 테스트가 포함되므로 랜딩 페이지 가져오기는 관리자 작업으로 사용됩니다. 관리자는 가져오기를 수행하는 사용자에 대해 읽기, 쓰기, 만들기 및 삭제 권한이 있어야 합니다 `/apps`. 사용자에게 이러한 권한이 없으면 가져오기에 실패합니다.

>[!NOTE]
>
>디자인 가져오기는 읽기, 쓰기, 만들기 및 삭제 권한이 필요한 관리 도구로 사용되므로 `/apps`에서는 프로덕션 환경에서 디자인 가져오기를 사용하지 않는 것이 좋습니다.
...
가져온 디자인 패키지에 문제가 있을 경우, 예를 들어, 패키지 내에 없는 이미지/스크립트를 참조하는 경우 디자인 가져오기는 이와 같은 문제를 목록 양식으로 표시합니다. 문제 목록을 보려면, 일반 사용자 인터페이스에서 랜딩 페이지 도구 모음에 있는 문제 링크를 클릭합니다. 다음 이미지에서 **문제** 링크가 임포트 문제 창을 엽니다.
...
랜딩 페이지를 지울 때 컨텐츠 변경 사항을 제거할 수 있습니다. 를 클릭하면 **아니요**&#x200B;로 지정하는 경우 컨텐츠 변경 사항이 유지되며, 즉 의 구조가 유지됩니다 `jcr:content/importer`는 유지되고 Importer 페이지 구성 요소와 의 리소스만 유지됩니다 `etc/design` 가 제거됩니다. 반면에 **예**, `jcr:content/importer` 도 제거됩니다.
