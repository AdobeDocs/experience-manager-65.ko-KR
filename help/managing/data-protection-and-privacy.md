...
>[!CAUTION]
>
>해당 문서에 설명된 세부 내용은 Adobe Experience Manager로 제한됩니다.
>
...
AEM 제품 영역에 대한 아래의 규제 설명서 섹션을 참조하십시오.

## AEM Foundation {#aem-foundation}

[AEM Foundation의 데이터 보호 및 개인 정보 보호 요청 처리](/help/sites-administering/handling-gdpr-requests-for-aem-platform.md)를 참조하십시오.
...
[AEM Sites - 데이터 보호 및 개인 정보 보호 준비](/help/sites-administering/gdpr-compliance-sites.md)를 참조하십시오.
...
Adobe Experience Manager 통합은 데이터 보호 및 개인 정보 보호(예: GDPR 또는 CCPA) 준비 서비스를 이용합니다. 해당 통합과 관련하여 어떠한 Adobe Target 또는 Adobe Analytics의 개인 데이터도 AEM에 저장되지 않습니다.
...
* [Adobe Analytics 데이터 개인 정보 보호 워크플로](https://experienceleague.adobe.com/docs/analytics/admin/admin-tools/data-governance/an-gdpr-workflow.html?lang=ko)

## AEM Communities {#aem-communities}

AEM Communities는 [기본 제공 API](/help/communities/user-ugc-management-service.md)를 통해 데이터 주체에게 데이터 이동성 권리, 액세스 권리, 삭제권을 부여합니다. 이러한 API를 사용하면 사용자 생성 콘텐츠를 대량으로 삭제하거나 대량으로 내보내고 승인 가능한 ID를 통해 식별된 사용자 계정을 비활성화할 수 있습니다. 그러나 사용자 계정을 영구적으로 삭제하려면 CRXDE Lite에서 사용자 노드를 삭제함으로써 가능하며, 이를 통해 시스템에서 손쉽게 옵트아웃할 수 있는 필요성이 해결됩니다.

또한 AEM Communities는 대량 조정 콘솔을 통해 개인 정보 보호를 기본적으로 제공합니다. 이 콘솔을 사용하면 권한이 있는 멤버가 사용자의 기여 내용과 세부 정보를 찾아 삭제할 수 있습니다. 멤버 관리 콘솔을 사용하면 참여자를 금지하는 수준까지 제한할 수 있습니다. 또한 이 콘솔은 데이터 주체가 자신이 작성한 기여 내용을 삭제할 수 있는 권한을 부여합니다.

## AEM Forms {#aem-forms}

AEM Forms에는 비즈니스 프로세스를 조율하고 디지털 트랜잭션을 완료하기 위해 데이터를 캡처, 처리, 저장하는 구성 요소와 워크플로가 포함되어 있습니다. 다양한 구성 요소는 서로 다른 데이터 저장소를 사용하며 사용자 정의 데이터 저장소와의 통합도 허용합니다. 다음 설명서에서는 구성 요소의 데이터 보호 및 개인 정보 보호(예: GDPR 또는 CCPA) 워크플로를 지원하기 위해 사용자 데이터에 액세스하고 이를 처리하기 위한 절차와 지침을 설명합니다.
