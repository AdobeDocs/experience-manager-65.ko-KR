...
AEM을 구현할 때(특히 처음으로) [AEM 기능 및 워크플로우](https://business.adobe.com/products/experience-manager/adobe-experience-manager.html) 필요한 영역을 확인합니다.

사용 중인 AEM의 기능과 디자인에 미치는 영향을 고려해 보십시오. 예:
...
AEM은 다른 Adobe 제품, 타사 서비스 또는 둘 다에 통합될 수 있습니다. 이러한 워크플로우는 원하는 대로 성능과 기능을 높일 수 있습니다.
...
이전 버전에서 현재 버전으로 이동할 때에는 두 가지 옵션이 있습니다.

* 를 사용하십시오 [패키지 관리자](/help/sites-administering/package-manager.md) 이전 시스템에서 새 시스템으로 모든 컨텐츠 및 애플리케이션 코드를 내보냅니다.
* [업그레이드](/help/sites-deploying/upgrade.md) 기존 시스템. 일반적으로 이 방법이 권장됩니다.

## 기본 기초 규칙 {#basic-ground-rules}

어떤 프로젝트에서도 마찬가지로, 가능한 한 빨리 접점을 놓는 것이 중요하다. 이러한 규칙은 다음과 같습니다.
...
* **역할**
   ...
   역할은 명확히 정의되고 프로젝트에 관련된 모든 사람에게 알려져야 합니다. 또한 다음을 강조 표시하는 것이 좋습니다.
...
* **참여**
   ...
   가능한 한 빨리 관심 있는 사람들을 참여시킴으로써, 여러분은 그들이 되도록 격려할 수 있습니다 *이해* 를 입력합니다. 그렇게 하는 것은 성공에 대한 그들의 헌신을 증가시킨다.

   * 고객측에서 이 역할에는 시스템을 매일 사용하는 작성자가 포함됩니다
   * 프로젝트 팀 내에서 이러한 관여에는 품질 보증을 담당하는 사람도 포함됩니다. 고객의 요구 사항을 더 잘 이해할수록 테스트를 더 잘 계획할 수 있습니다.

* **통신 경로**

   * 통신 경로를 과도하게 형식화해서는 안 되지만, 특정 정의는 주요 사람이 항상 정보를 받고 따라서 최신 정보를 유지할 수 있도록 해야 합니다. 외부 당사자와의 소통에 특별한 관심을 기울여야 한다.

* **프로세스**

   정의된 프로세스는 개별 프로젝트에 따라 다릅니다. 다음 사항을 고려하여 이러한 프로세스를 단순하게 유지하십시오.

   * 타사와 상호 작용할 프로세스(및 통신 경로) 정의 예를 들어 설계 에이전시 및 기타 소프트웨어 공급업체 등이 있습니다.
   * 고객은 종종 자체 프로젝트 관리 및 보고 절차 및 도구를 갖게 됩니다.

* **추적 도구**

   버그, 작업 및 프로젝트의 다른 측면에 대한 정보를 추적하는 데 사용할 수 있는 다양한 도구가 있습니다. [잠재적 도구 개요](#overview-of-potential-tools) 자세한 내용

   * 여기에서 주목해야 할 핵심 사항은 정보의 한 복사본만 유지하고 정보를 공유하여 사용 중인 도구에 대한 액세스(따라서)하는 것입니다. 이 워크플로우는 유지 관리를 간소화하고 불일치를 방지하는 데 도움이 됩니다.
...
* **범위**
   ...
   * AEM 프로젝트.
   * 전체 프로젝트; 타사 소프트웨어, 테스트에 미치는 영향, 조직 문제 등 다양한 타사 소프트웨어를 포함합니다.
   * 특정 측면에서, 이 기능은 또한 무엇이 무엇인지 말하는 데 유용할 수 있습니다 *not* 프로젝트의 범위 내에서 본질적인 문제로 한정돼야만 하지만 잘못된 가정을 바로잡는 데 도움이 된다.

* **보고**

   보고하고자 하는 정보, 어떤 형태, 얼마나 자주, 누구에 대한 정보를 명확히 정의합니다.
...
이 정보는 프로젝트 안내서 내에 정의할 수 있습니다. Wiki를 사용하면 진행 중인 변경 사항이 효율적으로 처리되도록 할 수도 있습니다. 이러한 가정을 정의할 때 중요한 요소는 다음과 같습니다.

* 정보를 정의하고 유지 관리합니다
* 정보는 관련된 모든 사람들에게 분명하게 전달된다. 표준 프로젝트 관리 방식이지만 명확한 역할 정의와 양호한 커뮤니케이션이 프로젝트를 수행 또는 중단시킬 만큼 자주 반복될 수는 없습니다.
* 추적 중인 정보는 한 버전만 유지됩니다. 예를 들어 버그 추적 및 문제 추적.
...
지표는 웹 사이트의 품질에 대한 수량 측정을 정의하는 데 사용됩니다. 이러한 요소는 기본적으로 수행하려는 성능 목표에 대한 정의이며, 을 정의하는 데 사용할 수 있습니다 [KPI(주요 성과 지표)](#key-performance-indicators-and-target-metrics).

많은 지표를 정의할 수 있지만, 자주 정의하는 지표는 성능 및 동시성에 대한 목표를 다룹니다. 특히, 수량화하기가 어렵고 종종 쉽게 할 수 있는 요소들 *감정* 평가:

* &quot;웹 사이트: *너무 느림* 오늘&quot; - 언제 이 *느린 속도* 자격이 있습니까?

* &quot;모든 것&quot; *멈추기 위해 분쇄하다* 동료가 &quot; - 시스템에서 지원할 수 있는 동시 사용자는 몇 명입니까?
* &quot;내가 검색하면, *멈추기 위해 분쇄하다* &quot; - 어떤 검색 요청이 시스템에 영향을 줍니까?
...
* 달성하려는 최소 품질을 나타냅니다.
* 이러한 요소가 측정되는 방식을 정의합니다.
* 를 [주요 성능 지표](#key-performance-indicators-and-target-metrics)

타겟 지표를 정의할 때 항상 주의해야 하는 것처럼

* 너무 높게 설정하면, 그것을 달성할 수 없을 수도 있습니다
* 너무 낮은 변동을 설정하면 강조 표시되지 않을 수 있습니다
* 반복적이고 일관되게 측정할 수 있도록 하기 위해
* 측정되는 여러 요인들 간에 균형을 맞추다
* 테스트 환경과 관련된 특정 지표지만, 일부 지표에는 프로덕션 웹 사이트에 측정 및 재현성이 있어야 하는 실제 사례를 반영해야 합니다
* 웹 사이트에 대한 해당 중요도에 따라 지표의 우선 순위를 지정합니다
* 모니터링할 수 있는 세트로 지표 제한

프로젝트 개발 중에 적절하게 업데이트 및 조정할 수 있습니다. 프로젝트가 성공적으로 구현되면 설치 및 지속적인 작업을 위한 필수 서비스 수준을 모니터링/관리하는 데 도움이 되는 데 사용할 수 있습니다.

이러한 지표를 적절하게 사용하는 경우 유용한 도구를 제공할 수 있습니다. 무책임하게 사용될 때 그들은 시간을 낭비할 수 있다. 항상 여러분이 무엇을 측정하는지, 어떻게 측정하는지, 그리고 그 이유를 이해하세요.

>[!NOTE]
>
>이 섹션에서는 고려 사항에 대한 기본 원칙과 문제를 설명합니다. 각 설치는 다르므로 측정해야 할 실제 값은 다를 수 있습니다.

### 모든 것은 프로젝트 디자인에 있습니다 {#everything-rests-on-your-project-design}

측정된 모든 지표는 프로젝트 디자인의 영향을 받습니다. 반대로, 많은 문제들은 디자인 변화에 의해 가장 잘 해결된다.

따라서 타겟 지표를 정의합니다 *이전* 디자인을 결정하십시오. 이를 통해 이러한 요소를 기반으로 디자인을 최적화할 수 있습니다. 프로젝트가 개발되면 기본 디자인 원칙에 대한 작업은 어렵습니다.
...
디자인이 지침을 따르지 않는다고 생각되거나, 일부 의미를 잘 모르는 경우 이러한 문제를 명확히 하십시오. 프로그래밍 단계를 시작하거나 콘텐츠를 채우기 전에 다음을 수행하십시오.

### 인프라 {#infrastructure}

인프라를 정의하거나 평가하기 위해 다음과 같은 타겟 값을 정의하는 데 도움이 됩니다.
...
사용자의 상황 및 웹 사이트의 전략적 중요도에 따라 인프라를 정의하면 인프라를 평가하고 선택할 수 있습니다.
...
* 개별 페이지에서 응답 시간을 계산하며 다음을 수행합니다.
...
달성 가능한(평균) 응답 시간을 어떻게 결정할 수 있습니까? 문제는 경험이다.

* 웹 사이트에서의 경험
* AEM을 사용한 경험
* 평균 응답 시간을 초과하는 복잡한 페이지 인식(가능한 경우 이러한 페이지를 개별적으로 최적화해야 함)
...
* **AEM request.log를 사용하여 응답 시간 모니터링**
   ...
   성능 분석을 위한 좋은 시작점은 요청 로그입니다. 여러 가지 정보 중에서 개별 요청의 응답 시간을 확인할 수 있습니다. 자세한 내용은 [성능 최적화](/help/sites-deploying/configuring-performance.md) 자세한 내용
...
* 일반 성능에 미치는 영향
   ...
   * 검색 기능은 컨텐츠의 섹션이나 특별히 추출된 인덱스를 스캔해야 하므로 최적화되지 않은 경우 전체 시스템의 성능에 영향을 줄 수 있습니다
...
* 지속성 관리자
* 검색 색인
* 검색 기능의 복잡성 하나의 검색어를 입력할 수 있는 기본 검색 함수는 사용자가 AND/OR/NOT을 사용하여 복잡한 검색 문을 작성할 수 있는 고급 검색보다 빠릅니다.

이러한 검색 요청은 프로젝트 시작 시점부터 계획되고 통합되어야 합니다. 모니터링할 수 있는 메커니즘은 다음과 같습니다.
...
작성자와 게시 환경 모두에서 일부 사용자 및 방문자가 웹 사이트를 사용할 수 있도록 합니다. 테스트 시 사용한 것보다 숫자가 많기도 하지만 변화무쌍하고 예측하기도 어렵습니다. 성능에 부정적인 영향을 주지 않고 평균 동시 사용자 및 방문자 수에 대한 웹 사이트를 설계합니다. 다시 한 번, `request.log` 동시 실행 테스트를 수행하기 위해 자세한 내용은 [성능 최적화](/help/sites-deploying/configuring-performance.md) 자세한 내용

동시 사용자 수에 대한 Target은 환경 유형에 따라 다릅니다.

* **작성 환경**

   * 일반적으로 동시 사용자 수를 정확하게 예측할 수 있습니다. 동시에 일부 활성 상태인 것은 아니지만, 총 몇 명의 작성자가 있는지 알 수 있습니다.

* **게시 환경**

   * 게시 환경은 예측하기 더 어려우므로 대상 값을 선택해야 합니다. 즉, 현재 웹 사이트의 경험을 새로운 웹 사이트에 대한 현실적인 기대와 함께 기반으로 해야 합니다.
   * 특별 이벤트(예: 신규 인기 있는 컨텐츠를 게시할 때)는 기대치를 초과할 수 있으며 심지어 기능(특정 이벤트에 대한 티켓이 판매에 제공될 때 언론에 가끔 보고됨)을 초과할 수 있습니다.
...
* **용량 및 볼륨**
   ...
   | 출력 캐시 | AEM 서버의 서버 메모리(주 메모리 및 하드 드라이브). | 출력 캐시의 페이지 수 및 크기, 페이지당 종속성 수입니다. Dispatcher 캐시가 이 볼륨을 낮춥니다. |
...
특정 요구 사항에 따라 격리 또는 위의 분류를 설명하는 추가 지표를 정의하는 것이 유용할 수 있습니다.

그러나 웹 사이트의 모든 측면을 측정하고 정의하려고 하지 않고, 쉽고 안정적으로 작동하는 작은 정확한 코어 지표 세트를 사용하는 것이 좋습니다. 순전히 귀결에따라 웹 사이트가 변경되고 사용자에게 전달될 때 해당 웹 사이트가 발전하기 시작합니다.
...
>[!NOTE]
>
>프로덕션 환경의 현실적인 조건에서 조정, 최적화 및 사용자 교육을 허용하려면 프로젝트 시작을 **소프트 실행** (가용성, 복수 반복 횟수 감소) 및 **하드 론치** (전체 가용성 - 라이브)
...
그런 다음 결과 작업 목록에 따라 (높은 수준) 작업 정의에 대한 초기 예상 시간과 노력을 수행할 수 있습니다. 이러한 추정치에는 누가(고객 또는 파트너)가 무엇을 수행하고 언제 무엇을 하는지에 대한 표시가 포함되어야 합니다.
...
| 단계 | 작업 |
|---|---|
| 개발 | 모든 개발 요구 사항을 포함하는 각 구성 요소 노드에 대한 대략적인 추정 2~4시간. |
| 개발자 테스트 | 개발 15% |
| 후속 작업 | 개발 10% |
| 설명서 | 개발 15% |
| JavaDoc 설명서 | 개발 10% |
...
다음 목록은 사용할 수 있는 도구를 알려줍니다. 이것은 광범위한 추천 목록이 아니라 소개로 작성되었으며, 다른 도구를 사용하는 것을 막지 말아야 합니다.
...
<table>
 <tbody>
  ...
  <tr>
   <td>셀레늄</td>
   <td><a href="https://www.selenium.dev/">셀레늄</a> 는 오픈 소스 테스트 도구입니다. 테스트는 브라우저에서 직접 실행됩니다. 사용자가 작동하는 방식을 에뮬레이션합니다.</td>
  </tr>
  <tr>
  ...
  <tr>
   <td>Eclipse</td>
   <td><p>Eclipse는 다양한 프로젝트로 구성된 오픈 소스 IDE입니다. 이 API는 전체 수명주기에 걸쳐 소프트웨어를 구축, 배포 및 관리하기 위한 확장 가능한 프레임워크, 도구 및 런타임으로 구성된 개방형 개발 플랫폼을 구축하는 데 중점을 두고 있습니다.</p> <p>자세한 내용은 <a href="/help/sites-developing/howto-projects-eclipse.md">Eclipse를 사용하여 AEM 프로젝트를 개발하는 방법</a> 추가 정보.</p> </td>
  </tr>
  <tr>
   <td>IntelliJ</td>
