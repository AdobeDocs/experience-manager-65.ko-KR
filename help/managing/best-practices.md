...
AEM(Adobe Experience Manager)을 구현하기 위해 프로젝트를 관리하려면 프로젝트 구현 전 및 동시에 수행해야 하는 문제와 (관련) 의사 결정을 인식하도록 계획 및 이해가 필요합니다.
...
* 설명서, 다음을 기반: [체크리스트](/help/managing/best-practices-checklist.md), 자세한 내용:
...
* **시스템 통합**
   ...
   * 오프라인/온라인, 클라이언트측/브라우저측 또는 서드파티 시스템 중단 시 폴오버 처리와 같은 특별한 고려 사항과 함께 서드파티 시스템을 통합하는 방법
