...
프로젝트 규모에 따라 여러 작성자 인스턴스, 여러 게시 인스턴스 또는 둘 다로 구성되는 경우가 많습니다. 하위 수준에서 저장소가 여러 인스턴스로 클러스터링될 수도 있습니다.
...
게시 환경은 DMZ(Demilitarized Zone)에 있습니다. 이 환경은 방문자가 귀하의 콘텐츠(예: 웹 사이트 또는 모바일 애플리케이션 형식을 통해)에 액세스하여 상호 작용하는 환경으로, 인트라넷 내에서 공개적입니다. 게시 환경:
...
다른 콘텐츠 저장소 간에 패키지를 내보내고 가져와서 코드(예: 사용자 지정된 웹 애플리케이션 기능 및 디자인 템플릿)를 전송합니다. 이 복제를 자동 프로세스로 구성할 수 있습니다.
...
>[!NOTE]
>
>이는 스테이징 컨텐츠를 프로덕션과 계속 동기화해야 한다는 의미가 아닙니다. 정기적인 업데이트로도 충분하지만, 특히 새로운 코드 반복을 테스트하기 전에 업데이트해야 합니다. QA 및 개발 환경에 있는 컨텐츠는 자주 업데이트하지 않아도 되며, 이는 프로덕션 컨텐츠의 좋은 표현입니다.
