...
승인 테스트를 계획하고 설계할수록 최종 배포는 더 쉬워집니다. 고객 및 품질 보증 팀과 함께 정의해야 합니다.
...
다음 [Adobe 보안 검사 목록](/help/sites-administering/security-checklist.md) 는 설치 시 AEM의 보안을 유지하기 위해 제공되는 공식 체크리스트입니다. 여기에는 인스턴스의 무결성을 보장하기 위해 수행해야 하는 보안 조치와 확인 단계가 포함되어 있습니다.
...
적절한 담당자를 등록하여 관련 정보를 취하도록 합니다. [인증 시험](https://training.adobe.com/certification/exams.html#p=1&amp;solution=adobeExperienceManager).
...
주요 성과 지표(KPI)는 조직이 조직의 목표 및 목표에 대한 진행률을 정의하고 측정하는 데 도움이 됩니다. 조직이 임무를 분석하고 목표를 정의하면 해당 목표에 대한 진전을 측정해야 합니다. KPI는 측정을 위한 메커니즘을 제공합니다.
...
검토 위원회는 아키텍처와 관련된 모든 주요 이해 당사자를 대표해야 합니다. 일반적으로 이 조직은 전체 아키텍처를 검토하고 유지 관리하는 경영진 그룹으로 구성됩니다.
...
이 전략은 품질 보증(QA) 팀이 계획한 접근 방식과 함께 재사용 가능한 자동화된 스크립트에 대한 프레임워크를 정의합니다. 자동화 테스트에 대한 전반적인 계획을 간략하게 소개하여 다음과 같은 이점을 제공합니다.
...
배포를 자동화하면 배포 속도가 빨라지고 일관성이 유지됩니다. 자동화 전략은 다음과 같은 자동화 메커니즘의 구성을 간략하게 설명합니다.
...
새 애플리케이션에서 사용할 템플릿 및 구성 요소의 개요입니다. 상속 규칙, 권한 및 관계 등 세부 사항이 포함됩니다.
...
제안된 콘텐츠 아키텍처 문서입니다. 세부 사항에는 다음이 포함되어야 합니다(기타 포함).
...
보고와 관련하여 고객이 보유한 모든 정책 및/또는 요구 사항. 여기에는 다음이 포함될 수 있습니다.
...
사양 형식, 배송 및 승인과 관련하여 고객이 보유한 모든 지침.
...
프로젝트의 배포 및 릴리스를 모두 다루는 공식화된 정책. 여기에는 다음이 포함될 수 있습니다.
...
방법론을 정의할 때는 응용 프로그램을 개발하거나 유지하기 위해 프로젝트 팀이 만들고 완료한 특정 결과물과 아티팩트를 미리 정의해야 합니다.
...
* 중대한 장애 발생 시에도 운영을 유지해야 하는 비즈니스 크리티컬 기능
* 대체 시 필요한 프로세스
...
행복 경로는 예외 또는 오류 조건이 없는 기본 시나리오입니다. 모든 것이 예상대로 진행될 때 실행되는 활동 시퀀스로 구성됩니다.
...
이러한 함수에 대한 기본적인 내용은 일반적으로 알려져 있으므로 이 문서는 예상치가 아니어야 합니다.

### 높은 수준의 솔루션 설계 {#high-level-solution-design}

높은 수준의 솔루션 설계는 솔루션 개발에 사용될 아키텍처를 설명합니다. 아키텍처 다이어그램은 전체 시스템의 개요를 제공하며, 제품에 맞게 개발될 주요 구성 요소와 해당 인터페이스를 식별합니다.

### 높은 수준 시스템 맵 {#high-level-system-map}

이 시스템 맵은 시스템의 매우 높은 수준 다이어그램을 제공해야 합니다. 관련된 모든 시스템의 일반 맵이라는 점에서 솔루션 컨텍스트와 다르며, 이 다이어그램에는 인터페이스가 없습니다.
...
이 개념은 솔루션에 사용되는 프레임워크, 라이브러리 및 기타 아티팩트의 윤곽을 나타낼 수도 있습니다.
...
프로젝트 상태의 투명성을 도모하기 위해서는 모든 프로젝트 이해 관계자가 접근할 수 있어야 한다.
...
결과 권장 사항과 함께 솔루션을 모니터링하기 위해 기록해야 하는 사항을 정의하는 분석입니다.
...
구현이 성능 KPI를 충족하는지 확인하는 방법과 해당 KPI를 계속 충족하도록 솔루션의 규모를 조정하는 방법에 대한 개념적 문서입니다.
...
프로덕션 환경으로 Go-Live하기 전에 프로덕션 사인오프(PSO)를 부여해야 합니다. 이는 알려진 문제와 함께 프로덕션에 포함될 릴리스를 검토한 결과입니다. 로그오프는 Go Live 예약의 일부로 제공됩니다.
...
다음 [초기 예상 값](#project-efforts-initial-estimates) 는 높은 수준이었으며 구현에 대한 높은 수준의 요구 사항에 따라 만들어졌습니다.

이제 이를 검토, 개선하고 확장하여 최종 추정치를 제공합니다. 추정치는 프로젝트 관리, 컨설팅, 아키텍처, 테스트 및 개발 등 각 프로젝트 책임자가 제공해야 합니다.
...
계약 이정표, 송장 기간 또는 직원 요구 사항과 같이 프로젝트 구현과 관련된 특정 계약 조건을 강조해야 합니다.
...
위험 평가는 고객의 IT 및/또는 보안 부서에서 수행합니다.

프로젝트의 기술적 및 비즈니스 위험을 평가합니다. 보안 정책 준수를 보장하기 위한 진단 서비스가 필요합니다.
...
높은 수준의 개요를 포함하여 새 애플리케이션에 필요한 역할 및 액세스 권한과 관련된 개념에 대한 자세한 사양:
...
솔루션 준수를 보장하는 데 필요한 추가 정책과 보안 개념을 기반으로 특정 항목 체크리스트를 제안합니다.
...
서비스, 서블릿, 프레임워크 및 기타 구현 결정을 포함하여 소프트웨어 아키텍처에 대한 높은 수준의 전략

### 솔루션 검토 위원회 설치 및 회의 케이던스 세트 {#solution-review-board-established-and-meeting-cadence-set}

솔루션 검토 위원회는 일반적으로 고객 이해 당사자로 구성됩니다.
...
승인 및 수락 프로세스에서는 솔루션이 생산적인 환경에 출시되기 전에 이행해야 하는 기준을 간략하게 설명합니다.
...
고객, 프로젝트 스폰서 및 프로젝트 관리자 또는 컨설턴트는 다음을 지정해야 합니다.

* 프로젝트의 성공적인 결과를 정의하는 것.
...
시스템 아키텍처가 보안 정책을 준수하도록 하는 방법에 대한 높은 수준의 개요입니다. 이를 통해 다음을 수행할 수 있습니다.
...
필요한 템플릿에 대한 사양. 여기에는 parsys, 블루프린트 및 상속 매핑 등 세부 사항이 포함되어야 합니다.
...
테스트 개념은 QA, UAT, 성능, 보안 및 통합 테스트를 포함한 프로젝트 테스트에 대한 매우 높은 수준의 개요입니다.
...
통합 구현을 위해 적절한 설명서와 함께 모든 타사 시스템을 사용할 수 있는지 확인합니다.
...
예산안은 모든 관련자들에 의해 검토되고 타당성이 입증되어야 한다. 송장발행, 금액, 예산 보고 방법/시기 등 세부 사항을 확인해야 한다.

### 화이트 박스 테스트 결과 {#white-box-test-results}

화이트박스 테스트는 애플리케이션 기능과 달리 애플리케이션의 내부 구조나 작업을 테스트하는 방법입니다. 소프트웨어 테스트 프로세스의 단위, 통합 및 시스템 수준에서 화이트박스 테스트를 적용할 수 있습니다.
