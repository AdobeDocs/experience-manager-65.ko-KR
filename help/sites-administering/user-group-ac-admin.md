...
**사용자 계정** CRX는 사용자 계정에 보관된 세부 정보에 따라 사용자(개인 또는 다른 애플리케이션)를 식별하고 확인하여 액세스를 인증합니다.
...
* 사용자 계정에 대한 액세스 권한을 지정할 수 있습니다.
   ...
   그러나 관리를 단순화하기 위해 (대부분의 경우) 그룹 계정에 액세스 권한을 할당하는 것이 좋습니다. 각 개별 사용자에 대한 액세스 권한을 할당하면 관리가 매우 어려워집니다(인스턴스가 한 두 개만 있는 경우 특정 시스템 사용자는 예외임).
...
CRX를 사용하면 사용자 및 그룹 계정 모두에 대한 액세스 권한을 구성할 수 있습니다. 그런 다음 동일한 기본 평가 원칙이 양쪽 모두에 적용됩니다.

## 액세스 권한 평가 방법 {#how-access-rights-are-evaluated}

>[!NOTE]
>
>CRX 구현 [jsr-283으로 정의된 액세스 제어](https://www.adobe.io/experience-manager/reference-materials/spec/jcr/2.0/16_Access_Control_Management.html).
>
...
* 지정된 주체에 대해 지정된 노드에 대해 (최대) 1개의 거부 및 1개의 항목 허용이 있습니다. 구현은 항상 중복 항목을 지우고 동일한 권한이 허용 및 거부 항목 모두에 나열되지 않도록 합니다.
...
<table>
 <tbody>
  ...
  <tr>
   <td><i>단순하게 유지</i></td>
   <td><p>새로운 설치를 구성할 때 시간과 생각을 투자하면 보상이 잘 될 것입니다.</p> <p>명확한 구조를 적용하면 지속적인 유지 관리 작업이 간소화되어 현재 동료 및/또는 향후 후임자가 구현 중인 내용을 쉽게 이해할 수 있습니다.</p> </td>
  </tr>
  <tr>
   <td><i>테스트</i></td>
...
즉, 사용자 계정은 해당 계정으로 작동할 수 있는 다른 계정(사용자 또는 그룹)을 지정할 수 있습니다. 즉, user-B가 user-A를 가장하도록 허용되면 user-B는 user-A의 전체 계정 세부 정보(ID, 이름 및 액세스 권한 포함)를 사용하여 작업을 수행할 수 있습니다.

이를 통해 가장 계정은 가장 중인 계정을 사용하는 것처럼 작업을 완료할 수 있습니다(예: 부재 중 또는 과도한 로드 단기간 공유).

계정이 다른 계정을 가장하는 경우 확인하기가 매우 어렵습니다. 로그 파일에는 이벤트가 가장되었다는 사실에 대한 정보가 없습니다. 따라서 user-B가 user-A를 가장하는 경우 모든 이벤트는 user-A가 개인적으로 수행한 것처럼 표시됩니다.
...
1. 트리 구조를 탐색합니다.
1. 편집을 위해 열어야 하는 필수 계정을 클릭합니다.
1. 변경한 다음 해당 항목에 대한 저장(녹색 확인 기호)을 클릭합니다.
...
암호를 제외한 속성은 편집할 수 없으며 삭제하여 다시 만들어야 합니다.
...
포함 **액세스 제어** CRXDE Lite 탭 액세스 제어 정책을 정의하고 관련 권한을 할당할 수 있습니다.

예를 들어 **현재 경로** 왼쪽 창에서 필요한 리소스를 선택하고 오른쪽 아래 창의 액세스 제어 탭을 선택합니다.
...
* **사용자**
   ...
   이렇게 하면 다음 작업을 수행할 수 있습니다. **검색** 용 **사용자** 또는 **그룹**. 결과 목록에서 필요한 주도자를 선택한 다음 **확인** 을 클릭하여 값을 이전 대화 상자로 다시 이동합니다.

![crx_accesscontrol_selectprincipal](assets/crx_accesscontrol_selectprincipal.png)

>[!NOTE]
>
>관리를 단순화하려면 개별 사용자 계정이 아닌 그룹 계정에 액세스 권한을 할당하는 것이 좋습니다.
>
...
1. 사용 **권한 등록** 아이콘(**+**) 대화 상자를 열고 새 권한을 정의합니다.
...
1. 액세스 제어 정책을 정의하고 **확인** 저장. 새 정책:
   ...
   * 다음에 나열될 수 있음 **로컬 액세스 제어 정책**
   * 변경 사항은 다음에 반영됩니다. **유효 액세스 제어 정책**.

CRX는 선택 사항의 유효성을 검사합니다. 지정된 사용자에 대해 지정된 노드에 거부 1개 및 항목 허용 1개가 있습니다. 구현은 항상 중복 항목을 지우고 동일한 권한이 허용 및 거부 항목 모두에 나열되지 않도록 합니다.
...
1. 의 표에서 **로컬 액세스 제어 정책** 항목 오른쪽에 있는 빨간색 아이콘(-)을 클릭합니다.
