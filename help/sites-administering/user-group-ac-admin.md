...
그러면 계정이 속하는 모든 그룹이 표시됩니다. 상속됨 열은 다른 그룹의 구성원 자격 결과로 상속된 구성원 자격을 나타냅니다.
...
현재 그룹 계정이 속하는 모든 그룹이 표시됩니다. 상속됨 열은 다른 그룹의 구성원 자격 결과로 상속된 구성원 자격을 나타냅니다.
...
* **유효 액세스 제어 정책**
   ...
   이러한 정책은 모든 액세스 요청에 대해 현재 유효한 액세스 제어 정책입니다. 여기에는 로컬 정책과 상위에서 상속된 정책 모두에서 파생된 집계 정책이 표시됩니다.
...
* **보관소**
   ...
   저장소 수준 액세스 제어를 선택합니다. 예를 들어, 저장소를 위해서만 관련된 권한을 설정할 때 노드가 아니라 해당 `jcr:namespaceManagement` 권한을 설정합니다.
