...
>[!NOTE]
>
>이 설명서는 구독 기반 솔루션인 Adobe Campaign Standard과 AEM을 통합하는 방법을 설명합니다. Adobe Campaign 6.1을 사용 중인 경우 이러한 지침은 [Adobe Campaign 6.1](/help/sites-administering/campaignonpremise.md)과 통합을 참조하십시오.
...
AEM에서 Adobe Campaign을 사용하여 작업하는 경우 Adobe Campaign을 통해 이메일 및 양식을 전송하는 기능이 포함되며 [Adobe Campaign 작업](/help/sites-authoring/campaign.md)에 설명되어 있습니다.

또한 AEM과 [Adobe Campaign](https://docs.campaign.adobe.com/doc/standard/en/home.html)을(를) 통합하면 다음 항목에 관심이 있을 수 있습니다.
...
* 사용자가 배달을 만들고 편집할 수 있는 **deliveryPrepare**&#x200B;과 같은 특정 역할을 이 사용자에게 할당할 수 있습니다.
...
1. **Type** 필드에서 **Adobe Experience Manager** i를 선택하고 AEM 작성 인스턴스에 사용되는 액세스 매개 변수를 입력합니다.서버 주소, 계정 이름 및 암호

   >[!NOTE]
   >
   >URL 끝에 종료 **/** 슬래시를 추가하지 않으면 연결이 작동하지 않습니다.

1. **Enabled** 확인란을 선택한 다음 **Save**&#x200B;를 클릭하여 수정 내용을 저장합니다.
...
### AEM별 이메일 배달 템플릿 {#creating-an-aem-specific-email-delivery-template} 만들기
...
1. **작업** 표시줄의 확인 표시를 클릭하고 기존  **표준 이메일(메일)** 기본 템플릿을 선택한 다음 카피아이콘 **** 을 클릭하고 확인을 클릭하여  **선택을**&#x200B;활성화합니다.
1. **x**&#x200B;을 클릭하여 선택 모드를 비활성화하고 새로 만든 **표준 이메일(메일)** 템플릿을 연 다음 템플릿 대시보드의 작업 표시줄에서 **속성 편집**&#x200B;을 선택합니다.

   템플릿의 **Label**&#x200B;을 수정할 수 있습니다.

1. 속성 **콘텐츠** 섹션에서 **콘텐츠 소스**&#x200B;을 **Adobe Experience Manager**&#x200B;으로 변경합니다. 그런 다음 이전에 만든 외부 계정을 선택하고 **확인**&#x200B;을 클릭합니다.
...
>[!NOTE]
>
>복제 URL을 사용하지 않고 대신 공개 URL을 사용하는 경우 OSGi(**도구** > **웹 콘솔** > **OSGi 구성 AEM > 캠페인 통합 - 구성**):****
...
1. 제작 인스턴스에서 **AEM 로고** **&lt;a2/>** > **배포** > **복제** > **작성자**&#x200B;의 에이전트를 선택한 다음 **기본 에이전트**&#x200B;를 클릭합니다.
...
일반 지침은 [externalizer](/help/sites-developing/externalizer.md) 구성을 참조하십시오. Adobe Campaign 통합의 경우 `https://<host>:<port>/system/console/configMgr/com.day.cq.commons.impl.ExternalizerImpl`에서 `localhost:4503`를 가리키지 않고 Adobe Campaign 콘솔에서 연결할 수 있는 서버에 게시 서버를 구성해야 합니다.
