...
이러한 기능을 구성하고 사용자 정의하려면 [구성 요소에 대한 RTE 플러그인](#configure-the-plugin-features)을 구성합니다. 예를 들어 `paraformat` 플러그인을 사용하면 기본 `H1`, `H2` 및 기본적으로 제공되는 `H3` 이외의 지원되는 머리글 수준 수를 확장하는 등 추가적인 블록 수준 의미 요소를 추가할 수 있습니다.

RTE는 터치 지원 사용자 인터페이스 및 클래식 사용자 인터페이스에 대한 다양한 구성 요소에서 사용할 수 있습니다. 그러나 RTE를 사용하는 기본 구성 요소는 두 인터페이스 모두에서 사용할 수 있는 **Text** 구성 요소입니다. 다음 이미지는 `paraformat`을(를) 포함하여 플러그인 범위가 활성화된 RTE를 보여줍니다.
...
다양한 인터페이스에서 사용할 수 있는 RTE 기능 간의 차이점은 [Plugins 및 해당 기능](/help/sites-administering/rich-text-editor.md#aboutplugins)을 참조하십시오.
...
1. RTE에 따라 [구성 위치](/help/sites-administering/rich-text-editor.md#understand-the-configuration-paths-and-locations)를 확인하고 탐색합니다.
...
AEM의 액세스 가능성 기능을 추가로 확장하려면 추가 요소 및 특성을 사용하여 RTE를 기반으로 기존 구성 요소(예: **Text** 및 **표** 구성 요소)를 확장할 수 있습니다.

다음 절차에서는 보조 기술 사용자에게 데이터 테이블에 대한 정보를 제공하는 **Caption** 요소를 사용하여 **표** 구성 요소를 확장하는 방법을 보여 줍니다.

### 예 - 표 속성 대화 상자 {#example-adding-the-caption-to-the-table-properties-dialog}에 캡션을 추가합니다.

`TablePropertiesDialog` 생성자에서 캡션을 편집하는 데 사용되는 추가 텍스트 입력 필드를 추가합니다. `itemId`은(는) 해당 컨텐츠를 자동으로 처리하도록 `caption`(즉, DOM 속성 이름)로 설정해야 합니다.

**표**&#x200B;에서 DOM 요소의 속성을 명시적으로 설정하거나 제거합니다. 이 값은 `config` 개체의 대화 상자로 전달됩니다. DOM 속성은 해당 `CQ.form.rte.Common` 메서드(`com`은 `CQ.form.rte.Common`)를 사용하여 설정/제거해야 브라우저 구현에서 발생하는 일반적인 실수를 방지할 수 있습니다.
