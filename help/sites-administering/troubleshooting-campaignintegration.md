...
* AEM 기능의 경우 AEM 작성자 인터페이스에서 json 호출이 요청된다는 것을 알 수 있습니다.이로 인해 HTTP-500 오류가 발생하지 않습니다. HTTP-500 오류가 표시되면 `error.log`에서 자세한 내용을 확인하십시오.
...
## 설정 로케일 {#if-you-get-a-warning-about-your-setlocale}에 대한 경고가 표시되는 경우

Apache HTTPD 서비스를 시작하고 오류 `"Warning: setlocale: LC_CTYPE cannot change locale"`이(가) 표시되면 시스템에 **en_CA.ISO-8859-15 locale**&#x200B;이(가) 설치되어 있는지 확인하십시오.

`local -a`을 사용하여 설치되었는지 확인할 수 있습니다. 설치되어 있지 않으면 **/usr/local/neolane/nl6/env.sh** 스크립트를 패치하고 로케일을 설치된 스크립트로 변경할 수 있습니다.
...
## 클라우드 서비스 {#if-a-connection-to-ssl-displays-an-error-when-setting-up-the-cloud-service}를 설정할 때 SSL에 대한 연결에 오류가 표시되는 경우
...
* OSGi 구성([https://&lt;host>:&lt;port>/system/console/configMgr](http://localhost:4502/system/console/configMgr))의 *Apache Felix Http Service SSL 필터*&#x200B;를 각 헤더 설정으로 구성해야 합니다. [https://felix.apache.org/documentation/subprojects/apache-felix-http-service.html#using-the-ssl-filter](https://felix.apache.org/documentation/subprojects/apache-felix-http-service.html#using-the-ssl-filter)을(를) 참조하십시오.
...
사용자 지정 템플릿을 사용하는 경우 로그에 &quot;com.day.cq.mcm.campaign.servlets.util.ParameterMapper&quot; 오류가 표시됩니다. 이 경우 [패키지 공유](/help/sites-administering/package-manager.md#package-share)에서 Featurepack 6576을 설치해야 합니다. 이 문제는 acMapping 속성이 recipient.firstName 이외의 값으로 설정된 경우 Adobe Campaign 관리자 측에 빈 값이 만들어집니다.
