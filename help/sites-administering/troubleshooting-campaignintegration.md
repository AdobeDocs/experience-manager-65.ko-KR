...
AEM for Adobe Campaign Classic에서 메일 템플릿을 만들 때 템플릿의 `jcr:content` 노드에 값이 `mapRecipient`인 속성 `acMapping`을(를) 포함해야 합니다. 그렇지 않으면 AEM의 **페이지 속성**&#x200B;에서 Adobe Campaign Classic 템플릿을 선택할 수 없습니다. 필드가 비활성화되어 나타납니다.
