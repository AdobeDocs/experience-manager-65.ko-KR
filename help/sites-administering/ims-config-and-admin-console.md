...
description: 사용 방법 알아보기 [!DNL Admin Console] Adobe Experience Manager.
...
에서 사용자 및 그룹을 수동으로 만들 수 있음 [!DNL Admin Console] UI. 관리할 사용자가 많지 않은 경우 이 방법을 사용할 수 있습니다. 예를 들어 AEM 사용자가 50명 미만입니다.
...
사용자 동기화가 실행되면 조직의 Active Directory(또는 기타 호환되는 데이터 소스)에서 사용자 목록을 가져와서 [!DNL Admin Console]. 그런 다음 Adobe을 호출합니다 [!DNL User Management] API를 통해 [!DNL Admin Console] 조직의 디렉터리와 동기화됩니다. 변경 흐름은 완전히 한 가지 방식입니다. [!DNL Admin Console] 디렉터리로 푸시하지 마십시오.
...
>
>Adobe Developer 콘솔 설명서를 찾을 수 있습니다 [여기](https://developer.adobe.com/developer-console/docs/guides/).
>
>
...
다른 사용자의 경우 인스턴스에 IMS가 구성되어 있으면 IMS 기반 로그인을 사용할 수 있습니다. 사용자가 먼저 클릭합니다 **Adobe으로 로그인** 아래와 같이:
...
여기에서는 사용자가 의 몇 가지 그룹에도 할당되었습니다. [!DNL Admin Console]. (사용자 동기화 도구를 사용하여 LDAP에서 사용자 및 그룹을 동기화하거나 로컬에서 만들 수 있습니다. 다음을 참조하십시오 **에 대한 사용자 온보딩[!DNL Admin Console]** 이전).
