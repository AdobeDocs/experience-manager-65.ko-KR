...
에 대한 AEM 온보딩 [!DNL Admin Console] 은 AEM Managed Services 고객이 하나의 콘솔에서 모든 Experience Cloud 사용자를 관리할 수 있도록 허용합니다. 사용자 및 그룹을 AEM 인스턴스와 연결된 제품 프로필에 할당하여 특정 인스턴스에 로그인할 수 있도록 할 수 있습니다.
...
* 고객의 자체 SAML 2 호환 ID 공급자를 사용하는 통합 인증이 지원됩니다
...
사용자 동기화가 실행되면 이 기능은 조직의 Active Directory(또는 기타 호환되는 데이터 소스)의 사용자 목록을 가져와서 [!DNL Admin Console]. 그런 다음 Adobe을 호출합니다 [!DNL User Management] API를 통해 [!DNL Admin Console] 조직의 디렉터리와 동기화됩니다. 변경 흐름은 완전히 단방향 입니다. [!DNL Admin Console] 디렉터리로 푸시하지 마십시오.

시스템 관리자는 이 도구를 사용하여 고객 디렉터리에 있는 사용자 그룹을 의 제품 구성 및 사용자 그룹과 매핑할 수 있습니다. [!DNL Admin Console], 새 UST 버전에서는에서 사용자 그룹을 동적으로 만들 수도 있습니다. [!DNL Admin Console].
