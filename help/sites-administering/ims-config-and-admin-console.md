...
description: 사용 방법 알아보기 [!DNL Admin Console] AEM.
...
AEM 6.4.3.0은 [!DNL Admin Console] 에 대한 AEM 인스턴스 및 Adobe IMS(Identity Management 시스템) 기반 인증 지원 **AEM Managed Services** 고객.

에 대한 AEM 온보딩 [!DNL Admin Console] 은 AEM Managed Services 고객이 하나의 콘솔에서 모든 Experience Cloud 사용자를 관리할 수 있도록 허용합니다. AEM 인스턴스와 연결된 제품 프로필에 사용자를 할당하여 특정 인스턴스에 로그인할 수 있도록 할 수 있습니다.

## 주요 내용 {#key-highlights}

* AEM IMS 인증 지원은 AEM 작성자, 관리자 또는 개발자만을 위한 것이며 사이트 방문자와 같은 고객 사이트의 외부 최종 사용자를 위한 것이 아닙니다
* 다음 [!DNL Admin Console] 은 AEM Managed Services 고객을 IMS 조직으로, 해당 인스턴스를 제품 컨텍스트로 나타냅니다. 고객 시스템 및 제품 관리자는 인스턴스에 대한 액세스를 관리할 수 있습니다.
* AEM Managed Services은 고객 토폴로지를 [!DNL Admin Console]. 의 인스턴스당 AEM Managed Services 제품 컨텍스트의 인스턴스가 한 개 있습니다. [!DNL Admin Console].
* 의 제품 프로필 [!DNL Admin Console] 는 사용자가 액세스할 수 있는 인스턴스를 결정합니다.
...
* [!DNL User Management] (Adobe [!DNL Admin Console])은 고객 관리자가 계속 소유합니다.
...
### 조직 온보딩 대상 [!DNL Admin Console] {#onboarding-organizations-to-admin-console}

(으)로 온보딩 중인 고객 [!DNL Admin Console] 는 AEM 인증에 Adobe IMS를 사용하기 위한 전제 조건입니다.

첫 번째 단계로, 고객은 Adobe IMS에서 조직이 프로비저닝되어 있어야 합니다. Adobe 엔터프라이즈 고객은 의 IMS 조직으로 표시됩니다. [Adobe [!DNL Admin Console]](https://helpx.adobe.com/kr/enterprise/using/admin-console.html).

AEM Managed Services 고객은 이미 조직이 프로비저닝되어 있어야 하며 IMS 프로비저닝의 일부로 고객 인스턴스가에서 사용할 수 있게 됩니다. [!DNL Admin Console] 사용자 자격 및 액세스 관리.
...
1. 지정된 시스템 관리자는에 로그인하라는 초대를 받습니다. [!DNL Admin Console]
1. 시스템 관리자는 도메인을 클레임하여 도메인의 소유권을 확인합니다(이 예에서는 acme.com).
1. 시스템 관리자가 사용자 디렉터리를 설정합니다
1. 시스템 관리자는에서 IDP(ID 공급자)를 구성합니다 [!DNL Admin Console] SSO 설정용
1. AEM 관리자는 평소대로 로컬 그룹, 권한 및 권한을 관리합니다. 사용자 및 그룹 동기화 보기

>[!NOTE]
>
>IDP 구성을 포함한 Identity Management 기본 사항 Adobe에 대한 자세한 내용은 문서 를 참조하십시오 [이 페이지입니다.](https://helpx.adobe.com/kr/enterprise/using/set-up-identity.html)
>
>엔터프라이즈 관리 및 [!DNL Admin Console] 문서 보기 [이 페이지](https://helpx.adobe.com/kr/enterprise/managing/user-guide.html).

### 에 대한 사용자 온보딩 [!DNL Admin Console] {#onboarding-users-to-the-admin-console}

고객의 규모와 선호도에 따라 사용자를 온보딩하는 방법에는 세 가지가 있습니다.

1. 에서 사용자 및 그룹 수동으로 만들기 [!DNL Admin Console]
...
에서 사용자 및 그룹을 수동으로 만들 수 있음 [!DNL Admin Console] UI. 이 방법은 관리할 사용자가 많지 않은 경우 사용할 수 있습니다. 예를 들어 AEM 사용자 수가 50명 미만입니다.

고객이 이미 Analytics, Target 또는 Creative Cloud 애플리케이션과 같은 다른 Adobe 제품을 관리하기 위해 이 방법을 사용하고 있는 경우에도 사용자를 수동으로 만들 수 있습니다.
...
UST(User Sync Tool)를 사용하면 기업 고객은 Active Directory 또는 기타 테스트된 OpenLDAP 디렉터리 서비스를 사용하여 Adobe 사용자를 생성하거나 관리할 수 있습니다. 대상 사용자는 도구를 설치 및 구성할 수 있는 IT ID 관리자(Enterprise Directory 및 시스템 관리자)입니다. 오픈 소스 도구는 고객이 개발자에게 특정 요구 사항에 맞게 수정할 수 있도록 사용자 지정할 수 있습니다.

사용자 동기화가 실행되면 조직의 Active Directory(또는 기타 호환되는 데이터 소스)에서 사용자 목록을 가져와서 [!DNL Admin Console]. 그런 다음 Adobe을 호출합니다 [!DNL User Management] API를 통해 [!DNL Admin Console] 조직의 디렉터리와 동기화됩니다. 변경 흐름은 완전히 단방향 입니다. [!DNL Admin Console] 디렉터리로 푸시하지 마십시오.

시스템 관리자는 이 도구를 사용하여 고객 디렉토리에 있는 사용자 그룹을 의 제품 구성 및 사용자 그룹과 매핑할 수 있습니다. [!DNL Admin Console], 새 UST 버전에서는에서 사용자 그룹을 동적으로 만들 수도 있습니다. [!DNL Admin Console].

사용자 동기화를 설정하려면 조직에서는 를 사용하는 것과 같은 방식으로 자격 증명 세트를 만들어야 합니다 [[!DNL User Management] API](https://www.adobe.io/apis/cloudplatform/usermanagement/docs/setup.html).
...
>
>Adobe I/O 콘솔 설명서를 찾을 수 있습니다 [여기](https://developer.adobe.com/developer-console/docs/guides/).
>
>
>다음 [!DNL User Management] 사용자 동기화 도구에서 사용하는 API는 여기에서 다룹니다 [위치](https://adobe-apiplatform.github.io/umapi-documentation/en/).
...
고객 제품 관리자가 로그인하는 경우 [!DNL Admin Console]에서는 아래와 같이 AEM Managed Services 제품 컨텍스트의 여러 인스턴스를 볼 수 있습니다.

![screen_shot_2018-09-17at105804pm](assets/screen_shot_2018-09-17at105804pm.png)

이 예에서는 조직입니다 *AEM-MS-Onboard* 에는 스테이지, 프로덕션 등과 같은 다양한 토폴로지 및 환경에 걸쳐 32개의 인스턴스가 있습니다.
...
다른 사용자의 경우 인스턴스에 IMS가 구성되어 있으면 IMS 기반 로그인을 사용할 수 있습니다. 사용자가 먼저 **Adobe으로 로그인** 아래 표시된 단추:
...
초기 동안 Federated IDP가 구성된 경우 [!DNL Admin Console] 설정하면 사용자가 SSO용 고객 IDP로 리디렉션됩니다.
...
여기에서는 사용자가 의 몇 가지 그룹에도 할당되었습니다. [!DNL Admin Console]. ( 사용자 동기화 도구를 사용하여 LDAP에서 사용자 및 그룹을 동기화하거나 로컬에서 만들 수 있습니다. 섹션을 참조하십시오 **에 대한 사용자 온보딩[!DNL Admin Console]** 위 참조).
...
아래에 표시된 대로 그룹입니다 *AEM-GRP_008* 는 DAM 사용자의 권한 및 권한을 상속합니다. 이는 동기화된 그룹에 대한 권한을 관리하는 효과적인 방법이며 LDAP 기반 인증 방법에서도 일반적으로 사용됩니다.
