...
>[!NOTE]
>
>6.4로 업그레이드한 후에는 /etc에서 파일을 이동하는 것이 좋습니다. 자세한 내용은 [AEM 6.5의 일반적인 저장소 재구성](/help/sites-deploying/all-repository-restructuring-in-aem-6-5.md#translation-rules)을 참조하십시오.
...
* 하위 `property` 요소는 모든 리소스 유형에 대해 번역할 노드 속성을 식별합니다.
   ...
   * 옵션인 `translate` 속성은 번역되어 있지 않은 경우 `false`와 같습니다. 기본적으로 값은 `true`입니다. 이 속성은 이전 규칙을 오버라이드할 때 유용합니다.
...
다음과 같은 예시 규칙을 사용하면 모든 `text` 속성의 콘텐츠가 `/content` 노드 아래의 모든 페이지에 대해 번역됩니다. 이 규칙은 기초 텍스트 구성 요소 및 기초 이미지 구성 요소와 같이 `text` 속성에 콘텐츠를 저장하는 모든 구성 요소에 적용됩니다.
...
다음 예제에서는 모든 `text` 속성의 콘텐츠와 기본 이미지 구성 요소의 다른 속성을 모두 변환합니다. 다른 구성 요소에 이름이 같은 속성이 있는 경우 규칙이 적용되지 않습니다.
...
* 구성 요소로 확인되는 경로와 동일한 `resourceType` 특성.
* 에셋 바이너리(임베드된 에셋의 경우) 또는 참조된 에셋으로의 경로를 저장하는 속성의 이름과 동일한 `assetReferenceAttribute` 특성.
...
여기에서 **컨텍스트를 추가**&#x200B;할 수 있습니다. 경로를 추가할 수 있습니다.
...
UI를 통해 변경할 수 있는 특성은 4개입니다. `isDeep`, `inherit`, `translate` 및 `updateDestinationLanguage`.

**isDeep** 이 특성은 노드 필터에 적용할 수 있으며 기본적으로 true입니다. 노드(또는 상위 항목)에 필터에 지정된 속성 값이 있는 속성이 포함되어 있는지 확인합니다. false인 경우 현재 노드에서만 확인합니다.

예를 들어 상위 노드의 속성 `draftOnly`가 true로 설정되어 있는 경우에도 번역 작업에 하위 노드가 추가되어 초안 콘텐츠에 플래그를 지정됩니다. `isDeep`은(는) 상위 노드에 true인 속성 `draftOnly`이(가) 있는지 확인하고 이러한 하위 노드를 제외합니다.

편집기에서 **필터** 탭에서 **Is Deep**&#x200B;을(를) 선택/선택 취소할 수 있습니다.

![chlimage_1-59](assets/chlimage_1-59.jpeg)

다음은 UI에서 **Is Deep**&#x200B;을(를) 선택 취소한 경우의 결과 xml의 예입니다.
...
**inherit** 속성에 적용됩니다. 기본적으로 모든 속성은 상속되지만, 속성이 하위 속성에 상속되지 않도록 하려면 해당 속성을 false로 표시하여 특정 노드에만 적용되도록 할 수 있습니다.

UI에서 **속성** 탭에서 **상속**&#x200B;을 선택/선택 취소할 수 있습니다.

![chlimage_1-60](assets/chlimage_1-60.jpeg)

**translate** translate 특성은 속성을 변환할지 여부를 지정하는 데 사용됩니다.

UI에서 **속성** 탭에서 **번역**&#x200B;을 선택/선택 취소할 수 있습니다.

**updateDestinationLanguage** 이 특성은 `jcr:language`과(와) 같이 텍스트가 아닌 언어 코드가 있는 속성에 사용됩니다. 사용자는 텍스트가 아닌 언어 로케일을 소스에서 대상으로 번역하는 것입니다. 이러한 속성은 번역용으로 전송되지 않습니다.

UI에서 **속성** 탭에서 **번역**&#x200B;을 선택/선택 취소할 수 있지만 언어 코드를 값으로 하는 특정 속성에 대해서는 선택/선택 취소할 수 있습니다.
