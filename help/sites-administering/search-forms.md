...
A [조건자 범위](#predicates-and-their-settings)s는 즉시 사용할 수 있습니다. 속성 술어를 포함한 여러 술어를 추가하여 사용자가 지정한 단일 속성과 일치하는 에셋을 검색할 수 있습니다. 또는 옵션 조건자를 사용하여 특정 속성에 대해 지정한 하나 이상의 값과 일치하는 에셋을 검색할 수 있습니다.
...
* **옵션 경로**
  ...
  사용자는 술어 설정 탭의 경로 브라우저를 사용하여 경로를 선택할 수 있습니다. 선택 후 **+**&#x200B;아이콘을 사용하여 유효한 옵션 목록에 선택 항목을 추가한 다음 **-** 아이콘 제거(필요한 경우).
...
1. 필드를 추가하려면 **조건자 선택** 필요한 설명을 탭하고 필요한 위치로 드래그합니다. 예를 들어 **날짜 범위 조건자**:
...
1. **닫기** 구성을 반환하고 완료할 수 있도록 미리보기를 표시합니다.
...
1. 선택 **완료** 구성을 저장합니다.
1. 적절한 콘솔로 이동합니다(이 예제에서는 **사이트**) 및 를 엽니다 **검색** 레일. 다양한 옵션과 함께 새로 정의된 검색 양식이 표시됩니다. 검색 결과를 볼 수 있도록 필요한 옵션을 선택합니다.
