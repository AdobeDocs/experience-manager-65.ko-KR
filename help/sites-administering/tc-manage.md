...
번역 프로젝트를 사용하면 AEM 컨텐츠 번역을 관리할 수 있습니다. 번역 프로젝트는 다른 언어로 번역될 리소스를 포함하는 AEM [project](/help/sites-authoring/projects.md)의 유형입니다. 이러한 리소스는 언어 마스터에서 만든 [언어 사본](/help/sites-administering/tc-prep.md)의 페이지 및 자산입니다.
...
번역 프로젝트는 언어 및 번역 방법/공급업체에서 정의한 긴 실행 항목으로서, 세계화 조직의 지배 구조에 맞게 정의됩니다. 이러한 지표는 처음 번역하거나 수동으로 한 번 시작되어야 하며 컨텐츠 및 번역 업데이트 활동 전반에서 그대로 유지됩니다.
...
* **언어 복사본에는 페이지가 포함되지 않습니다.** AEM은 이 상황을 초기 번역으로 처리합니다. 페이지가 언어 복사본으로 즉시 복사되고 프로젝트에 포함됩니다. 번역된 페이지를 AEM으로 가져오면 AEM에서 바로 언어 복사본에 복사합니다.
* **언어 복사본에는 페이지가 이미 포함되어 있습니다.** AEM은 이 상황을 업데이트된 번역본으로 처리합니다. 론치가 만들어지고 페이지 사본이 론치에 추가되며 프로젝트에 포함됩니다. 론치를 사용하면 언어 사본에 커밋하기 전에 업데이트된 번역을 검토할 수 있습니다.
...
## 참조 패널 {#creating-translation-projects-using-the-references-panel}을(를) 사용하여 번역 프로젝트 만들기
...
번역한 후 언어 사본을 덮어쓰기 전에 [번역](#reviewing-and-promoting-updated-content)을 검토할 수 있습니다. 선택한 페이지에 대한 언어 복사본이 없으면 만들기 및 번역 탭이 표시되어 프로젝트 관련 명령에 액세스할 수 있습니다.
...
새 프로젝트를 만들 때처럼 페이지를 추가할 때 기존 언어 사본을 덮어쓰지 않도록 하기 위해 필요한 경우 페이지 사본이 론치에 추가됩니다. ([기존 언어 복사본용 번역 프로젝트 만들기](#performing-initial-translations-and-updating-existing-translations)를 참조하십시오.)
...
>[!NOTE]
>
>i18n 사전에 대한 자세한 내용은 [Translator를 사용하여 사전 관리](/help/sites-developing/i18n-translator.md)를 참조하십시오.
...
번역 요약 타일에는 번역 프로젝트에 대해 구성된 속성이 포함됩니다. 일반 [프로젝트 정보](/help/sites-authoring/projects.md#project-info) 외에 번역 탭에는 번역별 속성이 포함되어 있습니다.
...
1. **Copies** 머리글 아래에서 **언어 사본을 선택합니다.**
1. 특정 언어 사본을 선택한 다음 **비교가 기본 **와 비교 또는 **를 이전 **과 비교(해당되는 경우)를 클릭할 수 있습니다.
...
공급업체에서 번역한 파일을 검토한 후 번역 작업을 완료합니다. 인간 번역 워크플로우의 경우 번역을 완료하면 번역된 계약이 이행되었다는 뜻이며 번역을 번역 메모리에 저장하라는 의미입니다.
...
## 언어 복사본 구조 만들기 {#creating-the-structure-of-a-language-copy}

번역하고 있는 마스터 언어의 내용이 포함되도록 언어 사본을 채웁니다. 언어 사본을 채우려면 언어 사본의 언어 루트](/help/sites-administering/tc-prep.md#creating-a-language-root)를 [로 만들어야 합니다.
...
번역 프로젝트를 수동으로 만들 때는 [기본 속성](/help/sites-authoring/touch-ui-managing-projects.md#creating-a-project) 외에 다음과 같은 번역 관련 속성에 대한 값을 제공해야 합니다.

* **이름:** 프로젝트 이름.
* **소스 언어:** 소스 컨텐츠의 언어입니다.
* **Target 언어:** 컨텐츠가 번역되는 언어입니다.
* **번역 방법:** 인간 번역(Human Translation)을 선택하여 번역이 수동으로 수행되도록 지정합니다.
