...
>
>하지만, [대시보드 템플릿](#creating-a-dashboard-template)을 사용하여 공통 구성 및 대시보드 레이아웃을 공유할 수 있습니다.
...
AEM은 [SiteCatalyst](/help/sites-administering/adobeanalytics.md) 데이터의 여러 지표를 렌더링하는 구성 요소 세트와 함께 제공됩니다. 이러한 구성 요소들은 **대시보드** 섹션 아래의 사이드킥에 나열되어 있습니다.
...
**데이터** 를 가져오는 날짜부터 절대 또는 상대 날짜입니다.

**데이터** 를 가져오는 날짜-종료 또는 상대 날짜입니다.
...
대시보드 템플릿은 `/libs/cq/dashboards/templates/` 아래에 저장된다는 점을 제외하고 다른 페이지 템플릿과 같이 만들어집니다. [컨텐트 페이지 템플릿 만들기](/help/sites-developing/website.md#creating-the-contentpage-template) 섹션을 참조하십시오.
...
다음 Javascript 파일은 `geout.reporting.topauthors` [클라이언트 라이브러리](/help/sites-developing/clientlibs.md)에서 구성 요소 자체의 하위로 정의됩니다.

[QueryBuilder](/help/sites-developing/querybuilder-api.md)은 저장소를 쿼리하여 `cq:AuditEvent` 노드를 읽는 데 사용됩니다. 쿼리 결과는 작성한 기고물이 추출되는 JSON 개체입니다.
