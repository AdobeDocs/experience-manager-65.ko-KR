...
성능에 대한 부정적인 영향을 줄이기 위해 Adobe Experience Manager는 장시간 실행되는 특정 리소스 집약적인 작업을 비동기식으로 처리합니다.
...
비동기 처리에는 시스템 리소스의 가용성에 따라 여러 작업을 대기열에 넣고 이를 연속적으로 실행하는 작업이 포함됩니다.
...
AEM이 작업을 비동기적으로 처리할 때마다 받은 편지함과 이메일을 통해 알림을 받게 됩니다(활성화된 경우).
