...
description: AEM 설치 시 관리자 암호를 변경하는 방법에 대해 알아봅니다.
...
명령줄에서 AEM 인스턴스를 실행하는 일반적인 구문은 다음과 같습니다.
...
>[!NOTE]
>
>관리자 암호를 변경하라는 메시지는 새 AEM 인스턴스를 설치하는 동안에만 표시됩니다.
...
>[!NOTE]
>
>를 사용하는 경우 `-nointeractive` 가 없는 매개 변수 `-Dadmin.password.file` 시스템 속성에서 AEM은 사용자에게 변경을 요청하지 않고 기본 관리자 암호를 사용하며, 기본적으로 이전 버전의 동작을 복제합니다. 이 비대화형 모드는 설치 스크립트에서 명령줄을 사용하여 자동 설치에 사용할 수 있습니다.
