...
게시 및 [작성 환경 모두에서 다음 정보가](#propertiesavailableintheclientcontext) 표시됩니다.
...
* **편집**&#x200B;새 페이지가 열리고 프로필 속성을 [편집하거나 추가 또는 제거할 수 있습니다](#editingprofiledetails).
...
JSONP 표준은 동일한 원본 정책을 우회할 수 있도록 하는 JSON을 보완합니다. 따라서 웹 앱이 다른 도메인에 있는 서버와 통신할 수 없습니다. JSON은 JSON 개체를 함수 호출에서 래핑하여 다른 `<script>` 도메인에서 로딩할 수 있습니다(동일한 원본 정책에 대한 허용되는 예외).
...
클라이언트 컨텍스트를 편집하여 특정 속성의 값을 설정(또는 재설정)할 수 있습니다. 이렇게 하면 특정 시나리오를 테스트할 수 있습니다(특히 [세그멘테이션](/help/sites-administering/campaign-segmentation.md) 및 [캠페인에](/help/sites-classic-ui-authoring/classic-personalization-campaigns.md)유용합니다).
