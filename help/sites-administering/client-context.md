...
>[!NOTE]
>
>클라이언트 컨텍스트가 ContextHub로 대체되었습니다. 자세한 내용은 관련 [configuration]ch-configuring.md) 및 [developer](/help/sites-developing/contexthub.md) 설명서를 참조하십시오.
...
[게시 및 작성 환경 모두에서 다음 사항에 대한 정보](#propertiesavailableintheclientcontext)가 표시됩니다.
...
* **로드**
프로필 목록에서  [선택하고 테스트할 ](#loading-a-new-user-profile) 프로필을 로드할 수 있습니다.

* **재설정**
현재 사용자의 프로필 [을 ](#resetting-the-profile-to-the-current-user) 재설정할 수 있습니다.
...
**캠페인** 작성자가 특정 캠페인 경험을 시뮬레이션할 수 있습니다. 이 구성 요소는 일반적인 캠페인 해상도 및 경험 선택을 무시하고 다양한 순열에 대한 테스트를 활성화합니다.

캠페인 해결은 일반적으로 캠페인의 우선 순위 속성을 기반으로 합니다. 일반적으로 경험은 세그먼테이션을 기반으로 선택됩니다.

**장바구니** 제품 항목(제목, 수량, 가격 형식 지정 등), 해결된 판촉 행사(제목, 메시지 등)를 비롯한 장바구니 정보를 보여줍니다. 및 바우처(코드, 설명 등)를 참조하십시오.

또한 장바구니 세션 저장소는 ClientContextCartServlet을 사용하여 해결된 프로모션 변경 사항(세그멘테이션 변경 사항 기반)에 대해 서버에 알립니다.

**일반** 저장소스토어의 컨텐츠를 표시하는 일반 구성 요소입니다. 일반 저장소 속성 구성 요소의 하위 버전입니다.

범용 스토어는 사용자 지정 방식으로 데이터를 표시하는 JS 렌더러를 사용하여 구성해야 합니다.

**범용 저장소** 속성스토어의 내용을 표시하는 일반 구성 요소입니다. 일반 스토어 구성 요소의 상위 버전입니다.

범용 저장소 속성 구성 요소에는 축소판과 함께 구성된 속성을 나열하는 기본 렌더러가 포함되어 있습니다.

**위치** 위치클라이언트의 위도 및 경도를 표시합니다. HTML5 지리적 위치 API를 사용하여 현재 위치에 대한 브라우저를 쿼리합니다. 그러면 방문자에게 팝업이 표시되고, 브라우저가 방문자가 위치를 공유하는 것에 동의하는지 묻는 메시지가 표시됩니다.

Context Cloud에 표시될 때 구성 요소는 Google API를 사용하여 맵을 축소판으로 표시합니다. 이 구성 요소는 Google API [사용 제한](https://developers.google.com/maps/documentation/staticmaps/intro#Limits)의 적용을 받습니다.
...
JSONP 표준은 동일한 원본 정책을 우회하는 것을 허용하는 JSON을 보완하는 기능으로, 웹 앱이 다른 도메인에 있는 서버와 통신할 수 없도록 합니다. JSON 개체를 함수 호출에서 래핑하여 다른 도메인에서 `<script>`(동일한 원본 정책에 허용된 예외)으로 로드할 수 있도록 합니다.

JSONP 스토어는 다른 스토어와 비슷하지만 현재 도메인에 있는 해당 정보에 대한 프록시를 사용할 필요 없이 다른 도메인에서 오는 정보를 로드합니다. [JSONP](/help/sites-administering/client-context.md#storing-data-in-client-context-via-jsonp)를 통해 클라이언트 컨텍스트에 데이터 저장 예제를 참조하십시오.
...
**해결된** 세그먼트현재 해결된 세그먼트를 표시합니다(클라이언트 컨텍스트에 표시된 다른 정보에 따라 다름). 이것은 캠페인을 구성할 때 유용합니다.
...
**테크노그래픽** 저장이 구성 요소는 설치에 따라 다릅니다.

**열람** 제품구매자가 본 제품을 추적합니다. 최근에 본 제품 또는 장바구니에 아직 없는 가장 최근에 본 제품에 대해 쿼리할 수 있습니다.
...
#### 로드 프로필 아이콘 {#loading-a-new-visitor-profile-with-the-load-profile-icon}으로 새 방문자 프로필 로드
...
#### 프로필을 현재 사용자 {#resetting-the-profile-to-the-current-user}로 재설정
...
클라이언트 컨텍스트를 편집하여 특정 속성의 값을 설정(또는 재설정)할 수 있습니다. 이를 통해 특정 시나리오를 테스트할 수 있습니다(특히 [세그먼테이션](/help/sites-administering/campaign-segmentation.md) 및 [캠페인](/help/sites-classic-ui-authoring/classic-personalization-campaigns.md)에 유용).

![](assets/clientcontext_alisonparker_edit.png)

### 속성 구성 요소 {#adding-a-property-component} 추가

**ClientContext 디자인 페이지**&#x200B;를 연 후에는 사용 가능한 구성 요소를 사용하여 완전히 새 속성 **추가**&#x200B;할 수도 있습니다. 구성 요소는 사이드 킥이나 **새 구성 요소 삽입** 대화 상자에 나열되며 **구성 요소나 자산을 여기로 드래그하십시오.**>을 두 번 클릭한 후 여는 대화 상자입니다. box):

![](assets/clientcontext_alisonparker_new.png)

### 속성 구성 요소 {#removing-a-property-component} 제거

**ClientContext 디자인 페이지**&#x200B;를 연 후 더 이상 필요하지 않은 경우 **속성 제거**&#x200B;도 수행할 수 있습니다. 여기에는 기본적으로 제공되는 속성이 포함됩니다.**Reset**&#x200B;이 제거된 경우 이러한 항목이 복원됩니다.
