...
>
>
>* [상태 ](#health-check) 검사 기능을 사용하면 선택 필드를 사용하여 보고하려는 데이터를 지정할 수 있습니다.
>* [디스크 ](#disk-usage) 사용에서는 링크를 사용하여 저장소 구조를 드릴다운합니다.
...
제목 셀의 맨 오른쪽에 화살표 머리가 나타납니다([현재 정렬 메커니즘](#sorting-the-data)을 나타내는 제목 텍스트 오른쪽에 있는 화살표 헤드와 혼동하지 마십시오).
...
해당 열의 [열의 드롭다운 메뉴](#column-drop-down-menu)에서 **이 열을 기준으로 그룹을 선택할 수 있습니다.** 이렇게 하면 해당 열 내의 서로 다른 각 값에 따라 데이터가 그룹화됩니다. 그룹화할 열을 두 개 이상 선택할 수 있습니다. 열의 데이터가 부적절한 경우 옵션이 회색으로 표시됩니다.즉, 모든 항목은 고유하며 사용자 보고서의 사용자 ID 열과 같이 어떠한 그룹도 만들 수 없습니다.

하나 이상의 열이 그룹화된 후 **현재 데이터**&#x200B;이(가) 이 그룹을 기반으로 생성됩니다. 여러 열을 그룹화하는 경우 차트에도 표시됩니다.
...
적절한 열에는 [열의 드롭다운 메뉴](#column-drop-down-menu)에서 **필터 설정** 및/또는 **집계**&#x200B;을 구성할 수도 있습니다.
...
시간에 따른 데이터의 변경 차트는 **내역 데이터**&#x200B;에서 볼 수 있습니다. 이는 주기적으로 촬영되는 스냅샷에서 비롯됩니다.
...
1. **구성** 을 편집하여 스냅샷 생성 빈도를 정의합니다.시간별 또는 일별
1. **완료..** 스냅샷 수집을 시작하는 정의입니다.

   왼쪽 상단에 있는 빨간색/녹색 슬라이더 버튼은 스냅샷이 수집되는 시점을 나타냅니다.
...
* **간격**
   ...
   * 간격을 `Month`으로 설정하면 2월의 모든 스냅샷이 단일 값으로 집계됩니다(차트에 단일 &quot;점&quot;으로 표시됨).

요구 사항을 선택한 다음 **Go**&#x200B;을 클릭하여 보고서에 적용합니다. 추가 스냅샷이 생성된 후 디스플레이를 업데이트하려면 **Go**&#x200B;을 다시 클릭합니다.

![chlimage_1-43](assets/chlimage_1-43.png)

스냅샷을 수집하는 경우 다음을 수행할 수 있습니다.

* **마침..**&#x200B;을(를) 다시 사용하여 컬렉션을 다시 초기화합니다.

   **보고서 구조(예: 보고서에 지정된 열, 그룹화되고 정렬되고 필터링된 열 등)를 &quot;중단&quot;으로 완료합니다** . 스냅샷을 촬영하기 시작합니다.

* **편집** 대화 상자를 열어 **데이터 스냅샷 없음**&#x200B;을 선택하여 필요한 때까지 컬렉션을 종료합니다.

   **편집 전용** 은 스냅샷 찍기를 켜거나 끕니다. 스냅샷 찍기가 다시 켜지면 추가 스냅샷을 촬영하기 위해 마지막으로 완료되었을 때 보고서 상태를 사용합니다.

>[!NOTE]
>
>스냅샷은 `/var/reports/...` 아래에 저장됩니다. 이 경우 경로의 나머지 부분에서는 보고서가 끝날 때 생성된 각 보고서의 경로와 ID를 미러링합니다.
>
>
...
>[!NOTE]
>
>사전 구성된 보고서는 성능이 높지 않지만 프로덕션 환경에서 일별 스냅샷을 사용하는 것이 좋습니다. 웹 사이트에 활동이 많지 않은 시간에 이러한 일일 스냅샷을 실행할 수 있는 경우;**일 CQ 보고 구성**;에 대한 `Daily snapshots (repconf.hourofday)` 매개 변수로 정의할 수 있습니다.구성 방법에 대한 자세한 내용은 [OSGI 구성](/help/sites-deploying/configuring-osgi.md)을 참조하십시오.
...
사용자 지정된 보고서는 모든 시리즈에 대해 **Total** 값을 표시할 수도 있습니다. 이 항목은 일련의(범례에 있는 가로 행 및 항목)으로 표시됩니다.
...
**편집** 단추는 **보고서 편집** 대화 상자를 엽니다.

이 위치는 [내역 데이터](#historic-data)에 대한 스냅샷을 수집하는 기간이 정의되지만, 다양한 기타 설정도 정의할 수 있는 한 장소입니다.
...
* **스냅샷**
만들 스냅샷 빈도를 정의할 수 있습니다.일별, 시간별 또는 전혀 그렇지 않습니다.

### 데이터 로드 {#load-data}

**데이터 로드** 단추는 **수동으로 새로 고침 데이터**&#x200B;이 **[편집](#edit-report)**&#x200B;에서 선택된 경우에만 표시됩니다.
...
모든 구성 요소, 제품 표준 및 프로젝트별 포함 **편집** 대화 상자를 사용하면 사용자가 보고서의 시작점을 정의하는 **루트 경로**&#x200B;도 설정할 수 있습니다. 해당 루트 아래의 모든 구성 요소는 보고서에 고려됩니다.
...
페이지 활동 보고서는 감사 로그에서 모든 정보를 가져옵니다. 기본적으로 루트 경로는 감사 로그에 `/var/audit/com.day.cq.wcm.core.page`으로 구성됩니다.
...
사용자 보고서에서 **일반** 열을 사용할 수 있으므로 일반적으로 [사용자 프로필](/help/sites-administering/identity-management.md#profiles-and-user-accounts)에서 사용자 지정된 정보에 액세스할 수 있습니다.예를 들어, [프로필 정의에 필드 추가 아래에 자세히 나와 있는 즐겨찾기 색상](/help/sites-administering/identity-management.md#adding-fields-to-the-profile-definition)
...
* **기본 집계**
   ...
   하나 이상의 그룹화된 열이 있는 보고서에서 열을 그룹 해제하면 기본적으로 사용되는 집계를 정의합니다. `Count`, `Minimum`, `Average`, `Maximum`, `Sum`에서 필요한 집계를 선택합니다.

   예를 들어 `String` 필드에 대해 *Count*&#x200B;는 집계된 상태의 열에 대해 서로 다른 `String` 값의 수가 표시됨을 의미합니다.

**Extended** 탭에서 사용 가능한 집계와 필터를 정의할 수도 있습니다.
...
>[!CAUTION]
>
>게시 환경에 대해 **내역 데이터**&#x200B;를 사용하려면 페이지를 활성화하기 전에 작성 환경에 대한 보고서를 **마침**&#x200B;합니다.
...
게시 환경에서 보고서 구성이 허용되지 않으므로 **편집** 및 **완료** 단추를 사용할 수 없습니다. 그러나 스냅샷을 수집하는 경우 **내역 데이터** 보고서에 대해 **기간** 및 **간격**&#x200B;을 선택할 수 있습니다.

![reportsugpublish](assets/reportsucgpublish.png)

>[!CAUTION]
>
>이러한 보고서에 대한 액세스는 보안 문제가 될 수 있습니다.따라서 외부 방문자가 `/etc/reports`을(를) 사용할 수 없도록 디스패처를 구성하는 것이 좋습니다. 자세한 내용은 [보안 검사 목록](security-checklist.md)을 참조하십시오.
