...
다음 섹션에서는 블루프린트와 Live Copy 분기(수동으로 생성)에서 만들어진 새 페이지 `b`의 예를 사용하여 다양한 충돌 해결 방법을 설명합니다.
...
* live copy:`/b`
   ...
   Live Copy 분기에 수동으로 만든 페이지;1개의 하위 페이지가 있는 경우 `lc-level-1`.
...
<table>
 <tbody>
  ...
  <tr>
   ...
   <td><code> /lc-level-1</code> <br /> (live copy 분기에 수동으로 만든 페이지<br /> 하위 수준-1의 내용 포함)</td>
  </tr>
 </tbody>
</table>
...
* 이 처리기에 대한 서비스 등급이 낮습니다(&quot;i.e.사용자 지정된 처리기에 높은 등급이 필요할 것이라는 가정 하에 `service.ranking` 속성의 기본값 아래. 하지만 필요할 때 유연성을 제공하기 위한 최소 등급은 아닙니다.

이 충돌 핸들러는 블루프린트에 우선 순위를 부여합니다. Live Copy 페이지 `/b`이(가) `/b_msm_moved`(Live Copy 분기 내)로 이동되었습니다.

* live copy:`/b`

   Live Copy 내에서 `/b_msm_moved`(으)로 이동됩니다. 이렇게 하면 백업 기능이 수행되므로 컨텐츠 손실이 없습니다.
...
<table>
 <tbody>
  ...
  <tr>
   ...
   <td><code>b</code> <br /> 변경 금지live copy 분기에 수동으로 만들어졌고 이제 b_msm_moved라고 하는 원본 페이지의 컨텐츠를 포함합니다.<br /> </td>
  </tr>
  <tr>
   <td><code> /bp-level-1</code></td>
...
* [OSGi 구성](/help/sites-deploying/configuring-osgi.md);을 사용하여 구성할 수 있습니다.특히
...
수동으로 [충돌 처리 비활성화](#rollout-manager-and-conflict-handling)하면 AEM에서 충돌하는 페이지에 대해 작업을 수행하지 않습니다(충돌하지 않는 페이지는 예상대로 롤아웃됩니다).
