...
* 일치하는 로그인 경로를 LoginSelectorHandler에 표시하는 LoginPathProvider를 구현하는 OSGi 구성 요소입니다. granite:AuthenticationRequired mixin 형식의 수단을 통해 컨텐츠에 저장된 변경된 인증 요구 사항을 수신하는 관찰자를 등록하는 데 사용되는 RequirementsHandler에 대한 필수 참조가 있습니다.
* SlingAuthenticator에 인증 요구 사항 변경 사항을 알리는 RequirementsHandler를 구현하는 OSGi 구성 요소입니다.
