...
seo-title: Adobe Analytics 및 프레임워크 연결
...
### Adobe Analytics 데이터 센터 사용을 위한 AEM 구성 {#configuring-aem-to-use-your-adobe-analytics-data-centers}
...
[웹 콘솔을 사용하여 OSGi 번들](/help/sites-deploying/configuring-osgi.md#osgi-configuration-with-the-web-console) **Adobe AEM Analytics HTTP 클라이언트**&#x200B;을 구성합니다. AEM 페이지에서 데이터를 수집하는 보고서 세트를 호스팅하는 데이터 센터에 대해 **데이터 센터 URL**&#x200B;을 추가합니다.
...
## Adobe Analytics에 대한 연결 구성 {#configuring-the-connection-to-adobe-analytics}
...
## Activity Map {#configuring-for-the-activity-map}에 대한 구성
...
>
>예를 들어, 프레임워크는 *diweretail* 보고서 세트를 사용하도록 구성되어 있으며 작성자는 선택한 서버 인스턴스입니다. 페이지가 프레임워크와 함께 게시되더라도 Adobe Analytics에 대한 호출은 여전히 진행되지만, 이러한 호출에는 RSID가 포함되지 않습니다. 작성자 인스턴스의 호출에만 RSID가 포함됩니다.
...
1. 그런 다음 **실행 모드** 드롭다운(보고서 세트 ID 옆)을 사용하여 정보를 보고서 세트에 전송할 서버 인스턴스를 선택합니다.
...
>[!CAUTION]
>
>이러한 설정은 데이터의 전송 위치와 방법을 결정하므로, *이 설정*&#x200B;을(를) 변경하지 않고 Adobe Analytics 담당자가 대신 설정하도록 해야 합니다.
...
* **방문자 네임스페이스**
   ...
   * 예: 네임스페이스를 **CNAME**&#x200B;으로 변경하면 Adobe Analytics에 대한 호출이 기본값이 아닌 **CNAME.d1.omtrdc.net**&#x200B;과 같이 표시됩니다.

## 페이지를 Adobe Analytics 프레임워크 {#associating-a-page-with-a-adobe-analytics-framework}과 연결
...
1. **저장 및 닫기**&#x200B;를 선택합니다.
1. **[페이지를](/help/sites-authoring/publishing-pages.md)** 게시하여 페이지와 연결된 구성/파일을 활성화합니다.
1. 마지막 단계는 게시 인스턴스의 페이지를 방문하고 **검색** 구성 요소를 사용하여 키워드(예: 가지를)를 검색하는 것입니다.
...
Adobe Analytics 프레임워크을 삭제하려면 먼저 [을(를) 열어](#editing-adobe-analytics-configurations-and-or-frameworks)을(를) 편집하십시오.

그런 다음 사이드 킥의 **페이지** 탭에서 **Delete Framework**&#x200B;을 선택합니다.
