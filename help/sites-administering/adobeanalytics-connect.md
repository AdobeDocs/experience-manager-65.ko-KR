...
Adobe Analytics에서 AEM 페이지의 웹 데이터를 추적하려면 Adobe Analytics 클라우드 서비스 구성 및 Adobe Analytics 프레임워크를 만드십시오.
...
* **Adobe Analytics Framework:** Adobe Analytics 보고서 세트 속성과 CQ 변수 간의 매핑 집합. 프레임워크를 사용하여 웹 사이트 데이터가 Adobe Analytics 보고서를 채우는 방법을 구성합니다. 프레임워크는 Adobe Analytics 구성과 연관됩니다. 각 구성에 대해 여러 프레임워크를 만들 수 있습니다.
...
* **방문자 네임스페이스**
   ...
   * 예: 네임스페이스를 **CNAME으로** 변경하면 Adobe Analytics에 대한 호출이 기본값이 아닌 **CNAME.d1.omtrdc.net** 과같이 됩니다.

## Adobe Analytics 프레임워크와 페이지 연결 {#associating-a-page-with-a-adobe-analytics-framework}

페이지가 Adobe Analytics 프레임워크와 연결되면 페이지가 로드될 때 페이지가 Adobe Analytics로 데이터를 전송합니다. 페이지가 채우는 변수는 프레임워크의 Adobe Analytics 변수에서 매핑되고 검색됩니다. 예를 들어, 페이지 보기는 Adobe Analytics에서 검색됩니다.
...
1. 구성 **추가** 드롭다운을 사용하여 사용 가능한 옵션 **에서** Adobe Analytics를 선택합니다. 상속이 있는 경우 선택기를 사용할 수 있게 되기 전에 상속을 비활성화해야 합니다.

1. Adobe **Analytics** 드롭다운 선택기가 사용 가능한 옵션에 추가됩니다. 필요한 프레임워크 구성을 선택하려면 이 옵션을 사용합니다.
...
페이지가 Adobe Analytics 프레임워크와 연결되면 사이트 콘솔의 목록 보기에 페이지 보기 수가 표시될 수 있습니다.

자세한 [내용은 페이지 분석 데이터](/help/sites-authoring/page-analytics-using.md) 보기를 참조하십시오.
