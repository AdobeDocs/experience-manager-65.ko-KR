...
계속하기 전에 자격 증명을 사용하여 Adobe Analytics에 로그인할 수 있는지 확인하십시오. 다음 중 하나를 사용하여 다음을 수행합니다.
...
Adobe Analytics [데이터 센터](https://experienceleague.adobe.com/docs/analytics/analyze/reports-analytics/reporting-interface/overview-data-collection.html?lang=en) Adobe Analytics 보고서 세트와 관련된 데이터를 수집, 처리 및 저장합니다. Adobe Analytics 보고서 세트를 호스팅하는 데이터 센터를 사용하도록 AEM을 구성합니다. 데이터 센터는 계약에 언급되어 있습니다. 이 정보는 조직의 관리자에게 문의하십시오.

필요한 경우 다음 사항을 사용하여 올바른 데이터 센터로 라우팅하십시오. `https://api.omniture.com/`.

조직에서 특정 데이터 센터에서 데이터를 수집하거나 검색해야 하는 경우 다음을 사용합니다.
...
1. 에서 **보고서 세트** 측면 pod(기본 패널의 오른쪽)의 섹션에서 **항목 추가**. 그런 다음 드롭다운을 사용하여 보고서 세트 ID 를 선택합니다(예: `geometrixxauth`)에서 사용할 수 있습니다.

   >[!NOTE]
   >
   >왼쪽의 컨텐츠 파인더는 보고서 세트 ID를 선택하면 Adobe Analytics 변수(SiteCatalyst 변수)로 채워집니다.

1. 보고서 세트에 정보를 전송할 서버 인스턴스를 선택하려면 **실행 모드** 드롭다운(보고서 세트 ID 옆)을 클릭합니다.
...
* **서버 추적**
   ...
   * 에는 Adobe Analytics 호출을 전송하는 데 사용되는 URL이 포함되어 있습니다
      ...
      * `d1` - 정보를 전송하는 데이터 센터에 해당합니다. `d1`, `d2`, 또는 `d3`)
...
* **방문자 네임스페이스**
   ...
   * 예를 들어 네임스페이스를 **CNAME** 은 Adobe Analytics에 대한 호출이 다음과 같이 표시되도록 합니다. **CNAME.d1.omtrdc.net** 대신 을 사용해야 합니다.
...
1. 를 사용하십시오 **구성 추가** 선택할 드롭다운 **Adobe Analytics** 사용 가능한 옵션에서 상속이 있는 경우 선택기를 사용할 수 있게 되기 전에 상속을 비활성화합니다.

1. 에 대한 드롭다운 선택기 **Adobe Analytics** 이 사용 가능한 옵션에 추가됩니다. 필요한 프레임워크 구성을 선택합니다.

1. 선택 **저장 및 닫기**.
1. 페이지와 연결된 구성/파일을 활성화하려면 **[게시](/help/sites-authoring/publishing-pages.md)** 페이지를 클릭합니다.
1. 마지막 단계는 게시 인스턴스의 페이지를 방문하여 을 사용하여 키워드(예: 가지를)를 검색하는 것입니다 **검색** 구성 요소.
1. 그런 다음 적절한 도구를 사용하여 Adobe Analytics에 대한 호출을 확인할 수 있습니다. 예 [Adobe Experience Cloud Debugger](https://experienceleague.adobe.com/docs/experience-platform/debugger/home.html).
1. 제공된 예제를 사용하여 호출에는 eVar7에 입력한 값(즉, egplant)이 포함되어야 하며 이벤트 목록에는 event3이 포함되어야 합니다.
...
Adobe Analytics 구성을 편집할 때 **편집** 구성 페이지 자체에서 열기 **구성 요소 편집** 대화 상자.
