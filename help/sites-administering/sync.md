...
>[!CAUTION]
>
>사용자 동기화가 활성화된 경우에도 ***관리자*** 그룹 동기화는 지원되지 않습니다. 대신 &#39;비교 가져오기&#39; 실패 오류가 오류 로그에 기록됩니다.
>
...
* **작성자**
   ...
   * locate `Apache Sling Distribution Agent - Sync Agents Factory`
      ...
      * 편집할 기존 구성 선택(연필 아이콘)확인 `name`: **`socialpubsync`**
...
* **작성자**
   ...
   * 편집할 기존 구성 선택(연필 아이콘)확인 `property name`: **`socialpubsync-publishUser`**
...
* **게시 시**:
   ...
   * locate `Apache Sling Distribution Agent - Queue Agents Factory`
      ...
      * 편집할 기존 구성 선택(연필 아이콘)확인 `Name`: `socialpubsync-reverse`
...
* **각 게시 인스턴스에 대해 다음을 수행합니다**.
   ...
   * 편집할 기존 구성 선택(연필 아이콘)확인 `Name`: `socialpubsync-scheduled-trigger`
...
* **무시 가능 속성**변경 사항이 감지되면 무시되는 속성 목록입니다. 이러한 속성에 대한 변경 사항은 다른 변경 사항의 부작용으로 동기화될 수 있지만(동기화는 항상 노드 수준이기 때문에) 이러한 속성에 대한 변경 사항은 자체 동기화를 트리거하지 않습니다.
무시할 기본 속성:

   * cq:lastModified

* **Ignorable Nodes**동기화 중 완전히 무시되는 하위 경로. 이러한 하위 경로 아래의 어떤 것도 동기화되지 않습니다.
무시할 기본 노드:

   * .tokens
   * 시스템

* **가장 많이 배포된 폴더**는 동기화가 필요하지 않으므로 폴더가 무시됩니다. 몇 가지 예외가 여기에 나열되어 있습니다.
동기화할 기본 폴더
...
* 찾기 `Apache Sling Distribution Packaging - Vault Package Builder Factory`

   * `Builder name: socialpubsync-vlt`

* 편집 아이콘 선택
* 다음 두 개 추가: `Package Node Filters`
...
작성 환경에서 진단 프로그램이 실행되면 통과/실패 결과에 확인을 위해 구성된 게시 인스턴스의 목록을 표시하는 [INFO] 섹션이 포함됩니다.

목록에 포함된 URL은 해당 인스턴스에 대한 진단을 실행하는 각 게시 인스턴스의 URL입니다. url 매개 변수 `syncUser` 는 진단 URL에 추가되며 이 값은 2단계에서 만든 *승인된 동기화 사용자* 로 [설정됩니다](#createauthuser).
...
#### (작성자) 하나의 Apache Sling 배포 전송 자격 증명 - DistributionTransportSecretProvider 기반 사용자 자격 증명 {#author-one-apache-sling-distribution-transport-credentials-user-credentials-based-distributiontransportsecretprovider}
