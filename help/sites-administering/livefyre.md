...
AEM 6.5에는 미리 설치된 Livefyre 기능 패키지 1.2.6이 포함되어 있습니다. 이 패키지는 AEM Sites와의 제한된 Livefyre 통합만 포함되며 업데이트된 패키지를 설치하기 전에 제거해야 합니다. 최신 패키지를 사용하면 Sites, Assets, Commerce 등 AEM과 Livefyre가 완벽하게 통합됩니다.

>[!NOTE]
>
>AEM-LF 패키지의 일부 기능은 SCF(소셜 구성 요소 프레임워크)에 따라 다릅니다. Livefyre 기능 팩을 비커뮤니티 사이트의 일부로 사용하는 경우 웹 사이트의 작성자 클라이언트에서는 *cq.social.scf*&#x200B;을(를) 종속성으로 선언해야 합니다. LF 기능 팩을 커뮤니티 웹 사이트의 일부로 사용하는 경우 이 종속성은 이미 선언되어야 합니다.

1. AEM 홈 페이지에서 왼쪽 레일에 있는 **도구** 아이콘을 클릭합니다.
1. **배포 > 패키지**&#x200B;로 이동합니다.
1. 패키지 관리자에서 미리 설치된 Livefyre 기능 패키지가 표시될 때까지 스크롤한 다음 패키지 제목 **cq-social-livefyre-pkg-1.2.6.zip**&#x200B;을 클릭하여 옵션을 확장합니다.
1. **자세히 > 제거**&#x200B;를 클릭합니다.
...
1. AEM 홈 페이지에서 왼쪽 레일에 있는 **도구** 아이콘을 클릭한 다음 **일반 > 구성 브라우저**&#x200B;로 이동합니다.
...
1. AEM 홈 페이지에서 왼쪽 레일에 있는 **도구** 아이콘을 클릭한 다음 **배포 > Livefyre 구성**&#x200B;으로 이동합니다.
1. 새 Livefyre 구성을 만들 구성 폴더를 선택한 다음 **만들기**&#x200B;를 클릭합니다.
   ...
   >[!NOTE]
   >
   >폴더에 클라우드 구성이 활성화되어 있어야 Livefyre 구성을 추가할 수 있습니다. 구성 폴더는 [구성 브라우저에서 만들고 관리됩니다.](/help/sites-administering/configurations.md)
   >
...
1. 조직의 Livefyre 자격 증명을 입력한 다음 **OK**&#x200B;을 클릭합니다.
...
1. CRXDE Lite을 사용하여 */libs/social/integrations/livefyre/components/authorizableomponent/authentlib*&#x200B;을 */apps/social/integrations/livefyre/components/authorizableconent/authclientlib*&#x200B;로 복사합니다.
1. */apps/social/integrations/livefyre/components/authorizablecomponent/authclientlib/auth.js*&#x200B;를 편집하고 저장하여 사용자의 요구 사항에 맞는 Livefyre 인증 위임을 구현합니다.
...
## AEM Sites {#use-livefyre-with-aem-sites}과 함께 Livefyre 사용

### Livefyre 구성 요소를 페이지에 추가 {#add-livefyre-components-to-a-page}
...
1. 디자인 모드의 **구성 요소** 사이드 패널에서 메뉴에서 **Livefyre**&#x200B;을 선택하여 목록을 사용 가능한 Livefyre 구성 요소로 제한합니다.
...
1. **구성** 아이콘(렌치)을 클릭하여 구성 대화 상자를 엽니다.
1. **이 구성 요소를 편집하려면 Livefyre 스튜디오**&#x200B;로 이동합니다.
1. Livefyre 스튜디오에서 앱을 편집합니다.

## AEM Assets {#use-livefyre-with-aem-assets}과 함께 Livefyre 사용

### 권한 요청 및 UGC를 AEM Assets {#request-rights-and-import-ugc-into-aem-assets}에 가져오기
...
>[!NOTE]
>
>자산을 사용하여 UGC를 가져오기 전에 Livefyre 스튜디오에서 소셜 계정 및 권한 요청 계정을 설정해야 합니다. [설정 참조:권한 요청](https://docs.adobe.com/content/help/en/livefyre/using/rights-requests/c-how-requesting-rights-works.html)을 참조하십시오.
...
1. 가져올 자산을 선택합니다. 선택한 자산은 **선택한** 탭에서 자동으로 계산되고 저장됩니다.
1. **선택 사항**:선택 탭 **** 을 클릭하고 가져올 선택한 UGC 컨텐츠를 검토합니다.
1. **다음**&#x200B;을 클릭합니다.
...
1. 권한 요청에 대해 각 자산에 대해 다음 옵션 중 하나를 선택합니다.
   ...
   * **수동으로 권한** 요청 Instagram을 통해 복사하여 컨텐츠 소유자에게 수동으로 전송할 수 있는 메시지를 수신합니다.
   * **콘텐츠 권한** 수동 속성을 지정하여 개별 자산에 대한 권한을 무시합니다.
   ...
   * **메시지** 권한 부여를 요청하는 콘텐츠 소유자에게 메시지를 보냅니다.
   * **콘텐츠 권한** 수동 속성을 지정하여 개별 자산에 대한 권한을 무시합니다.
...
1. 오른쪽 위 모서리에서 **완료**&#x200B;를 클릭하여 권한 요청 워크플로우를 마칩니다.
...
### AEM Commerce {#import-product-catalogs-into-livefyre-with-aem-commerce}를 사용하여 제품 카탈로그를 Livefyre로 가져오기
