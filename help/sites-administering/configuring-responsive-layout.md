...
* [**레이아웃 컨테이너**](/help/sites-authoring/responsive-layout.md#adding-a-layout-container-and-its-content-edit-mode) 구성 요소
   ...
   이 구성 요소는 응답형 격자 내에 구성 요소를 추가 및 배치할 수 있도록 해주는 격자 단락 시스템을 제공합니다. 페이지의 기본 parsys로 사용하거나 구성 요소 브라우저에서 작성자가 사용할 수 있습니다.
...
배포자를 포함하려면 사이트를 장치 그룹에 연결해야 합니다. 다음을 참조하십시오 [장치 목록 추가](/help/sites-developing/responsive.md#adding-the-devices-list) (클래식 및 터치에 적합한 UI 모두).
...
* 제목 및 너비 정의:
   ...
   * 제목은 필요한 경우 방향을 사용하여 일반 장치 그룹화를 설명합니다(예: 전화, 태블릿, 태블릿 가로).
...
또한 다음을 생성해야 합니다. [클라이언트 라이브러리](https://docs.adobe.com/content/docs/en/aem/6-0/develop/the-basics/clientlibs.html) 추가 구성 및 함수 호출을 제공합니다. 다음 LESS 추출은 프로젝트에 추가해야 하는 최소값의 예입니다.
...
응답형 컨테이너 내에 있는 구성 요소는 응답형 격자 크기에 따라 크기가 (해당 HTML DOM 요소와 함께) 조정됩니다. 따라서 이러한 상황에서는 고정 너비(포함된) DOM 요소의 정의를 피하거나 업데이트하는 것이 좋습니다.
