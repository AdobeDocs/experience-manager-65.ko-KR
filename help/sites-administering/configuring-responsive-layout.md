...
* [**레이아웃 컨테이너**](/help/sites-authoring/responsive-layout.md#adding-a-layout-container-and-its-content-edit-mode) 구성 요소
   ...
   * 레이아웃 컨테이너를 정의할 수 있습니다.
      ...
      * 모두.
         ...
         사용자가 이 페이지 내에 추가 레이아웃 컨테이너를 추가할 수 있도록 하면서 레이아웃 컨테이너를 페이지의 표준으로 지정할 수 있습니다.예를 들어 열 컨트롤을 만들려면
...
>[!NOTE]
>
>바로 사용 가능한 설치에서 응답형 레이아웃이 [We.Retail 참조 사이트](/help/sites-developing/we-retail.md)에 대해 구성되었습니다. 다른 페이지에 대해서는 여전히 [레이아웃 컨테이너 구성 요소](#enable-the-layout-container-component-for-page)를 활성화해야 합니다.
...
중단점은 해당 템플릿(또는 컨텐츠) 폴더 아래의 `.context.html`의 `<jcr:content>` 섹션 내에 있습니다.
...
### 레이아웃 컨테이너를 기본 Parsys {#set-layout-container-as-main-parsys}으로 설정
...
#### LESS {#css-for-breakpoints-using-less}을(를) 사용하는 중단점에 대한 CSS
...
응답형 그리드에 포함된 응용 이미지의 컨텐츠 크기를 적절하게 조정하고 업데이트하려면 `afterEdit` 리스너에 설정된 `REFRESH_PAGE` 리스너를 포함된 모든 구성 요소의 `EditConfig` 파일에 추가해야 합니다.
...
작성자는 이러한 작업을 통해 **레이아웃 컨테이너** 구성 요소의 인스턴스를 페이지로 드래그할 수 있습니다.
...
* **작성 환경**
   ...
   페이지에 대한 [디자인 모드](/help/sites-authoring/default-components-designmode.md)레이어 컨테이너&#x200B;**구성 요소를 활성화하려면 &lt;a0/>디자인 모드&lt;a1/>를 사용하십시오.**
