...
### 약식 롤아웃을 수행하는 방법 {#shallow-rollout}

AEM as a Cloud Service을 사용할 때 하위 페이지를 전파하지 않고 단일 특정 페이지를 롤아웃해야 하는 경우가 있을 수 있습니다. 올바르게 구성되지 않은 경우 페이지 롤아웃에 대한 일반적인 curl 명령에 하위 페이지가 실수로 포함될 수 있습니다. 이 섹션에서는 curl 명령을 조정하여 지정된 페이지의 약식 롤아웃을 수행하고 추가 하위 페이지를 제외하는 방법에 대해 설명합니다.

약식 롤아웃을 수행하려면 다음 단계를 수행합니다.

1. 매개 변수를 `type=deep`에서 `type=page`(으)로 변경하여 기존 curl 명령을 수정합니다.
1. curl 명령에 다음 구문을 사용합니다.
