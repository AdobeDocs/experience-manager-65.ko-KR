...
## Personalization 및 커뮤니티 컨텐츠 {#personalization-and-community-content}

블로그, 포럼, 캘린더와 같은 커뮤니티 기능은 일반적으로 UGC(사용자 생성 컨텐츠)라고 하는 커뮤니티 컨텐츠를 생성합니다. UGC가 여러 AEM 인스턴스([게시 팜](/help/communities/topologies.md))로 구성된 게시 환경에 입력되면, 한 가지 주요 문제는 모든 인스턴스에서 UGC를 동기화하는 방법입니다.

[AEM Communities 6.1](/help/communities/overview.md) 확장을 사용하면 UGC용 [일반 저장소](/help/communities/working-with-srp.md)를 사용하여 이 문제를 해결합니다. 개인화와 관련하여 커뮤니티에는 사이트 방문자가 Facebook 및 Twitter으로 로그인할 수 있는 옵션을 제공하는 기능인 [소셜 로그인](/help/communities/social-login.md)이(가) 포함되어 있습니다.
