...
추가 `granite:AuthenticationRequired` 구성된 지원되는 경로 내의 mixin 유형에서는 새 항목이 포함된 책임 처리기의 OSGi 등록이 업데이트됩니다 `sling.auth.requirements` 속성을 사용합니다. 지정된 인증 요구 사항이 선택 사항을 지정하는 경우 `granite:loginPath` 속성을 사용하면 인증 요구 사항에서 제외하기 위해 값이 &#39;-&#39; 접두사로 Authenticator에 추가로 등록됩니다.
...
에 의해 정의된 확장 `JackrabbitAccessControlManager` 사용자가 액세스 제어 정책을 편집할 수 있도록 허용하는 CUG 정책은 항상 모든 주도자에게 영향을 주듯이 CUG 액세스 제어 관리를 통해 구현되지 않습니다. 다음과 같이 나열된 `PrincipalSetPolicy` 다른 모든 주체는 대상 노드에 의해 정의된 트리에서 컨텐츠를 읽을 수 없지만 읽기 액세스 권한이 부여됩니다.
...
>[!NOTE]
>
>위에 언급된 지정된 대상 노드에 대한 수정 사항은 `RequirementHandler` 가 구성되었으며 대상이 지원되는 경로에 정의된 트리에 포함됩니다(구성 옵션 섹션 참조).
>
...
로그인 경로를 포함한 새 인증 요구 사항을 만드는 절차. 참고: `RequirementHandler` 대상 노드가 포함된 트리에 대해 구성되었습니다.
...
기존 인증 요구 사항을 제거하는 절차. Apache Sling Authenticator가 `RequirementHandler` 대상 노드가 포함된 트리에 대해 구성되었습니다.
 ...
 <tbody>
  ...
  <tr>
   ...
   <td>
    ...
    </ul> <p>또한, <a href="#configuration-options">구성 옵션</a> 아래의 제품에서 사용할 수 있습니다.</p> </td>
  </tr>
  <tr>
   <td>구성 정책</td>
...
새로운 구현을 통해 기본 인증 모델의 액세스 제어 설정은 생성, 수정 또는 제거되는 CUG의 영향을 받지 않습니다. 대신 새 유형의 정책이 `PrincipalSetPolicy` 대상 노드에 대한 추가 액세스 제어 컨텐츠로 적용됩니다. 이 추가 정책은 대상 노드의 1차 하위 구성요소로 지정되며, 있는 경우 기본 정책 노드의 동위가 됩니다.
...
이전 CUG 지원에서 추가 인증 모델로 이동하면 유효한 읽기 권한을 평가하는 방식이 변경됩니다. 에 설명된 대로 [Jackrabbit 설명서](https://jackrabbit.apache.org/oak/docs/security/authorization/composite.html), 주어진 주도자가 `CUGcontent` Oak 저장소에 구성된 모든 모델에 대한 권한 평가가 읽기 권한을 부여하는 경우에만 읽기 액세스 권한이 부여됩니다.
...
CUG 정책의 새 AEM 기본 설정 평가에서는 &#39;게시&#39; 실행 모드에서만 사용할 수 있습니다. 자세한 내용은 [AEM 6.3 이후의 기본 구성](#default-configuration-since-aem) 자세한 내용 지정된 경로에 대한 유효한 정책을 컨텐츠에 저장된 정책과 비교하여 확인할 수 있습니다. CUG에 대한 권한 평가가 활성화된 경우에만 유효한 정책이 표시됩니다.
...
1. CUG를 확인할 루트 경로를 입력하고 키를 누릅니다 **연습 실행** 버튼을 클릭합니다. 선택한 위치에서 전환 가능한 CUG를 검색합니다.
