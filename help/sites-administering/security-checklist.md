...
최신 버전을 설치했는지 확인하십시오. [Adobe에서 제공한 보안 핫픽스](https://helpx.adobe.com/kr/experience-manager/kb/aem63-available-hotfixes.html).

### AEM 및 OSGi 콘솔 관리자 계정에 대한 기본 암호 변경 {#change-default-passwords-for-the-aem-and-osgi-console-admin-accounts}

Adobe은 설치 후 권한이 있는 의 암호를 변경할 것을 강력히 권장합니다 [**AEM** `admin` 계정](#changing-the-aem-admin-password) (모든 인스턴스에서).

이러한 계정에는 다음이 포함됩니다.

* 더 AEM `admin` account

   AEM 관리자 계정의 암호를 변경한 후에는 CRX에 액세스할 때 새 암호를 사용해야 합니다.
...
웹 콘솔에 액세스하는 데 사용되는 암호도 변경해야 합니다. 이 작업은 (으)로 완료되었습니다. [OSGI 구성](/help/sites-deploying/configuring-osgi.md) 의 다음 속성을 업데이트하려면 **Apache Felix OSGi 관리 콘솔**:
...
Adobe은 특히 404 및 500개의 HTTP 응답 코드에 대해 사용자 지정 오류 처리기 페이지를 정의하여 정보 공개를 방지할 것을 권장합니다.

>[!NOTE]
>
>다음을 참조하십시오 [사용자 지정 스크립트 또는 오류 핸들러를 만드는 방법](https://helpx.adobe.com/experience-manager/kb/CustomErrorHandling.html) 자세한 내용은 기술 자료 문서를 참조하십시오.

### Dispatcher 보안 검사 목록 완료 {#complete-dispatcher-security-checklist}

AEM Dispatcher는 인프라의 중요한 부분입니다. Adobe은 다음을 완료할 것을 강력히 권장합니다. [dispatcher 보안 검사 목록](https://helpx.adobe.com/kr/experience-manager/dispatcher/using/security-checklist.html).
...
* 다음 **복제 사용자** 또는 **에이전트 사용자 Id** 또한 관리자가 아닌 복제되어야 하는 콘텐츠만 볼 수 있는 사용자여야 합니다. 복제 사용자는 작성자 시스템에서 복제할 콘텐츠를 게시자에게 보내기 전에 수집하는 데 사용됩니다.
...
>[!NOTE]
>
>샘플 We.Retail 애플리케이션은에서 이 인스턴스가 실행되는 경우 제거됩니다. [프로덕션 준비 모드](/help/sites-administering/production-ready.md). 어떤 이유로든 그렇지 않은 경우 패키지 관리자로 이동하여 샘플 콘텐츠를 제거한 다음 모든 We.Retail 패키지를 검색하여 제거할 수 있습니다. 자세한 내용은 [패키지를 사용한 작업](package-manager.md).
...
다음 [Eclipse용 AEM 개발자 도구](/help/sites-developing/aem-eclipse.md) apache Sling 도구 지원 설치(org.apache.sling.tooling.support.install)를 배포합니다.
...
CRX WebDAV 및 Apache Sling에서 CSRF(크로스 사이트 요청 위조)와 관련된 알려진 보안 문제를 해결하려면 레퍼러 필터에 대한 구성을 추가해야 합니다.
...
1. 다음 확인: `Allow Empty` 필드(비어 있거나 누락된 레퍼러 헤더를 허용하려는 경우)
   ...
   >[!CAUTION]
   >
   >다음과 같은 명령줄 도구를 사용하는 동안 레퍼러를 제공하는 것이 좋습니다. `cURL` CSRF 공격에 시스템을 노출할 수 있으므로 빈 값을 허용하지 않습니다.

1. 이 필터에서 확인에 사용해야 하는 방법 편집 `Filter Methods` 필드.

1. 클릭 **저장** 변경 사항을 저장합니다.

### OSGI 설정 {#osgi-settings}

일부 OSGI 설정은 응용 프로그램을 보다 쉽게 디버깅할 수 있도록 기본적으로 설정됩니다. 이러한 내부 정보가 외부로 유출되지 않도록 게시 및 작성자 생산성 인스턴스에서 변경해야 합니다.

>[!NOTE]
>
>을 제외하고 아래의 모든 설정 **일별 CQ WCM 디버그 필터** 이(가) 자동으로 다음에 의해 포함됨: [프로덕션 준비 모드](/help/sites-administering/production-ready.md). 이러한 이유로 프로덕션 환경에서 인스턴스를 배포하기 전에 모든 설정을 검토하는 것이 좋습니다.

다음 각 서비스의 경우 지정된 설정을 변경해야 합니다.
...
자세한 내용은 을 참조하십시오. [OSGi 구성 설정](/help/sites-deploying/osgi-configuration-settings.md).
...
서비스 거부(DoS) 공격은 의도한 사용자가 컴퓨터 리소스를 사용할 수 없게 하려고 시도하는 것입니다. 이 작업은 리소스를 오버로드하여 수행하는 경우가 많습니다. 예를 들면 다음과 같습니다.

* 외부 소스의 요청이 쇄도하는 경우.
* 시스템이 성공적으로 전달할 수 있는 것보다 더 많은 정보를 요청합니다.
...
* 콘텐츠 페이지에 URL을 무제한으로 요청하여 해당 URL에는 핸들, 일부 선택기, 확장 프로그램 및 접미사가 포함될 수 있으며, 이러한 모든 URL은 수정할 수 있습니다.
   ...
   * `.../en.html/SuffixDosAttack`
   ...
   모든 유효한 변형(예: 반환 `200` 응답 및 가 캐시되도록 구성됨)이 dispatcher에 의해 캐시되므로 결국 전체 파일 시스템이 되고 추가 요청에 대한 서비스가 제공되지 않습니다.

이러한 공격을 방지하기 위한 많은 구성 지점이 있으며, 여기서는 AEM과 직접적으로 관련된 것들에 대해서만 논의합니다.
...
* 두 번째로 렌더러 또는 스크립트는 요청의 특정 부분(예: 선택기 및/또는 확장)과 함께 리소스 속성에서 배치됩니다.

>[!NOTE]
>
>이에 대해서는 다음에서 자세히 다룹니다. [Sling 요청 처리](/help/sites-developing/the-basics.md#sling-request-processing).

이러한 접근 방식을 통해 Sling은 매우 강력하고 유연하지만 항상 그렇듯이 신중하게 관리해야 하는 유연성이 있습니다.

DoS 오용을 방지하기 위해 다음을 수행할 수 있습니다.

1. 응용 프로그램 수준에서 컨트롤을 통합합니다. 가능한 변형 수로 인해 기본 구성은 불가능합니다.
...
1. 문제 영역일 수 있는 기본 렌더러의 구성을 확인합니다.
   ...
   * 특히 트리 구조를 여러 레벨에 걸쳐 가로지를 수 있는 JSON 렌더러입니다.

      예를 들어, 요청은 다음과 같습니다.

      `http://localhost:4502/.json`

      전체 저장소를 JSON 표현으로 덤프할 수 있습니다. 이로 인해 심각한 서버 문제가 발생할 수 있습니다. 이러한 이유로 Sling은 최대 결과 수에 대한 제한을 설정합니다. JSON 렌더링의 깊이를 제한하려면 다음 값을 설정할 수 있습니다.
   ...
   * 예방 조치로 다른 기본 렌더러(HTML, 일반 텍스트, XML)를 비활성화합니다. 를 구성하여 다시 [Apache Sling GET 서블릿](/help/sites-deploying/osgi-configuration-settings.md#apache-sling-get-servlet).
   >[!CAUTION]
   >
   >JSON 렌더러를 비활성화하지 마십시오. AEM의 일반 작업에 필요합니다.
...
AEM은 기본 제공 색인을 제공하지 않으므로 `FormChooserServlet`, 쿼리에서 양식 선택기를 사용하면 비용이 많이 드는 저장소 순회를 트리거하며 일반적으로 AEM 인스턴스를 중지합니다. 양식 선택기는 **&amp;ast;.form.&amp;ast;** 쿼리의 문자열입니다.
...
1. 항목을 클릭한 후 **고급 검색 필요** 다음 창에서 실행하십시오.
...
작성자 및 게시 환경 모두에서 WebDAV를 비활성화해야 합니다. 이 작업은 적절한 OSGi 번들을 중지하여 수행할 수 있습니다.
...
1. 이 번들을 중지하려면 [작업] 열에서 [중지] 단추를 클릭하십시오.

1. 번들 목록에서 다음 이름의 번들을 다시 찾습니다.
...
AEM 6.1 이후 사용자(승인 가능 노드라고도 함) ID 노드 이름이 저장되는 방식이 `AuthorizableNodeName` 인터페이스. 새 인터페이스는 더 이상 노드 이름에 사용자 ID를 표시하지 않지만 대신 임의의 이름을 생성합니다.
...
1. 삭제 **Apache Jackrabbit Oak 무작위 승인 가능 노드 이름** 웹 콘솔에서 OSGi 구성.
   ...
   더 쉬운 조회를 위해 이 구성에 대한 PID는 입니다. **org.apache.jackrabbit.oak.security.user.RandomAuthorizableNodeName**.
...
이 작업을 수행하기 전에 키 복제는 버전 간에 다르게 수행되는데, 이는 키가 저장되는 방식이 6.3 및 이전 버전 간에 다르기 때문입니다.
