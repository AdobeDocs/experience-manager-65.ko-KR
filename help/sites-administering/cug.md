...
* [이 그룹을 필요한 ](#applying-your-closed-user-group-to-content-pages) 페이지에 적용하고 CUG 구성원이 사용할 로그인 페이지를 선택(또는 만들기)합니다.또한 컨텐츠 페이지에 CUG를 적용할 때 지정됩니다.

* [일부 양식의 링크를 보호 영역](#linking-to-the-realm) 내에 있는 하나 이상의 페이지에 만들면 표시되지 않습니다.
* [사용 중인 ](#configure-dispatcher-for-cugs) Dispatcher 구성
...
1. CUG에 할당한 모든 사용자 활성화;이 경우 `cug_access`의 모든 구성원이
1. 비공개 사용자 그룹을 활성화하여 게시 환경에서 사용할 수 있도록 합니다.이 예에서 `cug_access`입니다.
...
1. 다음 창에서 CUG를 찾아 추가합니다. 이 경우 **cug_access**&#x200B;이라는 그룹을 추가합니다. 마지막으로 **저장**&#x200B;을 누릅니다.
...
1. **OK**&#x200B;을 클릭하여 사양을 저장합니다.

게시 환경의 프로필 및 로그인 및 로그아웃할 양식 제공에 대한 자세한 내용은 [Identity Management](/help/sites-administering/identity-management.md)을 참조하십시오.

## 영역 {#linking-to-the-realm}에 연결
...
## CUG {#configure-dispatcher-for-cugs}에 대한 발송자 구성
...
CUG에 대해 dispatcher.any 파일](https://helpx.adobe.com/experience-manager/dispatcher/using/dispatcher-configuration.html#enabling-secure-sessions-sessionmanagement)에서 [세션 관리를 구성합니다. CUG 페이지에 대한 액세스가 요청될 때 사용되는 인증 처리기에 따라 세션 관리를 구성하는 방법이 결정됩니다.
...
1. `/directory`;을 정의하여 [/sessionmanagement](https://helpx.adobe.com/experience-manager/dispatcher/using/dispatcher-configuration.html#enabling-secure-sessions-sessionmanagement) 구성예를 들면 다음과 같습니다.
...
1. [/allowAuthorized](https://helpx.adobe.com/experience-manager/dispatcher/using/dispatcher-configuration.html#caching-when-authentication-is-used)을 `0`(으)로 설정합니다.
