...
>
>* API의 사용을 설명하기 위한 독립형 AEM 기본 e커머스 예입니다. 기존 데이터 표시 및 마케팅 캠페인과 함께 제품 데이터, 장바구니 및 결제를 제어하는 데 사용할 수 있습니다. 이 경우 제품 데이터베이스가 AEM이 기본적으로 저장소에 저장됩니다(Adobe의 [JCR](https://docs.adobe.com/content/docs/en/spec/jcr/2.0/index.html) 구현).
>
>  
표준 AEM 설치에는 [일반 eCommerce 구현](/help/sites-administering/generic.md)의 기본이 포함되어 있습니다.
...
다음 두 카테고리를 구별할 수 있는 경우, 의미 있는 구조(`cq:Page` 노드 트리)로 URL을 명확하게 할 수 있으므로 클래식 AEM 컨텐츠 관리에 매우 가까울 수 있습니다.

* *구조적 *카테고리

   *product*;을(를) 정의하는 카테고리 트리예를 들면 다음과 같습니다.
...
* 유지 관리하고 AEM에서 사용할 수 있게 되었습니다.
   ...
   데이터 유형에 따라 [동기화된](#catalog-maintenance-data-synchronization)이거나 직접 액세스한 경우입니다.예를 들어 제품 가격과 같은 매우 휘발성 및 중요 데이터는 항상 최신 상태로 유지되도록 모든 페이지 요청의 경제 엔진에서 검색됩니다.

두 경우 모두 제품 데이터를 AEM에 입력/가져온 경우 **Products** 콘솔에서 볼 수 있습니다. 다음은 제품의 카드 및 목록 보기 정보입니다.
...
다중 언어 스토어의 경우 각 언어 트리에 대한 카탈로그를 개별적으로 가져오거나 [MSM](/help/sites-administering/msm.md)을 통해 복사할 수 있습니다.
...
* [eCommerce 엔진을 사용하는 경우 변경 내용이 동기화되어야 합니다.](#data-synchronization-with-an-ecommerce-engine-ongoing)
...
**Product** 구성 요소는 필요한 메타데이터(예: `cartPage` 및 `cartObject`에 대한 경로)를 전달하는 상위 페이지가 있는 페이지에 추가할 수 있습니다. 데모 사이트의 Geometrixx Outdoors에서 이 값은 `UserInfo.jsp`에서 제공합니다.

**제품** 구성 요소는 사용자의 개별 요구 사항에 따라 사용자 정의할 수도 있습니다.
...
>[!NOTE]
>
>AEM은 **바우처**&#x200B;라는 용어를 사용하며, 이것은 **쿠폰**&#x200B;이라는 용어와 비슷합니다.
...
* *product-X*&#x200B;을(를) 구입하는 고객은 *product-Y*(두 제품)에 할인 혜택을 제공합니다.
...
* [campaign](/help/sites-authoring/personalization.md)이 설정/해제 시간을 지정합니다.
...
* 판촉 행사가 경험에서 개최되면 대상 세그먼트에 자동으로 적용할 수 있습니다.
   ...
   가 경험에 있으므로 세그먼트( `ordervalueover100`)가 확인할 때마다 자동으로 발생합니다.
...
>[!NOTE]
>
>[hybris ](https://www.hybris.com/modules/promotion) 홍보 및  [hybris는 장바구니에 영향을 주고 가격과 관련된 모든 것을 ](https://www.hybris.com/en/modules/voucher) 보증합니다. 판촉 특정 마케팅 컨텐츠(예: 배너 등)는 하이브리스 판촉 행사의 일부가 아닙니다.
...
>
>그렇지 않으면 이러한 다른 AEM 인스턴스는 엔진에서 동일한 사용자에 대한 계정을 만들려고 합니다. 이러한 작업은 엔진에서 `DuplicateUidException`이(가) 오며 실패합니다.
...
**새 주소 추가...를 클릭할 수 있습니다.주소록에 새 주소를 추가하는** 양식을 열고 **주소 추가**&#x200B;를 클릭합니다.
...
원하는 주소를 기본값으로 선택할 수 있습니다. 이 정보는 주소가 아닌 구매자의 프로필에 유지됩니다. 프로필 속성 `address.default`이(가) 값에 대해 선택한 주소의 경로로 설정됩니다.
...
* `Allison Parker`(으)로 다시 로그인 - 해당 제품이 복원됨
...
>[!NOTE]
>
>전자 상거래 엔진의 `admin` 계정(예: hybris)과 충돌할 수 있으므로 `admin` 계정으로 장바구니 컨텐츠를 복원하는 것을 테스트하지 않는 것이 좋습니다.
...
주문은 화면에서 확인되며 주문 추적[으로 추적할 수 있습니다.](#order-tracking)
