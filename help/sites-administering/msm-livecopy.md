...
1. 롤아웃 작업을 즉시(**지금**) 또는 다른 날짜/시간(**나중에**)으로 실행해야 하는지 지정합니다.

   ![롤아웃 블루프린트](assets/rollout-blueprint.png)

롤루트는 비동기 작업으로 처리되고 [**비동기 작업 상태&#x200B;**대시보드에서](asynchronous-jobs.md#monitor-the-status-of-asynchronous-operations)확인할 수 있습니다. 전역 탐색**->**도구**->**작업******- SafetyOperations -> Job **
...
1. 롤아웃 세부 사항을 확인하라는 메시지가 표시됩니다.
   ...
   * **예약**:
      ...
      롤아웃 작업을 즉시(**지금**) 또는 이후 날짜/시간(**나중에**)으로 실행해야 하는지 지정합니다.
...
1. 상속을 되돌린 후 페이지를 동기화할지 여부를 표시한 다음 Live Copy **다시 시작** 대화 상자에서 **다시 시작** 작업을확인합니다.
