...
* 페이지 이벤트(예: 수정 또는 복제)를 구독했습니다. 다음 [알림 받은 편지함](/help/sites-classic-ui-authoring/author-env-inbox.md#subscribing-to-notifications) 섹션에서는 이러한 이벤트를 구독하는 방법에 대해 설명합니다.

* 포럼 이벤트를 구독했습니다.
* 워크플로우에서 단계를 수행해야 합니다. 다음 [참가자 단계](/help/sites-developing/workflows-step-ref.md#participant-step) 섹션에서는 워크플로우에서 이메일 알림을 트리거하는 방법을 설명합니다.

전제 조건:

* 사용자는 프로필에 유효한 이메일 주소가 정의되어 있어야 합니다.
* 다음 **일별 CQ 메일 서비스** 을(를) 올바르게 구성해야 합니다.
...
AEM에서 이메일을 보낼 수 있도록 **일별 CQ 메일 서비스** 을(를) 올바르게 구성해야 합니다. 웹 콘솔에서 구성을 볼 수 있습니다. AEM을 사용하여 작업할 때 이러한 서비스에 대한 구성 설정을 관리하는 방법에는 몇 가지가 있습니다. 다음을 참조하십시오. [OSGi 구성](/help/sites-deploying/configuring-osgi.md) 를 참조하십시오.

다음과 같은 제한이 적용됩니다.

* 다음 **SMTP 서버 포트** 은(는) 25 이상이어야 합니다.

* 다음 **SMTP 서버 호스트 이름** 비워 둘 수 없습니다.
* 다음 **&quot;보낸 사람&quot; 주소** 비워 둘 수 없습니다.

을(를) 사용하여 문제를 디버깅하는 데 도움이 됩니다 **일별 CQ 메일 서비스**, 다음과 같은 서비스 로그를 볼 수 있습니다.
...
페이지 또는 포럼 이벤트 알림을 구독하면 보낸 사람 이메일 주소가 로 설정됩니다. `no-reply@acme.com` 기본값입니다. 다음을 구성하여 이 값을 변경할 수 있습니다. **알림 이메일 채널** 웹 콘솔의 서비스입니다.

보낸 사람 이메일 주소를 구성하려면 `sling:OsgiConfig` 노드를 리포지토리에 추가합니다. 다음 절차에 따라 CRXDE Lite을 사용하여 노드를 직접 추가합니다.

1. CRXDE Lite에서 다음 폴더를 추가합니다. `config` 애플리케이션 폴더 아래에 있습니다.
...
1. 내 `jcr_root/apps/*app_name*/config folder`, (이)라는 이름의 파일 만들기 `com.day.cq.wcm.notification.email.impl.EmailChannel.xml`

1. 노드를 나타내는 다음 XML을 추가합니다.

   `<?xml version="1.0" encoding="UTF-8"?> <jcr:root xmlns:sling="https://sling.apache.org/jcr/sling/1.0" xmlns:jcr="https://www.jcp.org/jcr/1.0" jcr:primaryType="sling:OsgiConfig" email.from="name@server.com"/>`
1. 값 바꾸기 `email.from` 속성( `name@server.com`)을 클릭하여 제품에서 사용할 수 있습니다.

1. 파일을 저장합니다.

## 워크플로우 이메일 알림 서비스 구성 {#configuring-the-workflow-email-notification-service}

워크플로우 이메일 알림을 받을 때 보낸 사람 이메일 주소와 호스트 URL 접두어가 모두 기본값으로 설정됩니다. 다음을 구성하여 이러한 값을 변경할 수 있습니다. **일별 CQ 워크플로우 이메일 알림 서비스** 웹 콘솔에서 게시할 수 있습니다. 이 경우 저장소에서 변경 사항을 유지하는 것이 좋습니다.
...
기본 영어 템플릿( `en.txt`)는 다음과 같이 정의됩니다.
...
위치 &lt;text_x> 정적 텍스트와 동적 문자열 변수의 혼합일 수 있습니다. 페이지 알림을 위한 이메일 템플릿 내에서 다음 변수를 사용할 수 있습니다.
...
#### 포럼 알림을 위한 이메일 템플릿 맞춤화 {#customizing-email-templates-for-forum-notification}

포럼 알림을 위한 영어 이메일 템플릿을 사용자 정의하려면 다음을 수행하십시오.
...
위치 `<text_x>` 정적 텍스트와 동적 문자열 변수의 혼합일 수 있습니다.

포럼 알림을 위한 이메일 템플릿 내에서 다음 변수를 사용할 수 있습니다.

* `${time}`, 이벤트 날짜 및 시간입니다.

* `${forum.path}`: 포럼 페이지의 경로입니다.
...
>[!NOTE]
>
>위치 `<text_x>` 정적 텍스트와 동적 문자열 변수의 혼합일 수 있습니다. 의 각 행 `<text_x>` 항목은 백슬래시()로 끝나야 합니다. `\`), 마지막 인스턴스를 제외하고, 백슬래시가 없으면 `<text_x>` 문자열 변수입니다.
>
...
메서드 `${payload.path.open}` 은 작업 항목의 페이로드에 대한 경로를 표시합니다. 예를 들어 Sites의 페이지에 대해 다음을 수행합니다. `payload.path.open` 는 과 유사합니다. `/bin/wcmcommand?cmd=open&path=…`.; 이것은 서버 이름이 없기 때문에 템플릿에서 이 앞에 를 추가합니다. `${host.prefix}`.
...
1. CRXDE에서 파일을 추가합니다 `<language-code>.txt` 아래:
...
>[!NOTE]
>
>다음 `<language-code>` 이메일 템플릿의 파일 이름으로 사용되는 두 글자로 된 소문자 언어 코드는 AEM에서 인식해야 합니다. 언어 코드의 경우 AEM은 ISO-639-1을 사용합니다.

## AEM Assets 이메일 알림 구성 {#assetsconfig}

AEM Assets의 컬렉션이 공유되거나 공유되지 않으면 사용자는 AEM에서 이메일 알림을 받을 수 있습니다. 이메일 알림을 구성하려면 다음 단계를 수행합니다.

1. 위에서 설명한 대로 이메일 서비스를 구성합니다. [메일 서비스 구성](/help/sites-administering/notification.md#configuring-the-mail-service).
1. 관리자로 AEM에 로그인합니다. 클릭 **도구** >  **작업** >  **웹 콘솔** 웹 콘솔 구성을 엽니다.
1. 편집 **일별 CQ DAM 리소스 컬렉션 서블릿**. 선택 **이메일 보내기**. **저장**&#x200B;을 클릭합니다.
...
1. 다음에서 프로젝트를 만듭니다. `https://console.developers.google.com/projectcreate`
1. 프로젝트를 선택한 다음 로 이동합니다. **API 및 서비스** - **대시보드 - 자격 증명**
1. 요구 사항에 따라 OAuth 동의 화면 구성
1. 다음에 나오는 업데이트 화면에서 다음 두 범위를 추가합니다.
   * `https://mail.google.com/`
   * `https://www.googleapis.com//auth/gmail.send`
1. 범위를 추가했으면 다음으로 돌아갑니다. **자격 증명** 왼쪽 메뉴에서 **자격 증명 만들기** - **OAuth 클라이언트 ID** - **데스크탑 앱**
...
1. 다음과 같이 필요한 정보를 입력합니다.
   ...
   * 범위: `https://www.googleapis.com/auth/gmail.send` 및 `https://mail.google.com/`. 을 눌러 두 개 이상의 범위를 추가할 수 있습니다. **+** 구성된 각 범위의 오른쪽에 있는 단추입니다.
...
1. 브라우저에서 새 탭을 열고 다음으로 이동 `http://serveraddress:serverport/services/mailer/oauth2/authorize`. SMTP 공급자의 페이지로 리디렉션됩니다(이 경우 Gmail).
1. 로그인 및 필요한 권한 부여에 대한 동의
1. 동의를 하면 토큰이 저장소에 저장됩니다. 다음에서 액세스할 수 있습니다. `accessToken` 게시 인스턴스에서 이 URL에 직접 액세스: `http://serveraddress:serverport/crx/de/index.jsp#/conf/global/settings/mailer/oauth`
...
1. 아래의 앱에 대한 권한을 선택한 다음 **권한 추가**&#x200B;를 클릭합니다.
   ...
   * `openid`
   * `offline_access`
1. 다음으로 이동 **인증** - **플랫폼 추가** - **웹**, 및 **리디렉션 Url** 섹션에서 OAuth 코드를 리디렉션하기 위해 다음 URL을 추가한 다음 키를 누릅니다 **구성**:
...
1. 다음과 같이 필요한 정보를 입력합니다.
   ...
   * 인증 코드 리디렉션 Url: `http://localhost:4503/services/mailer/oauth2/token`
