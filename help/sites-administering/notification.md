...
* 페이지 이벤트(예: 수정 또는 복제)를 구독했습니다. [알림 받은 편지함](/help/sites-classic-ui-authoring/author-env-inbox.md#subscribing-to-notifications) 섹션에서는 이러한 이벤트에 가입하는 방법을 설명합니다.
...
AEM에서 이메일을 보내려면 **일 CQ 메일 서비스**&#x200B;를 올바르게 구성해야 합니다. 웹 콘솔에서 구성을 볼 수 있습니다. AEM을 사용하는 경우 이러한 서비스에 대한 구성 설정을 관리하는 방법에는 여러 가지가 있습니다.자세한 내용 및 권장 방법은 [OSGi](/help/sites-deploying/configuring-osgi.md) 구성을 참조하십시오.
...
**일 CQ 메일 서비스**&#x200B;와의 문제를 디버깅하는 데 도움이 되도록 서비스의 로그를 볼 수 있습니다.
...
페이지 또는 포럼 이벤트 알림에 가입하면 보낸 사람 이메일 주소는 기본적으로 `no-reply@acme.com`으로 설정됩니다. 웹 콘솔에서 **알림 이메일 채널** 서비스를 구성하여 이 값을 변경할 수 있습니다.

전자 메일 주소를 구성하려면 보관소에 `sling:OsgiConfig` 노드를 추가합니다. 다음 절차를 사용하여 CRXDE Lite을 사용하여 노드를 직접 추가합니다.

1. CRXDE Lite에서 응용 프로그램 폴더 아래에 `config`이라는 폴더를 추가합니다.
1. 구성 폴더에서 다음 이름의 노드를 추가합니다.

   `com.day.cq.wcm.notification.email.impl.EmailChannel` 유형  `sling:OsgiConfig`

1. `String` 속성을 `email.from`이라는 노드에 추가합니다. 값에 사용할 이메일 주소를 지정합니다.

1. **모두 저장**&#x200B;을 클릭합니다.

다음 절차를 사용하여 컨텐츠 패키지 소스 폴더의 노드를 정의합니다.

1. `jcr_root/apps/*app_name*/config folder`에서 `com.day.cq.wcm.notification.email.impl.EmailChannel.xml`라는 파일을 만듭니다.
...
워크플로우 이메일 알림을 받으면 보낸 사람 이메일 주소와 호스트 URL 접두사가 모두 기본값으로 설정됩니다. 웹 콘솔에서 **일 CQ Workflow 이메일 알림 서비스**&#x200B;를 구성하여 이 값을 변경할 수 있습니다. 이렇게 하면 저장소의 변경 사항을 유지하는 것이 좋습니다.
...
여기서 `<text_x>`은 정적 텍스트와 동적 문자열 변수의 혼합일 수 있습니다.
...
#### 워크플로우 알림에 대한 이메일 템플릿 사용자 지정 {#customizing-email-templates-for-workflow-notification}
...
>[!NOTE]
>
>여기서 `<text_x>`은 정적 텍스트와 동적 문자열 변수의 혼합일 수 있습니다. `<text_x>` 항목의 각 행은 마지막 인스턴스를 제외하고 백슬래시가 없으면 `<text_x>` 문자열 변수의 끝을 나타내는 백슬래시( `\`)로 끝나야 합니다.
>
...
`${payload.path.open}` 메서드는 작업 항목의 페이로드 경로를 표시합니다. 예를 들어 사이트의 경우 `payload.path.open`은 `/bin/wcmcommand?cmd=open&path=…`과 비슷합니다.;서버 이름이 없는 경우이므로 템플릿에 `${host.prefix}`이(가) 접두사로 붙습니다.
...
>[!NOTE]
>
>이메일 템플릿의 파일 이름으로 사용되는 `<language-code>`은 AEM에서 인식되는 두 글자 소문자 언어 코드여야 합니다. 언어 코드의 경우 AEM은 ISO-639-1을 사용합니다.
...
1. **일 CQ DAM 리소스 수집 서블릿**&#x200B;을 편집합니다. **이메일 보내기**&#x200B;를 선택합니다. **저장**&#x200B;을 클릭합니다.
