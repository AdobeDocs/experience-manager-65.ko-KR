...
찾기를 클릭하면 찾기 및 바꾸기 대화 상자가 투명해지고, 바꾸기를 클릭하면 불투명해집니다. 이로써 작성자는 대체될 텍스트를 검토할 수 있습니다. 사용자가 모두 바꾸기 를 클릭하면 대화 상자가 닫히고 교체된 항목 수가 표시됩니다.
...
| `fallbackBlockTag` | 문자열 | 에 포함되지 않은 블록 태그가 있는 모든 블록에 사용되는 블록 태그를 정의합니다. `allowBlockTags`. `p` 대부분의 경우 충분합니다. |
...
>[!NOTE]
>
>대화 상자에서 RTE를 사용할 때(클래식 UI) 리치 텍스트 편집에 최적화된 스타일 시트를 지정할 수 있습니다. 기술적인 제한 사항으로 인해 CSS 컨텍스트가 편집기에서 손실되므로 이 컨텍스트를 에뮬레이션하여 WYSIWYG 경험을 개선할 수 있습니다.
>
...
1. 노드 아래 `styles`, 새 노드 만들기(이라고도 함) `styles`) 사용 가능한 목록을 유지하려면 다음 작업을 수행하십시오.

   * **이름** `styles`
   * **유형** `cq:WidgetCollection`

1. 아래에 새 노드 만들기 `styles` 개별 스타일을 나타내는 노드:
...
1. 스타일 노드 아래에 새 노드를 만듭니다. 다음을 참조하십시오 [새 스타일 지정](#stylesindropdown).
   * 이름: `jpn-word-wrap`
   * 유형: `nt:unstructure`
...
1. 스타일시트를 만들고 해당 경로를 지정합니다. 다음을 참조하십시오 [스타일시트 위치 지정](#locationofstylesheet). 스타일시트에 다음 내용을 추가합니다. 원하는 대로 배경색을 변경합니다.
...
이후 (재)구성의 경우 형식을 더 추가하려면 지침의 해당 부분만 따르십시오.
...
1. 아래 `paraformat` 노드 형식 목록을 보관할 새 노드를 만듭니다.

   * **이름** `formats`
   * **유형** `cq:WidgetCollection`

1. 아래에 새 노드 만들기 `formats` 노드에는 개별 형식에 대한 세부 정보가 포함됩니다.
...
>[!CAUTION]
>
고유한 특수 문자를 추가하면 기본 선택이 무시됩니다. 필요한 경우 자체 선택 항목에서 이러한 문자를 (재)정의합니다.
...
1. 아래 `chars` 개별 문자 정의를 보유할 새 노드를 추가합니다.
...
1. 아래 `chars` 문자 범위의 정의를 보유할 새 노드를 추가합니다.
...
1. 아래 `table` 노드 다음 새 노드를 만듭니다(필수 사항).
...
1. 새 노드 만들기(아래) `tableStyles` 또는 `cellStyles` 개별 스타일을 나타내는 적절한 노드):
...
1. 다음에서 `../items/text` 구성 요소에 대한 대화 상자 정의에서 새 속성을 만듭니다.
...
1. CRXDE Lite을 사용하여 프로젝트의 텍스트 구성 요소를 찾습니다.
1. 와 동일한 수준에 새 노드 만들기 `<rtePlugins-node>`즉, 의 상위 노드 아래에 노드를 만듭니다. `<rtePlugins-node>`:
...
1. 아래 `htmlRules`를 클릭하여 새 노드를 만듭니다.
...
1. 아래 `links` 노드는 필요에 따라 속성을 정의합니다.
   ...
   * 링크의 대상 속성을 처리하는 방법의 정의입니다. 새 노드 만들기:
