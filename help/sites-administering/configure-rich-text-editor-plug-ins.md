...
| 유형 | 문자열 | 문자열[](다중 문자열;[유형]을 [문자열]으로 설정하고 [다중 in CRXDE Lite]을 클릭합니다. | 문자열 |
...
* 클립보드 내용을 일반 텍스트로 붙여넣습니다. 복사한 콘텐트에서 스타일 및 서식 요소의 모든 요소를 제거하고 AEM 구성 요소에 삽입합니다. 아래의 `plaintext`을(를) 사용하여 구성되었습니다.
...
1. `edit` 노드에서 다음 세부 정보를 사용하여 속성을 만듭니다.
   ...
   * **이름** `defaultPasteMode`
   * **유형** `String`
   * **값** 필요한 붙여넣기 모드  `browser`중  `plaintext`하나  `wordhtml`입니다.
...
<table>
 <tbody>
  ...
  <tr>
   ...
   <td><p>표를 붙여넣을 때의 동작을 정의합니다.<br /> </p> <p>이 노드에는 테이블 붙여넣기가 허용되는지 여부를 정의하려면 속성 <code>allow</code>(유형 <code>Boolean</code>)이 있어야 합니다.</p> <p><code>allow</code>이(가) <code>false</code>으로 설정된 경우 <code>ignoreMode</code> 속성(유형<code> String</code>)을 지정하여 붙여넣기가 수행된 테이블 컨텐츠가 처리되는 방식을 정의해야 합니다. <code>ignoreMode</code>에 대한 유효한 값은 다음과 같습니다.</p>
    <ul>
  ...
  <tr>
   <td>list</td>
   <td>nt:unstructured</td>
   <td><p>목록을 붙여넣을 때의 동작을 정의합니다.<br /> </p> <p>목록 붙여넣기가 허용되는지 여부를 정의하려면 <code>allow</code> 속성(<code>Boolean</code> 유형)이 있어야 합니다.</p> <p><code>allow</code>이(가) <code>false</code>으로 설정된 경우 <code>ignoreMode</code> 속성(<code>String</code> 유형)을 지정하여 붙여넣은 목록 컨텐트를 처리하는 방법을 정의해야 합니다. <code>ignoreMode</code>에 대한 유효한 값은 다음과 같습니다.</p>
    <ul>
...
>
>리치 텍스트 편집기는 ID가 `CQrte`인 컨테이너 DOM 요소를 사용하므로 보고 편집하기 위해 다른 스타일을 제공하는 데 사용할 수 있습니다.
>
>
```
>#CQ td {
...
AEM을 사용하여 일본어 언어 컨텐츠를 작성하는 작성자는 문자 스타일을 적용하여 나누기가 필요하지 않은 줄 바꿈을 방지할 수 있습니다. 이를 통해 작성자는 원하는 위치에서 문장이 지워질 수 있습니다. 이 기능의 스타일은 CSS 스타일 시트에서 미리 정의된 CSS 클래스를 기반으로 합니다.
...
1. Create a new node under the styles node. [새 스타일](#stylesindropdown) 지정을 참조하십시오.
   * 이름: `jpn-word-wrap`
   * 유형:&#39;nt:구조 해제
...
1. 스타일시트를 만들고 해당 경로를 지정합니다. See [specify location of stylesheet](#locationofstylesheet). Add the following contents to the stylesheet. 원하는 대로 배경색을 변경합니다.
...
>[!CAUTION]
RTE의 단락 형식을 구성할 때 단락 태그 &lt;p>를 서식 옵션으로 제거하지 마십시오. `<p>` 태그가 제거된 경우, 내용 작성자는 추가 형식이 구성된 경우에도 **단락 형식** 옵션을 선택할 수 없습니다.
...
1. Create a new node under the `formats` node, this hold details for an individual format:
...
표준 AEM 설치에서 `misctools` 플러그인이 특수 문자(`specialchars`)에 대해 활성화되면 즉시 기본 선택 항목을 사용할 수 있습니다.예를 들어, 저작권 및 상표 심볼입니다.
...
1. `chars` 아래에서 개별 문자 정의를 저장할 새 노드를 추가합니다.
...
1. 구성 요소 내에서 노드 `<rtePlugins-node>/table`으로 이동합니다. 노드가 없으면 노드를 만듭니다. 자세한 내용은 [플러그인](#activateplugin) 활성화를 참조하십시오.
...
1. `table` 노드에서 다음과 같은 새 노드를 만듭니다(필요한 경우).

   * 전체 테이블에 대한 스타일을 정의하려면(**표 속성** 아래에서 사용 가능):
...
>[!NOTE]
설치되지 않은 언어에 대해 검사가 시도된 경우 `Spell checking failed` 메시지가 표시됩니다. 표준 사전은 해당 읽어보기 파일과 함께 `/libs/cq/spellchecker/dictionaries`에 있습니다. 파일을 수정하지 마십시오.
...
1. CRXDE Lite을 사용하여 프로젝트의 텍스트 구성 요소를 찾습니다.
1. Create a new node at the same level as `<rtePlugins-node>`, 즉 `<rtePlugins-node>`의 부모 노드 아래에 노드를 만듭니다.
...
1. `links` 노드에서 필요에 따라 속성을 정의합니다.
   ...
   * 외부 링크에 대한 CSS 스타일
      ...
      * **유형** `String`
      * **** CSS 클래스의 이름을 지정합니다(이전 &#39;.&#39; 없이).;예를 들어 `.cssClass` 대신 `cssClass`가 사용됩니다.
   * 유효한 **프로토콜**&#x200B;의 배열. 지원되는 프로토콜은 `http://`, `https://`, `file://` 및 `mailto:`입니다.
