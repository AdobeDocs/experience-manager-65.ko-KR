...
|---|---|---|---|
| 이름 | 기능 | 기능 | 기능 |
| 유형 | 문자열 | 문자열[] (다중 문자열. [유형]을 [문자열]로 설정하고 [CRXDE Lite]에서 [다중]을 클릭합니다.) | 문자열 |
...
* **MS® Word 모드**: MS® Word에서 복사할 때 서식을 사용하여 표를 포함한 텍스트를 붙여넣습니다. 웹 페이지나 MS® 같은 다른 소스에서 텍스트를 복사하여 붙여넣을 수 없으며 부분 서식만 유지합니다.
...
* 클립보드 내용을 일반 텍스트로 붙여넣습니다. AEM 구성 요소에 삽입하기 전에 복사한 콘텐츠에서 스타일 및 서식의 모든 요소를 제거합니다. 을 사용하여 구성됨 `plaintext` 아래요.

* MS® Word에서 복사할 때 서식을 사용하여 표를 포함한 텍스트를 붙여넣습니다. 웹 페이지나 MS® 같은 다른 소스에서 텍스트를 복사하여 붙여넣을 수 없으며 부분 서식만 유지합니다. 을 사용하여 구성됨 `wordhtml` 아래요.

1. 구성 요소에서 다음 위치로 이동합니다. `<rtePlugins-node>/edit` 노드. 존재하지 않는 경우 노드를 만듭니다. 자세한 내용은 [플러그인 활성화](#activateplugin).
1. 다음에서 `edit` 노드에서 다음 세부 정보를 사용하여 속성을 만듭니다.
...
The paste-as-Microsoft-Word (`paste-wordhtml`) Microsoft® Word와 같은 다른 프로그램에서 AEM에 붙여넣을 때 허용되는 스타일을 명시적으로 정의할 수 있도록 모드를 추가로 구성할 수 있습니다.
...
다른 프로그램에서 AEM에 텍스트를 붙여넣을 때 허용되는 형식을 구성하려면 다음과 같이 하십시오.

1. 구성 요소에서 노드로 이동합니다 `<rtePlugins-node>/edit`. 존재하지 않는 경우 노드를 만듭니다. 자세한 내용은 [플러그인 활성화](#activateplugin).
1. 아래에 노드 만들기 `edit` HTML 붙여넣기 규칙을 유지할 수 있는 노드:

   * **이름** `htmlPasteRules`
   * **유형** `nt:unstructured`

1. 아래에 노드 만들기 `htmlPasteRules`허용되는 기본 형식에 대한 세부 정보를 보유할 수 있습니다.
...
1. 다른 형식은 다른 속성이나 노드를 사용하여 정의할 수도 있습니다. `htmlPasteRules` 노드. 모든 변경 사항을 저장합니다.

다음 속성을 사용할 수 있습니다. `htmlPasteRules`.
...
| `fallbackBlockTag` | 문자열 | 에 포함되지 않은 블록 태그가 있는 모든 블록에 사용되는 블록 태그를 정의합니다. `allowBlockTags`. `p` 보통 충분합니다. |
| 표 | nt:unstructured | 표를 붙여넣을 때의 동작을 정의합니다. 이 노드에는 속성이 있어야 합니다. `allow` (부울 유형) 테이블 붙여넣기를 허용할지 여부를 정의합니다. 허용이 로 설정된 경우 `false`, 속성을 지정해야 합니다 `ignoreMode` (유형 문자열) - 붙여넣은 테이블 콘텐츠를 처리하는 방법을 정의합니다. 유효한 값 `ignoreMode` 은(는) <ul> <li>`remove`: 테이블 컨텐츠를 제거합니다.</li> <li>`paragraph`: 테이블 셀을 단락으로 바꿉니다.</li> </ul> |
| list | nt:unstructured | 목록을 붙여넣을 때의 동작을 정의합니다. 속성이 있어야 합니다. `allow` (부울 유형) - 목록을 붙여넣을 수 있는지 여부를 정의합니다. If `allow` 이(가) (으)로 설정됨 `false`, 속성을 지정해야 합니다 `ignoreMode` (유형 문자열) - 붙여넣은 목록 콘텐츠를 처리하는 방법을 정의합니다. 유효한 값 `ignoreMode` 은(는) <ul><li> `remove`: 목록 컨텐츠를 제거합니다.</li> <li>`paragraph`: 목록 항목을 단락으로 바꿉니다.</li> </ul> |

유효한 예 `htmlPasteRules` 구조는 아래에 있습니다.
...
작성자는 스타일을 적용하여 텍스트 부분의 모양을 변경할 수 있습니다. 스타일은 CSS 스타일 시트에서 사전 정의한 CSS 클래스를 기반으로 합니다. 양식화된 내용은에 포함되어 있습니다. `span` 를 사용하는 태그 `class` css 클래스를 참조하는 속성입니다. 예: `<span class=monospaced>Monospaced Text Here</span>`
...
>[!NOTE]
>
>다음에 대한 스타일 정의 가능 [표 또는 표 셀](/help/sites-administering/configure-rich-text-editor-plug-ins.md#tablestyles). 이러한 구성에는 별도의 절차가 필요합니다.

### 스타일 드롭다운 선택기 목록 활성화 {#styleselectorlist}

이 작업은 스타일 플러그인을 활성화하여 수행합니다.

1. 구성 요소에서 노드로 이동합니다 `<rtePlugins-node>/styles`. 존재하지 않는 경우 노드를 만듭니다. 자세한 내용은 [플러그인 활성화](#activateplugin).
...
>
>리치 텍스트 편집기에서는 ID가 인 컨테이너 DOM 요소를 사용합니다. `CQrte` 보기 및 편집을 위해 다양한 스타일을 제공하는 데 사용할 수 있는 색상:
>
>`#CQ td {`
...
1. 아래에 노드 만들기 `styles` 개별 스타일을 표현할 수 있는 노드:

   * **이름**&#x200B;를 사용하여 이름을 지정할 수 있지만 스타일에 적합해야 합니다
   * **유형** `nt:unstructured`

1. 속성 추가 `cssName` css 클래스를 참조할 수 있도록 이 노드에 대해 다음을 수행합니다.
...
1. 스타일 노드 아래에 노드를 만듭니다. 다음을 참조하십시오 [새 스타일 지정](#stylesindropdown).
   * 이름: `jpn-word-wrap`
   * 유형: `nt:unstructure`

1. 속성 추가 `cssName` css 클래스를 참조할 수 있도록 노드에 대해 설명합니다. 이 클래스 이름은 일본어 자동 줄바꿈 기능에 대해 예약된 이름입니다.
   * 이름: `cssName`
   * 유형: `String`
...
1. 스타일 시트를 만들고 해당 경로를 지정합니다. 다음을 참조하십시오 [스타일 시트의 위치 지정](#locationofstylesheet). 스타일시트에 다음 내용을 추가합니다. 원하는 대로 배경색을 변경합니다.
   ...
   ![작성자가 일본어 자동 줄 바꿈 기능을 사용할 수 있도록 하는 스타일시트](assets/rte_jpwordwrap_stylesheet.jpg)
...
1. 구성 요소에서 노드로 이동합니다 `<rtePlugins-node>/paraformat`. 존재하지 않는 경우 노드를 만듭니다. 자세한 내용은 [플러그인 활성화](#activateplugin).
...
>[!CAUTION]
>
RTE의 단락 형식을 구성할 때 단락 태그를 제거하지 마십시오 &lt;p> 서식 지정 옵션으로 사용됩니다. 다음과 같은 경우 `<p>` 태그가 제거되면 콘텐츠 작성자는 **단락 형식** 추가 형식이 구성되어 있는 경우에도 옵션을 사용할 수 있습니다.
...
1. 동일한 노드에 설명 텍스트가 드롭다운 목록에 표시되도록 다른 속성을 추가합니다.
   ...
   * **이름** `description`
   * **유형** `String`
   * **값** 이 형식의 설명 텍스트입니다(예: 단락, 제목 1, 제목 2). 이 텍스트는 서식 선택 목록에 표시됩니다.
...
표준 AEM 설치에서 `misctools` 플러그인이 특수 문자(`specialchars`) 저작권 및 상표 기호와 같은 기본 선택은 즉시 사용할 수 있습니다.
...
1. 구성 요소에서 노드로 이동합니다 `<rtePlugins-node>/misctools`. 존재하지 않는 경우 노드를 만듭니다. 자세한 내용은 [플러그인 활성화](#activateplugin).
...
1. 아래 `misctools`를 클릭하고 특수 문자 구성을 보관할 노드를 만듭니다.

   * **이름** `specialCharsConfig`
   * **유형** `nt:unstructured`

1. 아래 `specialCharsConfig`를 클릭하고 문자 목록을 보관할 다른 노드를 만듭니다.
...
1. 구성 요소 내에서 노드로 이동합니다 `<rtePlugins-node>/table`. 존재하지 않는 경우 노드를 만듭니다. 자세한 내용은 [플러그인 활성화](#activateplugin).
1. 만들기 `features` 다음에 대한 속성 `table` 노드:
   ...
   * **유형** `String[]`
   >
   * **값** 필요에 따라 다음 중 하나 또는 둘 다:
   * `table` 스타일을 포함하여 표 속성을 편집할 수 있도록 허용합니다.
   * `cellprops` 스타일을 포함한 셀 속성을 편집할 수 있도록 허용합니다.

1. CSS 스타일 시트를 참조할 수 있도록 해당 위치를 정의합니다. 다음을 참조하십시오 [스타일 시트의 위치 지정](#locationofstylesheet) 이는 을 정의할 때와 동일하므로 [텍스트 스타일](#textstyles). 다른 스타일을 정의한 경우 위치를 정의할 수 있습니다.
1. 아래 `table` 노드를 생성하고 필요에 따라 다음 새 노드를 생성합니다.

   * 전체 표의 스타일을 정의하려면 **표 속성**):
...
1. 노드 만들기 (아래) `tableStyles` 또는 `cellStyles` 개별 스타일을 표현할 수 있도록 노드가 적절하게 표시됨):
...
맞춤법 검사 플러그인이 활성화되면 RTE는 각 해당 언어에 대한 사전을 사용합니다. 그런 다음 하위 트리의 언어 속성을 사용하거나 URL에서 언어를 추출하여 웹 사이트의 언어에 따라 선택합니다. 예를 들어 `/en/` 분기는 영어로 확인되며, `/de/` 독일어로 분기.

>[!NOTE]
>
메시지 `Spell checking failed` 설치되지 않은 언어에 대해 검사를 시도하면 표시됩니다. 표준 사전은 다음 위치에 있습니다. `/libs/cq/spellchecker/dictionaries`적절한 추가 정보 파일과 함께 제공됩니다. 파일을 수정하지 마십시오.

표준 AEM 설치에는 미국 영어 사전(`en_us`) 및 영국 영어 (`en_gb`). 사전을 더 추가하려면 다음 단계를 수행합니다.
...
1. 다음 중 하나를 수행하여 선택한 언어의 사전을 찾습니다.
   ...
   * 다음 위치에서 v2.x 사전 파일 검색: [https://wiki.openoffice.org/wiki/User:Khirano/Dictionaries](https://wiki.openoffice.org/wiki/User:Khirano/Dictionaries).
...
1. 를 찾습니다. `.aff` 및 `.dic` 파일. 파일 이름을 소문자로 유지합니다. 예를 들어, `de_de.aff` 및 `de_de.dic`.
1. 을(를) 로드합니다 `.aff` 및 `.dic` 저장소의 파일 `/apps/cq/spellchecker/dictionaries`.

>[!NOTE]
>
RTE 맞춤법 검사기는 온디맨드로 사용할 수 있습니다. 텍스트를 입력할 때 자동으로 실행되지 않습니다. 맞춤법 검사기를 실행하려면 [!UICONTROL 맞춤법 검사기] 을 클릭합니다. RTE는 단어의 맞춤법을 검사하고 맞춤법이 틀린 단어를 강조 표시합니다.
>
...
1. 다음에서 `undo` 노드를 생성하고 속성을 생성합니다.
...
1. 구성 요소에서 노드로 이동합니다 `<rtePlugins-node>/keys`. 존재하지 않는 경우 노드를 만듭니다. 자세한 내용은 [플러그인 활성화](#activateplugin).
1. 다음에서 `keys` 노드를 생성하고 속성을 생성합니다.

   * **이름** `tabSize`
   * **유형** `String`
   * **값** 표 작성기에 사용할 공백 문자 수
...
1. 다음에서 `lists` 노드, 만들기 `indentSize` 매개 변수:
...
1. 다음에서 `../items/text` 구성 요소에 대한 대화 상자 정의에서 노드를 사용하여 속성을 만듭니다.
...
1. 아래 `links` 노드에서 필요에 따라 속성을 정의합니다.
   ...
   * 유효한 배열 **프로토콜**. 지원되는 프로토콜은 다음과 같습니다 `http://`, `https://`, `file://`, 및 `mailto:`.
