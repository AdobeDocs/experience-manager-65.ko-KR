...
# Adobe Target과 통합하기 위한 전제 조건{#prerequisites-for-integrating-with-adobe-target}

AEM 및 Adobe Target](/help/sites-administering/target.md)의 [통합의 일부로, 게시 노드에서 Adobe Target에 등록하고, 복제 에이전트를 구성하고, 보안 활동 설정을 설정해야 합니다.
...
작성자 인스턴스에서 테스트 및 Target [복제 에이전트](/help/sites-deploying/replication.md)를 사용하도록 설정해야 합니다. AEM 설치에 [nosampleccontent](/help/sites-deploying/configure-runmodes.md#using-samplecontent-and-nosamplecontent) 실행 모드를 사용한 경우 이 복제 에이전트는 기본적으로 활성화되지 않습니다. 프로덕션 환경 보안에 대한 자세한 내용은 [보안 검사 목록](/help/sites-administering/security-checklist.md)을 참조하십시오.

1. AEM 홈 페이지에서 **도구** > **배포** > **복제**&#x200B;를 클릭하거나 탭합니다.
1. **작성자 에이전트**&#x200B;를 클릭하거나 탭합니다.
1. **테스트 및 Target(테스트 및 대상)** 복제 에이전트를 클릭하거나 탭한 다음 **편집**&#x200B;을 클릭하거나 탭합니다.
1. 활성화됨 옵션을 선택한 다음 **OK**&#x200B;을 클릭하거나 탭합니다.

   >[!NOTE]
   >
   >테스트 및 Target 복제 에이전트를 구성할 때 **전송** 탭에서 URI는 기본적으로 **tnt:///**&#x200B;으로 설정됩니다. 이 URI를 **https://admin.testandtarget.omniture.com**&#x200B;으로 바꾸지 마십시오.
   >
   >**tnt:///**&#x200B;으로 연결을 테스트하려고 하면 오류가 발생합니다. 이 URI는 내부용으로만 사용되며 **Test Connection**&#x200B;과 함께 사용할 수 없습니다.
...
**cq:ActivitySettings** 노드는 jcr:content node;**예: `/content/campaign/we-retail/master/myactivity/jcr:content/cq:ActivitySettings`의 활동 아래의 `/content/campaigns/*nameofbrand*`**&#x200B;아래의 CRXDE lite에서 사용할 수 있습니다. 이 노드는 구성 요소를 타깃팅한 후에만 만들어집니다.
...
이러한 설정을 통해 일반 사용자가 노드 속성에 액세스할 수 없도록 합니다. 작성자와 게시 시 동일한 ACL을 사용합니다. 자세한 내용은 [사용자 관리 및 보안](/help/sites-administering/security.md)을 참조하십시오.

## AEM Link Externalizer {#configuring-the-aem-link-externalizer} 구성

Adobe Target에서 활동을 편집할 때 AEM 작성자 노드에서 URL을 변경하지 않는 한 URL은 **localhost**&#x200B;을 가리킵니다. 내보낸 컨텐츠가 특정 *publish* 도메인을 가리키도록 하려면 AEM 링크 외부 도우미를 구성할 수 있습니다.
...
1. **Day CQ Link Externalizer**&#x200B;를 찾아 작성자 노드의 도메인을 입력합니다.
