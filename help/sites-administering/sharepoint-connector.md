...
커넥터는 여러 작업 영역에 대해 구성할 수도 있습니다. 이 경우 각 작업 영역은 작업 영역을 통해 노출되는 각 SharePoint 서버의 URL과 연결됩니다. 작업 영역을 추가하려면 작업 영역 매개 변수에 작업 영역 정의를 추가합니다. 작업 영역 정의 형식은 다음과 같습니다.`<name>`= `<url>` 여기서`<name>` 는 JCR 작업 영역의 이름이며`<url>` 해당 작업 영역에 대한 SharePoint 서버의 URL입니다.
