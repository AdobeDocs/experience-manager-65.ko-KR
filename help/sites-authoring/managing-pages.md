...
* **[이름](#name)**:
   ...
   * URI를 생성하는 데 사용됩니다.
   * 이 필드에 대한 사용자 입력은 선택 사항입니다. 지정하지 않으면 제목에서 이름이 파생됩니다. 자세한 내용은 [페이지 이름 제한 및 우수 사례](/help/sites-authoring/managing-pages.md#page-name-restrictions-and-best-practices) 섹션을 참조하십시오.
...
새 페이지를 만들 때 페이지 **이름**&#x200B;을 제공하면 AEM이 AEM 및 JCR에서 지정한 [규칙에 따라 이름을 확인](/help/sites-developing/naming-conventions.md)합니다. **이름** 필드에 잘못된 문자를 제출할 수 없습니다. AEM에서 잘못된 문자를 감지하면 필드에 설명 메시지가 강조 표시됩니다.
...
>[!NOTE]
>
>페이지는 페이지의 기반이 되는 템플릿이 허용되는 위치로만 이동할 수 있습니다. 자세한 내용은 [템플릿 가용성](/help/sites-developing/templates.md#template-availability)을 참조하십시오.
...
1. 다음에서 **대상 선택** 마법사의 단계에서는 다음 중 하나를 수행할 수 있습니다.
   ...
   * 사용 [열 보기](/help/sites-authoring/basic-handling.md#column-view) 페이지의 새 위치로 이동하려면 다음과 같이 하십시오.
...
콘솔에서 또는 개별 페이지를 편집할 때 [페이지 잠금/잠금 해제](/help/sites-authoring/editing-content.md#locking-a-page)할 수 있습니다. 페이지가 잠겼는지에 대한 정보도 두 위치에 표시됩니다.
