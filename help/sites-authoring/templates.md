...
* 정적 템플릿
   ...
   * 개발자가 [제공](/help/sites-developing/page-templates-static.md)하므로 작성자가 만들거나 편집할 수 없습니다.
   * 를 복사하여 새 페이지를 만들지만, 템플릿 이름이 정보를 위해 등록되더라도 이 뒤에 동적 연결이 없습니다.
   * 디자인 속성을 유지하려면 [디자인 모드](/help/sites-authoring/default-components-designmode.md)를 사용하십시오.
   * 정적 템플릿 편집은 개발자의 단독 작업이므로 자세한 내용은 개발자 문서 [페이지 템플릿 - 정적](/help/sites-developing/page-templates-static.md)을 참조하십시오.
...
템플릿을 사용하여 [페이지 만들기](/help/sites-authoring/managing-pages.md#creating-a-new-page)를 수행하는 경우 정적 템플릿과 편집 가능한 템플릿 사이에는 차이점과 표시가 없습니다. 페이지 작성자의 경우 프로세스가 투명합니다.
...
* **구성 요소 잠금 해제/잠금**
  ...
  여기에는 **초기 콘텐츠** 모드 또는 결과 페이지에서 구성 요소를 추가할 수 있도록 컨테이너 구성 요소의 잠금을 해제하는 작업이 포함됩니다. 컨테이너를 잠금 해제하기 전에 구성 요소/컨텐츠를 이미 컨테이너에 추가한 경우 **구조** 모드에서는 더 이상 표시되지 않지만 **초기 컨텐츠** 모드에서는 표시됩니다. **구조 모드**&#x200B;에서는 컨테이너 구성 요소 자체만 **허용된 구성 요소** 목록과 함께 표시됩니다.
