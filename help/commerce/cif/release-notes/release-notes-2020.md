...
* 템플릿 상속이 특정 카테고리 페이지에 추가되었습니다. 이 기능을 사용하면 모든 하위 카테고리가 특정 상위 카테고리에 대해 작성된 템플릿을 상속할 수 있으므로 비즈니스 사용자 효율성이 향상됩니다.
...
* Adobe 클라이언트 데이터 레이어에 수분을 제공하도록 확장되는 탐색 구성 요소입니다.
...
* 비즈니스 사용자가 AEM 컨텐츠 페이지에 이 구성 요소를 드래그 앤 드롭하여 커머스 데이터로 컨텐츠 페이지를 풍성하게 만들 수 있도록 해주는 새로운 카테고리 슬라이드 구성 요소가 추가되었습니다.

* 상거래 데이터를 전송하여 Adobe 클라이언트 데이터 레이어에 수분을 제공하도록 확장되는 CIF 핵심 구성 요소입니다. Adobe 클라이언트 데이터 레이어는 데이터를 수집하고 데이터를 디지털 분석 및 보고 서버에 전달하는 표준화된 방법입니다. 자세한 내용은 [Adobe 클라이언트 데이터 레이어](https://github.com/adobe/adobe-client-data-layer/wiki)를 참조하십시오.

* Magento 관리 UI 내에서 구성된 SEO 메타데이터(예: 제목, 메타 설명, 메타 키워드)를 자동으로 채우도록 확장된 제품 세부 사항 및 제품 목록 페이지
...
* Magento 2.4.0 스키마에 대한 쿼리를 지원합니다.

* 고객이 개인 정보를 업데이트할 수 있도록 계정 정보 기능이 추가되었습니다.

* 개발자가 &quot;추가 로드&quot; 단추를 페이지 매김 스타일로 표시하도록 이러한 구성 요소를 구성할 수 있도록 제품 목록 및 검색 결과 페이지에 대해 구현된 페이지 매김 스타일입니다.

* 고객이 계정 암호를 업데이트/재설정할 수 있도록 암호 재설정 페이지를 구현했습니다.

* 번들 제품 유형에 대한 지원을 사용할 수 있습니다.

* 개발자는 SEO 우수 사례를 따르도록 제품 슬라이드, 관련 제품 및 주요 카테고리 목록 구성 요소에 대한 HTML 태그를 구성할 수 있습니다.
...
* 컨텐츠 및 상거래 페이지 지원에 추가된 탐색 표시 구성 요소입니다.

* 랜딩 페이지 및 경험 조각에 대한 CIF 속성을 노출하기 위해 페이지 속성에 상거래 탭이 추가되었습니다.

* 자리 표시자 텍스트 표시 지원 옵션을 지원하도록 검색 막대 구성 요소 개선

* 간편한 사용자 지정을 지원하기 위해 제품 및 제품 티저 구성 요소에 유연성을 추가했습니다.
...
* CIF Venia Reference Site는 CIF Tranype 보고서에서 추출되었으며 현재 독립형 GitHub 저장소입니다.

* CIF Tranype은 AEM Project Lianype과 통합되었습니다. 새 프로젝트의 경우 시작 지점으로 [AEM 프로젝트 원형](https://github.com/adobe/aem-project-archetype)을 사용합니다.

* 로그인한 사용자가 자신의 주소를 관리할 수 있도록 주소록 관리를 추가했습니다.
...
* aem-core-cif-response-components 패키지를 간소화했습니다.
...
* 카테고리 쿼리를 제품 검색 쿼리와 결합하여 패싯된 검색을 최적화할 수 있습니다.

* 페이지 미리 보기의 카테고리/제품 선택기에서 cq:catalogPath를 확인합니다.
...
* Magento 2.3.5 스키마에 대한 쿼리를 지원합니다.

* 패싯된 검색 지원이 검색 페이지 및 제품 목록 페이지에 추가되어 구매자가 제품 패싯을 기반으로 검색 결과를 필터링할 수 있습니다.

* SEO 용도로 PDP/PLP URL을 사용자 지정하는 데 추가된 새로운 OSGi 서비스입니다. 자세한 내용은 이 [설명서](https://github.com/adobe/aem-core-cif-components/wiki/configuration)를 참조하십시오.
...
* 선택적 제목을 지원하도록 관련 제품, 제품 Carousel 및 주요 카테고리 구성 요소가 개선되었습니다.
...
* Magento 2.3.4 스키마에 대한 쿼리를 지원합니다.

* 범주 선택기에 검색 지원을 추가했습니다.
...
* 제품 콘솔 및 제품 선택기에서 제품 검색이 개선되었습니다.
...
* 고객이 상거래 프로젝트에서 XF를 만들 수 있도록 하는 XF(경험 조각) 구성 요소가 추가되었습니다.
...
* 특별 카테고리 목록 구성 요소에서 이미지를 변경/선택하는 옵션.
