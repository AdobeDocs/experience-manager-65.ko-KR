...
* 템플릿 상속이 특정 범주 페이지에 추가되었습니다. 이 기능을 사용하면 모든 하위 범주가 특정 상위 범주에 대해 만들어진 템플릿을 상속할 수 있으므로 비즈니스 사용자 효율성이 향상됩니다.
...
* 비즈니스 사용자가 AEM 콘텐츠 페이지에서 이 구성 요소를 드래그하여 놓아 상거래 데이터로 콘텐츠 페이지를 보강할 수 있도록 새 카테고리 캐러셀 구성 요소를 추가했습니다.

* 상거래 데이터를 전송하여 Adobe 클라이언트 데이터 레이어를 하이드레이션하도록 확장된 CIF 핵심 구성 요소입니다. Adobe 클라이언트 데이터 레이어 는 데이터를 수집하고 데이터를 디지털 분석 및 보고 서버에 전달하는 표준화된 방법입니다. 자세한 내용은 다음을 참조하십시오. [Adobe 클라이언트 데이터 레이어](https://github.com/adobe/adobe-client-data-layer/wiki).
...
* 제품 목록 및 검색 결과 페이지에 대해 소극적 로드 페이지 매김 스타일을 구현하여 개발자가 이러한 구성 요소에 &quot;추가 로드&quot; 단추를 페이지 매김 스타일로 표시하도록 구성할 수 있습니다.

* 구매자가 계정 암호를 업데이트/재설정할 수 있도록 암호 재설정 페이지가 구현되었습니다.
...
* 자리 표시자 텍스트를 표시하는 옵션을 지원하도록 검색 막대 구성 요소 개선
...
* 카테고리 쿼리를 제품 검색 쿼리와 결합하여 최적화된 면처리 검색입니다.
...
* SEO용으로 PDP/PLP URL을 사용자 지정하는 새 OSGi 서비스가 추가되었습니다. 자세한 내용은 다음을 참조하십시오. [설명서](https://github.com/adobe/aem-core-cif-components/wiki/configuration).
