---
title: 개념
description: AEM을 사용하는 eCommerce에 대한 일반 개념.
...
>
>이는 현재 데모 목적이거나 요구 사항에 따라 사용자 정의 구현을 위한 기본 토대입니다.
...
통합 레이어를 사용하기 위해 다양한 AEM 구성 요소가 제공됩니다. 현재 여기에는 다음이 포함됩니다.
...
>이는 현재 데모 목적이거나 요구 사항에 따라 사용자 정의 구현을 위한 기본 토대입니다.
>
>AEM eCommerce는 JCR을 기반으로 하는 일반 개발을 사용하여 AEM 내에서 구현됩니다.
>
>* API 사용을 보여주는 독립 실행형 AEM 네이티브 전자 상거래 예입니다. 기존 데이터 표시 및 마케팅 캠페인과 함께 제품 데이터, 장바구니 및 체크아웃을 제어하는 데 사용할 수 있습니다. 이 경우 제품 데이터베이스는 AEM(Adobe의 구현) 고유의 저장소에 저장됩니다 [JCR](https://www.adobe.io/experience-manager/reference-materials/spec/jcr/2.0/index.html)).
>
...
>[!NOTE]
>
>geometrixx 가져오기는 CSV 파일을 사용합니다. 구현 위의 주석에는 허용되는 스키마에 대한 설명(사용자 지정 속성이 허용됨)이 있습니다.

다음 [제품 서비스 관리자](https://helpx.adobe.com/experience-manager/6-5/sites/developing/using/reference-materials/javadoc/com/adobe/cq/commerce/pim/api/ProductServicesManager.html) 유지(에서 까지) [OSGi](/help/sites-deploying/configuring.md#osgi-configuration-settings))의 구현 목록  [제품 가져오기](https://helpx.adobe.com/experience-manager/6-5/sites/developing/using/reference-materials/javadoc/com/adobe/cq/commerce/pim/api/ProductImporter.html) 및 [카탈로그 블루프린트 가져오기](https://helpx.adobe.com/experience-manager/6-5/sites/developing/using/reference-materials/javadoc/com/adobe/cq/commerce/pim/api/CatalogBlueprintImporter.html) 인터페이스. 다음 목록에 나열되어 있습니다 **가져오기/상거래 공급자** importer 마법사의 드롭다운 필드( `commerceProvider` 속성을 이름)으로 사용할 수 있습니다.
...
다음 두 카테고리를 구분할 수 있으면 의미 있는 구조(의 트리)를 가진 명확한 URL을 지정할 수 있습니다. `cq:Page` 노드) 및 따라서 클래식 AEM 컨텐츠 관리에 매우 가까움):
...
* *마케팅* 카테고리
   ...
   기타 모든 범주 a *제품이 속할 수 있음*; 예:

   `/special-offers/christmas/shoes`)

### 제품 데이터 {#product-data}

제품을 묘사하고 관리하기 위해 해당 제품에 대한 다양한 정보를 보유할 수 있습니다.
...
* eCommerce 엔진에서 유지 관리되고 AEM에서 사용할 수 있습니다.
   ...
   데이터 유형에 따라 다음과 같습니다. [동기화됨](#catalog-maintenance-data-synchronization) 필요에 따라 또는 직접 액세스합니다. 예를 들어, 제품 가격과 같은 휘발성이 높고 중요한 데이터는 모든 페이지 요청에서 전자 상거래 엔진에서 검색되어 항상 최신 상태로 유지됩니다.
...
적절한 제품에 대해 변형에 대한 정보를 보유할 수도 있습니다. 예를 들어 의류 품목의 경우 사용할 수 있는 다양한 색상이 변형으로 유지됩니다.
...
언어와 마찬가지로 다국적 대기업도 여러 브랜드를 지원할 수 있습니다.
...
초기 가져오기 이후에는 제품 데이터가 변경될 수 없습니다.

eCommerce 엔진을 사용하는 경우 제품 데이터가 유지되며 AEM에서 사용할 수 있어야 합니다. 이 제품 데이터는 업데이트 시 동기화되어야 합니다.
...
PIM(eCommerce 엔진)에서 제품 수가 많은 대형 카탈로그(일반적으로 10만 개 이상)를 가져오면 많은 수의 노드로 인해 시스템에 영향을 줄 수 있습니다. 제품에 연결된 에셋(예: 제품 이미지)이 있는 경우 작성 인스턴스 속도가 느려질 수도 있습니다. 이는 이러한 에셋의 사후 처리가 CPU와 메모리 집약적이기 때문입니다.
...
* 작성자 환경:
   ...
   배경(예: 가져오기) 활동은 일반 사용자 활동(예: 페이지 편집)과 동시에 발생할 수 있으며, 프론트엔드 성능이 더 높은 우선 순위를 고려할 때(일반적으로) 온라인 작성자가 보이는 잘못된 성능은 Go-Live 결정을 차단할 수 있는 불편을 초래할 수 있습니다.
...
* 최대한 중복을 방지하십시오(현지화 참조). 또는 그렇게 할 때 중복이 몇 개의 노드로 이어질지 생각해 보십시오.
* 쿼리 최적화를 준비하기 위해 가능한 한 콘텐츠에 태그를 지정해 보십시오.
   ...
   는 콘텐츠 수준(예: 국가, 언어, 카테고리, 브랜드, 제품)당 하나의 태그를 가져야 합니다. 검색 중
...
* 기술 스택에서 매우 요소화된 콘텐츠 액세스 모델 및 서비스를 계획합니다. 이는 일반적인 모범 사례이지만 최적화 단계에서 매우 자주 읽히는(그리고 번들 캐시를 채우지 않으려는) 데이터에 대한 애플리케이션 캐시를 추가할 수 있으므로 더 중요합니다.

   예를 들어 속성 관리는 제품 가져오기를 통해 업데이트되는 데이터와 관련이 있으므로 캐싱에 매우 적합한 후보인 경우가 많습니다.
...
* 가격 책정: 일반적으로 페이지가 표시/새로 고쳐질 때마다 eCommerce 엔진에서 검색됩니다.
...
다음 **제품** 구성 요소는 필요한 메타데이터(예: 의 경로)를 제공하는 상위 페이지가 있는 모든 페이지에 추가할 수 있습니다. `cartPage` 및 `cartObject`). 데모 사이트, Geometrixx Outdoors에서 다음에서 제공합니다. `UserInfo.jsp`.
...
카탈로그를 만들면 AEM 내에서 업데이트하고 사용자 지정할 수 있는 각 제품에 대한 개별 구성 요소를 제공하기 때문에 제품당 10개의 노드가 사용됩니다. 카탈로그에 수백 개 또는 수천 개의 제품이 포함된 경우 이러한 많은 수의 노드가 문제가 될 수 있습니다. 문제를 방지하기 위해 프록시 페이지를 사용하여 카탈로그를 만들 수 있습니다.
...
바우처와 함께 프로모션을 사용하면 다음과 같은 시나리오를 실현할 수 있습니다.
...
프로모션은 일반적으로 제품 정보 관리자가 아닌 마케팅 관리자가 관리합니다.
...
* [경험](/help/sites-authoring/personalization.md) *다음 범위 내* 캠페인은 에셋(티저 페이지, 프로모션 등)이 해당되는 대상 세그먼트에 따라 그룹화하는 데 사용됩니다
...
* 프로모션이 경험 내에 표시되지 않는 경우(캠페인에서만) 대상에 자동으로 적용할 수 없습니다. 그러나 구매자가 장바구니에 바우처를 입력하고 해당 바우처가 프로모션을 참조하는 경우 여전히 실행될 수 있습니다.
   ...
   은 경험 외부에 있으므로 자동으로 실행되지 않습니다(예: 세그먼테이션 기반). 그러나 문서 캠페인 내의 여러 경험에서 찾을 수 있는 바우처에서 참조됩니다. 이러한 바우처 코드를 장바구니에 입력하면 프로모션이 실행됩니다.

>[!NOTE]
>
>[hybris 프로모션](https://www.hybris.com/modules/promotion) 및 [hybris 바우처](https://www.hybris.com/en/modules/voucher) 장바구니에 영향을 주고 가격과 관련된 모든 항목을 다룹니다. 프로모션별 마케팅 콘텐츠(배너 등)는 hybris 프로모션의 일부가 아닙니다.
...
1. 사용자 계정이 두 시스템에 모두 존재합니다.

   1. 필요한 작업이 없습니다.

1. 사용자 계정은 AEM에만 존재합니다.

   1. 사용자는 AEM에 저장되는 동일한 계정 ID와 임의의 암호를 사용하여 eCommerce 엔진에 생성됩니다.
...
1. 사용자 계정은 eCommerce 엔진에만 존재합니다.
   ...
   1. 계정은 동일한 계정 ID 및 암호로 AEM에 만들어집니다.
...
가입 후 구매자는 자신의 행동을 추적하고 주문을 이행할 수 있도록 계정으로 로그인할 수 있습니다.
...
계정 정보를 쉽게 관리할 수 있는 페이지가 있습니다. 다음을 클릭하여 액세스할 수 있습니다. **내 계정** geometrixx 페이지의 맨 위에서 또는 로 이동하여 `/content/geometrixx-outdoors/en/user/account.html`.

![chlimage_1-13](/help/sites-administering/assets/chlimage_1-13.png)

### 주소록 {#address-book}

사이트에는 게재, 청구 및 대체 주소를 포함하여 다양한 주소가 저장되어야 합니다. 기본 주소 형식을 기반으로 하는 양식을 사용하여 구현하거나 AEM에서 제공하는 주소록 구성 요소를 사용할 수 있습니다.
...
다음을 클릭할 수 있습니다. **새 주소 추가...** 주소록에 새 주소를 추가합니다. 양식을 열고 을 입력한 다음 **주소 추가**.
...
쇼핑할 때 쇼핑객은 제품 페이지를 탐색하고 장바구니에 보관할 항목을 선택합니다. 체크아웃을 진행할 때 주문을 넣을 수 있습니다.
...
두 경우 모두, 항목이 로그인/로그아웃(동일한 컴퓨터/브라우저에만 있음) 동안 장바구니에 유지되며(및 복원할 수 있음), 예:

* 다음으로 찾아보기 `anonymous` 장바구니에 제품 추가
* 다음으로 로그인 `Allison Parker` - 장바구니가 비어 있음
* 장바구니에 제품 추가
* 로그아웃 - 장바구니에 대한 제품이 표시됩니다. `anonymous`

* 다음으로 다시 로그인 `Allison Parker` - 제품이 복원됨
...
체크아웃 전에 가격 변경 사항이 발생할 때 두 시스템에 모두 반영됩니다.
...
주문 이행 및 추적은 일반적으로 eCommerce 엔진에서 관리합니다. 적용된 바우처 및 프로모션을 포함하여 모든 관련 세부 정보가 표시되는 주문 내역 구성 요소를 사용하여 AEM에서 정보를 표시할 수 있습니다. 예:
...
eCommerce 엔진을 사용하는 경우 eCommerce 검색 API가 eCommerce 엔진 솔루션에서 완전히 구현될 수 있으므로 즉시 사용할 수 있도록 제공되는 eCommerce 검색 구성 요소를 사용할 수 있습니다. 면처리 검색을 사용하면 JCR 및/또는 엔진을 검색할 수 있습니다.
