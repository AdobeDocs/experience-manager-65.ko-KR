...
>[!NOTE]
>
>WAR 배포에서 Dynamic Media를 사용하는 경우 [다이내믹 미디어 설명서](/help/assets/config-dynamic.md#enabling-dynamic-media)를 참조하십시오.
...
* 실행 모드는 `author`입니다.
* 인스턴스(저장소, Felix OSGI 환경, 번들 등) `${user.dir}/crx-quickstart`에 설치됨. 여기서 `${user.dir}`은 현재 작업 디렉터리이고 crx-quickstart의 이 경로는 `sling.home`입니다.
...
* sling.home:배포하기 전에 AEM war 파일의 `WEB-INF/web.xml`파일에 있는 `sling.home` 매개 변수를 구성합니다.
...
* `error.log`파일 뒤에 모든 콘텐츠가 설치되어 있는지 확인
* `/system/console`에서 모든 번들이 설치되었음을 확인합니다.

#### 동일한 응용 프로그램 서버에 있는 두 개의 인스턴스 {#two-instances-on-the-same-application-server}
...
aem 웹 응용 프로그램을 설치하기 위해 deployment-scanner를 사용하는 경우 인스턴스의 xml 파일에 `deployment-timeout` 특성이 설정된 `deployment-timeout,`을(를) 늘리는 것이 좋을 수 있습니다(예: `configuration/standalone.xml)`).
...
* `${myDomain}/config/config.xml`보안 구성 섹션에 추가:
   ...
   * `<enforce-valid-basic-auth-credentials>false</enforce-valid-basic-auth-credentials>` https://xmlns.oracle.com/weblogic/domain/1.0/domain에서  [올바른 위치](https://xmlns.oracle.com/weblogic/domain/1.0/domain.xsd) 를 참조하십시오(기본적으로 섹션의 끝에 위치를 지정함).
...
* **Tomcat 서버 준비**
   ...
   * VM 메모리 설정 증가:
      ...
      * `bin/catalina.bat`(unix의 경우 `catalina.sh` 재전송)에서 다음 설정을 추가합니다.
