...
description: 애플리케이션 서버에 AEM을 설치하는 방법을 알아봅니다.
...
>[!NOTE]
>
>`JAR` 및 `WAR` AEM이 릴리스된 파일 유형입니다. 이러한 포맷은 Adobe이 약속한 지원 수준에 맞게 품질 보증을 받고 있습니다.

이 섹션에서는 애플리케이션 서버와 함께 Adobe Experience Manager(AEM)를 설치하는 방법을 설명합니다. 다음을 참조하십시오. [지원되는 플랫폼](/help/sites-deploying/technical-requirements.md#servlet-engines-application-servers) 섹션에서는 개별 애플리케이션 서버에 대해 제공되는 특정 지원 수준을 확인할 수 있습니다.
...
* 인스턴스(저장소, Felix OSGI 환경, 번들 등) 이(가)에 설치되었습니다. `${user.dir}/crx-quickstart`위치 `${user.dir}` 는 현재 작업 디렉토리이며 crx-quickstart에 대한 이 경로는 이라고 합니다. `sling.home`
...
데모 목적으로 작성자 및 게시 인스턴스를 하나의 애플리케이션 서버에 설치하는 것이 적절할 수 있습니다. 이를 위해 다음을 수행합니다.
...
1. war 파일의 이름이 서로 다르도록 이름을 변경합니다. 예를 들어, 한 파일의 이름은 aemaauthor.war로 바뀌고 다른 파일의 이름은 aempublish.war로 바뀝니다.
1. 더 높은 메모리 설정 사용(예: 기본 AEM 인스턴스 사용): -Xmx3072m
1. 두 웹 애플리케이션을 배포합니다.
...
* 기본 인증 헤더가 통과하도록 허용:
   ...
   * AEM에서 사용자를 인증할 수 있는 한 가지 방법은 WebSphere 서버의 전역 관리 보안을 비활성화하는 것입니다. 이렇게 하려면 보안 -> 전역 보안으로 이동하고 관리 보안 활성화 확인란을 선택 취소하고 서버를 저장하고 다시 시작합니다.
...
* VM 메모리 설정 늘리기:
   ...
   * 열기 `${myDomain}/bin/setDomainEnv.cmd` (resp .sh)WLS_MEM_ARGS를 검색합니다. (예: `WLS_MEM_ARGS_64BIT=-Xms256m -Xmx2048m`
...
* **Tomcat 서버 준비**
   ...
   * VM 메모리 설정 늘리기:
      ...
      * `set "JAVA_OPTS= -Xmx2048m`
   * Tomcat은 설치 시 관리자나 관리자 액세스를 제공하지 않습니다. 따라서 수동으로 편집해야 합니다 `tomcat-users.xml` 이러한 계정에 대한 액세스를 허용하려면 다음을 수행하십시오.
   ...
   * Manager-gui를 사용하여 AEM 웹 애플리케이션을 설치하는 경우 기본적으로 50MB의 업로드 크기만 허용하므로 업로드된 파일의 최대 크기를 늘려야 합니다. 이를 위해 관리자 웹 응용 프로그램의 web.xml을 엽니다.
      ...
      max-file-size 및 max-request-size를 최소 500MB로 늘리려면 다음을 참조하십시오 `multipart-config` 그러한 예는 다음과 같습니다 `web.xml` 파일.
