...
Tar 마이크로 커널의 콜드 스탠바이 용량을 사용하면 하나 이상의 대기 Adobe Experience Manager(AEM) 인스턴스를 기본 인스턴스에 연결할 수 있습니다. 동기화 프로세스는 기본 인스턴스에서 대기 인스턴스로만 수행됨을 의미합니다.
...
대기 모드에서는 동기화 프로세스 중에 높은 CPU 사용량이 예상됩니다. 프로시저는 다중 스레드가 아니므로 여러 코어를 사용하여 속도를 높일 수 없습니다. 데이터를 변경하거나 전송하지 않으면 측정 가능한 활동이 없습니다. 연결 속도는 하드웨어 및 네트워크 환경에 따라 다르지만 저장소 크기나 SSL 사용에 따라 다르지 않습니다. 초기 동기화에 필요한 시간이나 주 노드에서 많은 데이터가 변경되었을 때를 예상할 때 이 점을 염두에 두십시오.
...
>[!NOTE]
>
>Dispatcher와 콜드 대기 설정의 일부인 서버 사이에 로드 밸런서를 추가하는 것이 좋습니다. 로드 밸런서는 사용자 트래픽을 **기본** 인스턴스. 이는 일관성을 보장하고 콜드 대기 메커니즘이 아닌 다른 방법으로 대기 인스턴스에서 콘텐츠가 복사되지 않도록 하기 위해 필요합니다.
...
TarMK 콜드 대기 설정을 생성하려면 먼저 기본 설치 폴더의 전체 설치 폴더를 새 위치로 파일 시스템 복사를 수행하여 대기 인스턴스를 생성합니다. 그런 다음 역할을 지정하는 실행 모드로 각 인스턴스를 시작할 수 있습니다( `primary` 또는 `standby`).

다음은 하나의 마스터와 하나의 대기 인스턴스로 설정을 만들기 위해 따라야 하는 절차입니다.

1. AEM을 설치합니다.

1. 인스턴스를 종료하고 설치 폴더를 콜드 대기 인스턴스가 실행되는 위치에 복사합니다. 다른 컴퓨터에서 실행 중인 경우에도 각 폴더에 설명 이름(예: *aem 기본* 또는 *aem 대기*)를 사용하여 인스턴스를 구분합니다.
1. 기본 인스턴스의 설치 폴더로 이동하여 다음을 수행합니다.
   ...
   1. 아래의 기본 노드 저장소 및 데이터 저장소에 대한 필수 구성을 만듭니다. `aem-primary/crx-quickstart/install/install.primary`
...
1. 에 대한 Apache Sling 로깅 로거 만들기 **org.apache.jackrabbit.oak.segment** 패키지. 다음과 같이 로그 수준을 &quot;Debug&quot;로 설정하고 로그 출력을 별도의 로그 파일로 지정합니다. */logs/tarmk-coldstandby.log*. 자세한 내용은 [로깅](/help/sites-deploying/configure-logging.md).
...
1. 그런 다음 대기 인스턴스를 준비합니다. 기본 인스턴스와 동일한 단계를 수행하여 이 작업을 수행할 수 있습니다.
   ...
   1. 라는 폴더 만들기 `install.standby` 아래에 `aem-standby/crx-quickstart/install`
   ...
   1. 라는 폴더 만들기 `crx3` 아래에 `aem-standby/crx-quickstart/install`
...
>[!NOTE]
>
>인스턴스의 존재 여부를 확인하여 인스턴스의 역할을 언제든지 확인할 수 있습니다. **기본** 또는 **대기** sling 설정 웹 콘솔에서 모드를 실행합니다.
>
...
대기 모드 *tarmk-coldstandby.log*&#x200B;와 같은 항목을 볼 수 있습니다.
...
대기 상태에서 *error.log*&#x200B;다음과 같은 항목이 표시됩니다.
...
또한 비공유로 실행할 때도 `FileDataStore`, 다음과 같은 메시지는 이진 파일이 제대로 전송되고 있는지 확인합니다.
...
* **구성 유지:** 활성화되면 기존 OSGi 구성 파일 대신 저장소에 구성이 저장됩니다. Adobe은 기본 구성을 대기 모드로 설정하지 않도록 프로덕션 시스템에서 이 설정을 비활성화하는 것을 권장합니다.
...
>[!NOTE]
>
>Adobe은 기본 및 대기 저장소 ID가 서로 달라 오프로딩과 같은 서비스에 대해 별도로 식별할 수 있도록 할 것을 권장합니다.
>
>이를 보장하기 위한 가장 좋은 방법은 삭제하는 것입니다 *sling.id* 대기 상태에서 인스턴스를 다시 시작합니다.
...
1. 콜드 대기 인스턴스로 작동하도록 새로 생성된 클론을 다시 구성합니다. 다음을 참조하십시오 [AEM TarMK 콜드 대기 설정 생성.](/help/sites-deploying/tarmk-cold-standby.md#creating-an-aem-tarmk-cold-standby-setup)
...
Adobe은 시간에 따른 과도한 저장소 증가를 방지하기 위해 유지 관리를 정기적으로 실행할 것을 권장합니다. 콜드 대기 저장소 유지 관리를 수동으로 수행하려면 아래 단계를 따르십시오.
...
1. 유지 관리 프로세스가 완료되고 인스턴스가 다시 시작된 후:
   ...
   * 대기에서 데이터 저장소 가비지 수집은 **BlobGarbageCollection** MBean - `startBlobGC()`. 다음 **RepositoryManagement** 대기 모드에서는 MBean을 사용할 수 없습니다.
