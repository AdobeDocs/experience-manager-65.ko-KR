...
아래 다이어그램은 성능 문제를 해결하기 위해 수행해야 하는 단계에 대한 지침을 제공하기 위한 것입니다. 읽기 쉽도록 5개의 섹션으로 나뉩니다.
...
<table>
 <tbody>
  ...
  <tr>
   ...
   <td>브라우저에서 표준 HTTP 요청 분석을 사용하여 요청 흐름을 분석할 수 있습니다. Chrome에서 이 작업을 수행하는 방법에 대한 자세한 내용은 위의 링크를 참조하십시오.<br /> </td>
  </tr>
  <tr>
  ...
  <tr>
   ...
   <td><p>다음 확인: <a href="https://helpx.adobe.com/experience-manager/dispatcher/using/dispatcher-configuration.html#debugging">Dispatcher 디버깅 설명서</a> 요청이 제대로 캐시되는지 확인합니다.<br /> </p> </td>
  </tr>
  <tr>
   <td><strong>5단계</strong></td>
   <td>Dispatcher가 AEM을 통해 각 요청을 인증하려고 합니까?</td>
   <td>Dispatcher가 를 전송하는지 확인 <code>HEAD</code> 캐시된 리소스를 전달하기 전에 인증을 위해 AEM에 요청합니다. 다음을 찾아 이 작업을 수행할 수 있습니다. <code>HEAD</code> AEM의 요청 <code>access.log</code>. 자세한 내용은 <a href="/help/sites-deploying/configure-logging.md">로깅</a>.<br /> </td>
  </tr>
  <tr>
  ...
  <tr>
   ...
   <td><br /> <p>사용 <a href="/help/sites-developing/tough-day.md">Tough Day</a> 프로덕션 인스턴스에서 "실제" 조건을 복제합니다. 현실적이지 않은 경우 다른 네트워크 컨텍스트에서 프로덕션 인스턴스(또는 동일한 스테이징 인스턴스)를 테스트해야 합니다.<br /> </p> </td>
  </tr>
  <tr>
  ...
  <tr>
   ...
   <td>네트워크 레이어 조사</td>
   <td><p>네트워크 계층에서 포화 및 지연 문제를 조사합니다.</p> <p>작성 계층의 경우 지연 시간이 100밀리초를 넘지 않는 것이 좋습니다.</p> <p>성능 최적화 팁에 대한 자세한 내용은 <a href="https://helpx.adobe.com/experience-manager/kb/performance-tuning-tips.html">이 페이지</a>.</p> </td>
  </tr>
  <tr>
  ...
  <tr>
   ...
   <td>느린 요청 찾기</td>
   <td><p>를 분석하여 느린 요청이 있는지 확인할 수 있습니다. <code>request.log</code> 또는 를 사용하여 <code>rlog.jar</code>.</p> <p>rlog.jar 사용에 대한 자세한 내용은 이 페이지를 참조하십시오.</p> <p>다음을 참조하십시오 <a href="/help/sites-deploying/monitoring-and-maintaining.md#using-rlog-jar-to-find-requests-with-long-duration-times">rlog.jar를 사용하여 긴 지속 시간이 있는 요청 찾기</a>.<br /> </p> <p> </p> </td>
  </tr>
  <tr>
  ...
  <tr>
   ...
   <td>Dispatcher 수준에서 세션 관리를 사용하여 AEM 서버 오프로드</td>
  ...
  <tr>
   ...
   <td><br />
    <ul>
     <li><a href="https://helpx.adobe.com/experience-manager/dispatcher/using/page-invalidate.html#invalidating-dispatcher-cache-from-the-authoring-environment">작성자 계층에 대한 캐시 무효화</a></li>
     <li><a href="https://helpx.adobe.com/experience-manager/dispatcher/using/page-invalidate.html#invalidating-dispatcher-cache-from-a-publishing-instance">게시 계층에 대한 캐시 무효화.</a></li>
    </ul> </td>
  </tr>
  <tr>
   <td><strong>37단계 및 38단계</strong></td>
   <td>레이지 로드</td>
   <td><a href="https://docs.adobe.com/ddc/en/gems/aem-web-performance.html">AEM 웹 성능에서 Gem 세션 을 참조하십시오.</a><br /> </td>
  </tr>
  <tr>
   <td><strong>39단계</strong></td>
   <td>연결 오버헤드를 줄이려면 사전 연결 사용</td>
   <td>위에 표시된 Gem 세션 을 참조하십시오. W3c에 대한 추가 설명서 사전 연결:<a href="https://www.w3.org/TR/resource-hints/#dfn-preconnect"> https://www.w3.org/TR/resource-hints/#dfn-preconnect</a></td>
  </tr>
  <tr>
  ...
  <tr>
   ...
   <td>Keep-Alive</td>
   <td><p>다음 <code>Keep-Alive</code> 다른 연결 재사용 요청에 헤더가 표시됩니까? 그렇지 않으면 각 요청이 다른 연결 설정으로 연결되어 불필요한 오버헤드가 발생하는 것입니다. (브라우저의 표준 HTTP 요청 분석)</p> <p>다음을 확인할 수 있습니다. <a href="/help/sites-administering/proxy-jar.md">프록시 서버 도구</a> Keep-Alive 연결을 확인합니다.<br /> </p> </td>
  </tr>
  <tr>
