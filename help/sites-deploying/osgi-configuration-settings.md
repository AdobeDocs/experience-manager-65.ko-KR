...
OSGi &quot;*는 응용 프로그램이 작고 재사용 가능한 공동 작업 구성 요소에서 만들어질 수 있도록 허용하는 표준화된 원시 도형을 제공합니다. 이러한 구성 요소는 애플리케이션으로 구성하고*&quot;을 배포할 수 있습니다.

따라서 번들을 중지하거나 설치하고 개별적으로 시작할 수 있으므로 번들을 손쉽게 관리할 수 있습니다. 상호 종속성은 자동으로 처리됩니다. 각 OSGi 구성 요소([OSGi 사양](https://www.osgi.org/Specifications/HomePage) 참조)는 다양한 번들 중 하나에 포함되어 있습니다. AEM에서 작업하는 경우 이러한 번들의 구성 설정을 관리하는 방법에는 몇 가지가 있습니다.자세한 내용 및 권장 방법은 [OSGi](/help/sites-deploying/configuring-osgi.md) 구성을 참조하십시오.
...
>[!NOTE]
>
>[AEM 도구](https://helpx.adobe.com/experience-manager/kb/tools/aem-tools.html)의 일부인 OSGi 구성 비교 도구를 사용하여 기본 OSGi 구성을 나열할 수 있습니다.
...
* **관리자 암호**&#x200B;는 설치 후 인스턴스 [보안](/help/sites-administering/security-checklist.md)을(를) 변경해야 합니다.
...
* **Plugins**, the main navigation items (console plugins) to be available in  **the** Apache Felix Web ManagementConsole as top level menu items. 공간 및 리소스가 필요하므로 필요하지 않은 항목을 비활성화합니다.

>[!CAUTION]
>
>다음을 구성해야 합니다.
>
>**Apache** Felix  **Web Management Console 자체에 액세스하기 위한 자격 증명인 사용자 이름 및**암호입니다.
>인스턴스의 [security](/help/sites-administering/security-checklist.md)을(를) 확인하려면 초기 설치 후 암호를 변경해야 합니다.
...
* **로거** 이름 및  **로그** 형식을 사용하여 요청 및 액세스 로깅의 위치와 형식을 구성합니다(기본값: `request.log`). 이 로그 파일은 웹 체인과 관련된 성능 또는 디버깅 기능을 분석할 때 반드시 필요합니다.
이것은 [Apache Sling 요청 로거](#apacheslingrequestlogger)와(와) 쌍입니다.

자세한 내용은 [AEM 로깅](/help/sites-deploying/configure-logging.md) 및 [Sling 로깅](https://sling.apache.org/site/logging.html)을 참조하십시오.

**Apache Sling Eventing 스레드** 풀구성:
...
**Apache Sling GET** Servlet렌더링의 일부 측면을 구성합니다.

* **자동** 인덱스로 찾아보기를 위한 디렉토리 렌더링을 활성화/비활성화합니다.
* **HMTL** ,  **일반 텍스트**,  **JSON** 또는  **** XML **과 같은 기본 변환을 활성화(또는 비활성화)**합니다.
JSON을 비활성화해서는 안 됩니다.
...
* S **소스 VM** 및 **Target VM**&#x200B;에서 JDK 버전을 런타임 JVM으로 사용한 JDK 버전으로 정의합니다.

* 프로덕션 인스턴스의 경우:

   * **디버그 정보 생성** 비활성화

**Apache Sling JCR** Installer이러한 매개 변수에는 구성이 필요하지 않지만 개발 또는 디버깅할 때 유용할 수 있습니다. 예를 들어 설치 폴더는 패키지를 체크 인/체크 아웃하거나 만드는 데 유용합니다.

* **설치 폴더 이름** regexpand  **설치 폴더의** 최대 계층 깊이- 저장소 폴더의 설치 리소스를 검색할 위치 및 깊이를 지정합니다. 와일드카드를 사용할 때(와 같이)*/install) 모든 적절한 일치 항목을 검색합니다(예: `/libs/sling/install` 및 `/libs/cq/core/install`).
...
**Apache Sling JSP Script** HandlerJSP 스크립트 핸들러에 대한 성능 관련 설정을 구성합니다. 성능을 향상시키려면 가능한 한 많이 비활성화해야 합니다.
...
**Apache Sling 로깅** 구성구성:

* **중앙 로깅** 구성 **의 위치 및 로그 수준을 정의하는 로그**&#x200B;도메인 로그 파일(error.log). 수준은 `DEBUG`, `INFO`, `WARN`, `ERROR` 및 `FATAL` 중 하나로 설정할 수 있습니다.

* **로그 파일** 및  **로그 파일** 임계값을 사용하여 로그 파일의 크기와 버전 순환을 정의합니다.
...
* **카테고리** 를 정의하는 데 어려움을 겪습니다.예를 들어 com.day.cq에 대한 로그만 있을 수 있습니다.

* **팩토리 구성**&#x200B;을 사용하면 필요한 다양한 로그 수준 및 범주에 맞는 추가 구성을 추가할 수 있습니다.
...
**Apache Sling Main** Servlet구성:

* **무한 재귀 및** 과도한 스크립트  **호출으로부터 시스템을** 보호하기 위한 요청당 호출 수 및 재귀깊이.

**Apache Sling MIME Type** Service구성:

* **MIME** Typekit을 사용하여 프로젝트에 필요한 항목을 시스템에 추가합니다. 따라서 파일의 `GET` 요청이 파일 형식 및 응용 프로그램 연결에 필요한 올바른 내용 유형 헤더를 설정할 수 있습니다.
...
이 ID는 [Apache Sling Customizable Request Data Logger](#apacheslingcustomizablerequestdatalogger)와 함께 사용됩니다.
...
>
>그렇지 않은 경우 Felix 콘솔을 사용하여 **URL 매핑**&#x200B;에 변경 사항이 있으면 AEM이 다음 시작 시 덮어쓸 수 있습니다.

**Apache Sling Servlet/Script Resolver 및 Error** HandlerSling Servlet 및 Script Resolver에는 다음과 같은 여러 작업이 있습니다.

1. 요청을 처리할 Servlet 또는 Script를 선택하는 데 `ServletResolver`로 사용됩니다.

1. 이것은 `SlingScriptResolver` 역할을 합니다.

1. 요청 처리 서블릿과 스크립트를 해결하는 데 사용되는 것과 같이 오류 처리 서블릿과 스크립트를 선택하는 동일한 알고리즘을 사용하여 `ErrorHandler` 인터페이스를 구현하여 오류 처리를 관리합니다.
...
* **기본** 익스텐션기본 동작을 사용할 익스텐션 목록입니다. 즉, 리소스 유형의 마지막 경로 세그먼트를 스크립트 이름으로 사용할 수 있습니다.
...
**일 CQ** 안티스팸 사용된 스팸 방지 서비스(Akismet)를 구성합니다. 이를 위해서는 다음을 등록해야 합니다.

* **공급자**
* **API 키**
* **등록된 URL**

**Adobe [MOCK]Configure this to control the handling of client libraries (css or js);** 예를 들어 기본 구조가 표시되는 방식을 포함합니다.

* 프로덕션 인스턴스의 경우:

   * **Minify**(CRLF 및 공백 문자 제거)를 활성화합니다.
...
* JS 개발(특히 방화벽/디버깅 시):
   ...
   * 타이밍에 관심 있는 경우 **타이밍**&#x200B;을 활성화합니다.
   * JS 콘솔 로그 메시지를 보려면 **디버그** 콘솔을 활성화합니다.

>[!CAUTION]
>
>**Minify** 또는 **Gzip**&#x200B;에 대한 설정을 변경할 때 `/var/clientlibs`의 컨텐츠도 삭제해야 합니다. 이 버전은 Clientlibs의 캐시된 버전이며 다음에 요청할 때 다시 빌드됩니다.
...
[폐쇄된 사용자 그룹](/help/sites-administering/cug.md)을 사용할 때(다른 그룹 중) 구성할 수 있습니다.

* **HTTP 영역**
* **기본 로그인 페이지**

**CQ Link Checker** ServiceCheck 요일 및 필요한 경우 다음을 구성합니다.

* **스케줄러** 기간에서 외부 링크를 자동으로 확인하는 간격을 정의합니다.

* 실패한 외부 링크가 나쁜 것으로 간주되는 기간 동안 **잘못된 링크 허용치 간격**&#x200B;을 선택합니다.
* **링크 확인 대체 패턴** 지정을 참조하십시오.

**일 CQ 링크 검사기** 작업단일 링크 검사기 작업(외부 링크를 확인하는 작업)에 대한 설정을 구성합니다.

* **연결 테스트 간격** 및 **잘못된 링크 테스트 간격**&#x200B;에 정의된 간격을 확인합니다.
...
**일 CQ MCM** 뉴스레터뉴스레터에 사용되는 다양한 설정을 구성합니다.

**일 CQ 루트** 매핑구성:

* **Target** 경로 &quot;&quot;에 대한 요청이 리디렉션되는 위치를  `/`정의합니다.
...
**Adobe Granite SSO Authentication** HandlerSSO(Single Sign On) 구성 세부 사항;이러한 문제는 LDAP와 함께 기업 작성자 설정에서 자주 필요합니다.

다양한 구성 속성을 사용할 수 있습니다.

* **이**
인증 처리기가 활성 상태인 경로. 이 매개 변수를 비워 두면 인증 처리기가 비활성화됩니다. 예를 들어 경로 / 는 전체 저장소에 대해 인증 핸들러를 사용합니다.
...
* **매개 변수**
이름사용자 ID를 제공할 수 있는 요청 매개 변수의 이름입니다.

* **사용자**
맵선택한 사용자의 경우 HTTP 요청에서 추출한 사용자 이름을 자격 증명 개체에서 다른 이름으로 바꿀 수 있습니다. 매핑이 여기에 정의되어 있습니다. 사용자 이름이 
...
**일 CQ WCM 디버그** 필터페이지에 액세스할 때 ?debug=layout과 같은 접미사를 사용할 수 있으므로 개발하는 경우 유용합니다. 예를 들어 https://localhost:4502/cf#/content/geometrixx/en/support.html?debug=layout에서는 개발자에게 관심이 있는 레이아웃 정보를 제공합니다.
...
* 작성자 인스턴스의 경우 `edit`, `disable,preview` 또는 `analytics`일 수 있습니다.
다른 모드는 사이드 킥에서 액세스할 수 있고, 접미사 `?wcmmode=disabled`를 사용하여 프로덕션 환경을 에뮬레이션할 수 있습니다.

* 게시 인스턴스에서 다른 모드에 액세스할 수 없도록 하려면 이 모드를 `disabled`으로 설정해야 합니다.

>[!NOTE]
>
>이 설정은 [프로덕션 준비 모드](/help/sites-administering/production-ready.md)에서 AEM을 실행하는 경우 프로덕션 인스턴스에 대해 자동으로 구성됩니다.

**CQ WCM Link Checker** Configurator구성:

* **컨텐츠 기반** 링크 검사기 구성에 대한 위치 목록을 지정하는 재작성 구성 목록입니다. 구성은 실행 모드를 기반으로 할 수 있습니다.linkchecker 설정은 다를 수 있으므로 작성 환경과 게시 환경을 구분하는 것이 중요합니다.
...
**CQ WCM Version** Manager사용자 시스템에서 버전을 관리하는 경우 및 방법:
...
**Day CQSE HTTP** ServiceCQ Servlet Engine 제어:
...
SSL 관련 옵션에 대한 자세한 내용 및 CQSE용 HTTPS를 사용하는 방법에 대한 전체 설명은 [HTTP Over SSL 활성화](/help/sites-administering/ssl-by-default.md)를 참조하십시오.
...
**AEM** 과 CDN 간 CDN 리writerCommunication을 보증하여 자산/이진 파일이 최종 사용자에게 안전한 방법으로 전달되어야 합니다. 여기에는 두 가지 작업이 포함됩니다.
...
1. `cdn_sign` 매개 변수와 함께 요청을 AEM에 전달하도록 CDN을 구성해야 합니다.
...
**CDNRewriter** 내부 이미지 URL을 CDN URL로 재작성하는 리필터입니다.
