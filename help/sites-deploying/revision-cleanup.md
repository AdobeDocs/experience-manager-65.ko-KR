...
저장소에 대한 각 업데이트는 새 콘텐츠 개정을 만듭니다. 결과적으로 각 업데이트 시 저장소의 크기가 커집니다. 저장소 증가를 제어하지 않으려면 디스크 리소스를 확보하기 위해 이전 버전을 정리해야 합니다. 이 유지 관리 기능을 개정 정리 라고 합니다. AEM 6.0 이후 오프라인 루틴으로 사용할 수 있습니다.
...
**온라인 개정 정리는 개정 정리를 수행하는 권장 방법입니다.** 오프라인 수정 버전 정리는 새로운 스토리지 형식으로 마이그레이션하기 전이나 Adobe 고객 지원 센터에서 요청하는 경우와 같이 예외적인 경우에만 사용해야 합니다.
