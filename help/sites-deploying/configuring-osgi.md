...
이렇게 하면 번들을 개별적으로 중지, 설치 또는 시작할 수 있으므로 쉽게 관리할 수 있습니다. 상호 종속성이 자동으로 처리됩니다. 각 OSGi 구성 요소( [OSGi 사양](https://www.osgi.org/Specifications/HomePage))는 다양한 번들 중 하나에 포함됩니다.
...
* 스크립트를 사용하여 구성 롤아웃을 수행하여 구성 세부 정보를 제안할 수 있습니다.
...
>[!NOTE]
>
>웹 콘솔에서 수행된 변경 사항은 다음과 같이 저장소에 저장됩니다. [구성 파일](#osgi-configuration-with-configuration-files). 이러한 구성 요소는 추가 설치에서 다시 사용하기 위해 콘텐츠 패키지에 포함할 수 있습니다.

>[!NOTE]
>
>웹 콘솔에서 Sling 기본값과 관련된 기본 설정을 언급하는 설명을 모두 표시합니다.
>
>Adobe Experience Manager에는 자체 기본값이 있으므로 설정된 기본값이 콘솔에 기록된 기본값과 다를 수 있습니다.

웹 콘솔로 구성을 업데이트하려면 다음을 수행하십시오.

1. 액세스 **구성** 웹 콘솔의 탭은 다음 중 하나를 수행합니다.

   * 의 링크에서 웹 콘솔 열기 **도구 -> 작업** 메뉴 아래의 제품에서 사용할 수 있습니다. 콘솔에 로그인한 후 다음과 같은 드롭다운 메뉴를 사용할 수 있습니다.
...
1. 대화 상자가 열립니다. 여기에서 필요에 따라 편집할 수 있습니다. 예를 들어 **로그 수준** 끝 `INFO`:
   ...
   >[!NOTE]
   >
   >업데이트는 다음과 같이 저장소에 저장됩니다. [구성 파일](#osgi-configuration-with-configuration-files). 나중에 이러한 매개 변수를 찾으려면(예: 다른 인스턴스에서 사용할 콘텐츠 패키지에 포함) 영구 ID( `PID`).
...
1. **저장**&#x200B;을 클릭합니다.
   ...
   >[!NOTE]
   >
   >이제 관련 항목을 찾을 수 있습니다 [구성 파일](#osgi-configuration-with-configuration-files): 예를 들어 다른 인스턴스에서 사용할 콘텐츠 패키지에 포함할 수 있습니다.
...
이러한 구성 요소는 콘텐츠 패키지에 포함되어 다른 인스턴스에서 다시 사용할 수 있습니다.

>[!NOTE]
>
>구성 파일의 형식은 매우 구체적입니다. 자세한 내용은 [Sling Apache 설명서](https://sling.apache.org/documentation/development/slingstart.html#default-configuration-format) 전체 세부 정보.
>
>이러한 이유로 웹 콘솔에서 실제 변경 작업을 수행하여 구성 파일을 만들고 유지 관리하는 것이 좋습니다.

웹 콘솔에는 변경 내용이 저장된 저장소 위치가 표시되지 않지만 쉽게 찾을 수 있습니다.

1. 구성 파일 만들기 기준 [웹 콘솔에서 초기 변경](#osgi-configuration-with-the-web-console).
1. CRXDE Lite 열기.
1. 다음에서 **도구** 메뉴 선택 **쿼리 ...** .
1. 다음에 대한 쿼리 제출 **유형** `SQL` 업데이트한 구성의 PID를 검색합니다.
...
웹 콘솔을 사용할 수 있을 뿐만 아니라 저장소에서 구성 세부 정보를 정의할 수도 있습니다. 이를 통해 서로 다른 실행 모드를 쉽게 구성할 수 있습니다.

이러한 구성은 다음을 만들어 수행됩니다. `sling:OsgiConfig` 시스템이 참조할 저장소의 노드입니다. 이러한 노드는 OSGi 구성을 미러링하며 이에 대한 사용자 인터페이스를 형성합니다. 구성 데이터를 업데이트하려면 노드 속성을 업데이트합니다.

저장소에서 구성 데이터를 수정하면 적절한 유효성 검사 및 일관성 검사와 함께 웹 콘솔을 사용하여 변경한 것처럼 변경 사항이 관련 OSGi 구성에 즉시 적용됩니다. 다음에서 구성을 복사하는 작업에도 적용됩니다. `/libs/` 끝 `/apps/`.

동일한 구성 매개변수를 여러 위치에 배치할 수 있으므로 시스템은 다음과 같습니다.
...
>[!NOTE]
>
>읽기 전용 [특정 인스턴스에 대해서만 저장소 기반 구성을 정의하는 방법](https://helpx.adobe.com/experience-manager/kb/RunModeDependentConfigAndInstall.html).
...
1. 특정 여부 [실행 모드](/help/sites-deploying/configure-runmodes.md) 필수 항목입니다. 폴더를 만듭니다.
...
1. 여부 **구성** 또는 **출하 시 구성** 필요합니다.
1. 구성할 개별 매개 변수. 다시 만들어야 하는 기존 매개 변수 정의를 포함합니다.
...
1. 구성이 이미 존재합니까? `/libs`? 인스턴스의 모든 구성을 나열하려면 **쿼리** 다음 SQL 쿼리를 제출하는 CRXDE Lite의 도구:
...
1. 구성할 각 매개 변수에 대해 이 노드에 속성을 만듭니다.
   ...
   * 값: 필요에 따라.
   ...
   구성할 매개 변수에 대한 속성만 만들면 되며 다른 매개 변수는 AEM에서 설정한 대로 기본값을 사용합니다.

1. 모든 변경 사항을 저장합니다.

   웹 콘솔의 변경 사항과 마찬가지로 서비스를 다시 시작하여 노드가 업데이트되는 즉시 변경 사항이 적용됩니다.
...
즉, 의 일반 구성이 `/libs` 에서 프로젝트별 구성에 의해 마스킹될 수 있습니다. `/apps`.
...
1. 런타임 시 웹 콘솔의 구성이 우선적으로 적용되므로 웹 콘솔의 구성이 즉시 적용됩니다.
1. 에서 구성 수정 `/apps` 즉시 효력이 발생합니다.
1. 에서 구성 수정 `/libs` 의 구성에 의해 마스킹되지 않는 한 즉시 적용됩니다. `/apps`.

### 여러 실행 모드 해결 방법 {#resolution-of-multiple-run-modes}

실행 모드 특정 구성의 경우, 여러 실행 모드가 결합될 수 있습니다. 예를 들어 다음 스타일의 구성 폴더를 만들 수 있습니다.

`/apps/*/config.<runmode1>.<runmode2>/`

이러한 폴더의 구성은 모든 실행 모드가 시작 시 정의된 실행 모드와 일치하는 경우 적용됩니다.

예를 들어 인스턴스가 실행 모드로 시작된 경우 `author,dev,emea`, 의 구성 노드 `/apps/*/config.emea`, `/apps/*/config.author.dev/` 및 `/apps/*/config.author.emea.dev/` 이 적용되는 동안 구성 노드가 `/apps/*/config.author.asean/` 및 `/config/author.dev.emea.noldap/` 이 적용되지 않습니다.
...
일치 실행 모드 수가 가장 많은 구성은 전체 PID에 적용됩니다.
...
* 다음에 의해 변경된 설정 `admin` 저장 위치: `*.config` 아래에 있는 파일:
   ...
   * 이는 OSGi 구성 관리자의 개인 데이터 영역이며,에 의해 지정된 모든 구성 세부 정보를 포함합니다. `admin`로 식별됩니다.
   * 이는 구현 세부 정보이며 이 디렉터리를 직접 편집해서는 안 됩니다.
   * 그러나 백업 및/또는 여러 설치를 위해 복사본을 만들 수 있도록 이러한 구성 파일의 위치를 아는 것이 유용합니다.
...
>[!CAUTION]
>
>다음을 수행해야 합니다. ***절대 안 함*** 아래의 폴더 또는 파일을 편집합니다.
>
>`/crx-quickstart/launchpad/config`
