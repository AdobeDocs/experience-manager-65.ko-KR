...
* 스크립트를 사용하여 구성 롤아웃을 수행하여 구성 세부 사항을 전파할 수 있습니다.
...
>[!NOTE]
>
>웹 콘솔에서 수행된 변경 내용은 [구성 파일](#osgi-configuration-with-configuration-files). 이러한 파일은 추가 설치에서 재사용할 수 있도록 컨텐츠 패키지에 포함할 수 있습니다.

>[!NOTE]
>
>웹 콘솔에서 기본 설정을 언급하는 설명은 Sling 기본값과 관련이 있습니다.
>
>Adobe Experience Manager에는 자체 기본값이 있으므로 설정되는 기본값이 콘솔에 설명된 기본값과 다를 수 있습니다.
...
1. 대화 상자가 열립니다. 필요에 따라 여기에서 편집할 수 있습니다. 예를 들어 **로그 수준** to `INFO`:

   ![chlimage_1-140](assets/chlimage_1-140.png)

   >[!NOTE]
   >
   >업데이트는 다음과 같이 저장소에 저장됩니다. [구성 파일](#osgi-configuration-with-configuration-files). 예를 들어 다른 인스턴스에서 사용할 컨텐츠 패키지에 포함하도록 이러한 파일을 나중에 찾으려면 영구 ID( `PID`).
...
1. **저장**&#x200B;을 클릭합니다.
   ...
   >[!NOTE]
   >
   >이제 관련 항목을 찾을 수 있습니다 [구성 파일](#osgi-configuration-with-configuration-files). 예를 들어 다른 인스턴스에서 사용할 컨텐츠 패키지에 를 포함하려면,
...
이러한 파일은 컨텐츠 패키지에 포함할 수 있으며 다른 인스턴스에서 재사용할 수 있습니다.

>[!NOTE]
>
>구성 파일의 형식은 고유합니다. 자세한 내용은 [Sling Apache 설명서](https://sling.apache.org/documentation/development/slingstart.html#default-configuration-format) 자세한 내용
>
...
이러한 구성은 `sling:OsgiConfig` 시스템이 참조할 저장소의 노드입니다. 이러한 노드는 OSGi 구성을 미러링하며 사용자 인터페이스가 해당 노드에 형성됩니다. 구성 데이터를 업데이트하려면 노드 속성을 업데이트합니다.

저장소에서 구성 데이터를 수정하는 경우 해당 변경 사항이 관련 OSGi 구성에 즉시 적용됩니다. 적절한 유효성 검사 및 일관성 검사와 함께 웹 콘솔을 사용하여 변경한 것과 같습니다. 이 워크플로우는에서 구성을 복사하는 작업에도 적용됩니다. `/libs/` to `/apps/`.

동일한 구성 매개 변수가 여러 위치에 있으므로 시스템은
...
>[!NOTE]
>
>읽기 [특정 인스턴스에 대해서만 저장소 기반 구성을 정의하는 방법](https://experienceleague.adobe.com/docs/experience-cloud-kcs/kbarticles/KA-17500.html?lang=ko).
...
1. 특정 [실행 모드](/help/sites-deploying/configure-runmodes.md) 필수 여부 폴더를 만듭니다.
...
1. 구성이 `/libs`? 인스턴스의 모든 구성을 나열하려면 **쿼리** 다음 SQL 쿼리를 제출하기 위한 CRXDE Lite 도구:
...
1. 구성할 각 매개 변수에 대해 이 노드에서 속성을 만듭니다.
   ...
   * 값: 필요한 경우.
   ...
   구성할 매개 변수에 대해서만 속성을 만들어야 하며, 다른 매개 변수는 AEM에서 설정한 대로 기본값을 사용합니다.

1. 모든 변경 내용을 저장합니다.

   변경 사항은 서비스를 다시 시작하여 노드가 업데이트될 때 적용됩니다(웹 콘솔에서 수행된 변경 사항).
...
의 일반 구성 `/libs` 에서는 프로젝트별 구성으로 마스킹할 수 있습니다. `/apps`.
...
1. 웹 콘솔에서 구성을 수정하는 것은 런타임 시 우선하므로 즉시 적용됩니다.
...
* 에 의해 변경된 설정 `admin` 에 저장됨 `*.config` 파일 위치:
   ...
   * 이 영역은 OSGi 구성 관리자의 개인 데이터이며, `admin`로 설정되어야 합니다.
   * 이 영역은 구현 세부 정보이므로 이 디렉토리를 직접 편집하지 않아야 합니다.
   * 그러나 이러한 구성 파일의 위치를 알고 있으면 백업 또는 다중 설치를 위해 또는 다음 두 작업을 모두 수행할 수 있습니다.
