...
* **URI**
   ...
   * Dispatcher Flush 에이전트는 `https://localhost:8000/dispatcher/invalidate.cache`
...
1. 편집 **클릭** - **에이전트 설정** 대화 상자가 **열립니다. 직렬화** 유형은이미 기본값으로 정의되었으므로 이 설정은 그대로 유지됩니다.
...
Dispatcher **Flush** 복제 에이전트가 작성자에서 활성 상태가 아닙니다. 동일한 URI를 사용하여 게시 환경에서 동일한 페이지에 액세스할 수 있습니다.예를 들면 다음과 같습니다 `https://localhost:4503/etc/replication/agents.publish/flush.html`.
