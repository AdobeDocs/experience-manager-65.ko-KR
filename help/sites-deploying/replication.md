...
>
>여러 게시 인스턴스의 경우 [사용자 동기화](/help/sites-administering/sync.md)가 활성화되면 사용자 데이터가 Sling이 배포됩니다.
...
* 게시 환경의 서블릿은 요청을 받고 수신된 컨텐트를 게시합니다.기본 서블릿은 `https://localhost:4503/bin/receive`입니다.
...
AEM [Communities](/help/communities/overview.md)에서는 UGC에 대한 복제를 사용하지 않습니다. 대신, 커뮤니티를 배포하려면 UGC에 대한 공용 저장소가 필요합니다([커뮤니티 콘텐츠 저장소](/help/communities/working-with-srp.md) 참조).

### 복제 - 박스 밖 {#replication-out-of-the-box}

AEM의 표준 설치에 포함된 Adobe-소매 웹 사이트를 사용하여 복제를 표현할 수 있습니다.

이 예를 따르고 기본 복제 에이전트를 사용하려면 다음과 함께 [AEM](/help/sites-deploying/deploy.md)을(를) 설치해야 합니다.
...
#### 복제 에이전트 - 박스 밖 {#replication-agents-out-of-the-box}
...
* 정적 에이전트
이는 &quot;노드의 정적 표현을 파일 시스템에 저장하는 에이전트&quot;입니다.
예를 들어 기본 설정을 사용하는 경우 콘텐츠 페이지와 dam 에셋은 `/tmp` 아래에 HTML 또는 적절한 에셋 형식으로 저장됩니다. 구성에 대해서는 `Settings` 및 `Rules` 탭을 참조하십시오.
...
* **에이전트 사용자 ID**
   ...
   * 게시 환경에서 컨텐츠 만들기 및 쓰기
   ...
   >[!CAUTION]
   >
   >게시 환경의 에이전트의 경우 이 계정 *은(는) 컨텐츠를 복제하는 데 필요한 만들기/쓰기 액세스 권한을 가져야 합니다.*
...
* **별칭 업데이트**
   ...
   이 옵션을 선택하면 Dispatcher에 대한 별칭 또는 별칭 경로 무효화 요청이 활성화됩니다. 또한 [Dispatcher Flush 에이전트 구성](/help/sites-deploying/replication.md#configuring-a-dispatcher-flush-agent)을 참조하십시오.
...
* **URI**
   ...
   * Dispatcher Flush 에이전트는 `https://localhost:8000/dispatcher/invalidate.cache`에 복제할 수 있습니다.
   ...
   Dispatcher Flush 에이전트의 경우 URI 속성은 경로 기반 가상 호스트 항목을 사용하여 팜 간 구별하는 경우에만 사용되며 이 필드를 사용하여 팜을 대상으로 하여 무효화합니다. 예를 들어 팜 #1에는 가상 호스트가 `www.mysite.com/path1/*`이고 팜 #2에는 가상 호스트가 `www.mysite.com/path2/*`입니다. `/path1/invalidate.cache`의 URL을 사용하여 첫 번째 팜을 타깃팅하고 `/path2/invalidate.cache`을 사용하여 두 번째 팜을 타깃팅할 수 있습니다.
...
MSSL을 사용하여 게시 인스턴스에 복제 에이전트를 연결하는 방법에 대한 자세한 내용은 [상호 SSL](/help/sites-deploying/mssl-replication.md)을 사용하여 복제를 참조하십시오.

### 작성 환경에서 복제 에이전트 구성 {#configuring-your-replication-agents-from-the-author-environment}

작성 환경의 도구 탭에서 작성 환경(**의 에이전트) 또는 게시 환경(**&#x200B;게시 환경의 에이전트(**)에 있는 복제 에이전트를 구성할 수 있습니다.** 다음 절차는 작성 환경에 대한 에이전트의 구성을 설명하지만 두 가지 모두를 사용할 수 있습니다.

>[!NOTE]
>
>디스패처가 작성자 또는 게시 인스턴스에 대한 HTTP 요청을 처리할 때 복제 에이전트의 HTTP 요청에는 PATH 헤더가 포함되어야 합니다. 다음 절차 외에 클라이언트 헤더의 발송자 목록에 PATH 헤더를 추가해야 합니다. ([/clientheaders (Client Headers)](https://helpx.adobe.com/experience-manager/dispatcher/using/dispatcher-configuration.html#specifying-the-http-headers-to-pass-through-clientheaders) 참조. [](https://helpx.adobe.com/experience-manager/dispatcher/using/dispatcher-configuration.html#specifying-the-http-headers-to-pass-through-clientheaders)


1. AEM의 **도구** 탭에 액세스합니다.
1. **Replication**(폴더를 열려면 왼쪽 창)을 클릭합니다.
...
보안상의 이유로 대부분의 네트워크 토폴로지는 외부 서비스를 인터넷과 같은 신뢰할 수 없는 네트워크에 노출하는 하위 네트워크인&#x200B;*의 연결*&#x200B;을 허용하지 않습니다.

게시 환경은 일반적으로 DMZ에 있으므로 컨텐츠를 다시 작성 환경으로 가져오려면 작성 인스턴스에서 연결을 시작해야 합니다. 이 작업은 다음을 통해 수행됩니다.

* 컨텐츠가 배치된 게시 환경의 *outbox*&#x200B;입니다.
* 새 컨텐츠에 대해 주기적으로 게시되는 작성자 환경의 에이전트(게시)입니다.

>[!NOTE]
>
>AEM [Communities](/help/communities/overview.md)의 경우 게시 인스턴스에서 사용자가 생성한 컨텐츠에 대한 복제가 사용되지 않습니다. [커뮤니티 콘텐츠 저장소](/help/communities/working-with-srp.md)를 참조하십시오.

이를 위해서는 다음을 수행해야 합니다.

**작성** 환경의 역방향 복제 에이전트게시 환경의 보낸 편지함에서 정보를 수집하는 활성 구성 요소 역할을 합니다.
...
1. **편집**&#x200B;을 클릭합니다. - **에이전트 설정** 대화 상자가 열립니다. **직렬화 유형**&#x200B;이 이미 기본값으로 정의되어 있습니다. 이 작업은 그대로 남아 있어야 합니다.
...
문제가 발생하면 작성 인스턴스의 로그를 확인할 수 있습니다. 필요한 세부 정보에 따라 위와 같이 **에이전트 설정** 대화 상자를 사용하여 **로그 수준**&#x200B;을 `Debug`로 설정할 수도 있습니다.

>[!NOTE]
>
>개별 게시 환경에 복제할 다른 컨텐츠를 선택하는 데 [에이전트 사용자 ID](#agentuserid)를 사용하는 것과 함께 사용할 수 있습니다. 각 게시 환경의 경우:
>
>1. 해당 게시 환경에 복제할 복제 에이전트를 구성합니다.
>1. 사용자 계정 구성;특정 게시 환경에 복제될 컨텐츠를 읽는 데 필요한 액세스 권한
>1. 사용자 계정을 복제 에이전트의 **에이전트 사용자 ID**&#x200B;로 지정합니다.
...
1. **편집**&#x200B;을 클릭합니다. **에이전트 설정** 대화 상자가 열립니다.
   ...
   * **설정** 탭에서 다음을 수행합니다.
      ...
      * **활성화됨**&#x200B;을 활성화합니다.
      * **설명**&#x200B;을 입력합니다.
      * **직렬화 유형**&#x200B;을 `Dispatcher Flush`으로 두거나 새 에이전트를 만드는 경우 이렇게 설정합니다.
...
1. **도구** 탭으로 돌아오면 **Dispatcher Flush** 에이전트(**의 에이전트)를** Activate&lt;a4/>할 수 있습니다.****
...
두 `agents` 은 해당 환경에 대한 구성 정보를 보유하며 해당 환경이 실행 중일 때만 활성화됩니다. 예를 들어 `agents.publish`은 게시 환경에서만 사용됩니다. 다음 스크린샷은 AEM WCM에 포함된 작성 환경의 게시 에이전트를 보여줍니다.
...
1. 왼쪽 탐색 레일에서 **[!UICONTROL Replication - Agents on Author]**&#x200B;로 이동하고 **[!UICONTROL 기본 에이전트]**&#x200B;을 두 번 클릭합니다.
   * 기본 게시 복제 에이전트에도 연결할 수도 있습니다. 이 에이전트는 `http://serveraddress:serverport/etc/replication/agents.author/publish.html`(으)로 바로 이동합니다
...
[htmlhttps://helpx.adobe.com/experience-manager/kb/ReplicationFailureAfterNewIP.](https://helpx.adobe.com/experience-manager/kb/PagePermissionsNotReplicatedWithUser.html)
