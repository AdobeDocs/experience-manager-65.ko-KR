...
* **HTTP 메서드**
  ...
  Dispatcher 플러시 에이전트의 경우, 이는 거의 항상 GET이며 변경해서는 안 됩니다(POST는 다른 가능한 값일 수 있음).
...
게시 환경은 일반적으로 DMZ에 있으므로 콘텐츠를 다시 작성자 환경으로 가져오려면 작성자 인스턴스에서 연결을 시작해야 합니다. 이 작업은 다음 작업을 통해 수행됩니다.
...
>[!NOTE]
>
>AEM [Communities](/help/communities/overview.md)의 경우 게시 인스턴스의 사용자 생성 컨텐츠에 복제가 사용되지 않습니다. [커뮤니티 콘텐츠 저장소](/help/communities/working-with-srp.md)를 참조하세요.
...
**게시 환경의 역방향 복제 에이전트(보낼 편지함)** - &quot;보낼 편지함&quot; 역할을 하는 수동 요소입니다. 사용자 입력은 여기에 배치되는데, 여기서 작성자 환경의 에이전트에 의해 수집됩니다.
...
>1. 특정 게시 환경에 복제되는 콘텐츠를 읽는 데 필요한 액세스 권한으로 사용자 계정을 구성합니다.
...
작성자에서 **Dispatcher 플러시** 복제 에이전트가 활성화되지 않았습니다. 동등한 URI를 사용하여 게시 환경의 동일한 페이지에 액세스할 수 있습니다(예: `https://localhost:4503/etc/replication/agents.publish/flush.html`).
...
일괄 복제는 개별 페이지 또는 자산을 복제하지 않습니다. 시간 또는 크기에 따라 두 개의 첫 번째 임계값이 트리거될 때까지 기다립니다.

그런 다음 모든 복제 항목을 패키지에 압축하고, 단일 파일로 게시자에게 복제합니다.

Publisher는 모든 항목의 압축을 풀고 저장한 다음 작성자에게 다시 보고합니다.
