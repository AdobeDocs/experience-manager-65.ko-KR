...
개발 환경에 대해 AEM 서버에 임베드되도록 Solr 인덱스를 구성하거나 원격 인스턴스로 오프로드하여 프로덕션 및 스테이징 환경에서 검색 확장성을 개선할 수 있습니다. 오프로딩 검색은 확장성을 향상시키지만 이로 인해 지연을 도입하므로 필요한 경우가 아니면 권장되지 않습니다. Solr 통합을 구성하는 방법 및 Solr 인덱스를 만드는 방법에 대한 자세한 내용은 [Oak 쿼리 및 색인화 설명서](/help/sites-deploying/queries-and-indexing.md#the-solr-index)를 참조하세요.
...
효율적인 JCR 쿼리 및 색인 정의를 만들 수 있도록 [JCR 쿼리 치트 시트](assets/JCR_query_cheatsheet-v1.1.pdf)를 다운로드하여 개발 중 참조할 수 있습니다. 여기에는 쿼리 성능이 다양한 여러 시나리오를 포괄하는 QueryBuilder, XPath, SQL-2에 대한 샘플 쿼리가 포함되어 있습니다. 이 치트시트는 또한 Oak 색인을 구축하거나 사용자 정의하는 방법에 대한 권장 사항도 제공합니다. 이 치트 시트의 콘텐츠는 AEM 6.5 및 AEM as a Cloud Service에 적용됩니다.

## 리인덱싱 {#re-indexing}

이 섹션에서는 Oak 색인을 다시 색인화하는 **only** 허용 이유에 대해 간략히 설명합니다.

아래에 설명된 이유 외에, Oak 인덱스 리인덱스를 시작하면 동작이 변경되거나 문제가 해결되고, AEM에서 로드가 불필요하게 증가합니다. **아니요**.
...
아래 설명된 기준과 일치하지 않는 AEM에서 문제가 발생하는 경우 문제를 해결하지 않으므로 **색인을 다시 색인화하지 마십시오**.
...
>[!NOTE]
>
>AEM 6.5에서 [oak-run.jar는 MongoMK 또는 RDBMK 리포지토리에 대한 리인덱싱에 대해 유일하게 지원되는 메서드](/help/sites-deploying/indexing-via-the-oak-run-jar.md#reindexingapproachdecisiontree)입니다.
...
>[!NOTE]
>
>이전 섹션에서는 AEM 컨텍스트에서 [Apache Oak 설명서](https://jackrabbit.apache.org/oak/docs/query/indexing.html#reindexing)의 Oak 리인덱싱 지침을 요약하고 프레임화합니다.
...
* Oak 텍스트 사전 추출은 배포 가능한 텍스트(예: PDF Oak, Word 문서, PPT, TXT 등)가 포함된 대량의 파일(바이너리)이 있는 리포지토리에서 Lucene 인덱스(예: `/oak:index/damAssetLucene`)를 다시 인덱싱/인덱싱하는 데 권장됩니다.
...
* 리포지토리의 **모두** 후보 콘텐츠 처리를 리인덱싱합니다. 전체 텍스트를 추출할 바이너리가 많거나 복잡하면 전체 텍스트 추출을 수행하는 데 드는 향상된 계산 부담이 AEM에 있습니다. 텍스트 사전 추출은 텍스트 추출의 &quot;계산적으로 비용이 많이 드는 작업&quot;을 AEM의 데이터 저장소에 직접 액세스하는 격리된 프로세스로 이동하여 AEM에서의 오버헤드와 리소스 경합을 방지합니다.

이진 추출이 활성화된 AEM에 **new** lucene 인덱스 배포를 지원합니다.

* 이진 추출이 활성화된 새 색인이 AEM에 배포되면 Oak은 다음에 비동기 전체 텍스트 색인 실행을 위해 모든 후보 콘텐츠를 자동으로 색인화합니다. 위의 리인덱싱에서 설명한 것과 동일한 이유로 인해 AEM에 과도한 로드가 발생할 수 있습니다.
...
*단계 2(a-c)는 데이터 저장소와만 상호 작용하므로 AEM의 정상 작동 중에 실행될 수 있습니다.*
...
미리 추출된 텍스트는 바이너리 지문에 의해 CSV로 식별된다. 이진 파일이 동일한 경우 AEM 인스턴스 간에 동일한 미리 추출된 텍스트를 사용할 수 있습니다. AEM Publish은 일반적으로 AEM Author의 하위 집합이므로, AEM Author에서 미리 추출된 텍스트는 종종 AEM Publish을 다시 인덱싱하는 데 사용할 수 있습니다(AEM Publish에 추출된 텍스트 파일에 대한 파일 시스템 액세스 권한이 있다고 가정).
...
3a. Lucene 인덱스 [리인덱싱](#how-to-re-index)이(가) AEM에서 호출됩니다.
