...
이 접근 방식의 단점은 기본적으로 AEM 쿼리가 ACL을 준수하므로 사용자가 액세스할 수 없는 결과가 숨겨지지만 Solr 서버에 검색을 외부화하면 이 기능이 지원되지 않는다는 것입니다. 이러한 방식으로 검색이 외부화되려면 사용자가 보지 말아야 할 결과를 제시하지 않도록 각별한 주의가 필요합니다.
