...
>[!NOTE]
>
>Apache Oak 및 AEM 지속형의 주요 개념에 대한 자세한 내용은 [AEM 플랫폼 소개](/help/sites-deploying/platform.md)를 참조하십시오.
...
외부 Blob 저장소(일반적으로 데이터 저장소라고 함)를 사용한 저장소 마이그레이션에 대한 지원이 다른 조합으로 제공됩니다. 가능한 마이그레이션 경로는 외부 `FileDataStore`을 사용하는 CRX2 저장소에서 `S3DataStore`을 사용하여 Oak 저장소로 이동하는 것입니다.
...
사용자 지정 Java 논리 및 `CommitHooks`을(를) 사용하여 구현할 수도 있습니다. 사용자 지정 `RepositoryInitializer` 클래스는 사용자 지정 값으로 저장소를 초기화하기 위해 구현할 수 있습니다.
...
#### 컨텐츠 {#selective-migration-of-content} 선택적 마이그레이션
...
두 저장소 사이에 데이터를 복사해야 하고 두 인스턴스에 서로 다른 컨텐츠 경로가 있는 경우 `--merge-path` 매개 변수에서 해당 경로를 정의할 수 있습니다. CRX2Oak는 새 노드만 대상 저장소에 복사하고 기존 노드를 그대로 유지합니다.
...
이러한 유형의 경우 `--copy-versions` 매개 변수가 추가되어 유용합니다. 저장소를 마이그레이션하거나 복사하는 동안 버전 노드를 건너뛸 수 있습니다.

`--copy-orphaned-versions=true`을(를) 추가하여 고아 버전을 복사할지 여부를 선택할 수도 있습니다.
...
<table>
 <tbody>
  ...
  <tr>
   <td>빠른 시작 모드</td>
   <td>CRX2Oak를 실행할 때 명령줄에 <strong>—log-level TRACE</strong> 또는 <strong>—log-level DEBUG </strong> 옵션을 추가할 수 있습니다. 이 모드에서는 로그가 자동으로 <strong>upgrade.log 파일</strong>로 리디렉션됩니다.</td>
  </tr>
  <tr>
   <td>독립 실행형 모드</td>
...
>[!NOTE]
>
>자세한 내용은 [Write Consists](https://docs.mongodb.org/manual/reference/connection-string/#write-concern-options)의 MongoDB 연결 문자열 설명서를 참조하십시오.
