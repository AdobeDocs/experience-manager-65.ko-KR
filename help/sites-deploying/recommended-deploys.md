...
AEM Communities의 경우 UGC(사용자 생성 컨텐츠)는 복제되지 않습니다. TarMK 팜에서 UGC를 지원하는 방법은 [AEM Communities 고려 사항](#considerations-for-aem-communities)을 참조하십시오.
...
>[!NOTE]
>
>위의 다이어그램에서 AEM Server 3 및 AEM Server 4는 데이터 센터 2의 AEM 서버와 데이터 센터 1의 MongoDB 주 노드 사이에서 [여기](/help/sites-deploying/aem-with-mongodb.md#checklists)에 설명된 요구 사항보다 높은 네트워크 지연을 가정하는 비활성 상태로 표시됩니다. 예를 들어 가용성 영역을 사용하여 최대 지연 시간이 요구 사항과 호환되는 경우 데이터 센터 2의 AEM 서버도 활성 상태가 되어 여러 데이터 센터에 걸쳐 활성 상태의 AEM 클러스터가 생성될 수 있습니다.
...
>[!NOTE]
>
>Tough Day는 배포된 하드웨어 구성 맥락에서 고객 애플리케이션의 성능을 평가하는 데 사용할 수 있습니다. 이 도구에 대한 자세한 내용은 [여기](/help/sites-developing/tough-day.md)를 참조하세요.
...
* Adobe Experience Manager [article](https://www.mongodb.com/lp/contact/mongodb-adobe-experience-manager)에 대한 MongoDB를 참조하십시오.
* MongoDB 프로덕션 [검사 목록](https://docs.mongodb.org/manual/administration/production-checklist/)을 검토합니다.
* 온라인 [여기](https://university.mongodb.com/)에서 사용 가능한 MongoDB의 인증 수업에 참석하십시오.
...
[AEM Communities](/help/communities/overview.md)을(를) 배포하려는 사이트의 경우 게시 환경에서 커뮤니티 구성원이 게시한 UGC를 처리하기에 최적화된 [배포를 선택](/help/communities/working-with-srp.md#characteristicsofstorageoptions)하는 것이 좋습니다.

[일반 저장소](/help/communities/working-with-srp.md)를 사용하면 작성자와 다른 게시 인스턴스 간에 UGC를 복제하지 않아도 일관된 UGC 보기를 확인할 수 있습니다.
