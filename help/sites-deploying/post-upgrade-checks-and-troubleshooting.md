...
업그레이드 중에 AEM은 사용자 지정을 백업하고 `/var/upgrade/PreUpgradeBackup/<time-stamp-of-upgrade>` 아래에 저장하려고 합니다. CRXDE Lite에서 이 폴더를 보려면 [일시적으로 CRXDE Lite을 사용](/help/sites-administering/enabling-crxde-lite.md)해야 할 수 있습니다.
...
이 섹션에는 AEM 6.3으로 업그레이드하는 과정에서 발생할 수 있는 몇 가지 문제 시나리오가 포함되어 있습니다.
...
또한 새로운 AEM 6.5 인스턴스의 번들 목록을 업그레이드된 인스턴스와 비교하여 업그레이드되지 않은 번들을 감지하는 것이 좋습니다. 이를 통해 `error.log`에서 검색할 내용의 범위가 더 넓어집니다.
...
제거된 API는 이전 릴리스 중 하나에서 더 이상 사용되지 않는 것으로 표시되어야 합니다. 이 사용 중단 알림에서 코드의 직접 마이그레이션에 대한 지침을 찾을 수 있습니다. Adobe은 가능한 경우 버전 변경 사항을 나타내는 시맨틱 버전 관리를 목표로 합니다.
