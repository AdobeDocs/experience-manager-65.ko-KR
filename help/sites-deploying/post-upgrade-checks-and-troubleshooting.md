...
* 업그레이드 프레임워크에서 감지된 업그레이드 실패는 단일 업그레이드 보고서에서 중앙 집중화될 수 있습니다.
* 업그레이드 보고서에는 필요한 수동 개입에 대한 지표가 포함됩니다.

이를 충족하기 위해 의 로그 생성 방식이 변경되었습니다. `upgrade.log` 파일.
...
error.log는 target 버전 jar를 사용하여 AEM을 시작하는 동안 및 후에 신중하게 검토해야 합니다. 모든 경고 또는 오류를 검토해야 합니다. 일반적으로 로그 시작 부분에서 문제를 찾는 것이 가장 좋습니다. 로그 후반부에 발생하는 오류는 실제로 파일의 초기에 호출되는 근본 원인의 부작용일 수 있습니다. 반복되는 오류 및 경고가 발생하는 경우 아래를 참조하십시오. [업그레이드 관련 문제 분석](/help/sites-deploying/post-upgrade-checks-and-troubleshooting.md#analyzing-issues-with-the-upgrade).

### OSGi 번들 확인 {#verify-osgi-bundles}

OSGi 콘솔로 이동합니다. `/system/console/bundles` 번들이 시작되지 않았는지 확인하십시오. 설치된 상태의 번들이 있는 경우 `error.log` 을 클릭하여 루트 문제를 확인합니다.

### Oak 버전 확인 {#verify-oak-version}

업그레이드 후 Oak 버전이 **1.10.2**. Oak 버전을 확인하려면 OSGi 콘솔로 이동하여 Oak Core, Oak Commons, Oak Segment Tar 등 Oak 번들과 연결된 버전을 확인합니다.

### Inspect PreUpgradeBackup 폴더 {#inspect-preupgradebackup-folder}

업그레이드 중에 AEM은 사용자 정의 항목을 백업하고 아래에 저장합니다. `/var/upgrade/PreUpgradeBackup/<time-stamp-of-upgrade>`. CRXDE Lite에서 이 폴더를 보려면 다음을 수행해야 합니다. [일시적으로 CRXDE Lite 활성화](/help/sites-administering/enabling-crxde-lite.md).

타임스탬프가 있는 폴더에는 이름이 인 속성이 있어야 합니다. `mergeStatus` (값: `COMPLETED`. 다음 **처리 중** 폴더는 비어 있어야 하며 **덮어씀** 노드는 업그레이드 중에 덮어쓴 노드를 나타냅니다. 아래에 있는 콘텐츠 **남은 음식** 노드는 업그레이드 중에 안전하게 병합할 수 없는 콘텐츠를 나타냅니다. 구현이 하위 노드에 종속되어 있고 업그레이드된 코드 패키지로 아직 설치되지 않은 경우 이들 노드는 수동으로 병합해야 합니다.

스테이지 또는 프로덕션 환경에서 이 연습을 따라 CRXDE Lite을 비활성화합니다.

### 페이지의 초기 유효성 검사 {#initial-validation-of-pages}

AEM의 여러 페이지에 대해 초기 유효성 검사를 수행합니다. 작성자 환경을 업그레이드하는 경우 시작 페이지 및 시작 페이지( `/aem/start.html`, `/libs/cq/core/content/welcome.html`). 작성자 및 게시 환경 모두에서 몇 개의 애플리케이션 페이지를 열고 올바르게 렌더링하는 스모크 테스트를 실행합니다. 문제가 발생하면 `error.log` 문제 해결
...
파일 데이터 저장소를 사용하는 경우 데이터 저장소 가비지 수집 작업이 활성화되어 있고 주간 유지 관리 목록에 추가되어 있는지 확인합니다. 지침이 요약되어 있습니다 [여기](/help/sites-administering/data-store-garbage-collection.md).
...
MongoMK 또는 새 TarMK 세그먼트 형식을 사용하는 경우 개정 정리 작업이 사용으로 설정되고 일별 유지 관리 목록에 추가되었는지 확인합니다. 요약된 지침 [여기](/help/sites-deploying/revision-cleanup.md).
...
CRX2에서 Oak로 데이터를 마이그레이션하는 방법은 CQ 5.4 기반의 소스 인스턴스로 시작하는 모든 시나리오에 적합해야 합니다. 이 문서의 업그레이드 지침에 따라 다음을 준비해야 합니다. `repository.xml`, JAAS를 통해 시작된 사용자 지정 사용자 지정 인증자가 없는지, 마이그레이션을 시작하기 전에 인스턴스가 불일치하는지 확인했습니다.

마이그레이션에 여전히 실패할 경우 다음을 검사하여 근본 원인을 파악할 수 있습니다. `upgrade.log`. 문제를 아직 알 수 없는 경우 고객 지원 센터에 보고하십시오.

### 업그레이드가 실행되지 않았습니다. {#the-upgrade-did-not-run}

준비 단계를 시작하기 전에 **소스** 인스턴스: 먼저 java -jar aem-quickstart.jar 명령으로 실행합니다. 이 작업은 quickstart.properties 파일이 제대로 생성되었는지 확인하는 데 필요합니다. 누락된 경우 업그레이드가 작동하지 않습니다. 또는 아래를 보고 파일이 있는지 확인할 수 있습니다. `crx-quickstart/conf` 소스 인스턴스의 설치 폴더에 있습니다. 또한 업그레이드를 시작하기 위해 AEM을 시작할 때 java -jar aem-quickstart.jar 명령을 사용하여 실행해야 합니다. 시작 스크립트에서 시작하는 것은 업그레이드 모드에서 AEM을 시작하지 않습니다.

### 패키지 및 번들 업데이트 실패  {#packages-and-bundles-fail-to-update-}

업그레이드 중에 패키지를 설치하지 못하는 경우 패키지가 포함된 번들도 업데이트되지 않습니다. 이 범주의 문제는 일반적으로 데이터 저장소의 잘못된 구성으로 인해 발생합니다. 또한 다음과 같이 표시됩니다. **오류** 및 **경고** error.log의 메시지. 이러한 대부분의 경우 기본 로그인이 작동하지 않을 수 있으므로 구성 문제를 검사하고 찾기 위해 CRXDE를 직접 사용할 수 있습니다.

### 일부 AEM 번들이 활성 상태로 전환되지 않음 {#some-aem-bundles-are-not-switching-to-the-active-state}

번들이 시작되지 않는 경우 충족되지 않은 종속성이 있는지 확인해야 합니다.
...
문제의 원인이 된 변화가 절대적으로 필요했는지를 확인하고 그렇지 않다면 되돌리는 것도 최선이다. 또한 엄격한 의미 체계 버전 관리를 따라 패키지 내보내기의 버전 증가가 필요 이상으로 증가했는지 확인하십시오.

### 잘못된 플랫폼 UI {#malfunctioning-platform-ui}

업그레이드 후 제대로 작동하지 않는 특정 UI 기능의 경우 먼저 인터페이스의 사용자 지정 오버레이를 확인해야 합니다. 일부 구조가 변경되었을 수 있으며 오버레이는 업데이트가 필요하거나 더 이상 사용되지 않을 수 있습니다.
...
대부분의 경우 이러한 문제의 근본 원인은 시작되지 않은 번들 또는 패키지가 설치되지 않은 번들과 동일하며, 구성 요소를 처음 사용할 때 문제가 시작되는 유일한 차이점입니다.
