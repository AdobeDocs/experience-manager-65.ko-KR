...
Adobe은 이 문서의 다음 정보에 자세히 설명된 대로 플랫폼에서 Adobe Experience Manager(AEM)를 지원합니다.
...
<table>
 <tbody>
  ...
  <tr>
   ...
   <td>고객 프로젝트의 성공을 보장하기 위해 Adobe은 제한된 지원 프로그램 내에서 모든 지원을 제공하며 특정 조건이 충족되어야 합니다. R 레벨 지원에는 공식적인 고객 요청 및 Adobe 확인이 필요합니다. 자세한 내용은 Adobe 고객 지원 센터에 문의하십시오.</td>
  </tr>
 </tbody>
</table>
...
응용 프로그램을 실행하려면 JDK(Java Development Kit) 배포에서 제공하는 Java Virtual Machine이 필요합니다.

Adobe Experience Manager은 다음 버전의 Java Virtual Machine과 함께 작동합니다.

>[!CAUTION]
>
>프로덕션 환경의 안전과 보안을 보장하고 최신 Java 업데이트를 설치하기 위해 Java 공급업체에서 보안 게시판을 추적하는 것이 좋습니다.
...
1. Oracle은 Oracle Java SE 제품에 대한 &quot;장기 지원&quot;(LTS) 모델로 이동되었습니다. Java 9, Java 10 및 Java 12는 Oracle에서 LTS가 아닌 릴리스입니다(참조) [Oracle Java SE 지원 로드맵](https://www.oracle.com/technetwork/java/eol-135779.html)). 프로덕션 환경에 AEM을 배포하기 위해 Adobe은 Java의 LTS 릴리스에 대해서만 지원을 제공합니다. 공개 업데이트의 종료 시점을 넘어 LTS 릴리스의 모든 유지 관리 업데이트를 포함하여 Oracle Java SE JDK의 지원 및 배포는 Oracle Java SE 기술을 사용하는 모든 AEM 고객을 위해 Adobe이 직접 지원합니다. 다음을 참조하십시오. [Adobe Experience Manager에 대한 Java 지원 정책](assets/Java_Policy_for_Adobe_Experience_Manager.pdf) 추가 정보.


1. IBM JRE는 WebSphere Application Server와 함께만 지원됩니다.

1. Azul Zulu OpenJDK LTS 버전은 버전 6.5 SP9부터 온-프레미스 AEM 배포에 대해 지원됩니다. Azul Zulu JDK LTS 버전의 지원 및 배포는 고객이 Azul에서 직접 라이선스를 받아야 합니다.
...
| MongoDB Enterprise 4.4 | 저장소 | A: 지원됨 `[2, 3, 4]` |
...
| Microsoft Server 2016 | Forms 데이터베이스 | A: 지원됨 |
| **Apache Lucene(Quickstart 내장)** | 검색 서비스 | A: 지원됨 |
| Apache Solr | 검색 서비스 | A: 지원됨 |

1. &#39;파일 시스템&#39;에는 POSIX와 호환되는 블록 저장소가 포함되어 있습니다. 여기에는 네트워크 스토리지 기술이 포함됩니다. 파일 시스템 성능이 달라질 수 있으며 전체 성능에 영향을 줄 수 있습니다. 네트워크/원격 파일 시스템과 함께 테스트 AEM을 로드하는 것이 좋습니다.
...
>
>Adobe 고객 지원 센터에서 AEM과의 MongoDB 사용과 관련된 자격 조건을 충족하는 문제를 지원합니다.
>
...
>
>지원되는 관계형 데이터베이스로 AEM 6.5를 실행하려면 데이터베이스 공급업체와의 별도의 지원 계약이 필요합니다. Adobe 고객 지원 센터는 AEM 6.5의 관계형 데이터베이스 사용과 관련된 자격 문제를 지원합니다.
>
...
| Debian 배포 기반의 Linux에는 다음이 포함됩니다. 우분투 | A: 지원됨 `[1]` `[2]` |
| Linux, SUSE 배포 기반 | A: 지원됨 `[1]` |
| Microsoft 윈도우 서버 2019 `[4]` | R: 신규 계약에 대한 제한된 지원 `[5]` |
...
1. Linux 커널 2.6, 3. x, 4. x와 5. x에는 Red Hat Enterprise Linux, CentOS, Oracle Linux 및 Amazon Linux를 비롯한 Red Hat 배포판의 파생물이 포함되어 있습니다. AEM Forms 추가 기능 기능은 CentOS 7, Red Hat Enterprise Linux 7, Red Hat Enterprise Linux 8 및 Red Hat Enterprise Linux 9에서만 지원됩니다.
...
Adobe Experience Manager은 이 페이지에 나열된 기술 요구 사항과 Adobe의 표준 지원 약관에 따라 Microsoft Azure 및 Amazon Web Services(AWS)와 같은 클라우드 컴퓨팅 환경의 가상 컴퓨터에서 실행될 수 있습니다.

클라우드 기반 환경의 경우 AEM 제품 라인의 최신 오퍼링을 검토하십시오. Adobe Experience Manager as a Cloud Service 다음을 참조하십시오 [Adobe Experience Manager as a Cloud Service 설명서](https://experienceleague.adobe.com/docs/experience-manager-cloud-service.html?lang=en) 을 참조하십시오.

또한 Adobe은 Azure 또는 AWS에 AEM을 배포할 수 있는 Adobe Managed Services를 제공합니다. Adobe Managed Services는 이러한 클라우드 컴퓨팅 환경에서 AEM을 배포하고 운영하는 경험과 기술을 전문가에게 제공합니다. 다음을 참조하십시오 [adobe Managed Services에 대한 추가 설명서](https://www.adobe.com/marketing-cloud/enterprise-content-management/managed-services-cloud-platform.html?aemClk=t).

Azure 또는 AWS 또는 기타 클라우드 컴퓨팅 환경에서 AEM을 배포하는 다른 모든 경우에는 이 페이지에 나열된 기술 사양을 준수하여 Adobe의 지원이 가상 컴퓨팅 환경에 포함됩니다. Azure Blob Storage 또는 AWS S3 등 이 페이지에 나열된 기술 요구 사항의 일부로 클라우드 서비스가 특별히 지원되지 않는 한, 이러한 클라우드 환경에서 실행되는 AEM과 관련하여 보고된 문제는 클라우드 컴퓨팅 환경과 관련된 모든 클라우드 서비스와 별도로 재현할 수 있어야 합니다.

Adobe Managed Services 이외의 Azure 또는 AWS에 AEM을 배포하는 방법에 대한 권장 사항의 경우, Adobe은 선택한 클라우드 환경에서 AEM 배포를 지원하는 클라우드 공급자 또는 Adobe 파트너와 직접 작업하는 것을 강력히 권장합니다. 선택한 클라우드 공급업체 또는 파트너는 특정 성능, 로드, 확장성 및 보안 요구 사항을 충족하도록 아키텍처 크기 조정 사양, 설계 및 구현을 담당합니다.

### Dispatcher 플랫폼(웹 서버) {#dispatcher-platforms-web-servers}

Dispatcher는 캐싱 및 로드 밸런싱 구성 요소입니다. [최신 Dispatcher 버전 다운로드](https://helpx.adobe.com/experience-manager/dispatcher/release-notes.html). Experience Manager 6.5를 사용하려면 Dispatcher 버전 4.3.2 이상이 필요합니다.
...
1. Apache httpd 소스 코드를 기반으로 구축된 웹 서버는 기반이 되는 httpd 버전과 동일한 수준의 지원을 받게 됩니다. 확실하지 않은 경우 Adobe에게 각 서버 제품과 관련된 지원 수준을 확인하도록 요청하십시오. 다음의 경우:
...
AEM 사용자 인터페이스는 더 큰 화면(일반적으로 노트북 및 데스크탑 컴퓨터)과 태블릿 폼 팩터(예: Apple iPad 또는 Microsoft Surface)에 최적화되어 있습니다. 휴대폰 폼 팩터는 지원되지 않습니다.
...
1. Firefox의 확장 지원 릴리스 [mozilla.org에서 자세히 알아보기](https://www.mozilla.org/en-US/firefox/organizations/faq/)
1. Apple iPad 지원

### 웹 사이트에 대해 지원되는 브라우저 {#supported-browsers-for-websites}

일반적으로 AEM Sites에서 렌더링하는 웹 사이트에 대한 브라우저 지원은 AEM 페이지 템플릿, 디자인 및 구성 요소 출력의 구현에 따라 다르며, 따라서 이러한 부분을 구현하는 당사자를 제어합니다.

### WebDAV 클라이언트 {#webdav-clients}

**Microsoft Windows 7+**

SSL로 보호되지 않는 AEM 인스턴스에 Microsoft Windows 7+를 연결하려면 Windows에서 비보안 네트워크를 통한 기본 인증을 활성화해야 합니다. 이 경우 WebClient의 Windows 레지스트리를 변경해야 합니다.
...
Windows에서 WebDav 클라이언트의 응답성을 개선하려면 - 을 참조하십시오. [Microsoft 지원 KB 2445570](https://support.microsoft.com/kb/2445570)
...
특별한 구성이 필요하지 않아 원활한 작업이 가능합니다. 필요한 경우 네트워크 유형에 적절한 형식을 사용하여 IP 주소를 지정할 수 있습니다.

즉, IP 주소를 지정해야 하는 경우 다음 중에서 선택할 수 있습니다(필요에 따라).

* IPv6 주소 예 `https://[ab12::34c5:6d7:8e90:1234]:4502`

* IPv4 주소 예 `https://123.1.1.4:4502`

* 예를 들어 서버 이름 `https://www.yourserver.com:4502`
...
>
>Dynamic Media - Scene7 모드를 실행하는 Dynamic Media 고객의 경우 (즉, **dynamicmedia_scene7** runmode), 추가 시스템 요구 사항은 없으며 AEM과 동일한 시스템 요구 사항만 있습니다. Dynamic Media - Scene7 모드 아키텍처는 AEM에 임베드된 서비스가 아닌 클라우드 기반 이미지 서비스를 사용합니다.
...
>[!NOTE]
>
>로케일이 LC_CTYPE이 다음과 같지 않도록 설정된 경우 `en_US.UTF-8`: Dynamic Media이 작동하지 않습니다. 해당 값이 무엇인지 보려면 명령 프롬프트에서 &quot;locale&quot;을 입력하십시오. 로 설정되지 않은 경우 AEM을 실행하기 전에 &quot;export LC_CTYPE=&quot;을 입력하여 LC_CTYPE 환경 변수를 빈 문자열로 설정합니다.
...
>[!NOTE]
>
>**NUMA 아키텍처** AMD64 및 Intel EM64T를 지원하는 프로세서가 장착된 시스템은 일반적으로 NUMA(Non-Uniform Memory Architecture) 플랫폼으로 구성되며, 이는 커널이 단일 메모리 노드를 구성하는 대신 부트 시 여러 메모리 노드를 구성함을 의미합니다.
>
...
>[!NOTE]
>
>**서버 호스트 이름을 확인해야 합니다.** 서버의 호스트 이름을 IP 주소로 확인할 수 있는지 확인합니다. 불가능한 경우 정규화된 호스트 이름과 IP 주소를 추가합니다. **/etc/hosts**:
>
...
* 다음에서 Microsoft Visual Studio 2010 재배포 가능 파일 가져오기 [https://www.microsoft.com/en-us/download/details.aspx?id=13523](https://www.microsoft.com/en-us/download/details.aspx?id=13523)
* 다음에서 Microsoft Visual Studio 2013 재배포 가능 파일 가져오기 [https://www.microsoft.com/en-us/download/details.aspx?id=40784](https://www.microsoft.com/en-us/download/details.aspx?id=40784)
* 다음에서 Microsoft Visual Studio 2015 재배포 가능 파일 가져오기 [https://www.microsoft.com/en-us/download/details.aspx?id=48145](https://www.microsoft.com/en-us/download/details.aspx?id=48145)

Windows x86의 경우:

* 다음에서 Microsoft Visual Studio 2010 재배포 가능 파일 가져오기 [https://www.microsoft.com/en-in/download/details.aspx?id=5555](https://www.microsoft.com/en-in/download/details.aspx?id=5555)
* 다음에서 Microsoft Visual Studio 2013 재배포 가능 파일 가져오기 [https://www.microsoft.com/en-in/download/details.aspx?id=40769](https://www.microsoft.com/en-in/download/details.aspx?id=40769)
* 다음에서 Microsoft Visual Studio 2015 재배포 가능 파일 가져오기 [https://www.microsoft.com/en-us/download/details.aspx?id=52685](https://www.microsoft.com/en-us/download/details.aspx?id=52685)
...
### PDF 생성기에 대한 소프트웨어 지원 {#software-support-for-pdf-generator}
...
>[!NOTE]
>
>PDF 생성기는 지원되는 운영 체제 및 애플리케이션의 영어, 프랑스어, 독일어 및 일본어 버전만 지원합니다.
>
>또한
>
>* PDF 생성기에 32비트 버전 필요 [Acrobat 2020 classic track 버전 20.004.30006](https://helpx.adobe.com/acrobat/release-note/release-notes-acrobat-reader.html) 또는 Acrobat 2017 버전 17.011.30078을 사용하여 변환을 수행할 수 있습니다.
>* OpenOffice용 PDF 생성기 변환은 Windows 및 Linux에서만 지원됩니다.
>* PDF 생성기는 Microsoft Office Professional Plus의 32비트 소매 버전 및 Windows 운영 체제에서 전환하는 데 필요한 기타 소프트웨어만 지원합니다.
>* PDF 생성기는 Linux 운영 체제에서 OpenOffice의 32비트 및 64비트 버전을 지원합니다.
>* PDF 생성기가 Microsoft Office 365를 지원하지 않습니다.
>* OCR PDF, Optimize PDF 및 Export PDF 기능은 Windows에서만 지원됩니다.
>* Acrobat 버전은 PDF 생성기 기능을 사용할 수 있도록 AEM Forms과 번들로 제공됩니다. 번들형 버전은 AEM Forms PDF 생성기와 함께 사용하기 위해 AEM Forms 라이선스 기간 동안 AEM Forms에서만 프로그래밍 방식으로 액세스해야 합니다. 자세한 내용은 배포에 따른 AEM Forms 제품 설명([On-Premise](https://helpx.adobe.com/legal/product-descriptions/adobe-experience-manager-on-premise.html) 또는 [Managed Services](https://helpx.adobe.com/legal/product-descriptions/adobe-experience-manager-managed-services.html))
>* PDF 생성기 서비스는 Microsoft Windows 10을 지원하지 않습니다.
>* PDF 생성기가 Microsoft Visio 2019를 사용하여 파일을 변환하지 못했습니다. Microsoft Visio 2016을 계속 사용하여 .VSD 및 .VSDX 파일을 변환할 수 있습니다.
>* PDF 생성기가 Microsoft Project 2019를 사용하여 파일을 변환하지 못했습니다. Microsoft Project 2016을 계속 사용하여 .VSD 및 .VSDX 파일을 변환할 수 있습니다.
>
...
* **운영 체제:**
   ...
   * Linux(64비트 시스템에서 32비트 및 32비트 애플리케이션 지원) 32비트 클라이언트 라이브러리를 설치하는 단계는 [64비트 RedHat Linux에서 XMP 추출 및 다시 쓰기를 활성화하는 방법](https://helpx.adobe.com/experience-manager/kb/enable-xmp-write-back-64-bit-redhat.html).
...
### AEM Assets이 Linux에서 메타데이터가 많은 에셋을 처리하기 위한 요구 사항 {#assetsonlinux}
