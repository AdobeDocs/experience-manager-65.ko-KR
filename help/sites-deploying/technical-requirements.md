...
| ® J9 VM - 빌드 2.9, JRE 1.8.0 | A: 지원되는 `[2]` |  |
| ® J9 VM - 빌드 2.8, JRE 1.8.0 | A: 지원되는 `[2]` |  |
| Azul Zulu OpenJDK 11 - 64비트 | A: 지원되는 `[3]` | |
| Azul Zulu OpenJDK 8 - 64비트 | A: 지원되는 `[3]` | |

1. Oracle이 Oracle Java™ SE 제품에 대한 &quot;장기 지원&quot;(LTS) 모델로 전환했습니다. Java™ 9, Java™ 10 및 Java™ 12는 Oracle에서 제공하는 비 LTS 릴리스입니다([Oracle Java™ SE 지원 로드맵](https://www.oracle.com/technetwork/java/eol-135779.html) 참조). 프로덕션 환경에 AEM을 배포하기 위해 Adobe은 Java™의 LTS 릴리스에 대해서만 지원을 제공합니다. LTS 릴리스의 모든 유지 관리 업데이트를 포함하여 Oracle Java™ SE JDK의 지원 및 배포는 Adobe Java™ SE 기술을 사용하는 모든 AEM 고객을 위해 Oracle에서 직접 지원합니다. Adobe Experience Manager에 대한 [Java™ 지원 정책](assets/Java_Policy_for_Adobe_Experience_Manager.pdf)을 참조하세요.   **중요: Oracle Java™ 17 및 21은 [AEM 6.5 LTS](https://experienceleague.adobe.com/ko/docs/experience-manager-65-lts/content/implementing/deploying/introduction/technical-requirements)에서 지원됩니다.**

1. ® JRE는 WebSphere® Application Server와만 지원됩니다.
...
| ® DB2® 10.5 | 저장소 및 Forms 데이터베이스 | R: 제한된 지원 `[5]` |
| Oracle 데이터베이스 12c (12.1.x) | 저장소 및 Forms 데이터베이스 | R: 제한된 지원 |
| Oracle 데이터베이스 19c | 저장소 및 Forms 데이터베이스 | R: 제한된 지원 |
| ® SQL Server 2016 | Forms 데이터베이스 | A: 지원됨 |
| ® SQL Server 2019(더 이상 사용되지 않음) | Forms 데이터베이스 | A: 지원됨 |
| ® SQL Server 2022 | Forms 데이터베이스 | A: 지원됨 |
...
6. AEM Forms에만 적용 가능:
   ...
   * ® SQL Server 2016에 대한 지원이 제거되었으며 Microsoft® SQL Server 2019 및 Microsoft® SQL Server 2022에 대한 지원이 추가되었습니다.
7. AEM Forms에서는 지원되지 않습니다.
...
| 웹 프로필 7.0 및 ® JRE 1.8을 사용한 IBM® WebSphere® Application Server Continuous Delivery(LibertyProfile) | R: 새 계약에 대한 지원이 제한됨 `[2]` |
| ® WebSphere® Application Server 9.0 및 IBM® JRE 1.8 | R: 새 계약에 대한 지원이 제한됨 `[1]` `[2]` |
| ® WebSphere® 응용 프로그램 서버 9.0.0.10 | R: 새 계약에 대한 지원이 제한됨 `[1]` `[2]` |
...
| JBoss® Application Server <sup>[2] [3]을(를) 사용하는 JBoss® EAP 7.4 | A: 지원됨 |
...
| Linux®, Ubuntu 포함 데비안 배포 기반 | A: 지원되는 `[1]` `[2]` |
| Linux®, SUSE® 배포 기반 | A: 지원되는 `[1]` |
| ® 윈도우 서버 2022 | R: 제한된 지원 |
| ® Windows Server 2019 `[4]`(더 이상 사용되지 않음) | R: 새 계약에 대한 지원이 제한됨 `[5]` |
| ® Windows Server 2016 `[4]` | R: 새 계약에 대한 지원이 제한됨 `[5]` |
| ® Windows Server 2012 R2 | Z: 지원되지 않음 |
...
4. ® Windows 프로덕션 배포는 6.5로 업그레이드하는 고객 및 비프로덕션 사용을 위해 지원됩니다. 새 배포는 AEM Sites 및 Assets에 대한 온디맨드 요청입니다.
...
>
>* ® Visual C++ 2008 재배포 가능
>* ® Visual C++ 2010 재배포 가능
>* ® Visual C++ 2012 재배포 가능
>* ® Visual C++ 2013 재배포 가능
>* ® Visual C++ 2019(VC14.28 이상) 재배포 가능

### 가상 및 클라우드 컴퓨팅 환경 {#virtual-cloud-computing-environments}

Adobe Experience Manager은 클라우드 컴퓨팅 환경의 가상 컴퓨터에서 실행될 수 있습니다. 이러한 환경에는 이 페이지에 나열된 기술 요구 사항 및 Adobe의 표준 지원 약관에 따라 실행되는 Microsoft® Azure 및 Amazon Web Services(AWS)가 포함됩니다.
...
Adobe Managed Services 외부의 Azure 또는 AWS에 AEM을 배포하는 방법에 대한 권장 사항을 보려면, Adobe에서 클라우드 공급자와 직접 작업하는 것이 좋습니다. 또는 선택한 클라우드 환경에서 AEM의 배포를 지원하는 Adobe 파트너와 협력합니다. 선택한 클라우드 공급업체 또는 파트너는 특정 성능, 로드, 확장성 및 보안 요구 사항을 충족하도록 아키텍처 크기 조정 사양, 설계 및 구현을 담당합니다.
...
| ® IIS 10(Internet Information Server) | A: 지원됨 |
| ® IIS 8.5(Internet Information Server) | Z: 지원되지 않음 |
...
1. Apache httpd 소스 코드를 기반으로 구축된 웹 서버는 기반이 되는 httpd 버전만큼 많은 지원을 제공합니다. 확실하지 않은 경우 Adobe에 각 서버 제품과 관련된 지원 수준을 확인하도록 요청하십시오. 다음과 같은 경우:
   ...
   1. HTTP 서버는 실행 중인 운영 체제의 일부로 전달되었습니다. 예: ® HTTP Server, Oracle HTTP Server
...
® Windows 7+를 SSL로 보호되지 않는 AEM 인스턴스에 연결할 경우 Windows에서 비보안 네트워크를 통한 기본 인증을 활성화해야 합니다. WebClient의 Windows 레지스트리를 변경해야 합니다.
...
Windows에서 Dynamic Media를 사용하려면 ® Visual Studio 2010, 2013 및 2015 x64 및 x86용 재배포 가능 패키지를 설치하십시오.
