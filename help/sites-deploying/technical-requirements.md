...
>
라이선스에는 표준 복제본 세트가 포함되어 있으며, 이 세트는 작성자 또는 게시 배포에 사용할 수 있는 하나의 기본 인스턴스와 두 개의 보조 인스턴스로 구성됩니다.
>
MongoDB에서 작성 및 게시를 모두 실행하려면 두 개의 별도 라이선스를 구입해야 합니다.
>
Adobe 고객 지원 센터는 AEM에서 MongoDB 사용과 관련된 적격 문제를 지원합니다.
>
자세한 내용은 Adobe Experience Manager용 MongoDB 페이지를](https://www.mongodb.com/lp/contact/mongodb-adobe-experience-manager) 참조하십시오[.

>[!NOTE]
>
위에 나열된 지원되는 관계형 데이터베이스는 서드 파티 소프트웨어이며 AEM 라이선싱 패키지에 포함되어 있지 않습니다.
>
...
1. AEM Forms은 Microsoft® Windows Server 2016에 대한 지원을 제거했습니다.
...
AEM 사용자 인터페이스는 대형 화면(일반적으로 노트북 및 데스크탑 컴퓨터) 및 태블릿 폼 팩터(예: Apple iPad 또는 Microsoft® Surface)에 최적화되어 있습니다. 휴대폰 폼 팩터는 지원되지 않습니다.
...
이 섹션에서는 Adobe Experience Manager 및 해당 추가 기능 실행에 대한 특별 참고 사항과 자세한 정보를 제공합니다.

### IPv4 및 IPv6 {#ipv-and-ipv}

Adobe Experience Manager(인스턴스, Dispatcher)의 모든 요소는 IPv4 및 IPv6 네트워크 모두에 설치할 수 있습니다.

특별한 구성이 필요하지 않으므로 작동이 원활합니다. 필요한 경우 네트워크 유형에 적합한 형식을 사용하여 IP 주소를 지정합니다.
...
Dynamic Media을 활성화하면 다음과 같은 추가 기술 요구 사항이 적용됩니다.

>[!NOTE]
>
이러한 시스템 요구 사항은 **Dynamic Media - 하이브리드 모드를 사용하는 경우에만** 적용됩니다. Dynamic Media - 하이브리드 모드에는 특정 운영 체제에서만 인증된 이미지 서버가 포함되어 있습니다.
>
Dynamic Media - Scene7 모드(즉, **dynamicmedia_scene7** 실행 모드)를 실행하는 Dynamic Media 고객의 경우 추가 시스템 요구 사항이 없습니다. AEM과 동일한 시스템 요구 사항만 있습니다. Dynamic Media - Scene7 모드 아키텍처는 AEM에 임베드된 서비스가 아닌 클라우드 기반 이미지 서비스를 사용합니다.
...
>[!NOTE]
>
**SELinux 비활성화:** SELinux가 켜져 있으면 이미지 제공이 작동하지 않습니다. 이 옵션은 기본적으로 활성화되어 있습니다. 이 문제를 해결하려면 /etc/selinux/config **파일을 편집**&#x200B;하고 SELinux 값을 다음에서 변경하십시오.
>
...
>
따라서 Adobe는 이러한 시스템을 실행하는 경우 커널이 이러한 프로세스를 종료하지 않도록 numa=off **부팅 옵션을 사용하여** NUMA를 끄는 것이 좋습니다.

>[!NOTE]
>
**서버 호스트 이름을 해석해야 함:** 서버의 호스트 이름을 IP 주소로 해석할 수 있는지 확인하십시오. 가능하지 않은 경우 완전한 호스트 이름과 IP 주소를 /etc/hosts **에**&#x200B;추가하십시오.
>
...
<table>
 <tbody>
  ...
  <tr>
   <td><a href="https://helpx.adobe.com/acrobat/release-note/release-notes-acrobat-reader.html">Acrobat 2017 클래식 트랙</a> 최신 버전(사용되지 않음)</td>
   <td>XPS, 이미지 형식(BMP, GIF, JPEG, JPG, TIF, TIFF, PNG, JPF, JPX, JP2, J2K, J2C, JPC), HTML, HTM, DWG, DXF 및 DWF</td>
  </tr>
  <tr>
  ...
  <tr>
   <td>Microsoft® Office 2016(사용되지 않음)</td>
   <td>DOC, DOCX, XLS, XLSX, PPT, PPTX, RTF 및 TXT</td>
  </tr>
  <tr>
   <td>워드퍼펙트 2020<br /> </td>
  ...
  <tr>
   <td>Microsoft® Project 2016(사용되지 않음)<br /> </td>
   <td>MPP</td>
  </tr>
  <tr>
   <td>오픈오피스 4.1.10</td>
...
* 1GB RAM(32비트) 또는 2GB RAM(64비트 OS)
