...
Adobe는 이 문서의 다음 정보에 따라 플랫폼에서 AEM(Adobe Experience Manager)을 지원합니다.
...
Adobe Experience Manager 설치에 필요한 최소 요구 사항:
...
>* [AEM Forms 추가 기능 패키지](/help/forms/using/installing-configuring-aem-forms-osgi.md) 시 15GB의 임시 공간이 필요합니다.
>
...
이 문서에서는 Adobe Experience Manager에 대해 지원되는 클라이언트 및 서버 플랫폼을 나열합니다. Adobe는 권장 구성 및 기타 구성에 대해 여러 수준의 지원을 제공합니다.

### 지원되는 구성 {#supported-configurations}

Adobe는 이러한 구성을 권장하며 표준 소프트웨어 유지 관리 계약의 일부로 완벽한 지원을 제공합니다.
...
<table>
 <tbody>
  ...
  <tr>
   ...
   <td>Adobe는 고객의 성공적인 프로젝트 진행을 위해 특정 조건이 충족되어야 하는 제한된 지원 프로그램 내에서 모든 지원을 제공합니다. R 수준 지원은 공식적인 고객 요청과 Adobe의 확인을 필요로 합니다. 자세한 내용은 Adobe 고객 지원 센터에 문의하십시오.</td>
  </tr>
 </tbody>
</table>
...
Adobe Experience Manager는 다음과 같은 버전의 Java Virtual Machines로 작동합니다.
...
1. Oracle Java SE 기술을 사용하는 모든 AEM 고객을 위해 Adobe가 직접 LTS 릴리스의 모든 유지 관리 업데이트를 비롯하여 Oracle Java SE JDK의 지원 및 배포를 지원합니다. 자세한 내용은 [Adobe Experience Manager Q&amp;A에 대한](assets/adobe-oracle-java-license-agreement.pdf) Oracle Java 지원을 참조하십시오.
...
1. AEM Forms 업그레이드 고객을 위해 지원됩니다. 새 설치에는 지원되지 않습니다.
...
>
>MongoDB를 사용하여 AEM 배포를 최대한 활용하려면 Adobe에서는 전문가 지원을 통해 MongoDB Enterprise 버전의 라이선스를 부여하는 것이 좋습니다. 자세한 [내용은 권장 배포를](/help/sites-deploying/recommended-deploys.md#prerequisites-and-recommendations-when-deploying-aem-with-mongomk) 참조하십시오.
>
...
>
>자세한 내용은 Adobe Experience Manager [용 MongoDB 페이지를 참조하십시오](https://www.mongodb.com/lp/contact/mongodb-adobe-experience-manager).

>[!NOTE]
>
>위에 나열된 지원 관계형 데이터베이스는 타사 소프트웨어이며 AEM 라이선스 패키지에 포함되어 있지 않습니다.
>
>지원되는 관계형 데이터베이스와 함께 AEM 6.5를 실행하려면 데이터베이스 공급업체와의 별도의 지원 계약이 필요합니다. Adobe 고객 지원 센터는 AEM 6.5를 사용하여 관계형 데이터베이스 사용과 관련된 자격 있는 문제를 지원합니다.
>
...
1. 애플리케이션 서버에 대한 AEM 6.5 배포를 시작하면 제한된 지원으로 이동합니다. 기존 고객은 AEM 6.5로 업그레이드하여 애플리케이션 서버를 계속 사용할 수 있습니다. 신규 고객의 경우 위의 Level-R 설명에 명시된 지원 기준과 지원 프로그램을 함께 받게 됩니다.
...
1. Linux 커널 2.6, 3.x 및 4.x에는 Red Hat Enterprise Linux, CentOS, Oracle Linux, Amazon Linux 등 Red Hat 배포에서 파생되는 내용이 포함되어 있습니다. AEM 양식 추가 기능은 CentOS 7 및 Red Hat Enterprise Linux 7에서만 지원됩니다.
1. AEM Forms는 Ubuntu 16.04 LTS에서만 지원됩니다.
1. Adobe Managed Services에서 지원되는 Linux 배포
1. Microsoft Windows 제작 배포는 6.5로 업그레이드하고 제작 과정 이외의 다른 사용자도 사용할 수 있습니다. 새 배포는 AEM 사이트 및 자산에 대한 온요청입니다.
1. AEM Forms는 지원 수준 R 제한 없이 Microsoft Windows Server에서 지원됩니다.

### 가상 및 클라우드 컴퓨팅 환경 {#virtual-cloud-computing-environments}

Adobe Experience Manager는 Microsoft Azure 및 Amazon Web Services(AWS)와 같은 클라우드 컴퓨팅 환경의 가상 컴퓨터에서 이 페이지에 나열된 기술 요구 사항을 준수하고 Adobe의 표준 지원 약관에 따라 실행됩니다.
...
Azure 또는 AWS 또는 기타 클라우드 컴퓨팅 환경에 AEM을 배포하는 다른 모든 경우, 이 페이지에 나열된 기술 사양에 따라 Adobe의 지원이 가상 컴퓨팅 환경에 포함됩니다. 클라우드 서비스가 이 페이지에 나열된 기술 요구 사항(예: Azure Blob 스토리지 또는 AWS S3)의 일부로 특별히 지원되지 않는 한, 이러한 클라우드 환경에서 실행되는 AEM과 관련된 보고된 문제는 클라우드 컴퓨팅 환경 관련 클라우드 서비스와 독립적으로 재생산되어야 합니다.

Azure 또는 AWS에서 AEM을 배포하는 방법에 대한 권장 사항은 Adobe Managed Services 외부에 있는 Adobe에서 원하는 클라우드 환경에서 AEM의 배포를 지원하는 클라우드 제공업체 또는 Adobe 파트너와 직접 작업하는 것이 좋습니다. 선택한 클라우드 제공업체 또는 파트너는 고객의 특정 성능, 로드, 확장성 및 보안 요구 사항을 충족하기 위해 아키텍처의 크기 지정, 설계 및 구현에 대한 책임을 집니다.
...
>
>Mozilla Firefox, Google Chrome 및 Microsoft Edge 릴리스는 몇 개월마다 업데이트됩니다. Adobe는 Adobe Experience Manager에 대한 업데이트를 제공하여 이러한 브라우저의 향후 버전에 대해 아래에 명시된 지원 수준을 유지할 것입니다.
...
Microsoft Windows 7+와 SSL로 보안이 유지되지 않는 AEM 인스턴스에 성공적으로 연결하려면 Windows에서 보안되지 않은 네트워크에 대한 기본 인증이 활성화되어 있어야 합니다. 이렇게 하려면 WebClient의 Windows 레지스트리를 변경해야 합니다.
...
* IPv4 및 IPv6 네트워크 설치에 대한 기본 대/소문자 `localhost` 가 해석됩니다(예: `https://localhost:4502`

### AEM Dynamic Media Add-on에 대한 요구 사항 {#requirements-for-aem-dynamic-media-add-on}

AEM Dynamic Media는 기본적으로 비활성화됩니다. 다이내믹 미디어 [를 활성화하려면 여기를 참조하십시오](/help/assets/config-dynamic.md#enabling-dynamic-media).

Dynamic Media가 활성화되면 다음과 같은 추가 기술 요구 사항이 적용됩니다.

>[!NOTE]
>
>이러한 시스템 요구 사항은 **다이내믹 미디어 - 하이브리드 모드를 사용하는 경우에만** 적용됩니다. 다이내믹 미디어 - 하이브리드 모드에는 포함된 이미지 서버가 있으며, 이는 특정 운영 체제에서만 인증됩니다.
>
>다이내믹 미디어 - Scene7 모드(즉, **dynamicmedia_scene7** runmode)를 실행하는 다이내믹 미디어 고객의 경우 추가 시스템 요구 사항이 없습니다. AEM과 동일한 시스템 요구 사항만 해당됩니다. 다이내믹 미디어 - Scene7 모드 아키텍처는 AEM에 포함된 서비스가 아닌 클라우드 기반 이미지 서비스를 사용합니다.
...
>[!NOTE]
>
>LC_CTYPE이 같지 않도록 로케일이 설정된 경우 다이내믹 미디어 `en_US.UTF-8`가 작동하지 않습니다. 명령 프롬프트에서 해당 값이 &quot;locale&quot;이라는 유형을 확인합니다. 이렇게 설정되지 않은 경우 AEM을 실행하기 전에 &quot;export LC_CTYPE=&quot;를 입력하여 LC_CTYPE 환경 변수를 빈 문자열로 설정합니다.
...
>
>따라서 시스템에서 실행 중인 경우 커널이 이러한 프로세스를 죽이는 것을 방지하기 위해 **numa=off** boot 옵션을 사용하여 NUMA를 끄는 것이 좋습니다.

>[!NOTE]
>
>**서버 호스트 이름은 다음 문제를 해결해야 합니다.** 서버의 호스트 이름이 IP 주소로 확인되는지 확인하십시오. 그렇지 않을 경우 정규화된 호스트 이름과 IP 주소를 **/etc/hosts에 추가하십시오**.
>
...
* https://www.microsoft.com/en-us/download/details.aspx?id=13523에서 Microsoft Visual Studio 2010 재배포 가능 버전을 [다운로드하십시오.](https://www.microsoft.com/en-us/download/details.aspx?id=13523)
* https://www.microsoft.com/en-us/download/details.aspx?id=40784에서 Microsoft Visual Studio 2013 재배포 가능 버전을 [다운로드하십시오.](https://www.microsoft.com/en-us/download/details.aspx?id=40784)
* https://www.microsoft.com/en-us/download/details.aspx?id=48145에서 Microsoft Visual Studio 2015 재배포 가능 버전을 [다운로드하십시오.](https://www.microsoft.com/en-us/download/details.aspx?id=48145)

Windows x86의 경우:

* https://www.microsoft.com/en-in/download/details.aspx?id=5555에서 Microsoft Visual Studio 2010 재배포 가능 버전을 [다운로드하십시오.](https://www.microsoft.com/en-in/download/details.aspx?id=5555)
* https://www.microsoft.com/en-in/download/details.aspx?id=40769에서 Microsoft Visual Studio 2013 재배포 가능 버전을 [다운로드하십시오.](https://www.microsoft.com/en-in/download/details.aspx?id=40769)
* https://www.microsoft.com/en-us/download/details.aspx?id=52685에서 Microsoft Visual Studio 2015 재배포 가능 버전을 [다운로드하십시오.](https://www.microsoft.com/en-us/download/details.aspx?id=52685)
...
>* Acrobat 버전은 AEM Forms와 함께 번들로 제공되므로 PDF Generator 기능을 사용할 수 있습니다. 번들 버전은 AEM Forms PDF Generator와 함께 사용하기 위해 AEM Forms 라이선스의 기간동안 AEM Forms에서만 프로그래밍 방식으로 액세스해야 합니다. 자세한 내용은 배포(온프레미스[또는](https://helpx.adobe.com/legal/product-descriptions/adobe-experience-manager-on-premise.html) 관리 서비스)에 따라 AEM Forms 제품 설명을 [참조하십시오](https://helpx.adobe.com/legal/product-descriptions/adobe-experience-manager-managed-services.html).&quot;
   >
   >
...
XMP 쓰이백은 다음과 같은 플랫폼 및 파일 포맷에 대해 지원되고 활성화됩니다.
...
XMPFilesProcessor 프로세스를 사용하려면 라이브러리 GLIBC_2.14가 필요합니다. GLIBC_2.14를 포함하는 Linux 커널(예: Linux 커널 버전 3.1.x)을 사용합니다. PSD 파일과 같은 많은 양의 메타데이터가 포함된 에셋 처리 성능이 향상됩니다. 이전 버전의 GLIBC를 사용하면 로그에 오류가 발생합니다 `com.day.cq.dam.core.impl.handler.xmp.NCommXMPHandler Failed to read XMP`.
