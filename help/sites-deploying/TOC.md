...
+ AEM 플랫폼 소개 {#introduction}
   ...
   + [MongoDB와 AEM](aem-with-mongodb.md)
+ AEM 배포 {#deploying}
   ...
   + [TarMK 콜드 대기로 AEM을 실행하는 방법](tarmk-cold-standby.md)
   + [AEM 6.5의 RDBMS 지원](rdbms-support-in-aem.md)
   + [Oak-run Jar를 통한 인덱싱](indexing-via-the-oak-run-jar.md)
   + [Oak-run.jar 색인화 사용 사례](oak-run-indexing-usecases.md)
...
+ AEM 구성 {#configuring}
   ...
   + [정적 개체의 만료](expiration-static-objects.md)
   ...
   + [SSL을 통한 HTTP 활성화](https://experienceleague.adobe.com/docs/experience-manager-65/administering/security/ssl-by-default.html)
...
+ AEM 6.5로 업그레이드 {#upgrading}
   ...
   + [패턴 감지기를 사용한 업그레이드 복잡성 평가](pattern-detector.md)
   + [AEM 6.5의 이전 버전과의 호환성](backward-compatibility.md)
   + [업그레이드 프로시저](upgrade-procedure.md)
   + [즉석 업그레이드 수행](in-place-upgrade.md)
   + [업그레이드 중 다운타임을 줄이기 위해 오프라인 리인덱싱을 사용](upgrade-offline-reindexing.md)
   + [소극적 컨텐츠 마이그레이션](lazy-content-migration.md)
   ...
   + [Application Server 설치 업그레이드 단계](app-server-upgrade.md)
   + [업그레이드 후 제거된 오래된 번들 목록](obsolete-bundles.md)
+ 저장소 재구성 {#restructuring}
   + [AEM 6.5의 저장소 재구성](repository-restructuring.md)
   + [AEM 6.5의 일반적인 저장소 재구성](all-repository-restructuring-in-aem-6-5.md)
   + [AEM 6.5의 Sites 저장소 재구성](sites-repository-restructuring-in-aem-6-5.md)
   + [AEM 6.5의 Assets 저장소 재구성](assets-repository-restructuring-in-aem-6-5.md)
   + [AEM 6.5에서 Dynamic Media 저장소 재구성](dynamicmedia-repository-restructuring-in-aem-6-5.md)
   + [AEM 6.5에서 Forms 저장소 재구성](forms-repository-restructuring-in-aem-6-5.md)
   + [AEM 6.5의 전자 상거래 저장소 재구성](ecommerce-repository-restructuring-in-aem-6-5.md)
   + [6.5의 AEM Communities에 대한 저장소 재구성](communities-repository-restructuring-in-aem-6-5.md)
+ 우수 사례 {#practices}
   + [모범 사례 배포](best-practices.md)
