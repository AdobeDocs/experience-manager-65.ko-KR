...
작성 인스턴스의 느린 성능 분석이 복잡해질 수 있습니다. 첫 번째 단계에서는 성능이 저하되는 기술 스택의 수준을 파악해야 합니다.

다음 진단트리는 병목 현상을 줄이는 지침을 제공합니다.
...
이 문제는 일반적으로 운영 체제의 데스크톱 환경이 확장명이 .jar인 파일을 열도록 구성되는 방식에 문제가 있음을 나타냅니다. Java™이 설치되어 있지 않거나 지원되지 않는 버전의 Java™을 사용하고 있음을 나타낼 수도 있습니다.
...
* Java™ 또는 Sun Java™이 나열되는지 확인하고 이 목록을 사용하여 AEM WCM을 실행해 보십시오. 여러 Java™ 버전이 설치되어 있는 경우 지원되는 버전을 선택합니다.
...
CRX 자체는 메모리 풋프린트가 적습니다. CRX 내에서 실행 중인 응용 프로그램에 더 큰 메모리 요구 사항이 있거나 메모리 사용량이 많은 작업(예: 큰 트랜잭션)을 요청하는 경우, CRX가 실행되는 JVM 인스턴스는 적절한 메모리 설정으로 시작해야 합니다.
...
이 메서드는 heap 덤프 파일(**java_..hprof**)를 만들 수 있습니다. 힙이 생성된 후에도 프로세스가 계속 실행될 수 있습니다. 일반적으로 한 개의 더미 덤프 파일이 문제를 분석할 수 있습니다.
...
저장소 자체가 성공적으로 실행되어도 AEM WCM 시작 화면이 자동으로 표시되지 않는 경우가 있습니다. 이 문제는 운영 체제 설정, 브라우저 구성 또는 유사한 요소에 따라 달라질 수 있습니다.
...
### 웹 사이트가 Java™ 11에서 간헐적으로 로드되지 않거나 실패합니다. {#the-website-does-not-load-or-fails-intermittently-with-java11}

AEM 6.5가 Java™ 11에서 실행 중이며 웹 사이트가 간헐적으로 로드되지 않거나 실패할 수 있는 알려진 문제가 있습니다.

이 문제가 발생하면 다음을 수행하십시오.

1. 를 엽니다. `sling.properties` 아래의 파일 `crx-quickstart/conf/` 폴더
1. 다음 라인을 찾습니다.
...
>[!NOTE]
>
>Adobe은 AEM을 삭제하기 전에 리포지토리를 백업할 것을 권장합니다. 전체 &lt;cq-installation-directory>저장소에서 삭제할 수도 있습니다. 삭제하기 전에 리포지토리 데이터를 유지하려면 &lt;cq-installation-directory>/crx-quickstart/repository 폴더를 삭제하기 전에 다른 폴더에 배치합니다.

AEM 설치 시 외부 저장소(예: 데이터베이스 서버)를 사용하는 경우 폴더를 제거해도 데이터가 자동으로 제거되지는 않지만 저장소 구성이 제거되므로 JCR 컨텐츠를 복원하기가 어렵습니다.

### JSP 파일은 JBoss®에 컴파일되지 않습니다. {#jsp-files-are-not-compiled-on-jboss}

JBoss®에 Experience Manager을 위해 JSP 파일을 설치하거나 업데이트하며 해당 서블릿이 컴파일되지 않은 경우 JBoss® JSP 컴파일러가 올바르게 구성되어 있는지 확인하십시오. 자세한 내용은
[JBoss®의 JSP 컴파일 문제](https://helpx.adobe.com/experience-manager/kb/jsps-dont-compile-jboss.html) 문서.
