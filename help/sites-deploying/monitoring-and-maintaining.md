...
`stdout.log`
시작 중에 이벤트를 나타내는 로깅 메시지를 보관합니다.`WARN`
...
Provides a log of all upgrade operations that runs from `com.day.compat.codeupgrade` and `com.adobe.cq.upgradesexecutor` packages.
   ...
   * `<*cq-installation-dir*>/crx-quickstart/repository`
`revision.log`
저널링 정보 개정`com.adobe.cq.upgradesexecutor`
...
로그 항목은 많은 로그 항목을 생성하므로 로그를 필요한 것보다 오래 디버그 로그 수준으로 두어서는 안 됩니다.`org.apache.sling.commons.log.level``/libs/sling/config/org.apache.sling.commons.log.LogManager`[](/help/sites-deploying/osgi-configuration-settings.md#apacheslingloggingconfiguration)

>[!CAUTION]디버그 파일의 한 줄은 대개 DEBUG로 시작하고 로그 수준, 설치 프로그램 작업 및 로그 메시지를 제공합니다. 예:
로그 수준은 다음과 같습니다.
...
| 작업이 실패했으며 설치 관리자를 계속할 수 없습니다. | 1 | 오류 |
|---|---|---|
| 작업이 실패했습니다. 설치가 진행되지만 AEM WCM의 일부가 올바르게 설치되지 않아 작동하지 않습니다. | 2 | 경고 |
| 작업에 성공했지만 문제가 발생했습니다. AEM WCM이 제대로 작동하지 않거나 작동하지 않을 수 있습니다. | 3 | 정보 |
| 그 행동은 성공했다. | 사용자 정의 로그 파일 만들기 {#create-a-custom-log-file} | [!NOTE] |
...
>[!NOTE]특정 상황에서는 다른 로그 수준의 사용자 지정 로그 파일을 만들 수 있습니다. 다음과 같은 방법으로 저장소에서 이 작업을 수행할 수 있습니다.
아직 존재하지 않는 경우 프로젝트에 사용할 새 구성 폴더( `sling:Folder`)를 만듭니다 `/apps/<*project-name*>/config`.

아래에서 새 Apache `/apps/<*project-name*>/config`Sling Logging [Logger 구성 노드를 만듭니다](/help/sites-deploying/osgi-configuration-settings.md#apacheslingloggingloggerconfigurationfactoryconfiguration).

1. 이름: `org.apache.sling.commons.log.LogManager.factory.config-<*identifier*>` (로거인 경우)`/apps/<*project-name*>/config`
1. 여기서 `<*identifier*>` 는 인스턴스를 식별하기 위해 입력한 자유 텍스트로 대체됩니다(이 정보를 생략할 수 없음).[](/help/sites-deploying/osgi-configuration-settings.md#apacheslingloggingloggerconfigurationfactoryconfiguration)
...
1. 유형: 문자열
   ...
   * 값: 로거가 메시지를 기록할 OSGi 서비스를 지정합니다. 예를 들어, 다음 모두`org.apache.sling.commons.log.names`
   ...
   * 값: 필요한 로그 수준(, `debug``info`, `warn` 또는 `error`)을 지정합니다. for example `debug`
      ...
      이름: `org.apache.sling.commons.log.pattern``info``warn``error``debug`

   * 유형: `String`

      * 값: 필요에 따라 로그 메시지의 패턴을 지정합니다. 예를 들면`org.apache.sling.commons.log.pattern`
   ...
   >org.apache.sling.commons.log.names에는 값이 있어야 합니다.`Throwable`

   >[!NOTE]
   로그 작성기 경로는 `crx-quickstart` 위치를 기준으로 합니다.

   >[!NOTE]따라서 다음과 같이 지정된 로그 파일
   `logs/thelog.log`
   쓰기 대상:
...
&quot;(예: ` `&lt;*cq-installation-dir*>/`crx-quickstart/`)
   ...
   >이 단계는 새 작성기가 필요한 경우에만 필요합니다(예: 기본 작성기와 다른 구성).
   [!CAUTION]` `**`crx-quickstart/`

1. 새 로깅 작성자 구성은 기존 기본값이 적합하지 않은 경우에만 필요합니다.

   >[!CAUTION]명시적 작성기가 구성되지 않은 경우 시스템은 기본값을 기반으로 암시적 작성기를 자동으로 생성합니다.
   ...
   로거 와 마찬가지로 인스턴스 `<*identifier*>` 를 식별하기 위해 입력한 자유 텍스트로 대체됩니다(이 정보를 생략할 수 없음). 예, `org.apache.sling.commons.log.LogManager.factory.writer-MINE`](/help/sites-deploying/osgi-configuration-settings.md#apacheslingloggingwriterconfigurationfactoryconfiguration)
   ...
   * 값: 로거에 지정된 파일과 일치하도록 로그 파일을 지정합니다.`org.apache.sling.commons.log.file`
   ...
   * 유형: `Long`
   ...
   >[!NOTE]최대 파일 크기
   `org.apache.sling.commons.log.file.size`시간/날짜 일정
   ...
   * 기본값은 &#39;.&#39;입니다.yyyy-MM-dd(일별 로그 회전).`KB``MB``GB`
   * 예를 들어 2010년 1월 20일 자정(또는 이 시간 후 첫 번째 로그 메시지가 정확해지는 경우) ../logs/error.log의 이름이 ../logs/error.log.2010-01-20으로 변경됩니다. 1월 21일에 로깅은 다음 변경 시간에 롤오버될 때까지 ../logs/error.log으로 출력됩니다.`java.util.SimpleDateFormat`
   ...
   | `'.'yyyy-MM-dd-HH-mm`참고: 시간/날짜를 지정할 때: | 단일 따옴표(&#39; &#39;); 이는 특정 문자가 패턴 문자로 해석되지 않도록 하기 위한 것입니다. |
   옵션의 아무 위치에서나 유효한 파일 이름에 허용되는 문자만 사용하십시오.
   1. 선택한 툴로 새로운 로그 파일을 읽을 수 있습니다.
   1. 이 예제로 만든 로그 파일이 됩니다 `../crx-quickstart/logs/myLogFile.log`.


1. 또한 Felix Console은 Sling Log 지원에 대한 정보를 제공합니다. `../system/console/slinglog`예를 들면 다음과 같습니다 `https://localhost:4502/system/console/slinglog`.

   감사 레코드 찾기 {#finding-the-audit-records}

감사 기록은 누가 언제 무엇을 했는지 기록하도록 되어 있다. AEM WCM 및 OSGi 이벤트 모두에 대해 서로 다른 감사 레코드가 생성됩니다.`../system/console/slinglog``https://localhost:4502/system/console/slinglog`

### 페이지 작성 시 표시되는 AEM WCM 감사 레코드 {#aem-wcm-audit-records-shown-when-page-authoring}

페이지를 엽니다.

#### 사이드 킥에서 잠금 아이콘이 있는 탭을 선택한 다음, **감사 로그를 두 번 클릭합니다.**

1. 현재 페이지의 감사 레코드 목록을 보여주는 새 창이 열립니다.
1. ![screen_shot_2012-02-02at43601pm](assets/screen_shot_2012-02-02at43601pm.png)
1. 창을 **닫으려면 확인을** 클릭합니다.

   저장소 내의 AEM WCM 감사 레코드 {#aem-wcm-auditing-records-within-the-repository}](assets/screen_shot_2012-02-02at43601pm.png)

1. 폴더 내에서 `/var/audit` 감사 레코드는 리소스에 따라 보관됩니다. 개별 레코드와 포함된 정보를 볼 수 있을 때까지 드릴다운할 수 있습니다.**

#### 이러한 항목은 페이지를 편집할 때 표시되는 것과 동일한 정보를 포함합니다.{#aem-wcm-auditing-records-within-the-repository}
...
## 필요한 대기열이 모두 활성화됩니까?{#monitoring-your-replication-agents}

비활성화된 대기열이 계속 필요합니까?[](/help/sites-deploying/replication.md)
...
* 복제 에이전트를 모니터링하려면:`enabled``idle``active``blocked`
...
1. 결과 창에는 대상 및 상태를 포함하여 작성 환경에 대한 모든 복제 에이전트에 대한 개요가 표시됩니다.****
1. 해당 에이전트에 대한 자세한 정보를 표시하려면 해당 에이전트 이름(링크임)을 클릭합니다.****

   ![chlimage_1](assets/chlimage_1.jpeg)

1. 여기에서 다음을 수행할 수 있습니다.

   ![에이전트가 활성화되어 있는지 확인합니다.](assets/chlimage_1.jpeg)
   ...
   * **게시 인스턴스의 역방향 복제 출력 상자에 대해 &quot;연결 테스트&quot; 링크를 사용하지 마십시오.**
   ...
   >[!CAUTION]Outbox 큐에 대한 복제 테스트가 수행되는 경우 테스트 복제보다 오래된 항목은 모든 역방향 복제를 사용하여 다시 처리됩니다.
...
[성능 최적화](/help/sites-deploying/configuring-performance.md) (Performance Optimization)는 개발 과정에서 초점을 맞춘 인터랙티브한 프로세스입니다. 배포 후에는 일반적으로 특정 간격이나 이벤트 후에 검토됩니다.`enabled``disabled``active``idle``blocked`

## 최적화를 위한 정보를 수집하는 동안 사용되는 메서드를 지속적인 모니터링에 사용할 수도 있습니다.{#monitoring-performance}
...
>[!NOTE]다음은 일반적인 성능 문제와 이를 찾아 대응하는 방법에 대한 제안입니다.
영역[](/help/sites-deploying/configuring-performance.md#configuring-for-performance)

증상

| 용량 증가 | 볼륨을 줄이려면.. | 클라이언트 | 높은 클라이언트 CPU 사용. |
|---|---|---|---|
| 더 높은 성능을 가진 클라이언트 CPU를 설치합니다. | 레이아웃 간소화(HTML) | 서버 CPU 사용량이 낮습니다. | 더욱 빨라진 브라우저로 업그레이드 |
|  | 클라이언트측 캐시 향상 | 어떤 고객은 빠르고, 어떤 고객은 느립니다. | 서버 |
|  | 네트워크 |  |  |
| 서버와 클라이언트 모두에서 CPU 사용량이 낮습니다. |  |  |  |
| 네트워크 병목 현상을 제거합니다. | 클라이언트 캐시의 구성을 개선/최적화합니다. | 서버에서의 로컬 브라우징이(비교적) 빠릅니다. | 네트워크 대역폭 증가 |
|  | 웹 페이지의 &quot;무게&quot;(예: 이미지 감소, 최적화된 HTML) 감소 | 웹 서버 | 웹 서버의 CPU 사용량이 높습니다. |
| 웹 서버를 클러스터링합니다. | 페이지당 히트 수 감소(방문). | 하드웨어 부하 균형 조정기를 사용합니다. | 애플리케이션 |
|  |  | 서버 CPU 사용량이 높습니다. |  |
| AEM 인스턴스를 클러스터링합니다. | CPU 및 메모리 돼지 검색 및 제거(코드 검토, 타이밍 출력 등 사용) | 높은 메모리 사용. | 모든 수준의 캐싱 향상 |
...
### 이 중 일부는 운영 체제에 따라 다릅니다.{#tools-for-monitoring-and-analyzing-performance}
...
웹 사이트의 트래픽 모니터링 {#monitoring-traffic-on-your-website}`tail -f``request.log``request.log`[`rlog.jar`](#using-rlog-jar-to-find-requests-with-long-duration-times)

요청 로그는 각 요청을 수행한 응답과 함께 등록합니다.`request.log`` [yourkit](https://www.yourkit.com/)`

#### 특정 기간(예: 다양한 24시간 이상) 내의 모든 GET 항목을 합산하여 웹 사이트의 평균 트래픽에 대해 설명할 수 있습니다.{#monitoring-traffic-on-your-website}
...
`<*cq-installation-dir*>/crx-quickstart/logs/request.log`이 로그에는 요청이나 응답당 한 줄이 있습니다.
...
#### 따라서 위와 같이 스크립트를 사용하여 관련 정보를 추출하고 통계를 작성할 수 있습니다.{#monitoring-search-response-times-with-the-request-log}
...
#### 부정적인 영향을 보기 전에 시스템에서 처리할 수 있는 동시 사용자 수를 확인하기 위해 테스트를 수행해야 합니다. 다시 스크립트를 사용하여 로그 파일에서 결과를 추출할 수 있습니다.{#monitoring-the-number-and-impact-of-concurrent-users}

특정 시간 범위(예: 1분) 내에서 수행된 요청 수 모니터링`request.log`
...
다음 명령은 가능한 인수를 보여 줍니다.`<*cq-installation-dir*>/crx-quickstart/opt/helpers`

예를 들어 파일을 매개 변수로 지정하는 `request.log` 파일을 실행하고 가장 긴 기간이 있는 첫 번째 요청 10개를 표시할 수 있습니다.`request.log`
...
특수 사건(예: 가비지 수집 등)의 영향을 최소화하려면 메모리 누수를 식별하고 선택적으로 응답 시간을 분석하기 위해 `apachebench` (예: 추가 설명서의 [ab[#$tu387] 참조)와 같은 도구를 사용하는 것이 좋습니다.
...
요청 트래픽(특정 기간 동안의 요청 수)에 대한 정보는 인스턴스의 로드를 나타냅니다. 이 정보는 [request.log](#interpreting-the-request-log)에서 추출할 수 있지만 카운터를 사용하면 데이터 수집을 자동화하여 다음을 확인할 수 있습니다.`-c`

### 활동의 현저한 차이점(&quot;많은 요청&quot;과 &quot;낮은 활동&quot;을 구별함){#request-counters}

인스턴스가 사용되지 않는 경우[](#interpreting-the-request-log)
...
### AEM 인스턴스를 시작합니다.{#monitoring-performance-using-jconsole}
...
1. 이후 다른 옵션을 선택할 수 있습니다.`Local``com.day.crx.quickstart.Main`

   (J)VisualVM을 사용하여 성능 모니터링 {#monitoring-performance-using-j-visualvm}](assets/chlimage_1-1.png)

   JDK 1.6부터는 도구 명령을 사용할 수 `jvisualvm` 있습니다. JDK 1.6을 설치한 후 다음을 수행할 수 있습니다.

### AEM 인스턴스를 시작합니다.{#monitoring-performance-using-j-visualvm}

[!NOTE]

1. Java 5를 사용하는 경우 JVM을 시작하는 java 명령줄에 `-Dcom.sun.management.jmxremote` 인수를 추가할 수 있습니다. JMX는 Java 6에서 기본적으로 활성화되어 있습니다.

   >[!NOTE]다음 중 하나를 실행합니다.
   `jvisualvm`: JDK 1.6 bin 폴더에서(테스트된 버전)

1. `visualvm`: VisualVM [에서[#$tu419] 다운로드할 수 있음(Breding Edge 버전)

   * 
   * 

1. 이후 모니터를 비롯한 다른 옵션을 선택할 수 있습니다.`Local``com.day.crx.quickstart.Main`

   ![chlimage_1-3](assets/chlimage_1-3.png)

   이 도구를 사용하여 스레드 덤프와 메모리 헤드 덤프를 생성할 수 있습니다. 이 정보는 기술 지원 팀이 요청하는 경우가 많습니다.

   정보 수집 {#information-collection}](assets/chlimage_1-3.png)
...
### 다음 정보가 유용할 수 있습니다.{#information-collection}
...
* 몇 명의 작성자가 이 시스템을 사용하고 있습니까? {#how-many-authors-are-working-with-the-system}](#how-many-requests-per-hour-do-you-have-on-the-author-system-at-peak-time)
* [설치 후 시스템을 사용한 작성자 수를 보려면 명령줄을 사용합니다.](#how-many-requests-per-hour-do-you-have-on-the-publish-system-at-peak-time)

#### 지정된 날짜에 작업하는 작성자 수를 보려면 다음을 수행하십시오.{#how-many-authors-are-working-with-the-system}
...
* 이 시스템에서 현재 몇 페이지를 관리하고 있습니까? {#how-many-pages-do-you-currently-maintain-on-this-system}**`//element(*, cq:AuditEvent)[@cq:type='Activate']`
...
* **설치 이후 총 롤아웃 수를 확인하려면 저장소 쿼리를 사용합니다. CRXDE 사용 - 도구 - 쿼리:**`//element(*, cq:Page)`
...
* 한 달에 평균 Live Copy 수는 얼마입니까? {#what-is-the-average-number-of-live-copies-per-month}**`//element(*, cq:AuditEvent)[@cq:type='PageRolledOut']`
...
* AEM Assets을 사용하는 경우 현재 자산에 몇 개의 자산을 유지하고 있습니까? {#if-you-use-aem-assets-how-many-assets-do-you-currently-maintain-in-assets}**`//element(*, cq:LiveSyncConfig)`
...
* 자산의 평균 크기는 얼마입니까? {#what-is-the-average-size-of-the-assets}**`/`
* 폴더의 전체 크기를 확인하려면 다음을 `/var/dam` 수행하십시오.**`/jcr:root/content/dam//element(*, dam:Asset)`

#### WebDAV를 사용하여 저장소를 로컬 파일 시스템에 매핑합니다.{#what-is-the-average-size-of-the-assets}

명령줄 사용:`/var/dam`

1. 평균 크기를 얻으려면, 글로벌 크기를 (위에서 얻음)의 총 자산 수로 `/var/dam` 나눕니다.

1. 현재 사용되는 템플릿은 몇 개입니까? {#how-many-templates-are-currently-used}
   ...
   현재 서버에 있는 템플릿 수를 보려면 저장소 쿼리를 사용합니다. CRXDE 사용 - 도구 - 쿼리:`/var/dam`
...
* 현재 몇 개의 구성 요소가 사용됩니까? {#how-many-components-are-currently-used}**`/`
* **현재 서버에 있는 구성 요소 수를 보려면 저장소 쿼리를 사용합니다. CRXDE 사용 - 도구 - 쿼리:**`//element(*, cq:Template)`
...
* 최고 시간에 작성자 시스템에 몇 개의 요청이 있습니까? {#how-many-requests-per-hour-do-you-have-on-the-author-system-at-peak-time}**`/`
* **최대 시간에 작성자 시스템에 있는 시간당 요청을 확인하려면 다음을 수행하십시오.**`//element(*, cq:Component)`

#### 설치 이후 총 요청 수를 확인하려면 명령줄을 사용합니다.{#how-many-requests-per-hour-do-you-have-on-the-author-system-at-peak-time}

시작 및 종료 날짜를 결정하려면

1. 이 값을 사용하여 설치 후 경과된 시간과 시간당 평균 요청 수를 계산합니다.
...
1. 최대 시간에 게시 시스템에 몇 개의 요청이 있습니까? {#how-many-requests-per-hour-do-you-have-on-the-publish-system-at-peak-time}
...
[슬로우 및 차단된 프로세스 분석[#$tu508]

* 

   * 

### 시스템에 메모리가 부족한 경우 성능 저하 및 하위 텍스트를 포함한 오류 메시지 등 다양한 방법으로 볼 수 있습니다.{#out-of-memory}
...
### 디버그 정보 수집을 비활성화했는지 여부; 다음을 포함하여 다양한 위치에서 구성할 수 있습니다.{#disk-i-o}
...
* [Apache Sling Java 스크립트 처리기](/help/sites-deploying/osgi-configuration-settings.md#apacheslingjavascripthandler)
   ...
   * [기술 자료:](/help/sites-deploying/monitoring-and-maintaining.md#activating-the-debug-log-level)[](/help/sites-deploying/configuring.md#loggersandwritersforindividualservices)

* [열려 있는 파일이 너무 많습니다.[#$tu530]
* 
...
### 기본 설정이 적합하지 않을 경우 JVM을 조정하기 전에 GC 성능을 모니터링하고 평가하는 방법을 설정해야 합니다. 여기에는 더미 크기, 알고리즘 및 기타 측면을 비롯한 모니터링 요소가 포함될 수 있습니다.{#jvm-tuning}
...
` [https://www.ibm.com/developerworks/library/j-ibmtools2/](https://www.ibm.com/developerworks/library/j-ibmtools2/)`이러한 설정은 &quot;전체 열기&quot; JMX 연결에 사용됩니다.
...
* 따라서 사용 중인 메모리 양, GC 알고리즘, 실행 시간, 애플리케이션 성능에 미치는 영향을 확인할 수 있습니다. 이렇게 하지 않으면 조정은 &quot;무작위적 처리 장치&quot;에 불과합니다.   [!NOTE]
