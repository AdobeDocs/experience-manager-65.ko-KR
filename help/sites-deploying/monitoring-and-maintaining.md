...
여기서 중요한 요소는 잠재적인 문제를 인식하려면 시스템이 정상적인 조건에서 어떻게 보이고 작동하는지 알아야 한다는 것입니다. 이는 시스템을 모니터링하고 일정 기간 동안 정보를 수집하는 것이 가장 좋습니다.

| 확인 | 고려 사항 | 댓글 / 작업 |
|---|---|---|
| 백업 계획. |  | 방법 보기 [인스턴스 백업](/help/sites-deploying/monitoring-and-maintaining.md#backups). |
| 재해 복구 계획 | 귀사의 재해 복구 지침 |  |
| 오류 추적 시스템은 보고 문제에 사용할 수 있습니다. | 예를 들어, [부가질라](https://www.bugzilla.org/), [jira](https://www.atlassian.com/software/jira/)또는 기타 여러 항목 중 하나를 선택할 수 있습니다. |  |
| 파일 시스템을 모니터링하고 있습니다. | 사용 가능한 디스크 공간이 부족하면 CRX 저장소가 &quot;고정&quot;됩니다. 공간을 사용할 수 있게 되면 다시 시작됩니다. | &quot; `*ERROR* LowDiskSpaceBlocker`사용 가능한 공간이 부족해지면 로그 파일에서 메시지를 볼 수 있습니다. |
...
백업 정책과 백업 대상 및 백업 시점 등에 대한 추가 고려 사항을 준수해야 할 경우

* 시스템 및 데이터의 중요도
* 소프트웨어 또는 데이터가 변경되는 빈도.
* 데이터 볼륨, 백업 수행 시 필요한 시간과 같이 가끔 용량이 문제가 될 수 있습니다.
* 사용자가 온라인 상태일 때 백업을 수행할 수 있는지 여부 및 가능한 경우 성능에 미치는 영향
* 사용자의 지리적 분포, 즉 언제 백업하는 것이 가장 좋습니까?(영향을 최소화하기 위해)
* 재해 복구 정책: 백업 데이터를 저장할 위치(예: 오프사이트, 특정 매체 등)에 대한 지침이 있습니까?

전체 백업은 정기적으로(예: 매일, 매주 또는 매월) 수행되며 그 사이에는 증분 백업이 있습니다(예: 시간별, 일별 또는 주별).

>[!CAUTION]
>
>프로덕션 인스턴스의 백업을 구현할 때 테스트 *필수* 백업을 성공적으로 복원할 수 있도록 해야 합니다.
>
>이 기능이 없으면 백업이 무용지물이 될 수 있습니다(최악의 경우).

>[!NOTE]
>
>백업 성능에 대한 자세한 내용은 다음을 참조하십시오. [백업 성능](/help/sites-deploying/configuring-performance.md#backup-performance) 섹션.

### 소프트웨어 설치 백업 {#backing-up-your-software-installation}

설치 후 또는 구성에 중요한 변경 사항이 있으면 소프트웨어 설치를 백업하십시오.

이렇게 하려면 다음을 수행해야 합니다. [전체 저장소 백업](#backing-up-your-repository) 다음:
...
>[!CAUTION]
>
>타사 애플리케이션 서버를 운영하는 경우 추가 폴더가 다른 위치에 있을 수 있으며 백업해야 할 수도 있습니다. 다음을 참조하십시오 [응용 프로그램 서버에 AEM을 설치하는 방법](/help/sites-deploying/application-server-install.md) 애플리케이션 서버 설치에 대한 자세한 내용

>[!CAUTION]
>
>파일 데이터 저장소의 증분 백업이 지원됩니다. 다른 구성 요소(예: Lucene 인덱스)에 증분 백업을 사용할 때 삭제된 파일도 백업에서 삭제된 것으로 표시되는지 확인하십시오.
...
다음 [백업 및 복원](/help/sites-administering/backup-and-restore.md) crx 설명서의 섹션에서는 CRX 저장소의 백업과 관련된 모든 문제를 다룹니다.

온라인 &quot;핫&quot; 백업을 만드는 방법에 대한 자세한 내용은 [온라인 백업 만들기](/help/sites-administering/backup-and-restore.md#online-backup).
...
다음 **버전 제거** 도구는 주간 유지 관리 작업으로 사용할 수 있습니다. 을(를) 처음 사용하기 전에 을(를) 추가한 다음 구성해야 합니다. 그 후에는 요청 시 또는 매주 실행할 수 있습니다.
...
1. 다음 **버전 삭제** 작업이 추가됩니다. 카드 작업을 사용하여 다음을 수행합니다.
...
`/content/wknd`.
      ...
      >
      >하위 항목이 너무 많은 경로를 정의하면 제거 수행에 필요한 시간이 크게 늘어날 수 있습니다.
...
>[!CAUTION]
>
>클래식 UI 대화 상자를 사용하여 다음을 수행할 수 있습니다. [시험 실행](#analyzing-the-console) 구성:
>
>* http://localhost:4502/etc/versioning/purge.html
>
>삭제된 노드는 저장소를 복원하지 않으면 되돌릴 수 없습니다. 구성을 관리해야 하므로 제거하기 전에 항상 드라이 실행을 수행하는 것이 좋습니다.
...
* 다음 **[!DNL Shirts]** 버전 사용 기간이 2일보다 길기 때문에 버전이 삭제됩니다.
...
Adobe Experience Manager(AEM)와 관련된 감사 레코드 및 로그 파일은 다양한 위치에서 찾을 수 있습니다. 다음은 어디에서 찾을 수 있는지에 대한 개요를 제공하기 위해 제공되는 것입니다.
...
로그 파일 순환은 정기적으로 새 파일을 만들어 파일의 증가를 제한하는 프로세스를 의미합니다. AEM에서 라는 로그 파일 `error.log` 은(는) 지정된 규칙에 따라 하루에 한 번 회전합니다.

* 다음 `error.log` 파일 이름은 {original_filename} 패턴에 따라 변경됩니다 `.yyyy-MM-dd`. 예를 들어 2010년 7월 11일에는 현재 로그 파일의 이름이 바뀝니다 `error.log-2010-07-10`, 그런 다음 새 `error.og` 이(가) 만들어졌습니다.
...
>[!NOTE]
>
>ImageServer 및 s7액세스 로그는 **시스템/콘솔/상태**번들리스트**페이지에서 생성된 전체 다운로드**패키지에 포함되지 않습니다. 다음을 보유한 경우 지원 목적으로 [!DNL Dynamic Media] 문제, 고객 지원 센터에 문의할 때 ImageServer 및 s7액세스 로그도 추가하십시오.
...
>[!CAUTION]
>
>디버그 로그 수준에서 로그를 필요 이상으로 오래 두지 마십시오. 로그 항목이 많이 생성되어 리소스가 소모되기 때문입니다.
...
>[!NOTE]
>
>Adobe Experience Manager을 사용하여 작업할 때 이러한 서비스에 대한 구성 설정을 관리하는 방법에는 몇 가지가 있습니다. 를 참조하십시오. [OSGi 구성](/help/sites-deploying/configuring-osgi.md) 를 참조하십시오.

상황에 따라 다른 로그 수준으로 사용자 지정 로그 파일을 만들 수 있습니다. 저장소에서 다음 작업을 수행할 수 있습니다.

1. 아직 존재하지 않는 경우 새 구성 폴더를 만듭니다( `sling:Folder`)을 참조하십시오 `/apps/<project-name>/config`.
1. 아래 `/apps/<project-name>/config`, 새 노드에 대한 노드 만들기 [Apache Sling 로깅 로거 구성](/help/sites-deploying/osgi-configuration-settings.md#apacheslingloggingloggerconfigurationfactoryconfiguration):
...
1. 이 노드에서 다음 속성을 설정합니다.
   ...
   * 필요에 따라 다른 매개 변수를 구성합니다.
   ...
   >로그 메시지 {5}
   >
   >로그 호출에 가 포함된 경우 `Throwable` 스택 추적이 메시지에 추가됩니다.
   ...
   >(즉, 다음 옆) `<cq-installation-dir>/crx-quickstart/`)
...
1. 이 단계는 새 작성기가 필요한 경우(즉, 기본 작성기와 다른 구성을 사용하는 경우)에만 필요합니다.
   ...
   >
   >명시적 Writer가 구성되어 있지 않으면 시스템은 기본적으로 암시적 Writer를 자동으로 생성합니다.
   ...
   * 이름: `org.apache.sling.commons.log.LogManager.factory.writer-<identifier>` (작성자이므로)
   ...
   * 필요에 따라 다른 매개 변수를 구성합니다.
   ...
   >
   >* 숫자로 크기 제한을 지정할 수 있습니다. 크기 표시기가 제공되지 않으면 바이트 수로 간주하거나 크기 표시기 중 하나를 추가할 수 있습니다. `KB`, `MB`, 또는 `GB` (대/소문자가 무시됩니다.)
   >* 시간/날짜 일정을 다음으로 지정할 수 있습니다. `java.util.SimpleDateFormat` 패턴. 이는 파일이 회전된 후 기간을 정의하며, 또한 회전된 파일에 추가되는 접미사(식별용)도 정의합니다.

   >
   >기본값은 &#39;.&#39;입니다.yyyy-MM-dd(일별 로그 회전용)
   >
   >따라서 예를 들어 2010년 1월 20일 자정(또는 그 이후의 첫 번째 로그 메시지가 정확할 때)에 ../logs/error.log 의 이름이 ../logs/error.log.2010-01-20으로 변경됩니다. 1월 21일에 대한 로깅은 다음 일 변경 시 롤오버될 때까지 (새 항목 및 빈 항목) ../logs/error.log에 출력됩니다.
   >
   >| `'.'yyyy-MM` | 매월 초에 순환 |
   >|---|---|
   ...
   >1. 작은 따옴표(&#39; &#39;) 쌍 안에 리터럴 텍스트를 &quot;이스케이프&quot;해야 합니다.
      >
      >    특정 문자가 패턴 문자로 해석되는 것을 방지하기 위한 것입니다.
   >
...
1. 선택한 도구로 새 로그 파일을 읽습니다.
   ...
   이 예제에서 만든 로그 파일은 다음과 같습니다 `../crx-quickstart/logs/myLogFile.log`.

Felix 콘솔은에서 Sling 로그 지원에 대한 정보도 제공합니다. `../system/console/slinglog`; 예 `https://localhost:4502/system/console/slinglog`.
...
1. 페이지를 엽니다.
1. 사이드 킥에서 잠금 아이콘이 있는 탭을 선택한 다음 를 두 번 클릭합니다. **감사 로그..**
1. 새 창이 열리고 현재 페이지에 대한 감사 레코드 목록이 표시됩니다.
...
다음 범위 내 `/var/audit` 폴더, 감사 레코드는 리소스에 따라 유지됩니다. 개별 레코드와 개별 레코드에 포함된 정보를 볼 수 있을 때까지 드릴다운할 수 있습니다.
...
1. 액세스 **도구** AEM의 탭
1. **복제**&#x200B;를 클릭합니다.
1. 예를 들어 왼쪽 또는 오른쪽 창에서 적절한 환경에 대한 에이전트 링크를 두 번 클릭합니다 **작성자의 에이전트**.
...
1. 적절한 에이전트 이름(링크임)을 클릭하여 해당 에이전트에 대한 자세한 정보를 표시합니다.
   ...
   * 에이전트가 활성화되었는지 확인합니다.
   * 복제 타겟을 참조하십시오.
   * 복제 큐가 현재 활성(활성화) 상태인지 확인합니다.
   * 큐에 항목이 있는지 확인합니다.
   * **새로 고침** 또는 **지우기** 대기열 항목의 표시를 업데이트하려면 항목을 입력하고 대기열에서 나가는 것을 확인하는 데 도움이 됩니다.
   ...
   * **강제 다시 시도** 필요한 경우 모든 대기열 항목에서
   ...
   >
   >이러한 항목이 큐에 이미 있는 경우 다음 XPath JCR 쿼리를 통해 찾을 수 있으므로 제거해야 합니다.
   >
   >`/jcr:root/var/replication/outbox//*[@cq:repActionType='TEST']`

또한 모든 복제 에이전트(다음 위치에 있음)를 감지하는 솔루션을 개발할 수 있습니다. `/etc/replication/author` 또는 `/etc/replication/publish`)를 클릭한 다음 에이전트의 상태( )를 확인합니다. `enabled`, `disabled`) 및 기본 대기열( `active`, `idle`, `blocked`).

## 성능 모니터링 {#monitoring-performance}

[성능 최적화](/help/sites-deploying/configuring-performance.md) 는 개발 중에 포커스를 받는 대화형 프로세스입니다. 배포 후 일반적으로 특정 간격이나 이벤트 후에 검토됩니다.

최적화를 위해 정보를 수집하는 동안 사용되는 방법은 또한 지속적인 모니터링에 사용될 수 있다.

>[!NOTE]
>
>특정 [성능 향상을 위한 구성](/help/sites-deploying/configuring-performance.md#configuring-for-performance) 을(를) 확인할 수도 있습니다.

다음은 발생하는 일반적인 성능 문제를 파악하고 이에 대응하는 방법에 대한 제안과 함께 나열합니다.
...
|  | 서버에서 로컬로 탐색하는 것이 (비교적) 빠릅니다. | 네트워크 대역폭 향상. | 웹 페이지의 &quot;가중치&quot;를 줄입니다(예: 적은 이미지, 최적화된 HTML). |
| 웹 서버 | 웹 서버의 CPU 사용량이 높습니다. | 웹 서버를 클러스터링합니다. | 페이지당(방문) 히트 수를 줄입니다. |
|  |  | 하드웨어 로드 밸런서를 사용합니다. |  |
| 애플리케이션 | 서버 CPU 사용량이 많습니다. | AEM 인스턴스를 클러스터링합니다. | CPU 및 메모리 호그를 검색하고 제거합니다(코드 검토, 타이밍 출력 등 사용). |
|  | 메모리 사용량이 많습니다. |  | 모든 수준에서 캐싱을 개선합니다. |
|  | 응답 시간이 짧습니다. |  | 템플릿 및 구성 요소(예: 구조, 논리) 최적화 |
| 저장소 |  |  |  |
| 캐시 |  |  |  |

성능 문제는 연결 속도, CPU 로드 등의 일시적인 성능 저하를 포함하여 웹 사이트와 아무런 관련이 없는 여러 가지 원인으로 인해 발생할 수 있습니다.

모든 방문자 또는 방문자의 하위 집합에만 영향을 줄 수도 있습니다.

일반적인 성능을 최적화하거나 특정 문제를 해결하기 전에 이러한 모든 정보를 얻고, 정렬하고, 분석해야 합니다.
...
<table>
 <tbody>
  ...
  <tr>
   <td>트러스/스트랩</td>
   <td>페이지 로드</td>
   <td><p>시스템 호출 및 신호를 추적하는 Unix/Linux 명령 로그 수준을 다음으로 높임 <code>INFO</code>.</p> <p>요청당 페이지 로드 수, 페이지 등을 분석합니다.</p> </td>
  </tr>
  <tr>
   <td>스레드 덤프</td>
   <td>JVM 스레드를 관찰합니다. 경합, 잠금 및 장기 달리기 사용자를 식별합니다.</td>
   <td><p>운영 체제에 따라 다음 작업을 수행합니다.<br /> - Unix/Linux: <code>kill -QUIT &lt;<em>pid</em>&gt;</code><br /> - Windows(콘솔 모드): Ctrl-Break<br /> </p> <p>다음과 같은 분석 도구도 사용할 수 있습니다. <a href="https://github.com/irockel/tda">TDA</a>.<br /> </p> </td>
  </tr>
  <tr>
   <td>더미 덤프</td>
   <td>성능 저하를 유발하는 메모리 부족 문제.</td>
   <td><p>다음을 추가합니다.<br /> <code>-XX:+HeapDumpOnOutOfMemoryError</code><br /> AEM에 대한 java 호출 옵션.</p> <p>다음을 참조하십시오. <a href="https://docs.oracle.com/javase/8/docs/technotes/guides/troubleshoot/prepapp002.html#CEGBHDFH">JVM 문제 해결을 위한 옵션/플래그 페이지</a>.</p> </td>
  </tr>
  <tr>
   <td>시스템 호출</td>
   <td>타이밍 문제를 식별합니다.</td>
   <td><p>호출 대상 <code>System.currentTimeMillis()</code> 또는 <code>com.day.util</code>.Timing은 코드나 를 통해 타임스탬프를 생성하는 데 사용됩니다. <a href="#html-comments">HTML 주석</a>.</p> <p><strong>참고:</strong> 필요에 따라 활성화/비활성화할 수 있도록 구현해야 합니다. 시스템이 원활하게 실행될 때 통계 수집에 따른 오버헤드가 필요하지 않습니다.</p> </td>
  </tr>
  <tr>
   <td>아파치 벤치</td>
   <td>메모리 누수를 식별하고 응답 시간을 선택적으로 분석합니다.</td>
   <td><p>기본 사용:</p> <p><code>ab -k -n &lt;<em>requests</em>&gt; -c &lt;<em>concurrency</em>&gt; &lt;<em>url</em>&gt;</code></p> <p>다음을 참조하십시오 <a href="#apache-bench">아파치 벤치</a> 및 <a href="https://httpd.apache.org/docs/2.4/programs/ab.html">매뉴얼 페이지</a> 전체 세부 정보.</p> </td>
  </tr>
  <tr>
   <td>검색 분석</td>
   <td> </td>
  ...
  <tr>
   <td>자바 비행 기록 장치</td>
   <td>JFR(Java Flight Recorder)은 실행 중인 Java 애플리케이션에 대한 진단 및 프로파일링 데이터를 수집하는 도구입니다.</td>
   <td><a href="https://docs.oracle.com/javase/8/docs/technotes/guides/troubleshoot/tooldescr004.html#BABJJEEE">https://docs.oracle.com/javase/8/docs/technotes/guides/troubleshoot/tooldescr004.html#BABJJEEE</a></td>
  </tr>
  <tr>
   <td>JConsole</td>
   <td>JVM 지표 및 스레드를 관찰합니다.</td>
   <td><p>사용: jconsole</p> <p>다음을 참조하십시오 <a href="https://docs.oracle.com/javase/8/docs/technotes/guides/management/jconsole.html">jconsole</a> 및 <a href="#monitoring-performance-using-jconsole">JConsole을 사용한 성능 모니터링</a>.</p> <p><strong>참고:</strong> JDK 1.8에서 JConsole은 플러그인(예: Top 또는 TDA(Thread Dump Analyzer))을 사용하여 확장할 수 있습니다.</p> </td>
  </tr>
  <tr>
   <td>자바 비주얼 브이엠</td>
   <td>JVM 지표, 스레드, 메모리 및 프로파일링을 관찰합니다.</td>
   <td><p>사용법: visualvm 또는 visualvm<br /> </p> <p>다음을 참조하십시오 <a href="https://docs.oracle.com/javase/8/docs/technotes/guides/visualvm/">visualvm</a> 및 <a href="#monitoring-performance-using-j-visualvm">(J)VisualVM을 사용하여 성능 모니터링</a>.</p> <p><strong>참고:</strong> JDK 1.8을 사용하면 플러그인을 사용하여 VisualVM을 확장할 수 있습니다. VisualVM은 JDK 9 이후 중단됩니다. 대신 Java Flight Recorder를 사용하십시오.</p> </td>
  </tr>
  <tr>
  ...
  <tr>
   ...
   <td><p>페이지 렌더링에 대한 타이밍 통계를 보려면 다음을 사용할 수 있습니다. <strong>Ctrl-Shift-U</strong> 과 함께 <code>?debugClientLibs=true</code> 를 URL로 설정합니다.</p> </td>
  </tr>
  <tr>
   <td>CPU 및 메모리 프로파일링 툴<br /> </td>
   <td><a href="#interpreting-the-request-log">개발 중 느린 요청을 분석할 때 사용됨</a>.</td>
   <td>예를 들어, <a href="https://www.yourkit.com/">사용자 키트</a>. 또는 <a href="https://docs.oracle.com/javase/8/docs/technotes/guides/troubleshoot/tooldescr004.html#BABJJEEE">자바 비행 기록 장치</a>.</td>
  </tr>
  <tr>
   <td><a href="#information-collection">정보 수집</a></td>
   <td>설치 상태가 진행 중입니다.</td>
   <td>설치에 대해 가능한 한 많이 알고 있으면 성능 변화를 일으켰을 수 있는 원인과 이러한 변화가 정당한지 추적하는 데 도움이 될 수도 있습니다. 중요한 변경 사항을 쉽게 볼 수 있도록 이러한 지표를 정기적으로 수집해야 합니다.</td>
  </tr>
 </tbody>
</table>
...
다음 `request.log` 은 요청이 소요되는 시간을 파악할 수 있는 기본 제공 방법을 제공합니다. 개발 목적으로 다음과 같은 작업을 수행하는 것이 유용합니다 `tail -f` 다음 `request.log` 응답 속도가 느려지는지 확인하십시오. 더 큰 것을 분석하다 `request.log` 다음을 권장합니다. [사용 `rlog.jar` 응답 시간을 정렬 및 필터링할 수 있습니다](#using-rlog-jar-to-find-requests-with-long-duration-times).

에서 &quot;느린&quot; 페이지를 격리하는 것이 좋습니다. `request.log`그런 다음 더 나은 성능을 위해 개별적으로 조정합니다. 일반적으로 구성 요소별 성능 지표를 포함하거나 와 같은 성능 프로파일링 도구를 사용하여 수행됩니다. ` [yourkit](https://www.yourkit.com/)`.
...
작은 스크립트를 사용하여 로그 파일에서 필요한 정보를 추출하고 원하는 통계를 조합할 수 있습니다. 이를 통해 어떤 페이지 또는 페이지 유형이 느리는지, 전반적인 성능이 만족스러운지 확인할 수 있습니다.
...
그러나 응답 시간을 결정했으면 왜 요청이 시간을 소요하는지, 응답을 개선하기 위해 수행할 수 있는 작업을 분석해야 할 수 있습니다.
...
* 특정 시간 범위(예: 1분) 내에 수행된 요청의 수 모니터링
* 특정 수의 사용자가 모두 동일한 요청을 동시에 (가능한 한 가깝게) 수행하는 경우의 효과를 테스트합니다(예: 30명의 사용자가 클릭). **저장** 동시에.
...
AEM에는 다음 위치에 있는 다양한 도우미 도구가 포함되어 있습니다.
`<cq-installation-dir>/crx-quickstart/opt/helpers`

이 중 하나, `rlog.jar`를 사용하여 빠르게 정렬할 수 있습니다 `request.log` 따라서 요청이 가장 긴 시간부터 가장 짧은 시간까지 지속 시간별로 표시됩니다.
...
예를 들어 을 지정하여 실행할 수 있습니다 `request.log` 을 매개 변수로 사용하고, 기간이 가장 긴 첫 번째 요청 10개를 표시합니다.
...
개인을 연결해야 할 수도 있습니다 `request.log` 대용량 데이터 샘플에서 이 작업을 수행해야 하는 경우 파일입니다.

### 아파치 벤치 {#apache-bench}

특별한 경우(쓰레기 수거 등)의 영향을 최소화하기 위해서는 다음과 같은 도구를 사용하는 것이 좋다. `apachebench` (예를 들어, [ab](https://httpd.apache.org/docs/2.4/programs/ab.html) 추가 문서 참조) - 메모리 누수를 식별하고 응답 시간을 선택적으로 분석하는 데 도움이 됩니다.
...
위의 숫자는 기본 AEM 설치에 포함된 geometrixx 회사 페이지에 액세스하는 표준 MAcBook Pro 노트북(2010년 중반)에서 가져온 것입니다. 페이지는 매우 간단하지만 성능에 최적화되지 않았습니다.

`apachebench` 또한 모든 동시 요청에 대해 요청당 시간을 평균으로 표시합니다. 다음을 참조하십시오. `Time per request: 54.595 [ms]` (모든 동시 요청 간 평균). 동시성 매개 변수의 값을 변경할 수 있습니다 `-c` (한 번에 수행할 여러 요청 수) 을 입력하여 효과를 볼 수 있습니다.

### 요청 카운터 {#request-counters}

요청 트래픽(특정 기간 동안의 요청 수)에 대한 정보는 인스턴스에 대한 로드를 나타냅니다. 이 정보는에서 추출할 수 있습니다. [request.log](#interpreting-the-request-log), 하지만 카운터를 사용하면 데이터 수집이 자동화되어 다음 항목을 볼 수 있습니다.

* 활동의 중요한 차이점(즉, &quot;많은 요청&quot;과 &quot;낮은 활동&quot;을 구분함)
* 인스턴스가 사용되고 있지 않을 때
* 모든 다시 시작(카운터가 0으로 다시 설정됨)

정보 수집을 자동화하기 위해 RequestFilter 를 설치하여 모든 요청에 대해 카운터를 증가시킬 수도 있습니다. 여러 카운터는 서로 다른 기간에 사용할 수 있습니다.
...
모든 프로젝트에는 다음이 포함되는 것이 좋습니다. `html comments` 서버 성능. 좋은 공개 사례를 많이 찾을 수 있습니다. 페이지를 선택하고 볼 페이지 소스를 연 다음 맨 아래로 스크롤하면 다음과 같은 코드를 볼 수 있습니다.
...
1. 다음 범위 내에서 `Local` 응용 프로그램, 두 번 클릭 `com.day.crx.quickstart.Main`: 개요가 기본값으로 표시됩니다.
   ...
   이 작업 후 다른 옵션을 선택할 수 있습니다.

### (J)VisualVM을 사용하여 성능 모니터링 {#monitoring-performance-using-j-visualvm}

JDK 6-8의 경우 도구 명령 `visualvm` 을(를) 사용할 수 있습니다. JDK를 설치한 후 다음과 같은 작업을 수행할 수 있습니다.

1. AEM 인스턴스를 시작합니다.

   >[!NOTE]
   >
   >Java 5를 사용하는 경우 `-Dcom.sun.management.jmxremote` jvm을 시작하는 java 명령줄에 대한 인수입니다. JMX는 Java 6을 사용하여 기본적으로 활성화됩니다.
...
설치에 대해 가능한 한 많이 알고 있으면 성능에 변화를 일으켰을 수 있는 원인과 이러한 변화가 정당한지 추적하는 데 도움이 될 수 있습니다. 중요한 변경 사항을 쉽게 볼 수 있도록 이러한 지표를 정기적으로 수집해야 합니다.
...
서버 설치 이후 총 페이지 활성화 수를 보려면 CRXDE - Tools - Query를 통해 저장소 쿼리를 사용합니다.
...
설치 이후 총 롤아웃 수를 확인하려면 CRXDE - Tools - Query를 통해 저장소 쿼리를 사용합니다.
...
1. 다음 명령줄을 사용합니다.
   ...
   평균 크기를 가져오려면 전역 크기를 의 총 에셋 수로 나눕니다. `/var/dam` (위에서 가져옴).
...
피크 타임에 작성자 시스템에 있는 시간당 요청을 결정하려면 다음을 수행하십시오.

1. 설치 이후 총 요청 수를 확인하려면 다음 명령줄을 사용하십시오.
...
>* [기본 제공 프로파일러를 사용하여 분석](https://helpx.adobe.com/experience-manager/kb/AnalyzeUsingBuiltInProfiler.html)
...
시스템의 메모리가 부족한 경우 성능 저하 및 하위 텍스트를 포함한 오류 메시지를 포함하여 다양한 방식으로 볼 수 있습니다.
...
시스템에 디스크 공간이 부족하거나 디스크 스래싱이 시작되는 것을 발견하는 경우 다음을 참조하십시오.
...
* 기술 자료:
   ...
   * [열려 있는 파일이 너무 많음](https://helpx.adobe.com/experience-manager/kb/TooManyOpenFiles.html)
...
재부팅할 때마다(때때로 1주일 이상 늦게) 인스턴스 성능이 저하되는 것을 발견하면 다음을 확인할 수 있습니다.
...
JVM(Java Virtual Machine)은 튜닝과 관련하여 크게 개선되었습니다(특히 Java 7 이후). 이러한 이유로, 적절한 고정 JVM 크기를 지정하고 기본값을 사용하는 것이 종종 적합할 것입니다.

기본 설정이 적합하지 않은 경우 JVM을 튜닝하기 전에 GC 성능을 모니터링하고 평가하는 방법을 설정하는 것이 중요합니다. 여기에는 힙 크기, 알고리즘 및 기타 측면을 포함한 모니터링 요소가 포함될 수 있습니다.
...
이렇게 하면 사용 중인 메모리 양, 사용 중인 GC 알고리즘, 실행 소요 시간 및 애플리케이션 성능에 미치는 영향을 파악할 수 있습니다. 이것이 없다면, 동조는 단지 &quot;무작위로 뒤틀리는 꼭지들&quot;일 뿐입니다.

>[!NOTE]
>
>oracle의 VM의 경우 다음 정보도 제공됩니다.
>
