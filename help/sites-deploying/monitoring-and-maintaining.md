...
다음 **버전 삭제** 도구는 주간 유지 관리 작업으로 사용할 수 있습니다. 를 처음 사용하기 전에 추가한 다음 구성해야 합니다. 그 이후에는 요청 시 또는 주별 단위로 실행할 수 있습니다.
...
1. 선택 **버전 삭제** 을 클릭합니다. **새 작업 추가** 대화 상자. Then **저장**.

   ![버전 삭제 추가](assets/version-purge-add-new-task.png)

1. 다음 **버전 삭제** 작업이 추가됩니다. 카드 작업을 사용하여 다음을 수행할 수 있습니다.
...
이 프로세스에서는 처리된 모든 노드가 나열됩니다. 프로세스 중에 노드는 다음 상태 중 하나를 가질 수 있습니다.
...
* 다음 **[!DNL Shirts]** 버전 버전이 2일을 초과하므로 제거됩니다.
...
>[!NOTE]
>
>ImageServer 및 s7access 로그는 **system/console/status-Bundelist** 페이지에서 생성된 **Download Full **package에 포함되지 않습니다. 지원 목적으로 [!DNL Dynamic Media] 고객 지원에 문의할 때 ImageServer 및 s7access 로그도 추가하시기 바랍니다.
