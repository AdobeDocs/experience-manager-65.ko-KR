...
활성화된 주제 목록을 만드는 논리는 먼저 허용 목록에 있는 모든 주제를 허용한 다음 차단 목록에 있는 주제를 제거합니다. 기본적으로 모든 항목이 활성화되어 있고(허용 목록 값은 `*`) 차단 목록이 비활성화되어 있지 않습니다(에 값이 없음).
...
|---|---|---|
| 주제 허용 목록 | job.consumermanager.whitelist | 로컬 JobManager 서비스가 처리하는 주제 목록입니다. &amp;ast;의 기본값모든 항목을 등록된 TopicConsumer 서비스로 보냅니다. |
...
1. 작성자에 **복제 에이전트**&#x200B;를 만듭니다. (](/help/sites-deploying/replication.md) 복제 에이전트에 대한 [설명서 참조). **제목**&#x200B;을 지정합니다. **이름**&#x200B;은 명명 규칙을 따라야 합니다.
