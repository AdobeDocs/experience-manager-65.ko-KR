...
## MongoMK / RDMBMK {#reindexingmongomk} 다시 인덱싱
...
`oak-run.jar` 인덱싱 접근 방식에 따라 아래 다이어그램의 다시 색인 수행 단계 중 양쪽에 다양한 단계가 있습니다.
...
>[!NOTE]
>
>이 시나리오에 대한 자세한 내용은 [다시 색인 - DocumentNodeStore](/help/sites-deploying/oak-run-indexing-usecases.md#reindexdocumentnodestore)을 참조하십시오.
...
이것은 oak-run.jar의 새로운 인덱싱 기능을 사용하기 전에 사용되는 방법입니다. Oak 인덱스에서 `reindex=true` 속성을 설정하여 수행할 수 있습니다.

시간 및 성능 효과를 고객이 색인에 적용할 수 있는 경우 이 방법을 사용할 수 있습니다. 보통 중소규모의 AEM 설치 경우입니다.

![6](assets/6.png)

### oak-run.jar {#onlinere-indexingtarmkusingoak-run-jar}을 사용하는 온라인 TarMK의 재인덱싱
...
Offline re-indexing of TarMK는 단일 `oak-run.jar` 주석이 필요하므로 TarMK에 대한 가장 간단한 `oak-run.jar` 기반 다시 인덱싱 접근 방식입니다. 그러나 AEM 인스턴스를 종료해야 합니다.
...
### oak-run.jar {#out-of-bandre-indexingtarmkusingoak-run-jar}을 사용하는 대역 외 TarMK의 재인덱싱

>[!NOTE]
>
>이 시나리오에 대한 자세한 내용은 [대역 외 재색인 - SegmentNodeStore](/help/sites-deploying/oak-run-indexing-usecases.md#outofbandreindexsegmentnodestore)을 참조하십시오.
...
### ACS SURE 인덱스 {#creatingandupdatingindexdefinitionsontarmkusingacsensureindex}을(를) 사용하여 TarMK에서 색인 정의 만들기 및 업데이트

>[!NOTE]
>
>ACS Ensure Index는 커뮤니티에서 지원하는 프로젝트이며 Adobe 지원에서 지원되지 않습니다.

이를 통해 컨텐츠 패키지를 통해 배송 색인 정의를 수행할 수 있으므로 나중에 다시 색인 플래그를 `true`으로 설정하여 다시 색인화할 수 있습니다. 이렇게 하면 색인 재작성에 시간이 오래 걸리지 않는 작은 설정에 적용됩니다.
...
### oak-run.jar {#creatingandupdatingindexdefinitionsontarmkusingoak-run-jar}을 사용하여 TarMK에서 인덱스 정의 만들기 및 업데이트

비 `oak-run.jar` 메서드를 사용하여 다시 인덱싱할 때 시간 또는 성능에 미치는 영향이 너무 높은 경우 다음 `oak-run.jar` 기반 방법을 사용하여 TarMK 기반 AEM 설치에서 Lucene 인덱스 정의를 가져오고 다시 인덱싱할 수 있습니다.

![10](assets/10.png)

### oak-run.jar {#creatingandupdatingindexdefinitionsonmonogmkusingoak-run-jar}을 사용하여 MonogMK에서 색인 정의 만들기 및 업데이트

비 `oak-run.jar` 메서드를 사용하여 다시 인덱싱할 때 시간 또는 성능에 미치는 영향이 너무 높은 경우 다음 `oak-run.jar` 기반 방법을 사용하여 MongoMK 기반 AEM 설치에서 Lucene 색인 정의를 가져오고 다시 색인화할 수 있습니다.
