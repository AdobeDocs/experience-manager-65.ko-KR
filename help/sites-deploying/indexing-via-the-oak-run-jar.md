...
* **AEM Publish 팜(AE Publish 팜은 항상 TarMK여야 함)**
...
TarMK의 오프라인 리인덱싱은 단일 `oak-run.jar` 주석이 필요하므로 TarMK에 대한 가장 간단한 `oak-run.jar` 기반 리인덱싱 접근 방식입니다. 그러나 AEM 인스턴스를 종료해야 합니다.
