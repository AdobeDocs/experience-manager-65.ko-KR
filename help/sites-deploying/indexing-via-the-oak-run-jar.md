...
1. Oak-run은 대역 외 지원을 제공합니다. 프로덕션 조건에서 프로덕션 인스턴스에서 다시 인덱싱할 수 없는 경우 중요한 성능에 영향을 주지 않도록 복제된 환경을 다시 인덱싱하는 데 사용할 수 있습니다.
...
에 따라 `oak-run.jar` 색인 지정 접근 방식에서는 아래 다이어그램의 색인 재지정 수행 단계 양쪽에 다양한 단계가 있습니다.
...
TarMK의 오프라인 리인덱싱은 가장 간단합니다. `oak-run.jar` TarMK에 대한 기반 재인덱싱 접근 방식은 단일 `oak-run.jar` 댓글. 그러나 AEM 인스턴스를 종료해야 합니다.
...
비색인을 사용한 색인 재지정으로 인한 시간 또는 성능 영향`oak-run.jar` 메서드가 너무 높습니다. `oak-run.jar` 기반 접근 방식은 TarMK 기반 AEM 설치에서 Lucene 색인 정의를 가져오고 다시 색인화하는 데 사용할 수 있습니다.
...
비색인을 사용한 색인 재지정으로 인한 시간 또는 성능 영향`oak-run.jar` 메서드가 너무 높습니다. `oak-run.jar` 기본 접근 방식은 MongoMK 기반 AEM 설치에서 Lucene 색인 정의를 가져오고 다시 색인화하는 데 사용할 수 있습니다.
