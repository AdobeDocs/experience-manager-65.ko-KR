...
1. 이는 더 큰 저장소에서 재색인화 시간에 영향을 주는 재색인화 시간을 감소시킵니다
...
MongoMK(및 RDBMK) AEM 설치를 다시 인덱싱하는 데 권장되는 방법입니다. 다른 방법은 사용할 수 없습니다.

이 프로세스는 클러스터의 단일 AEM 인스턴스에 대해서만 실행해야 합니다.

![oak-run.jar를 사용하여 MongoMK 또는 RDBMK에 대한 온라인 재인덱싱](assets/5.png)
...
![TarMK에 대한 온라인 재인덱싱](assets/6.png)
...
![oak-run.jar를 사용하여 TarMK의 온라인 재인덱싱](assets/7.png)
...
![oak-run.jar를 사용하여 TarMK의 오프라인 재인덱싱](assets/8.png)
...
![oak-run.jar를 사용하여 대역 외 TarMK의 재인덱싱](assets/9.png)
...
이를 통해 컨텐츠 패키지를 통해 배송 인덱스 정의를 수행할 수 있으므로 나중에 다시 인덱스 플래그를 로 설정하여 재색인화를 수행할 수 있습니다 `true`. 이렇게 하면 색인 재지정 시간이 오래 걸리지 않는 작은 설정에 작동합니다.
...
![oak-run.jar를 사용하여 TarMK에서 색인 정의 만들기 및 업데이트](assets/10.png)
...
![oak-run.jar를 사용하여 MonogMK에서 색인 정의 만들기 및 업데이트](assets/11.png)
