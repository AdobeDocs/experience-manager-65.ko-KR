...
* 각 요청을 완전히 분석하려면 호출 스택의 각 요소를 표시한 다음 각 요청의 평균 처리 시간을 합하면 됩니다.
...
* **지연 설정** - 여러 시나리오에서 백업 지연 설정도 10ms(기본값), 1ms 및 0ms 값을 사용하여 가변하여 이 설정이 백업 성능에 미치는 영향을 조사했습니다.
...
비교를 위해 파일 시스템 백업(&#39;tar&#39;)을 사용하여 얻은 처리량이 동일한 저장소 파일을 백업합니다. tar의 성능은 비교할 수 있지만 지연 시간이 0으로 설정된 백업보다 약간 높습니다. 작은 지연이라도 설정하면 백업 처리량이 크게 줄어들고 10ms의 기본 지연으로 처리량이 크게 줄어듭니다. 전체 애플리케이션 사용량이 낮거나 애플리케이션이 유휴 상태일 때 백업을 예약할 수 있는 경우 기본 값 아래의 지연을 줄여 백업을 보다 빠르게 진행할 수 있습니다.
