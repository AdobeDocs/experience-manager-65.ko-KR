...
* [시작](#getting-started)
   ...
   * [작성자 및 Publish 설치](#author-and-publish-installs)
...
* **웹 응용 프로그램 서버**: AEM을 독립 실행형 모드(통합 Jetty 웹 서버 포함)로 배포하거나 타사 응용 프로그램 서버 내의 웹 응용 프로그램으로 배포할 수 있습니다.
* **웹 응용 프로그램 프레임워크**: AEM에는 RESTful의 콘텐츠 지향 웹 응용 프로그램 작성을 단순화하는 Sling 웹 응용 프로그램 프레임워크가 통합되어 있습니다.
...
* **Publish**: 게시된 콘텐츠를 일반에게 제공하는 AEM 인스턴스입니다.
...
* **Managed Services - Adobe Experience Manager용 Cloud Manager**: AEM이 배포되고 Adobe Managed Services에서 관리됩니다.
...
**최적화된 IT 비용:** 사전 예방적 지침 및 전문 지식은 조직이 최신 버전의 AEM을 최신 상태로 유지할 수 있도록 지원합니다. Adobe 플래티넘 유지 관리 및 지원은 AMS Enterprise/Basic의 새로운 배포에 자동으로 포함되며, 조직의 미션 크리티컬 애플리케이션을 유지 관리하는 데 도움이 되는 기술 전문 지식 및 운영 경험을 제공합니다. 무료 기본 Analytics 또는 Target 기능은 분석 및 개인화에 대한 요구 사항이 제한된 중간 시장 조직에 특히 더 많은 가치를 제공합니다.
...
다음 섹션에서는 **독립 실행형 설치**&#x200B;에 대해 설명합니다. 응용 프로그램 서버에 AEM 설치에 대한 자세한 내용은 [응용 프로그램 서버 설치](/help/sites-deploying/application-server-install.md)를 참조하십시오.
...
1. **jar** 및 **license** 파일을 이 디렉터리에 배치합니다.
   ...
   `license.properties` 파일을 제공하지 않으면 AEM에서 브라우저를 시작할 때 라이선스 키를 입력할 수 있는 **시작** 화면으로 리디렉션합니다. 아직 라이선스 키가 없는 경우 Adobe에 유효한 라이선스 키를 요청해야 합니다.
